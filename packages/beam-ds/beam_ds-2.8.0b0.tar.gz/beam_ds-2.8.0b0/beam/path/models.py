...
class GoogleStoragePath(PureBeamPath):
    ...
    def stat(self):
        ...
        return {
            ...
            'last_modified': metadata.get('updated', datetime.now()).timestamp(),
            'etag': metadata.get('etag'),
            'content_type': metadata.get('contentType'),
            'owner': metadata.get('owner', {}).get('entity'),
            'permissions': os.stat_result(
                (0, 0, 0, 0, 0, 0, metadata.get('size', 0),
                 metadata.get('updated', datetime.now()).timestamp(),
                 metadata.get('updated', datetime.now()).timestamp(),
                 metadata.get('updated', datetime.now()).timestamp()))
        }
