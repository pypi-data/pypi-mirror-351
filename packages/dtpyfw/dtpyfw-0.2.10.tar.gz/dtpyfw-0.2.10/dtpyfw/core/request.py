...
def request(
    ...
    **kwargs: Any,
    ...
    if 'headers' in kwargs:
        headers.update(jsonable_encoder(kwargs.pop('headers')))
    if 'params' in kwargs:
    ...
    try:
        ...
        raise RequestException(500, 'Request sending error', 'dtpyfw.core.request')
    ...
    try:
        ...
        if push_logs:
            ...
            footprint.leave(**error_context, log_type='error', message='Invalid JSON response')
        raise RequestException(500, 'Response parsing error', 'dtpyfw.core.request')
    ...
    if internal_service:
        ...
        if success:
            ...
            if push_logs:
                ...
                footprint.leave(**error_context, log_type='error', message='Service reported failure')
            raise RequestException(status, body.get('message'), 'dtpyfw.core.request')
