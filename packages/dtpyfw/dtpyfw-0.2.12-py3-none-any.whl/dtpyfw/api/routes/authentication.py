    ...
    def __call__(self, request: Request):
        ...
        if auth_token is None or auth_token != self.real_value:
            raise RequestException(
                controller='dtpyfw.middlewares.authentication.HeaderAuthChecker',
...
class QueryAuthChecker:
    ...
    def __call__(self, request: Request):
        ...
        if auth_token is None or auth_token != self.real_value:
            raise RequestException(
                controller='dtpyfw.middlewares.authentication.QueryAuthChecker',
                message='Wrong credential.',
                status_code=403,
            )
...
def auth_data_class_to_dependency(authentication: Auth) -> List[Depends]:
    if authentication.auth_type == AuthType.HEADER:
        checker = HeaderAuthChecker(
            key=authentication.header_key,
            real_value=authentication.real_value
        )
        return [
            Depends(checker),
