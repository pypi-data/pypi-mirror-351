...
def _render_env_file(target_dir: Path) -> None:
    """Create a `.env` file inside *target_dir* with a fresh SECRET_KEY."""

    secret_key = secrets.token_urlsafe(32)
    env_path = target_dir / ".env"
    content = (
        "# Environment variables for the generated Prompta project\n"
        f"SECRET_KEY={secret_key}\n"
        "DATABASE_URL=sqlite:///./prompta.db\n"
        "DEBUG=true\n"
    )
    env_path.write_text(content)


def _write_project_marker(target_dir: Path, project_name: str) -> None:
    """Create a hidden marker file so other commands can detect the project."""

    marker = target_dir / ".prompta_project"
    marker.write_text(json.dumps({"name": project_name, "version": 1}))
...
def create_project_command(name: str, force: bool) -> None:  # pragma: no cover
    ...
    # Generate .env with secret key and defaults
    _render_env_file(project_dir)

    # Write project marker
    _write_project_marker(project_dir, project_name=name)
