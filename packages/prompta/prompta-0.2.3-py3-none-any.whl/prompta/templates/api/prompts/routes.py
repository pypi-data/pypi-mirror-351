...
async def download_prompts(
    ...
    tags: Optional[List[str]] = Query(None, description="Filter by tags"),
    include_content: bool = Query(True, description="Include prompt content in response"),
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            for prompt in prompts:
                if prompt.current_version and prompt.current_version.content:
                    ...
                    safe_name = "".join(c for c in prompt.name if c.isalnum() or c in (' ', '-', '_')).rstrip()
...
@router.get("/download/by-project/{project_name}", response_model=PromptDownloadResponse)
