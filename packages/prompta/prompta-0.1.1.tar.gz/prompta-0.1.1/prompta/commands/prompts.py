"""Prompt management commands for Ruru CLI."""
...
from ..exceptions import AuthenticationError, NotFoundError, RuruAPIError
...
def download_command(
    ...
    try:
        ...
        if format == "zip":
            ...
            filename = f"ruru-prompts-{'-'.join(filename_parts)}.zip"
            ...
            filename = f"ruru-prompts-{'-'.join(filename_parts)}.json"
...
def download_project_command(
    ...
    try:
        ...
        # Save JSON file
        filename = f"ruru-project-{project_name}.json"
        json_path = output_dir / filename
...
def download_directory_command(
    ...
    try:
        ...
        filename = f"ruru-directory-{safe_dir_name}.json"
        json_path = output_dir / filename
...
def download_tags_command(
    ...
    try:
        ...
        # Save JSON file
        filename = f"ruru-tags-{'-'.join(tag_list)}.json"
        json_path = output_dir / filename
