...
class Config:
    ...
    # API Configuration
    api_url: str = "https://api.ruru.dev"
    api_timeout: int = 30
    ...
    cache_directory: str = "~/.ruru/cache"
...
class ConfigManager:
    ...
    def get_api_key(self, explicit_key: Optional[str] = None) -> Optional[str]:
        ...
        if self.config.use_keyring:
            try:
                return keyring.get_password("ruru", "api_key")
            except Exception:
    ...
    def set_api_key(self, api_key: str) -> None:
        ...
        """
        if self.config.use_keyring:
            keyring.set_password("ruru", "api_key", api_key)

    def clear_api_key(self) -> None:
        """Clear API key from keyring."""
        if self.config.use_keyring:
            try:
                keyring.delete_password("ruru", "api_key")
