...
def get_users_by_role(role_name, headers, county_unit=None):
    url = ACL_BASE_URL + 'accounts/users-with-role'
    data = {'role_name': role_name, 'county_unit': county_unit}
    return MakeExternalCall(method='POST', data=data, headers=headers, url=url).make_api_call()
...
def get_users_by_team(team_name, role, county_unit, headers):
    ...
    return MakeExternalCall(method='POST', data=payload, headers=headers, url=url).make_api_call()
