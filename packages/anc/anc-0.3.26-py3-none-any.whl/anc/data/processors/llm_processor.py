...
class PretrainProcessor(Processor):
    ...
    def __init__(
        ...
        if special_tokens is not None:
            ...
            self.special_tokens = []
        self.special_token_ids = [int(i) for i in special_token_ids]
    ...
    def transform(self, item, is_last_sample=False):
        ...
        if len(tokens) > self.max_seq_len and not self.seq_split_config.allow_split:
            logging.warn(f"encounter sample with token length larger than {self.max_seq_len}, will ignore and continue")
            return None
        ...
        self.sample_cnt += 1
        yield from [{"input_ids": tokens, "loss_mask": loss_mask, "text": item['content']}]
    ...
    def _build_chitchat_prompt(
        ...
        **kwargs
        ...
        Args:
            ...
            no_query_mask: Whether to disable masking for queries
