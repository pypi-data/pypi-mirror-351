...
def model(ctx, model_input, schema, mode):
    """command like: anc quant model path/to/model """
...
@click.option(
   '--dag-id',
   required=True,
   help='DAG ID: only_quant_pipeline/quant_pipeline/only_quant_sea_pipeline',
   type=click.Choice(['only_quant_pipeline', 'quant_pipeline', 'only_quant_sea_pipeline'])
)
