...
from .constants import CORE_REST_OBJECTS, FIELD_HINTS, SECONDARY_LOOKUP_FIELDS
from .datatypes import TYPE_TO_PYTHON
from .utils import full_class_name, pythonize_attribute_name, pythonize_class_name

XNATPY_NAMESPACE = "https://gitlab.com/radiology/infrastructure/xnatpy"
with open(os.path.join(os.path.dirname(__file__), "header.py")) as fin:
    FILE_HEADER = fin.read()
    ...
    def root_base_class(self, topxsd=False):
        ...
        while not base.startswith("XNAT"):
            try:
                ...
                self.logger.debug("Class list: {}".format(self.parser.class_list.keys()))
                raise
...
class AttributePrototype(object):
    def __init__(
        self, parser, name, logger, type=None, parent_class=None, parent_property=None, min_occur=None, max_occur=None
    ):
        self.parser = parser
        self.name = name
        self.logger = logger
...
class BaseClassWriter(BaseWriter):
    ...
    def tostring(self):
        header = self.header()
        properties = "\n\n".join(
            self.print_property(p) for p in self.attributes.values() if not self.hasattr(p.clean_name)
        )
    ...
    def header(self):
        ...
        if base_mixin is not None:
            ...
            header += "class {name}(mixin.{mixin}, {base}):\n".format(
                name=self.python_name, base=self.python_base_class, mixin=base_mixin.__name__
            )
            header += "\n\n    # END HEADER\n"
        else:
...
class SimpleClassWriter(BaseClassWriter):
    ...
    def create_listing(self, field_name, secondary_lookup):
        ...
        return XNATSimpleListing(
            ...
            field_name=field_name, secondary_lookup=secondary_lookup, element_class_name=self.name
        )
...
class SubObjectClassWriter(BaseClassWriter):
    def create_listing(self, field_name, secondary_lookup):
        ...
        return XNATSubListing(parent=self,
                              field_name='{field_name}',
                              secondary_lookup_field={secondary_lookup},
                              xsi_type='{type_}')""".format(
            field_name=field_name, secondary_lookup=secondary_lookup, element_class_name=self.name, type_=self.name
        )
...
class ObjectClassWriter(BaseClassWriter):
    ...
    @property
    def init(self):
        return """    def __init__(self, uri=None, xnat_session=None, id_=None, datafields=None, parent=None, {lookup}=None, **kwargs):
...
""".format(
            name=self.python_name, lookup=SECONDARY_LOOKUP_FIELDS[self.name]
        )
...
class AttributeWriter(BaseWriter):
    ...
    def __repr__(self):
        ...
        return "<{} {} (parent: {}, element: {})>".format(type(self).__name__, self.name, parent, element)
...
class SubObjectPropertyWriter(AttributeWriter):
    ...
    def {clean_name}(self):
        ...
        return self.get_object("{name}"{xsi_type_arg})""".format(
            clean_name=self.clean_name,
            docstring=docstring,
            name=self.name,
            type_=self.type,
...
class ListingPropertyWriter(AttributeWriter):
    ...
    @caching
    def {clean_name}(self):{docstring}""".format(
            clean_name=self.clean_name, docstring=docstring
        )
        if self.element_class is not None:
            element_class = self.element_class
            property_base += element_class.writer.create_listing(
                secondary_lookup=secondary_lookup, field_name=field_name
            )
...
class SchemaParser(object):
    ...
    def parse_schema_xmlstring(self, xml, schema_uri):
        ...
        if self.debug:
            self.logger.debug("Found {} unknown tags: {}".format(len(self.unknown_tags), self.unknown_tags))
    ...
    def parse_schema_uri(self, xnat_session, schema_uri):
        ...
        try:
            ...
            if 'action="/j_spring_security_check"' in data:
                ...
                self.logger.error(
                    "The server returned an error. You probably do not"
                ...
                if self.debug:
                    self.logger.debug(
                        "XML schema request returned the following response: [{}] {}".format(resp.status_code, data)
                    )
            return False
        ...
        while len(visited) < len(self.class_list) and yielded_anything and tries < 25:
            ...
            for key, value in self.class_list.items():
                ...
                if base is not None and not base.startswith("xs:") and base not in visited:
                    if self.debug:
                        self.logger.info(
                            "Wait with processing {} because base {} is not yet processed".format(value.name, base)
                        )
                    continue
                ...
                if value.parent_class is not None and value.parent_class not in visited:
                    if self.debug:
                        self.logger.info(
                            "Wait with processing {} because parent {} is not yet processed".format(
    ...
    def _parse_attribute(self, element):
        ...
        if self._current_class is not None:
            ...
            new_property = AttributePrototype(
                self,
                name=name,
                logger=self.logger,
                type=type_,
    ...
    def _parse_complex_type(self, element):
        ...
        new_class = ClassPrototype(
            self, name=name, namespace=namespace, logger=self.logger, field_name=field_name, parent_class=parent_class
        )
    ...
    def _parse_extension(self, element):
        ...
        if new_base.startswith("xs:"):
            ...
            new_base_class = ClassPrototype(self, name=new_base, namespace=namespace, logger=self.logger, simple=True)

            new_base_class.attributes[new_prop] = AttributePrototype(
                self, name=new_prop, logger=self.logger, type=previous_base
            )
    ...
    def _parse_restriction(self, element):
        ...
        if old_type is not None and old_type != new_type:
            raise ValueError(
                "Trying to override a type from a restriction!? (from {} to {})".format(old_type, new_type)
            )
    ...
    def _parse_simple_type(self, element):
        ...
        new_class = ClassPrototype(self, name=name, namespace=namespace, logger=self.logger, simple=True)

        new_property = AttributePrototype(self, name="value", logger=self.logger, type=None)
    ...
    def _parse_sqlfield(self, element):
        if self.debug:
            self.logger.debug(
                "CLASS: {}, PROP: {}, ELEMENT: {}".format(
                    self._current_class.name, self._current_property.name, element
                )
            )
    ...
    def prune_tree(self):
                ...
                if element_class is not None and len(element_class.attributes) == 1:
                    ...
                    if element_property.property_type == "listing":
                        ...
                        if new_element_class is not None:
                            ...
                            new_element_class.field_name = "{}/{}".format(
                                element_class.field_name, new_element_class.field_name
                            )
                        ...
                        if element_class.name in self.class_list:
                            if self.debug:
                                self.logger.info(
                                    "Removing class {} from parser, it only has one element!".format(element_class.name)
                                )
        ...
        for cls in self.class_list.values():
            if cls.parent_class in renamed:
                ...
                if self.debug:
                    self.logger.info(
                        f"Fixing parent name after rename in pruning from {cls.parent_class} to {new_name}"
                    )
        ...
        for cls in self.class_list.values():
            for property_key, prop in cls.attributes.items():
                ...
                if prop.element_class.simple:
                    if self.debug:
                        self.logger.debug(
                            "Found simple mapping {}.{} -> {}".format(cls.name, property_key, prop.element_class.name)
                        )
                        ...
                        if self.debug:
                            self.logger.debug(
                                "Too many attributes to simplify {} (found {} attributed)".format(
                                    prop.element_class.name, len(prop.element_class.attributes)
                                )
                            )
                        continue
    ...
    def write(self, code_file):
        ...
        code_str = string_template.safe_substitute(
            SCHEMAS=schemas, FILE_SECONDARY_LOOKUP=SECONDARY_LOOKUP_FIELDS["xnat:fileData"]
        )
