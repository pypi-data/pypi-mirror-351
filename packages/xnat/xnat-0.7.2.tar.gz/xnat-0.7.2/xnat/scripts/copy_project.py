...
class XNATProjectCopier:
    ...
    def copy_fields(self, source, destination, prefix=""):
        for field_id, value in source.fields.items():
            ...
            destination.fields[field_id] = html.unescape(value)
            print("{prefix}copying field: {}".format(field_id, prefix=prefix))
    ...
    def copy_resource(self, source_resource, dest_resource, prefix=""):
        ...
        # Create resource-specific tmp file
        with tempfile.TemporaryDirectory(dir=self.temp_dir) as tmpdir:

            source_resource.download_dir(tmpdir, verbose=False)
        ...
        for source_resource in source_object.resources.values():
            ...
            try:
                dest_resource = dest_class(
                    parent=dest_object, label=source_resource.label, content=source_resource.content
                )
    ...
    def copy_experiment(self, source_experiment, dest_experiment):
        ...
        for scan in source_experiment.scans.values():
            ...
            if scan.id not in dest_experiment.scans:
                ...
                if dest_class is None:
                    print(
                        "     [WARNING] {} class not found on destination server, skipping".format(
                            source_experiment.__xsi_type__
                        )
                    )
                    continue
        ...
        for assessor_id, source_assessor in source_experiment.assessors.items():
            ...
            if source_assessor.__xsi_type__ != "xnat:mrAssessorData":
                ...
                print(
                    "    Avoiding creation of invalid class xnat:mrAssessorData,"
                ...
                try:
                    ...
                    try:
                        ...
                        if "not include parseable files" in exception.args[0]:
                            print(
                                "    [ WARNING] Experiment did not include parsable dicom files, creating empty experiment"
                            )
