...
class ExtractEnrollmentLabels:
    def __init__(self):
        major_data = ReadData('专业库',['cengci','major_name','major_ljdm','one_fenlei','one_fenlei_dm','two_fenlei','two_fenlei_dm','qidalei','qidalei_dm'],replace_th=False).data
...
class UpdateName():
    ...
    """
    def __init__(self):
        self.school_names_change = sorted(ReadData('更新院校名称名单').data, key=lambda x: len(x[0]), reverse=True)
...
class GetSchoolNameBz():
    def __init__(self):
        school_names = [school[0].replace('（','(').replace('）',')') for school in ReadData('院校名称').data]
        self.school_names = sorted(school_names, key=lambda x: len(x), reverse=True)
        self.ssq = [x for ssq in get_ssq().values() for x in ssq]

    def __call__(self,v):
        v = v.replace('（', '(').replace('）', ')')
        # if '北京师范大学' in v and '香港浸会大学联合国际学院' in v:
        #     v = '北京师范大学-香港浸会大学联合国际学院'
...
class GetMajorNameBz():
    def __init__(self):
        major_names = [major[0].replace('（','(').replace('）',')') for major in ReadData('专业名称').data]
...
def school_ljdm():
    ...
    """
    school_ljdm_dict = {}
    for school_ljdm, school_name in ReadData('院校库', ['院校逻辑代码', '院校名称']).data:
    # for school_ljdm, school_name, school_name_change in ReadData('院校库', ['院校逻辑代码', '院校名称', 'school_name_change']).data:
    #     for school_name in [school_name] + school_name_change.split('、'):
        school_ljdm_dict[school_name] = school_ljdm
...
school_names = lambda :[x[0] for x in ReadData('院校名称').data]
