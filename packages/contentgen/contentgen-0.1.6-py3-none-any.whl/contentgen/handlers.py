...
print(f"Path to .env: {dotenv_path}")
    ...
    async def post(self):
        try:
            ...
            try:
                ...
                # Ensure JSON format
                formatting = (
                    "Correctly format the provided string so that it can be parsed as JSON without any errors (make sure that it follows all the rules for escaped characters and single lines, do NOT try to escape new lines, keep them as '\n'):\n"
                    + output
                )
                formatted_response = model.generate_content(formatting)
                print("Pre-formatting" + output)
                output = formatted_response.text.strip()
                print("Post-formatting" + output)
