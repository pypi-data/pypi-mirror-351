...
# Global state
search_index: Optional[TantivySearch] = None
...
def initialize_server(
    ...
    global search_index, original_data, config

    # Load data
    print(f"Loading data from {data_file}...")
    original_data = load_jsonl_data(data_file)
    ...
    print(f"Successfully initialized index with {len(original_data)} documents")


def create_server(description: str) -> FastMCP:
    ...
    def read_documents(document_ids: List[str]) -> str:
        if search_index is None:
            return "Error: Search index not initialized"
...
def main():
    ...
    try:
        ...
        # Create MCP server with custom description
        mcp = create_server(args.description)
