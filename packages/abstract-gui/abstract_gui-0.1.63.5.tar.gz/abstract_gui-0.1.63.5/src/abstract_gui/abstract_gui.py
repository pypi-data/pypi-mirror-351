from .simple_gui_functions_manager import SimpleGuiFunctionsManager,SimpleGuiFunctionsManagerSingleton,sg,ensure_nested_list,make_component,get_gui_fun
from abstract_utilities.path_utils import get_current_path
from abstract_utilities.list_utils import make_list_add,make_list
...
def get_unique_key(key=None,keys=[]):
    og_key = key or '--DEFAULT--'
    i = 0
    while True:
        if key in keys:
            key = eatAll(og_key,['-'])
...
def get_input_layout(title:str="Input Window",type:str="Folder",text:str="please enter your input",default:str=None,args:dict={},initial_folder:str=get_current_path(),):
    """
    Function to get a browser GUI based on the type specified.
    ...
    for values in args:
        ...
        key,keys = get_unique_key(key=values.get("key",text),keys=keys)
        values["text"]=text
        values["default"]=default
        values["key"]=key
        layout.append([get_gui_fun('Text',args={"text":text}),get_gui_fun('Input',args=values)])
    layout.append([get_gui_fun('OK'), get_gui_fun('Cancel')])
...
def get_input(title:str="Input Window",text:str="please enter your input",initial_folder:str=get_current_path(),default:str=None,args:dict={},exit_events:(str or list)=['Cancel'],return_events:(str or list)=['OK']):
    """
