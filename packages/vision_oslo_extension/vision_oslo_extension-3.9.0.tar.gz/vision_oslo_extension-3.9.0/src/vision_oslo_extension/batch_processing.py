...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    print("Sub-option Selected --> Option {}".format(option_select))
...
def rms_summary_output(simname,df_list,sum_df_list,time_windows,feeder_list):
    ...
    for time in time_windows:
        print("Processing {} Current / Power Summary...".format(time))
...
def branch_data_process(simname, option_select,time_start, time_end, time_step, text_input, time_increment):
    ...
    if option1 in ["4","5","6"]:
        ...
        branch_input = [node for node in branch_input if node]  # Remove empty strings
    ...
    # generate List file
    if not os.path.isfile(filename):
        if not oslo_extraction.list_extract(simname,time_start,time_end,"2"):
            return False
