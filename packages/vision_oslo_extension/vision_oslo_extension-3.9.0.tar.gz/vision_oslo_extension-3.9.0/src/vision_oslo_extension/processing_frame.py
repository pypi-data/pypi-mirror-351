...
class P01(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_post_processing(option1,input1,input2, input3),width = 20, height =2)
        button.pack()
    ...
    def run_post_processing(self,option1,input1,input2,input3):
        try:
            ...
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("post_processing.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P02(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_post_processing(option1,input1,input2,input3,input4),width = 20, height =2)
        button.pack()
    ...
    def run_post_processing(self,option1,input1,input2,input3,input4):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            main_option = SharedVariables.main_option
...
class P03(BasePage):
    ...
    def run_batch_processing(self,option1,input1,input2,input3):
        try:
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process,
                                      args=("average_load.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P04(BasePage):
    def __init__(self, parent, controller):
        ...
        explain = tk.Message(master=self.headframe, text = 'This will produce a text file stating the Umean useful (ZONE) for the selected geographical section and list of Umeanuseful (TRAIN) in ascending order. User needs to make sensible choice of Trains for the assessment.',aspect = 1200, font = controller.text_font)
        explain.pack()

        option1 = tk.StringVar(value = "0") # Initialize with a value not used by the radio buttons
        choice1 = tk.Radiobutton(master=self.optionframe, text = 'Option 1: Processing whole time window (Suggest for Umean (TRAIN))', value="1", variable=option1)
        choice1.grid(row = 0, column = 0, sticky = "w", padx=5, pady=5)

        choice2 = tk.Radiobutton(master=self.optionframe, text = 'Option 2: Processing customised time window (Suggest for Umean (Zone), Umean (Train) still whole time window)',value="2", variable=option1)
        choice2.grid(row = 1, column = 0, sticky = "w", padx=5, pady=5)

        explain1 = tk.Label(master=self.optionframe, text = 'NOTE: Branch List File should define the assessed geographic range.',font = controller.text_font)
        explain1.grid(row = 3, column = 0, sticky = "w", padx=5, pady=5)
        

        label4 = tk.Label(master=self.inputframe, text = 'Branch File Name (Required):',font = controller.text_font)
...
class P05(BasePage):
    ...
    def run_batch_processing(self,option1,input1,input2,input3):
        try:
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("protection_if.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P06(BasePage):
    ...
    def run_batch_processing(self,option1,input1,input2,input3):
        try:
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("grid_connection.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P07(BasePage):
    ...
    def run_batch_processing(self,option1,input1,input2,input3):
        try:
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("ole_processing.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P08(BasePage):
    ...
    def run_batch_processing(self,option1,input1,input2,input3):
        try:
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process,
                                      args=("sfc_assess.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P09(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_batch_processing(option1,input1,input2),width = 20, height =2)
        button.pack()
    ...
    def run_batch_processing(self,option1,input1,input2):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            main_option = SharedVariables.main_option
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("batch_processing.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P11(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_batch_processing(option1,input3),width = 20, height =2)
        button.pack()
    ...
    def run_batch_processing(self,option1,input3):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            main_option = SharedVariables.main_option
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("dc_summary.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P12(BasePage):
    ...
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_SEF_processing(option1,input1,input2,input3),width = 20, height =2)
        button.pack()
    ...
    def run_SEF_processing(self,option1,input1,input2,input3):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            main_option = SharedVariables.main_option
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("dc_single_end_feeding.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class P14(BasePage):
    ...
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_FVP_processing(option1,input1,input2,input3),width = 20, height =2)
        button.pack()
    ...
    def run_FVP_processing(self,option1,input1,input2,input3):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            main_option = SharedVariables.main_option
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("dc_falling_voltage_protection.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
