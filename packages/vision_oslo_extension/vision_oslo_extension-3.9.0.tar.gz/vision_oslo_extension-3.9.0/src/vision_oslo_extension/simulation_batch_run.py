...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    print("Batch Running Programm Started...")
    print("")
...
def run_simulation(control,maximum_running_time):
    ...
    for i in range(len(control)):
        ...
        escaped_path = re.escape(sim_path)

        # program allows 5*5 times 25 seconds to open the simulation
        while retries > 0:
            try:
                app = Application(backend='win32').connect(title_re=f'VISION - {escaped_path}')
                app_window = app.window(title_re=f'VISION - {escaped_path}')
        ...
        while retries > 0:
            try:
                ...
                SharedMethods.print_message(f"WARNING: Unexpected error: {e}. Retrying after 1 second...","33")
                time.sleep(1)
                retries -= 1
                ...
                if simulator_handles: # simualtor process started
                    ...
                    try:
                        ...
                        if status.exists(timeout=1):
                            ...
                            # reaching here means this is the pre simulation warning window
                            send_keys('{ENTER}')
                    ...
                    SharedMethods.print_message("WARNING: Simulator window not initilized. Retrying after 5 seconds...","33")
                    time.sleep(5)
                    retries -= 1
            except Exception as e:
        ...
        try:
            ...
            if status.exists(timeout=5):
                ...
                if simulation_success:
                    ...
                    SharedMethods.print_message(f'\nWARNING: Simulation {simname} terminated due to errors...',"33")
                send_keys('{ENTER}')
                time.sleep(0.5)
        except Exception as e:
