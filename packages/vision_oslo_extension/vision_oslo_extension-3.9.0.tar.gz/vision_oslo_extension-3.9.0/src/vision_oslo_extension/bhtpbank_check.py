...
def main(sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    print("Please enter the NUMBER OF CARS of {}:(RANGE:1-20)(awaiting input)".format(bhtpname))
    ...
    print("Summary: Checking {}: {} Car traction profile.".format(bhtpname,carnumber))
    print("         bhtpbank comment with '{}'.".format(bhtpcomment))
    if regen == True:
...
def TE_check(TE_list,vol_list,speed_list,warning_sum):
    ...
    for index, value in enumerate(vol_list):
        if len(TE_list[index]) > 20:
            text = 'ERROR: Traction data at {} V has more than 20 speed entries.' \
                .format(value)
...
def BE_check(TE_list_b,vol_list_b,speed_list_b,warning_sum):
    ...
    for index, value in enumerate(vol_list_b):
        if TE_list_b[index][0] != 0:
            text = 'ERROR: Braking effort at {} V for 0 speed is Non-Zero.' \
                .format(value)
            SharedMethods.print_message(text,"31")
            warning_sum.append(text)
...
#        print('\033[1;33m') # color control warning message Yellow
    ...
    for index, value in enumerate(vol_list): # value = votage
        ...
        if diff > 5:
            print('\033[1;33m') # color control warning message Yellow
            if BE_flag == False:
                text = "WARNING: Traction part: auxiliary power at {} V is not consistent at different speeds!".format(value)
            else:
                text = "WARNING: Braking part: auxiliary power at {} V is not consistent at different speeds!".format(value)
            SharedMethods.print_message(text,"33")
            warning_sum.append(text)
    ...
    if BE_flag == False:
        ...
        for index, value in enumerate(vol_list):
            print("{:.2f} kW at {:.0f} V".format(aux_list[index],value))
        
        print("Minimum stationary auxiliary power per car is {:.2f} kW".format(min(aux_list)/carnumber))
        print("Maximum stationary auxiliary power per car is {:.2f} kW".format(max(aux_list)/carnumber))
