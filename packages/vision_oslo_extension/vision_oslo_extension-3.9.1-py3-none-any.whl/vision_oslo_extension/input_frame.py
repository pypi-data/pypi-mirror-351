...
from vision_oslo_extension import bhtpbank_check
...
class S01(BasePage):
    def __init__(self, parent, controller):
        ...
        choice2 = tk.Label(master=self.optionframe, text = 'Option 3: Traction BHTPBANK File Check (Fill Below Information and Click Run)',font = controller.text_font)
        choice2.grid(row = 4, column = 0, sticky = "w", padx= 10, pady=5)
    ...
    def run_bhtp_check(self,input1,input3):
        try:
            ...
            continue_process = bhtpbank_check.main(sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step)
            ...
            #                           args=("bhtpbank_check.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            # thread.start()
...
class S04(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command=lambda: self.run_cif_check(option1,input3),width = 20, height =2)
        button.pack()
    ...
    def run_cif_check(self,option1,input3):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            main_option = SharedVariables.main_option
            ...
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("cif_prepare.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class S05(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command=lambda: self.run_cif_guidance(),width = 20, height =2)
        button.pack()
    ...
    def run_cif_guidance(self):
        try:
            ...
            # Run the batch processing function in a separate thread
            thread = threading.Thread(target=SharedMethods.launch_new_thread_or_process, 
                                      args=("cif_output_analysis.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
