...
upload_jobs = {
    ...
    "mocks": [
        lambda monkeypatch: monkeypatch.setattr("cidc_api.shared.gcloud_client.revoke_upload_access", MagicMock()),
        lambda monkeypatch: monkeypatch.setattr("cidc_api.shared.gcloud_client.publish_upload_success", MagicMock()),
    ],
}
...
def test_resource_post(resource, config, cidc_api, clean_db, monkeypatch):
    ...
    if "POST_setup" in config:
        for setup_resource in config["POST_setup"]:
            res = client.post(setup_resource, json=resource_requests[setup_resource]["json"])
...
@pytest.mark.parametrize("resource, config", resource_requests_with_key("additional_records"))
