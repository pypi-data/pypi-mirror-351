...
snippets= {}
group1_snippets = [
    'DFS', 'BFS', 'A* algo', 'Iterative IDS', 'uniform cost search', 
    'hill climbing Search', 'Best First Search', 'AlphaBeta Purning',
    'generate moves', 'Lab 5', 'GA', 'BFS Pathfinding', 'DFS Pathfinding',
    'A* Search Structure', 'Hill Climbing Local Search Structure',
    'Min-Max Algorithm Structure', 'Alpha Beta Pruning Structure',
    'Tic Tac Toe Game Structure', 'N Queen Problem Backtracking',
    'Genetic Algorithm Structure', 'Knapsack Problem Dynamic Programming'
]
"""
For clipboard:
    ...
    def Sender(self):
        while self.run:
            ...
            if msg == 'list':
                ...
                # Show group 1 snippets
                snippet_indexing = {i: key for i, key in enumerate(snippets.keys()) if key in group1_snippets}
                if snippet_indexing:
                    print("Group 1 Snippets:")
                    for i, key in snippet_indexing.items():
                        print(f'{i}. {key}')
                    try:
                        required_snippet = int(input('Enter the snippet number: '))
                        ...
                        continue
                    try:
                        snippet_key = snippet_indexing[required_snippet]
                    ...
                    print("No snippets in Group 1")
            elif msg == 's2':
                # Show group 2 snippets
                snippet_indexing = {i: key for i, key in enumerate(snippets.keys()) if key not in group1_snippets}
                if snippet_indexing:
                    print("Group 2 Snippets:")
                    for i, key in snippet_indexing.items():
                        print(f'{i}. {key}')
                    try:
                        required_snippet = int(input('Enter the snippet number: '))
...
6. s1: List and select snippets from Group 1
7. s2: List and select snippets from Group 2
8. copy <name_or_number>: Copy specific snippet to clipboard
9. file <filename>: Send file (alpha)
...
# Define which snippets belong to group 1
group1_snippets = ['DFS', 'BFS', 'A* algo', 'Iterative IDS', 'uniform cost search', 
                   'hill climbing Search', 'Best First Search', 'AlphaBeta Purning']

# Group 2 contains data science related snippets (not in group1_snippets):
# - pd csv, pd df
# - Basic Plotting Matplotlib, Basic Plotting Plotly
# - Sklearn related snippets
# - Neural network/perceptron snippets
# - metric_imp
