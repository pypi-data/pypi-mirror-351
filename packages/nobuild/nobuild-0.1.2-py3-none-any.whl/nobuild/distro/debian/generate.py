...
from .repo import copy_files, copy_repo, copy_extra
...
def generate(flavor: str | None = None, cache: bool = True):
    console = Console()
    ...
    if os.path.isdir("./work"):
        ...
        subprocess.run(f"lb clean{" --binary" if cache else ""}", shell=True, cwd="./work")
    else:
        console.print("Generating live-build configuration...")
        cmd = f'lb config --archive-areas "main contrib non-free non-free-firmware" --bootloaders "grub-efi grub-pc" --bootappend-live "boot=live components" {config["extra"].get("config_flag", "")}'
