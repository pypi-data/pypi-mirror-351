...
def text_filter(sender, **kwargs):
    ...
    if qs:
        ...
        cache_documents = (
            cache_documents
                .filter(data_vector_q)
                .annotate(score=SearchRank(F("data_vector"), search_query))
        )

    return cache_documents

def search_data(
    qs, filters, wiki_configs, documents_limited_to, omit_filter_results=False, limit=None, offset=0, rank_constant=60
):
    document_limit_or = Q()
    ...
    if qs:
        searches = [
            ...
            for identity, search in search_filter.send(
                sender=None, cache_documents=results, qs=qs
            )
    ...
    if searches:
        ...
        returned_results = returned_results[offset : None if not limit else offset + limit]
...
def search(
    q, filters, offset, num_per_page, template_config, wiki_configs, documents_limited_to, fmt
):
    ...
    (returned_results, filter_results, num_results) = search_data(
        [q], filters, wiki_configs, documents_limited_to, False, num_per_page, offset
    )
...
def search_global(request, fmt):
    ...
    return search(q, f, offset, num_per_page, global_config, configs, results_limit, fmt)
...
def search_collection(request, collection_name, fmt):
    ...
    return search(q, f, offset, num_per_page, configs.first(), configs, results_limit, fmt)
...
def explore(
    ...
    (returned_results, filter_results, num_results) = search_data(
        qs, filters, wiki_configs, results_limit, not filters_only, num_per_page, offset
    )
...
def get_spreadsheet(request, name, fmt):
    ...
    if fmt == "json":
        ...
        return JsonResponse(
            {
                ...
                "fields": sorted(spreadsheet_spec.fields),
        ...
        def write_spreadsheet(add_header, add_row, spreadSheetWriter):
            spreadsheet_field_names = sorted(spreadsheet_spec.fields)
