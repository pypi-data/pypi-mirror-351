...
class FlowDiagram:
    ...
    directional arrows with arrowheads on a transparent or opaque background.
    Supports configurable padding between images and captions,
    dynamic caption height calculation, automatic text wrapping, and optional transparency.
    """
    def __init__(
        self,
        spacing=20,
        ...
        # Determine background with optional alpha override
        if not isinstance(background, tuple) or (len(background) not in (3, 4)):
            raise ValueError("background must be an RGB or RGBA tuple")
        # Ensure RGBA
        if len(background) == 3:
            bg_rgb = background
            bg_alpha = 0 if transparent else 255
        else:
            bg_rgb = background[:3]
            bg_alpha = background[3] if transparent else 255
        self.background = (bg_rgb[0], bg_rgb[1], bg_rgb[2], bg_alpha)
