from santou.ck_table_list.ck_table import *
from santou.db_tool.db1 import DB1
from santou.db_tool.moneySeparatedLineEdit import moneySeparatedLineEdit
...
from santou.db_tool.TimeSpinBox import TimeSpinBox
from santou.save_qmt import save_qmt
from santou.db_tool.LittleSwitchButton import LittleSwitchButton
from santou.tool.dbanniu import SwitchButton
...
import json
import time
from santou.db_tool.yzTool import yzTool
...
class db_ym1(QMainWindow):
    ...
    def __init__(self):
        ...
        # 初始化时设置输入框状态
        self.disable_ltsz_inputs()  # 初始禁用流通市值输入框
        ...
        # 在__init__方法末尾添加输入验证
        self.setup_input_validations()

    def setup_input_validations(self):
        # 流通市值输入框验证
        self.ltsz_min.editingFinished.connect(lambda: self.validate_zero_input(self.ltsz_min))
        self.ltsz_max.editingFinished.connect(lambda: self.validate_zero_input(self.ltsz_max))

        # 股价输入框验证
        self.price_min.editingFinished.connect(lambda: self.validate_zero_input(self.price_min))
        self.price_max.editingFinished.connect(lambda: self.validate_zero_input(self.price_max))

        # 换手率输入框验证
        self.hsl_min.editingFinished.connect(lambda: self.validate_zero_input(self.hsl_min))
        self.hsl_max.editingFinished.connect(lambda: self.validate_zero_input(self.hsl_max))

    def validate_zero_input(self, input_field):
        ...
        try:
            value = float(text)
            if value <= 0:
                QMessageBox.warning(self, "输入错误", "输入值必须大于0！")
                input_field.clear()
        except ValueError:
            QMessageBox.warning(self, "输入错误", "输入无效！")
            input_field.clear()
    ...
    def enable_hsl_inputs(self):
        """启用换手率输入框"""
        self.hsl_min.setEnabled(True)
        self.hsl_max.setEnabled(True)
        self.hsl_min.setStyleSheet("background-color: white;")
        self.hsl_max.setStyleSheet("background-color: white;")

    def disable_hsl_inputs(self):
        """禁用换手率输入框并清空内容"""
        self.hsl_min.clear()
        self.hsl_max.clear()
        self.hsl_min.setEnabled(False)
        self.hsl_max.setEnabled(False)
        self.hsl_min.setStyleSheet("background-color: #f0f0f0;")
        self.hsl_max.setStyleSheet("background-color: #f0f0f0;")
    ...
    def showEvent(self, event):
        ...
        self.checkbox_first.setChecked(True)
    ...
    def handle_db1_data(self, item):
        try:
            ...
            # 打板复选框
            db = cele_data.get('db', [])
            self.checkbox_first.setChecked('首板' in db)
            self.checkbox_second.setChecked('二板' in db)
            self.checkbox_third.setChecked('三板及以上' in db)
            ...
            # 打一字板复选框
            if cele_data.get('dyzb', '0') == '1':
                self.dyzb.is_checked = True
            ...
            # 流通市值输入框
            if "ltsz_min" in cele_data:
                self.ltsz_switch.is_checked = True
                self.ltsz_min.setText(cele_data.get('ltsz_min', ''))
            if "ltsz_max" in cele_data:
                self.ltsz_switch.is_checked = True
                self.ltsz_max.setText(cele_data.get('ltsz_max', ''))
            ...
            # 换手率
            if "hsl_min" in cele_data:
                self.hsl_switch.is_checked = True
                self.hsl_min.setText(cele_data.get('hsl_min', ''))
            if "hsl_max" in cele_data:
                self.hsl_switch.is_checked = True
                self.hsl_max.setText(cele_data.get('hsl_max', ''))
            ...
            # 买入时机选择框
            buy_condition_value = cele_data.get('buy_condition', '触发涨停价时买入')
            self.buy_condition.setCurrentText(buy_condition_value)
            self.on_buy_condition_changed(buy_condition_value)
    ...
    def format_input(self, text):
        ...
        try:
            ...
            logger.error('db_ym1:format_input', exc_info=True)
            pass
    ...
    def load_username_account_data(self):
        ...
        if acc_data:
            ...
            # 通过信号发送数据到主线程
            self.combo_data_ready.emit(xz_data)
        ...
        if text:
            ...
            def check_connection():
                try:
                    ...
                    logger.error('db_ym1:check_connection:连接验证时出现异常', exc_info=True)
    ...
    def on_dyzb_state_changed(self):
        """
        当打一字板复选框状态变化时触发
        :param state: 复选框的状态（Qt.Checked 或 Qt.Unchecked）
        """

        if  self.dyzb.is_checked ==True:
            # 如果复选框被选中，设置时间为 9:25 并禁用输入框
            self.start_time_spinbox.setValue(9 * 60 + 30)  # 9:25
            self.start_time_spinbox.setEnabled(False)  # 禁用输入框
        else:
            # 如果复选框未被选中，设置时间为 9:30 并启用输入框
            self.start_time_spinbox.setValue(9 * 60 + 30)  # 9:30
            self.start_time_spinbox.setEnabled(True)  # 启用输入框
       # self.save_data_by_thread()
    def off_dyzb_state_changed(self):
        if self.dyzb.is_checked == False:
            # 如果复选框未被选中，设置时间为 9:30 并启用输入框
            self.start_time_spinbox.setValue(9 * 60 + 30)  # 9:30
            self.start_time_spinbox.setEnabled(True)  # 启用输入框
        else:
    ...
    def on_refresh_button_clicked(self):
        ...
        if self.qmt_trader is None:
            ...
            try:
                ...
                # 如果获取余额失败，显示错误信息
                logger.error('db_ym1:on_refresh_button_clicked:连接失败', exc_info=True)
    ...
    def _update_codes_list_thread(self, row_count):
        try:
            ...
            logger.error('db_ym1:_update_codes_list_thread:更新代码列表时出现异常', exc_info=True)
        finally:
    ...
    def _update_11_13_thread(self, row_count):
        try:
            ...
            logger.error('db_ym1:_update_11_13_thread:更新代码列表时出现异常', exc_info=True)
    ...
    def update_no_trader_data_thread(self, row_count):
        try:
            ...
            logger.error('db_ym1:update_no_trader_data_thread:更新代码列表时出现异常', exc_info=True)
    ...
    def enable_ltsz_inputs(self):
        """启用流通市值输入框"""
        self.ltsz_min.setEnabled(True)
        self.ltsz_max.setEnabled(True)
        self.ltsz_min.setStyleSheet("background-color: white;")
        self.ltsz_max.setStyleSheet("background-color: white;")

    def disable_ltsz_inputs(self):
        """禁用流通市值输入框并清空内容"""
        self.ltsz_min.clear()
        self.ltsz_max.clear()
        self.ltsz_min.setEnabled(False)
        self.ltsz_max.setEnabled(False)
        self.ltsz_min.setStyleSheet("background-color: #f0f0f0;")
        self.ltsz_max.setStyleSheet("background-color: #f0f0f0;")
    ...
    def create_top_page(self):
        ...
        self.combo_box_qmt.setPlaceholderText("请在交易连接设置中配置")  # 设置占位符文本
        ...
        # 添加首版 二板 三板
        db_widget = QWidget()
        db_widget.setFixedWidth(205)
        db_layout = QHBoxLayout(db_widget)
        db_layout.setAlignment(Qt.AlignLeft)
        self.checkbox_first = QCheckBox("首板")

        self.checkbox_second = QCheckBox("二板")
        self.checkbox_third = QCheckBox("三板及以上")
        db_layout.addWidget(self.checkbox_first)
        db_layout.addWidget(self.checkbox_second)
        db_layout.addWidget(self.checkbox_third)
        first_row_layout.addWidget(db_widget)
        # 添加首版 二板 三板结束

        # 打最先涨停的前 N 个板
        first_limit_widget = QWidget()
        first_limit_widget.setFixedWidth(164)  # 设置 QWidget 的宽度为 200 像素
        ...
        # 标签
        first_limit_label = QLabel("打最先涨停的前")
        ...
        self.first_limit_combo.addItems([str(i) for i in range(1, 9)])  # 添加选项 1 到 8
        self.first_limit_combo.setCurrentText("1")  # 设置默认值
        first_limit_layout.addWidget(self.first_limit_combo)
        # 标签
        first_limit_layout.addWidget(QLabel("个板"))
        # 将打最先涨停布局添加到主布局中
        ...
        # 赋值添加说明已经打了几个板
        dbsm_widget = QWidget()
        dbsm_widget.setFixedWidth(95)
        dbsm_layout = QHBoxLayout(dbsm_widget)
        self.dbsm1 = QLabel("已打")
        self.dbsm2 = QLabel("0")
        self.dbsm3 = QLabel("个板")
    ...
    def create_right_page(self):
        ...
        # 添加打一字板
        yzb_widget = QWidget()
        yzb_layout = QHBoxLayout(yzb_widget)
        yzb_layout.setSpacing(0)  # 设置组件之间的间距为 0 像素
        yzb_layout.setContentsMargins(0, 0, 0, 0)  # 设置布局的边距为 0
        yzb_widget.setFixedWidth(390)
        # bdjjztb_widget.setStyleSheet("border: 1px solid black; border-radius: 5px; padding: 5px;")
        self.dyzb = LittleSwitchButton()
        self.dyzb_label = QLabel(" 打一字板，不选只打非一字板")
        self.dyzb_label.setStyleSheet("font-size: 14px;")
        self.dyzb.switched_on.connect(self.on_dyzb_state_changed)
        self.dyzb.switched_off.connect(self.off_dyzb_state_changed)
        yzb_layout.addWidget(self.dyzb)  # 添加新的 QCheckBox
        yzb_layout.addWidget(self.dyzb_label)  # 添加新的 QCheckBox
        self.right_layout.addWidget(yzb_widget)
        # 添加打一字板结束
        ...
        self.ltsz_switch = LittleSwitchButton()
        self.ltsz_switch.switched_on.connect(self.enable_ltsz_inputs)
        self.ltsz_switch.switched_off.connect(self.disable_ltsz_inputs)

        market_value_layout.addWidget(self.ltsz_switch)
        zrspj = QLabel(" 流通市值: ")
        market_value_layout.addWidget(zrspj)
        zrspj.setStyleSheet("font-size: 14px;")
        # 输入框 1
        self.ltsz_min = QLineEdit()
        self.ltsz_min.setStyleSheet("background-color: white;")
        self.ltsz_min.setFixedWidth(80)  # 设置输入框宽度
        self.ltsz_min.setPlaceholderText("最小市值")  # 设置占位符文本
        self.ltsz_min.setValidator(QDoubleValidator())  # 限制只能输入数字
        market_value_layout.addWidget(self.ltsz_min)
        # 标签
        z_ltsz = QLabel("亿 至 ")
        market_value_layout.addWidget(z_ltsz)
        z_ltsz.setStyleSheet("font-size: 14px;")
        # 输入框 2
        self.ltsz_max = QLineEdit()
        self.ltsz_max.setStyleSheet("background-color: white;")
        self.ltsz_max.setFixedWidth(80)  # 设置输入框宽度
        self.ltsz_max.setPlaceholderText("最大市值")  # 设置占位符文本
        self.ltsz_max.setValidator(QDoubleValidator())  # 限制只能输入数字
        market_value_layout.addWidget(self.ltsz_max)
        # 标签
        market_value_layout.addWidget(QLabel("亿"))
        # 将流通市值布局添加到主布局中
        self.right_layout.addWidget(market_value_widget)


        # 股价输入框
        price_widget = QWidget()
        price_widget.setFixedWidth(260)  # 设置 QWidget 的宽度为 200 像素
        ...
        # 换手率开始
        hsl_widget = QWidget()
        hsl_widget.setFixedWidth(270)  # 设置 QWidget 的宽度为 200 像素
        hsl_layout = QHBoxLayout(hsl_widget)
        hsl_layout.setSpacing(0)  # 设置组件之间的间距为 0 像素
        hsl_layout.setContentsMargins(0, 0, 0, 0)  # 设置布局的边距为 0
        self.hsl_switch = LittleSwitchButton()
        self.hsl_switch.switched_on.connect(self.enable_hsl_inputs)
        self.hsl_switch.switched_off.connect(self.disable_hsl_inputs)
        hsl_layout.addWidget( self.hsl_switch)
        hsl = QLabel(" 换手率: ")
        hsl.setStyleSheet("font-size: 14px;")
        hsl_layout.addWidget(hsl)
        # 输入框 1
        self.hsl_min = QLineEdit()
        self.hsl_min.setStyleSheet("background-color: white;")
        self.hsl_min.setFixedWidth(80)  # 设置输入框宽度
        self.hsl_min.setValidator(QDoubleValidator())  # 限制只能输入数字
        hsl_layout.addWidget(self.hsl_min)
        # 标签
        ...
        # 输入框 2
        self.hsl_max = QLineEdit()
        self.hsl_max.setStyleSheet("background-color: white;")
        self.hsl_max.setFixedWidth(80)  # 设置输入框宽度
        self.hsl_max.setValidator(QDoubleValidator())  # 限制只能输入数字
        hsl_layout.addWidget(self.hsl_max)
        ...
        # 买入时机说明和选择框
        buy_condition_widget = QWidget()
        buy_condition_widget.setFixedWidth(400)  # 设置 QWidget 的宽度为 400 像素
        self.buy_condition_layout = QHBoxLayout(buy_condition_widget)  # 改为垂直布局
        self.buy_condition_layout.setSpacing(10)  # 设置组件之间的间距为 10 像素
        self.buy_condition_layout.setContentsMargins(0, 0, 0, 0)  # 设置布局的边距为 0
        self.buy_condition_layout.setAlignment(Qt.AlignLeft)
        # 添加买入时机说明标签
        buy_condition_label = QLabel("打板时机：")
        buy_condition_label.setStyleSheet("font-size: 14px;")  # 设置标签字体大小
        self.buy_condition_layout.addWidget(buy_condition_label)

        # 添加选择框
        self.buy_condition = QComboBox()
        self.buy_condition.setStyleSheet("""
            QComboBox {
                font-size: 14px;  /* 设置字体大小 */
            }
            QComboBox QAbstractItemView {
                font-size: 14px;  /* 设置下拉项的字体大小 */
            }
        """)
        self.buy_condition.setFixedHeight(25)
        self.buy_condition.setStyleSheet("background-color: white;")
        self.buy_condition.setFixedWidth(250)  # 设置选择框宽度
        self.buy_condition.addItem("触发涨停价时买入")
        self.buy_condition.addItem("涨停且卖一无封单时买入")
        self.buy_condition.addItem("涨停炸板再次封板时买入")
        self.buy_condition.addItem("买一封板资金大于X万时买入（卖一无封单）")
        self.buy_condition.addItem("涨停且卖一封单小于X万时买入")
        self.buy_condition.addItem("剩卖一时买入")
        self.buy_condition.addItem("剩卖二二档时买入")
        self.buy_condition.addItem("剩卖三三档时买入")
        self.buy_condition.currentTextChanged.connect(self.on_buy_condition_changed)  # 绑定事件
        self.buy_condition_layout.addWidget(self.buy_condition)
        self.right_layout.addWidget(buy_condition_widget)

        # 添加输入框和标签（初始隐藏）
        self.fbzj_widget = QWidget()
        self.fbzj_layout = QHBoxLayout(self.fbzj_widget)
        self.right_layout.addWidget(self.fbzj_widget)
    ...
    def on_buy_condition_changed(self, text):
        """当选择框内容变化时触发"""
        # 清空布局前删除旧控件引用
        if hasattr(self, 'capital_input'):
            self.capital_input.deleteLater()
            del self.capital_input  # 删除属性

        if hasattr(self, 'myfd_input'):
            self.myfd_input.deleteLater()
            del self.myfd_input

        # 清空 self.fbzj_layout 中的所有内容
        self.clear_layout(self.fbzj_layout)

        # 根据选择的选项动态添加内容
        if text == "买一封板资金大于X万时买入（卖一无封单）":
            # 创建水平布局并添加输入框和标签
            self.fbzj_widget.setFixedWidth(280)
            capital_layout = QHBoxLayout()
            self.capital_input = QLineEdit()
            self.capital_input.setValidator(QDoubleValidator())  # 限制只能输入数字
            self.capital_input.setStyleSheet("background-color: white;")
            self.capital_input.setFixedWidth(80)
            self.capital_input.setPlaceholderText("")
            self.capital_input.setValidator(QDoubleValidator())
            capital_label = QLabel("万≤买一封板资金（卖一无封单）时买入")
            capital_layout.addWidget(self.capital_input)
            capital_layout.addWidget(capital_label)
            self.fbzj_layout.addLayout(capital_layout)

        elif text == "涨停且卖一封单小于X万时买入":
            # 创建水平布局并添加输入框和标签
            self.fbzj_widget.setFixedWidth(260)
            myfd_layout = QHBoxLayout()
            myfd_label = QLabel("涨停且卖一封单≤")
            self.myfd_input = QLineEdit()
            self.myfd_input.setValidator(QDoubleValidator())  # 限制只能输入数字
            self.myfd_input.setFixedWidth(80)
            self.myfd_input.setStyleSheet("background-color: white;")
            myfdw_label = QLabel("万时买入")
            myfd_layout.addWidget(myfd_label)
            myfd_layout.addWidget(self.myfd_input)
            myfd_layout.addWidget(myfdw_label)
            self.fbzj_layout.addLayout(myfd_layout)
    ...
    def on_switch_on(self):
        ...
        if not self.validate_input_ranges():
            self.switch_button.is_checked = False
            return
        # 新增验证逻辑：至少选择一个打板类型（首板/二板/三板）
        if not ( self.checkbox_first.isChecked() or self.checkbox_second.isChecked() or self.checkbox_third.isChecked()):
            self.switch_button.is_checked = False
            QMessageBox.warning(self, "警告", "请至少选择一种打板类型（首板/二板/三板及以上）！")
            return

        # 新增验证逻辑：至少选择一个板块（主板/创业板/北交所）
        if not (self.checkbox_main.isChecked() or self.checkbox_gem.isChecked() or self.checkbox_star.isChecked()):
            self.switch_button.is_checked = False
            QMessageBox.warning(self, "警告", "请至少选择一个市场板块（主板/创业板/北交所）！")
            return
        ...
        if self.checkbox_first.isChecked():
            db.append(self.checkbox_first.text())

        if self.checkbox_second.isChecked():
            db.append(self.checkbox_second.text())

        if self.checkbox_third.isChecked():
            db.append(self.checkbox_third.text())

        # 打一字板
        jon["db"] = db
        if self.dyzb.is_checked:
            jon["dyzb"] = "1"
        ...
        if self.ltsz_switch.is_checked == True:
            ltsz_min = self.ltsz_min.text().strip()  # 最小市值
            ltsz_max = self.ltsz_max.text().strip()  # 最大市值
            if len(ltsz_min) > 0:
                jon['ltsz_min'] = ltsz_min.strip()
            if len(ltsz_max) > 0:
                jon['ltsz_max'] = ltsz_max.strip()
        ...
        # 换手率
        if self.hsl_switch.is_checked == True:
            hsl_min = self.hsl_min.text().strip()
            hsl_max = self.hsl_max.text().strip()
            if len(hsl_min) > 0:
                jon['hsl_min'] = hsl_min.strip()
            if len(hsl_max) > 0:
                jon['hsl_max'] = hsl_max.strip()

        # 获取“买入时机”选择框的值
        buy_condition_value = self.buy_condition.currentText().strip()  # 买入时机
        # 存储买入时机的值
        jon['buy_condition'] = buy_condition_value

        # 买一封板资金
        if buy_condition_value == "买一封板资金大于X万时买入（卖一无封单）":
            if hasattr(self, 'capital_input'):
                if self.capital_input.isWidgetType():
                    fbzj = self.capital_input.text().strip()
                    jon['buyfbzj'] = fbzj

        # 卖一封板资金
        if buy_condition_value == "涨停且卖一封单小于X万时买入":
            if hasattr(self, 'myfd_input'):
                if self.myfd_input.isWidgetType():
                    fbzj = self.myfd_input.text().strip()
                    jon['myfd'] = fbzj
        ...
        for row in range(self.win._model.rowCount()):
            ...
            if checkbox_item is not None and checkbox_item.checkState() == Qt.Checked:
                ...
                if code_item is not None and price_item is not None and ztzt_item is not None and ltsz_item is not None :
                    ...
                    ltsz=ltsz_item.text()
                    hsl = hsl_item.text()
                    #print(f"{code}-{price}-{ztzt}-{ltsz}")
                    yz=yzTool()
                    if yz.yz_gp(jon,code,ztzt,ltsz,zr_price,hsl):
                        selected_data[code_item.text()] = zr_price
                    yz =None

        if not selected_data:
            QMessageBox.warning(self, "警告", "没有符合条件的股票，请检查买入条件是否合理？")
            self.switch_button.is_checked = False
            return
        ...
        if not self.db1:
            self.db1=DB1()
        db1=self.db1
        db1.init_db_data(selected_data,self.qmt_trader,jon,True,self.switch_button,self.dbsm2,self)
        # 连接打板程序信号，用于关闭switch_burtton
        db1.stopped.connect(self.on_db1_stopped)  # 连接信号
        #修改打了几个板的状态
    ...
    def jebz_info_add(self,params):
        ...
        # 不存在才提示，存在就不用重复提示了
        if code not in db_ym1.yebz_code:
            db_ym1.yebz_code.append(code)
            action = 'save_cj_data'
            deal = DeaiClientData()
            cj_status="0"
            czlx="db"
    ...
    def disable_checkboxes_and_inputs(self):
        ...
        checkboxes_top = [self.checkbox_all, self.checkbox_main, self.checkbox_gem, self.checkbox_star,
                          self.checkbox_first, self.checkbox_second, self.checkbox_third]
        ...
        # 底部页面的输入框
        inputs_bottom = [self.ltsz_min, self.ltsz_max, self.price_min, self.price_max,
                         self.hsl_min, self.hsl_max, self.start_time_spinbox, self.end_time_spinbox]
        if hasattr(self, 'capital_input'):
        ...
        for input_field in inputs_bottom:
            if input_field is not None and input_field.isWidgetType():
                try:
                    ...
                    logger.error('db_ym1:disable_checkboxes_and_inputs', exc_info=True)
                    pass

        # 买入时机选择框
        self.buy_condition.setEnabled(False)
        ...
        switchs = [self.dyzb, self.ltsz_switch, self.price_switch, self.hsl_switch]
    ...
    def enable_checkboxes_and_inputs(self):
        ...
        # 买入时机选择框
        self.buy_condition.setEnabled(True)
    ...
    def validate_input_ranges(self):
        """验证所有启用的输入框范围是否有效"""
        # 流通市值
        if self.ltsz_switch.is_checked:
            ltsz_min = self.ltsz_min.text().replace(',', '').strip()
            ltsz_max = self.ltsz_max.text().replace(',', '').strip()
            if not ltsz_min or not ltsz_max:
                QMessageBox.warning(self, "输入错误", "流通市值必须填写最小值和最大值！")
                return False
            try:
                if float(ltsz_min) >= float(ltsz_max):
                    QMessageBox.warning(self, "输入错误", "流通市值最小值必须小于最大值！")
                    return False
            except ValueError:
                QMessageBox.warning(self, "输入错误", "流通市值输入无效！")
                return False

        # 换手率
        if self.hsl_switch.is_checked:
            hsl_min = self.hsl_min.text().replace(',', '').strip()
            hsl_max = self.hsl_max.text().replace(',', '').strip()
            if not hsl_min or not hsl_max:
                QMessageBox.warning(self, "输入错误", "换手率必须填写最小值和最大值！")
                return False
            try:
                if float(hsl_min) >= float(hsl_max):
                    QMessageBox.warning(self, "输入错误", "换手率最小值必须小于最大值！")
                    return False
            except ValueError:
                QMessageBox.warning(self, "输入错误", "换手率输入无效！")
                return False
