...
class AnswerFormatterFilter(ReasonerFilter):
    ...
    def is_valid_answer(answer: str) -> bool:
        ...
        if answer==None or not re.search(r'\\boxed{.*}', answer):
            return False
    ...
    def _load_input(self):
        if self.storage is not None:
            value_list = self.storage.read_json(
                [self.input_key], eval_stage=self.eval_stage, format='SFT_Single', syn='syn_qa', stage=self.stage, pipeline_id=self.pipeline_id, category="reasoning"
            )
            value_list = [        
