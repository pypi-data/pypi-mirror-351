...
def safe_json_loads(s):
    # 1. 先尝试原始 orjson.loads
    try:
        return orjson.loads(s)
    except Exception as e:
        print(f"orjson.loads 失败: {e}，尝试修复…")
...
class MyScaleStorage(DataFlowStorage, ABC):
    ...
    def read_columns(
        ...
        with self._db_operation():
            ...
            self.logger.debug(f"Executing results: rows = {rows}")
    ...
    def get_all_columns(self) -> List[str]:
        ...
        rows = self.client.execute(schema_query)
            ...
            for i in range(0, len(data), batch_size):
                ...
                for row in batch:
                    ...
                    for col in columns:
                        value = row[col]
                        if col == 'data':
                            print("batch_write,before processing json_str, value = ",value)
                            # 使用 repr() 来正确处理所有转义字符
                            if isinstance(value, (dict, list)):
                                json_str = repr(orjson.dumps(value).decode('utf-8'))
                                # 去掉 repr 添加的外层引号
                                json_str = json_str[1:-1]
                                row_values.append(f"'{json_str}'")
                            else:
                                # 如果已经是字符串，直接使用 repr
                                escaped_value = repr(value)[1:-1]  # 去掉外层引号
                                row_values.append(f"'{escaped_value}'")

                            print("batch_write,after processing json_str, row_values = ",row_values)
                        else:
                            if isinstance(value, str):
                               ...
                               self.logger.debug(f"batch_write value is not a str, value = {value}")
    ...
    def read_code_json(self, key_list, **kwargs):
        ...
        print("after read_columns,rows = ",rows)
    ...
    def write_data(self, data: list, **kwargs) -> None:
        ...
        with self._db_operation():
            try:
                ...
                # 4. 准备删除语句 - 使用参数化查询更安全
                delete_sql = f"""
                DELETE FROM {self.config.db_name}.{self.config.table_name}
                WHERE id IN %(ids)s
                """

                # 5. 执行数据库操作
                self.logger.debug(f"Writing ' {delete_sql} items to database")
                self.client.execute(delete_sql, {'ids': tuple(ids)})  # 先删除旧数据
                self.logger.debug(f"Writing {len(updated_values)} items to database")
                self.batch_write(updated_values)  # 再插入新数据
                all_data = self.read_columns(
                    columns=self.get_all_columns(),
                )
                self.logger.debug(f"after write_data.batch_write,all_data = {all_data}")
                self.logger.debug(f"Successfully wrote and deleted {len(data)} items")
                self.logger.debug(f"Successfully wrote and deleted {len(data)} items")
    ...
    def write_eval(
        ...
        with self._db_operation():
            ...
            WHERE id IN %(ids)s
            """
            self.client.execute(delete_sql, {'ids': tuple(ids)})
            ...
            self.logger.debug(f"After write_eval.batch_write,all_data2 = {all_data}")
