...
class AnswerPipelineRoot:
    ...
    def _load_input(self):
        if hasattr(self, 'storage'):
            ...
            if not value_list:
                self.logger.warning("No data found in database with the given conditions")
                return pd.DataFrame(columns=[self.input_key, 'id', self.input_gt_key])
            ...
            return pd.DataFrame([
                {
                    **item['data'],
                    'id': str(item['id']),
                    self.input_gt_key: item['data'].get(self.input_gt_key, None)  # 使用 input_gt_key 而不是硬编码的 'golden_answer'
                }
    ...
    def run(self):
        ...
        if not hasattr(self, "storage"):
            ...
            if not self.input_gt_key or self.input_gt_key not in df.columns:
                self._write_output(self.output_file_without_gt, df, None)
        ...
        if self.input_answer_key in df.columns:
            ...
            df[self.input_gt_key] = df.apply(lambda row: extract_gt(row[self.input_answer_key],
                                                                    row.get(self.input_gt_key, None)),  # 使用 get 方法安全地获取字段值
                                            axis=1)
