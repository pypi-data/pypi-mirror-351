    ...
    def _load_input(self):
        if hasattr(self, 'storage'):
            value_list = self.storage.read_json(
                [self.input_key], eval_stage=self.eval_stage, syn='syn_q', format='SFT_Single', maxmin_scores=[{'max_score': self.read_max_score, 'min_score': self.read_min_score}], stage=self.stage, pipeline_id=self.pipeline_id, category="reasoning"
            )
            return pd.DataFrame([
    ...
    def _write_output(self, save_path, dataframe, extractions):
        ...
        if hasattr(self, 'storage'):
            ...
            for row in output_rows:
                print(f"2.++++++++++++row = {row}")
                primary = self.safe_int(row.get("primary_category"))
                print(f"3.++++++++++++primary = {primary}")
                secondary = self.safe_int(row.get("secondary_category"))
                print(f"4.++++++++++++secondary = {secondary}")
    ...
    def run(self):
        ...
        for (idx, row), classification_str in zip(dataframe.iterrows(), responses):
            try:
                ...
                # 提取 primary 的整数部分
                primary_match = re.search(r"(\d+)", primary_raw)
                primary_category = primary_match.group(1) if primary_match else ""

                # 提取 secondary 小数点后的数字
                secondary_match = re.search(r"\d+\.(\d+)", secondary_raw)
                secondary_category = secondary_match.group(1) if secondary_match else ""

                dataframe.at[idx, "primary_category"] = primary_category
                dataframe.at[idx, "secondary_category"] = secondary_category
