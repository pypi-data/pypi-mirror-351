...
class RemoteSession:
    ...
    async def get_response(self, timeout_sec:int=TIMEOUT_SEC) -> str:
        if self.rs_session is None:
            raise RuntimeError("Session is not connected.")
        ...
        try:
            ...
            chat_lines = await self.get_chat_lines()
            response = chat_lines[-1]
            last_line = response.get("message", None)
        except Exception as e:
            log(ERROR, f"RemoteSession.get_response(): Exception while getting response: {e}")
    
        if last_line is not None and last_line.startswith("{"):
            # It's a JSON response
            try:
                last_line = last_line.replace("\n", "").replace("\r", "")
                result = json.loads(last_line)
            except Exception as e:
                log(WARNING, f"RemoteSession.get_response(): Could not parse the JSON response: {e}")
                result = last_line
        else:
            result = last_line
    ...
    async def submit_message(self, prompt:str, role:str="user", json_output=None, structured_outout=None, timeout_sec:int=TIMEOUT_SEC, recommended_plan:str=None, recommended_tools:List[str]=None) -> None:
        if self.rs_session is None:
            raise RuntimeError("Session is not connected.")
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=BLAZOR_SSL_CERTIFICATE_VERIFICATION)) as client:
                ...
                data = {
                    ...
                    "structured_output": structured_outout,
...
async def unit_test():
    ...
    rs = RemoteSession()
    msg = await rs.connect(endpoint_base=endpoint, session_id="test-s-1", token="40f81678-05a5-40d6-9aa5-8df53809a767")
