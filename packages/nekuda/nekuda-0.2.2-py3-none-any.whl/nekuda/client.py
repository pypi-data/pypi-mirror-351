...
class NekudaClient:
    ...
    def request_card_reveal_token(
        ...
        Returns:
            CardRevealTokenResponse containing the reveal token and API path
        """
        ...
        response = self._request(
            ...
            context=context,
            response_model=CardRevealTokenResponse,
        )

        # Type assertion since we know we're getting a model when response_model is provided
        assert isinstance(response, CardRevealTokenResponse)

        # For backward compatibility, ensure the response has the expected fields
        if hasattr(response, "reveal_token") and not hasattr(response, "reveal_path"):
            response.reveal_path = "/api/v1/wallet/reveal_card_details"
    ...
    def reveal_card_details(
        ...
        Returns:
            CardDetailsResponse containing card details
        """
        ...
        # Type assertion since we know we're getting a model when response_model is provided
        assert isinstance(response, CardDetailsResponse)
        return response
