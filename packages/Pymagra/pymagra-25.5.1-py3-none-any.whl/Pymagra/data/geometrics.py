...
class Geometrics:
    ...
    def append_data(
        self, x, y, d, year, month, day, hour, minute, second, time, mark, line
    ):
        """
    ...
    def read_stn(self, infile, n_block, h1_sensor=-1.4, h2_sensor=-0.4, dispo=0):
        ...
        for lin in lines:
            ...
            if int(line[0]) == 0:
                ...
                time_m.append(utils.date2julian(day_m[-1], month_m[-1], year_m[-1]))
    ...
    def read_dat(self, file, n_block, h1_sensor, h2_sensor, dispo):
        ...
        h1_sensor : float
            ...
            The z-coordinate is positive downward, therefore, this value is
            normally negative
        h2_sensor : float
                ...
                if line == line0:
                    ...
                    if n_lines > 0:
                        fo.write(
                            ...
                            + f"{ndat:11d}{line-1:12d}{int(mark)+1:12d}  "
                            + "36\n"
                        )
                    x = float(val[col_x])
                    y = float(val[col_y])
        ...
        with open(file, "w") as fh:
            ...
            for i in counter[::-1]:
                if self.line[i] != line0:
                    ...
                    if ncount > 0:
                        fh.write(
                            f"3{self.x[i+1]:14.3f}{self.y[i+1]:13.3f} "
                            + f"{self.hour[i+1]:02d}:"
                    ...
                    fh.write(
                        f"3{self.x[i]:14.3f}{self.y[i]:13.3f} "
                        + f"{self.hour[i]:02d}:"
                        + f"{self.minute[i]:02d}:"
                ...
                ncount += 1
            fh.write(
                f"3{self.x[0]:14.3f}{self.y[0]:13.3f} "
    ...
    def write_dat(self, file):
        ...
        with open(file, "w", encoding="utf-8") as fo:
            if np.isclose(np.std(self.z), 0.0):
                ...
                for i in counter[::-1]:
                    fo.write(
                        f"{self.x[i]:12.3f}{self.y[i]:12.3f}"
