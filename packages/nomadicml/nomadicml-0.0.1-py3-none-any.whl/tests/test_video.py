...
from nomadicml.types import VideoSource, ProcessingStatus, Severity, EventType
from nomadicml.exceptions import ValidationError, AnalysisError, NomadicMLError
import time
...
class TestVideoClient:
    ...
    def test_upload_video_youtube_with_missing_url(self, client):
        """Test upload_video with YOUTUBE source but missing youtube_url."""
        with pytest.raises(ValidationError):
            client.video.upload_video(source=VideoSource.YOUTUBE)
    ...
    # Override the actual validate_youtube_url in the module namespace
    @patch("nomadicml.video.validate_youtube_url")
    @patch("nomadicml.video.VideoClient.get_user_id")
    @patch("nomadicml.client.NomadicML._make_request")
    def test_upload_video_youtube_success(self, mock_make_request, mock_get_user_id,
                                     mock_validate_url, client):
        ...
        # Assertions
        assert result == {"status": "success", "video_id": "test_video_id"}
        mock_validate_url.assert_called_once_with("https://www.youtube.com/watch?v=test_video")
    ...
    def test_wait_for_analysis_failure(self, mock_get_status, client):
        ...
        with pytest.raises(NomadicMLError, match="Error waiting for completion: Video analysis failed: Analysis failed"):
            client.video.wait_for_analysis("test_video_id", poll_interval=0)  # Use 0 to skip sleep
    ...
    def test_upload_and_analyze_success(self, mock_get_analysis, mock_wait,
        ...
        mock_get_analysis.return_value = {
            "video_id": "test_video_id",
            "events": [{"type": "Traffic Violation", "time": 10.5}]
        }
