...
class RuntimeSpec(Enum):
    pass


class CPURuntimeSpec(RuntimeSpec):
    SMALL = auto()
    MEDIUM = auto()
    LARGE = auto()


class GPURuntimeSpec(RuntimeSpec):
    SMALL = auto()
    MEDIUM = auto()
    LARGE = auto()
    LARGE_CA = auto()
...
class Runtime:
    def __init__(self, name: str, image: str, tag: str, spec: RuntimeSpec):
        self.name = name
        self.image = image
        self.tag = tag
        self.spec = spec
...
class CPURuntime(Runtime):
    def __init__(self, image: str, tag: str, spec: CPURuntimeSpec):
        if not isinstance(spec, CPURuntimeSpec):
            raise ValueError(spec)
        super().__init__("CPU", image, tag, spec)


class GPURuntime(Runtime):
    def __init__(self, image: str, tag: str, spec: GPURuntimeSpec):
        if not isinstance(spec, GPURuntimeSpec):
            raise ValueError(spec)
        super().__init__("GPU", image, tag, spec)
    ...
    def execute(self, context):
        ...
        def submit_job() -> tuple[str, str]:
            request_body = {
                ...
                "input_nb": self.processor.notebook_path(),
