...
class TestMemorySyncIntegration:
    ...
    def test_strategy_initialization_includes_memory_sync(self, strategy_manager, temp_project):
        ...
        # Verify memory.db was created (SQLiteMemoryManager initialization)
        memory_db = agor_dir / "memory.db"
        assert memory_db.exists()
        
        # Verify memory manager has memory sync capabilities
        assert hasattr(strategy_manager.memory_manager, 'memory_sync_manager')
    ...
    def test_agent_discovery_initializes_memory_sync(self, agent_helper, temp_project):
        ...
        with patch('agor.tools.agent_coordination.SQLiteMemoryManager') as mock_memory:
            mock_instance = MagicMock()
            mock_instance.memory_sync_manager = MagicMock()
            mock_instance.active_memory_branch_name = "test-branch"
            ...
            # Verify SQLiteMemoryManager was called
    ...
    def test_agent_completion_saves_memory_state(self, agent_helper, temp_project):
        ...
        with patch('agor.tools.agent_coordination.SQLiteMemoryManager') as mock_memory:
            mock_instance = MagicMock()
            mock_instance.memory_sync_manager = MagicMock()
            mock_instance.shutdown_and_sync.return_value = True
            mock_memory.return_value = mock_instance
            ...
            # Verify shutdown_and_sync was called with correct parameters
            mock_instance.shutdown_and_sync.assert_called_once_with(
                commit_message="agent1: Test completion",
                restore_original_branch_override=None
            )

    def test_strategy_status_shows_memory_sync_info(self, strategy_manager, temp_project, capsys):
        ...
        with patch.object(strategy_manager.memory_manager, 'memory_sync_manager', MagicMock()):
            with patch.object(strategy_manager.memory_manager, 'active_memory_branch_name', "test-branch"):
                # Show status
    ...
    def test_memory_sync_graceful_failure(self, agent_helper, temp_project):
        ...
        with patch('agor.tools.agent_coordination.SQLiteMemoryManager') as mock_memory:
            mock_instance = MagicMock()
            mock_instance.memory_sync_manager = MagicMock()
            mock_instance.shutdown_and_sync.return_value = False  # Simulate failure
            mock_memory.return_value = mock_instance
    ...
    def test_memory_sync_initialization_error_handling(self, agent_helper, temp_project, capsys):
        """Test error handling during memory sync initialization."""
        # Create .agor directory
        agor_dir = temp_project / ".agor"
        agor_dir.mkdir()
        
        # Mock SQLiteMemoryManager to raise an exception
        with patch('agor.tools.agent_coordination.SQLiteMemoryManager', side_effect=Exception("Test error")):
    ...
    def test_parallel_divergent_strategy_with_memory_sync(self, strategy_manager, temp_project):
        ...
        agor_dir = temp_project / ".agor"
        assert agor_dir.exists()
        assert (agor_dir / "memory.db").exists()
    ...
    def test_memory_sync_with_multiple_agents(self, temp_project):
        ...
        with patch('agor.tools.agent_coordination.SQLiteMemoryManager') as mock_memory:
            ...
            assert mock_instance.shutdown_and_sync.call_count == 2


class TestMemorySyncHotkeysIntegration:
    """Test integration of memory sync hotkeys with agent workflows."""
    ...
    def test_memory_sync_hotkeys_available(self):
        """Test that memory sync hotkeys are available in the registry."""
        from agor.tools.sqlite_hotkeys import SQLITE_HOTKEY_REGISTRY
        
        # Verify all 4 memory sync hotkeys are registered
        assert "mem-sync-start" in SQLITE_HOTKEY_REGISTRY
        assert "mem-sync-save" in SQLITE_HOTKEY_REGISTRY
        assert "mem-sync-restore" in SQLITE_HOTKEY_REGISTRY
        assert "mem-sync-status" in SQLITE_HOTKEY_REGISTRY

    def test_memory_sync_hotkey_error_handling(self):
        """Test that memory sync hotkeys handle errors gracefully."""
        from agor.tools.sqlite_hotkeys import mem_sync_status_hotkey
        
        # Mock MemorySyncManager to raise an exception
        with patch('agor.tools.sqlite_hotkeys.MemorySyncManager', side_effect=Exception("Test error")):
            result = mem_sync_status_hotkey()
