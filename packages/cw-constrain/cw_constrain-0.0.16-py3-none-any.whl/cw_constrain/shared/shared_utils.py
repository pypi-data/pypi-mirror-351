...
def load_cw_search_upper_limits(run, search):
    ...
    grandparent_dir = os.path.dirname(os.path.dirname(this_dir)) 
    data_path = os.path.join(grandparent_dir, "limits")       

    if search == 'FH':
        if run == 'O2':
            with open(data_path+'FH_95UL.txt', 'r') as fid:
        ...
        if run == 'O3a':
            file_path = data_path+'Downloads/csv files/o3a_ul_all_simplified.csv'
