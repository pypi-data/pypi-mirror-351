...
from openssl_encrypt.modules.crypt_core import encrypt_file, decrypt_file, PQC_AVAILABLE, LIBOQS_AVAILABLE
from openssl_encrypt.modules.keystore_utils import get_pqc_key_for_decryption, extract_key_id_from_metadata
...
class TestHQCMLKEMKeystoreIntegration(unittest.TestCase):
    ...
    def _generate_mock_pqc_key(self, algorithm):
        ...
        mock_public_key = (b'MOCK_PQC_PUBLIC_KEY_FOR_' + algorithm.encode() + f"_{timestamp}".encode()) * 10
        mock_private_key = (b'MOCK_PQC_PRIVATE_KEY_FOR_' + algorithm.encode() + f"_{timestamp}".encode()) * 15
    ...
    def test_hqc_keystore_basic_operations(self):
        ...
        for algorithm in test_algorithms:
            with self.subTest(algorithm=algorithm):
                ...
                self.assertEqual(retrieved_public, public_key, f"Public key mismatch for {algorithm}")
                self.assertEqual(retrieved_private, private_key, f"Private key mismatch for {algorithm}")
    ...
    def test_ml_kem_keystore_basic_operations(self):
        ...
        for algorithm in test_algorithms:
            with self.subTest(algorithm=algorithm):
                ...
                self.assertEqual(test_key['algorithm'], algorithm, f"Algorithm mismatch in metadata")
    ...
    def test_hqc_file_encryption_with_keystore(self):
        ...
        self.assertEqual(retrieved_private, private_key, f"Private key should match for {algorithm}")
    ...
    def test_ml_kem_dual_encryption_keystore(self):
        ...
        retrieved_public, retrieved_private = self.keystore.get_key(key_id, file_password=self.file_password)
        
        self.assertEqual(retrieved_public, public_key, f"Public key mismatch for dual encryption {algorithm}")
        self.assertEqual(retrieved_private, private_key, f"Private key mismatch for dual encryption {algorithm}")
    ...
    def test_keystore_security_levels(self):
        ...
        for level in security_levels:
            with self.subTest(security_level=level):
                ...
                self.assertIsNotNone(key_id, f"Key should be added with security level {level.value}")
                
                # Verify key can be retrieved
                retrieved_public, retrieved_private = temp_keystore.get_key(key_id)
                self.assertEqual(retrieved_public, public_key, f"Key retrieval should work with security level {level.value}")
    ...
    def test_concurrent_keystore_operations(self):
        ...
        def add_and_remove_key(algorithm, thread_id):
            ...
            try:
                ...
                public_key, private_key = self._generate_mock_pqc_key(f"{algorithm}_thread_{thread_id}")
        ...
        print(f"Concurrent keystore operations test: {len(successes)} successes, {len(failures)} failures")
        for result in results:
            print(f"  {result}")
        
        # Most operations should succeed (allow for some potential race conditions)
        self.assertGreater(len(successes), len(failures), "Most concurrent operations should succeed")
