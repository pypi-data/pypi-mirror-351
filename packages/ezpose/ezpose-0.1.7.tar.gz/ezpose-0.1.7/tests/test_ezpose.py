...
def is_SO3(rot: SO3):
    assert np.testing.assert_array_equal(rot.as_matrix() @ rot.as_matrix().T, np.eye(3))
...
def test_identity():
    rot = SO3.random()
    assert rot == rot.from_rot6d(rot.as_rot6d())

    rots = SO3.random(10)
    assert all(rots == rots.from_rot6d(rots.as_rot6d()))
    # assert SE3.identity().as_matrix() == np.eye(4

    T = SE3.random(10)
    assert all(T == T.from_pose9d(T.as_pose9d()))
