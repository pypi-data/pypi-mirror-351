...
class TreeEntityXControl(EntityXControl):
    ...
    def has_same_name(self, entity):
        ...
        for path in self.td.paths:
            ...
            if not path.isAllowRepeatOfAll:
                ...
                im = self.exists([
                    {path.nameFieldName:entity.get(path.nameFieldName, '')}, 
                    {self.td.idFieldName:('!=',entity.get(self.td.idFieldName, -1))}
                ])
            ...
            if not path.isAllowRepeat and path.isAllowRepeatOfAll:
                ...
                im = self.exists([
                    { self.td.parentIdFieldName:entity.get(self.td.parentIdFieldName, None) }, 
                    { path.nameFieldName:entity.get(path.nameFieldName, '') }, 
                    { self.td.idFieldName:('!=',entity.get(self.td.idFieldName, -1)) }
                ])
                if im.data:
    ...
    def validate_name(self, entity, is_add=True, fields=None):
        """ 
        ...
        for path in self.td.paths:
            if is_add or fields == None or path.nameFieldName in fields:
                name = entity.get(self.td.nameFieldName, '')
                if not name:
                    im.success = False
    ...
    def fix_pattern(self, entity):
        ...
        # 更新自身
        # 如果自身是最顶级分类
        if mu.to_int(entity.get(self.td.parentIdFieldName)) < 1:
            entity[self.td.parentIdFieldName] = None
            entity['depth'] = 1
            entity['path'] = entity.get(self.td.idFieldName, -1)
            for path in self.td.paths:
                entity[path.namePathFieldName] = entity.get(path.nameFieldName)
        else:
            im = self.get_one(entity.get(self.td.parentIdFieldName))
            if not im.success:
                return im
            parent = im.data
            setattr(entity, "depth", int(getattr(parent, "depth")) + 1)
            setattr(entity, "path",
                getattr(parent, "path") +
                "," +
                entity.get(self.td.idFieldName, -1)
            )
            for path in self.td.paths:
                setattr(entity, path.namePathFieldName,
                    getattr(parent, path.namePathFieldName) +
                    "/" +
                    entity.get(path.nameFieldName)
                )
            # 他的父亲HasChildren必须是True
            setattr(parent, "hasChildren", True)
            im = super().update(parent, "hasChildren")
            if not im.success:
                return im
            
        # 更新自身HasChildren属性
        im = self.exists({self.td.parentIdFieldName:entity.get(self.td.idFieldName, -1)})
        if not im.success:
            return im
        entity['hasChildren'] = im.data
        # has_children = self.exists(self.td.parentIdFieldName == entity.get(self.td.idFieldName, -1))
        # setattr(entity, "hasChildren", has_children)

        im = super().update(entity, ",".join(self.treePatternFields))
        if not im.success:
            return im

        # 更新子节点
        children = self.get_list(self.td.parentIdFieldName == entity.get(self.td.idFieldName, -1))
    ...
    def add(self, entity):
        im = mu.IM()
        if self.has_same_name(entity):
            im.msg = '添加失败，已包含相同名称的节点。'
            return im

        im = self.validate_name(entity)
        if not im.success:
            return im
        ...
        im = self.fix_pattern(entity)
        if not im.success:
            return im

        # 确保在反馈的im中结果是实体
        im.data = entity
        im.id = entity.get(self.td.idFieldName, -1)
        return im

    def update(self, entity, fields="*"):
        im = mu.IM()
        if fields == "hasChildren":
            setattr(entity, "hasChildren", self.has_children(entity.get(self.td.idFieldName, -1)))
            return super.update(entity, "hasChildren")
            ...
            im = self.validate_name(entity, False, fields)
            if not im.success:
                return im
    ...
    def has_children(self, entity):
        return self.exists(self.td.parentIdFieldName == entity.get(self.td.idFieldName, -1))
    ...
    def get_children_ids(self, id, recursive=False, include_me=False):
        if id < 0:
            id = 0
        lst = self.get_children(id, recursive, include_me, self.td.idFieldName)
        return ",".join([str(getattr(item, self.td.idFieldName)) for item in lst])

    def get_children(self, id, recursive=False, include_me=False, fields_and_extends="*"):
        rlst = []
        lst = self.get_list(self.td.parentIdFieldName == id, order_by="sort", fields=fields_and_extends)
        if not lst:
            if include_me:
