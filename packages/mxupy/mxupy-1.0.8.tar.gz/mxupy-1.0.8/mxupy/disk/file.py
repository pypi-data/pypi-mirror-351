...
from mxupy import IM, check_access_token
...
def read_file(filename, type='user', user_id=-1, sub_dir='', download=False):
    """ 读取用户文件
    ...
    Args:
        ...
        download (bool): 下载模式否，下载模式会增加文件的头部信息
...
# @check_access_token
def upload_user_file(file:UploadFile, keep = True, override = False, sub_dir = '', *, chunk_index = -1, total_chunks = 1, user_id, access_token):
    """ 上传用户文件
...
def download_file_from_server(url: str, save_path: str):
    ...
    try:
        ...
        return IM(False, 'Download failed: ' + str(e))