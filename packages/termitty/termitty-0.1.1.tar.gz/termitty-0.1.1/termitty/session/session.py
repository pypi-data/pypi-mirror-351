...
from ..recording.formats import Recording
...
class TermittySession:
    ...
    def execute(self,
        ...
        # Record command input if recording
        if self._recorder and self._recorder.is_recording:
            self._recorder.record_input(command + '\n')
        ...
        if self._recorder and self._recorder.is_recording:
            # Record stdout and stderr as they would appear
            if result.stdout:
                self._recorder.record_output(result.stdout)
            if result.stderr:
                self._recorder.record_output(result.stderr)
    ...
    def start_recording(self, filename: str, format: str = 'native') -> None:
        """
        Start recording the terminal session.
        
        Args:
            filename: Path to save the recording
            format: Recording format ('native' or 'asciinema')
        
        Raises:
            SessionStateException: If already recording
        """
        if self._recorder and self._recorder.is_recording:
            raise SessionStateException("Already recording a session")
        
        # Get terminal dimensions
        width = config.terminal.width
        height = config.terminal.height
        if self.state.terminal:
            width = self.state.terminal.screen.width
            height = self.state.terminal.screen.height
        
        # Create recorder
        self._recorder = SessionRecorder(
            width=width,
            height=height,
            title=f"Termitty session - {self.transport.connection_info.get('host', 'localhost')}"
        )
        
        self._recording_file = filename
        self._recorder.start()
        
        logger.info(f"Started recording session to {filename}")
    ...
    def stop_recording(self) -> 'Recording':
        ...
        Returns:
            The completed recording object
        
        Raises:
            SessionStateException: If not recording
        """
        ...
        # Save to file using the recorder's save method
        self._recorder.save(self._recording_file)
        
        logger.info(f"Saved recording to {self._recording_file}")
    ...
    def add_recording_marker(self, name: str, description: Optional[str] = None) -> None:
        """
        ...
        if not self._recorder:
            raise SessionStateException("No recording in progress")
        self._recorder.add_marker(name, description)
