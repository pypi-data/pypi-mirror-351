...
def _get_exports_folder():
    ...
    exports_zip = os.path.join(os.path.dirname(__file__), 'Exports.zip')

    def _unzip():
        if util.safe_exists(exports_folder):
            if os.path.getmtime(exports_folder) < os.path.getmtime(exports_zip):
                util.safe_rmtree(exports_folder)
            else:
                return
        with zipfile.ZipFile(exports_zip, 'r') as zip_ref:
            zip_ref.extractall(exports_folder)

    util.os_lock('_get_exports_folder', _unzip)
...
def test_export_example_and_test_data():
    ...
    #
    # Then make sure to zip up the contents of "crab/sq-run-data-dir" without the "/cache", "/licenses", or "/log"
    # directories. TODO CRAB-47665: The "/configuration" dir must also be excluded until the config works universally.
    # Then replace "crab/sdk/pypi/spy-example-and-test-data-folder.zip" and commit it to the repo.
    ...
    exports_zip = os.path.join(os.path.dirname(__file__), 'Exports.zip')
    with zipfile.ZipFile(exports_zip, 'w') as zip_ref:
        for root, dirs, files in util.safe_walk(path):
            for file in files:
                # noinspection PyTypeChecker
                zip_ref.write(os.path.join(root, file), util.safe_relpath(os.path.join(root, file), path))
...
@pytest.mark.system
def test_zip_files_are_updated_in_sync():
    """
    If the various zip files that are used by SPy documentation or tests are updated, please make sure the
    spy-example-and-test-data-folder.zip file is also updated.
    Follow the instructions in the docstring of test_export_example_and_test_data() to perform this update.
    """

    def _get_test_setup_zip_file():
        return os.path.normpath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..',
                                             'spy-example-and-test-data-folder.zip'))

    def _get_test_zip_files():
        test_dir = os.path.join(os.path.dirname(__file__), '*.zip')
        return util.safe_glob(test_dir)

    def _get_documentation_zip_files():
        docs_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'docs',
                                'Documentation', 'Support Files', '*.zip')
        return util.safe_glob(docs_dir)

    def _get_last_commit_date_of_file(_file_path):
        try:
            result = subprocess.run(
                ["git", "log", "-n", "1", "--format=format:%ad", "--date=iso8601", "--", _file_path],
                capture_output=True, text=True, check=True)
            return pd.to_datetime(result.stdout.strip())
        except Exception as e:
            return f"Error: {e}"

    setup_file = _get_test_setup_zip_file()
    setup_file_commit_date = _get_last_commit_date_of_file(setup_file)
    print(f'Baseline commit for {setup_file} was at {setup_file_commit_date}')
    for test_zip_file in _get_test_zip_files():
        commit_date = _get_last_commit_date_of_file(test_zip_file)
        print(f'Commit for {test_zip_file} was at {commit_date}')
        assert commit_date <= setup_file_commit_date, f'{test_zip_file} has a newer commit date than {setup_file}'
    for doc_zip_file in _get_documentation_zip_files():
        commit_date = _get_last_commit_date_of_file(doc_zip_file)
        print(f'Commit for {doc_zip_file} was at {commit_date}')
        assert commit_date <= setup_file_commit_date, f'{doc_zip_file} has a newer commit date than {setup_file}'
...
def test_tree_items():
    ...
    def _verify_correct_items(_area, _workbooks):
        ...
        _expected_items = [{'Name': 'Compressor Power', 'ID': _correct_item_ids[0]},
                           {'Name': 'Compressor Power', 'ID': _correct_item_ids[1]},
                           {'Name': tree_file_items_df.iloc[0]['Name'], 'ID': tree_file_items_df.iloc[0]['ID']},
                           {'Name': tree_file_items_df.iloc[1]['Name'], 'ID': tree_file_items_df.iloc[1]['ID']},
                           ]
        ...
        for _worksheet in _workbooks[0].worksheets:  # type: Worksheet
            ...
            _worksheet_items = [{'Name': item['name'], 'ID': item['id']} for item
                                in _current_workstep.data['state']['stores']['sqTrendSeriesStore']['items']]
            print(f'Now checking trend items in worksheet "{_worksheet.name}" ({_worksheet.id})')
            print(f'Expected items: {_expected_items}')
            print(f'Actual items: {_worksheet_items}')
...
def test_datasource_map_by_asset():
    ...
    for as_template in [False, True]:
        as_template_with_label = test_name if as_template else None
        workbooks = spy.workbooks.load(
            get_full_path_of_export('Example Analysis (D833DC83-9A38-48DE-BF45-EB787E9E8375)'),
            as_template_with_label=as_template_with_label
        )
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with util.safe_open(
                    os.path.join(temp, 'Datasource_Map_Time Series CSV Files_Example Data_Example Data.json'),
                    'w') as f:
                json.dump(example_datasource_map, f)

            label = test_name if not as_template else None
            push_df = spy.workbooks.push(workbooks, label=label, path=test_name, datasource_map_folder=temp,
...
def test_globals():
    ...
    local_compressor_high_id = '0EFF49E8-CA74-FFC0-9738-4A1618BB5C05'
...
def test_annotation_interested_in_capsule():
    ...
    # Seeq outputs different types of Capsule IDs depending on the Seeq version, 65 vs 66
    try:
        # New detailId format, but it might not work if this is an older version of Seeq
        # Versions below R66.10.X output a full detailId with query information, but R66.10+ does not.
        capsule_id = f'1672531200000000000:{capsules_result.capsules.capsules[0].id}:1672617600000000000'
