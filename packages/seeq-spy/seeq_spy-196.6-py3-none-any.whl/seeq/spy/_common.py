...
def validate_hex_string(hex_string):
    match = re.search(HEX_COLORCODE_REGEX, hex_string)
    if not match:
        raise SPyValueError(f"{hex_string} is not a valid hex color code")
...
def convert_to_timestamp(value: Optional[Union[int, str]], tz: Optional[str]) -> pd.Timestamp:
    """
