...
class BleClient:
    ...
    async def get_live_data(
        ...
        await self.start_live_data_notifications(_interpret_live_data)
        await asyncio.wait_for(data_transfer_finished.wait(), timeout=timeout)
        await self.stop_live_data_notifications()

        return live_data
    
    async def start_live_data_notifications(
            self, 
    ...
    async def _live_data_notifications_handler(self, sender, data):
        ...
        messenger.debug(f"Live data packet received: {data}")
