...
class VirtualenvManagerWidget(QWidget, Ui_VirtualenvManagerWidget):
    ...
    def __updateButtons(self):
        ...
        for itm in self.venvList.selectedItems():
            if (
                ...
                and itm.data(0, VirtualenvManagerWidget.MetadataRole).environment_type
                != "remote"
            ):
    ...
    def on_venvList_itemSelectionChanged(self):
        ...
        if len(selectedItems) == 1:
            envMetaData = selectedItems[0].data(0, VirtualenvManagerWidget.MetadataRole)
            self.descriptionEdit.setPlainText(envMetaData.description)
            self.interpreterEdit.setText(envMetaData.interpreter)
        else:
            self.descriptionEdit.clear()
            self.interpreterEdit.clear()
    ...
    def __populateVenvList(self):
        ...
        """
        typeNamesMapping = {
            n[0]: n[1] for n in self.__manager.getEnvironmentTypeNames()
        }
...
class VirtualenvManagerWindow(EricMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        ericApp().registerObject("VirtualEnvManager", self.__virtualenvManager)
    ...
    def getLocale(self):
        """
        Public method to get the locale of the Virtualenv Manager Window.

        @return name of the locale
        @rtype str
        """
        return Startup.getLocale()
