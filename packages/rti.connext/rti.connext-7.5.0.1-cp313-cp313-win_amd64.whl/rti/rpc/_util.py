...
class MetaRequestReply(type):
    ...
    def default_writer_qos(cls):
        ...
        qos.data_writer_resource_limits.max_remote_readers = (
            rti.connextdds.LENGTH_UNLIMITED
        )
    ...
    def default_reader_qos(cls):
        ...
        qos.reliability.kind = rti.connextdds.ReliabilityKind.RELIABLE
        qos.reliability.max_blocking_time = rti.connextdds.Duration(10, 0)
        qos.history.kind = rti.connextdds.HistoryKind.KEEP_ALL
        qos.data_reader_protocol.rtps_reliable_reader.min_heartbeat_response_delay = (
            rti.connextdds.Duration.zero
        )
        qos.data_reader_protocol.rtps_reliable_reader.max_heartbeat_response_delay = (
            rti.connextdds.Duration.zero
        )
        qos.data_reader_protocol.rtps_reliable_reader.nack_period = (
            rti.connextdds.Duration(0, 500000000)
        )
        return qos
...
def get_topic(
    ...
    if custom_topic is not None:
        ...
        if isinstance(custom_topic, str):
            topic = get_or_create_topic_from_name(
                type_class, data_type, participant, custom_topic
            )
        else:
            topic = custom_topic
            if not (
                isinstance(topic, type_class.Topic)
                or isinstance(topic, type_class.ContentFilteredTopic)
            ):
        ...
        topic = get_or_create_topic_from_name(
            type_class, data_type, participant, topic_name
        )
    if not cft_allowed and isinstance(topic, type_class.ContentFilteredTopic):
        raise rti.connextdds.InvalidArgumentError(
            "ContentFilteredTopic cannot be used for this endpoint"
        )
    return topic
...
def get_listener_class(data_type):
    ...
    if data_type in reader_listeners:
        return reader_listeners[data_type]
    ...
    reader_listeners[data_type] = listener_class
    return listener_class


def _update_topic_alias_format(
    endpoint_qos: Union[rti.connextdds.DataWriterQos, rti.connextdds.DataReaderQos],
    participant_qos: rti.connextdds.DomainParticipantQos,
    service_name: Optional[str]
):
    """
    Update the topic alias format property in the endpoint QoS if the ROS interoperability
    is enabled and the topic alias format is not already set.
    """
    ROS_ENABLE_INTEROPERABILITY = "dds.ros.enable_interoperability"
    TOPIC_ALIAS_FORMAT = "dds.topic.alias_format"

    enable_ros_interoperability = (
        endpoint_qos.property.try_get(ROS_ENABLE_INTEROPERABILITY)
        or participant_qos.property.try_get(ROS_ENABLE_INTEROPERABILITY)
        or "0"
    ).lower() in ("1", "true", "yes")

    if not enable_ros_interoperability:
        return

    if any(
        qos.property.exists(TOPIC_ALIAS_FORMAT)
        for qos in (endpoint_qos, participant_qos)
    ):
        return

    role_name = endpoint_qos.entity_name.role_name
    is_datawriter = isinstance(endpoint_qos, rti.connextdds.DataWriterQos)
    requester_format = "rq/%Request" if service_name is None else "rq/%"
    replier_format = "rr/%Reply" if service_name is None else "rr/%"

    if role_name == "Requester":
        alias_format = requester_format if is_datawriter else replier_format
    elif role_name == "Replier":
        alias_format = replier_format if is_datawriter else requester_format
    else:
        return

    endpoint_qos.property[TOPIC_ALIAS_FORMAT] = alias_format
...
def create_writer(
    ...
    if datawriter_qos is None:
        ...
        # Make a copy of the input argument since we need to modify it
        datawriter_qos = rti.connextdds.DataWriterQos(datawriter_qos)

    if datawriter_qos.entity_name.role_name is None:
        datawriter_qos.entity_name.role_name = role_name

    _update_topic_alias_format(datawriter_qos, participant.qos, service_name)

    if isinstance(writer_type, rti.connextdds.DynamicType):
        writer = rti.connextdds.DynamicData.DataWriter._create_disabled(
            publisher, topic, datawriter_qos
        )
    else:
        writer = rti.connextdds.DataWriter._create_disabled(
            publisher, topic, datawriter_qos
        )
    return writer
...
def create_reader(
    ...
    if cft_guid is not None:
        ...
        topic = reader_cls.ContentFilteredTopic(
            topic, filter_name, rti.connextdds.Filter(filter_expression)
        )
    ...
    if datareader_qos is None:
        ...
        # Make a copy of the input argument since we need to modify it
        datareader_qos = rti.connextdds.DataReaderQos(datareader_qos)
    if datareader_qos.entity_name.role_name is None:
        datareader_qos.entity_name.role_name = role_name

    if not datareader_qos.property.exists(
        "dds.data_reader.history.generate_not_alive_no_writers_sample"
    ):
    ...
    if not datareader_qos.property.exists(
        ...
        # This is the standard XTypes 1.3 behaviour, and is required by RPC to
        # support extensibility (adding/removing operations and exceptions to an
        # operation)
        datareader_qos.property[
            "dds.sample_assignability.accept_unknown_union_discriminator"
        ] = "2"

    _update_topic_alias_format(datareader_qos, participant.qos, service_name)
    ...
    reader = reader_cls.DataReader._create_disabled(subscriber, topic, datareader_qos)
...
def wait_for_samples(
    ...
    while min_count > 0:
        if min_count == 1:
            if not waitset.wait_one(remaining_wait):
                return False
        else:
...
def validate_related_request_id(related_request_id):
    ...
    if (
        related_request_id.writer_guid == rti.connextdds.Guid.automatic
        or related_request_id.writer_guid == rti.connextdds.Guid.unknown
    ):
        raise rti.connextdds.InvalidArgumentError("related_request_id.writer_guid")
    if (
        related_request_id.sequence_number == rti.connextdds.SequenceNumber.automatic
        or related_request_id.sequence_number == rti.connextdds.SequenceNumber.unknown
        or related_request_id.sequence_number == rti.connextdds.SequenceNumber.maximum
        or related_request_id.sequence_number == rti.connextdds.SequenceNumber.zero
    ):
...
def match_count(
    ...
    pub_count = len(
        [
            pub
            for pub in pubs
            if reader.matched_publication_data(pub).publication_name.role_name
            == role_name
        ]
    )
    ...
    sub_count = len(
        [
            sub
            for sub in subs
            if writer.matched_subscription_data(sub).subscription_name.role_name
            == role_name
        ]
    )
...
class RequestReplyBase(RRMETA):
    ...
    def __init__(
        ...
        if use_waitset:
            ...
            self._any_sample_condition = rti.connextdds.ReadCondition(
                self._reader, rti.connextdds.DataState.any
            )
            self._notread_sample_condition = rti.connextdds.ReadCondition(
                self._reader, rti.connextdds.SampleState.NOT_READ
            )
    ...
    def close(self) -> None:
        ...
        if self._closed:
            raise rti.connextdds.AlreadyClosedError(
                "This request-reply object has already been closed"
            )
        participant = self._reader.subscriber.participant
