...
class Sequence(ctypes.Structure):
    ...
    def get_elements_ptr(self):
        ...
        return ctypes.cast(self._contiguous_buffer, ctypes.POINTER(self._element_type))
    ...
    def copy_from_buffer(self, buffer, element_count: int):
        ...
        ctypes.memmove(
            self._contiguous_buffer, buffer, element_count * self._element_size
        )
...
def create_sequence_type(element_ctype):
    ...
    if issubclass(element_ctype, Sequence):
        raise TypeError(
            "Sequences of sequences are not supported: wrap the inner sequence in a @idl.alias or @idl.struct"
        )
    ...
    return type(
        ...
        (Sequence,),
        {"_element_type": element_ctype, "_element_size": ctypes.sizeof(element_ctype)},
    )
