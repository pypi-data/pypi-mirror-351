...
class CopyPrimitiveToPyInstruction(Instruction):
    ...
    def __init__(
        self, field_name: str, is_optional: bool = False, field_index: int = -1
    ):
        super().__init__(
            field_name,
            is_primitive=True,
...
class CopyAggregationToPyInstruction(Instruction):
    ...
    def __init__(
        ...
        super().__init__(
            field_name, is_optional=is_optional, field_factory=field_factory
        )
...
class CopyStrToBytesInstructionMixin(StringInstruction):
    ...
    def __init__(
        self, field_name: str, field_index: int, is_optional: bool, bound: int
    ):
        super().__init__(field_name, field_index=field_index, is_optional=is_optional)
        self.bound = bound
...
class CopyBytesToWStrInstruction(
    WideStringConstantsMixin, CopyBytesToStrInstructionMixin
):
...
class CopyWStrToBytesInstruction(
    WideStringConstantsMixin, CopyStrToBytesInstructionMixin
):
...
class CopyConstructedArrayToListInstruction(Instruction):
    ...
    def __init__(
        self, field_name: str, py_element_factory, element_program: "SampleProgram"
    ):
...
class SampleProgram:
    ...
    def __init__(
        self, instructions: List[Instruction], type_plugin: dds._TypePlugin = None
    ) -> None:
...
class UnionSampleProgram:
    ...
    def execute(self, dst, src):
        ...
        try:
            ...
            instruction = self.instructions.get(discriminator, self.default_instruction)
            ...
            if instruction is not None:
                if self.type_plugin is None:
                    ...
                    if instruction.field_factory is not None:
                        ...
                        setattr(
                            dst, instruction.field_name, instruction.field_factory()
                        )

                instruction.execute(dst=dst, src=src)
            else:
                # Unknown discriminator: keep it but set the value to None
                setattr(dst, "value", None)
...
def get_field_factory(field_type, member_annotations) -> Callable[[], Any]:
    field_type = reflection_utils.remove_classvar(field_type)
    ...
    if reflection_utils.is_sequence_type(field_type):
        ...
        return type_utils.get_optimal_collection_factory(element_type, dimensions)
...
class SamplePrograms:
    ...
    def __init__(
        ...
        if reflection_utils.is_enum(py_type):
            ...
            if is_union:
                (
                    self.c_to_py_program,
                    self.py_to_c_program,
                ) = self._create_union_programs(
                ...
                (
                    self.c_to_py_program,
                    self.py_to_c_program,
                ) = self._create_struct_programs(
                    py_type, type_plugin, member_annotations
                )

    def _create_struct_programs(self, py_type: type, type_plugin, member_annotations):
        ...
        return SampleProgram(c_to_py_instructions), SampleProgram(
            py_to_c_instructions, type_plugin
        )

    def _create_union_programs(self, py_type: type, type_plugin, member_annotations):
        discr = union_discriminator(py_type)
        (
            c_to_py_discr_instructions,
            py_to_c_discr_instructions,
        ) = self._create_class_instructions(
            (discr,),
            first_index=0,
            type_plugin=type_plugin,
            member_annotations=member_annotations,
    ...
    def _create_class_instructions(
        self, fields, first_index, type_plugin, member_annotations
    ):
        ...
        for field in fields:
            ...
            plugin_instruction_index = (
                type_plugin.get_instruction_index_from_member_index(i)
            )
    ...
    def _create_array_instructions(
        self, field, is_optional: bool, field_factory, sequence_annotations
    ):
        if is_optional:
        ...
        if reflection_utils.is_primitive_or_enum(element_type):
            ...
            if reflection_utils.supports_buffer_protocol(factory_type):
                c_to_py_instr = CopyPrimitiveArrayToBufferInstruction(field.name)
                py_to_c_instr = CopyBufferToArrayInstruction(field.name)
            else:
                c_to_py_instr = CopyPrimitiveArrayToListInstruction(field.name)
                py_to_c_instr = CopyPrimitiveListToArrayInstruction(field.name)
            ...
            c_to_py_instr = CopyStrArrayToListInstruction(
                field.name, element_instructions[0]
            )

            py_to_c_instr = CopyStrListToArrayInstruction(
                field.name, element_instructions[1]
            )
        else:
            element_ts = element_type.type_support
            ...
            py_to_c_instr = CopyClassListToArrayInstruction(
                field.name, element_ts._sample_programs.py_to_c_program
            )
    ...
    def _get_field_factory(self, field, member_annotations) -> Callable[[], Any]:
        ...
        return get_field_factory(field.type, member_annotations)
