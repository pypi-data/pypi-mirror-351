...
def _create_array_ctype(element_ctype: type, array_info: annotations.ArrayAnnotation):
    if issubclass(element_ctype, csequence.Sequence):
        raise TypeError(
            "Arrays of sequences are not supported: wrap the sequence in a @idl.alias or @idl.struct"
        )
...
def _get_member_ctype(py_type, member_annotations):
    ...
    if reflection_utils.is_sequence_type(py_type):
        ...
        element_py_type = reflection_utils.get_underlying_type(py_type)
        element_ctype = _get_member_ctype(element_py_type, element_annotations.value)
        ...
        elif reflection_utils.is_constructed_type(element_py_type):
            # Optimization: constructed types cache their sequence type
            return element_py_type.type_support.c_type_seq
        else:
...
def create_ctype_from_dataclass(py_type, member_annotations):
    ...
    return type(
        ...
        (ctypes.Structure,),
        {"__doc__": f"Equivalent C type for {py_type.__name__}", "_fields_": c_fields},
    )


def create_ctype_from_union_dataclass(
    py_type, discriminator, cases, member_annotations
):
    ...
    for field in cases:
        ...
        member_type = _get_member_ctype(
            reflection_utils.remove_classvar(field.type), current_annotations
        )
        c_fields.append((field.name, member_type))
...
def _get_member_dynamic_type(
    py_type, member_annotations, type_factory: dds._GenericTypePluginFactory
):
    if reflection_utils.is_optional_type(py_type):
        underlying_type = reflection_utils.get_underlying_type(py_type)
        return _get_member_dynamic_type(
            underlying_type, member_annotations, type_factory
        )
    ...
    if reflection_utils.is_sequence_type(py_type):
        ...
        if array_info.is_array:
            ...
            return type_factory.create_sequence(member_dynamic_type, bound.value)
...
def create_dynamic_type_from_dataclass(
    ...
    for field in actual_fields:
        ...
        default_value = annotations.find_annotation(
            current_annotations, cls=annotations.DefaultAnnotation
        )

        min_value = annotations.find_annotation(
            current_annotations, cls=annotations.MinAnnotation
        )

        max_value = annotations.find_annotation(
            current_annotations, cls=annotations.MaxAnnotation
        )

        member_args.append(
            (field.name, member_type, member_id.value, is_key.value,
             is_optional, False, default_value.value, min_value.value, max_value.value)
        )

    drtn = annotations.find_annotation(type_annotations, annotations.TypeNameAnnotation)
    type_name = drtn.value or py_type.__name__
    ...
    if base_type is None:
        dynamic_type = type_factory.create_struct(
            type_name, extensibility.value, get_size(c_type), c_type_offsets
        )
    else:
        base_type = (
            base_type.type_support._dynamic_type_ref
            if not is_public_type
            else base_type.type_support.dynamic_type
        )
        dynamic_type = type_factory.create_struct(
            type_name, base_type, extensibility.value, get_size(c_type), c_type_offsets
        )
...
def create_dynamic_type_from_union_dataclass(
    ...
    for field in cases:
        ...
        member_args.append(
            (field.name, member_type, case.labels,
             member_id.value, False, default_value.value, min_value.value, max_value.value)
        )
...
def create_dynamic_type_from_alias_dataclass(
    ...
    if is_key.value:
        raise TypeError("An @idl.alias cannot be a key")
    if reflection_utils.is_optional_type(field.type):
        raise TypeError("An @idl.alias cannot be optional")
    ...
    drtn = annotations.find_annotation(
        current_annotations, annotations.TypeNameAnnotation
    )
    type_name = drtn.value or py_type.__name__
    dynamic_type = type_factory.create_alias(type_name, member_type, get_size(c_type))
...
def create_dynamic_type_from_enum(py_type, type_annotations=None):
    ...
    return _internal_type_factory.create_enum(
        py_type.__name__, extensibility.value, enum_values
    )
...
class TypeSupport:
    ...
    def __init__(
        ...
        sample_program_options: Optional[
    ...
    def _create_dynamic_type(self, is_public: bool):
        if self.kind == TypeSupportKind.ENUM:
            return create_dynamic_type_from_enum(self.type, self.type_annotations)
    ...
    def _create_c_sample(self, sample):
        if not self.allow_duck_typing and type(sample) is not self.type:
            raise TypeError(f"Expected type {self.type}, got {type(sample)}")

        c_sample = self.c_type()
        self._plugin_dynamic_type.initialize_sample(c_sample)
        try:
            self._sample_programs.py_to_c_program.execute(src=sample, dst=c_sample)
        except:
    ...
    def _convert_to_c_sample(self, c_sample, py_sample):
        ...
        self._sample_programs.py_to_c_program.execute(src=py_sample, dst=c_sample)
    ...
    def _create_py_sample(self, c_sample_ptr):
        ...
        self._sample_programs.c_to_py_program.execute(src=c_sample, dst=py_sample)
        return py_sample
    ...
    def serialize(self, sample):
        """Serialize a data sample of this type into a cdr byte buffer"""
    ...
    def deserialize(self, buffer) -> Any:
        """Create a data sample of this type by deserializing a cdr byte buffer"""
    ...
    def get_serialized_sample_size(self, sample) -> int:
        """Returns the serialized size of a given data sample of this type"""
        c_sample = self._create_c_sample(sample)
        try:
    ...
    def is_unbounded(self) -> bool:
        ...
        return (
            self.max_serialized_sample_size
            >= annotations.UNBOUNDED - _SERIALIZATION_OVERHEAD_BYTES
        )
    ...
    def type_name(self) -> str:
        """Returns the default registered type name for the type"""
        drtn = annotations.find_annotation(
            self.type_annotations, annotations.TypeNameAnnotation
        )
        return drtn.value or self.type.__name__

    def to_dynamic_data(self, sample: Any) -> dds.DynamicData:
        """Converts a data sample of this type to DynamicData"""
    ...
    def from_dynamic_data(self, sample: dds.DynamicData) -> Any:
        """Converts a DynamicData sample to a data sample of this type"""
        return self.deserialize(sample.to_cdr_buffer())

    def to_string(
        self,
        sample: Any,
        format: dds.PrintFormatProperty = dds.PrintFormatProperty.default,
    ) -> str:
        """Converts a data sample of this type to a string based on the provided
        format. To convert to and from JSON, prefer to_json, from_json instead.
        """
        dd_sample = self.to_dynamic_data(sample)
        return dd_sample.to_string(format)

    def from_string(self, string: str, format_kind: dds.PrintFormatKind) -> Any:
        """Converts a string to a data sample of this type based on the provided
        format (only JSON is supported by this function). To convert to and from
        JSON, prefer to_json, from_json instead.
        """
        dynamic_type = self._get_public_dynamic_type().get_dynamic_type_ref()
        dd_sample = dds.DynamicData(dynamic_type)
        dd_sample.from_string(string, format_kind)
        return self.from_dynamic_data(dd_sample)

    def to_json(self, sample: Any) -> str:
        """Converts a data sample of this type to a json string"""
        return self.to_string(sample, dds.PrintFormatProperty.json)

    def from_json(self, json_str: str) -> Any:
        """Converts a json string to a data sample of this type"""
        return self.from_string(json_str, dds.PrintFormatKind.JSON)
