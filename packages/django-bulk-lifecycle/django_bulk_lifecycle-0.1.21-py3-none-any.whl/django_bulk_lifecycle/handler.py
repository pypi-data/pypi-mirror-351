...
class TriggerHandler(metaclass=TriggerHandlerMeta):
    ...
    def handle(
        ...
        **kwargs,
        ...
        # Ensure lists are non-null and of equal length for HookCondition
        new_records = new_records or []
        old_records = old_records if old_records is not None else [None] * len(new_records)
            ...
            if condition is not None:
                if isinstance(condition, HookCondition):
                    ...
                    for new, old in zip(new_records, old_records):
                        # Peek at field and expected value
                        watched_field = getattr(condition, "field", None) or getattr(condition, "field_name", None)
                        actual_val = getattr(new, watched_field, None)
                        expected = getattr(condition, "expected", None) or getattr(condition, "value", None)
                        logger.debug(
