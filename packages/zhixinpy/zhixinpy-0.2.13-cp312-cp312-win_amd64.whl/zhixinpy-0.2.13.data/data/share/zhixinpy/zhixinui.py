...
def get_license_file():
    license_file = ""
    try:
        license_file = _settings.get_env_var("LICENSEFILE")
    except:
...
def get_license_data():
    if g_verify_result["code"] == 0:
        ret = f'授权给{g_verify_result.get("email")},详细信息点击知心-知心配置-注册流程'
    elif g_verify_result["code"] == 1:
        ret = g_verify_result["msg"] + ", 仍可使用基础功能"
    elif g_verify_result["code"] == 2:
        ret = g_verify_result["msg"] + ", 仍可使用基础功能，请重新申请授权文件"
    elif g_verify_result["code"] == 3:
        ret = g_verify_result["msg"]
    else:
        ret = "未定义的错误类型"
    return ret
...
def load_register_file(ctrl):
    ...
    shutil.copy(filename, os.path.join(os.environ["appdata"], "xlOil", "zhixinpy.lic"))
    MsgBox(
...
from zhixinpy.ribbon import show_config_dialog, load_config, show_registration_dialog, open_website
from zhixinpy.ribbon_chart_unified import (
    show_wordcloud_dialog, show_line_dialog, show_pie_dialog, show_bar_dialog,
    show_scatter_dialog, show_geo_dialog, show_geoline_dialog, show_map_dialog,
    show_pie_bar_dialog, show_rank_dialog, show_line3d_dialog
)

gallery_4_images = [{'file_name':'chart-wordcount.png', 'chart_name':'词云图'}]
...
def get_gallery_4_image(ctrl, index: int):
    image_path = Path(os.path.join(sys.prefix, "share", "zhixinpy")) / gallery_4_images[index]["file_name"]
	...
	<ribbon>
		<tabs>
			<tab id="tab1" label="❤️知心❤️" insertAfterMso="TabHome">
                <group id="chartGroup" label="知心函数">
					...
					<menu id="toolMenu" size="large" label="工具" imageMso="ControlToolboxOutlook" itemSize="normal">
						<button id="tool1" label="二维码生成" imageMso="PivotTableSelectData" onAction="display_formula" tag="zt_Qrcode" screentip="用于生成和解析二维码的功能"/>
						<button id="tool2" label="图像处理" imageMso="PictureInsertFromFile" onAction="display_formula" tag="zt_Image" screentip="支持图像加载、格式转换和基础处理操作"/>
						<button id="tool3" label="视频下载" imageMso="ToolboxVideo" onAction="display_formula" tag="zt_Video" screentip="实现视频编解码和基础剪辑功能"/>
						<button id="tool4" label="文件下载" imageMso="FileSave" onAction="display_formula" tag="zt_Download" screentip="支持多线程文件下载和断点续传"/>
						<button id="tool5" label="发送邮件" imageMso="SendStatusReport" onAction="display_formula" tag="zt_Mail" screentip="提供SMTP邮件发送和接收功能"/>
						<button id="tool6" label="大语言模型" imageMso="HappyFace" onAction="display_formula" tag="zt_LLM" screentip="基础自然语言处理接口"/>
						<button id="tool7" label="扩展语言模型" imageMso="HappyFace" onAction="display_formula" tag="zt_LLMEx" screentip="支持自定义模型的扩展接口"/>
						<button id="tool8" label="文本生成" imageMso="TextBoxInsert" onAction="display_formula" tag="zt_LLMGen" screentip="实现AI文本生成和续写功能"/>
						<button id="tool9" label="爬虫" imageMso="HyperlinkInsert" onAction="display_formula" tag="zt_Crawler" screentip="单页面内容抓取和解析工具"/>
						<button id="tool10" label="爬虫与翻页" imageMso="HyperlinkInsert" onAction="display_formula" tag="zt_CrawlerAll" screentip="分布式网站全内容抓取系统"/>
						<button id="tool11" label="公钥生成" imageMso="Lock" onAction="display_formula" tag="zt_GenPubKey" screentip="生成RSA/ECC非对称加密公钥"/>
						<button id="tool12" label="数据加密" imageMso="FileDocumentEncrypt" onAction="display_formula" tag="zt_Encrypt" screentip="支持AES/DES对称加密算法"/>
						<button id="tool13" label="数据解密" imageMso="EncryptMessage" onAction="display_formula" tag="zt_Decrypt" screentip="解密已加密的文件和数据流"/>
						<button id="tool14" label="文字转语音" imageMso="SoundInsertFromFile" onAction="display_formula" tag="zt_tts" screentip="将文本转换为自然语音输出"/>
						<button id="tool15" label="桌面通知" imageMso="_3DLightingClassic" onAction="display_formula" tag="zt_Notice" screentip="发送桌面和移动端推送通知"/>
						<button id="tool16" label="API调用" imageMso="DataRefreshAll" onAction="display_formula" tag="zt_API_CALL" screentip="执行RESTful API请求和响应处理"/>
						<button id="tool17" label="数据转换" imageMso="ConvertTextToTable" onAction="display_formula" tag="zt_json_to_table" screentip="将JSON数据转换为表格格式"/>
						<button id="tool19" label="媒体转换" imageMso="OmsNewMultimediaMessage" onAction="display_formula" tag="zt_Convert_Media" screentip="支持音视频格式互转和压缩"/>
					</menu>
					<gallery id="chart4" imageMso="ChartTypeBarInsertGallery" label="文本与可视化" size="large"
                        columns="1" rows="6" itemWidth="240" itemHeight="180"
					...
					<menu id="chartMenu" size="large" label="图表" imageMso="ChartTypeBarInsertGallery">
						<menu id="chart1" label="基础统计" imageMso="Chart3DColumnChart" itemSize="normal">
							<button id="chart1-1" label="折线图" imageMso="ChartTypeBarInsertGallery" onAction="show_line_dialog" screentip="展示连续数据的变化趋势（2D）"/>
							<button id="chart1-2" label="三维折线图" imageMso="ChartTypeBarInsertGallery" onAction="show_line3d_dialog" screentip="折线图的3D版本，适合空间维度趋势展示"/>
							<button id="chart1-3" label="面积图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Area" screentip="类似折线图，强调数量的累计区域大小"/>
							<button id="chart1-4" label="柱状图" imageMso="ChartTypeBarInsertGallery" onAction="show_bar_dialog" screentip="类别数据对比常用图表，支持横/竖展示"/>
							<button id="chart1-5" label="直方图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Histogram" screentip="展示连续数值分布的频率"/>
							<button id="chart1-6" label="饼图" imageMso="ChartTypeBarInsertGallery" onAction="show_pie_dialog" screentip="展示占比结构"/>
							<button id="chart1-7" label="箱型图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Box" screentip="展示分布特征（中位数、四分位等）"/>
							<button id="chart1-8" label="小提琴图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Violin" screentip="结合箱型图+密度图，展示分布形态"/>
							<button id="chart1-9" label="累积分布函数图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Ecdf" screentip="展示累积分布，便于对比分布"/>
							<button id="chart1-10" label="条带图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Strip" screentip="展示分布情况，类似散点，但在一维上"/>
						</menu>
						<menu id="chart2" label="散点与空间分布" imageMso="Chart3DColumnChart" itemSize="normal">
							<button id="chart2-1" label="散点图" imageMso="ChartTypeBarInsertGallery" onAction="show_scatter_dialog" screentip="二维空间下的点分布图"/>
							<button id="chart2-2" label="三维散点图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Scatter3d" screentip="增加空间维度，更复杂数据分析适用"/>
							<button id="chart2-3" label="地图散点图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_ScatterMap" screentip="在地图上标出散点，通常用于地理数据分析"/>
							<button id="chart2-4" label="地图路径图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_LineMap" screentip="地图上的连线路径，常用于轨迹/路线可视化"/>
						</menu>
						<menu id="chart3" label="层级与流向关系" imageMso="Chart3DColumnChart" itemSize="normal">
							<button id="chart3-1" label="桑基图(财报)" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Sankey_income" screentip="展示收入流向、来源关系，强调数量流动"/>
							<button id="chart3-2" label="桑基图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_sankey" screentip="一般用途桑基图，表示能量/数据/流程转移"/>
							<button id="chart3-3" label="矩形树图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Treemap" screentip="层级结构展示，每层级用面积表示权重"/>
							<button id="chart3-4" label="旭日图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Sunburst" screentip="圆形层级结构图，适合展示分层类别数据"/>
							<button id="chart3-5" label="地图" imageMso="MapContactAddress" onAction="show_map_dialog" screentip="在地图中显示数据"/>
							<button id="chart3-6" label="坐标图" imageMso="CreateMap" onAction="show_geo_dialog" screentip="在地图中显示坐标"/>
							<button id="chart3-7" label="坐标路线图" imageMso="CreateMap" onAction="show_geoline_dialog" screentip="在地图中显示两地之间的传送路径"/>
							<button id="chart3-8" label="3D曲面图" imageMso="CreateMap" onAction="display_formula" tag="zc_surface3d" screentip="3D曲面图"/>
							<button id="chart3-9" label="饼状柱状图" imageMso="CreateMap" onAction="show_pie_bar_dialog" screentip="饼图与柱状图相互转换"/>
							<button id="chart3-10" label="地图柱状图" imageMso="CreateMap" onAction="display_formula" tag="zc_map_bar" screentip="地图与柱状图相互转换"/>
							<button id="chart3-11" label="3D柱状图" imageMso="CreateMap" onAction="display_formula" tag="zc_bar_3d" screentip="3D柱状图"/>
							<button id="chart3-12" label="排名图" imageMso="CreateMap" onAction="show_rank_dialog" screentip="从数据中提取前N排名图"/>
						</menu>
					</menu>
				...
				<group id="quick" autoScale="false" centerVertically="false" label="快速">
					...
					<button id="quick-4" label="标准数字格式" imageMso="GroupNumber" onAction="standard_number_format" screentip="数字格式化" supertip="将选定区域的单元格数字格式设置为标准格式。" />
				...
				<group id="chart" autoScale="false" centerVertically="false" label="单图格式化">
					<button id="chart-1" label="图表格式化" imageMso="AutoFormatDialog" onAction="select_chart_stardard_format" screentip="选择图表标准格式" supertip="为当前活动图表设置标准格式，包括线条颜色和坐标轴字体样式。" />
					<button id="chart-2" label="加标注" imageMso="TagMarkComplete" onAction="select_chart_add_dash_callout" screentip="添加虚线标注" supertip="为当前活动图表添加一个虚线标注。" />
					<button id="chart-3" label="加虚框" imageMso="JotShapeRectangle" onAction="select_chart_add_dash_box" screentip="添加虚线框" supertip="为当前活动图表添加一个虚线框。" />
					<button id="chart-4" label="加横虚线" imageMso="ControlSpecialEffectChiseled" onAction="select_chart_add_horizon_line" screentip="添加水平线" supertip="为当前活动图表添加一条水平线。" />
					<button id="chart-5" label="加竖虚线" imageMso="VerticallyDistributed" onAction="select_chart_add_vertical_line" screentip="添加垂直线" supertip="为当前活动图表添加一条垂直线。" />
					<button id="chart-6" label="加箭头" imageMso="ShapeStraightConnectorArrow" onAction="select_chart_add_arrow" screentip="添加箭头" supertip="为当前活动图表添加一个箭头。" />
				</group>
				...
				<group id="env" autoScale="false" centerVertically="false" label="知心配置">
					...
					<menu id="helpmenu" size="large" label="帮助" imageMso="Help">
						...
						<button id="env-7" label="导入注册码" imageMso="AdpPrimaryKey" onAction="load_register_file" screentip="载入注册码zhixin.lic" supertip="联系zhixin_excel@163.com获得注册码文件zhixin.lic后，通过本按钮导入，之后点“关于知心”查看是否更新了注册信息" />
					</menu>
				</group>
			</tab>
		</tabs>
...
def _set_image_size(image, mode, width, height):
...
async def zt_Video(url, media_type="video", proxy=None):
    ...
    # 先检查%appdata%\xlOil\bin\下有无yt-dlp.exe ffmpeg.exe
    if not os.path.exists(
        os.path.join(os.environ["appdata"], "xlOil", "bin", "yt-dlp.exe")
    ):
        raise Exception(r"请下载yt-dlp.exe并放到%appdata%\xlOil\bin\yt-dlp.exe")
    if not os.path.exists(
        os.path.join(os.environ["appdata"], "xlOil", "bin", "ffmpeg.exe")
    ):
        raise Exception(r"请下载ffmpeg.exe并放到%appdata%\xlOil\bin\ffmpeg.exe")
...
async def zt_Mail(
    ...
    if not MAIL_SENDER:
        raise Exception(
            "点击知心配置-配置文件,填入发送地址。例如：MAIL_SENDER=zhixin_excel@163.com"
        )
    ...
    async for status in func_tools.mail(MAIL_SENDER,MAIL_PASSWORD, MAIL_SMTP_SERVER, subject, body, recipients, filenames):
        yield status
...
async def zt_LLM(
    ...
    示例
        =zLLM("用五岁小孩口吻告诉我量子力学")
        =zLLM("把{}翻译成英文", "量子力学")
        =zLLM("这个{}是邮箱地址则输出Y,否则输出N", "aa@bb.com")
    语法
...
@xlo.func(
    ...
    args={
        ...
        "system_content": "[可选 默认system_content] system角色定义",
...
def zt_GenPubKey(user_privkey: str) -> str:
    ...
    pub_key_path = os.path.join(os.environ["appdata"], "xlOil", "pubkey.txt")
    if os.path.exists(pub_key_path):
        return f"公钥已存在，请先手工删除{pub_key_path}"
...
@xlo.func(
    group="知心-工具",
    args={
        "url": "API 地址，例如 https://api.example.com/data",
        "method": "[可选 默认GET] HTTP 方法，如 GET、POST",
        "params": "[可选 默认空] 查询参数，JSON 格式",
        "headers": "[可选 默认空] 请求头，JSON 格式",
        "body": "[可选 默认空] 请求体，适用于 POST/PUT",
        "extract": "[可选 默认空] 指定要提取的JSON字段名",
    },
)
async def zt_API_CALL(
    url: str,
    method: str = "GET",
    params: str = "",
    headers: str = "",
    ...
    """调用 API 并返回 Excel 友好的表格数据
    示例:
        =zt_API_CALL("https://api.example.com/users")
        =zt_API_CALL("https://api.example.com/weather", "GET", '{"city":"Beijing"}')
        =zt_API_CALL("https://api.example.com/data", "POST", "", '{"Authorization":"Bearer abc"}', '{"key":"value"}')
        =zt_API_CALL("https://api.example.com/data", "GET", "", "", "", "c")  # 提取 "c" 字段
    """
    ...
    try:
        ...
        raise ValueError("参数、请求头或请求体必须是合法的 JSON 格式")

    from zhixinpy import func_tools

    async for status in func_tools.api_call(
        url, method, params_dict, headers_dict, body_dict, extract
    ):
        yield status
...
@xlo.func(
    group="知心-工具",
    args={
        "json_data": "JSON 数据，字符串格式",
        "jsonpath_expr": "JSONPath 表达式，例如 $.data[*].value",
    },
)
def zt_json_to_table(json_data: str, jsonpath_expr: str):
    """解析 JSON 并按 JSONPath 提取数据，返回 Excel 友好的表格
    示例:
        =zt_json_to_table('{"a": 1, "b": 2}', "$.a")  # 直接返回 1
        =zt_json_to_table('{"a": 1, "b": 2}', "$.*")  # 直接返回表 [1, 2]
        =zt_json_to_table('{"data": [{"x":1,"y":2}, {"x":3,"y":4}]}', "$.data[*]")  # 返回表格 [[1,2], [3,4]]
    """

    from zhixinpy import func_tools

    return func_tools.json2table(json_data, jsonpath_expr)
...
async def zt_Convert_Media(src: str, dest: str):
    """多媒体文件格式转化
    示例:
        =convert_media('input.m4a','output.mp3')
    备注:
    ...
    cmd = func_tools.convert_media_cmd(src=src, dest=dest)
    python_path = os.environ.get("PYTHONPATH", "").split(os.pathsep)[0]
...
@xlo.func(
    ...
    args={
        ...
        "connections": "【带表头的表格，选择区域】含源节点、目标节点、连接值、同比",
        "title": "桑基图标题，默认空",
    },
)
def zc_Sankey_income(nodes: xlo.Array(), connections: xlo.Array(), title=""):
    """专门用于可视化财报利润表
    示例
        =zc_Sankey_income(区域1, 区域2)
        参数1举例：
        节点名  节点类型
        北美     收入
        东亚     收入
    ...
    for i, row in nodes.iterrows():
        ...
        if (
            ...
            if row["nodeName"] not in middle_node_data:
                return f"错误：中间节点{row['nodeName']}没有数据"

    connections = connections[(connections["source"] != connections["destination"])]

    connections = connections[
        (connections["value"] > 0.05) | (connections["value"] < -0.05)
    ]
    connections = connections.merge(
        nodes, left_on="destination", right_on="nodeName", how="left"
    )
    connections = connections.merge(
        nodes[["nodeName", "rownum"]].rename(
            columns={"nodeName": "source", "rownum": "sourceRownum"}
        ),
        on="source",
        how="left",
    )
    connections["color"] = connections["colorindex"].apply(
        lambda x: CONNECTION_COLORS[x]
    )
    ...
    for i, row in nodes.iterrows():
        if (
            ...
            the_value = np.round(
                np.sum(connections[connections["source"] == row["nodeName"]]["value"]),
                2,
            )
            the_yoy = connections[connections["source"] == row["nodeName"]]["yoy"].iloc[
                0
            ]
            the_yoy_str = (
            ...
            nodes.at[i, "label"] = f"{row['nodeName']}<br>{the_value}亿({the_yoy_str})"
        elif (
            row["nodeName"] not in connections["source"].values
            and row["nodeName"] in connections["destination"].values
        ):  # 结束节点
            if (
                row["nodeName"] == "净利润"
            ):  # 净利润这个特殊处理，因为它有其他利润以及营业利润流转到它
                the_row = connections[
                    (connections["destination"] == "净利润")
                    & (connections["source"] == "营业利润")
                ].iloc[0]
            else:
                the_row = connections[
                    connections["destination"] == row["nodeName"]
                ].iloc[0]
            ...
            the_value = middle_node_data.get(row["nodeName"]).get("value")
            the_yoy = middle_node_data.get(row["nodeName"]).get("yoy")
            the_yoy_str = (
                f"<span style='color: green;'>{the_yoy:.2%}</span>"
                if the_yoy < 0
                else f"<span style='color: red;'>{the_yoy:.2%}</span>"
...
def zc_Line(data_frame: xlo.Array(), x: str, y: str, color: str = "", **kwargs):
    ...
    =zc_Line()
    示例：
        zc_Line(data_frame, query="字段名=='字段值'", kwargs={"title": "趋势图"}) -> 折线图至浏览器
        zc_Line(data_frame, kwargs={"line_style": "dashed", "x":"字段1", "y":"字段2", "date_columns": "字段1"}) -> 折线图片
    """
    if data_frame.shape[1] < 2:
        return "错误：必须带有表格名"
    data_frame = load_to_dataframe(data_frame)
...
def zc_Pie(data_frame: xlo.Array(), values: str, names: str = "", **kwargs):
    """
    =zc_Pie()
    简介
        饼图
...
def zc_WordCloud(
    data_frame: xlo.Array(), word_column: str = "词语", weight_column: str = "权重"
):
    """根据输入的表格数据生成词云图。
    =zc_WordCloud()
    示例
        zc_WordCloud(data_frame=data_frame, word_column="词语", weight_column="权重")
    """
    from zhixinpy import func_chart
    ...
    return func_chart.wordcloud(
        df=df, word_column=word_column, weight_column=weight_column
    )
...
def zc_Geo(
    ...
    """
    if str(maptype).lower() == 'china' or maptype == '中国':  # 中国  china China 都要变成china
        maptype = 'china'
    if str(maptype).lower() == 'world' or maptype in ('世界', '全世界', '全球'):  # 世界  world World 都要变成world
        maptype = 'world'
    from zhixinpy import func_chart
...
def zc_GeoLine(
    ...
    return func_chart.geoline(df, src_column, dest_column, weight_column, maptype, title)
...
def zc_Map(
    ...
    return func_chart.themap(df, city_column, weight_column, maptype, title)
...
def zc_bar_3d(
    ...
    x_list = data_frame[0][1:]  # 第一列作为x轴
    # 第二行开始,每行的第一个作为y的元素
    y_list = [data_frame[i][0] for i in range(1, len(data_frame))]
    # data为 第二行第二列开始 元素内容为 [(0,0, value), (0,1, value), (0,2, value), ...] vaue是None的话, 则为0
    data = [
        [i, j, data_frame[i + 1][j + 1] if data_frame[i + 1][j + 1] else 0]
        for i in range(len(data_frame) - 1)
        for j in range(len(data_frame[0]) - 1)
    ]
    return func_chart.bar_3d(list(x_list) , list(y_list) , data, title)
...
def zc_rank2(
    ...
    """根据输入的表格数据生成排名图。
    示例
        =zc_rank(data_frame=data_frame, date_column='年份'，name_column="公司名")
    """
    from zhixinpy import func_chart
    df = load_to_dataframe(data_frame)
    if df.empty:    
    ...
    return func_chart.rank2(df, date_columns, name_column, limit, title)
...
def zc_Network(
    ...
    return func_chart.draw_network(df, src_col, dest_col, value_col, category_col, src_size_col, dest_size_col)
...
class SinaQuoteRTD(xlo.RtdPublisher):
    ...
    def connect(self, num_subscribers):
        ...
        if self.done():
            ...
            async def run():
                await self._sinaQuote.subscribe(
    ...
    def disconnect(self, num_subscribers):
        if num_subscribers == 0:
            ...
            async def run():
                await self._sinaQuote.unsubscribe(
                    self.__symbol, self.__field, self.__update
                )
