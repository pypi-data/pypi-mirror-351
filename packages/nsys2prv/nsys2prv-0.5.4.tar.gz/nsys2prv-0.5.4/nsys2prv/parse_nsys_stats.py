...
def main():
    ...
    print("nsys2prv v0.5.4")

    class ShowVersion(argparse.Action):
        def __call__(self, parser, namespace, values, option_string):
            print("export SQLite schema version compatibility version 3.16.3")
    ...
    parser.add_argument("-t", "--trace", help="Comma separated names of events to translate: [mpi_event_trace, nvtx_pushpop_trace, nvtx_startend_trace, cuda_api_trace, gpu_metrics, openacc, nccl, graphs]")
    ...
    if int(minor_version["value"].iloc[0]) > 16 or int(minor_version["value"].iloc[0]) < 11:
        print(f"\033[93m Warning! The SQLite schema version {int(minor_version['value'].iloc[0])} is greater or lower than the one supported (11 < v =< 16). If unexpected behaviour occurs, please report it. \033[00m")
    ...
    if context_info["deviceId"].unique().size == 1 and context_info.shape[0] > 1:
        print(f"\033[93m Warning! Only one unique device ID can be detected in resource identification, but multiple processes.\nIf this is not intended, some features will not be available. Please, make sure that the GPU bindings are correctly done and that every process identifies its own GPU with a unique device [0 .. N-1].  \033[00m")
    ...
    if MULTIREPORT:
        ...
        if t_metrics:
            gpu_metrics_agg['Pid'] = gpu_metrics_agg['deviceId'].map(context_info.set_index("deviceId")["processId"])
    ...
    dfs_streams_to_concat = [kernels_df[['Device', 'Strm', 'deviceid', 'Pid', 'hostname']]]
    ...
    if t_metrics:
        ...
        aux_streams["thread"] = aux_streams["thread"] + aux_streams["Pid"].map(tasks_set.set_index('Pid')['thread']) + 1 # This probably does not work when multiple devices are ran by the same process
    ...
    if t_nvtx:
        ...
        if t_nccl:
            ...
            json_expanded = pd.json_normalize(nvtx_nccl_df["jsonText"]).reset_index(drop=True)
...
14    Gradient 14
    ...
    if t_nccl:
        ...
        for ti in tasks_set.index:
            ...
            def find_corr_id(row, df_B_task):
                ...
                # Return the first CorrID if matches are found, otherwise None
                return filtered_B["CorrID"].iloc[0] if not filtered_B.empty else None

            # Apply the function to each row of DataFrame A
            df_A_task = nvtx_nccl_df[(nvtx_nccl_df["task"] == ti) & nvtx_nccl_df["payload_valid"]]
            df_B_task = cuda_api_df[(cuda_api_df["Name"].str.contains("LaunchKernel")) & (cuda_api_df["task"] == ti)]

            df_A_task["CorrID"] = df_A_task.apply(find_corr_id, axis=1, df_B_task=df_B_task)
            partial_searches.append(df_A_task)

        nvtx_nccl_df = pd.concat(partial_searches, ignore_index=True)
        nccl_df = nccl_df.merge(nvtx_nccl_df[nccl_payload_event_dict["Name"].tolist() + ["CorrID"]], how="left", on="CorrID")
