...
def do_scenereview_list(collections,data):
    """
    场景 评测  提取关键词
    """
    info = {
...
def do_waterlevelline_autotest(collections, clientinfo, score_threshold):
    ...
    for i in collections:
        ...
        if "软件申请" in i['ext']['BPO标注-AP-分发技能']:
            keywprd = software_asset_sku_structure(i['ext']['BPO标注-AP-资产名称'])
        if "设备申请" in i['ext']['BPO标注-AP-分发技能']:
            keywprd = equipmentrequest_structure(i['ext']['BPO标注-AP-资产名称'])
