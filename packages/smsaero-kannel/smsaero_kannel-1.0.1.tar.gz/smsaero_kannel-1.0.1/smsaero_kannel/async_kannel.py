...
class AsyncKannel:
        ...
        **kwargs,
        ...
        try:
            ...
            async with session.get(send_url, params=request_params) as response:
                ...
                message_id = result.split(" ")[-1].strip("[]") if len(result.split(" ")) > 2 else None
    ...
    async def _sms_status(self, message_id: str) -> typing.Dict:
        """
        ...
        try:
            ...
            async with session.get(status_url, params=params) as response:
                if response.status != 200:
                    raise KannelConnectionError(f"Ошибка получения статуса: {await response.text()}")
