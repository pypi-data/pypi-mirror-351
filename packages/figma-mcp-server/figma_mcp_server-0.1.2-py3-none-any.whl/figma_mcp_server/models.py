...
class LayoutConstraints(BaseModel):
    """Represents layout constraints for a node."""
    horizontal: Optional[str] = None  # "left", "right", "left_right", "center", "scale"
    vertical: Optional[str] = None  # "top", "bottom", "top_bottom", "center", "scale"


class RelativePosition(BaseModel):
    """Represents relative position within parent container."""
    x: Optional[float] = None  # X offset from parent's origin
    y: Optional[float] = None  # Y offset from parent's origin
    relativeX: Optional[float] = None  # X position as percentage of parent width (0.0 - 1.0)
    relativeY: Optional[float] = None  # Y position as percentage of parent height (0.0 - 1.0)


class SimplifiedLayout(BaseModel):
    """Represents layout properties of a node."""
    mode: Optional[str] = None  # "none", "row", "column", "grid"
    ...
    # Size and positioning information for declarative UI
    width: Optional[Union[float, str, StyleReference]] = None  # Absolute width or "fill", "hug", etc.
    height: Optional[Union[float, str, StyleReference]] = None  # Absolute height or "fill", "hug", etc.
    minWidth: Optional[Union[float, StyleReference]] = None
    maxWidth: Optional[Union[float, StyleReference]] = None
    minHeight: Optional[Union[float, StyleReference]] = None
    maxHeight: Optional[Union[float, StyleReference]] = None
    
    # Layout behavior
    grow: Optional[float] = None  # layoutGrow equivalent (flex grow)
    shrink: Optional[float] = None  # layoutShrink equivalent (flex shrink)
    alignSelf: Optional[str] = None  # How this item aligns itself in parent
    
    # Positioning
    positioning: Optional[str] = None  # "absolute", "relative", "auto"
    constraints: Optional[LayoutConstraints] = None  # Layout constraints
    position: Optional[RelativePosition] = None  # Position relative to parent or absolute
...
class SimplifiedNode(BaseModel):
    ...
    # Position and size information for declarative UI
    absoluteBounds: Optional[BoundingBox] = None  # Absolute position and size
    relativeBounds: Optional[BoundingBox] = None  # Position relative to parent
    ...
    variantProperties: Optional[Dict[str, List[str]]] = None  # For component sets, available variant properties
