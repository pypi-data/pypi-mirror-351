...
def sanitize_name(name: str) -> str:
    ...
    # Remove control characters like \b (backspace)
    cleaned = ''.join(c for c in name if ord(c) >= 32 or c in '\t\n\r')
    
    # Replace spaces and special characters with underscores
    sanitized = "".join(c if c.isalnum() else "_" for c in cleaned)
...
def clean_figma_name(name: str) -> str:
    """
    Clean a Figma name for better readability in output.
    Removes control characters but preserves readable special characters.
    
    Args:
        name: The input name from Figma
        
    Returns:
        Cleaned name for display purposes
    """
    if not name:
        return ""
    
    # Remove control characters like \b (backspace) but keep readable special chars
    cleaned = ''.join(c for c in name if ord(c) >= 32 or c in '\t\n\r')
