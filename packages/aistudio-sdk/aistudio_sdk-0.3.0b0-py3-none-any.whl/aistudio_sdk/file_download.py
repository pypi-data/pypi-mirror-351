...
from .config import (
    ...
    API_FILE_DOWNLOAD_TIMEOUT, API_FILE_DOWNLOAD_CHUNK_SIZE, FILE_HASH)
...
def _repo_file_download(
    ...
    *,
    ...
    if repo_type == REPO_TYPE_MODEL:
        repo_file = get_git_info(repo_id, file_path, revision)
...
def get_git_info(repo_id, file_path, revision):
    """
    get meta
    """
    user_name, repo_name = repo_id.split('/')
...
def download_file(
    ...
    """
    headers = header_fill()
    if AISTUDIO_PARALLEL_DOWNLOAD_THRESHOLD_MB * 1000 * 1000 < file_meta[
...
def download_part_with_retry(params):
    ...
    while True:
        try:
            ...
            log.warn('Downloading: %s failed, reason: %s will retry' %
                           (model_file_path, e))
            retry.sleep()
