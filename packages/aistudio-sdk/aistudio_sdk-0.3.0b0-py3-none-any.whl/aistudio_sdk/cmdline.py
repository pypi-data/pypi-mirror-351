...
WORKER_FILE = os.path.expanduser(f'{cache_home}/.cache/aistudio/.auth/download_max_workers')
    ...
    if args.model:
        if len(args.files) == 1:  # download single file
            ...
            snapshot_download(
                ...
                revision=args.revision,
                local_dir=args.local_dir,
                allow_file_pattern=convert_patterns(args.include),
                ignore_file_pattern=convert_patterns(args.exclude),
                max_workers=args.max_workers)
    elif "token" in args:
