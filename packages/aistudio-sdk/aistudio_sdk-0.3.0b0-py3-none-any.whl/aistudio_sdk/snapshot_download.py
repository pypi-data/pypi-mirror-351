        ...
        if repo_type == REPO_TYPE_MODEL:
            if local_dir:
                ...
                directory = os.path.join(system_cache, 'models',
                                         *repo_id.split('/'))
            print(
                f'Downloading Model from remot to directory: {directory}')
            ...
            group_or_owner, name = repo_id.split('/')
            group_or_owner = group_or_owner.strip()
            name = name.strip()
            revision_detail = DEFAULT_DATASET_REVISION

            log.info('Fetching dataset repo file list...')
            repo_files = None # todo
...
def get_model_files(
                    model_id: str,
                    revision: Optional[str] = DEFAULT_MODEL_REVISION) -> (List[dict], dict):
    """List the models files.

    """
    endpoint = os.getenv("STUDIO_GIT_HOST", default=STUDIO_GIT_HOST_DEFAULT)
    if revision:
        path = '%s/api/v1/repos/%s/git/trees/%s' % (
            endpoint, model_id, revision)
    else:
        path = '%s/api/v1/repos/%s/git/trees/master' % (
            endpoint, model_id)
    ...
    files = []
    for file in d['tree']:
        if file['path'] == '.gitignore' or file['path'] == '.gitattributes':
            continue
