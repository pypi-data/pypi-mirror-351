...
def stats(org: str, edge_ids: str, run_id: str, ca_label: str, force: bool, verbose: bool):
    """edge rotate certs"""
    target_org_id = org
    ...
    logger.info(
        f"Starting Rotating Certs with ca_label: {ca_label} with force: {force} on Org: {target_org_id}, Edges: {edge_ids}"
    )
    logger.info("\n")
    ...
    if target_org_id.upper() != "ALL":
        try:
            ...
            if not org:
                ...
                return
            rotate_certs_on_all_edges(org, edge_ids, force, verbose)
        ...
        get_all_org_all_edge_stats(force, verbose)


def get_all_org_all_edge_stats(force: bool, verbose: bool):
    for org in org_service.details.items():
        rotate_certs_on_all_edges(org, None, force, verbose)


def rotate_certs_on_all_edges(org: dict, edge_ids: str, force: bool, verbose: bool):
    ...
    try:
        ...
        for edge in edges:
            ...
            if edge["providerLabel"].lower() not in allowed_providers:
                logger.warn(
                    f"Cert rotation is not supported on {edge.get('providerLabel')} provider edge {edge_id}, org: {target_org_id}"
                )
                continue
            """
