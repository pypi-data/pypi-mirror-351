...
def stats(org: str, edge_ids: str, run_id: str, verbose: bool):
    ...
    logger.info(f"Started gather edge migration stats:")
    logger.info("\n")
...
def get_all_template_images_and_vms_stats(org: dict, edge: dict, edge_stats: dict, verbose: bool):
    ...
    for template in admin.template.items(org_id=target_org_id, search="edgeDeploymentId $eq " + edge_id):
        if template.get("templateType") not in supported_template_types:
            logger.warn(
                f"Not supported template type {template.get('templateType')}, so template {template.get('id')} is skipped"
            )
            continue
        template_count += 1
            ...
            for agent in agents:
                if agent.get("name") == "HAI-Agent":
                    ...
                    if is_valid_agent_version(agent.get("agentVersion")):
                        is_valid_agent = True
                        logger.info(
                            f"{target_org_id}/{edge_id}/{template_id}: hai agent version {agent.get('agentVersion')} is VALID"
                        )
                    break
        else:
    ...
    for template_stats in all_template_stats:
        ...
        #    continue
        unique_image_id = (
            template_stats.get("stream_id")
            + " - "
            + str(template_stats.get("marker_id"))
            + " - "
            + str(template_stats.get("version_id"))
        )
    ...
    evaluate_edge_eligibility = False
    for template_stats in all_template_stats:
        if template_stats.get("is_vm_image_on_valid_agent_version") or template_stats.get(
            "all_vms_on_valid_agent_version"
        ):
            evaluate_edge_eligibility = True
            break

    if evaluate_edge_eligibility:
        edge_migration_eligible = True
        are_all_templates_vms_migrated = True
        for template_stats in all_template_stats:
            if (
                "is_using_custom_vm_image" in template_stats.keys()
                and template_stats.get("is_using_custom_vm_image") == False
                and "is_vm_image_on_valid_agent_version" in template_stats.keys()
                and template_stats.get("is_vm_image_on_valid_agent_version") == False
            ) or (
                "all_vms_on_valid_agent_version" in template_stats.keys()
                and template_stats.get("all_vms_on_valid_agent_version") == False
            ):
                edge_migration_eligible = False

            if "are_all_vms_migrated" in template_stats.keys() and template_stats.get("are_all_vms_migrated") == False:
...
def get_edge_stats(edge: dict, verbose: bool):
    ...
    edge_stats["regional_mqtt_url"] = data_util.deep_get_attr(
        edge, "privateEndpointDetails.dnsRecord", raise_on_not_found=False
    )
    ...
    if (
        (
            edge_fqdn_auto_generated == False
            or (edge_fqdn_auto_generated == True and edge_mqtt_url and ".omnissa." in edge_mqtt_url)
        )
...
def get_template_vms_stats(target_org_id: str, edge_id: str, template: dict, template_stats: dict, verbose: bool):
    ...
    vm_count = 0
    while True:
        raw_list = inventory.vm.raw_list(
            template["id"], org_id=target_org_id, vm_ids=None, size=BATCH_SIZE, page=page_num
        )
        page_num += 1
        ...
        for vm in vms:
            ...
            if (
                ...
                and (
                    (
                        "edge_fqdn_auto_generated" in template_stats
                        and template_stats["edge_fqdn_auto_generated"] == False
                    )
