...
def login(
    ...
    **kwargs,
    ...
    if api_token:
        ...
        sglib.auth.use_oauth_token(new_token)
    else:
        # auto detect mode.
        pass

    interactive = (
        not csp.apiToken and not csp.clientId and not csp.basic and not csp.accessToken and not csp.refreshToken
    )
    ...
    if interactive and not login_support.identify_client_id(csp.url):
        return ctxp.error(
            "The interactive login on the specified stack is not yet available. Try a different authentication method."
        )

    # If there's existing login info and the org ID is different, logout first
    existing_auth_data = ctxp.profile.auth.get()
    if existing_auth_data:
        ret = sglib.auth.details_from_token(existing_auth_data.token)
        if csp.orgId and ret["org"].id != csp.orgId:
            if not interactive:
                _echo("Switching to organization " + csp.orgId)
            ctxp.profile.auth.delete()

    oauth_token = sglib.auth.login(force_refresh=refresh, panic_on_failure=False)
    if not oauth_token:
        if interactive:
...
def _validate_auth_method(org: str, api_token: str, client_id: str, client_secret: str, basic: str, browser: bool):
    ...
    if basic:
        ...
        client_id, client_secret = sglib.auth._decode_http_basic_auth_token(basic)
