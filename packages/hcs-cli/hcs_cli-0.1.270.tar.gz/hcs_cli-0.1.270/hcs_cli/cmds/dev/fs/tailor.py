...
_service_dependency = {
    ...
    "clouddriver": ["pki"],
    "lcm": ["inventory", "credentials", "clouddriver"],
    "admin": ["lcm"],
    "portal": ["admin"],
}
...
_scenario_dependency_map = {
    "lcm-framework": ["lcm", "inv-status-sync", "smart-capacity-management", "scheduler-control-service"]
}
...
def tailor(for_scenario: str, **kwargs):
    ...
    temp_dependencies = set(_core_infra + _ring0)
    open_dependencies = list(dependencies)
    while open_dependencies:
        service = open_dependencies.pop()
        if service in temp_dependencies:
            continue
        temp_dependencies.add(service)
        new_dependencies = _service_dependency.get(service, [])
        for dep in new_dependencies:
            if dep in temp_dependencies:
                continue
            open_dependencies.append(dep)
            temp_dependencies.add(dep)
    ...
    click.confirm(f"Remove unnecessary pods?", abort=True)
