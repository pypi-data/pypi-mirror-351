...
def _client():
    global client
    if client is None:
        client = sglib.hcs_client(profile.current().hcs.url)
...
def _delete_impl(type: str, fn_filter: callable = None):
    ctx = f"/synthetic-testing/v1/{type}"
    deleted = 0
    while True:
        page = _client().get(ctx + "?size=200")
        progress = 0
