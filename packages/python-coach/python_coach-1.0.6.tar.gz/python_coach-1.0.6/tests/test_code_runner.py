from code_runner.runner import CodeRunner
...
def solution(n: int) -> str:
    ...
    """
    result = CodeRunner().run_code(_runner_cases, code, "solution")
    assert result is not None
    assert len(list(result._all_cases)) > 0
...
def solution(n: int) -> str:
    ...
    result = code_runner.run_code(_runner_cases, code, "solution")
    assert result is not None
