...
def is_identical_type(
    ...
    """
    Checks whether two type annotations are semantically identical.

    Supports nested types including:
    - Callable internals represented as lists
    - Union types (order-insensitive comparison)
    - Optional types via NoneType

    :param a: first type annotation to compare
    :param b: second type annotation to compare
    :param strict_optional: if false, treat Optional[T] and T as identical
    :param covariant: if true, allow subclassing in one direction
    :param contravariant: if true, allow subclassing in the other direction
