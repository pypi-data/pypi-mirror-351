    ...
    while True:
        ...
        if args[0] == "exit" or args[0] == "quit": 
            ...
            if query:
                ...
                # Search for models using the manager
                filtered_models = ollama_manager.search_models(query=query, limit=20)
                ...
                if not filtered_models:
                    ...
                    try:
                        ...
                        if hf_models:
                            ...
                            # Create choices for Hugging Face models
                            choices = []
                            for m in hf_models:
                                model_id = m.get('id', m.get('name', ''))
                                model_desc = m.get('description', m.get('desc', ''))
                                ...
                                choices.append(questionary.Choice(
                                    title=f"{model_id:<30} {model_size:<10} [HuggingFace] {model_desc}",
                                    value=model_id
                                ))
