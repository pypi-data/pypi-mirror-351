...
def choose_model(action_desc, callback):
    ...
    # Create choices from predefined models
    choices = [
        questionary.Choice(
    ...
    # Add custom installed models that aren't in the predefined list
    custom_choices = []
    for m in installed_models:
        model_name = m.get('name', '')
        if model_name and model_name not in predefined_model_names:
            # For custom models, use the size from Ollama and add a note that it's a custom model
            size = m.get('size', '')
            custom_choices.append(
                questionary.Choice(
                    title=f"{model_name:<25} {size}  [Custom installed model]",
                    value=model_name
                )
            )
    
    # If we have custom models, add a separator and then the custom models
    if custom_choices:
        choices.append(questionary.Separator("--- Custom Installed Models ---"))
