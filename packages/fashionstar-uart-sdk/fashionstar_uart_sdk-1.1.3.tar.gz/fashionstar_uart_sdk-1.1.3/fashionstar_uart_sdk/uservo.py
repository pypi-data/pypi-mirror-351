...
class UartServoManager:
	...
	def response_query_servo_monitor(self, param_bytes):
		...
		angle /= 10.0
		temp =1000 
		# float(1/((math.log(10000*(temp_raw)/(10000*(4096-(temp_raw))))/3435)+1/(273.15+25))-273.15)
		if servo_id not in self.servos:
			return
        # 更新
	...
	def update(self, is_empty_buffer=False, wait_response=False, timeout=0.004):
		'''舵机管理器的定时器回调函数'''
	...
	def monitor_update(self, is_empty_buffer=False, timeout=0.003):
		'''舵机管理器的定时器回调函数'''
		t_start = time.time() # 获取开始时间
		buffer = bytearray()
		start_flag = 0
			...
			if bytes_waiting:
				if ok_flag == 0:
					...
					if bytes_waiting >= 21:
						...
						if len(buffer) >=147:
							break
						# hex_str = ' '.join(f"0x{byte:02X}" for byte in buffer)
						# print(hex_str)			
				if (time.time() - t_start) > timeout:
					print("记录",t_start,"当前",time.time())
					break	
