...
@tool
def get_native_balance(address: str, use_wei: bool = True, decimal_places: int = None) -> str:
    """
    ...
    if not use_wei:
        ...
        if decimal_places is not None:
            balance_decimal = balance_decimal.quantize(Decimal('0.' + '0' * decimal_places))
...
@tool
def transfer_native_token(state: Annotated[dict, InjectedState], to: str, amount: float) -> str:
    """
...
@tool
def transfer_erc20_token(state: Annotated[dict, InjectedState], to: str, amount: float, token_symbol_or_address: str) -> str:
    """
    ...
    try:
        ...
        token_contract = w3.eth.contract(address=token_info["address"], abi=chain_info[erc20Abi_string])
...
def wrap_token(state: Annotated[dict, InjectedState], amount: float) -> str:
    ...
    try:
        ...
        contract = w3.eth.contract(address=weth_address, abi=chain_info[wrapperAbi_string])
        amount_in_wei = w3.to_wei(amount, 'ether')
...
@tool
def swap_token(state: Annotated[dict, InjectedState], from_token_symbol_or_address: str, to_token_symbol_or_address: str, amountIn: float) -> str:
    """
    ...
    try:
        ...
        try:
            router = w3.eth.contract(address=router_address, abi=chain_info[routerAbi_string])
            from_token_contract = w3.eth.contract(address=from_token["address"], abi=chain_info[erc20Abi_string])
        except Exception as e:
        ...
        try:
            ...
            token_balance = from_token_contract.functions.balanceOf(account.address).call()
            if token_balance < amount_in_wei:
                return f"Insufficient token balance. Have: {token_balance}, Need: {amount_in_wei}"

            current_allowance = from_token_contract.functions.allowance(account.address, router_address).call()
        except Exception as e:
