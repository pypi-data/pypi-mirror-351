...
from .utils import median_wh, rand_ortho1
...
class SugiyamaLayout(object):
    ...
    def rank_all(self, roots, optimize=False):
        self._edge_inverter()
        r = list(
            filter(lambda x: len(list(x.e_in())) == 0 and x not in roots, self.g.sV)
        )
        ...
        for e in self.g.E():
            if hasattr(e, "view"):
                ...
                if self.ctrls["cons"] and r0 > r1:
                    dy = e.v[0].view.h / 2.0 + self.yspace / 3.0
                    x, y = zip(l[0], l[1])
                    y = max(y) + dy
                    l.insert(1, (x[0], y))
                    l.insert(2, (x[1], y))
                    dy = e.v[1].view.h / 2.0 + self.yspace / 3.0
                    x, y = zip(l[-1], l[-2])
                    y = min(y) - dy
                    l.insert(-1, (x[0], y))
                    l.insert(-2, (x[1], y))
