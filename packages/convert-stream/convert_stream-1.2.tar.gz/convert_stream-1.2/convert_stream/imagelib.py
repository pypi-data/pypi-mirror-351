...
class ImplementInvertColorOpenCv(ABCInvertColor):
    ...
    def _image_opencv_tobytes(self, img:MatLike) -> bytes:
        ...
        _, buffer = cv2.imencode('.png', img)  # Codifica como PNG (ou use '.jpg' para JPEG)
        return buffer.tobytes()  # Obtém os bytes da imagem
        
    def set_gaussian_blur(self):
        # Aplica um filtro Gaussiano para reduzir o ruído
        _blurred:MatLike = cv2.GaussianBlur(self._image_bytes_to_opencv(self.image_bytes), (3, 3), 0.5)
        self.image_bytes = self._image_opencv_tobytes(_blurred)

    def __set_black(self):
...
class ImageInverColor(ABCInvertColor):
    ...
    @classmethod
    def create_from_file(cls, f:File, *, library:LibraryImage=LibraryImage.OPENCV) -> 'ImageInverColor':
    ...
    @classmethod
    def create_from_bytes(cls, bt:bytes, *, library:LibraryImage=LibraryImage.OPENCV) -> 'ImageInverColor':
        if library == LibraryImage.OPENCV:
...
class ImageObject(ABCImageObject):
    """
    ...
    @classmethod
    def create_from_bytes(cls, image_bytes: bytes, *, library:LibraryImage=LibraryImage.OPENCV) -> 'ImageObject':
    ...
    @classmethod
    def create_from_file(cls, filepath: File, *, library:LibraryImage=LibraryImage.OPENCV) -> 'ImageObject':
        print(f'Obtendo imagem do arquivo: {filepath.basename()}')
        try:
            if library == LibraryImage.PIL:
