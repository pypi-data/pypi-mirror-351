...
class CollectionMongoDB:
    ...
    def __init__(self, shareddata, database, period, source, tablename,
        ...
        """
        '''
        Initialize a collection handler for a MongoDB database with specified parameters.
        
        Parameters:
            shareddata (object): An object containing shared resources, including MongoDB client.
            database (str): Name of the database.
            period (str): Time period identifier for the data.
            source (str): Source identifier for the data.
            tablename (str): Name of the collection/table.
            records (optional): Initial records to populate the collection (default is None).
            names (list, optional): List of field names for the collection schema (default is None).
            formats (list, optional): List of data formats corresponding to the field names (default is None).
            size (int, optional): Size parameter affecting indexing behavior (default is None).
            hasindex (bool, optional): Whether to create and maintain indexes on the collection (default is True).
            create_if_not_exists (bool, optional): Whether to create the collection if it does not exist (default is True).
            overwrite (bool, optional): Whether to overwrite existing data (default is False).
            user (str, optional): User or client namespace for MongoDB access (default is 'master').
            tabletype (int, optional): Type of table storage: 1 for DISK,
        """
        self.type = tabletype
    ...
    def flatten_dict(self, d, parent_key='', sep='->'):
        ...
        Parameters:
            ...
            sep (str): The separator string used between concatenated keys.
    ...
    def sort_dict(obj: object) -> object:
        """
        Recursively sort the input object if it is a dictionary or list.
        
        For dictionaries, returns a new dictionary with keys sorted in ascending order,
        and applies the sorting recursively to all nested dictionaries or lists.
        For lists, applies the sorting recursively to each element.
        For other data types, returns the object unchanged.
        
        This ensures consistent ordering of dictionary keys, which is useful for hashing
        or comparing nested data structures.
        
        Args:
            obj (object): The input object to sort, which can be a dictionary, list, or other data type.
        
        Returns:
            object: A new object with dictionaries sorted by keys recursively.
        """
        if isinstance(obj, dict):
    ...
    def get_hash(obj: dict) -> str:
        ...
        This method first sorts the dictionary using CollectionMongoDB.sort_dict to ensure consistent ordering,
        then encodes it into BSON format, and finally computes the SHA-256 hash of the encoded bytes.
        
        Args:
            obj (dict): The dictionary to be hashed.
