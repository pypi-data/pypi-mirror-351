...
@app.route('/api/table/<database>/<period>/<source>/<tablename>', methods=['GET', 'POST','DELETE'])
@swag_from(docspath)
...
@app.route('/api/collection/<database>/<period>/<source>/<tablename>', methods=['GET', 'POST', 'PATCH', 'DELETE'])
@swag_from(docspath)
...
def rss():
    """
    Handle GET requests to the '/api/rss' endpoint to retrieve RSS feed entries from a MongoDB collection.
    
    This endpoint supports optional query parameters to filter the RSS feed entries:
    - startdate: Return entries published on or after this date (ISO format expected).
    - news_text: Case-insensitive substring match against the title of the entries.
    
    Authentication is required; unauthorized requests receive a 401 response.
    
    Returns a JSON response containing up to 20 RSS feed entries sorted by publication time in descending order.
    
    In case of errors, returns a JSON response with an error message and HTTP status 500.
    """
    try:
...
def news():
    """
    Handle GET requests to the '/api/news' endpoint to retrieve news articles from the MongoDB collection.
    
    Query Parameters:
    - startdate (optional): Filter news articles published on or after this date (ISO format).
    - news_text (optional): Case-insensitive substring to search for in the news title.
    
    Authentication:
    - Requires successful authentication via the `authenticate` function; returns 401 Unauthorized if authentication fails.
    
    Behavior:
    - Queries the 'news' collection in MongoDB using the provided filters.
    - Returns up to 20 news articles sorted by publication time in descending order.
    - Responds with a JSON array of news articles on success.
    - Returns a JSON error message with status 500 if an exception occurs during processing.
    """
    try:
