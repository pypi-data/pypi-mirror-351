...
def compare_routines(routine1,routine2):
    """
    Compares two routines by hashing them and checking if their hashes are equal.
    ...
    Returns:
        bool: True if both routines have the same hash (are considered equal), False otherwise.
    """
...
def hash_routine(routine):
    """
    Generate a hash string representation of a routine dictionary.
    
    The function constructs a string by concatenating the values of specific keys
    from the input dictionary `routine` in the following order and format:
    ...
    Parameters:
        routine (dict): A dictionary that may contain the keys 'repo', 'branch',
                        'routine', and 'args'.
...
def validate_command(command):
    """
    Cleans and validates the input 'command' dictionary by sanitizing its contents.
    
    - Removes the 'args' key if its value is empty, the string 'nan' (case-insensitive), or a NaN float.
    - Converts 'args' to a string if it is neither a string nor a dictionary.
...
def process_command(command,routines,batch_jobs):
    ...
    This function manages different job types including batch jobs, commands, installations, routines,
...
def load_last_hash(venv_root: Path) -> str | None:
    """
    Load the stored requirements hash from the .last_requirements_hash file in the given virtual environment root directory.
    
    Attempts to read the file as a JSON object and extract the 'req_hash' key. If the file is missing, returns None.
    If the file contents are not valid JSON (e.g., legacy or corrupted), falls back to returning the raw file content as a stripped string.
...
def run_routine(command, routine, quiet=False):
    """
    Executes a specified routine from a given repository command, handling environment setup, repository installation, and subprocess management.
    
    Parameters:
        command (dict): A dictionary containing details about the routine to run, including:
            - 'repo' (str): The repository name.
            - 'routine' (str): The routine or script to execute.
            - Optional 'args' (str or dict): Arguments to pass to the routine.
        routine (dict): A dictionary to store subprocess and process information, and optionally containing a 'hash' and 'date' for status tracking.
    ...
    Behavior:
        ...
        - Encodes arguments if provided as a dictionary using BSON and base64.
        ...
        - Updates the routine dictionary with subprocess and threading information.
...
def remove_finished_routines(routines):
    ...
    Each routine is expected to be a dictionary that may contain:
    - a 'process' key with a psutil.Process object,
    - a 'subprocess' key with a subprocess.Popen object,
    - a 'thread' key with a threading.Thread object,
    - and optionally 'stderr' and 'command' information for logging errors.
    
    The function removes routines whose associated process or thread is no longer running.
    If a subprocess has exited with a non-zero exit code, an error is logged including any captured standard error output.
...
def remove_finished_batch_jobs(batch_jobs):
    ...
    """
    Removes finished or errored batch jobs from the given list of batch job routines.
    
    Each routine is checked for its completion status based on subprocess exit codes or thread aliveness.
    For completed or errored jobs, a status message is posted to a remote collection via ClientAPI.
    The function returns a filtered list excluding finished or errored routines, along with counts of finished and errored jobs.
    
    Parameters:
        batch_jobs (list of dict): A list of batch job routines, where each routine may contain keys such as 'process', 'subprocess', 'thread', 'date', 'hash', 'stderr', and 'start_time'.
    
    Returns:
        tuple:
            - list of dict: Filtered list of batch job routines excluding finished or errored ones.
            - int: Number of finished batch jobs.
            - int: Number of batch jobs that ended with errors.
    """
...
def start_server(port, nproc, nthreads):
    # run API server
    """
    Starts an API server by constructing and executing a command with specified port and thread count.
...
def start_logger():
    # run logger
    """
    Initializes and starts a logging routine by constructing a command dictionary with predefined parameters,
    including the sender, target computer (retrieved from environment variables), job type, repository, and routine name.
    It records the start time, creates a routine dictionary to hold the command and process/thread information,
    and then executes the routine using the run_routine function.
    """
    command = {
        "sender": "MASTER",
        "target": os.environ['USER_COMPUTER'],
...
def isrunning(command,routines):
    """
    Check if a given command is currently running within a list of routines.
