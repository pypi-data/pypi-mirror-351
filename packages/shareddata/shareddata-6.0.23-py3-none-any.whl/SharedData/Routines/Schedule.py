...
class Schedule:
    ...
    Methods:
        __init__(schedule_name, kinesis=False):
        ...
        run():
            Sends commands to start pending
    """
    def __init__(self, schedule_name, kinesis=False):
        """
        Initializes the object with a schedule name and optional Kinesis flag.
        
        Parameters:
            schedule_name (str): The name of the schedule to be used.
            kinesis (bool, optional): Flag indicating whether to use Kinesis. Defaults to False.
    ...
    def update(self):
        """
        Updates the object's logs, batch status, and realtime status, then combines the realtime and batch data into a single schedule DataFrame with a reset index.
        """
        self.UpdateLogs()
    ...
    def UpdateLogs(self):
        """
        Updates the schedule DataFrame with the latest log timestamps and message ages from various Logger log DataFrames.
        
        This method performs the following updates for each routine in the schedule:
        - Retrieves the most recent run, started, error, completed, last, and heartbeat log entries from the corresponding Logger DataFrames.
        - Filters log entries to only include those occurring at or after the scheduled runtimes.
        ...
        - Fills missing age values with a default large value (1000.0) to indicate absence of recent messages.
        
        The schedule DataFrame is expected to have a 'routine' column used as an index for matching log entries.
        
        After processing, the updated schedule DataFrame replaces the existing one in the instance.
        """
        Logger.getLogs()
    ...
    def UpdateRealtimeStatus(self):       
        """
        Update the real-time status of scheduled routines based on their message ages, run times, and dependencies.
        
        This method processes the `self.schedule` DataFrame to determine the current status of routines marked as real-time (`isrealtime == True`). It uses a state machine approach to assign statuses such as 'RUNNING', 'STARTED', 'STARTING', 'RESTART', and 'WAITING DEPENDENCIES' based on the age of heartbeat, start, and run messages, as well as runtime schedules and dependency statuses. External routines have their status appended with ' EXTERNAL'.
        
        The resulting status updates are stored in `self.realtime`.
        """
        local_tz = pytz.timezone(str(get_localzone()))
        now = datetime.now().astimezone(tz=local_tz) 
    ...
    def run(self):
        """
        Processes and dispatches pending or restarting scheduled routines by constructing and sending appropriate command messages.
        
        For each routine in the schedule with status 'PENDING', 'START', or 'RESTART', this method prepares a command containing details such as target computer, repository, routine script, branch (if any), and arguments (if provided). It then sends the command via client api to the 'command' channel, updates the routine's status to 'STARTING', logs the dispatch time and message, and records the event in the logger.
        
        Returns the updated schedule DataFrame with modified statuses and timestamps.
        """
        newcommand = False
        sched = self.schedule
    ...
    def save(self):
        ...
        Compares the current schedule with the last saved schedule, and if they differ,
        updates the last saved schedule, creates a Metadata object for today's date under
        the schedule's directory, and stores a copy of the schedule in it. It also processes
        the 'runtimes' column by removing timezone information from datetime entries before saving.
        
        Assumes:
        - `self.schedule` and `self.lastschedule` are pandas DataFrames.
        - `self.schedule_name` is a string representing the schedule's name.
        - `Metadata` is a class that handles saving metadata to a specified path.
        - The 'runtimes' column contains datetime objects or strings that can be compared to 'nan'.
        """
