...
class CaptureCollector(object):
    ...
    def CaptureVariableInternal(self, value, depth, limits, expressionsCount=0, can_enqueue=True):
        ...
        if isinstance(value, _PRIMITIVE_TYPES):
            ...
            if value_len <= expression_limit and value_len >= min(max_value_len, remaining_buffer_size):
                ...
                raise ValueError(messages.ERROR_EXPRESSION_IS_BIGGER_THAN_MAX_OBJECT_SIZE)
                ...
                return {
                    "status": {
                        "isError": True,
                        "refersTo": "VARIABLE_VALUE",
                        "description": {"format": (messages.ERROR_EXPRESSION_IS_BIGGER_THAN_MAX_WATCH_EXPRESSION_SIZE), "parameters": [value_len]},
                    }
                }
