...
from .filter import FilterGroup, FilterGroupType, Filter
...
class OhmClient:
  ...
  def __build_where_clause(self, filters: FilterGroup | List[Filter]) -> str:
    """Build a WHERE clause from a FilterGroup.

    Args:
        filters: Either a FilterGroup object or a List of Filter objects. If a list is provided,
                it will be treated as a FilterGroup with AND logic.
    ...
    if isinstance(filters, list):
      filters = FilterGroup(type=FilterGroupType.AND, filters=filters)
    ...
    # Process all items in filters.filters, which can be Filter or FilterGroup objects
    for item in filters.filters:
      if isinstance(item, Filter):
        ...
        # Recursively process nested FilterGroup
    ...
    if conditions:
      join_str = ' AND ' if filters.type == FilterGroupType.AND else ' OR '
      return f'WHERE {join_str.join(conditions)}'
  ...
  def __get_data(
    ...
    filters: Optional[FilterGroup | List[Filter]] = None,
  ) -> pd.DataFrame:
    """Fetch data from the database."""
    columns_sql = self.__build_columns_sql(columns)
    where_clause_sql = self.__build_where_clause(filters)

    final_sql = f'SELECT {columns_sql} FROM "{table_name}" {where_clause_sql}'
  ...
  def get_dataset_cycle_data(
    self, columns: Optional[List[str]] = None, filters: Optional[FilterGroup] = None
  ) -> pd.DataFrame:
    """
