...
class GenericLLMDetector(LlmBaseDetector):
    ...
    async def detect(self, query_string: str) -> DetectionResult:
        ...
        llm_response = litellm_completion(model=self.model_name, messages=[{"role": "system", "content": llm_prompt}])
...
class LlmModerations(LlmBaseDetector):
    ...
    async def detect(self, query_string: str) -> DetectionResult:
        ...
        llm_response = litellm_completion(model=self.model_name, messages=[{"role": "system", "content": content}])
        if not llm_response.choices or len(llm_response.choices) == 0:
