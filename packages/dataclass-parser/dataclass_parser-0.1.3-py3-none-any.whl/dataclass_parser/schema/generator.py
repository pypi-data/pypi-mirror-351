...
class SchemaGenerator:
    __slots__ = ()
    TYPE_MAPPING: ClassVar[dict[type[Any], type[SimpleFieldParser]]] = TYPE_MAPPING_BASE
    MAX_RECURSION_DEPTH: ClassVar[int] = 3
    _current_recursion_depth: ClassVar[dict[type[Any], int]] = {}

    @classmethod
    def _resolve_forward_refs(cls, type_hint: type[Any], module: str) -> type[Any]:
    ...
    @classmethod
    def _resolve_type(cls, type_hint: type[Any], module: str) -> tuple[type[Any], tuple[type[Any], ...]]:
        resolved_type = cls._resolve_forward_refs(type_hint, module)
        base_type = get_origin(resolved_type) or resolved_type
        return base_type, cls.filter_args(get_args(type_hint))

    @classmethod
    def _get_parser_cls(cls, type_hint: type[Any], module: str) -> SimpleFieldParser:
        base_type, type_args = cls._resolve_type(type_hint, module)

        if base_type is Union or base_type is UnionType:
            return UnionFieldParser(
                type_hint,
                [cls._get_parser_cls(type_, module) for type_ in type_args],
            )
        ...
        if is_dataclass(base_type) and hasattr(base_type, 'set_schema'):
            ...
            if current_depth >= cls.MAX_RECURSION_DEPTH:
                return cls._get_ellipsis_parser(type_hint)

            cls._current_recursion_depth[base_type] = current_depth + 1

            base_type.set_schema(cls.generate_schema(base_type))
            return EntityFieldParser(base_type, None)

        for field_type in cls.TYPE_MAPPING:
            if issubclass(base_type, field_type):
                return cls.TYPE_MAPPING[field_type](
                    base_type,
                    [cls._get_parser_cls(type_, module) for type_ in type_args] or None,
                )
        msg = f"Can't find field parser for type {base_type!s}"
        raise ValueError(msg)

    @classmethod
    def _get_ellipsis_parser(cls, type_hint: type[Any]) -> SimpleFieldParser:
    ...
    @classmethod
    def generate_schema(cls, entity: type[EntityBase]) -> dict[str, SimpleFieldParser]:
        schema: dict[str, SimpleFieldParser] = {}
        for field in fields(entity):
            parser = cls._get_parser_cls(field.type, entity.__module__)
            schema[field.name] = parser
        return schema

    @classmethod
    def filter_args(cls, args: tuple[type[Any], ...]) -> tuple[type[Any], ...]:
