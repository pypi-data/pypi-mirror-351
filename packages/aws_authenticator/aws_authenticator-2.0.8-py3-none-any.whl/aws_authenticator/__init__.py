...
class AWSAuthenticator:
    ...
    def profile(self):
        ...
        try:
            if self._profile_name is None and self._region_name is None:
                session = boto3.Session()
            elif self._profile_name is not None and self._region_name is None:
                session = boto3.Session(
                    profile_name=self._profile_name
                )
            elif self._profile_name is None and self._region_name is not None:
                session = boto3.Session(
            ...
            raise Exception(f"AWS profile login: {str(e)}") from e

    def iam(self):
        """Login with IAM access key credentials."""
        try:
            if self._session_token is None and self._region_name is None:
                session = boto3.Session(
                    aws_access_key_id=self._access_key_id,
                    aws_secret_access_key=self._secret_access_key
                )
            elif self._session_token is None and self._region_name is not None:
                session = boto3.Session(
                    aws_access_key_id=self._access_key_id,
                    aws_secret_access_key=self._secret_access_key,
                    region_name=self._region_name
                )
            elif self._session_token is not None and self._region_name is None:
                session = boto3.Session(
                    aws_access_key_id=self._access_key_id,
                    aws_secret_access_key=self._secret_access_key,
            ...
            raise Exception(f"AWS IAM login: {str(e)}") from e
    ...
    def sso(self):
        ...
        try:
            ...
            raise Exception(f"AWS SSO login: {str(e)}") from e
...
def get_params():
    ...
    myparser.add_argument(
        "-v", "--version", action="version", version="%(prog)s 2.0.8"
    )
    myparser.add_argument(
        "-m",
        "--auth_method",
