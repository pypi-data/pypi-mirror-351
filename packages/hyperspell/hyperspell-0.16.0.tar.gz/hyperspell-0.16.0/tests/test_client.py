...
from hyperspell._base_client import (
    DEFAULT_TIMEOUT,
...
class TestHyperspell:
            ...
            for frame in diff.traceback:
                if any(
                    ...
                    for fragment in [
                        ...
                        "hyperspell/_legacy_response.py",
                        "hyperspell/_response.py",
                        # pydantic.BaseModel.model_dump || pydantic.BaseModel.dict leak memory for some reason.
                        "hyperspell/_compat.py",
    ...
    def test_base_url_trailing_slash(self, client: Hyperspell) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_base_url_no_trailing_slash(self, client: Hyperspell) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_absolute_request_url(self, client: Hyperspell) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
    ...
    def test_omit_retry_count_header(
        self, client: Hyperspell, failures_before_success: int, respx_mock: MockRouter
    ) -> None:
        client = client.with_options(max_retries=4)
