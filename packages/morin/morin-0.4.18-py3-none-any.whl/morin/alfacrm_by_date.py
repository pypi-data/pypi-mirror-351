...
class ALFAbyDate:
    ...
    def get_group(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/group/index',{'created_at_to': self.common.flip_date(date)},{'created_at_from': self.common.flip_date(date)},{'removed': 1})
    ...
    def get_regular_lesson(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/regular-lesson/index', {'date_to': date},{'date_from': date})
    ...
    def get_task(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/task/index', {'due_date_from': date},{'due_date_to': date})
    ...
    def get_lesson(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/lesson/index', {'date_to': date},
    ...
    def update_customer(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/customer/index', {'updated_at_to': self.common.flip_date(date)},{'updated_at_from': self.common.flip_date(date)},{'removed': 1},{'is_study': 2})
    ...
    def update_group(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/group/index', {'updated_at_to': self.common.flip_date(date)},{'updated_at_from': self.common.flip_date(date)},{'removed': 1})
    ...
    def update_lesson(self, date):
        try:
            ...
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/lesson/index',
                                               {'updated_at_to': self.common.flip_date(date)},
