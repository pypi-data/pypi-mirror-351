    ...
    if non_compliant_count > 0:
        console.print(
            Panel(
                ...
                f"This program creates filenames from the deflines of the provided FASTA "
                f"file. Found [bold]{non_compliant_count}[/bold] non-compliant and "
                f"[bold]{compliant_count}[/bold] compliant sequence headers.\n\n"
                f"Use [bold]--rename-sequences[/bold] to allow automatic renaming (a "
                f"[dim]_filename_map.tsv[/dim] file will be created to track the mapping), "
                f"or update your FASTA headers to use only letters, numbers, hyphens, "
                f"underscores, and periods.\n\n"
                f"[bold]Autorenaming example:[/bold]\n"
                f"• Non-compliant: [dim]{first_non_compliant}[/dim]\n"
                f"• Would become:  [dim]{first_non_compliant_safe}[/dim]",
...
def run(
    ...
    rename_sequences: bool = typer.Option(  # noqa: B008
        False,
        "--rename-sequences",
        "-r",
        help="Rename sequences if their FASTA headers contain non-compliant characters.",
    ),
