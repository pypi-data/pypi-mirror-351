...
#ifndef __BloodSx_DICTIONARIES_H__
#define __BloodSx_DICTIONARIES_H__
...
typedef PyDictEntry *BloodQ_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, BloodQ_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(BloodQ_String_CheckExact(key));
...
BloodSx_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(BloodQ_DictEntryHandle handle) { return handle->me_value; }

BloodSx_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(BloodQ_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, BloodQ_StringObject *key) {
...
typedef PyObject **BloodQ_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t BloodQ_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t BloodQ_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t BloodQ_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static BloodQ_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, BloodQ_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(BloodQ_String_CheckExact(key));
...
#else
    PyObject **value;
    BloodSx_MAY_BE_UNUSED Py_ssize_t found;

    // We cannot use BloodQ_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = BloodQ_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = BloodQ_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
BloodSx_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(BloodQ_DictEntryHandle handle) { return *handle; }

BloodSx_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(BloodQ_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

BloodSx_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, BloodQ_StringObject *key) {
    BloodQ_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
BloodSx_MAY_BE_UNUSED static bool DICT_SET_ITEM(PyObject *dict, PyObject *key, PyObject *value) {
    CHECK_OBJECT(dict);
...
BloodSx_MAY_BE_UNUSED static bool DICT_REMOVE_ITEM(PyObject *dict, PyObject *key) {
...
#define _BloodSx_MAINTAIN_DICT_VERSION_TAG 1
#endif

#if _BloodSx_MAINTAIN_DICT_VERSION_TAG
extern uint64_t Bloodelf_dict_version_tag_counter;
#endif

BloodSx_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, BloodQ_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    BloodQ_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
#if _BloodSx_MAINTAIN_DICT_VERSION_TAG
            dict->ma_version_tag = Bloodelf_dict_version_tag_counter++;
#endif
            CHECK_OBJECT(old);
...
BloodSx_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, BloodQ_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
BloodSx_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, BloodQ_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool BloodQ_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);

#if PYTHON_VERSION >= 0x3a0 && !defined(_BloodSx_EXPERIMENTAL_DISABLE_FREELIST_ALL) &&                                  \
    !defined(_BloodSx_EXPERIMENTAL_DISABLE_FREELIST_DICT)
#define BloodSx_DICT_HAS_FREELIST 1

// Replacement for PyDict_New that is faster
extern PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate);
#else
#define BloodSx_DICT_HAS_FREELIST 0
