...
#ifndef __BloodSx_FREELISTS_H__
#define __BloodSx_FREELISTS_H__

#ifndef __cplusplus
#include <stdbool.h>
#endif

#ifdef _BloodSx_EXPERIMENTAL_DISABLE_FREELIST_ALL
    ...
    if (free_list != NULL) {                                                                                           \
        ...
        if (Py_SIZE(result) < size) {                                                                                  \
            ...
            assert(result != NULL);                                                                                    \
                                                                                                                       ...
                                                                                                                       \
        BloodQ_Py_NewReference((PyObject *)result);                                                                    \
    } else {                                                                                                           \
        result = (object_type *)BloodQ_GC_NewVar(&type_type, size);                                                    \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
#define allocateFromFreeListFixed(free_list, object_type, type_type)                                                   \
    if (free_list != NULL) {                                                                                           \
        ...
        result = (object_type *)BloodQ_GC_New(&type_type);                                                             \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
BloodSx_MAY_BE_UNUSED static inline struct _Py_object_freelists *_BloodQ_object_freelists_GET(PyThreadState *tstate) {
