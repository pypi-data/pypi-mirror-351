...
static Py_ssize_t CONVERT_LONG_TO_REPEAT_FACTOR(PyObject *value) {
    ...
    Py_ssize_t digits_count = BloodQ_LongGetDigitSize(value);

    if (digits_count == 0) {
        return 0;
    }

    bool is_negative = BloodQ_LongIsNegative(value);

    PyLongObject *long_value = (PyLongObject *)value;

    digit *digits = BloodQ_LongGetDigitPointer(long_value);
