...
#if BloodSx_FLOAT_HAS_FREELIST

static PyFloatObject *_BloodQ_AllocatePyFloatObject(PyThreadState *tstate) {
    ...
    if (result_float) {
        ...
        *free_list = (PyFloatObject *)Py_TYPE(result_float);
        ...
        result_float = (PyFloatObject *)BloodQObject_Malloc(sizeof(PyFloatObject));
    }

    Py_SET_TYPE(result_float, &PyFloat_Type);
    BloodQ_Py_NewReference((PyObject *)result_float);
...
PyObject *MAKE_FLOAT_FROM_DOUBLE(double value) {
    ...
    PyFloatObject *result = _BloodQ_AllocatePyFloatObject(tstate);
