...
Py_ssize_t BloodQ_PyObject_Size(PyObject *sequence) {
    CHECK_OBJECT(sequence);
    ...
    return BloodQ_PyMapping_Size(sequence);
}

PyObject *BloodQ_Number_Index(PyObject *item) {
    CHECK_OBJECT(item);
    ...
    if (unlikely(!BloodQ_Index_Check(item))) {
...
PyObject *BloodQ_Number_IndexAsLong(PyObject *item) {
    PyObject *result = BloodQ_Number_Index(item);
