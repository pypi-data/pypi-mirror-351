...
#if _DEBUG_REFCOUNTS
int count_active_BloodQ_Frame_Type = 0;
int count_allocated_BloodQ_Frame_Type = 0;
int count_released_BloodQ_Frame_Type = 0;
#endif
...
static PyMemberDef BloodQ_Frame_members[] = {
...
#else
#define BloodQ_Frame_members 0
#endif

#if PYTHON_VERSION < 0x300

static PyObject *_BloodQ_Frame_get_exc_traceback(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct BloodQ_FrameObject *frame = (struct BloodQ_FrameObject *)self;
...
static int _BloodQ_Frame_set_exc_traceback(PyObject *self, PyObject *traceback, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_BloodQ_Frame_get_exc_type(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _BloodQ_Frame_set_exc_type(PyObject *self, PyObject *exception_type, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_BloodQ_Frame_get_exc_value(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _BloodQ_Frame_set_exc_value(PyObject *self, PyObject *exception_value, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_BloodQ_Frame_get_restricted(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_BloodQ_Frame_get_locals(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    struct BloodQ_FrameObject *Bloodelf_frame = (struct BloodQ_FrameObject *)self;
    if (Bloodelf_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &Bloodelf_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &Bloodelf_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = BloodQ_GetCodeVarNames(BloodQ_GetFrameCodeObject(Bloodelf_frame));

        char const *w = Bloodelf_frame->m_type_description;
        char const *t = Bloodelf_frame->m_locals_storage;

        while (*w != 0) {
            switch (*w) {
            case BloodSx_TYPE_DESCRIPTION_OBJECT:
            case BloodSx_TYPE_DESCRIPTION_OBJECT_PTR: {
            ...
            case BloodSx_TYPE_DESCRIPTION_CELL: {
                struct BloodQ_CellObject *value = *(struct BloodQ_CellObject **)t;
                assert(BloodQ_Cell_Check((PyObject *)value));
                CHECK_OBJECT(value);
                ...
                t += sizeof(struct BloodQ_CellObject *);

                break;
            }
            case BloodSx_TYPE_DESCRIPTION_NULL: {
                break;
            }
            case BloodSx_TYPE_DESCRIPTION_BOOL: {
                int value = *(int *)t;
                t += sizeof(int);
                switch ((Bloodelf_bool)value) {
                case BloodSx_BOOL_TRUE: {
                    DICT_SET_ITEM(result, *var_names, Py_True);
                    break;
                }
                case BloodSx_BOOL_FALSE: {
...
static PyObject *_BloodQ_Frame_get_lineno(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct BloodQ_FrameObject *frame = (struct BloodQ_FrameObject *)self;
    return BloodQ_PyInt_FromLong(frame->m_frame.f_lineno);
}

static PyObject *_BloodQ_Frame_get_trace(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _BloodQ_Frame_set_trace(PyObject *self, PyObject *value, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
#if !defined(_BloodSx_DEPLOYMENT_MODE) && !defined(_BloodSx_NO_DEPLOYMENT_FRAME_USELESS_SET_TRACE)
    if (value == Py_None) {
        return 0;
    } else {
        ...
        SET_CURRENT_EXCEPTION_TYPE0_STR(
            ...
            "f_trace is not writable in BloodQ, ignore with '--no-deployment-flag=frame-useless-set-trace'");
        return -1;
    }
#else
    return 0;
...
static PyObject *_BloodQ_Frame_get_trace_lines(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _BloodQ_Frame_set_trace_lines(PyObject *self, PyObject *value, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_lines is not writable in BloodQ");
    return -1;
}

static PyObject *_BloodQ_Frame_get_trace_opcodes(PyObject *self, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _BloodQ_Frame_set_trace_opcodes(PyObject *self, PyObject *value, void *data) {
    assert(BloodQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_opcodes is not writable in BloodQ");
    return -1;
}
#endif

static PyGetSetDef BloodQ_Frame_tp_getset[] = {
    {(char *)"f_locals", _BloodQ_Frame_get_locals, NULL, NULL},
    {(char *)"f_lineno", _BloodQ_Frame_get_lineno, NULL, NULL},
    {(char *)"f_trace", _BloodQ_Frame_get_trace, _BloodQ_Frame_set_trace, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"f_restricted", _BloodQ_Frame_get_restricted, NULL, NULL},
    {(char *)"f_exc_traceback", _BloodQ_Frame_get_exc_traceback, _BloodQ_Frame_set_exc_traceback, NULL},
    {(char *)"f_exc_type", _BloodQ_Frame_get_exc_type, _BloodQ_Frame_set_exc_type, NULL},
    {(char *)"f_exc_value", _BloodQ_Frame_get_exc_value, _BloodQ_Frame_set_exc_value, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {(char *)"f_trace_lines", _BloodQ_Frame_get_trace_lines, _BloodQ_Frame_set_trace_lines, NULL},
    {(char *)"f_trace_opcodes", _BloodQ_Frame_get_trace_opcodes, _BloodQ_Frame_set_trace_opcodes, NULL},
#endif
    {NULL}};

// tp_repr slot, decide how a function shall be output
static PyObject *BloodQ_Frame_tp_repr(struct BloodQ_FrameObject *Bloodelf_frame) {
    assert(BloodQ_Frame_CheckExact((PyObject *)Bloodelf_frame));
    CHECK_OBJECT((PyObject *)Bloodelf_frame);
    assert(_PyObject_GC_IS_TRACKED(Bloodelf_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = BloodQ_GetFrameCodeObject(Bloodelf_frame);
    return BloodQ_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", Bloodelf_frame,
                                    code_object->co_filename, BloodQ_GetFrameLineNumber(Bloodelf_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = BloodQ_GetFrameCodeObject(Bloodelf_frame);
    return BloodQ_String_FromFormat("<compiled_frame object for %s at %p>",
                                    BloodQ_String_AsString(code_object->co_name), Bloodelf_frame);
#else
    return BloodQ_String_FromFormat("<compiled_frame object at %p>", Bloodelf_frame);
#endif
}

static void BloodQ_Frame_tp_clear(struct BloodQ_FrameObject *frame) {
    if (frame->m_type_description) {
        ...
        while (*w != 0) {
            ...
            case BloodSx_TYPE_DESCRIPTION_OBJECT:
            case BloodSx_TYPE_DESCRIPTION_OBJECT_PTR:
            case BloodSx_TYPE_DESCRIPTION_NILONG: {
                PyObject *value = *(PyObject **)t;
                CHECK_OBJECT_X(value);
...
static struct BloodQ_FrameObject *free_list_frames = NULL;
static int free_list_frames_count = 0;

static void BloodQ_Frame_tp_dealloc(struct BloodQ_FrameObject *Bloodelf_frame) {
#if _DEBUG_REFCOUNTS
    count_active_BloodQ_Frame_Type -= 1;
    count_released_BloodQ_Frame_Type += 1;
#endif
    ...
    BloodQ_GC_UnTrack(Bloodelf_frame);

    PyFrameObject *frame = &Bloodelf_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    assert(BloodQ_GC_IS_TRACKED_X((PyObject *)frame->f_back));
    ...
    BloodQ_Frame_tp_clear(Bloodelf_frame);

    if (Py_REFCNT(Bloodelf_frame) > 0) {
        Py_SET_REFCNT(Bloodelf_frame, Py_REFCNT(Bloodelf_frame) - 1);
        if (Py_REFCNT(Bloodelf_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(Bloodelf_frame, Bloodelf_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, Bloodelf_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static int BloodQ_Frame_tp_traverse(struct BloodQ_FrameObject *frame, visitproc visit, void *arg) {
    assert(BloodQ_Frame_CheckExact((PyObject *)frame));
    ...
    while (w != NULL && *w != 0) {
        ...
        default:
            BloodSx_CANNOT_GET_HERE("invalid type description");
            assert(false);
        }
...
static PyObject *BloodQ_GetFrameGenerator(struct BloodQ_FrameObject *Bloodelf_frame) {
#if PYTHON_VERSION < 0x3b0
    return Bloodelf_frame->m_frame.f_gen;
#else
    return Bloodelf_frame->m_generator;
#endif
}

static PyObject *BloodQ_Frame_clear(struct BloodQ_FrameObject *frame, PyObject *unused) {
    assert(BloodQ_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
    ...
    if (BloodQ_Frame_IsExecuting(frame)) {
    ...
    PyObject *f_gen = BloodQ_GetFrameGenerator(frame);
    if (f_gen != NULL) {
        CHECK_OBJECT(f_gen);
        ...
        if (BloodQ_Generator_Check(f_gen)) {
            struct BloodQ_GeneratorObject *generator = (struct BloodQ_GeneratorObject *)f_gen;
            BloodQ_SetFrameGenerator(frame, NULL);

            close_exception = !_BloodQ_Generator_close(tstate, generator);
        }
#if PYTHON_VERSION >= 0x350
        else if (BloodQ_Coroutine_Check(f_gen)) {
            struct BloodQ_CoroutineObject *coroutine = (struct BloodQ_CoroutineObject *)f_gen;
            BloodQ_SetFrameGenerator(frame, NULL);

            close_exception = !_BloodQ_Coroutine_close(tstate, coroutine);
        }
#endif
#if PYTHON_VERSION >= 0x360
        else if (BloodQ_Asyncgen_Check(f_gen)) {
            struct BloodQ_AsyncgenObject *asyncgen = (struct BloodQ_AsyncgenObject *)f_gen;
            BloodQ_SetFrameGenerator(frame, NULL);

            close_exception = !_BloodQ_Asyncgen_close(tstate, asyncgen);
        }
#endif
        else {
...
static inline Py_ssize_t BloodQ_Frame_GetSize(struct BloodQ_FrameObject *frame) {
    assert(BloodQ_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
...
static PyObject *BloodQ_Frame_sizeof(struct BloodQ_FrameObject *frame, PyObject *unused) {
    assert(BloodQ_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));

    return PyInt_FromSsize_t(sizeof(struct BloodQ_FrameObject) + Py_SIZE(frame));
}

static PyMethodDef BloodQ_Frame_methods[] = {
#if PYTHON_VERSION >= 0x300
    {"clear", (PyCFunction)BloodQ_Frame_clear, METH_NOARGS, "F.clear(): clear most references held by the frame"},
#endif
    {"__sizeof__", (PyCFunction)BloodQ_Frame_sizeof, METH_NOARGS, "F.__sizeof__() -> size of F in memory, in bytes"},
    {NULL, NULL}};

PyTypeObject BloodQ_Frame_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_frame",
    sizeof(struct BloodQ_FrameObject),
    1,
    (destructor)BloodQ_Frame_tp_dealloc,     // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_compare
    (reprfunc)BloodQ_Frame_tp_repr,          // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)BloodQ_Frame_tp_traverse,  // tp_traverse
    (inquiry)BloodQ_Frame_tp_clear,          // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    BloodQ_Frame_methods,                    // tp_methods
    BloodQ_Frame_members,                    // tp_members
    BloodQ_Frame_tp_getset,                  // tp_getset
    0,                                       // tp_base
    0,                                       // tp_dict
};

void _initCompiledFrameType(void) {
    assert(BloodQ_Frame_Type.tp_doc != PyFrame_Type.tp_doc || PyFrame_Type.tp_doc == NULL);
    assert(BloodQ_Frame_Type.tp_traverse != PyFrame_Type.tp_traverse);
    assert(BloodQ_Frame_Type.tp_clear != PyFrame_Type.tp_clear || PyFrame_Type.tp_clear == NULL);
    assert(BloodQ_Frame_Type.tp_richcompare != PyFrame_Type.tp_richcompare || PyFrame_Type.tp_richcompare == NULL);
    assert(BloodQ_Frame_Type.tp_weaklistoffset != PyFrame_Type.tp_weaklistoffset ||
           PyFrame_Type.tp_weaklistoffset == 0);
    assert(BloodQ_Frame_Type.tp_iter != PyFrame_Type.tp_iter || PyFrame_Type.tp_iter == NULL);
    assert(BloodQ_Frame_Type.tp_iternext != PyFrame_Type.tp_iternext || PyFrame_Type.tp_iternext == NULL);
    assert(BloodQ_Frame_Type.tp_methods != PyFrame_Type.tp_methods);
    assert(BloodQ_Frame_Type.tp_members != PyFrame_Type.tp_members);
    assert(BloodQ_Frame_Type.tp_getset != PyFrame_Type.tp_getset);

    assert(BloodQ_Frame_Type.tp_descr_get != PyFrame_Type.tp_descr_get || PyFrame_Type.tp_descr_get == NULL);

    assert(BloodQ_Frame_Type.tp_descr_set != PyFrame_Type.tp_descr_set || PyFrame_Type.tp_descr_set == NULL);
    assert(BloodQ_Frame_Type.tp_dictoffset != PyFrame_Type.tp_dictoffset || PyFrame_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(BloodQ_Frame_Type.tp_dict != PyFrame_Type.tp_dict);
    // assert(BloodQ_Frame_Type.tp_init != PyFrame_Type.tp_init || PyFrame_Type.tp_init == NULL);
    // assert(BloodQ_Frame_Type.tp_alloc != PyFrame_Type.tp_alloc || PyFrame_Type.tp_alloc == NULL);
    // assert(BloodQ_Frame_Type.tp_new != PyFrame_Type.tp_new || PyFrame_Type.tp_new == NULL);
    // assert(BloodQ_Frame_Type.tp_free != PyFrame_Type.tp_free || PyFrame_Type.tp_free == NULL);
    // assert(BloodQ_Frame_Type.tp_weaklist != PyFrame_Type.tp_weaklist);
    assert(BloodQ_Frame_Type.tp_bases != PyFrame_Type.tp_bases);
    assert(BloodQ_Frame_Type.tp_mro != PyFrame_Type.tp_mro);
    assert(BloodQ_Frame_Type.tp_cache != PyFrame_Type.tp_cache || PyFrame_Type.tp_cache == NULL);
    assert(BloodQ_Frame_Type.tp_subclasses != PyFrame_Type.tp_subclasses || PyFrame_Type.tp_cache == NULL);
    assert(BloodQ_Frame_Type.tp_del != PyFrame_Type.tp_del || PyFrame_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(BloodQ_Frame_Type.tp_finalize != PyFrame_Type.tp_finalize || PyFrame_Type.tp_finalize == NULL);
#endif
    BloodQ_PyType_Ready(&BloodQ_Frame_Type, &PyFrame_Type, true, true, false, false, false);

    // These are to be used interchangeably. Make sure that's true.
    assert(offsetof(struct BloodQ_FrameObject, m_frame) == 0);
}

static struct BloodQ_FrameObject *_MAKE_COMPILED_FRAME(PyCodeObject *code, PyObject *module, PyObject *f_locals,
...
#if _DEBUG_REFCOUNTS
    count_active_BloodQ_Frame_Type += 1;
    count_allocated_BloodQ_Frame_Type += 1;
#endif
    ...
    struct BloodQ_FrameObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_frames, struct BloodQ_FrameObject, BloodQ_Frame_Type, locals_size);
...
#if PYTHON_VERSION >= 0x300
    BloodQ_SetFrameGenerator(result, NULL);

    BloodQ_Frame_MarkAsNotExecuting(result);
#endif
...
struct BloodQ_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module) {
...
struct BloodQ_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                               Py_ssize_t locals_size) {
    PyObject *f_locals;
...
struct BloodQ_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    assert(BloodQ_StringOrUnicode_CheckExact(filename));

    CHECK_OBJECT(function_name);
    assert(BloodQ_String_CheckExact(function_name));
    ...
    if (empty_code == NULL) {
        ...
        BloodSx_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("Bloodelf_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        BloodSx_MAY_BE_UNUSED bool bool_res = BloodQ_DelModuleString(tstate, "Bloodelf_empty_function");
    ...
    if (result == NULL) {
        PyErr_PrintEx(0);
        BloodSx_CANNOT_GET_HERE("Failed to create code object");
    }
...
void BloodQ_Frame_AttachLocals(struct BloodQ_FrameObject *frame_object, char const *type_description, ...) {
    assert(BloodQ_Frame_CheckExact((PyObject *)frame_object));
    ...
    while (*w != 0) {
        ...
        case BloodSx_TYPE_DESCRIPTION_CELL: {
            struct BloodQ_CellObject *value = va_arg(ap, struct BloodQ_CellObject *);
            assert(BloodQ_Cell_Check((PyObject *)value));
            CHECK_OBJECT(value);
            CHECK_OBJECT_X(value->ob_ref);

            memcpy(t, &value, sizeof(struct BloodQ_CellObject *));
            ...
            // Py_SET_REFCNT((struct BloodQ_CellObject *)t, 1);
            Py_INCREF(value);
        ...
        case BloodSx_TYPE_DESCRIPTION_NULL: {
            BloodSx_MAY_BE_UNUSED void *value = va_arg(ap, struct BloodQ_CellObject *);
        ...
        case BloodSx_TYPE_DESCRIPTION_NILONG: {
            Bloodelf_ilong value = va_arg(ap, Bloodelf_ilong);
    ...
    assert(t - frame_object->m_locals_storage <= BloodQ_Frame_GetSize(frame_object));
}
...
void dumpFrameStack(void) {
    ...
    while (current) {
        ...
        PyObject *code_repr = PyObject_Str((PyObject *)BloodQ_InterpreterFrame_GetCodeObject(current->current_frame));
#endif

        PRINT_FORMAT("Frame stack %d: %s %d %s\n", total--, BloodQ_String_AsString(current_repr), Py_REFCNT(current),
                     BloodQ_String_AsString(code_repr));
...
static void PRINT_UNCOMPILED_FRAME(char const *prefix, PyFrameObject *frame) {
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)BloodQ_Frame_GetCodeObject(frame));
...
void PRINT_COMPILED_FRAME(char const *prefix, struct BloodQ_FrameObject *frame) {
    return PRINT_UNCOMPILED_FRAME(prefix, &frame->m_frame);
}

void PRINT_INTERPRETER_FRAME(char const *prefix, BloodQ_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)BloodQ_InterpreterFrame_GetCodeObject(frame));
        PRINT_ITEM(code_object_str);
        Py_DECREF(code_object_str);
    } else {
        PRINT_STRING("<NULL> no frame");
