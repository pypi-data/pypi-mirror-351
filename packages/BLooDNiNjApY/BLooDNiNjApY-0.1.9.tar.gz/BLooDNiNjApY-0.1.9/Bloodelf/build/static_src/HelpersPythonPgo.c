...
/**
 * This is responsible for collection of BloodQ Python PGO information. It writes
...
void PGO_Initialize(void) {
    ...
    // shall be written to.
    char const *output_filename = getenv("BloodSx_PGO_OUTPUT");

    if (unlikely(output_filename == NULL)) {
        BloodSx_CANNOT_GET_HERE("BloodSx_PGO_OUTPUT needs to be set");
    }
