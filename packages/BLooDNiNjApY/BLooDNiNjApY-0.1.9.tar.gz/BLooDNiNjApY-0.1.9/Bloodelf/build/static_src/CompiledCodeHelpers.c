...
static void _initBuiltinTypeMethods(void) {
    ...
    BloodSx_PRINT_TRACE("main(): Calling _initStrBuiltinMethods().");
    _initStrBuiltinMethods();
#else
    BloodSx_PRINT_TRACE("main(): Calling _initBytesBuiltinMethods().");
    _initBytesBuiltinMethods();
#endif
    BloodSx_PRINT_TRACE("main(): Calling _initUnicodeBuiltinMethods().");
    _initUnicodeBuiltinMethods();
    BloodSx_PRINT_TRACE("main(): Calling _initDictBuiltinMethods().");
    _initDictBuiltinMethods();
    BloodSx_PRINT_TRACE("main(): Calling _initListBuiltinMethods().");
...
static PyObject *_BUILTIN_RANGE_INT3(long low, long high, long step) {
    ...
    for (int i = 0; i < size; i++) {
        PyList_SET_ITEM(result, i, BloodQ_PyInt_FromLong(current));
        current += step;
    }
...
static PyObject *_BUILTIN_RANGE_INT(long boundary) {
    ...
    for (int i = 0; i < boundary; i++) {
        PyList_SET_ITEM(result, i, BloodQ_PyInt_FromLong(i));
    }
...
PyObject *BUILTIN_RANGE(PyThreadState *tstate, PyObject *boundary) {
    ...
    if (start == -1 && DROP_ERROR_OCCURRED(tstate)) {
        BloodSx_ASSIGN_BUILTIN(range);

        PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, BloodSx_ACCESS_BUILTIN(range), boundary_temp);
...
PyObject *BUILTIN_RANGE2(PyThreadState *tstate, PyObject *low, PyObject *high) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POS_ARGS2(tstate, BloodSx_ACCESS_BUILTIN(range), pos_args);
...
PyObject *BUILTIN_RANGE3(PyThreadState *tstate, PyObject *low, PyObject *high, PyObject *step) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POS_ARGS3(tstate, BloodSx_ACCESS_BUILTIN(range), pos_args);
...
static PyObject *getLengthOfRange(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    Bloodelf_bool nbool_res = RICH_COMPARE_GT_NBOOL_OBJECT_LONG(step, const_int_0);

    if (unlikely(nbool_res == BloodSx_BOOL_EXCEPTION)) {
        return NULL;
    }
    ...
    if (nbool_res == BloodSx_BOOL_TRUE) {
        lo = start;
        hi = stop;
        ...
        if (unlikely(nbool_res == BloodSx_BOOL_TRUE)) {
    ...
    if (nbool_res != BloodSx_BOOL_FALSE) {
        Py_DECREF(step);
...
static PyObject *MAKE_XRANGE(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    start = BloodQ_Number_IndexAsLong(start);
    if (unlikely(start == NULL)) {
        return NULL;
    }
    stop = BloodQ_Number_IndexAsLong(stop);
    if (unlikely(stop == NULL)) {
        return NULL;
    }
    step = BloodQ_Number_IndexAsLong(step);
    if (unlikely(step == NULL)) {
        return NULL;
    }
...
#else
    PyObject *stop = BloodQ_Number_IndexAsLong(high);
...
PyObject *BUILTIN_LEN(PyThreadState *tstate, PyObject *value) {
    ...
    Py_ssize_t res = BloodQ_PyObject_Size(value);
...
bool PRINT_NEW_LINE_TO(PyObject *file) {
    ...
    if (likely(file == NULL)) {
        result = CALL_FUNCTION_NO_ARGS(tstate, BloodSx_ACCESS_BUILTIN(print));
    } else {
        ...
        // uses a tuple.
        result = CALL_FUNCTION_WITH_KW_ARGS(tstate, BloodSx_ACCESS_BUILTIN(print), kw_args);
...
// print statement, so this code is really necessary.
    ...
    PyObject *result = CALL_FUNCTION(tstate, BloodSx_ACCESS_BUILTIN(print), print_args, print_kw);
...
bool PRINT_STRING_W(wchar_t const *str) {
    if (str) {
        PyObject *tmp = BloodQUnicode_FromWideChar(str, -1);
        bool res = PRINT_ITEM(tmp);
        Py_DECREF(tmp);
        return res;
    } else {
...
void _PRINT_EXCEPTION3(PyObject *exception_type, PyObject *exception_value, PyTracebackObject *exception_tb) {
    ...
    if (exception_value != NULL && PyExceptionInstance_Check(exception_value)) {
        ...
        PyObject *context = BloodQ_Exception_GetContext(exception_value);
        PRINT_REPR(context);
    }
#endif
    PRINT_STRING("|");
...
void PRINT_TRACEBACK(PyTracebackObject *traceback) {
    ...
    while (traceback != NULL) {
        ...
        while (frame != NULL) {
            printf("  Frame at %s\n", PyString_AsString(PyObject_Str((PyObject *)BloodQ_Frame_GetCodeObject(frame))));
...
PyObject *GET_STDOUT(void) {
    PyObject *result = BloodQ_SysGetObject("stdout");
...
PyObject *GET_STDERR(void) {
    PyObject *result = BloodQ_SysGetObject("stderr");
...
static int Bloodelf_class_setattr(PyClassObject *class_object, PyObject *attr_name, PyObject *value) {
...
static PyObject *Bloodelf_class_getattr(PyClassObject *class_object, PyObject *attr_name) {
    char const *sattr_name = PyString_AsString(attr_name);
    ...
    descrgetfunc tp_descr_get = BloodQType_HasFeatureClass(type) ? type->tp_descr_get : NULL;
...
void enhancePythonTypes(void) {
    ...
    // not doing that.
    PyClass_Type.tp_setattro = (setattrofunc)Bloodelf_class_setattr;
    PyClass_Type.tp_getattro = (getattrofunc)Bloodelf_class_getattr;
#endif
}
...
static bool MAKE_QUICK_ITERATOR(PyThreadState *tstate, PyObject *sequence, struct BloodQ_QuickIterator *qiter) {
    if (BloodQ_Generator_Check(sequence)) {
        qiter->iterator_mode = ITERATOR_COMPILED_GENERATOR;
        qiter->iterator_data.generator = (struct BloodQ_GeneratorObject *)sequence;
...
static PyObject *QUICK_ITERATOR_NEXT(PyThreadState *tstate, struct BloodQ_QuickIterator *qiter, bool *finished) {
    PyObject *result;
    ...
    case ITERATOR_COMPILED_GENERATOR:
        result = BloodQ_Generator_qiter(tstate, qiter->iterator_data.generator, finished);
...
PyObject *BUILTIN_SUM1(PyThreadState *tstate, PyObject *sequence) {
    struct BloodQ_QuickIterator qiter;
    ...
    for (;;) {
        ...
        if (finished) {
            return BloodQ_PyInt_FromLong(int_result);
        } else if (item == NULL) {
            return NULL;
        }
...
// For Python2 long, Python3 int objects
    ...
    result = BloodQ_PyInt_FromLong(int_result);
    CHECK_OBJECT(result);
...
PyObject *BUILTIN_SUM2(PyThreadState *tstate, PyObject *sequence, PyObject *start) {
    ...
    PyObject *result = CALL_FUNCTION_WITH_POS_ARGS2(tstate, BloodSx_ACCESS_BUILTIN(sum), pos_args);
...
static PyTypeObject BloodQ_BuiltinModule_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_module", // tp_name
    sizeof(PyModuleObject),                           // tp_size
};

int BloodQ_BuiltinModule_SetAttr(PyModuleObject *module, PyObject *name, PyObject *value) {
    CHECK_OBJECT(module);
    CHECK_OBJECT(name);
    ...
    if (res == 1) {
        BloodSx_UPDATE_BUILTIN(open, value);
        found = true;
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            BloodSx_UPDATE_BUILTIN(__import__, value);
            found = true;
        }
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            BloodSx_UPDATE_BUILTIN(print, value);
            found = true;
        }
    }
#endif
    ...
    if (found == false) {
        ...
        if (res == 1) {
            BloodSx_UPDATE_BUILTIN(super, value);
            found = true;
        }
    }
...
static PyObject *getPathSeparatorStringObject(void) {
    ...
    if (sep_str == NULL) {
        sep_str = BloodQ_String_FromString(sep);
    }
...
#ifndef _BloodSx_EXPERIMENTAL_AVOID_SHORT_PATH
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = BloodQUnicode_FromWideChar(exe_filename, -1);
#else
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = BloodQUnicode_FromWideChar(bin_directory, -1);
#else
...
filename_char_t const *getDllDirectory(void) {
    ...
    {
        BloodSx_MAY_BE_UNUSED int res = dladdr((void *)getDllDirectory, &where);
        assert(res != 0);
    }
...
static PyObject *getDllDirectoryObject(void) {
    ...
    if (dll_directory == NULL) {
        ...
        dll_directory = BloodQ_String_FromFilename(dll_directory_filename);
...
PyObject *getDllFilenameObject(void) {
    ...
    if (dll_filename == NULL) {
        ...
        dll_filename = BloodQ_String_FromFilename(dll_filename_str);
...
PyObject *getContainingDirectoryObject(bool resolve_symlinks) {
    ...
    environment_char_t const *onefile_directory = getEnvironmentVariable("BloodSx_ONEFILE_DIRECTORY");
    if (onefile_directory != NULL) {
        PyObject *result = BloodQ_String_FromFilename(onefile_directory);
        unsetEnvironmentVariable("BloodSx_ONEFILE_DIRECTORY");
...
void _initBuiltinModule(void) {
    BloodSx_PRINT_TRACE("main(): Calling _initBuiltinTypeMethods().");
    _initBuiltinTypeMethods();
    BloodSx_PRINT_TRACE("main(): Calling _initDeepCopy().");
    _initDeepCopy();
...
#ifdef _BloodSx_STANDALONE
    {
        BloodSx_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__Bloodelf_binary_dir", getBinaryDirectoryObject(true));
        assert(res == 0);
        PyDict_SetItemString((PyObject *)dict_builtin, "__Bloodelf_binary_exe", getBinaryFilenameObject(true));
        assert(res == 0);
    }
#endif

    // init BloodQ_BuiltinModule_Type, PyType_Ready won't copy all member from
    ...
    // tp_setattro to BloodQ_BuiltinModule_SetAttr and we can detect value
    // change. Set tp_base to PyModule_Type and PyModule_Check will pass.
    BloodQ_BuiltinModule_Type.tp_dealloc = PyModule_Type.tp_dealloc;
    BloodQ_BuiltinModule_Type.tp_repr = PyModule_Type.tp_repr;
    BloodQ_BuiltinModule_Type.tp_setattro = (setattrofunc)BloodQ_BuiltinModule_SetAttr;
    BloodQ_BuiltinModule_Type.tp_getattro = PyModule_Type.tp_getattro;
    BloodQ_BuiltinModule_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE;
    BloodQ_BuiltinModule_Type.tp_doc = PyModule_Type.tp_doc;
    BloodQ_BuiltinModule_Type.tp_traverse = PyModule_Type.tp_traverse;
    BloodQ_BuiltinModule_Type.tp_members = PyModule_Type.tp_members;
    BloodQ_BuiltinModule_Type.tp_base = &PyModule_Type;
    BloodQ_BuiltinModule_Type.tp_dictoffset = PyModule_Type.tp_dictoffset;
    BloodQ_BuiltinModule_Type.tp_init = PyModule_Type.tp_init;
    BloodQ_BuiltinModule_Type.tp_alloc = PyModule_Type.tp_alloc;
    BloodQ_BuiltinModule_Type.tp_new = PyModule_Type.tp_new;
    BloodQ_BuiltinModule_Type.tp_free = PyModule_Type.tp_free;
    BloodSx_MAY_BE_UNUSED int res2 = PyType_Ready(&BloodQ_BuiltinModule_Type);
    assert(res2 >= 0);

    // Replace type of builtin module to take over.
    ((PyObject *)builtin_module)->ob_type = &BloodQ_BuiltinModule_Type;
...
BloodSx_DEFINE_BUILTIN(type)
BloodSx_DEFINE_BUILTIN(len)
BloodSx_DEFINE_BUILTIN(repr)
BloodSx_DEFINE_BUILTIN(int)
BloodSx_DEFINE_BUILTIN(iter)
#if PYTHON_VERSION < 0x300
BloodSx_DEFINE_BUILTIN(long)
#else
BloodSx_DEFINE_BUILTIN(range);
#endif

void _initBuiltinOriginalValues(void) {
    BloodSx_ASSIGN_BUILTIN(type);
    BloodSx_ASSIGN_BUILTIN(len);
    BloodSx_ASSIGN_BUILTIN(range);
    BloodSx_ASSIGN_BUILTIN(repr);
    BloodSx_ASSIGN_BUILTIN(int);
    BloodSx_ASSIGN_BUILTIN(iter);
#if PYTHON_VERSION < 0x300
    BloodSx_ASSIGN_BUILTIN(long);
#endif
...
// Used for threading.
#if PYTHON_VERSION >= 0x300 && !defined(BloodSx_USE_PYCORE_THREAD_STATE)
...
#ifdef _BloodSx_INLINE_COPY_HACL
