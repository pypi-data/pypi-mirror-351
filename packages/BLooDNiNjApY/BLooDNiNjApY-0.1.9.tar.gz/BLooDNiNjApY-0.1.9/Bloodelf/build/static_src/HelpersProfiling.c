...
/**
 * This is responsible for profiling BloodQ using "vmprof".
 */
...
void startProfiling(void) {
    tempfile_profile = fopen("Bloodelf-performance.dat", "w+b");
    ...
    PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, PyObject_GetAttrString(vmprof_module, "enable"),
                                                     BloodQ_PyInt_FromLong(fileno(tempfile_profile)));
...
void stopProfiling(void) {
    ...
    FILE *tempfile_times = fopen("Bloodelf-times.dat", "wb");
