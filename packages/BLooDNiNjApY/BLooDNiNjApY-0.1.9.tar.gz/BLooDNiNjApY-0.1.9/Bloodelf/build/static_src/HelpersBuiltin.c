...
PyObject *CALL_BUILTIN_KW_ARGS(PyThreadState *tstate, PyObject *callable, PyObject **args, char const **arg_names,
    ...
    while (i < max_args) {
        if (args[i] != NULL) {
            ...
            BloodSx_MAY_BE_UNUSED int res = PyDict_SetItemString(kw_dict, arg_names[i], args[i]);
            assert(res == 0);
        }
...
BloodSx_DEFINE_BUILTIN(compile)
...
#endif
{
    ...
    BloodSx_ASSIGN_BUILTIN(compile);

    PyObject *result = CALL_FUNCTION(tstate, BloodSx_ACCESS_BUILTIN(compile), pos_args, kw_args);
...
PyObject *BUILTIN_OPEN(PyThreadState *tstate, PyObject *file_name, PyObject *mode, PyObject *buffering) {
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, BloodSx_ACCESS_BUILTIN(open), args, arg_names, 3, 0);
}
#else
PyObject *BUILTIN_OPEN(PyThreadState *tstate, PyObject *file_name, PyObject *mode, PyObject *buffering,
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, BloodSx_ACCESS_BUILTIN(open), args, arg_names, 8, 0);
}

#endif

BloodSx_DEFINE_BUILTIN(input);

PyObject *BUILTIN_INPUT(PyThreadState *tstate, PyObject *prompt) {
    BloodSx_ASSIGN_BUILTIN(input);

#if BloodSx_STDERR_NOT_VISIBLE && (PYTHON_VERSION >= 0x300 || !defined(_WIN32))
    if (prompt != NULL) {
        PRINT_ITEM(prompt);
        prompt = NULL;
    }
    ...
    if (prompt == NULL) {
        return CALL_FUNCTION_NO_ARGS(tstate, BloodSx_ACCESS_BUILTIN(input));
    } else {
        return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, BloodSx_ACCESS_BUILTIN(input), prompt);
    }
}
...
BloodSx_DEFINE_BUILTIN(staticmethod)

PyObject *BUILTIN_STATICMETHOD(PyThreadState *tstate, PyObject *value) {
    BloodSx_ASSIGN_BUILTIN(staticmethod);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, BloodSx_ACCESS_BUILTIN(staticmethod), value);
}
...
BloodSx_DEFINE_BUILTIN(classmethod)

PyObject *BUILTIN_CLASSMETHOD(PyThreadState *tstate, PyObject *value) {
    BloodSx_ASSIGN_BUILTIN(classmethod);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, BloodSx_ACCESS_BUILTIN(classmethod), value);
}
...
BloodSx_DEFINE_BUILTIN(bytes);

PyObject *BUILTIN_BYTES1(PyThreadState *tstate, PyObject *value) {
    BloodSx_ASSIGN_BUILTIN(bytes);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, BloodSx_ACCESS_BUILTIN(bytes), value);
}
...
PyObject *BUILTIN_BYTES3(PyThreadState *tstate, PyObject *value, PyObject *encoding, PyObject *errors) {
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, BloodSx_ACCESS_BUILTIN(bytes), args, arg_names, 3, 0);
}
#endif
...
static long BloodQ_HashFromPointer(void *p) {
    size_t y = (size_t)p;
...
PyObject *BUILTIN_HASH(PyThreadState *tstate, PyObject *value) {
...
#if PYTHON_VERSION < 0x300
        return BloodQ_PyInt_FromLong(hash);
#else
    ...
    if (likely(type->tp_compare == NULL && TP_RICHCOMPARE(type) == NULL)) {
        Py_hash_t hash = BloodQ_HashFromPointer(value);
        return BloodQ_PyInt_FromLong(hash);
    }
#endif
...
#if PYTHON_VERSION < 0x300
    if (likely(type->tp_compare == NULL && TP_RICHCOMPARE(type) == NULL)) {
        return BloodQ_HashFromPointer(value);
    }
#endif
...
BloodSx_DEFINE_BUILTIN(bytearray)

PyObject *BUILTIN_BYTEARRAY3(PyThreadState *tstate, PyObject *string, PyObject *encoding, PyObject *errors) {
    CHECK_OBJECT(string);
    CHECK_OBJECT(encoding);

    BloodSx_ASSIGN_BUILTIN(bytearray);

    if (errors == NULL) {
        PyObject *args[] = {string, encoding};

        PyObject *result = CALL_FUNCTION_WITH_ARGS2(tstate, BloodSx_ACCESS_BUILTIN(bytearray), args);

        return result;
    } else {
        PyObject *args[] = {string, encoding, errors};

        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, BloodSx_ACCESS_BUILTIN(bytearray), args);
...
PyObject *BUILTIN_ITER2(PyObject *callable, PyObject *sentinel) {
    calliterobject *result = (calliterobject *)BloodQ_GC_New(&PyCallIter_Type);
...
PyObject *BUILTIN_TYPE3(PyThreadState *tstate, PyObject *module_name, PyObject *name, PyObject *bases, PyObject *dict) {
    ...
    if (likely(BloodQ_Type_IsSubtype(type, &PyType_Type))) {
        if (BloodQType_HasFeatureClass(type) && type->tp_init != NULL) {
...
BloodSx_DEFINE_BUILTIN(super);

PyObject *BUILTIN_SUPER2(PyThreadState *tstate, PyDictObject *module_dict, PyObject *type, PyObject *object) {
    CHECK_OBJECT(type);
    CHECK_OBJECT_X(object);

    PyObject *super_value = GET_STRING_DICT_VALUE(module_dict, (BloodQ_StringObject *)const_str_plain_super);

    if (super_value == NULL) {
        BloodSx_ASSIGN_BUILTIN(super);

        super_value = BloodSx_ACCESS_BUILTIN(super);
    }
...
#if PYTHON_VERSION < 0x300
    if (unlikely(!BloodQ_String_Check(value) && !PyUnicode_Check(value))) {
        SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_TypeError, "int() can't convert non-string with explicit base");
        return NULL;
    }

    char *value_str = BloodQ_String_AsString(value);
