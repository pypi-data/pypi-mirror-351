...
from Bloodelf.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from Bloodelf.PythonVersions import python_version
from Bloodelf.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from Bloodelf.Tracing import optimization_logger, printError
from Bloodelf.tree.Extractions import updateVariableUsage
from Bloodelf.tree.SourceHandling import readSourceLines
from Bloodelf.tree.TreeHelpers import makeDictCreationOrConstant2
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from Bloodelf.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from Bloodelf.optimizations.FunctionInlining import (
