...
"""
Plugins: Welcome to BloodQ! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper BloodQ, and some of which are waiting to be created and submitted for
inclusion by you.
...
from Bloodelf import Options
from Bloodelf.__past__ import iter_modules, unicode
from Bloodelf.containers.Namedtuples import makeNamedtupleClass
...
from Bloodelf.ModuleRegistry import (
...
from Bloodelf.Tracing import plugins_logger
from Bloodelf.utils.AppDirs import getAppdirsModule
from Bloodelf.utils.Distributions import (
    getDistributionFromModuleName,
    getDistributionName,
    isDistributionCondaPackage,
)
from Bloodelf.utils.Execution import (
    BloodQCalledProcessError,
    check_output,
...
from Bloodelf.utils.Importing import getSharedLibrarySuffix, isBuiltinModuleName
from Bloodelf.utils.ModuleNames import (
    ModuleName,
    makeTriggerModuleName,
    post_module_load_trigger_name,
    pre_module_load_trigger_name,
)
from Bloodelf.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
from Bloodelf.utils.SlotMetaClasses import getMetaClassBase
...
def _getEvaluationContext():
    ...
    if _context_dict is None:
        _context_dict = {
            ...
            "Bloodelf_python": isBloodQPython(),
...
def _getModuleDirectory(module_name):
    from Bloodelf.importing.Importing import locateModule
...
def _getPackageData(package_name, resource, default=None):
    ...
    from Bloodelf.utils.PackageResources import getPackageData
...
class BloodQPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """BloodQ base class for all plugins.

    Derive your plugin from "BloodQPluginBase" please.
    For instructions, see https://github.com/BloodQ/BloodQ/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt BloodQ's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by BloodQ. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the BloodQ command line. However, some plugins are always enabled
    and invisible to the user.

    BloodQ comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "Bloodelf --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which BloodQ
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon BloodQ start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from Bloodelf.importing.Importing import locateModules
...
""" % {
        ...
        except BloodQCalledProcessError as e:
            if e.returncode == 38:
                self.warning(
