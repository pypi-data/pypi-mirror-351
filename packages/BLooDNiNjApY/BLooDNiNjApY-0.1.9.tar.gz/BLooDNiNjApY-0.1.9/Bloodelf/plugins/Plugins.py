...
from Bloodelf import Options, OutputDirectories
from Bloodelf.__past__ import basestring, iter_modules
from Bloodelf.build.DataComposerInterface import deriveModuleConstantsBlobName
from Bloodelf.containers.OrderedDicts import OrderedDict
from Bloodelf.containers.OrderedSets import OrderedSet
from Bloodelf.Errors import BloodQSyntaxError
from Bloodelf.freezer.IncludedDataFiles import IncludedDataFile
from Bloodelf.freezer.IncludedEntryPoints import IncludedEntryPoint
from Bloodelf.ModuleRegistry import addUsedModule
from Bloodelf.PythonVersions import python_version
from Bloodelf.Tracing import plugins_logger, printLine
...
from .PluginBase import BloodQPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, BloodQPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "BloodQPluginDetector", ""
            ) != plugin_class.__name__.replace("BloodQPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match BloodQPlugin* and BloodQPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import Bloodelf.plugins.standard

    _loadPluginClassesFromPackage(Bloodelf.plugins.standard)

    try:
        import Bloodelf.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(Bloodelf.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from Bloodelf.importing.Importing import getModuleNameAndKindFromFilename
        from Bloodelf.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from Bloodelf.tree.Building import buildModule
    ...
    def onModuleUsageLookAhead(cls, module_name, module_filename, module_kind):
        ...
        def getModuleSourceCode():
            ...
            from Bloodelf.tree.SourceHandling import readSourceCodeFromFilename
    ...
    def onModuleInitialSet():
        ...
        from Bloodelf.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from Bloodelf.ModuleRegistry import getDoneModules
        ...
        for plugin in getActivePlugins():
            ...
            if value is not None:
                ...
                for key, value in sorted(value.items()):
                    ...
                    if not key.startswith("Bloodelf_"):
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from Bloodelf.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in BloodQ".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not BloodQPluginBase
            and issubclass(obj, BloodQPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or BloodQ arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of BloodQ plugin '%s' not given."
