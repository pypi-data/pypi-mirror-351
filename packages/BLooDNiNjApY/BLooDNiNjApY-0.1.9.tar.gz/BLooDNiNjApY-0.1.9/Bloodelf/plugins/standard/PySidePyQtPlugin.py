...
from Bloodelf.PythonFlavors import isAnacondaPython
from Bloodelf.PythonVersions import python_version
from Bloodelf.utils.Distributions import getDistributionFromModuleName
from Bloodelf.utils.FileOperations import getFileList, listDir
from Bloodelf.utils.ModuleNames import ModuleName
from Bloodelf.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class BloodQPluginQtBindingsPluginBase(BloodQPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "Bloodelf_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_Bloodelf_patch_level', 0)"
                    ),
                ),
    ...
    def _getBloodQPatchLevel(self):
        """Does it include the BloodQ patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().Bloodelf_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__Bloodelf_binary_dir):
    os.environ["PATH"] = __Bloodelf_binary_dir + ";" + path
"""
            yield (
                code,
...
class BloodQPluginPyQt5QtPluginsPlugin(BloodQPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        BloodQPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the BloodQ support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = BloodQPluginQtBindingsPluginBase._getQtInformation(self)
...
class BloodQPluginDetectorPyQt5QtPluginsPlugin(BloodQPluginBase):
    detector_for = BloodQPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == BloodQPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact BloodQ commercial if you need it."
            )


class BloodQPluginPySide2Plugins(BloodQPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getBloodQPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://Bloodelf.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getBloodQPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return BloodQPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in BloodQPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getBloodQPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: BloodQ does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class BloodQPluginDetectorPySide2Plugins(BloodQPluginBase):
    detector_for = BloodQPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == BloodQPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class BloodQPluginPySide6Plugins(BloodQPluginQtBindingsPluginBase):
...
class BloodQPluginDetectorPySide6Plugins(BloodQPluginBase):
    detector_for = BloodQPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == BloodQPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class BloodQPluginPyQt6Plugins(BloodQPluginQtBindingsPluginBase):
...
class BloodQPluginDetectorPyQt6Plugins(BloodQPluginBase):
    detector_for = BloodQPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == BloodQPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class BloodQPluginNoQt(BloodQPluginBase):
