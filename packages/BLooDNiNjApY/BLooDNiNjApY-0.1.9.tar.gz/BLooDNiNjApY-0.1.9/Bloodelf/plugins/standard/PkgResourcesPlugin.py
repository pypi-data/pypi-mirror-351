...
BloodQ can detect some things that "pkg_resources" may not even be able to during
...
from Bloodelf.utils.Utils import withNoDeprecationWarning


class BloodQPluginResources(BloodQPluginBase):
...
class BloodQProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__Bloodelf_loader_type, BloodQProvider)
"""

        yield (
            code,
            """Registering BloodQ loader with "pkg_resources".""",
        )
