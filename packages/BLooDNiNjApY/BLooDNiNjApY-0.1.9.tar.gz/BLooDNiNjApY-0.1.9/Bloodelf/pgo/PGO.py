...
"""Python level PGO handling in BloodQ."""

import os
import struct

from Bloodelf.__past__ import xrange
from Bloodelf.Options import getPythonPgoUnseenModulePolicy
from Bloodelf.Tracing import pgo_logger
...
def readPGOInputFile(input_filename):
    ...
    with open(input_filename, "rb") as input_file:
        ...
        if header != b"KAY.PGO":
            pgo_logger.sysexit(
                "Error, file '%s' is not a valid PGO input for this version of BloodQ."
                % input_filename
            )
