...
""" CType classes for PyObject *, PyObject **, and struct BloodQ_CellObject *

"""

from Bloodelf.__past__ import iterItems, xrange
from Bloodelf.code_generation.ErrorCodes import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from Bloodelf.code_generation.templates.CodeTemplatesVariables import (
...
from Bloodelf.Constants import getConstantValueGuide, isMutable
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToBloodQBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
        ...
        emit(
            "%s = %s == 0 ? BloodSx_BOOL_FALSE : BloodSx_BOOL_TRUE;"
            % (to_name, truth_name)
        )
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = BloodQ_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct BloodQ_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "BloodQ_Cell_New1(%s)" % init_from
        else:
            return "BloodQ_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "BloodQ_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct BloodQ_CellObject *%s" % variable_code_name
