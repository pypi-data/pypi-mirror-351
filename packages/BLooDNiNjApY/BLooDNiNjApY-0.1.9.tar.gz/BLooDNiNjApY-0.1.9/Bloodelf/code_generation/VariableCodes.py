...
def generateAssignmentVariableCode(statement, emit, context):
    ...
    if variable.isModuleVariable():
        ...
        if source_shape is tshape_bool and variable_declaration.c_type == "Bloodelf_bool":
            tmp_name = context.allocateTempName("assign_source", "Bloodelf_bool")
        elif (
            source_shape is tshape_int_or_long
            and variable_declaration.c_type == "Bloodelf_ilong"
        ):
            tmp_name = context.allocateTempName("assign_source", "Bloodelf_ilong")
        else:
            tmp_name = context.allocateTempName("assign_source")
