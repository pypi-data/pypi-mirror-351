...
""" Command line options of BloodQ.

These provide only the optparse options to use, and the mechanic to actually
do it, but updating and checking module "Bloodelf.Options" values is not in
...
from Bloodelf.PythonFlavors import getPythonFlavorName
from Bloodelf.PythonVersions import isPythonWithGil
from Bloodelf.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from Bloodelf.utils.FileOperations import getFileContentByLine
from Bloodelf.utils.Utils import (
    getArchitecture,
...
from Bloodelf.Version import getCommercialVersion, getBloodQVersion

# Indicator if we were called as "Bloodelf-run" in which case we assume some
# other defaults and work a bit different with parameters.
_Bloodelf_binary_name = os.path.basename(sys.argv[0])
if _Bloodelf_binary_name == "__main__.py":
    _Bloodelf_binary_name = "%s -m Bloodelf" % os.path.basename(sys.executable)
is_Bloodelf_run = _Bloodelf_binary_name.lower().endswith("-run")

if not is_Bloodelf_run:
    usage_template = (
...
parser = makeOptionsParser(usage=usage_template % _Bloodelf_binary_name)
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run BloodQ, this
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run BloodQ, most
...
Python binary to use for Scons. Otherwise BloodQ can
use what you run BloodQ with, or find Python installation,
...
and BloodQ will then find it and include it and all the modules found below that
...
and BloodQ will then find it and include it in the binary or extension module
...
with "Bloodelf-onefile-unpack" rather than a stream that only the onefile
...
warnings_group = parser.add_option_group("Control the warnings to be given by BloodQ")
...
warnings_group.add_option(
    ...
    help="""\
Allow BloodQ to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the BloodQ website. The mnemonic is the part
...
debugger is automatically chosen unless specified by name with the BloodSx_DEBUGGER_CHOICE
...
remove bloat, add hidden dependencies. Check the BloodQ Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by BloodQ. This is
...
created by BloodQ. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in BloodQ, do not use for
...
development_group = parser.add_option_group("BloodQ Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for BloodQ development only. Takes
...
Note: This is experimental and not working with standalone modes of BloodQ yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of BloodQ commercial.
...
intended for developing plugins and BloodQ package configuration. Use e.g.
...
def _considerPluginOptions(logger):
    ...
    from Bloodelf.plugins.Plugins import (
...
def getBloodQProjectOptions(logger, filename_arg, module_mode):
    """Extract the BloodQ project options.

    Note: This is used by BloodQ project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of BloodQ handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)Bloodelf-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'Bloodelf-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'Bloodelf-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'Bloodelf-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'Bloodelf-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'Bloodelf-project-else' not currently allowed after nested Bloodelf-project-if.",
                    )
...
def _considerGithubWorkflowOptions(phase):
    ...
    for key, value in json.loads(os.environ["BloodSx_WORKFLOW_INPUTS"]).items():
        if not value:
            continue
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getBloodQProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
...
pass them to the compiled program execution."""
    ...
    return is_Bloodelf_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from Bloodelf.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from Bloodelf.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from Bloodelf.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from Bloodelf.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.edit_module_code:
        from Bloodelf.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from Bloodelf.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from Bloodelf.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
