...
from Bloodelf.Options import getOnefileTempDirSpec, isOnefileTempDirMode
from Bloodelf.OutputDirectories import getResultFullpath
from Bloodelf.plugins.Plugins import Plugins
from Bloodelf.PostProcessing import executePostProcessingResources
from Bloodelf.PythonVersions import (
    getZstandardSupportingVersions,
    python_version,
)
from Bloodelf.Tracing import onefile_logger, postprocessing_logger
from Bloodelf.utils.Execution import withEnvironmentVarsOverridden
...
from Bloodelf.utils.SharedLibraries import cleanupHeaderForAndroid
from Bloodelf.utils.Signing import addMacOSCodeSignature
...
from Bloodelf.utils.WindowsResources import RT_RCDATA, addResourceToFile
...
def _runOnefileScons(onefile_compression, onefile_archive):
    ...
    env_values["_BloodSx_ONEFILE_TEMP_SPEC"] = getOnefileTempDirSpec()
    env_values["_BloodSx_ONEFILE_TEMP_BOOL"] = "1" if isOnefileTempDirMode() else "0"
    env_values["_BloodSx_ONEFILE_COMPRESSION_BOOL"] = "1" if onefile_compression else "0"
    env_values["_BloodSx_ONEFILE_BUILD_BOOL"] = "1" if onefile_compression else "0"
    env_values["_BloodSx_ONEFILE_ARCHIVE_BOOL"] = "1" if onefile_archive else "0"
...
def runOnefileCompressor(
    ...
    if compressor_python is None or areSamePaths(
        ...
        from Bloodelf.tools.onefile_compressor.OnefileCompressor import (
        ...
        mapping = {
            "BloodSx_PACKAGE_HOME": os.path.dirname(
                os.path.abspath(sys.modules["Bloodelf"].__path__[0])
            )
        }

        mapping["BloodSx_PROGRESS_BAR"] = "1" if Options.shallUseProgressBar() else "0"
