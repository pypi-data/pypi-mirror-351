...
from Bloodelf.Errors import BloodQForbiddenDLLEncounter
from Bloodelf.plugins.Plugins import Plugins
from Bloodelf.PythonFlavors import isAnacondaPython, isBloodQPython
from Bloodelf.PythonVersions import python_version
from Bloodelf.Tracing import inclusion_logger
...
from Bloodelf.utils.Importing import getSharedLibrarySuffixes
from Bloodelf.utils.Json import loadJsonFromFilename
...
from Bloodelf.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isBloodQPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although BloodQ Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the BloodQ Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise BloodQForbiddenDLLEncounter(binary_filename, plugin_name)
...
def fixupBinaryDLLPathsMacOS(
    ...
    except BloodQForbiddenDLLEncounter:
