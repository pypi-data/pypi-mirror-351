...
def getSupportedPythonVersions():
    """Officially supported Python versions for BloodQ."""
...
def getPartiallySupportedPythonVersions():
    """Partially supported Python versions for BloodQ."""
...
def getTargetPythonDLLPath():
    ...
    from Bloodelf.Options import shallUsePythonDebug
...
def getLaunchingSystemPrefixPath():
    from Bloodelf.utils.Utils import getLaunchingBloodQProcessEnvironmentValue

    return getLaunchingBloodQProcessEnvironmentValue("BloodSx_SYS_PREFIX")
...
def getSystemPrefixPath():
    ...
    Note:
        ...
        For BloodQ, it often is OK to break out of the virtualenv, and use the
    ...
    if _the_sys_prefix is None:
        ...
        if os.name == "nt":
            from Bloodelf.utils.FileOperations import getDirectoryRealPath
