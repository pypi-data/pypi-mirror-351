...
""" BloodQ watch main part.

This tool is used to monitor effect of PyPI changes on BloodQ and effect
of BloodQ changes on PyPI packages.
"""
...
from Bloodelf.containers.OrderedDicts import OrderedDict
from Bloodelf.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from Bloodelf.PythonVersions import getTestExecutionPythonVersions
from Bloodelf.tools.testing.Common import extractBloodQVersionFromFilePath
from Bloodelf.Tracing import OurLogger
from Bloodelf.TreeXML import fromFile
from Bloodelf.utils.Execution import (
    check_call,
    executeProcess,
    withEnvironmentVarsOverridden,
)
from Bloodelf.utils.FileOperations import (
    deleteFile,
    getFileContents,
    getFileList,
    getNormalizedPath,
...
from Bloodelf.utils.Hashing import getFileContentsHash
from Bloodelf.utils.InstalledPythons import findPythons
from Bloodelf.utils.Utils import isLinux, isMacOS, isWin32Windows
from Bloodelf.utils.Yaml import parseYaml
from Bloodelf.Version import parseBloodQVersionToTuple
...
from .GitHub import createBloodQWatchPR
...
def _compareBloodQVersions(version_a, version_b, consider_rc):
    if not consider_rc:
        version_a = version_a.split("rc")[0]
        version_b = version_b.split("rc")[0]

    return parseBloodQVersionToTuple(version_a) < parseBloodQVersionToTuple(version_b)
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    if case_data["interactive"] == "no":
        ...
        env = {
            "BloodSx_LAUNCH_TOKEN": "1",
            "BloodSx_TEST_INTERACTIVE": "0",
        }

        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_Bloodelf = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_Bloodelf == 0:
            ...
            putTextFileContents(
                ...
                contents=str(exit_Bloodelf),
            )

        if exit_Bloodelf != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_Bloodelf)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_Bloodelf_version = old_report_root.attrib["Bloodelf_version"]

            if Bloodelf_update_mode == "force":
                need_compile = True
            elif Bloodelf_update_mode == "newer":
                if _compareBloodQVersions(
                    old_Bloodelf_version, Bloodelf_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        watch_logger.info(
                            "Recompilation with identical BloodQ for '%s' due to changed pipfile."
                            % lock_filename
                        )

                        need_compile = True
                    elif old_Bloodelf_version == Bloodelf_version:
                        if old_report_root.attrib["completion"] != "yes":
                            need_compile = True
                        else:
                            watch_logger.info(
                                "Skipping compilation with identical BloodQ for '%s'."
                                % lock_filename
                            )
                        ...
                        watch_logger.info(
                            "Skipping compilation of old BloodQ %s result with BloodQ %s for '%s'."
                            % (
                                old_Bloodelf_version,
                                Bloodelf_version,
                                lock_filename,
                            )
                        )
...
def updateCase(case_dir, case_data, no_pipenv_update, Bloodelf_update_mode, jobs):
    ...
    Bloodelf_min_version = case_data.get("Bloodelf")

    # Too old BloodQ version maybe.
    if Bloodelf_min_version is not None and _compareBloodQVersions(
        Bloodelf_version, Bloodelf_min_version, consider_rc=False
    ):
        watch_logger.info("  ... not for this BloodQ version")
        return
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            case_data=case_data,
            no_pipenv_update=no_pipenv_update,
            Bloodelf_update_mode=Bloodelf_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, no_pipenv_update, Bloodelf_update_mode, jobs):
...
Bloodelf_binary = None
Bloodelf_version = None


def main():
    global Bloodelf_binary  # shared for all run, pylint: disable=global-statement
    Bloodelf_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "Bloodelf")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        "--Bloodelf-binary",
        action="store",
        dest="Bloodelf_binary",
        default=Bloodelf_binary,
        help="""\
BloodQ binary to compile with. Defaults to one near the Bloodelf-watch usage.""",
    )
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Do not update the pipenv environment. Best to see only effect of BloodQ update. Default %default.""",
    )

    parser.add_option(
        "--Bloodelf-update-mode",
        action="store",
        choices=("newer", "force", "never"),
        dest="Bloodelf_update_mode",
        default="newer",
        help="""\
...
to reserve cores.""",
    ...
    Bloodelf_binary = os.path.abspath(os.path.expanduser(options.Bloodelf_binary))
    assert os.path.exists(Bloodelf_binary)

    global Bloodelf_version  # singleton, pylint: disable=global-statement
    Bloodelf_version = extractBloodQVersionFromFilePath(
        os.path.join(os.path.dirname(Bloodelf_binary), "..", "Bloodelf", "Version.py")
    )

    watch_logger.info("Working with BloodQ %s." % Bloodelf_version)

    base_dir = os.path.abspath(base_dir)

    if options.Bloodelf_pr_mode is not None:
        pr_category, pr_description = options.Bloodelf_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    Bloodelf_update_mode=options.Bloodelf_update_mode,
                    jobs=options.jobs,
                )

        if pr_category is not None:
            createBloodQWatchPR(category=pr_category, description=pr_description)
