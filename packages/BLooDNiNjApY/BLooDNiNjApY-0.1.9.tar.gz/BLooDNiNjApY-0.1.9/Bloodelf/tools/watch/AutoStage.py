...
""" Stage changes in BloodQ-Watch automatically
...
from Bloodelf.tools.quality.Git import (
    getCheckoutFileChangeDesc,
    getFileHashContent,
    putFileHashContent,
    updateGitFile,
)
from Bloodelf.TreeXML import fromString, toString
from Bloodelf.utils.FileOperations import getFileContents, withTemporaryFile
...
def onCompilationReportChange(filename, git_stage):
    ...
    new_Bloodelf_version = new_report.attrib["Bloodelf_version"]
    changed = False
    if old_report.attrib["Bloodelf_version"] != new_Bloodelf_version:
        old_report.attrib["Bloodelf_version"] = new_Bloodelf_version
