...
from Bloodelf.tools.release.Release import (
    getBranchName,
...
from Bloodelf.utils.Utils import getArchitecture, isWin32Windows

from .Podman import getPodmanExecutablePath

containers_logger = OurLogger("BloodQ-Containers", base_style="blue")
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m Bloodelf --version"
    ...
    container_tag_name = "Bloodelf-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.Bloodelf.net:116.202.30.188")
    else:
