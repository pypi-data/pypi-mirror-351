...
import sys
import Bloodelf.Options

Bloodelf.Options.is_full_compat = False
...
import Bloodelf.code_generation.BinaryOperationHelperDefinitions
import Bloodelf.code_generation.CodeGeneration
import Bloodelf.code_generation.ComparisonCodes
import Bloodelf.code_generation.Namify
import Bloodelf.nodes.NetworkxNodes
import Bloodelf.nodes.PackageMetadataNodes
import Bloodelf.nodes.PackageResourceNodes
import Bloodelf.nodes.SideEffectNodes
import Bloodelf.nodes.TensorflowNodes
import Bloodelf.specs.BuiltinBytesOperationSpecs
import Bloodelf.specs.BuiltinDictOperationSpecs
import Bloodelf.specs.BuiltinListOperationSpecs
import Bloodelf.specs.BuiltinStrOperationSpecs
import Bloodelf.specs.BuiltinTypeOperationSpecs
import Bloodelf.specs.HardImportSpecs
import Bloodelf.tree.Building
from Bloodelf.containers.OrderedDicts import OrderedDict
from Bloodelf.containers.OrderedSets import OrderedSet
from Bloodelf.nodes.ImportNodes import hard_modules_non_stdlib
from Bloodelf.nodes.NodeMetaClasses import NodeCheckMetaClass
from Bloodelf.nodes.shapes.BuiltinTypeShapes import (
    tshape_bool,
    tshape_bytes,
    tshape_dict,
    tshape_int,
...
from Bloodelf.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "Bloodelf.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_dict",
    Bloodelf.specs.BuiltinDictOperationSpecs,
    python2_dict_methods,
    python3_dict_methods,
    ("fromkeys",),
)
...
processTypeShapeAttribute(
    "tshape_str",
    Bloodelf.specs.BuiltinStrOperationSpecs,
    python2_str_methods,
    python3_str_methods,
)
...
processTypeShapeAttribute(
    "tshape_bytes",
    Bloodelf.specs.BuiltinBytesOperationSpecs,
    (),
    python3_bytes_methods,
)
...
processTypeShapeAttribute(
    "tshape_list",
    Bloodelf.specs.BuiltinListOperationSpecs,
    python2_list_methods,
    python3_list_methods,
)
...
processTypeShapeAttribute(
    "tshape_type",
    Bloodelf.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(Bloodelf.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "Bloodelf/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from Bloodelf.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from Bloodelf.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from Bloodelf.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from Bloodelf.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from Bloodelf.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from Bloodelf.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "Bloodelf/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "Bloodelf/nodes/ChildrenHavingMixins.py"
    filename_python2 = "Bloodelf/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "Bloodelf/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from Bloodelf.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "Bloodelf/nodes/HardImportNodesGenerated.py"
