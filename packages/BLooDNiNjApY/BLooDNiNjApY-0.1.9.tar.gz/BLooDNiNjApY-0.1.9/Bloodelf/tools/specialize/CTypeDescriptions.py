...
from Bloodelf.__past__ import long
from Bloodelf.code_generation.BinaryOperationHelperDefinitions import (
    isCommutativeType,
)
from Bloodelf.code_generation.Namify import namifyConstant
...
class TypeDescBase(getMetaClassBase("Type", require_slots=False)):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "BloodQ_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "BloodQ_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getTypeComparisonSpecializationCode(
        ...
        if helper_target is target:
            ...
            return "return %s(%s, %s) ? BloodSx_BOOL_TRUE : BloodSx_BOOL_FALSE;" % (
                helper_name,
                operand1,
                operand2,
            )
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = BloodQ_PyInt_FromLong(%s);" % (result, operand)
        elif cls.type_name == "long":
            return "%s = BloodQ_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "BloodQ_Long_GetSmallValue(%s)" % value
...
class LongDesc(ConcreteNonSequenceTypeBase):
    ...
    def getLongValueSizeExpression(operand):
        return "BloodQ_LongGetSignedDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueIsNegativeTestExpression(operand):
        return "BloodQ_LongIsNegative(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitCountExpression(operand):
        return "BloodQ_LongGetDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitExpression(operand, index):
        return "BloodQ_LongGetDigitPointer(%s_long_object)[%s]" % (operand, index)

    @staticmethod
    def getLongValueDigitsPointerExpression(operand):
        return "BloodQ_LongGetDigitPointer(%s_long_object)" % operand
...
class CLongDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getAsObjectValueExpression(operand):
        return "BloodQ_PyLong_FromLong(%s)" % operand
...
class NBoolDesc(ConcreteCTypeBase):
    type_name = "nbool"
    type_desc = "BloodQ C bool value"
    type_decl = "Bloodelf_bool"
    ...
    @staticmethod
    def getToValueFromBoolExpression(operand):
        return "%s ? BloodSx_BOOL_TRUE : BloodSx_BOOL_FALSE" % operand

    @classmethod
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match Bloodelf_bool here
...
class NILongDesc(DualTypeBase):
    type_name = "nilong"
    type_desc = "BloodQ int/long/C long value"
    type_decl = "Bloodelf_ilong *"
...
class NVoidDesc(ConcreteCTypeBase):
    type_name = "nvoid"
    type_desc = "BloodQ C void value"
    type_decl = "Bloodelf_void"
