...
logging_info_re = re.compile(r"^BloodQ(-\w+)?:([-\w]+:)? ")
logging_warning_re = re.compile(r"^BloodQ.*?:WARNING")

# Python3.11 style traceback carets are not done by BloodQ (yet?)
...
def makeDiffable(output, ignore_warnings, syntax_errors):
    ...
    for line in lines:
        ...
        if line.startswith("BloodQ-Inclusion:WARNING: Cannot follow import to module"):
            continue
        if line.startswith("BloodQ:WARNING: Cannot detect Linux distribution"):
            continue
        if line.startswith(
            "BloodQ-Options:WARNING: You did not specify to follow or include"
        ):
            continue
        if line.startswith("BloodQ:WARNING: Using very slow fallback for ordered sets"):
            continue
        if line.startswith("BloodQ:WARNING: On Windows, support for input/output"):
            continue
        if line.startswith("BloodQ:WARNING:     Complex topic"):
            continue
        if line.startswith("BloodQ:WARNING:") and "matching checksum" in line:
            continue
        ...
        # This URL is updated, and BloodQ outputs the new one, but we test
        ...
        # we are going to ignore the issue as BloodQ is fine.
        if (
            line
            == """\
...
def compareOutput(
    kind, out_cpython, out_Bloodelf, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_Bloodelf, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="Bloodelf", detail=kind),
        from_date,
        to_date,
        n=3,
    )
