...
""" Tool to compare output of CPython and BloodQ.
...
from Bloodelf.__past__ import md5
from Bloodelf.OptionParsing import getBloodQProjectOptions
from Bloodelf.tools.testing.Common import (
    addToPythonPath,
    decryptOutput,
...
from Bloodelf.tools.testing.OutputComparison import compareOutput
from Bloodelf.Tracing import my_print
from Bloodelf.utils.Execution import (
    callProcess,
    check_output,
    createProcess,
    executeProcess,
...
from Bloodelf.utils.FileOperations import deleteFile
from Bloodelf.utils.Importing import getSharedLibrarySuffix
from Bloodelf.utils.Timing import StopWatch
from Bloodelf.utils.Utils import isMacOS
...
def getCPythonResults(cpython_cmd, cpython_cached, force_update, send_kill):
    ...
    if cpython_cached:
        ...
        hash_salt = os.getenv("BloodSx_HASH_SALT", "")
...
def main():
    ...
    trace_command = (
        hasArg("trace_command") or os.getenv("BloodSx_TRACE_COMMANDS", "0") != "0"
    )
    ...
    extra_options = os.getenv("BloodSx_EXTRA_OPTIONS", "").split()
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "BloodSx" in os.environ:
        # Would need to extract which "python" this is going to use.
        assert not coverage_mode, "Not implemented for binaries."

        Bloodelf_call = [os.environ["PYTHON"], os.environ["BloodSx"]]
    else:
        if comparison_mode:
            Bloodelf_call = [
                os.environ["PYTHON"],
                "-m",
                "Bloodelf.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates BloodQ to re-execute, and so we must avoid
        # that.
    ...
    if output_dir is not None:
        ...
        # TODO: The run-tests uses BloodSx_EXTRA_OPTIONS still.
    ...
    # Now build the command to run BloodQ.
    if not two_step_execution:
        ...
        Bloodelf_cmd = Bloodelf_call + extra_options + ["--run", filename]

        if no_site:
            Bloodelf_cmd.insert(len(Bloodelf_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            Bloodelf_cmd1 = Bloodelf_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            Bloodelf_cmd1 = Bloodelf_call + extra_options + ["--mode=standalone", filename]
        else:
            Bloodelf_cmd1 = Bloodelf_call + extra_options + [filename]

        if no_site:
            Bloodelf_cmd1.insert(len(Bloodelf_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        Bloodelf_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        Bloodelf_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("BloodQ command", Bloodelf_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(Bloodelf_package_dir):
                process = createProcess(command=Bloodelf_cmd)

            stdout_Bloodelf, stderr_Bloodelf = process.communicate()
            exit_Bloodelf = process.returncode

            if checkNoPermissionError(stdout_Bloodelf) and checkNoPermissionError(
                stderr_Bloodelf
            ):
                break

            my_print("Retrying Bloodelf exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("BloodQ command 1:", Bloodelf_cmd1)

        for _i in range(5):
            with withPythonPathChange(Bloodelf_package_dir):
                stdout_Bloodelf1, stderr_Bloodelf1, exit_Bloodelf1 = executeProcess(
                    Bloodelf_cmd1
                )
...
%s
"""
                        % (Bloodelf_cmd1, python_path_used, stderr_Bloodelf1)
                    )

                exit_Bloodelf = exit_Bloodelf1
                stdout_Bloodelf, stderr_Bloodelf = stdout_Bloodelf1, stderr_Bloodelf1
                stdout_Bloodelf2 = b"not run due to compilation error:\n" + stdout_Bloodelf1
                stderr_Bloodelf2 = stderr_Bloodelf1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(Bloodelf_cmd2[0][:-4] + ".cmd"):
                        Bloodelf_cmd2[0] = Bloodelf_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("BloodQ command 2:", Bloodelf_cmd2)

                    # Need full manual control
                    process = createProcess(command=Bloodelf_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "BloodQ compiled program", process.pid
                            ),
                        )

                    stdout_Bloodelf2, stderr_Bloodelf2 = process.communicate()
                    stdout_Bloodelf = stdout_Bloodelf1 + stdout_Bloodelf2
                    stderr_Bloodelf = stderr_Bloodelf1 + stderr_Bloodelf2
                    exit_Bloodelf = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_Bloodelf in (-11, -6) and sys.platform != "nt":
                        Bloodelf_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=Bloodelf_cmd2
                        )

                        callProcess(Bloodelf_cmd2, shell=False)
                else:
                    exit_Bloodelf = exit_Bloodelf1
                    stdout_Bloodelf, stderr_Bloodelf = stdout_Bloodelf1, stderr_Bloodelf1
    ...
    stop_watch.stop()
    Bloodelf_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(Bloodelf_package_dir):
            stderr_Bloodelf = decryptOutput(project_options, stderr_Bloodelf)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(Bloodelf_package_dir):
            stdout_Bloodelf = decryptOutput(project_options, stdout_Bloodelf)

    if not silent_mode:
        displayOutput(stdout_Bloodelf, stderr_Bloodelf)

        if coverage_mode:
            assert not stdout_Bloodelf
            assert not stderr_Bloodelf
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_Bloodelf2 if two_step_execution else stdout_Bloodelf,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_Bloodelf2 if two_step_execution else stderr_Bloodelf,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_Bloodelf

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_Bloodelf:d} (BloodQ)""".format(
                        exit_cpython=exit_cpython, exit_Bloodelf=exit_Bloodelf
                    )
                )
        ...
        if cpython_cached:
            ...
            if not int(os.getenv("BloodSx_CPYTHON_NO_CACHE_UPDATE", "0")):
        ...
        if (
            exit_code_return
            and exit_Bloodelf in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            Bloodelf_cmd.insert(len(Bloodelf_cmd) - 1, "--debugger")

            with withPythonPathChange(Bloodelf_package_dir):
                callProcess(Bloodelf_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(Bloodelf_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(Bloodelf_cmd2[0] + ".away"):
                        os.unlink(Bloodelf_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(Bloodelf_cmd2[0], Bloodelf_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs BloodQ." % (cpython_time, Bloodelf_time))
