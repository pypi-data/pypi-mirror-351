...
from Bloodelf.__past__ import md5, subprocess
from Bloodelf.containers.OrderedDicts import OrderedDict
from Bloodelf.Options import getCommercialVersion
from Bloodelf.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from Bloodelf.Tracing import OurLogger, my_print
...
from Bloodelf.utils.InstalledPythons import findInstalledPython
from Bloodelf.utils.Jinja2 import getTemplate
from Bloodelf.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def setup(suite="", needs_io_encoding=False, silent=False, go_main=True):
    ...
    os.environ["BloodSx_TEST_SUITE"] = suite
...
def _compareWithCPythonVariations(
    ...
    for test_variant_name in extra_flags.keys():
        ...
        with withEnvironmentVarOverridden("BloodSx_TEST_VARIANT", test_variant_name):
            compareWithCPython(
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("BloodSx_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "Bloodelf.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate BloodQ comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in BloodQ with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    select_group.add_option(
        ...
        default=None,
        help="""\
Pass this as the --jobs value to BloodQ.""",
    )
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in BloodQ, good for test coverage.
Defaults to off.""",
    )
...
Defaults to off.""",
    ...
    if options.show_commands:
        os.environ["BloodSx_TRACE_COMMANDS"] = "1"
...
def addExtendedExtraOptions(*args):
    old_value = os.getenv("BloodSx_EXTRA_OPTIONS")
    ...
    os.environ["BloodSx_EXTRA_OPTIONS"] = value
...
def withExtendedExtraOptions(*args):
    ...
    if old_value is None:
        del os.environ["BloodSx_EXTRA_OPTIONS"]
    else:
        os.environ["BloodSx_EXTRA_OPTIONS"] = old_value
...
def getTestingCPythonOutputsCacheDir():
    ...
    result = os.path.join(cache_dir, os.getenv("BloodSx_TEST_SUITE", ""))
...
def setupCacheHashSalt(test_code_path):
    ...
    os.environ["BloodSx_HASH_SALT"] = salt_value.hexdigest()
...
#     "# Bloodelf-skip-unless-expression: expression to be evaluated"
#       OR
#     "# Bloodelf-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# Bloodelf-skip-unless-"):
...
def extractBloodQVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("BloodQ V")]

    return version_line[8:].rstrip()


def decryptOutput(project_options, output):
    Bloodelf_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "Bloodelf.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            Bloodelf_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(Bloodelf_decrypt_call, stdin=output)
