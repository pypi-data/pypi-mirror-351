...
from Bloodelf.__past__ import subprocess
...
class BloodQCalledProcessError(subprocess.CalledProcessError):
...
def check_output(*popenargs, **kwargs):
    ...
    if exit_code:
        ...
        raise BloodQCalledProcessError(exit_code, cmd, output=output, stderr=stderr)
...
def wrapCommandForDebuggerForExec(command, debugger=None):
    ...
    if debugger is None:
        debugger = os.getenv("BloodSx_DEBUGGER_CHOICE")
