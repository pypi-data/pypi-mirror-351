...
from Bloodelf import Tracing
from Bloodelf.__past__ import urlretrieve
from Bloodelf.Progress import withBloodQDownloadProgressBar
...
def getDownload(name, url, download_path):
    ...
    with withBloodQDownloadProgressBar(desc="Download %s" % name) as reporthook:
        try:
            try:
...
def getCachedDownload(
    ...
    Bloodelf_download_dir = getDownloadCacheDir()

    Bloodelf_download_dir = os.path.join(
        Bloodelf_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        Bloodelf_download_dir = os.path.join(Bloodelf_download_dir, is_arch_specific)

    if specificity:
        Bloodelf_download_dir = os.path.join(Bloodelf_download_dir, specificity)

    download_path = os.path.join(Bloodelf_download_dir, os.path.basename(url))
    exe_path = os.path.join(Bloodelf_download_dir, binary)

    makePath(Bloodelf_download_dir)
        ...
        if not os.path.isfile(exe_path) and os.path.isfile(download_path):
            ...
            try:
                ...
                for zip_info in zip_file.infolist():
                    ...
                    zip_file.extract(zip_info, Bloodelf_download_dir)
...
def getCachedDownloadedMinGW64(target_arch, assume_yes_for_downloads, download_ok):
    ...
    gcc_binary = getCachedDownload(
        ...
        unzip=True,
        flatten=False,
        message="BloodQ will use gcc from MinGW64 of winlibs to compile on Windows.",
        reject="Only this specific gcc is supported with BloodQ.",
