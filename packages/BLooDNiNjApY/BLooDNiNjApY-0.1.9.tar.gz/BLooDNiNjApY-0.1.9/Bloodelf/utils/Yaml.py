...
from Bloodelf.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get BloodQ package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.Bloodelf-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "Bloodelf.plugins.standard",
                "stdlib2.Bloodelf-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "Bloodelf.plugins.standard",
                "stdlib3.Bloodelf-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "Bloodelf.plugins.commercial", "commercial.Bloodelf-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for BloodQ package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "Bloodelf-package-config-schema.json",
    )
