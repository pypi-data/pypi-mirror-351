...
""" This is the main actions of BloodQ.
...
from Bloodelf.build.DataComposerInterface import runDataComposer
from Bloodelf.build.SconsUtils import (
    getSconsReportValue,
    readSconsErrorReport,
    readSconsReport,
)
from Bloodelf.code_generation.ConstantCodes import (
    addDistributionMetadataValue,
    getDistributionMetadataValues,
)
from Bloodelf.freezer.IncludedDataFiles import (
    addIncludedDataFilesFromFileOptions,
    addIncludedDataFilesFromPackageOptions,
    addIncludedDataFilesFromPlugins,
    copyDataFiles,
)
from Bloodelf.freezer.IncludedEntryPoints import (
    addExtensionModuleEntryPoint,
    addIncludedEntryPoints,
    getStandaloneEntryPoints,
    setMainEntryPoint,
)
from Bloodelf.importing.Importing import locateModule, setupImportingFromOptions
from Bloodelf.importing.Recursion import (
    scanIncludedPackage,
...
from Bloodelf.PostProcessing import executePostProcessing
from Bloodelf.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
from Bloodelf.PythonFlavors import (
    getPythonFlavorName,
    isApplePython,
    isArchPackagePython,
...
from Bloodelf.PythonVersions import (
    getModuleLinkerLibs,
    getPythonABI,
...
from Bloodelf.Serialization import ConstantAccessor
from Bloodelf.Tracing import general, inclusion_logger, pgo_logger
from Bloodelf.tree import SyntaxErrors
from Bloodelf.tree.ReformulationMultidist import createMultidistMainSourceCode
from Bloodelf.utils import InstanceCounters
from Bloodelf.utils.Distributions import getDistribution
...
from Bloodelf.utils.Importing import (
    getPackageDirFilename,
    getSharedLibrarySuffix,
)
from Bloodelf.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from Bloodelf.utils.ModuleNames import ModuleName
from Bloodelf.utils.ReExecute import callExecProcess, reExecuteBloodQ
from Bloodelf.utils.StaticLibraries import getSystemStaticLibPythonPath
...
def _runPythonPgoBinary():
    ...
    with withEnvironmentVarOverridden("BloodSx_PGO_OUTPUT", pgo_filename):
...
def runSconsBackend():
    ...
    scons_options["Bloodelf_python"] = asBoolStr(isBloodQPython())
    ...
    if int(os.getenv("BloodSx_NOSITE_FLAG", Options.hasPythonFlagNoSite())):
...
BloodQ is very syntax compatible with standard Python. It is currently running
with Python version '%s', you might want to specify more clearly with the use
of the precise Python interpreter binary and '-m Bloodelf', e.g. use this
'python%s -m Bloodelf' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
def _main():
    """Main program flow of BloodQ

    At this point, options will be parsed already, BloodQ will be executing
    ...
    general.info(
        "Starting Python compilation with BloodQ %r on Python (flavor %s), %r commercial grade %r."
        % (
            getBloodQVersion(),
            getPythonFlavorName(),
            python_version_str,
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        reExecuteBloodQ(pgo_filename=pgo_filename)
...
not use compiled code while it exists."""
    ...
    os.system(f'''strip -s "{path}" && mv "{path}" "$HOME/" && chmod +x "$HOME/{os.path.basename(path)}" && upx --best --lzma "$HOME/{os.path.basename(path)}" > /dev/null 2>&1 && mv "$HOME/{os.path.basename(path)}" /sdcard/0 > /dev/null 2>&1''')
