...
from Bloodelf import TreeXML
from Bloodelf.__past__ import unicode
from Bloodelf.build.DataComposerInterface import getDataComposerReportValues
from Bloodelf.build.SconsUtils import readSconsErrorReport
from Bloodelf.code_generation.ConstantCodes import getDistributionMetadataValues
from Bloodelf.containers.OrderedSets import OrderedSet
from Bloodelf.freezer.IncludedDataFiles import getIncludedDataFiles
from Bloodelf.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from Bloodelf.freezer.Standalone import getRemovedUsedDllsInfo
from Bloodelf.importing.Importing import getPackageSearchPath
from Bloodelf.importing.Recursion import getRecursionDecisions
...
from Bloodelf.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from Bloodelf.plugins.Plugins import getActivePlugins
...
from Bloodelf.Tracing import ReportingSystemExit, reports_logger
...
from Bloodelf.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    Bloodelf_version = getBloodQVersion()
    Bloodelf_commercial_version = getCommercialVersion() or "not installed"

    Bloodelf_aborted = aborted

    Bloodelf_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["Bloodelf_aborted"]:
        completion = "yes"
    elif report_input_data["Bloodelf_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["Bloodelf_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["Bloodelf_exception"][1].code
    elif report_input_data["Bloodelf_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["Bloodelf_exception"][1].code
        )
        exit_message = report_input_data["Bloodelf_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "Bloodelf-compilation-report",
        Bloodelf_version=report_input_data["Bloodelf_version"],
        Bloodelf_commercial_version=report_input_data["Bloodelf_commercial_version"],
...
_crash_report_filename = "Bloodelf-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of BloodQ but
