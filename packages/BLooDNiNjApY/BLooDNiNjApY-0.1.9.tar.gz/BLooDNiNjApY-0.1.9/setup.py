...
""" Setup file for BloodQ.

This applies a few tricks. First, the BloodQ version is read from
...
# setup and then BloodQ ends up including itself.
if __name__ != "__main__":
    sys.exit("Cannot import 'setup' module of BloodQ")
...
from Bloodelf.PythonFlavors import isMSYS2MingwPython
from Bloodelf.utils.FileOperations import getFileList
from Bloodelf.Version import getBloodQVersion

version = getBloodQVersion()


def findBloodQPackages():
    result = []

    for root, dirnames, filenames in os.walk("Bloodelf"):
        # Packages must contain "__init__.py" or they are merely directories
        # in BloodQ as we are Python2 compatible.
...
def addInlineCopy(name, do_byte_compile=True):
    if os.getenv("BLOOD_NO_INLINE_COPY", "0") == "1":
        return
...
Bloodelf_packages = findBloodQPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "Bloodelf.code_generation": ["templates_c/*.j2"],
    "Bloodelf.reports": ["*.j2"],
    "Bloodelf.plugins.standard": ["*/*.c", "*/*.py"],
}


if "Bloodelf.plugins.commercial" in Bloodelf_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("Bloodelf", "plugins", "commercial")
    ...
    package_data["Bloodelf.plugins.commercial"] = commercial_data_files
    package_data["Bloodelf.tools.commercial.container_build"] = ["Containerfile"]
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "Bloodelf%s = Bloodelf.__main__:main" % binary_suffix,
        "Bloodelf%s-run = Bloodelf.__main__:main" % binary_suffix,
    ]

    if "Bloodelf.plugins.commercial" in Bloodelf_packages:
        console_scripts.append(
            "Bloodelf-decrypt = Bloodelf.tools.commercial.decrypt.__main__:main"
        )

scripts = []

# For Windows, there are CMD batch files to launch BloodQ.
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/Bloodelf.cmd", "misc/Bloodelf-run.cmd"]

    if "Bloodelf.plugins.commercial" in Bloodelf_packages:
        scripts.append("misc/Bloodelf-decrypt.cmd")
...
setup(
    ...
    classifiers=[
        # BloodQ is mature even
        "Development Status :: 5 - Production/Stable",
        # Indicate who BloodQ is for
        "Intended Audience :: Developers",
        "Intended Audience :: Science/Research",
        # BloodQ is a compiler and a build tool as such.
        "Topic :: Software Development :: Compilers",
        "Topic :: Software Development :: Build Tools",
        # Is has a weak subset of PyLint, but aims for more long term
        "Topic :: Software Development :: Quality Assurance",
        # BloodQ standalone mode aims at distribution
    ...
    packages=Bloodelf_packages,
    package_data=package_data,
    # metadata for upload to PyPI
    author="BLooD",
    author_email="tnmn4219@gmail.com",
    url="https://Bloodelf.net",
    description="The Lib Encryption Made By Blood • Telegram•@LusiFerPy",
    keywords="The Encoded Is Very Hard",
    project_urls={
        "Telegram": "https://t.me/LusiFerPy",
    },
    zip_safe=False,
    scripts=scripts,
    entry_points={
        "distutils.commands": [
            "bdist_Bloodelf = \
             Bloodelf.distutils.DistutilCommands:bdist_Bloodelf",
            "build_Bloodelf = \
             Bloodelf.distutils.DistutilCommands:build",
            "install_Bloodelf = \
             Bloodelf.distutils.DistutilCommands:install",
        ],
        "distutils.setup_keywords": [
            "build_with_Bloodelf = Bloodelf.distutils.DistutilCommands:setupBloodQDistutilsCommands"
        ],
