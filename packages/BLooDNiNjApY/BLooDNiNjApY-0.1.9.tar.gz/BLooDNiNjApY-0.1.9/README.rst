.. image:: https://img.shields.io/pypi/pyversions/BloodQ.svg
   :target: https://pypi.org/project/BloodQ

.. image:: https://badge.fury.io/py/BloodQ.svg
   :target: https://pypi.org/project/BloodQ
...
####################
 BloodQ User Manual
####################
You use BLooDNiNjApY library this
 **VIP and STRONG** library,
It converts to a real dwarf
, not a fake one as in GSS or Clang.
It also contains protection against reverse
engineering attacks
and information analysis, 
and it contains encryption and information hiding. By BLooD **@LusiFerPy**
####################

This document is the recommended first read when you start using
**BloodQ**. On this page, you will learn more about **BloodQ**
...
BloodQ is **the** Python compiler. It is written in Python. It is a
...
BloodQ translates the Python modules into a C level program that then
...
To ensure smooth operation of **BloodQ**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as BloodQ
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (BloodQ filters away garbage outputs, but only for
...
.. important::
   ...
   BloodQ itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as BloodQ.
   ...
   BloodQ finds these needed Python versions (e.g. on Windows via
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute BloodQ because it is closely tied to implementation details
   of it.
...
good, but the e.g. BloodQ's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as BloodQ
...
The recommended way of executing BloodQ is ``<the_right_python> -m
Bloodelf`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what BloodQ has.

The next best way of executing BloodQ bare that is from a source
...
BloodQ. You just execute the ``Bloodelf`` and ``Bloodelf-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/Bloodelf`` and be good.
...
BloodQ has a ``--help`` option to output what it can do:

.. code:: bash

   Bloodelf --help

The ``Bloodelf-run`` command is the same as ``Bloodelf``, but with a
...
<https://Bloodelf.net/doc/download.html>`__ of BloodQ. But you can also
...
`BloodQ-Action <https://github.com/BloodQ/BloodQ-Action>`__ that you
...
platform compilation with BloodQ.

License
=======

BloodQ is licensed under the Apache License, Version 2.0; you may not
...
-  ``python -m pip install Bloodelf``

-  Verify using command ``python -m Bloodelf --version``
...
Do as you normally would. Running BloodQ on code that works incorrectly
...
.. code:: bash
   ...
   python -m Bloodelf --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``Bloodelf --help``. Including fewer
...
.. code:: bash
   ...
   python -m Bloodelf --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. note::
   ...
   calls that BloodQ cannot predict, and that come from a directory, for
...
.. code:: bash
   ...
   python -m Bloodelf --mode=module some_module.py
...
.. note::
   ...
   BloodQ, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use BloodQ like this:

.. code:: bash

   python -m Bloodelf --mode=module some_package --include-package=some_package
...
.. note::
   ...
   Alternatively, you can use the `file embedding of BloodQ commercial
   <https://Bloodelf.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m Bloodelf --standalone program.py
...
.. important::
   ...
   BloodQ does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | BloodQ trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create BloodQ Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add BloodQ Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. BloodQ keeps track of ones
...
enhancements to the BloodQ package configuration. We want 3rd party
...
.. code:: bash
   ...
   python -m Bloodelf --onefile program.py
...
creation of wheels for your software in place, putting BloodQ to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having BloodQ installed of course, simply execute the target
``bdist_Bloodelf`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not BloodQ
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass BloodQ options.
      command_options={
         'Bloodelf': {
            ...
            '--clang': None,
            # options with single values, e.g. enable a plugin of BloodQ
...
.. note::
   ...
   ``bool(os.getenv("USE_BloodSx", "True"))``. This is up to you.
...
And last, but not least, BloodQ also supports the new ``build`` meta, so
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "Bloodelf", "toml"]
   build-backend = "Bloodelf.distutils.Build"
...
.. note::
   ...
   For the ``Bloodelf`` requirement above absolute paths like
   ``C:\Users\...\BloodQ`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../BloodQ``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by BloodQ at all, but by setuptools. You can, however, use
   the data file embedding of BloodQ commercial. In that case, you
...
When given multiple times, BloodQ will create a binary that contains the
...
For integration with GitHub workflows there is this `BloodQ-Action
<https://github.com/BloodQ/BloodQ-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with BloodQ
         uses: BloodQ/BloodQ-Action@main
         with:
            Bloodelf-version: main
            script-name: your_main_program.py
            # many more BloodQ options available, see action doc, but it's best
            # to use Bloodelf-project: options in your code, so e.g. you can make
...
comments as explained in `BloodQ Options in the code`_ since on macOS
this should then be a bundle.

.. code:: python

   # Compilation mode, standalone everywhere, except on macOS there app bundle
   # Bloodelf-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    Bloodelf-project: --onefile
   # Bloodelf-project-if: {OS} == "Darwin":
   #    Bloodelf-project: --standalone
   #    Bloodelf-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``Bloodelf-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m Bloodelf --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m Bloodelf --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m Bloodelf --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m Bloodelf --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m Bloodelf --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py

.. note::

   With BloodQ, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to BloodQ
   and not be interpreted as BloodQ arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
BloodQ defaults to not show it, you can force it by using
...
them. Luckily, with BloodQ, they are easy to add for Windows.
...
.. code:: python
   ...
   # Bloodelf-project: --mode=onefile
   # Bloodelf-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
   ...
   if "BloodSx_ONEFILE_PARENT" in os.environ:
      splash_filename = os.path.join(
         tempfile.gettempdir(),
         "onefile_%d_splash_feedback.tmp" % int(os.environ["BloodSx_ONEFILE_PARENT"]),
      )
...
For analysis of your program and BloodQ packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to BloodQ. These
...
By default, BloodQ compiles without ``--deployment`` which leaves a set
...
With BloodQ however, this executes your program again, and puts these
...
That is why e.g. this happens with default BloodQ:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with BloodQ to include it'
...
re-enable it. You might want to use BloodQ Project options and an
...
Binaries compiled on Windows with default settings of BloodQ and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in BloodQ commercial there is actual support
and instructions for how to do it, seeing this as a typical commercial
only need. https://Bloodelf.net/doc/commercial.html
...
To aid that, BloodQ commercial has container based builds, that you can
...
compensated to support BloodQ development for Linux, there you need to
purchase it https://Bloodelf.net/doc/commercial.html but even a sponsor
...
better way, often relaunch themselves through this, and BloodQ needs and
...
.. code:: python
   ...
   if "BloodSx_LAUNCH_TOKEN" not in os.environ:
      sys.exit("Error, need launch token or else fork bomb suspected.")
   else:
      del os.environ["BloodSx_LAUNCH_TOKEN"]

Actually BloodQ is trying to get ahold of them without the deployment
...
Ask BloodQ to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of BloodQ, such that it avoids high usage of memory during compilation
...
then have imports on these, and BloodQ will compile them as well. Not
...
With the ``--jobs`` option of BloodQ, it will not start many C compiler
...
source code relative to it, BloodQ will not be able to see those.
...
BloodQ has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `BloodQ Package
Configuration <https://Bloodelf.net/doc/Bloodelf-package-config.html>`__
page.

Dependency creep in standalone
==============================

Some packages are a single import, but to BloodQ mean that more than a
...
BloodQ will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards BloodQ with it.
...
A package can be compiled with BloodQ, no problem, but when it comes to
...
would need improving such that BloodQ could supply its compiled module
...
BloodQ Options in the code
==========================

One clean way of providing options to BloodQ, that you will always use
...
.. code:: python
   ...
   # Bloodelf-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    Bloodelf-project: --mode=app
   # Bloodelf-project-else:
   #    Bloodelf-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # Bloodelf-project: --enable-plugin=pyside2
   # Bloodelf-project: --include-qt-plugins=qml
...
e.g. enable or disable certain BloodQ settings. The only thing BloodQ
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of BloodQ              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of BloodQ Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
.. code:: python
   ...
   # Bloodelf-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # Bloodelf-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.Bloodelf-package.config.yml
...
makes BloodQ emulate these options.
...
On Windows, with gcc, BloodQ supports using ``ccache.exe``, which it
...
BloodQ will pick up ``ccache`` if it's found in system ``PATH``, and it
will also be possible to provide if by setting ``BloodSx_CCACHE_BINARY``
...
automatic and included in BloodQ.
...
setup, which installs Homebrew and ccache in there. BloodQ picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with BloodQ otherwise, it's not the best for standalone
...
compilation results from C and BloodQ, is done in a platform dependent
directory as determined by the ``appdirs`` package. However, you can
override it with setting the environment variable ``BloodSx_CACHE_DIR``
...
to make BloodQ store some of these caches in an entirely separate space.

+------------------+-----------------------------------+----------------------------------------+
| Cache name       | Environment Variable              | Data Put there                         |
+==================+===================================+========================================+
| downloads        | BloodSx_CACHE_DIR_DOWNLOADS        | Downloads made, e.g. dependency walker |
+------------------+-----------------------------------+----------------------------------------+
| ccache           | BloodSx_CACHE_DIR_CCACHE           | Object files created by gcc            |
+------------------+-----------------------------------+----------------------------------------+
| clcache          | BloodSx_CACHE_DIR_CLCACHE          | Object files created by MSVC           |
+------------------+-----------------------------------+----------------------------------------+
| bytecode         | BloodSx_CACHE_DIR_BYTECODE         | Bytecode of demoted modules            |
+------------------+-----------------------------------+----------------------------------------+
| dll-dependencies | BloodSx_CACHE_DIR_DLL_DEPENDENCIES | DLL dependencies                       |
+------------------+-----------------------------------+----------------------------------------+

Runners
=======

Avoid running the ``Bloodelf`` binary, doing ``python -m Bloodelf`` will
...
BloodQ with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by BloodQ
properly.
...
both scan the freshly created binaries, while BloodQ wants to work with
...
external dependencies to Visual C Runtime libraries. BloodQ tries to
...
installing Visual C runtime libraries before executing a BloodQ
...
When using MingGW64 as downloaded by BloodQ, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your BloodQ
compiled dist folder.

Detecting BloodQ at run time
============================

BloodQ does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For BloodQ, we have the module
...
Providing extra Options to BloodQ C compilation
===============================================

BloodQ will apply values from the environment variables ``CCFLAGS``,
...
BloodQ will automatically target the architecture of the Python you are
...
bits when you download the Python. In the output of ``python -m Bloodelf
...
When you use ``--report=compilation-report.xml`` BloodQ will create an
...
changes to BloodQ. The report is, however, recommended for your bug
reporting.
...
BloodQ. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. BloodQ will just expand
the template with the compilation report data.

Currently, the following reports are included in BloodQ. You just use
the name as a filename, and BloodQ will pick that one instead.
...
performance from BloodQ. It's a work in progress and is updated as we
...
.. code:: bash
   ...
   python2 -m Bloodelf --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m Bloodelf --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
...
+-------------------+-------------------+----------------------+---------------------+
| BloodQ Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`BloodQ bug tracker <https://github.com/BloodQ/BloodQ/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or BloodQ source and re-run it.
...
bytecode with BloodQ's compiled function objects, so there is no way to
provide it.
