...
-  ``Bloodelf.tree.Building`` outputs node tree

-  ``Bloodelf.optimization`` enhances it as best as it can

-  ``Bloodelf.finalization`` prepares the tree for code generation

-  ``Bloodelf.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``Bloodelf.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``Bloodelf.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``Bloodelf_bool`` and ``Bloodelf_void`` and more are coming. Even for
...
These commands will make sure that the ``autoformat-Bloodelf-source`` is
...
There is a tool ``bin/autoformat-Bloodelf-source`` which is to apply
...
they must be lower case, like e.g. ``Bloodelf`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``Bloodelf.code_generation`` the code generation packages are located,
while the main interface is ``Bloodelf.code_generation.CodeGeneration``
...
``autoformat-Bloodelf-source`` does this for us.
...
`this location <https://Bloodelf.net/apidoc>`__ .
...
.. code:: bash
   ...
   ./bin/check-Bloodelf-with-pylint
...
parts are published as `BloodQ API <https://Bloodelf.net/apidoc>`__ and
...
``Bloodelf/tools/release/Documentation``, but are not executed each time,
...
.. note::
   ...
   ``Bloodelf.MainControl`` module.

For interfacing to Scons, there is the module
``Bloodelf.build.SconsInterface`` that will support calling ``scons`` -
...
-  ``Bloodelf_src``
   ...
   other helper codes, this will point to where ``Bloodelf.build`` package
   lives normally.
...
The search for modules used is driven by ``Bloodelf.importing.Importing``
module.

-  Quoting the ``Bloodelf.importing.Importing`` documentation:
   ...
   End quoting the ``Bloodelf.importing.Importing`` documentation.
...
The decision making and caching are located in the ``Bloodelf.tree``
package, in modules ``Bloodelf.tree.Recursion`` and
``Bloodelf.tree.ImportCache``. Each module is only considered once (then
...
``Bloodelf/build/static_src/MetaPathBasedLoader.c`` file provides the
...
.. note::
   ...
   In the actual code, ``Bloodelf.nodes.FrameNodes.StatementsFrame`` is
...
Quoting the ``Bloodelf.tree.ReformulationWhileLoopStatements``
documentation:
...
End quoting the ``Bloodelf.tree.ReformulationWhileLoopStatements``
documentation:
...
.. code:: bash
   ...
   Bloodelf --experimental=jinja_generated_add ...
...
like ``autoformat-Bloodelf-source`` is.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # Bloodelf-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from Bloodelf.tools.testing.Common import checkReferenceCount
...
And the current PDF under: https://Bloodelf.net/doc/Developer_Manual.pdf
