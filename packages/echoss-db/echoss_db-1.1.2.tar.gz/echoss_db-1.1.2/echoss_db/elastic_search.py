...
class ElasticSearch:
    ...
    def _bulk_operations(self, actions: list, chunk_size: int = 500, raise_on_error: bool = True,
                        max_retries: int = 3) -> tuple[int, list]:
        """
        주어진 액션(문서) 리스트를 특정 인덱스에 벌크로 실행합니다.
        성공/실패 결과를 반환하며, 로깅을 포함합니다.

        :param actions: Elasticsearch _bulk API에 전달할 action 리스트.
        ...
        :param chunk_size: 한 번에 전송할 문서의 개수.
        :param raise_on_error: 개별 문서 오류 발생 시 예외를 발생시킬지 여부.
                               False이면 오류가 발생해도 계속 진행하며 오류 리스트를 반환.
        :param max_retries: 네트워크/클라이언트 오류 발생 시 전체 요청에 대한 최대 재시도 횟수.
        ...
        try:
            ...
            if errors:
                ...
                for i, err_item in enumerate(errors):
                    ...
                    doc_id = err_item.get('index', {}).get('_id', 'N/A')
                    error_type = err_item.get('index', {}).get('error', {}).get('type', 'UnknownError')
                    error_reason = err_item.get('index', {}).get('error', {}).get('reason', 'No reason provided')
                    logger.error(f"  Failed item {i + 1} (ID: {doc_id}): Type={error_type}, Reason='{error_reason}'")
                    # logging.debug(f"  Full error item: {err_item}") # 디버깅용 상세 정보
    ...
    def bulk_upsert(self, index_name:str, doc_list: List[Dict], id_field: str,
