...
class formula:
    ...
    def map(self, func, *args):
        ...
        if k in (op_Or, op_OrRat, op_And, op_AndRat, op_AndNLM, op_Concat, op_Fusion):
            return formula.multop(k, [func(x, *args) for x in self])
        if k in (op_Star, op_FStar):
            return formula.bunop(k, func(self[0], *args), self.min(), self.max())
    ...
    for filename in sources:
        try:
            ...
            if filename[-1] == "|":
                ...
                if timeout is None:
                    p = automaton_stream_parser(proc.stdout.fileno(), filename, o)
                else:
                    try:
...
def _add_formula(meth, name=None):
    setattr(
        formula,
        name or meth,
        (lambda self, *args, **kwargs: globals()[meth](self, *args, **kwargs)),
    )
...
def _addmap(fun):
    def mapf(self, *args, **kwargs):
        return formulaiterator(map(lambda f: getattr(f, fun)(*args, **kwargs), self))
...
def randltl(ap, n=-1, **kwargs):
    ...
    **kwargs:
    ...
    rg = randltlgenerator(ap, opts, ltl_priorities, sere_priorities, boolean_priorities)
    ...
    if dump_priorities:
        ...
        if output == randltlgenerator.LTL:
            ...
            print(
                "Use argument boolean_priorities=STRING to set the "
            ...
            if output != randltlgenerator.SERE:
                print(
                    "Use argument ltl_priorities=STRING to set the "
                    "following LTL priorities:\n"
                )
                rg.dump_psl_priorities(dumpstream)
                print(dumpstream.str())
            print(
                "Use argument sere_priorities=STRING to set the "
    ...
    class _randltliterator:
        ...
        def __next__(self):
            ...
            if f is None:
                sys.stderr.write(
                    "Warning: could not generate a new "
...
def simplify(f, **kwargs):
    ...
    simp_opts = tl_simplifier_options(
        basics,
        synt_impl,
        event_univ,
        cont_checks,
...
for fun in dir(formula):
    if callable(getattr(formula, fun)) and (fun[:3] == "is_" or fun[:4] == "has_"):
        _addfilter(fun)
...
""".format(
        ...
        for ypos, a in enumerate(ap):
            ...
            last = 0
            txt += '<line x1="0" y1="{y}" x2="{w}" y2="{y}" {bgl}/>'.format(
                y=ypos * 50, **d
            )
            txt += '<text x="{x}" y="{y}" {txt}>{name}</text>'.format(
                x=3, y=ypos * 50 + 30, name=name, **d
            )
            for xpos, step in enumerate(l):
                if buddy.bdd_implies(step, pa):
                    ...
                    cur = 0
                txt += '<line x1="{x}" y1="{y1}" x2="{x}" y2="{y2}" {bgt}/>'.format(
                    x=(xpos + 1) * 50, y1=ypos * 50, y2=ypos * 50 + 50, **d
                )
                if cur != 0:
                    if last == -cur:
                        txt += '<line x1="{x}" y1="{y1}" x2="{x}" y2="{y2}" {red}/>'.format(
                            x=xpos * 50, y1=ypos * 50 + 5, y2=ypos * 50 + 45, **d
                        )
                    txt += '<line x1="{x1}" y1="{y}" x2="{x2}" y2="{y}" {red}/>'.format(
                        x1=xpos * 50,
                        x2=(xpos + 1) * 50,
                        y=ypos * 50 + 25 - 20 * cur,
                        **d,
...
<text x="{endcycle}" y="{height2}" {sml}>cycle</text>""".format(
            **d
        )
        return txt + "</svg>"
