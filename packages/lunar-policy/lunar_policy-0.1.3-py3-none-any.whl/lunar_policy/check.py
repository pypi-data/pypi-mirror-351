...
class Check:
    def __init__(
        ...
        if data is None:
            try:
                ...
                data = SnippetData.from_file(FSPath(path))
            except KeyError:
                raise ValueError(
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        isNoDataError = isinstance(exc_value, NoDataError)
        if exc_value is not None and not isNoDataError:
    ...
    def _get_query_result(
        ...
        if all_instances:
            results = self.data.get_all(value)
            if not results:
                return None, []
            ...
            result = self.data.get_merged(value)
            if result is None:
                return None, []
    ...
    def get(self, path: str) -> Any | None:
        ...
        result = self.data.get_merged(jsonPath)
        if result is None:
            raise NoDataError()
    ...
    def get_all(self, path: str) -> List[Any]:
        ...
        results = self.data.get_all(jsonPath)
