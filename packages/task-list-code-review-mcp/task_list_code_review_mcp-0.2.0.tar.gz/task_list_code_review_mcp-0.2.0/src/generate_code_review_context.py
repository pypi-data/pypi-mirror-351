...
def extract_prd_summary(content: str) -> str:
    ...
    if GEMINI_AVAILABLE and api_key:
        try:
            ...
            # Use configurable model for PRD summarization, default to gemini-2.0-flash-lite for fast summarization
            summary_model = os.getenv('GEMINI_SUMMARY_MODEL', 'gemini-2.0-flash-lite')
...
def send_to_gemini_for_review(context_content: str, project_path: str, temperature: float = 0.5) -> Optional[str]:
    """
    ...
    try:
        ...
        # Configure model selection (2.0 Flash by default for cost efficiency)
        model_config = os.getenv('GEMINI_MODEL', 'gemini-2.0-flash')
...
🛠️ ADVANCED OPTIONS:
  ...
  generate-code-review . --no-gemini --output /custom/path/review.md
