...
Working examples:
  ...
  review-with-ai context.md --model gemini-1.5-pro         # Reliable"""
    ...
    try:
        ...
        # Send to Gemini with model parameter if specified
        if model:
            # For now, we'll pass the model via environment variable
            # This could be enhanced to pass directly to send_to_gemini_for_review
            original_model = os.environ.get('GEMINI_MODEL')
            os.environ['GEMINI_MODEL'] = model
            try:
                gemini_output = send_to_gemini_for_review(context_content, project_path, temperature)
            finally:
