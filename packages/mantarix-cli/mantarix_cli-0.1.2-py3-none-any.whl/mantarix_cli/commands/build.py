...
import yaml
from mantarix.obfuscator import obfuscate_folder
from mantarix.utils import copy_tree, is_windows, slugify
from mantarix.utils.platform_utils import get_bool_env_var
from mantarix.version import update_version
from mantarix_cli.commands.base import BaseCommand
from mantarix_cli.utils.merge import merge_dict
from mantarix_cli.utils.project_dependencies import (
    get_poetry_dependencies,
    get_project_dependencies,
)
from mantarix_cli.utils.pyproject_toml import load_pyproject_toml
...
DEFAULT_TEMPLATE_URL = "gh:mantarix-dev/mantarix-build-template"
...
class Command(BaseCommand):
    ...
    def __init__(self, parser: argparse.ArgumentParser) -> None:
        ...
        self.pck_name = {
            "ads":"mantarix_ads",
            "audio":"mantarix_audio",
            "audio_recorder":"mantarix_audio_recorder",
            "camera":"mantarix_camera",
            "flashlight":"mantarix_flashlight",
            "geolocator":"mantarix_geolocator",
            "lightsensor":"mantarix_lightsensor",
            "lottie":"mantarix_lottie",
            "map":"mantarix_map",
            "permission_handler":"mantarix_permission_handler",
            "rive":"mantarix_rive",
            "video":"mantarix_video",
            "webview":"webview"
        }
        self.skip_flutter_doctor = get_bool_env_var("MANTARIX_CLI_SKIP_FLUTTER_DOCTOR")
        self.no_rich_output = get_bool_env_var("MANTARIX_CLI_NO_RICH_OUTPUT")
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            nargs="+",
            default=[],
            help="include extra Flutter Mantarix packages, such as mantarix_video, mantarix_audio, etc.",
        )
        parser.add_argument(
            "--info-plist",
            dest="info_plist",
    ...
    def validate_entry_point(self):
        ...
        if self.get_pyproject("tool.mantarix.app.path"):
            self.package_app_path = self.python_app_path.joinpath(
                cast(str, self.get_pyproject("tool.mantarix.app.path"))
            )

        self.python_module_name = Path(
            self.options.module_name
            or cast(str, self.get_pyproject("tool.mantarix.app.module"))
            or "main"
        ).stem
        ...
        if not self.python_module_file_path.exists():
            self.cleanup(
                1,
                f"{self.python_module_filename} not found in the root of Mantarix app directory. "
                f"Use --module-name option to specify an entry point for your Mantarix app.",
            )
        if self.obfuscate:
    ...
    def setup_template_data(
        ...
        base_url = (
            (
                ...
                or cast(str, self.get_pyproject("tool.mantarix.web.base_url"))
                or "/"
            )
            .strip("/")
            .strip()
        ...
        product_name = (
            ...
            or self.get_pyproject("tool.mantarix.product")
        ...
        split_per_abi = (
            ...
            if self.options.split_per_abi is not None
                ...
                self.get_pyproject("tool.mantarix.android.split_per_abi")
                if self.get_pyproject("tool.mantarix.android.split_per_abi") is not None
                else False
            )
        )
        ...
        for p in (
            ...
            or self.get_pyproject("tool.mantarix.permissions")
            or []
        ):
        ...
        info_plist = merge_dict(
            info_plist,
            (
                self.get_pyproject("tool.mantarix.macos.info")
                if self.package_platform == "Darwin"
                else self.get_pyproject("tool.mantarix.ios.info")
            )
            or {},
        )
        ...
        macos_entitlements = merge_dict(
            macos_entitlements,
            self.get_pyproject("tool.mantarix.macos.entitlement") or {},
        )
        ...
        android_permissions = merge_dict(
            android_permissions,
            self.get_pyproject("tool.mantarix.android.permission") or {},
        )
        ...
        android_features = merge_dict(
            android_features,
            self.get_pyproject("tool.mantarix.android.feature") or {},
        )
        ...
        android_meta_data = merge_dict(
            android_meta_data,
            self.get_pyproject("tool.mantarix.android.meta_data") or {},
        )
        ...
        deep_linking_scheme = (
            self.get_pyproject("tool.mantarix.ios.deep_linking.scheme")
            if self.package_platform == "iOS"
            else (
                self.get_pyproject("tool.mantarix.android.deep_linking.scheme")
                if self.package_platform == "Android"
                else self.get_pyproject("tool.mantarix.deep_linking.scheme")
            )
        )

        deep_linking_host = (
            self.get_pyproject("tool.mantarix.ios.deep_linking.host")
            if self.package_platform == "iOS"
            else (
                self.get_pyproject("tool.mantarix.android.deep_linking.host")
                if self.package_platform == "Android"
                else self.get_pyproject("tool.mantarix.deep_linking.host")
            )
        )
        ...
        self.template_data = {
            ...
            "route_url_strategy": (
                ...
                or self.get_pyproject("tool.mantarix.web.route_url_strategy")
                or "path"
            ),
            "web_renderer": (
                self.options.web_renderer
                or self.get_pyproject("tool.mantarix.web.renderer")
                or "canvaskit"
            ),
            "use_color_emoji": (
                "true"
                if self.options.use_color_emoji
                or self.get_pyproject("tool.mantarix.web.use_color_emoji")
                else "false"
            ),
            "pwa_background_color": (
                self.options.pwa_background_color
                or self.get_pyproject("tool.mantarix.web.pwa_background_color")
            ),
            "pwa_theme_color": (
                self.options.pwa_theme_color
                or self.get_pyproject("tool.mantarix.web.pwa_theme_color")
            ),
            ...
            "org_name": self.options.org_name or self.get_pyproject("tool.mantarix.org"),
            "company_name": (
                self.options.company_name or self.get_pyproject("tool.mantarix.company")
            ),
            "copyright": self.options.copyright
            or self.get_pyproject("tool.mantarix.copyright"),
            "team_id": self.options.team_id or self.get_pyproject("tool.mantarix.ios.team"),
            "options": {
    ...
    def get_flutter_dependencies(self) -> dict:
        ...
        flutter_dependencies = (
            self.get_pyproject("tool.mantarix.flutter.dependencies") or {}
        )
    ...
    def create_flutter_project(self):
        ...
        template_url = (
            ...
            or self.get_pyproject("tool.mantarix.template.url")
            or DEFAULT_TEMPLATE_URL
        )
        template_ref = self.options.template_ref or self.get_pyproject(
            "tool.mantarix.template.ref"
        )
        if not template_ref:
            template_ref = (
                version.Version(mantarix.version.version).base_version
                if mantarix.version.version
                else update_version()
            )
        template_dir = self.options.template_dir or self.get_pyproject(
            "tool.mantarix.template.dir"
        )
    ...
    def load_pubspec(self):
        ...
        # add local mantarix packages to a dest pubspec.yaml
        self.status.update(
        ...
        if not os.path.exists(self.mantarix_packages_path):
            ...
            console.log(
                f"Downloading Mantarix Packages From Github {self.emojis['loading']}... "
            )
            packages_zip_file = download_mantarix_client("packages.zip")
            with zipfile.ZipFile(packages_zip_file, "r") as zip_arch:
                zip_arch.extractall(self.mantarix_packages_path)
                zip_arch.close()
            os.remove(packages_zip_file)
            console.log(
                f"Removed Mantarix Packages Download cache {self.emojis['checkmark']}"
            )
        
        for dep in self.get_mantarix_extensions_imports(self.last_python_module_file_path if self.obfuscate else self.python_module_file_path):
            dep_path = os.path.join(self.mantarix_packages_path,dep)
            if os.path.exists(dep_path) and dep not in self.pubspec["dependencies"].keys():
                self.pubspec["dependencies"][dep] = {}
                self.pubspec["dependencies"][dep]["path"] = Path(dep_path).as_posix()
        console.log(
            f"Local Mantarix Packages Added To Project {self.emojis['checkmark']}"
        )

        self.pubspec = merge_dict(
            self.pubspec, self.get_pyproject("tool.mantarix.flutter.pubspec") or {}
        )
    ...
    def customize_icons_and_splash_images(self):
        ...
        if self.assets_path.exists():
            ...
            splash_dark_color = self.options.splash_dark_color or self.get_pyproject(
                "tool.mantarix.splash.dark_color"
            )
        ...
        adaptive_icon_background = (
            ...
            or self.get_pyproject("tool.mantarix.android.adaptive_icon_background")
        )
        ...
        self.pubspec["flutter_native_splash"]["web"] = (
            ...
            if self.options.no_web_splash is not None
                ...
                self.get_pyproject("tool.mantarix.splash.web")
                if self.get_pyproject("tool.mantarix.splash.web") is not None
                else True
            )
        )
        self.pubspec["flutter_native_splash"]["ios"] = (
            ...
            if self.options.no_ios_splash is not None
                ...
                self.get_pyproject("tool.mantarix.splash.ios")
                if self.get_pyproject("tool.mantarix.splash.ios") is not None
                else True
            )
        )
        self.pubspec["flutter_native_splash"]["android"] = (
            ...
            if self.options.no_android_splash is not None
                ...
                self.get_pyproject("tool.mantarix.splash.android")
                if self.get_pyproject("tool.mantarix.splash.android") is not None
                else True
            )
        )
    ...
    def package_python_app(self):
        ...
        target_arch = (
            ...
            or self.get_pyproject(f"tool.mantarix.{self.config_platform}.build_arch")
            or self.get_pyproject("tool.mantarix.build_arch")
        )
        if target_arch:
        ...
        if (
            ...
            if self.options.compile_app is not None
                ...
                self.get_pyproject("tool.mantarix.compile.app")
                if self.get_pyproject("tool.mantarix.compile.app") is not None
                else False
            )
            or
            self.obfuscate
        ...
        if (
            ...
            if self.options.compile_packages is not None
                ...
                self.get_pyproject("tool.mantarix.compile.packages")
                if self.get_pyproject("tool.mantarix.compile.packages") is not None
                else False
            )
            or
            self.obfuscate
        ...
        if (
            ...
            if self.options.cleanup_on_compile is not None
                ...
                self.get_pyproject("tool.mantarix.compile.cleanup")
                if self.get_pyproject("tool.mantarix.compile.cleanup") is not None
                else True
            )
            or
            self.obfuscate
        ...
        if not os.path.exists(app_zip_path):
            self.cleanup(1, "Mantarix app package app/app.zip was not created.")
    ...
    def flutter_build(self):
        ...
        android_signing_key_store = (
            ...
            or self.get_pyproject("tool.mantarix.android.signing.key_store")
        )
        if android_signing_key_store:
            build_env["MANTARIX_ANDROID_SIGNING_KEY_STORE"] = android_signing_key_store

        key_store_password = (
            self.options.android_signing_key_store_password
            or os.getenv("MANTARIX_ANDROID_SIGNING_KEY_STORE_PASSWORD")
        )
        key_password = self.options.android_signing_key_password or os.getenv(
            "MANTARIX_ANDROID_SIGNING_KEY_PASSWORD"
        )
        if key_store_password or key_password:
            build_env["MANTARIX_ANDROID_SIGNING_KEY_STORE_PASSWORD"] = (
                key_store_password if key_store_password else key_password
            )
            build_env["MANTARIX_ANDROID_SIGNING_KEY_PASSWORD"] = (
                key_password if key_password else key_store_password
            )

        android_signing_key_alias = (
            self.options.android_signing_key_alias
            or self.get_pyproject("tool.mantarix.android.signing.key_alias")
        )
        if android_signing_key_alias:
            build_env["MANTARIX_ANDROID_SIGNING_KEY_ALIAS"] = android_signing_key_alias
        ...
        for arg in self.get_pyproject("tool.mantarix.flutter.build_args") or []:
    ...
    def cleanup(
        ...
        if exit_code == 0:
            ...
            msg = (
                ...
                else "Error building Mantarix app - see the log of failed command above."
            )
                    ...
                    if match:
                        ...
                        if flutter_version < version.parse(MINIMAL_FLUTTER_VERSION):
                            flutter_msg = (
                                ...
                                + f"Mantarix build requires Flutter {MINIMAL_FLUTTER_VERSION} or above. "
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("mantarix.extensions"):
                        im_name = alias.name.replace("mantarix.extensions.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
            elif isinstance(node, ast.ImportFrom):
                if node.module and node.module.startswith("mantarix.extensions"):
                    for alias in node.names:
                        full_import = f"{node.module}.{alias.name}"
                        im_name = full_import.replace("mantarix.extensions.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("mantarix.core"):
                        im_name = alias.name.replace("mantarix.core.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
            elif isinstance(node, ast.ImportFrom):
                if node.module and node.module.startswith("mantarix.core"):
                    for alias in node.names:
                        full_import = f"{node.module}.{alias.name}"
                        im_name = full_import.replace("mantarix.core.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
        return imported_items

def download_mantarix_client(file_name):
    ver = version.Version(mantarix.version.version).base_version
