...
class MemoryProtocol:
    ...
    def record_memory(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> List[int]:
        if not self.initialized:
            self.initialize()
    ...
    def retrieve_memory(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        def retrieve_operation(client):
            try:
                ...
                for hit in results:
                    payload = hit.payload
                    # only load the relavent metadata : "intent", "previous_versions", "privacy_level"
                    metadata = {
                        "intent": payload["metadata"].get("intent", ""),
                        "previous_versions": payload["metadata"].get("previous_versions", "None"),
                        "privacy_level": payload["metadata"].get("privacy_level", "")
                    }
    ...
    def import_file(self, file_path: str, metadata: Dict[str, Any] = None) -> List[int]:
        ...
        try:
            ...
            # if the file ends with gpt.txt then just split with next line
            if file_path.lower().endswith('gpt.txt'):
                chunks = [m for m in content.split('\n') if len(m) > 10]
            elif is_markdown:
    ...
    def export_memories(self, export_path: Optional[str] = None, limit: int = 1000) -> str:
        """
        Export all (or up to `limit`) memory records to a JSON file.
        Args:
            export_path: Optional path to save the export. If None, defaults to app dir/qdrant_export.json
        ...
        def export_operation(client):
            ...
            for record in vectors:
                ...
                serializable_vectors.append(serializable_record)

            if export_path is None:
                export_path_final = str(get_app_dir() / "qdrant_export.json")
            else:
                export_path_final = export_path

            with open(export_path_final, "w", encoding="utf-8") as f:
                json.dump(serializable_vectors, f, indent=2, ensure_ascii=False)
            return export_path_final

        return self._with_qdrant(export_operation) 