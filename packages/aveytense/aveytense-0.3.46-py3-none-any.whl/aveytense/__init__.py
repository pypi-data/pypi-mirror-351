...
Documentation: https://aveyzan.glitch.me/aveytense

Submodules:
- `aveytense.types_collection` - types collection
- `aveytense.constants` - constants collection
- `aveytense.operators` (since 0.3.27a3) - extension of `operator` library
- `aveytense.util` - utility declarations
- `aveytense.games` - features classes for various games
"""
...
from ._ᴧv_collection import _subscript_builtins as _subb
from ._ᴧv_collection._util import _InternalHelper
from . import constants as _cp
from . import exceptions
from . import operators as _op
from . import types_collection # 0.3.41
from . import types_collection as __
...
def _purify(s: type, /):
    ...
    if not str(s).startswith("<class '") and not str(s).endswith("'>"):
        error = TypeError("expected a type or a class")
        raise error
...
class _Union:
    ...
    Class same as local class `typing._UnionGenericAlias`
    """
    ... 
...
class _Generic:
    ...
    Class same as local class `typing._GenericAlias`
    """
    ...
...
Class same as local class `types._GeneratorWrapper`
"""

if not hasattr(_typing, "_UnionGenericAlias"):
    # it will be typing._SpecialForm internal class instance
    _Union = _typing.cast(_Union, type(__.Union[int, str])) # type: ignore
    
else:
    _Union = _typing.cast(_Union, _typing._UnionGenericAlias) # type: ignore
    
_Generic = _typing.cast(_Generic, _typing._GenericAlias) # type: ignore

# _Generator = _typing.cast(_Generator, _types._GeneratorWrapper) # type: ignore
...
_T_richComparable = _var("_T_richComparable", bound = __.RichComparable)
_T_func = _var("_T_func", bound = _cal[..., __.Any])
_T_strBytes = _var("_T_strBytes", bytes, str) # 0.3.42
...
if _sys.version_info >= (3, 10):
    ...
    _U = _var("_U", bound = _Union) # type: ignore
    _UKT = _var("_UKT", bound = _Union) # type: ignore
    _UVT = _var("_UVT", bound = _Union) # type: ignore

# local enums

# class _ColorStyling(_tc.IntegerFlag): ### to 0.3.27
class _ColorStyling(__.Enum):
    """\\@since 0.3.26rc1. Internal class for `%` operator in class `aveytense.Color`."""
    NORMAL = 0
    BOLD = 1
    FAINT = 2
    ITALIC = 3
...
class _ColorAdvancedStyling(__.Enum):
    """\\@since 0.3.26rc2. Internal class for `%` operator in class `aveytense.Color`."""
...
@types_collection.runtime
class _ClearableAbc(types_collection.Protocol[_T]):
    """
    @since 0.3.42
...
# local type aliases
_Bits = _lit[3, 4, 8, 24]
_Clearable = _uni[str, Color, _ClearableAbc[_Any], util.MutableString, __.MutableMapping[_Any, _Any], __.MutableSequence[_Any], __.MutableUniqual[_Any], __.FrameType] # 0.3.42
_Color = _uni[__.ColorType, RGB]
_Mode = _uni[_cl.ModeSelection, _lit["and", "or"]] # 0.3.36
_Pattern = _uni[_T_strBytes, __.Pattern[_T_strBytes]] # 0.3.42
...
_AbroadMultiInitializer = _subb.list[_subb.list[int]]
_AbroadEachInitializer = _subb.list[_T]
...
if constants.VERSION_INFO >= (0, 3, 46) or True:
    
    if _sys.version_info >= (3, 10):
        
        _EllipsisType = __.EllipsisType
        
    else:
        
        _EllipsisType = __.ellipsis
        
_ProbabilityType = _uni[
    _T,
    __.Sequence[_uni[_T, int, _EllipsisType, None]],
    __.Uniqual[_uni[_T, int, _EllipsisType, None]],
    __.Mapping[_T, _uni[int, _EllipsisType, None]]
] # change 0.3.36, 0.3.46
    
# _ProbabilityTypeTmp = __.ProbabilityType[_T] # < 0.3.37
_ProbabilitySeqNoDict = _uni[list, _collections.deque, set, frozenset, tuple] # to be replaced with Sequence | AbstractSet on 0.3.42 or later
# _ProbabilitySeq = _uni[_ProbabilitySeqNoDict, dict] # 0.3.33; < 0.3.46
_ReckonNGT = _ab_mod.ReckonNGT
_SequenceGeneratorAwaitable = _uni[__.Sequence[_T], __.Generator[_T, _Any, _Any], __.Awaitable[_T]]

# This tuple will be used as a type tuple to inspect via isinstance()
# Version info statements are here to warrant support for Python 3.9 and older.
# 'types.UnionType' >= 3.10, 'types.GenericAlias' >= 3.9

if _sys.version_info >= (3, 10):
    # 0.3.35 (type), _Union & _Generic: 0.3.40, rest: 0.3.36
    _SupportedTypes = (type, _Union, _Generic, __.UnionType, __.GenericAlias) 
    
elif _sys.version_info >= (3, 9):
    _SupportedTypes = (type, _Union, _Generic, __.GenericAlias)
    
else:
    _SupportedTypes = (type, _Union, _Generic) 
...
def _is_bool_callback(v: _Any, /) -> __.TypeIs[__.Callable[[_Any], bool]]:
...
def _is_sequence_helper(v, /, type = _Any): # 0.3.36
    ...
    if reckon(_v) == 0:
        ...
        # the only way to determine if we are dealing with 'typing.Any' is inspecting variable with 'is' keyword
        if type is _Any:
            return True
    ...
    if not isinstance(type, _SupportedTypes + (tuple,)):
        error = TypeError("passed value to parameter '{}' must be a type, tuple of types, union or generic type".format("type"))
        raise error
        
    _placeholder = True
    
    # 0.3.44: See if used Python version has types.GenericAlias available (>=Py3.9)
    if _sys.version_info >= (3, 9):
        _generic = (_Generic, __.GenericAlias)
        
    else:
        _generic = (_Generic,)
    
    # 0.3.40: Using type(obj) in (A, B, ...) may be more secure, since there is no affection, like in isinstance(obj, (A, B, ...))
    if _builtins_type(type) in _generic: 
        ...
        try:
            ...
            # types.GenericAlias class came to force on Python 3.9
            if _sys.version_info >= (3, 9):
                _stringify = [_purify(e) for e in __.get_args(__.GenericAlias(list, type))]
                
            else:
                
                try:
                    _stringify = [_purify(e) for e in __.get_args(_Generic(_subb.list, type))]
                
                except:
                    
                    try:
                        _stringify = [str(e.__name__) for e in __.get_args(type)]
                        
                    except:
                        error = TypeError("generic types not allowed before Python 3.9")
                        raise error
            ...
            for e in _v:
                ...
                if _builtins_type(e) is __.Sequence and _builtins_type(e) is not tuple:
                    
                    # There we are inspecting the X | Y type union syntax, introduced in Python 3.10
                    if _sys.version_info >= (3, 10):
                        ...
                        Union = __.Union
                        _all_types.append("{}[Union[{}]]".format(_builtins_type(e).__name__, ", ".join([_purify(_builtins_type(e2)) for e2 in e])))
                ...
                if isinstance(type, tuple):
                    ...
                    for _type in type:
                        ...
                        if _type is not _builtins_type:
                            ...
                            error = TypeError("passed items to tuple in parameter '{}' must be types, not objects or modules. generic or union types aren't supported yet".format("type"))
                            raise error
...
class TenseOptions(_util.AbstractFinal): # 0.3.27a5
    ...
    if _cl.VERSION_INFO < (0, 3, 40) and False:
        
        initializationMessage = False
        """
        @since 0.3.27a5 https://aveyzan.glitch.me/aveytense#aveytense.TenseOptions
    ...
    """
    @since 0.3.27b1 https://aveyzan.glitch.me/aveytense#aveytense.TenseOptions
    ...
    if False: # < 0.3.46
        ...
        https://aveyzan.glitch.me/aveytense#aveytense.TenseOptions
...
class Tense( # 0.3.24
    ...
    __.BitwiseLeftOperable,
    __.BitwiseRightOperable
    ...
    types_collection = __ # 0.3.42
    util = _util # 0.3.42
    
    version = constants.VERSION
    versionInfo = constants.VERSION_INFO
    ...
    if versionInfo < (0, 3, 40) and False:
        ...
        def __init__(self):
            ...
            if TenseOptions.initializationMessage:
                print("\33[1;90m{}\33[1;36m INITIALIZATION\33[0m Class '{}' was successfully initalized. Line {}".format(
                    super().fencordFormat(), type(self).__name__, self.__frame
                ))
    ...
    def __repr__(self): # 0.3.40
        return "<{} defined in line {}>".format(self.__str__(), self.__frame)
    ...
    @classmethod
    def toList(self, v: __.Union[__.Iterable[_T], __.ListConvertible[_T], __.TupleConvertible[_T]], /):
        """
        @since 0.3.26rc3
    ...
    @classmethod
    @__.overload
    def isNone(self, v: _Any, /) -> __.TypeIs[None]: ...
    ...
    def isNone(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26b3
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isNone
    ...
    @classmethod
    @__.overload
    def isEllipsis(self, v: _Any, /) -> __.TypeIs[_EllipsisType]: ...
    ...
    def isEllipsis(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isEllipsis
        ...
        """
        _many = (v,) + _
        return _inspect_many(*_many, type = _EllipsisType, mode = mode)
    
    @classmethod
    @__.overload
    def isBool(self, v: _Any, /) -> __.TypeIs[bool]: ...
    ...
    def isBool(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26b3
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isBoolean
    ...
    @classmethod
    @__.overload
    def isBoolean(self, v: _Any, /) -> __.TypeIs[bool]: ...
    ...
    @classmethod
    @__.overload
    def isInt(self, v: _Any, /) -> __.TypeIs[int]: ...
    ...
    def isInt(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26b3
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isInteger
    ...
    @classmethod
    @__.overload
    def isInteger(self, v: _Any, /) -> __.TypeIs[int]: ...
    ...
    @classmethod
    @__.overload
    def isFloat(self, v: _Any, /) -> __.TypeIs[float]: ...
    ...
    def isFloat(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26b3
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isFloat
    ...
    @classmethod
    @__.overload
    def isComplex(self, v: _Any, /) -> __.TypeIs[complex]: ...
    ...
    def isComplex(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26b3
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isComplex
    ...
    @classmethod
    @__.overload
    def isStr(self, v: _Any, /) -> __.TypeIs[str]: ...
    ...
    def isStr(self, v, /, *_, mode = _MODE_AND):
        """
        @since 0.3.26b3
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isString
    ...
    @classmethod
    @__.overload
    def isString(self, v: _Any, /) -> __.TypeIs[str]: ...
    ...
    if _cl.VERSION_INFO < (0, 3, 34) and False:
        ...
        @classmethod
        @__.overload
        def isTuple(self, v: _Any, type: type[_T] = _Any, /) -> __.TypeIs[tuple[_T, ...]]:
            """
            \\@since 0.3.26rc1
        ...
        def isTuple(self, v: _Any, type: type[_T], /, *types: __.Unpack[_Ts]) -> __.TypeIs[tuple[_T, __.Unpack[_Ts]]]:
            """
            \\@since 0.3.26rc1
        ...
        @classmethod
        @__.overload
        def isTuple(self, v: _Any, /, type: tuple[type[_T], ...]) -> __.TypeIs[tuple[_T, ...]]: ...
        
        @classmethod
        @__.overload
        def isTuple(self, v: _Any, /, type: _U) -> __.TypeIs[tuple[_U, ...]]: ...
        
        @classmethod
        def isTuple(self, v, /, type = _Any):
            """
            @since 0.3.26rc1
            https://aveyzan.glitch.me/aveytense#aveytense.Tense.isTuple
    ...
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[type[_T], ...], /) -> __.TypeIs[tuple[_T, ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[tuple[type[_T], ...], ...], /) -> __.TypeIs[tuple[_T, ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[__.Union[type[_T], tuple[type[_T], ...]], ...], /) -> __.TypeIs[tuple[_T, ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[_U, ...], /) -> __.TypeIs[tuple[_U, ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[tuple[_U, ...], ...], /) -> __.TypeIs[tuple[_U, ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[__.Union[type[_T], tuple[_U, ...]], ...], /) -> __.TypeIs[tuple[__.Union[_T, _U], ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[__.Union[tuple[type[_T], ...], tuple[_U, ...]], ...], /) -> __.TypeIs[tuple[__.Union[_T, _U], ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[__.Union[type[_T], tuple[type[_T], ...], tuple[_U, ...]], ...], /) -> __.TypeIs[tuple[__.Union[_T, _U], ...]]: ...
    
    @classmethod
    @__.overload
    def isTuple2(self, v1: tuple[_Any, ...], v2: tuple[__.Unpack[_Ts]], /) -> __.TypeIs[tuple[__.Unpack[_Ts]]]: ...
            
    @classmethod
    def isTuple2(self, v1, v2, /):
        """
        @since 0.3.43
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isTuple2
        ...
        if self.isTuple(v1) and self.isTuple(v2) and reckon(v2) == 0:
            return True
        
        elif self.isTuple(v1) and self.isTuple(v2, (tuple,) + _SupportedTypes):
            ...
            if reckonIsLeast(v1, v2):
                ...
                for i in abroad(v1):
                    _placeholder = _placeholder and bool(self.isTuple((v1[i],), v2[i]))
                ...
                for i in abroad(v1[reckon(v2) - 1 :]):
                    _placeholder = _placeholder and bool(self.isTuple((v1[i],), v2[-1]))
    ...
    @classmethod
    @__.overload
    def isList(self, v: _Any, /, type: type[_T] = _Any) -> __.TypeIs[list[_T]]: ...
    
    @classmethod
    @__.overload
    def isList(self, v: _Any, /, type: tuple[type[_T], ...]) -> __.TypeIs[list[_T]]: ...
        
    @classmethod
    @__.overload
    def isList(self, v: _Any, /, type: _U) -> __.TypeIs[list[_U]]: ...
                    
    @classmethod
    def isList(self, v, /, type = _Any) -> __.TypeIs[list[_T]]:
        """
        @since 0.3.26rc1
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isList
    ...
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: type[_KT] = _Any, vtype: type[_VT] = _Any) -> __.TypeIs[dict[_KT, _VT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: type[_KT], vtype: tuple[type[_VT], ...]) -> __.TypeIs[dict[_KT, _VT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: tuple[type[_KT], ...], vtype: type[_VT] = _Any) -> __.TypeIs[dict[_KT, _VT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: tuple[type[_KT], ...], vtype: tuple[type[_VT], ...]) -> __.TypeIs[dict[_KT, _VT]]: ...

    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: type[_KT], vtype: _UVT) -> __.TypeIs[dict[_KT, _UVT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: _UKT, vtype: type[_VT] = _Any) -> __.TypeIs[dict[_UKT, _VT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: _UKT, vtype: tuple[type[_VT], ...]) -> __.TypeIs[dict[_UKT, _VT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: _UKT, vtype: _UVT) -> __.TypeIs[dict[_UKT, _UVT]]: ...
    
    @classmethod
    @__.overload
    def isDict(self, v: _Any, /, ktype: tuple[type[_KT], ...], vtype: _UVT) -> __.TypeIs[dict[_KT, _UVT]]: ...
    
    @classmethod
    def isDict(self, v, /, ktype = _Any, vtype = _Any):
        """
        @since 0.3.31
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isDict
    ...
    @classmethod
    @__.overload
    def isSet(self, v: _Any, /, type: type[_T] = _Any) -> __.TypeIs[set[_T]]: ...
    
    @classmethod
    @__.overload
    def isSet(self, v: _Any, /, type: tuple[type[_T], ...]) -> __.TypeIs[set[_T]]: ...
    
    @classmethod
    @__.overload
    def isSet(self, v: _Any, /, type: _U) -> __.TypeIs[set[_U]]: ...
            
    @classmethod
    def isSet(self, v, /, type = _Any):
        """
        @since 0.3.35
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isSet
    ...
    @classmethod
    @__.overload
    def isFrozenSet(self, v: _Any, /, type: type[_T] = _Any) -> __.TypeIs[frozenset[_T]]: ...
    
    @classmethod
    @__.overload
    def isFrozenSet(self, v: _Any, /, type: tuple[type[_T], ...]) -> __.TypeIs[frozenset[_T]]: ...
    
    @classmethod
    @__.overload
    def isFrozenSet(self, v: _Any, /, type: _U) -> __.TypeIs[frozenset[_U]]: ...
        
    @classmethod
    def isFrozenSet(self, v, /, type = _Any):
        """
        @since 0.3.35
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isFrozenSet
    ...
    @classmethod
    @__.overload
    def isDeque(self, v: _Any, /, type: type[_T] = _Any) -> __.TypeIs[_collections.deque[_T]]: ...
    
    @classmethod
    @__.overload
    def isDeque(self, v: _Any, /, type: tuple[type[_T], ...]) -> __.TypeIs[_collections.deque[_T]]: ...
    
    @classmethod
    @__.overload
    def isDeque(self, v: _Any, /, type: _U) -> __.TypeIs[_collections.deque[_U]]: ...
    
    @classmethod
    def isDeque(self, v, /, type = _Any):
        """
        \\@since 0.3.37 (added on 0.3.37a1)
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isDeque
    ...
    @classmethod
    @__.overload
    def isBytes(self, v: _Any, /) -> __.TypeIs[bytes]: ...
    ...
    def isBytes(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.35
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isBytes
    ...
    @classmethod
    @__.overload
    def isArray(self, v: _Any, /) -> __.TypeIs[__.array]: ... # not generic to comply with versions before Python 3.13
    ...
    def isArray(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.37
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isArray
    ...
    @classmethod
    @__.overload
    def isByteArray(self, v: _Any, /) -> __.TypeIs[bytearray]: ...
    ...
    def isByteArray(self, v, /, *_, mode = _MODE_AND): # not generic
        """
        \\@since 0.3.35
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isByteArray
    ...
    @classmethod
    @__.overload
    def isMemoryView(self, v: _Any, /) -> __.TypeIs[memoryview]: ...
    ...
    def isMemoryView(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.35
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isMemoryView
    ...
    @classmethod
    @__.overload
    def isClass(self, v: _Any, /) -> __.TypeIs[type[_Any]]: ... # >= 0.3.43
        
    @classmethod
    @__.overload
    def isClass(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ... # >= 0.3.43
    
    @classmethod
    def isClass(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.35
    ...
    @classmethod
    @__.overload
    def isFunction(self, v: _Any, /) -> __.TypeIs[_types.FunctionType]: ... # >= 0.3.43
        
    @classmethod
    @__.overload
    def isFunction(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ... # >= 0.3.43
    
    @classmethod
    def isFunction(self, v, /, *_, mode = _MODE_AND): # not generic
        """
        \\@since 0.3.35
    ...
    @classmethod
    @__.overload
    def isNumeric(self, v: _Any, /) -> bool: ...
    
    @classmethod
    @__.overload
    def isNumeric(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
    
    @classmethod
    def isNumeric(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.41
    ...
    @classmethod
    @__.overload
    def isFinalVar(self, v: _Any, /) -> __.TypeIs[_util.FinalVarType]: ...
    ...
    def isFinalVar(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.38
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isFinalVar
    ...
    @classmethod
    @__.overload
    def isSlice(self, v: _Any, /) -> __.TypeIs[slice]: ...
    
    @classmethod
    @__.overload
    def isSlice(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
    
    @classmethod
    def isSlice(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.41
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isSlice
    ...
    @classmethod
    @__.overload
    def isProperty(self, v: _Any, /) -> __.TypeIs[property]: ...
    
    @classmethod
    @__.overload
    def isProperty(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
    
    @classmethod
    def isProperty(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.41
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isProperty
    ...
    if versionInfo >= (0, 3, 42) or True:
        ...
        @classmethod
        @__.overload
        def isFinalProperty(self, v: tuple[type, str], /) -> bool: ...
        
        @classmethod
        @__.overload
        def isFinalProperty(self, v: tuple[type, str], /, *_: tuple[type, str], mode: _Mode = _MODE_AND) -> bool: ...
        
        @classmethod
        @__.overload
        def isFinalProperty(self, v: tuple[type, str], /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
        
    if versionInfo >= (0, 3, 43) or True:
        
        @classmethod
        @__.overload
        def isFinalProperty(self, v: _Any, /) -> __.TypeIs[_util.finalproperty[__.Any]]: ...
        
    else:
        
        @classmethod
        @__.overload
        def isFinalProperty(self, v: _Any, /) -> bool: ...

    if versionInfo >= (0, 3, 42) or True:
        
        @classmethod
        @__.overload
        def isFinalProperty(self, v: _Any, /, *_: tuple[type, str], mode: _Mode = _MODE_AND) -> bool: ...
        
        @classmethod
        @__.overload
        def isFinalProperty(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
        
        @classmethod
        def isFinalProperty(self, v, /, *_, mode = _MODE_AND):
            """
            \\@since 0.3.41
            https://aveyzan.glitch.me/aveytense#aveytense.Tense.isFinalProperty
            ...
            for one in _many:
                ...
                if self.isTuple(one, (type, str)) and reckon(one) == 2 and isinstance(one[0], type) and isinstance(one[1], str):
                    ...
                    if one[1] in one[0].__dict__:
                        ...
                        if mode in (_MODE_AND, "and"):
                            _placeholder = _placeholder and bool(isinstance(t, _util.finalproperty) or (self.isProperty(t) and (t.fset, t.fdel) == (None, None)))
                            
                        elif mode in (_MODE_OR, "or"):
                            _placeholder = _placeholder or bool(isinstance(t, _util.finalproperty) or (self.isProperty(t) and (t.fset, t.fdel) == (None, None)))
                    ...
                    if mode in (_MODE_AND, "and"):
                        _placeholder = _placeholder and bool(isinstance(one, _util.finalproperty) or (self.isProperty(one) and (one.fset, one.fdel) == (None, None)))
                        
                    elif mode in (_MODE_OR, "or"):
                        _placeholder = _placeholder or bool(isinstance(one, _util.finalproperty) or (self.isProperty(one) and (one.fset, one.fdel) == (None, None)))
    ...
    @classmethod
    @__.overload
    def isFinalClass(self, v: _Any, /) -> bool: ...
    
    @classmethod
    @__.overload
    def isFinalClass(self, v: _Any, /, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
    
    @classmethod
    def isFinalClass(self, v, /, *_, mode = _MODE_AND):
        """
        \\@since 0.3.41
    ...
    @classmethod
    def isUnbound(self, f: __.Callable[..., _Any], v: str, /):
        """
        @since 0.3.44
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.isUnbound
        ...
        # 0.3.46
        if _inspect.ismethod(f) and self.test(str(f), r"<unbound method '{}'>".format(f.__qualname__)):
            return True
        ...
        # don't permit custom messages
            ...
            if str(exc) in (
                "cannot access local variable '{}' where it is not associated with a value".format(v),
                "local variable '{}' referenced before assignment".format(v)
            ):
                return True
    ...
    @classmethod
    def any(self, i: __.Iterable[_T], /, condition: _cal[[_T], bool] = ...):
        """
        @since 0.3.26rc2
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.any
        
        Extension of inbuilt function `any()`.
        ...
        if not (self.isEllipsis(condition) or _is_bool_callback(condition)):
            error = TypeError("expected an one-argument callable or ellipsis in parameter 'condition'")
            raise error
        ...
        _cond = condition if not self.isEllipsis(condition) else lambda x: bool(x)
    ...
    @classmethod
    def all(self, i: __.Iterable[_T], /, condition: _cal[[_T], bool] = ...): # slash was after 'condition' parameter before (0.3.34)
        """
        @since 0.3.26rc2
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.all
    ...
    @classmethod
    @__.overload
    def test(self, target: str, pattern: _Pattern[str], flags: _FlagsType = 0) -> bool: ...
    
    @classmethod
    @__.overload
    def test(self, target: __.Buffer, pattern: _Pattern[bytes], flags: _FlagsType = 0) -> bool: ...
    
    @classmethod
    def test(self, target, pattern, flags = 0):
        """
        @since 0.3.42
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.test
        ...
        if (
            (self.isString(target) and (self.isString(pattern) or (isinstance(pattern, __.Pattern) and __.get_args(pattern) == (str,))) or
            ...
            error = TypeError("expected '{}' and '{}' to match one of following type patterns: 'str' + 'str', 'str' + 're.Pattern[str]', 'bytes' + 'collections.abc.Buffer', 'bytes' + 're.Pattern[bytes]', and expected '{}' to be an integer or 're.RegexFlag'".format(*tuple(_get_all_params(self.test))))
            raise error
    
    @classmethod
    @__.overload
    def startswith(self, target: __.Union[str, tuple[str, ...]], prefix: __.Union[str, tuple[str, ...]], /, mode: _Mode = _MODE_OR, start: int = ..., end: int = ...) -> bool: ...
    
    @classmethod
    @__.overload
    def startswith(self, target: __.Union[bytes, tuple[bytes, ...]], prefix: __.Union[__.Buffer, tuple[__.Buffer, ...]], /, mode: _Mode = _MODE_OR, start: int = ..., end: int = ...) -> bool: ...
    
    @classmethod
    def startswith(self, target, prefix, /, mode = _MODE_OR, start = ..., end = ...):
        """
        @since 0.3.42
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.startwith
        ...
        _params = _get_all_params(self.startswith)
        
        if self.any([target, prefix], lambda x: not self.isString(x) and not self.isBytes(x) and not self.isTuple(x, (str, bytes))):
        ...
        if self.any([start, end], lambda x: not self.isEllipsis(x) and not self.isInteger(x)):
            
            error = TypeError("expected integer or ellipsis in parameters '{}' and '{}'".format(*_params[3:]))
            raise error
        
        _start = 0 if self.isEllipsis(start) else start
        _end = reckon(target) - 1 if self.isEllipsis(end) else end
        
        if self.isString(target, prefix) or (self.isBytes(target) and isinstance(prefix, __.Buffer)):
            
            return bool(target.startswith(prefix, _start, _end))
        
        if (self.isString(target) and self.isTuple(prefix, str)) or (self.isBytes(target) and self.isTuple(prefix, __.Buffer)):
        ...
        elif (self.isTuple(target, str) and (self.isString(prefix) or self.isTuple(prefix, str))) or (self.isTuple(target, bytes) and (isinstance(prefix, __.Buffer) or self.isTuple(prefix, __.Buffer))):
    ...
    @classmethod
    @__.overload
    def endswith(self, target: __.Union[str, tuple[str, ...]], prefix: __.Union[str, tuple[str, ...]], /, mode: _Mode = _MODE_OR, start: int = ..., end: int = ...) -> bool: ...
    
    @classmethod
    @__.overload
    def endswith(self, target: __.Union[bytes, tuple[bytes, ...]], prefix: __.Union[__.Buffer, tuple[__.Buffer, ...]], /, mode: _Mode = _MODE_OR, start: int = ..., end: int = ...) -> bool: ...
    
    @classmethod
    def endswith(self, target, suffix, /, mode = _MODE_OR, start = ..., end = ...):
        """
        @since 0.3.42
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.endswith
        ...
        _params = _get_all_params(self.endswith)
        
        if self.any([target, suffix], lambda x: not self.isString(x) and not self.isBytes(x) and not self.isTuple(x, (str, bytes))):
        ...
        if self.isString(target, suffix) or (self.isBytes(target) and isinstance(suffix, __.Buffer)):
            
            return bool(target.endswith(suffix, _start, _end))
        
        if (self.isString(target) and self.isTuple(suffix, str)) or (self.isBytes(target) and self.isTuple(suffix, __.Buffer)):
        ...
        elif (self.isTuple(target, str) and (self.isString(suffix) or self.isTuple(suffix, str))) or (self.isTuple(target, bytes) and (isinstance(suffix, __.Buffer) or self.isTuple(suffix, __.Buffer))):
    ...
    if _cp.VERSION_INFO >= (0, 3, 78) and False: # >= 18.03.2025
        
        @classmethod
        def replace(self, target: _Target, pattern: _uni[_Pattern, tuple[str, ...]], value: _uni[_Target, _cal[[_re.Match[_Target]], _Target], tuple[str, ...]], count = -1, flags: _FlagsType = 0):
    ...
    @classmethod
    def expect(self, i: __.Iterable[_T], m: __.Union[str, range, _ab_mod.AbroadInitializer, __.Uniqual[int]] = ">= 1", /, condition: __.Callable[[_T], bool] = ...):
        """
        \\@since 0.3.40
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.expect
        ...
        if self.versionInfo >= (0, 3, 45) or True:
            ...
            if not isinstance(m, (str, range, _ab_mod.AbroadInitializer, __.Uniqual)) or (isinstance(m, (range, _ab_mod.AbroadInitializer, __.Uniqual)) and not Math.isPositive(list(m))): # all must satisfy
                error = TypeError("expected a valid string literal, like \"> 3\", or a positive integer set")
                raise error
            
        else:
        
            if not self.isString(m) or (self.isString(m) and not _re.match(r"^(\d+|len) ?(<=|>=|==|!=|<|>)$|^(<=|>=|!=|==|<|>) ?(\d+|len)$", m)):
                error = TypeError("expected a valid string literal, like \"> 3\"")
                raise error
        
        if not (self.isEllipsis(condition) or _is_bool_callback(condition)):
            error = TypeError("expected 'condition' parameter to be a callable or ellipsis. if it is callable, only one argument may be passed")
            raise error
        
        if m in (">= 1", "1 <="):
            return self.any(i, condition)
    ...
    def hasattr(self, o, attr, /, mode = _MODE_OR):
        """
        \\@since 0.3.34
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.hasattr
    ...
    @classmethod
    def group(self, *statements: _uni[__.Sequence[bool], __.Uniqual[bool]], mode = "and-or"):
        """
        \\@since 0.3.34
        ...
        for statement in statements:
            ...
            if not isinstance(statement, (__.Sequence, __.Uniqual)) or (isinstance(statement, (__.Sequence, __.Uniqual)) and not self.isList(list(statement), bool)):
    ...
    def equal(self, *v: _T):
        """
        \\@since 0.3.41
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.equal
    ...
    @classmethod
    def inequal(self, *v: _T):
        """
        \\@since 0.3.41
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.inequal
    ...
    def abroadConvect(self, *values: _AbroadConvectType[_T]):
        ...
        if reckon(values) == 0:
            error = __.MissingValueError("expected at least one item in parameter '{}'".format(_params[-1]))
            raise error
    ...
    def abroadLive(self, *values: _AbroadLiveType[_T]):
        ...
        if reckon(values) == 0:
            error = __.MissingValueError("expected at least one item in parameter 'values'.")
            raise error
    ...
    @classmethod
    def abroadFloaty(self, value1: _AbroadValue1[_T1], /, value2: _AbroadValue2[_T2] = None, modifier: _AbroadModifier[_T3] = None, div: __.FloatOrInteger = 10):
        """
        \\@since 0.3.25 \\
        \\@modified 0.3.29
        ```ts
    ...
    def abroadEach(self, value1, /, value2 = None, modifier = None, *, each = None):
        ...
        if self.versionInfo >= (0, 3, 44) or True:
            
            if not self.isNone(each) and (not callable(each) or (callable(each) and reckon(_util.ParamVar(each).positional, _util.ParamVar(each).universal) != 1)):
    ...
    @classmethod
    @__.deprecated("Deprecated since 0.3.46 and will be removed in 0.3.48")   
                ...
                if a == "pip":
                    ...
                    if self.versionInfo >= (0, 3, 44) or True:
                        ...
                        if not _pip_upgraded:
                            _subprocess.run([_sys.executable, "py", "-m", "ensurepip", "--upgrade"])
                            _pip_upgraded = True
                            
                        else:
                            print("The 'pip' module has been already upgraded. There is no necessity upgrading this module again, unless you got a message that it can be upgraded via 'pip install --upgrade pip'")
                        
                    elif self.versionInfo >= (0, 3, 42) and self.versionInfo < (0, 3, 44) and False:
                        
                        print("On 0.3.42 gimmick with PyPa get-pip has been excised, consider manually downloading and running it as 'py get-pip.py' (assuming it is in the same directory)")
                        print("See more information there: https://github.com/pypa/get-pip")
                        ...
                        if not _pip_upgraded:
                            ...
                            _subprocess.run(["py", os.path.dirname(os.path.realpath(__file__)) + "\\_get_pip.py"])
                            _pip_upgraded = True
    ...
    if versionInfo >= (0, 3, 42) or True:
        ...
        @classmethod
        def timeit(self, statement: __.Callable[[], _Any], /):
            """
            @since 0.3.26rc3 \\
            https://aveyzan.glitch.me/aveytense#aveytense.Tense.timeit
            ...
            if not callable(statement) or reckon(util.ParamVar(statement).all) != 0:
        ...
        def timeit(
            ...
            globals: __.Optional[dict[str, _Any]] = None
            ):
            """
            \\@since 0.3.26rc3
            ```
    ...
    @classmethod
    @__.overload
    def shuffle(self, v: _util.MutableString) -> _util.MutableString: ...
    
    @classmethod
    @__.overload
    def shuffle(self, v: _ab_mod.AbroadInitializer) -> list[int]: ...
    
    ## 0.3.42: Uniqual (AbstractSet) was in overload with Sequence and both returning 'list',
    # but now Uniqual instances will return 'set'; added Generator and Awaitable; Uniqual moved to
    # separate overload
    ## 0.3.45: After conclusion 'set' builtin cannot be shuffled nor reversed, it will actually
    # return a list. Uniqual is again in fusion with Sequence. Do NOT convert result to 'set'!
    if versionInfo >= (0, 3, 45) or True:
        
        @classmethod
        @__.overload
        def shuffle(self, v: __.Union[__.Sequence[_T], __.Uniqual[_T]]) -> list[_T]: ...
        
        @classmethod
        @__.overload
        def shuffle(self, v: __.Union[__.Generator[_T, _Any, _Any], __.Awaitable[_T]]) -> list[_T]: ...
        
    else:
        
        @classmethod
        @__.overload
        def shuffle(self, v: __.Uniqual[_T]) -> set[_T]: ...
        
        @classmethod
        @__.overload
        def shuffle(self, v: _SequenceGeneratorAwaitable[_T]) -> list[_T]: ...
    
    @classmethod
    @__.overload
    def shuffle(self, v: __.Mapping[_KT, _VT]) -> dict[_KT, _VT]: ...
    
    @classmethod
    def shuffle(self, v):
        """
        \\@since 0.3.26rc1
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.shuffle
        
        Shuffle a string, mapping or a sequence, and return shuffled iterable \\
        without changing passed iterable.
        """
        
        def _shuffle(x: __.Iterable[_T], /): # >= 0.3.42
        ...
        elif isinstance(v, __.Uniqual):
            
            if self.versionInfo >= (0, 3, 45):
                _v = _shuffle(v)
                
            else: # 0.3.45: part of code below doesn't actually work; returned set is sorted, hence list is returned
                _v = set(_shuffle(v))
        
        # generators may be rebuilt only when creating new function with at least one 'yield' instruction in its body
        elif isinstance(v, (__.Sequence, __.Generator)):
    ...
    @classmethod
    @__.overload
    def reverse(self, v: _util.MutableString) -> _util.MutableString: ...
    ...
    if versionInfo >= (0, 3, 45) or True:
        ...
        @classmethod
        @__.overload
        def reverse(self, v: __.Union[__.Sequence[_T], __.Uniqual[_T]]) -> list[_T]: ...
        
        @classmethod
        @__.overload
        def reverse(self, v: __.Union[__.Generator[_T, _Any, _Any], __.Awaitable[_T]]) -> list[_T]: ...
        
    else:
    
        @classmethod
        @__.overload
        def reverse(self, v: __.Uniqual[_T]) -> set[_T]: ...
        
        @classmethod
        @__.overload
        def reverse(self, v: _SequenceGeneratorAwaitable[_T]) -> list[_T]: ...
    
    @classmethod
    @__.overload
    def reverse(self, v: __.Mapping[_KT, _VT]) -> dict[_KT, _VT]: ...
    
    # >= 0.3.42
    @classmethod
    @__.overload
    def reverse(self, v: __.Reversible[_T]) -> __.Iterator[_T]: ...
    
    @classmethod
    def reverse(self, v):
        """
        \\@since 0.3.26rc2
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.reverse
        ...
        # generators (no matter if async or not) cannot be reversed, so we need to cast them to list first.
        # we cannot rebuild a generator though - send and throw type parameters will remain None, so we need
        # to define a new function with at least one 'yield' instruction
        elif isinstance(v, (__.Sequence, __.Generator)):
    ...
    @classmethod
    @__.overload
    def occurrences(self, v: _uni[__.Sequence[_T], __.Uniqual[_T]], *items: _T) -> int: ...
    
    @classmethod
    @__.overload
    def occurrences(self, v: __.Mapping[_KT, _VT], *items: _KT, mode: _lit["key"] = "key") -> int: ...
    
    @classmethod
    @__.overload
    def occurrences(self, v: __.Mapping[_KT, _VT], *items: _VT, mode: _lit["value"]) -> int: ...
    ...
    @classmethod
    @__.overload
    def difference(self, v1: type[_T1], v2: __.Mapping[str, _T2], /, invert: bool = False, value_check: bool = True) -> list[str]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: __.Mapping[str, _T1], v2: type[_T2], /, invert: bool = False, value_check: bool = True) -> list[str]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: __.Mapping[_KT1, _VT1], v2: __.Mapping[_KT2, _VT2], /, invert: _lit[False] = False, value_check: bool = True) -> list[_KT1]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: __.Mapping[_KT1, _VT1], v2: __.Mapping[_KT2, _VT2], /, invert: _lit[True], value_check: bool = True) -> list[_KT2]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: type[_T], v2: _uni[__.Sequence[str], __.Uniqual[str]], /, invert: bool = False, value_check: bool = True) -> list[str]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: _uni[__.Sequence[str], __.Uniqual[str]], v2: type[_T], /, invert: bool = False, value_check: bool = True) -> list[str]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: _ab_mod.AbroadInitializer, v2: _uni[__.Sequence[int], __.Uniqual[int]], /, invert: bool = False) -> list[int]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: _uni[__.Sequence[int], __.Uniqual[int]], v2: _ab_mod.AbroadInitializer, /, invert: bool = False) -> list[int]: ...
    ...
    @classmethod
    @__.overload
    def difference(self, v1: _uni[__.Sequence[_T1], __.Uniqual[_T1]], v2: _uni[__.Sequence[_T2], __.Uniqual[_T2]], /, invert: _lit[False] = False) -> list[_T1]: ...
    
    @classmethod
    @__.overload
    def difference(self, v1: _uni[__.Sequence[_T1], __.Uniqual[_T1]], v2: _uni[__.Sequence[_T2], __.Uniqual[_T2]], /, invert: _lit[True]) -> list[_T2]: ...
    ...
    @classmethod
    @__.overload
    def intersection(self, v: __.Iterable[_T], /, *_: __.Iterable[_Any]) -> list[_T]: ...
    ...
    def intersection(self, v, /, *_):
        ...
        if isinstance(v, bool):
            ...
            if reckon(_) == 0:
                ...
                if _sys.version_info >= (3, 9):
                    _checkery = bool(self.isList(_pre, __.Iterable))
                    
                else:
                    _checkery = self.all(_pre, lambda x: issubclass(type(x), __.Iterable))
    ...
    @classmethod
    @__.overload
    def union(self, v: __.Iterable[_T1], /, *_: __.Iterable[_T2]) -> list[_uni[_T1, _T2]]: ...
    ...
    @classmethod
    @__.overload
    def append(self, i: __.Mapping[_KT1, _VT1], /, *items: tuple[_KT2, _VT2]) -> dict[__.Union[_KT1, _KT2], __.Union[_VT1, _VT2]]: ...
    
    @classmethod
    @__.overload
    def append(self, i: __.Iterable[_T1], /, *items: _T2) -> list[__.Union[_T1, _T2]]: ...
    ...
    def append(self, i, /, *items):
        ...
        if self.versionInfo >= (0, 3, 46) or True:
            
            if isinstance(i, __.Mapping) and self.all(items, lambda x: self.isTuple(x) and reckon(x) == 2):
                
                return dict([(k, i[k]) for k in i] + [e for e in items])
                ...
                error = TypeError("expected a mapping + 2-item tuples or an iterable")
                raise error
    ...
    @classmethod
    @__.overload
    def extend(self, i1: __.Mapping[_KT1, _VT1], /, *i2: __.Mapping[_KT2, _VT2]) -> dict[_uni[_KT1, _KT2], _uni[_VT1, _VT2]]: ...
    
    @classmethod
    @__.overload
    def extend(self, i1: __.Iterable[_T1], /, *i2: __.Iterable[_T2]) -> list[_uni[_T1, _T2]]: ...
    
    @classmethod
    def extend(self, i1, /, *i2):
        """
        \\@since 0.3.27a4
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.extend
        ...
        Since 0.3.46 mappings are allowed; in this case all items in `i2` must be mappings too.
        """
        ...
        if self.versionInfo >= (0, 3, 46) or True:
            ...
            if _sys.version_info >= (3, 9):
                _checkery = (
                    self.isTuple(i2, __.Mapping),
                    self.isTuple(i2, __.Iterable)
                )
                
            else:
                _checkery = (
                    self.all(i2, lambda x: issubclass(type(x), __.Mapping)),
                    self.all(i2, lambda x: issubclass(type(x), __.Iterable))
                )
                
            if isinstance(i1, __.Mapping) and _checkery[0]:
                
                return dict([(k, i1[k]) for k in i1] + [(k, e[k]) for e in i2 for k in e])
            
            elif isinstance(i1, __.Iterable) and _checkery[1]:
                
                return [e for e in i1] + [e for iterable in i2 for e in iterable]
            
            else:
                
                error = TypeError("expected mappings only or iterables only")
                raise error
            ...
            # 0.3.41 - 0.3.45
            if not isinstance(i1, __.Iterable):
                error = TypeError("expected an iterable")
                raise error
    ...
    @classmethod
    @__.overload
    def exclude(self, i: __.Mapping[_KT, _VT], /, *items: _KT, filter: __.Literal["keys"] = "keys") -> dict[_KT, _VT]: ...
    
    @classmethod
    @__.overload
    def exclude(self, i: __.Mapping[_KT, _VT], /, *items: _VT, filter: __.Literal["values"]) -> dict[_KT, _VT]: ...
    
    @classmethod
    @__.overload
    def exclude(self, i: __.Iterable[_T], /, *items: _T) -> list[_T]: ...
    
    @classmethod
    def exclude(self, i, /, *items, filter = "keys"):
        """
        @since 0.3.34
    ...
    if _cl.VERSION_INFO >= (0, 3, 40) or True:

        @classmethod # below: append 'typing.IO[Any]' when tests are done on 0.3.48
        def clear(self, *v: _Clearable): # 0.3.27a4
            """
            \\@since 0.3.27a4 https://aveyzan.glitch.me/aveytense#aveytense.Tense.clear
            ...
            _getvars = {name: val for name, val in _frame.items() if val in v and self.isString(val)}
            ...
            for _v in v:
                ...
                if self.isString(_v):
                    ...
                    # 0.3.42
                        ...
                        _clear = self.cast(_v.clear, __.FunctionType)
                        
                        if _clear.__code__.co_argcount + _clear.__code__.co_kwonlyargcount == 0:
                    ...
                    # Not tested yet, please test. This part of the code will be possibly postponed too.
                    if _cl.VERSION_INFO >= (0, 3, 48) and False:
                    
                        if isinstance(_v, __.IO):
        ...
        @classmethod
        def clear(self, *v: __.Union[__.MutableSequence[_T], __.MutableUniqual[_T], __.MutableMapping[_Any, _T], Color, str]):
            """
            \\@since 0.3.27a4
            ```ts
            "class method" in class Tense
    ...
    if False:
        @classmethod
        def eval(self, source: __.Union[str, __.Buffer, _ast.Module, _ast.Expression, _ast.Interactive], ast = False):
        ...
        @classmethod
        @__.overload
        def eval(self, source: _uni[str, __.Buffer, __.CodeType], /, globals: _opt[dict[str, _Any]] = None, locals: _opt[__.Mapping[str, object]] = None) -> _Any:
            """\\@since 0.3.26rc2"""
            ...
        ...
        def eval(
            self,
            source: tuple[_uni[str, __.Buffer, _ast.Module, _ast.Expression, _ast.Interactive], _uni[str, __.Buffer, __.PathLike[_Any]], str],
            /,
            globals: _opt[dict[str, _Any]] = None,
            locals: _opt[__.Mapping[str, object]] = None,
            flags: int = 0,
    ...
    @classmethod
    @__.overload
    def bisect(self, a: __.SizeableItemGetter[_T_richComparable], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _cl.BisectMode = BISECT_RIGHT, *, key: None = None) -> int:
        """\\@since 0.3.26rc2"""
        ...
    
    @classmethod
    @__.overload
    def bisect(self, a: __.SizeableItemGetter[_T], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _cl.BisectMode = BISECT_RIGHT, *, key: _cal[[_T], _T_richComparable]) -> int:
        """\\@since 0.3.26rc2"""
        ...
    ...
    if False:
        @classmethod
        def insort(self, seq: __.MutableSequence[_T], item: _T, low: int = 0, high: _opt[int] = None, mode: _cl.InsortMode = INSORT_RIGHT, key: _opt[_cal[[_T], _T_richComparable]] = None):
        ...
        @classmethod
        @__.overload
        def insort(self, a: __.MutableSequence[_T_richComparable], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _cl.InsortMode = INSORT_RIGHT, *, key: None = None) -> None:
            """\\@since 0.3.26rc2"""
            ...
            
        @classmethod
        @__.overload
        def insort(self, a: __.MutableSequence[_T], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _cl.InsortMode = INSORT_RIGHT, *, key: _cal[[_T], _T_richComparable]) -> None:
            """\\@since 0.3.26rc2"""
            ...
    ...
    @classmethod
    def print(self, *values: object, separator: _opt[str] = " ", ending: _opt[str] = "\n", file: _uni[__.Writeable[str], __.Flushable, None] = None, flush = False, reprFirst = False):
        """
        \\@since 0.3.25
        ```
        "class method" in class Tense
        ...
        if TenseOptions.insertionMessage is True:
            ...
            if reprFirst:
                ...
                print((Color(Time.fencordFormat(), 8, 243) % Color.BOLD) + " " + (Color("INSERTION", 8, 45) % Color.BOLD), *tuple([_invoke_this(e) for e in values]), sep = separator, end = ending, file = file, flush = flush)
            
            else:
                
                print((Color(Time.fencordFormat(), 8, 243) % Color.BOLD) + " " + (Color("INSERTION", 8, 45) % Color.BOLD), *values, sep = separator, end = ending, file = file, flush = flush)
    ...
    @classmethod
    @__.overload
    def random(self, x: _uni[__.Sequence[_T], __.Uniqual[_T]], /) -> _T: ...
    ...
    def random(self, x, y = None, /):
        ...
        if self.isNone(y):
            ...
            elif isinstance(x, (__.Sequence, __.Uniqual)):
    ...
    @classmethod
    def uuidMd5(self, namespace: __.UUID, name: _uni[str, bytes]):
        """
        \\@since 0.3.26a1 \\
    ...
    @classmethod
    def uuidSha1(self, namespace: __.UUID, name: _uni[str, bytes]):
        """
        \\@since 0.3.26a1 \\
        \\@modified 0.3.31 (cancelled support for `tkinter.StringVar`)
        ```ts
    ...
    # what allows to prevent using overloading.
    if versionInfo < (0, 3, 46) and False:
        
        @classmethod
        @__.overload
        def pick(self, i: _uni[__.Sequence[_T], __.Uniqual[_T]], /, secure: bool = False) -> _T: ... # sets act the same as lists in this case
        
        @classmethod
        @__.overload
        def pick(self, i: __.Mapping[_KT, _VT], /, secure: bool = False) -> _VT: ...
    
    @classmethod # before 0.3.46 this signature had no annotations
    def pick(self, i: __.Union[__.Sequence[_T], __.Uniqual[_T], __.Mapping[_Any, _T]], /, secure: bool = False):
        """
        ...
        if isinstance(i, (__.Sequence, __.Uniqual)):
            ...
            return _secrets.choice(_i) if secure else _random.choice(_i)
            ...
            return _secrets.choice(_values) if secure else _random.choice(_values)
    ...
    @classmethod
    @__.overload
    def invert(self, v: _uni[int, float], /) -> float: ...
    
    @classmethod
    @__.overload
    def invert(self, v: _uni[__.Sequence[_uni[int, float]], __.Uniqual[_uni[int, float]], _ab_mod.AbroadInitializer], /) -> list[float]: ...
    
    @classmethod
    @__.overload
    def invert(self, v: __.Mapping[_KT, _VT], /) -> dict[_VT, _KT]: ...
    ...
    def invert(self, v, /):
        ...
        if self.isInteger(v) or self.isFloat(v):
        ...
        elif isinstance(v, (__.Sequence, __.Uniqual)) and self.all(v, lambda x: self.isInteger(x) or self.isFloat(x)):
            
            _filter = self.cast([1/e for e in v if e not in (0, .0)], list[float])
            
            if reckon(v) != reckon(_filter):
                error = ZeroDivisionError("division by zero")
                raise error
            
            return self.cast([-e for e in v], list[float])
    ...
    @classmethod
    @__.overload
    def first(self, i: _uni[__.Sequence[_T], __.Uniqual[_T]], /, condition: None = None, default: _S = None) -> _uni[_S, _T]: ...
    
    @classmethod
    @__.overload
    def first(self, i: _uni[__.Sequence[_T], __.Uniqual[_T]], /, condition: _cal[[_T], bool], default: _S = None) -> _uni[_S, _T]: ...
    ...
    def first(self, i, /, condition = None, default = None):
        ...
        if not _is_bool_callback(condition) and not self.isNone(condition):
    ...
    @classmethod
    @__.overload
    def last(self, i: _uni[__.Sequence[_T], __.Uniqual[_T]], /, condition: None = None, default: _S = None) -> _uni[_S, _T]: ...
    
    @classmethod
    @__.overload
    def last(self, i: _uni[__.Sequence[_T], __.Uniqual[_T]], /, condition: _cal[[_T], bool], default: _S = None) -> _uni[_S, _T]: ...
    ...
    def probability2(self, x: _T = 1, y: _T = 0, frequency = 1, length: _ProbabilityLengthType = 10000):
        ...
        https://aveyzan.glitch.me/aveytense#aveytense.Tense.probability2 \\
        https://aveyzan.glitch.me/aveytense/py/method.probability.html#2
        ```ts
        "class method" in class Tense
        ``` \n
        ``` \n
        ...
        if self.versionInfo < (0, 3, 46) and False:
    ...
    if versionInfo >= (0, 3, 46) or True:
        ...
        @classmethod
        def probability3(self, vf: __.Mapping[_T, __.Union[int, _EllipsisType]], /):
            """
            @since 0.3.46
            https://aveyzan.glitch.me/aveytense#aveytense.Tense.probability3
            ...
            if not isinstance(vf, __.Mapping):
            ...
            if reckon(vf) in (0, 1):
                ...
                _filter = dict([(k, vf[k]) for k in vf if self.isEllipsis(vf[k]) or (self.isInteger(vf[k]) and vf[k] > 0)])
                _keys, _values = [k for k in _filter], [_filter[k] for k in _filter]
                _values = [(1 if not self.isInteger(e) else e) for e in _values]
                
                if reckon(vf) != reckon(_filter):
                    error = TypeError("expected key values have positive integer or ellipsis")
                    raise error
                
                if self.all(_values, lambda x: x == 1):
                        
                    return self.pick(_keys)
                
                if reckon(vf) == 2:
                        
                    return self.probability2(_keys[0], _keys[1], _values[0], _values[0] + _values[1])
        ...
        @classmethod
        def probability(self, *vf: _ProbabilityType[_T]):
            """
            @since 0.3.8 (standard since 0.3.9) \\
            @lifetime ≥ 0.3.8 \\
            @modified 0.3.19, 0.3.24, 0.3.25, 0.3.26a3, 0.3.26b3, 0.3.26rc1, 0.3.26rc2, 0.3.31, 0.3.46 \\
            https://aveyzan.glitch.me/aveytense#aveytense.Tense.probability
            ...
            if reckon(vf) == 0:
                ...
                e = self.cast(e, __.Mapping[_T, _uni[int, __.EllipsisType, None]])
                _filter = dict([(k, e[k]) for k in e if (self.isEllipsis(e[k]) or self.isNone(e[k])) or (self.isInteger(e[k]) and e[k] > 0)])
                _keys, _values = [k for k in _filter], [_filter[k] for k in _filter]
                _values = [(1 if not self.isInteger(e) else e) for e in _values]
                ...
                for e in vf:
                    ...
                    if isinstance(e, __.Mapping):
                        ...
                        _e = self.extend(_e, {_keys[i]: _values[i] for i in abroad(_keys)})
                        ...
                        if reckon(_ie) not in (1, 2):
                            ...
                            _v1: _T = _ie[0]
                            _e = self.extend(_e, {_v1: 1})
                            
                        else:
                            
                            _v2: tuple[_T, int] = (_ie[0], 1 if not self.isInteger(_ie[1]) else _ie[1])
                            _e = self.extend(_e, {_v2[0]: _v2[1]})
                            
                    else:
                        
                        _v3: _T = e
                        _e = self.extend(_e, {_v3: 1})
                        
            return self.probability3(_e)
        ...
        def probability(self, *vf: _ProbabilityType[int], length: _ProbabilityLengthType = PROBABILITY_COMPUTE):
            ...
            @lifetime ≥ 0.3.8 \\
            @modified 0.3.19, 0.3.24, 0.3.25, 0.3.26a3, 0.3.26b3, 0.3.26rc1, 0.3.26rc2, 0.3.31, 0.3.46 \\
            https://aveyzan.glitch.me/aveytense/py/method.probability.html
            ```ts
            "class method" in class Tense
            ``` \n
            ``` \n
            ...
            # for 0.3.25 - 0.3.45
            def probability(
                *vf: int | list[int] | tuple[int, int | None] | dict[int, int | None] | deque[int],
                length: int = PROBABILITY_COMPUTE
            ): ...

            # for 0.3.24
            def probability(
                *valuesAndFrequencies: int | list[int] | tuple[int] | dict[int, int | None],
    ...
    @classmethod
    def until(self, desiredString: __.Union[str, __.Sequence[str]], /, message: _opt[str] = None, caseInsensitive: bool = True):
        """
        \\@since 0.3.25
        ```ts
        "class method" in class Tense
        ...
        if not isinstance(desiredString, (str, __.Sequence)) or (isinstance(desiredString, __.Sequence) and not self.isString(desiredString) and not self.isList(list(desiredString), str)):
    ...
    @classmethod
    def improvedBogoSort(self, i: __.Iterable[_T], /, key: __.Callable[[_T], __.RichComparable] = ...):
        """
        @since 0.3.42
        ...
        in a list, which will be returned, when list is sorted. This decreases amount of accesses, and shortens execution time.
        ...
        if self.hasattr(l, ("__gt__", "__lt__"), mode = self.constants.MODE_AND):
            pass
    ...
    def repeat(self, value: _T, times: _opt[int] = None, /): # 0.3.31: default value 'None'
        ...
        if times is None:
            ...
            if not self.isInteger(times) or times <= 0:
    ...
    @classmethod
    def cycle(self, i: __.Iterable[_T], /):
        """
        \\@since 0.3.34
        """
    ...
    if versionInfo >= (0, 3, 46) or True:
        ...
        def count(self, start: int = 0, step: int = 1):
            ...
            if not self.isTuple((start, step), int):
    ...
    @classmethod
    def starmap(self, f: _cal[..., _T], i: __.Iterable[__.Iterable[_Any]], /):
        """
        \\@since 0.3.35
        """
        if not callable(f) or not self.isString(self.invert(dict(util.ParamVar(f).variable)).get("<args>", None)):
...
if _cp.VERSION_INFO < (0, 3, 43) and False:
...
class RGB(_util.Final):
    ...
    def __str__(self):
        """
        @since 0.3.28 https://aveyzan.glitch.me/aveytense#aveytense.RGB.__str__
        """
        return "{}({})".format(type(self).__name__, ", ".join([str(e) for e in self.__rgb]))
        
    def __repr__(self):
        """
        @since 0.3.28 https://aveyzan.glitch.me/aveytense#aveytense.RGB.__repr__
        """
    ...
    def __hex__(self):
        """
        @since 0.3.28 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?hexadecimal_conversion
    ...
    def __int__(self):
        """
        @since 0.3.28 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?integer_conversion
    ...
    def __lt__(self, other): 
        """
        @since 0.3.28 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?comparison_operators
    ...
    @__.deprecated("deprecated since 0.3.41, will be removed 0.3.48. use `tuple` final property instead")
    def __pos__(self):
        """
        @since 0.3.28 https://aveyzan.glitch.me/aveytense#aveytense.RGB.tuple
    ...
    def __bytes__(self):
        """
        @since 0.3.41 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?bytes_conversion
    ...
    def __hash__(self):
        """
        @since 0.3.41 https://aveyzan.glitch.me/aveytense#aveytense.RGB.__hash__
    ...
    def __bool__(self):
        """
        @since 0.3.41 https://aveyzan.glitch.me/aveytense#aveytense.RGB.__bool__
    ...
    def __bin__(self):
        """
        @since 0.3.41 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?binary_conversion
    ...
    def __oct__(self):
        """
        @since 0.3.41 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?octal_conversion
    ...
    def hex(self):
        """
        @since 0.3.38 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?hexadecimal_conversion
    ...
    def oct(self):
        """
        @since 0.3.44 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?octal_conversion
    ...
    def bin(self):
        """
        @since 0.3.44 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?binary_conversion
    ...
    def tuple(self):
        """
        @since 0.3.38 https://aveyzan.glitch.me/aveytense#aveytense.RGB.tuple
    ...
    def r(self):
        """
        @since 0.3.45 https://aveyzan.glitch.me/aveytense#aveytense.RGB.?values
    ...
    def fromValue(n: _uni[int, str, _util.MutableString], /):
        """
        \\@since 0.3.36 https://aveyzan.glitch.me/aveytense#aveytense.RGB.fromValue
        ...
        if (Tense.isInteger(_n) and Math.isInRange(_n, 0, _cp.RGB_MAX)) or (
            Tense.isString(_n) and ((_is_hexadecimal(_n) or _is_decimal(_n) or _is_octal(_n) or _is_binary(_n)) and Math.isInRange(_int_conversion(_n), 0, _cp.RGB_MAX))
        ):
...
if _cl.VERSION_INFO < (0, 3, 36) and False:
    ...
    class File:
        ...
        def unpickle(self, *, fixImports = True, encoding = "ASCII", errors = "strict", buffers: _opt[__.Iterable[_Any]] = ()):
            "\\@since 0.3.26rc2"
...
class ChangeVar(__.UnaryOperable, __.Comparable, __.AdditionReassignable, __.SubtractionReassignable):
    ...
    ```py
    class ChangeVar: ...
    from aveytense import ChangeVar
    ```
...
class Color(__.ModuloOperable[_ColorStylingType, str], __.UnaryOperable):
    """
    \\@since 0.3.26rc1 \\
    \\@lifetime ≥ 0.3.26rc1
    
    https://aveyzan.glitch.me/aveytense#aveytense.Color
    ...
    **Warning**: 24-bit colors load longer than colors from lower bit shelves. In this case it is \\
    ...
    if _cl.VERSION_INFO >= (0, 3, 27) or True:
        ...
        def __init__(self, text: __.Union[str, _util.MutableString], /, bits: _Bits = 8, foregroundColor: _Color = None, backgroundColor: _Color = None): # slash since 0.3.26rc2
            ...
            for e in (foregroundColor, backgroundColor):
                ...
                if not isinstance(e, (int, str, RGB)) and not Tense.isNone(e):
    ...
    if _cl.VERSION_INFO < (0, 3, 27) and False:
    ...
    "\\@since 0.3.26rc2. Returns list of all non-underscore-preceded members of class `~.Color`"
...
Colour = Color
"""
\\@since 0.3.37: type alias `aveytense.Colour`
"""
