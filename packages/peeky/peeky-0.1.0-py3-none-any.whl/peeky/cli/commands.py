...
def config(
    ...
    """
    # Handle positional argument for API key
    if api_key is not None:
        # Store the API key for WHOIS
    ...
    if set_whois_key:
        # Get the API key either from the --key option or prompt for it
        api_key = whois_key_value
        
        # If no key was provided via --key, prompt for it
        if api_key is None:
...
def scan(
    ...
    """
    # Get connection data with process info
...
def clean(
    ...
    # Format and display idle processes
    tables = format_idle_processes_table(idle_processes)
    print_table(tables)
    
    # If list-only mode, just display processes and exit
    if list_only:
    ...
    if killed_processes:
        ...
        # List the processes that were killed
...
def kill(
    ...
    """
    # Check if target is a PID or port
    try:
    ...
    # If target is likely a port (below 65536)
    success = False
    if target_int < 65536:
        # Try as port first - this may involve killing multiple processes
        # Get all PIDs using this port before killing
        ...
        # Check if any of these processes are risky
        if not yes:
            for pid in pids:
        ...
        if success:
            ...
            # If no process using that port, try as PID
            # Check if it's a risky process
            if not yes:
...
def export(
    ...
    """
    # Get connection data with process info (same as scan command)
    connections = get_connection_with_process_info(
        port=port,
        tcp_only=tcp,
        udp_only=udp,
...
def secure():
    ...
    """
    # Identify exposed ports and security risks
...
def whois(
    ...
    # Show a message about privacy only for domains (API is used for domains only)
