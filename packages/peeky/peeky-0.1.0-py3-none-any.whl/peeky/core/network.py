...
def get_connections(port: Optional[int] = None, tcp_only: bool = False, udp_only: bool = False) -> List[Dict[str, Any]]:
    ...
    # Filter connections based on parameters
...
def get_connection_with_process_info(
    ...
    for conn in connections:
        # Skip connections without a PID
        if conn["pid"] is None:
            continue
        ...
        try:
            ...
            # Merge connection and process info
...
def detect_port_conflicts() -> Dict[int, List[Dict[str, Any]]]:
    ...
    for conn in connections:
        ...
        # Add process info if PID exists
    ...
    # Filter to only ports with conflicts (multiple processes)
...
def calculate_network_stats() -> Dict[str, Any]:
    ...
    # Calculate network throughput (this is a placeholder, real implementation would track over time)
