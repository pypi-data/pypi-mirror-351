...
def get_whois_from_api(target: str, is_ip: bool = False) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    if not api_key:
        # Instead of returning an error, provide demo mode information
        return {
            "demo_mode": True,
    ...
    # The APILayer WHOIS API only supports domain names, not IP addresses
    if is_ip:
        # For IP addresses, we don't use the API
        return {
            "demo_mode": True,
            "message": "The WHOIS API does not support direct IP address lookups. Using local resolution instead."
        }, None
    
    # For domains, use the query endpoint
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # The API response structure depends on the endpoint
            if 'result' in data:
            ...
            # Return the error message directly instead of wrapping it in demo mode
...
def get_ip_info(ip: str) -> Dict[str, Any]:
    ...
    """
    # Basic information we can get without external APIs
    result = {
        "target": ip,
        "type": "ip",
    }
    ...
    try:
        ...
        # Try to determine organization from hostname
    ...
    # Special case handling for common IPs
...
def perform_lookup(target: str, use_api: bool = True) -> Tuple[Dict[str, Any], Optional[str], str]:
    ...
    # For IP addresses, we use our local function
    if lookup_type == "ip":
        result = get_ip_info(target)
        return result, None, lookup_type
    
    # For domains, we can use the API
    # Basic information we can get without external APIs
    result = {
        "target": target,
        "type": lookup_type,
    ...
    # If we're using the API and have an API key, get data from there
    if use_api:
        api_data, error = get_whois_from_api(target, is_ip=False)
        
        # If the API returned demo mode information, add it to the result
        if api_data and "demo_mode" in api_data:
            ...
            # Fill in the missing data with local placeholders
            result = _add_placeholder_data(result)
        elif error:
            # If there's an error with the API, return the error
            return {}, error, lookup_type
        elif api_data:
            # Extract relevant information from API response
            result = _extract_api_data(api_data, result)
    else:
        # If we're not using the API, use local data
        result = _add_placeholder_data(result)
...
def _extract_api_data(api_data: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Make sure all required fields have values, even if API didn't provide them
