...
class TestSuggestionError(Exception):
...
class TestSuggestionEngine:
    ...
    def generate_suggestions(self, 
        ...
        if not nodes_file.exists():
            raise TestSuggestionError(f"Nodes file not found: {nodes_file}")
        ...
        if not target_nodes:
            raise TestSuggestionError(f"No matching nodes found for target: {target}")
        ...
        # Prioritize target nodes based on critical path
        prioritized_nodes = self._prioritize_nodes(target_nodes, critical_nodes)
    ...
    def _load_nodes(self, nodes_file: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise TestSuggestionError(f"Failed to load nodes: {str(e)}")
    ...
    def _load_critical_path(self, critical_path_file: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise TestSuggestionError(f"Failed to load critical path data: {str(e)}")
    ...
    def _prioritize_nodes(self, target_nodes: List[Dict[str, Any]], 
                         critical_nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _generate_heuristic_suggestions(self, node: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Try to read the file - check multiple possible locations
            source_paths = [
                PathResolver.to_absolute(file_path),                   # Default path resolution
                Path(file_path),                                       # Direct path (relative to cwd)
                repo_root / file_path,                                 # Path relative to repo root
                cwd / file_path,                                       # Path relative to cwd
                # Special cases for Django repo
                cwd / "django" / file_path,                            # django/ subfolder
                cwd / "django" / file_path.replace("django/", ""),     # Remove django/ prefix if it exists
                repo_root / "django" / file_path,                      # django/ subfolder from repo root
                repo_root / "django" / file_path.replace("django/", ""), # Remove django/ prefix from repo root
            ]
            
            # Also try with just the filename if we're struggling
            basename = os.path.basename(file_path)
            source_paths.extend([
                # Basename approaches
                cwd / basename,                                       # Just filename in cwd
                repo_root / basename,                                 # Just filename in repo root
                cwd / "django" / basename,                            # Just filename in django subfolder
                repo_root / "django" / basename,                      # Just filename in django subfolder from repo root
            ])
            ...
            for i, test_case in enumerate(test_cases):
                ...
                if not test_file.startswith("test_"):
                    test_file = "test_" + os.path.basename(test_file)
    ...
    def _find_source_file(self, file_path: str) -> Optional[Path]:
        """Find the source file for a given file path.
        
        Args:
            file_path: File path from node data
        ...
        for path in source_paths:
            if path.exists() and path.is_file():
                try:
                    ...
                    with open(path, 'r', encoding='utf-8') as f:
                        f.read(10)  # Read just a bit to confirm it's readable
                    return path
                except Exception:
                    continue
    ...
    def _generate_test_cases(self, func_name: str, params: List[str], 
        ...
        # Generate test case for each parameter
        for param in params:
            param_type = type_hints.get(param, "unknown")
            
            # Generate boundary conditions based on type
            if param_type in ("int", "float", "number"):
                test_cases.extend([
                    {
                        ...
                        "params": {param: "0"},
                        "expected": "expected_result",
                        "description": f"Test {func_name} with {param}=0"
                    },
                    {
                ...
                test_cases.extend([
                    {
                        "scenario": f"{param}_true",
                        "params": {param: "True"},
                        "expected": "expected_result",
                        "description": f"Test {func_name} with {param}=True"
                    },
                    {
                        "scenario": f"{param}_false",
                        "params": {param: "False"},
                        "expected": "expected_result",
                        "description": f"Test {func_name} with {param}=False"
                    }
                ])
                ...
                test_cases.extend([
                    ...
                    {
                        "scenario": f"{param}_special",
                        "params": {param: "\"!@#$%^&*()\""},
                        "expected": "expected_result",
                        "description": f"Test {func_name} with special characters in {param}"
                    }
                ])
                ...
                test_cases.extend([
                    ...
                    {
                        "scenario": f"{param}_single",
                        "params": {param: "[1]"},
                        "expected": "expected_result",
                        "description": f"Test {func_name} with single-item {param}"
                    }
                ])
                ...
                # Generic test case for unknown types
                test_cases.append({
                    "scenario": f"{param}_null",
                    "params": {param: "None"},
                    "expected": "expected_result",
                    "description": f"Test {func_name} with {param}=None"
                })
    ...
    def _generate_llm_suggestions(self, node: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                from testindex.llm.clients.openai_client import OpenAIClient
            except ImportError:
