...
class TopicsOperations(RestOperations):
    ...
    def get_topics(self):
        ...
        user_id = bottle.request.json.get("user_id")
    ...
    def get_topic_by_id(self, id):
        ...
        try:
            ...
            filter_params = FilterParams.from_tuples("topic_id", id)
            messages = self._messages_service.get_messages(context, filter_params)
    ...
    def register_routes(self, controller: RestController, auth: AuthorizerV1):
        ...
        controller.register_route_with_auth('get', '/topics/<id>', ObjectSchema(True)
                                  .with_optional_property("topic_id", TypeCode.String), auth.signed(),
        ...
        controller.register_route_with_auth('put', '/topics', ObjectSchema(True)
                                  .with_required_property("body", TopicV1Schema()), auth.signed(),
                                  self.update_topic)

        controller.register_route_with_auth('delete', '/topics/<id>', ObjectSchema(True)
