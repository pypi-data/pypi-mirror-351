...
class ThreadTestCase(TestCase):
    ...
    @skipUnless(version_info.major == 3 and version_info.minor == 11, 'Only works in Python 3.11')
    ...
    def test_error_3_11(self, log: mock.MagicMock):
        ...
        log.error.assert_called_once_with(
            ...
            (1, 0),
            {},
            f'Traceback (most recent call last):\n  File "{threads.__file__}", line 182, in wait\n    self.results.append(future.result())\n                        ^^^^^^^^^^^^^^^\n  File "{futures._base.__file__}", line 449, in result\n    return self.__get_result()\n           ^^^^^^^^^^^^^^^^^^^\n  File "{futures._base.__file__}", line 401, in __get_result\n    raise self._exception\n  File "{futures.thread.__file__}", line 58, in run\n    result = self.fn(*self.args, **self.kwargs)\n             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n  File "{__file__}", line 26, in div\n    return v1 / v2\n           ~~~^~~~\nZeroDivisionError: division by zero\n'
        )

    @skipUnless(version_info.major == 3 and version_info.minor == 12, 'Only works in Python 3.12')
    @mock.patch.object(threads, 'log')
    def test_error_3_12(self, log: mock.MagicMock):
        t1 = threads.Thread(target=div, args=(1, 0))
        t1.start()
        t1.join()
        log.error.assert_called_once_with(
            ...
            (1, 0),
            {},
            f'Traceback (most recent call last):\n  File "{threads.__file__}", line 182, in wait\n    self.results.append(future.result())\n                        ^^^^^^^^^^^^^^^\n  File "{futures._base.__file__}", line 449, in result\n    return self.__get_result()\n           ^^^^^^^^^^^^^^^^^^^\n  File "{futures._base.__file__}", line 401, in __get_result\n    raise self._exception\n  File "{futures.thread.__file__}", line 59, in run\n    result = self.fn(*self.args, **self.kwargs)\n             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n  File "{__file__}", line 26, in div\n    return v1 / v2\n           ~~~^~~~\nZeroDivisionError: division by zero\n'
        )
...
class ThreadPoolTestCase(TestCase):
    ...
    def test_silent_thread_3_11(self, error: mock.MagicMock):
        ...
        error.assert_called_once_with(
            ...
            (),
            {},
            f'Traceback (most recent call last):\n  File "{threads.__file__}", line 182, in wait\n    self.results.append(future.result())\n                        ^^^^^^^^^^^^^^^\n  File "{futures._base.__file__}", line 449, in result\n    return self.__get_result()\n           ^^^^^^^^^^^^^^^^^^^\n  File "{futures._base.__file__}", line 401, in __get_result\n    raise self._exception\n  File "{futures.thread.__file__}", line 58, in run\n    result = self.fn(*self.args, **self.kwargs)\n             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n  File "{__file__}", line 82, in fake_func_error\n    raise KeyError(\'MyKey\')\nKeyError: \'MyKey\'\n'
        )

    @skipUnless(version_info.major == 3 and version_info.minor == 12, 'Only works in Python 3.12')
    @mock.patch.object(threads.log, 'error')
    def test_silent_thread_3_12(self, error: mock.MagicMock):
        pool = threads.ThreadPool(concurrency=1, silent=True)
        pool.add(target=fake_func_error)
        pool.wait()
        error.assert_called_once_with(
