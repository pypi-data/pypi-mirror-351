...
def parse_cuts(
    ...
    """
    try:
        all_cuts = [(key, set(values.split(","))) for key, values in split_dict(cuts, ";")]
    except ValueError:
        raise HTTPException(400, "Malformed 'cuts' parameter") from None

    cuts_include = {key: values for key, values in all_cuts if not key.startswith("~")}
    cuts_exclude = {key.lstrip("~"): values for key, values in all_cuts if key.startswith("~")}
...
def parse_drilldowns(
    ...
    return [item for item in split_list(complementary_drilldowns or []) if item]
