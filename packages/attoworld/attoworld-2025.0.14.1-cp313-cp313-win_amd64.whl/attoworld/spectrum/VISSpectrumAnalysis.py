...
def eliminate_outliers(y, threshold: float=3, window_points: int=20):
    ...
    print(n_outliers, " outliers detected and replaced with mean of surrounding values.")

def read_spectrum_maya(filename, remove_offsets_individually=False, nm_smearing=1., eliminate_outliers_spectrum=False):
    ...
    for i in range(len(data.columns)-1):
        if nm_smearing > 0:
            spectrum_i = gaussian_filter1d(np.array(data.iloc[:, i+1]), sigma=nm_smearing/(wvl[1]-wvl[0]))
        else:
            spectrum_i = np.array(data.iloc[:, i+1])
        if eliminate_outliers_spectrum:
            eliminate_outliers(spectrum_i, threshold=3, window_points=20)
            spectrum_i = gaussian_filter1d(spectrum_i, sigma=final_nm_smearing/(wvl[1]-wvl[0]))
...
class SpectrumHandler:
    ...
    def __init__(self, filename: str = None, wavelengths = None, spectrum = None, remove_offsets_individually: bool=False, nm_smearing=1., eliminate_outliers_spectrum=False, filetype='MayaScarab'):
        ...
        if filetype == 'MayaScarab' and filename is not None:
            self.wvl, self.spectrum = read_spectrum_maya(filename, remove_offsets_individually, nm_smearing, eliminate_outliers_spectrum)
    ...
    def get_calibration_factor(self):
        if self.calibration_factor is None:
            print("Warning: Calibration factor not available. Computing it from given spectra...")
    ...
    def calibrate(self):
        ...
        if self.calibration_factor is None:
            print("Warning: Calibration factor not available. Try load_calibration_factor_from_file(). Returning original spectrum.")
        else:
    ...
    def save_calibration_factor_to_file(self, filename):
        ...
        if '.txt' in filename or '.csv' in filename:
            data = pandas.DataFrame({'Wavelength': self.wvl, 'Calibration Factor': self.calibration_factor,
                                     'Lamp Spectrum': self.calibration_lamp_spectrum, 'Measured Spectrum': self.spectrum})
            data.to_csv(filename, index=False, sep='\t')
        elif '.npz' in filename:
            np.savez(filename, wavelength=self.wvl, corr_factor_smoothed=self.calibration_factor,
                     lamp_ref=self.calibration_lamp_spectrum, lamp_measured=self.spectrum)
        else:
            raise ValueError("File type not supported. Use .txt, .csv or .npz.")

    def load_calibration_factor_from_file(self, filename):
        data = pandas.read_table(filename, sep='\t', keep_default_na=True, skiprows=0)
        self.calibration_factor = np.interp(self.wvl, np.array(data.iloc[:, 0]), np.array(data.iloc[:, 1]))
    ...
    def check_wvl_ascending(self):
        if self.wvl is not None and np.any(np.diff(self.wvl)) < 0:
            if np.all(np.diff(self.wvl) < 0):
                ...
                raise ValueError("Wavelength array must be either in ascending or descending order.")
        #same for calib spectrum
        if self.calibration_lamp_wvl is not None and np.any(np.diff(self.calibration_lamp_wvl)) < 0:
    ...
    def load_calibration_lamp_data(self, filename="./calibration_data/7315273LS-Deuterium-Halogen_CC-VISNIR.lmp"):
        ...
        self.calibration_lamp_wvl, self.calibration_lamp_spectrum = np.array(Data.iloc[:, 0]), np.array(Data.iloc[:, 1])
        self.calibration_lamp_spectrum = gaussian_filter1d(self.calibration_lamp_spectrum,
                                                self.nm_smearing / (self.calibration_lamp_wvl[1] - self.calibration_lamp_wvl[0]))
        self.check_wvl_ascending()
    ...
    def plot_calibration_data(self, low_lim=None, up_lim=None, low_lim_y=None, up_lim_y=None, wavelength_ROI: list = [420, 800]):
        ...
        ax.plot(self.wvl, self.spectrum / np.max(self.spectrum), linewidth=lineWidth, label='spectrometer reading')
        ...
        ax.plot(self.calibration_lamp_wvl, self.calibration_lamp_spectrum / np.max(self.spectrum) * multipl_factor_lamp_plot,
                linewidth=lineWidth, label='actual lamp spectrum')
        if self.calibration_factor is not None:
            ax.plot(self.wvl, self.calibration_factor, linewidth=lineWidth, label='calibration factor')
        ax.set_title('Calibration measurement maya Oceanoptics', fontsize=fontSize)
    ...
    def tukey_filter(self, edge1: float, edge2: float, edge1_width: float, edge2_width: float):
        ...
        if self.calibration_lamp_wvl is not None and self.calibration_lamp_spectrum is not None:
            window = asymmetric_tukey_window(self.calibration_lamp_wvl, edge1, edge2, edge1_width, edge2_width)
    ...
    def compute_calibration_factor_spectrometer(self, transmission_additional_optics=None, smoothing='poly', extend_calibration: bool = False, wavelength_ROI: list = [420, 800]):
        ...
        if transmission_additional_optics is not None:
            for sao in transmission_additional_optics:
                if not isinstance(sao, SpectrumHandler):
                    raise TypeError("Input of spectra_additional_optics must be a list of SpectrumHandler objects.")
                wvl_sao, spectrum_sao = sao.get_spectrum()
                if np.any(np.diff(wvl_sao)) < 0:
                    raise ValueError("Wavelength array of additional optics spectrum must be sorted in ascending order.")
                wvl_sao = np.concatenate((np.array([0]), wvl_sao, np.array([max(np.max(self.wvl), np.max(wvl_sao))])))
                spectrum_sao = np.concatenate((np.array([spectrum_sao[0]]), spectrum_sao, np.array([spectrum_sao[-1]])))
                self.spectrum = self.spectrum * np.interp(self.wvl, wvl_sao, spectrum_sao)
        ...
        if smoothing is None:
            ...
            if not isinstance(smoothing, int):
                raise TypeError("Input of smoothing must be 'poly', None or an integer.")
            if smoothing < 1:
        ...
        self.calibration_factor = self.calibration_factor / np.mean(self.calibration_factor[
        ...
        if extend_calibration:
            ...
            self.wvl = np.concatenate((np.linspace(0, self.wvl[0], 100), self.wvl, np.linspace(self.wvl[-1], self.wvl[-1], 100)))
            self.calibration_factor = np.concatenate((np.ones(100)*self.calibration_factor[0], self.calibration_factor, np.ones(100)*self.calibration_factor[-1]))
            self.calibration_factor = np.interp(wvl_stored, self.wvl, self.calibration_factor)
            self.wvl = wvl_stored
    ...
    def plot_spectrum(self, low_lim=None, up_lim=None):
        ...
        if low_lim is not None and up_lim is not None:
            ...
            ax.plot(self.wvl, self.spectrum, linewidth=lineWidth)
...
class MultiSpectrumHandler:
        ...
        if filenameList is not None:
            for filename in filenameList:
                self.spectrumHandlers.append(SpectrumHandler(filename=filename, remove_offsets_individually=remove_offsets_individually,
                                                             nm_smearing=nm_smearing, eliminate_outliers_spectrum=eliminate_outliers_spectrum, filetype=filetype))
        elif wavelengthList is not None and spectrumList is not None:
            for i in range(len(wavelengthList)):
                self.spectrumHandlers.append(SpectrumHandler(wavelengths=wavelengthList[i], spectrum=spectrumList[i],
                                                             remove_offsets_individually=remove_offsets_individually, nm_smearing=nm_smearing,
                                                             eliminate_outliers_spectrum=eliminate_outliers_spectrum, filetype=filetype))
            ...
            for spectrumHandler in spectrumHandlerList:
                if not isinstance(spectrumHandler, SpectrumHandler):
                    raise TypeError("Input of spectrumHandlerList must be a list of SpectrumHandler objects.")
    ...
    def plot(self, low_lim = None, up_lim = None):
        ...
        for i, spectrumHandler in enumerate(self.spectrumHandlers):
            ...
            if low_lim is not None and up_lim is not None:
                ax.plot(wvl[(wvl>low_lim)&(wvl<up_lim)], spectrum[(wvl>low_lim)&(wvl<up_lim)], linewidth=lineWidth, label=f'Spectrum {i+1}')
            else:
                ax.plot(wvl, spectrum, linewidth=lineWidth, label=f'Spectrum {i+1}')
        ax.set_title('Spectra', fontsize=fontSize)
        ax.set_xlabel('Wavelength [nm]', fontsize=fontSize)
        ax.set_ylabel('Intensity [a.u.]', fontsize=fontSize)
        ax.legend(fontsize=legendFontSize,
                  loc='upper right')
