...
def calibrate(data, column: int, calibration_file_path="./calibration_data/Reso_Spectrometer_CalibrationCorrection.npz", dark = None, dark_c = None, stitch: bool = False, smooth_points: int = 10, null_calibration: bool = False, wavelength_calibration_intercept: float = 3.538, wavelength_calibration_slope: float = 1.003427):
    ...
    if stitch:
        ...
        theoretically_complete_wvl = data[:, 7 * (column) + 1]*cal_slope + cal_intercept
        theoretically_complete_wvl = theoretically_complete_wvl[~np.isnan(theoretically_complete_wvl)]
        if theoretically_complete_wvl[-1]-theoretically_complete_wvl[0] < 900:
            raise ValueError('in function calibrate(), the spectrum to be stitched is not a full spectrum, please check the column index')

        extrema = []
        spectra = []
        for i in range(5):
            wvl_single_sp = data[:, 7 * (column - 5 + i) + 1]*cal_slope + cal_intercept
        ...
        if dark is None or dark_c is None:
            calibr_interpolated = np.interp(wavelength, wavelength_calibration, calibration_smoothed)
            if null_calibration:
                calibr_interpolated = calibr_interpolated*0. +1
            spectrum = (data[:, 7*column +2] - np.min(data[:, 7*column +2]))*calibr_interpolated
        else:
            calibr_interpolated = np.interp(wavelength, wavelength_calibration, calibration_smoothed)
            if null_calibration:
                calibr_interpolated = calibr_interpolated*0. +1
            spectrum = (data[:, 7*column +2] - dark[:, 7*dark_c +2])*calibr_interpolated
...
def plot_spectra_UVsp(filenameList, columnList, pdfFilename, legendItemList = None,
    ...
    if darkTupleList is not None:
        for (tp, data, column) in zip(darkTupleList, dataList, columnList):
            ...
            for i in range(len(data[:, 7*column+2])):
                if dark[i, 7*tp[1]+2 ] != dark[i, 7*tp[1]+2 ] and data[i, 7*column+2] == data[i, 7*column+2]:
                    print('Warning: in function plot_spectra_UVsp\ndark spectrum is shorter than the spectrum to be calibrated'
                          '\nplease check the column index of the dark spectrum')
                    break
                elif data[i, 7*column+2] != data[i, 7*column+2] and dark[i, 7*tp[1]+2 ] == dark[i, 7*tp[1]+2 ]:
    ...
    if darkTupleList is not None and not do_calibrate:
        for (tp, data, column) in zip(darkTupleList, dataList, columnList):
            dark = read_spectrometer_excel(tp[0])
            for i in range(len(data[:, 7*column+2])):
                data[i, 7*column+2] = data[i, 7*column+2]-dark[i, 7*tp[1]+2 ]
    ...
    if color_gradient:
        ...
        cmap = mpl.colormaps['magma'] #'viridis', 'plasma', 'inferno', 'magma', 'cividis'
