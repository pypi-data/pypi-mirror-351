...
class Group_Class_Error(dmde.ClassError):
    def __init__(self, cls_instance, message : str = None, entity_id: str = None):
        super().__init__(cls_instance=cls_instance, message=message, entity_id = entity_id)
...
async def search_by_name(
    ...
    if isinstance(group_name, str):
        filter_groups = [dg for dg in domo_groups if dg.name.lower() == group_name.lower()]

    if isinstance(group_name, list):
        filter_groups = [dg for dg in domo_groups if dg.name.lower() in [gname.lower() for gname in group_name]]


    if not filter_groups:
        raise Group_Class_Error(cls_instance = self, entity_id = self.auth.domo_instance, message = f"{len(domo_groups)} retrieved.  unable to find a group matching {group_name}")
...
async def upsert(
    ...
    try:
        await domo_group.update_metadata(
            group_type=group_type, description=description, debug_api=debug_api, session = session
        )
