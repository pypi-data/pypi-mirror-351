...
class SSHClient(object):
    ...
    def is_sshable(self):
        ...
        try:
            ...
            log.warning(f"{self.__ip}:{self.__port} | cannot create ssh session.")
            return False
    ...
    def __connect(self):
        ...
        try:
            ...
            log.error(f"{self.__ip}:{self.__port} | failed to ssh connect. err msg is {str(e)}")
            return False
            ...
            if promt_response:
                ...
                while not channel.closed or channel.recv_ready() or channel.recv_stderr_ready():
                    ...
                    if channel.recv_stderr_ready():
                        stderr=channel.recv_stderr(2024).decode('utf-8', 'ignore')
                ...
                log.info(
                    f" {self.__ip}:{self.__port} | successful to run cmd {cmd}, output is:{std_output} and stderr:{std_err}.")
