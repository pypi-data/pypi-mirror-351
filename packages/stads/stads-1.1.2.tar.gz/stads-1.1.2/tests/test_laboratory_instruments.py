...
class TestLaboratoryInstrument(unittest.TestCase):
    def setUp(self):
        self.instrument = LaboratoryInstrument(
            image_shape=(100, 100),
            number_of_frames=3,
    ...
    @patch.object(LaboratoryInstrument, '_run_adaptive_sampler', return_value=([30], [0.9]))
    @patch.object(LaboratoryInstrument, '_run_random_sampler', return_value=([25], [0.85]))
    @patch.object(LaboratoryInstrument, '_run_stratified_sampler', return_value=([20], [0.8]))
