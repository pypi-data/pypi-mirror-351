...
from src.stads import evaluation
from src.stads.config import REFERENCE_VIDEO_SEQUENCE
from src.stads.image_processing import generate_scan_pattern_from_pdf
from src.stads.interpolator import ImageInterpolator
from src.stads.stads_helpers import compute_local_moments_of_image, compute_pdf_from_gradients_image, \
    compute_local_temporal_variance, compute_optical_flow
from src.stads.microscope import sample_image_from_video_sequence
...
class AdaptiveSampler:
    ...
    def initialize_sampling(self):
        if self.initialSampling == 'uniform':
            randomSampler = RandomSampler(self.imageShape, self.sparsityPercent)
            return randomSampler.get_coordinates()
        elif self.initialSampling == 'stratified':
            stratifiedSampler = StratifiedSampler(self.imageShape, self.sparsityPercent)
