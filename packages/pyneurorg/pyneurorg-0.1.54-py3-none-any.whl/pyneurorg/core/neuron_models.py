# src/pyneurorg/core/neuron_models_DEBUG.py

"""
DEBUG VERSION of neuron models.
...
def _add_common_stim_syn_vars_and_flags(base_eqs_str: str, 
    ...
    """
    print(f"--- DEBUG [_add_common_stim_syn_vars_and_flags]: Called with ---")
    print(f"  num_stim_flags: {num_stim_flags}")
    print(f"  current_vars_units: {current_vars_units}")
    print(f"  Initial base_eqs_str:\n'''{base_eqs_str}'''")
    print(f"  Initial base_namespace_dict: {base_namespace_dict}")
    ...
    print(f"DEBUG: Current declarations to be added: {current_declarations}")
    ...
    print(f"DEBUG: Flag equations list: {flag_eqs_list}")
    print(f"DEBUG: Default flags init namespace: {default_flags_init_ns}")
    ...
    print(f"DEBUG: Final generated full_eqs_str:\n'''{full_eqs_str}'''")
    ...
    print(f"DEBUG: Final updated_namespace: {updated_namespace}")
    print(f"--- DEBUG [_add_common_stim_syn_vars_and_flags]: Returning ---")
...
def LIFNeuron(tau_m=10*b2.ms, v_rest=0*b2.mV, v_reset=0*b2.mV,
    ...
    """DEBUG: Leaky Integrate-and-Fire (LIF) neuron model."""
    print(f"--- DEBUG [LIFNeuron]: Called with num_stim_flags={num_stim_flags} ---")
    base_eqs = """
...
def IzhikevichNeuron(a=0.02/b2.ms, b=0.2*b2.nS, c=-65*b2.mV, d=2*b2.pA,
    ...
    """DEBUG: Izhikevich neuron model."""
    print(f"--- DEBUG [IzhikevichNeuron]: Called with num_stim_flags={num_stim_flags} ---")
...
def AdExNeuron(C_m=281*b2.pF, g_L=30*b2.nS, E_L=-70.6*b2.mV,
               ...
               num_stim_flags=16):
    """DEBUG: Adaptive Exponential I&F (AdEx) neuron model."""
    print(f"--- DEBUG [AdExNeuron]: Called with num_stim_flags={num_stim_flags} ---")
    base_eqs = """
...
def QIFNeuron(tau_m=20*b2.ms, v_rest=0*b2.mV, v_c=10*b2.mV, 
    ...
    """DEBUG: Quadratic Integrate-and-Fire (QIF) neuron model."""
    print(f"--- DEBUG [QIFNeuron]: Called with num_stim_flags={num_stim_flags} ---")
    base_eqs = """
...
def SimpleHHNeuron(C_m=1*b2.uF/b2.cm**2, E_Na=50*b2.mV, E_K=-77*b2.mV, E_L=-54.4*b2.mV,
                   ...
                   num_stim_flags=16):
    """DEBUG: Simplified Hodgkin-Huxley (HH) neuron model."""
    print(f"--- DEBUG [SimpleHHNeuron]: Called with num_stim_flags={num_stim_flags} ---")
    ...
    return {
        ...
        'refractory': refractory_period, 'namespace': full_namespace, 'method': 'exponential_euler' 
    }
...
def LIFCalciumFluorNeuron(tau_m=10*b2.ms, v_rest=0*b2.mV, v_reset=0*b2.mV,
                          ...
                          num_stim_flags=16):
    """DEBUG: LIF neuron with conceptual Calcium and Fluorescence dynamics."""
    print(f"--- DEBUG [LIFCalciumFluorNeuron]: Called with num_stim_flags={num_stim_flags} ---")
    base_eqs = """
    dv/dt = (-(v - v_rest_val) + R_m_val * (I_synaptic + I_stimulus_sum + I_tonic_val)) / tau_m_val : volt (unless refractory)
    ...
    base_namespace = {
        ...
        'v_default_init': v_rest, 'Ca_default_init': 0.0, 'F_default_init': 0.0,
    }
    full_eqs, full_namespace = _add_common_stim_syn_vars_and_flags(
        base_eqs, base_namespace, num_stim_flags, current_vars_units="amp"
    )
    ...
    return {
        'model': full_eqs, 'threshold': f'v > {v_thresh!r}',
