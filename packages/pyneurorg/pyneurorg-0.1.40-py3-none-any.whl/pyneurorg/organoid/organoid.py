...
# from .spatial import _ensure_um_quantity # No longer needed if spatial functions handle conversion
...
class Organoid:
    ...
    def add_neurons(self, name, num_neurons, model_name, model_params=None,
        ...
        Ensures that stimulus-related variables (I_stimulus_sum, flags)
        ...
        model_def = model_func(**current_model_params) # This now passes num_stimulus_flags
        ...
        if positions is not None:
            ...
            if neuron_positions_um_qty.shape[0] != num_neurons:
                 raise ValueError(f"Positions shape ({neuron_positions_um_qty.shape[0]},3) mismatch num_neurons ({num_neurons}).")
        ...
        # These should now be covered by _default_init from _add_stimulus_components_to_model
        # but explicit check for existence in equations is good.
        ...
        # Boolean flags (stim_target_flagX) have their _default_init (False)
        # already in final_initial_values from model_namespace_defaults via _add_stimulus_components_to_model.

        if initial_values: # User-provided values take highest precedence
        ...
        for var_name, value in final_initial_values.items():
            if hasattr(ng, var_name):
                try:
                    ...
                    print(f"Warning: Could not set initial value for '{var_name}' on NG '{name}'. Error: {e}")
    ...
    # ... (add_synapses, getters, __str__, __repr__ as previously defined) ...
