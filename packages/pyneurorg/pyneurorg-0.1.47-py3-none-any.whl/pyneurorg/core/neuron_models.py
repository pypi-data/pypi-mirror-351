...
def _add_common_stim_syn_vars_and_flags(base_eqs_str: str, 
    ...
    ----------
    base_eqs_str : str
        The base model equations string, which should use I_synaptic and I_stimulus_sum.
    base_namespace_dict : dict
...
def IzhikevichNeuron(a=0.02/b2.ms, b=0.2*b2.nS, c=-65*b2.mV, d=2*b2.pA,
                     v_init=-70*b defaults for these common variables
    updated_namespace = base_namespace_dict.copy()
    ...
    updated_namespace.setdefault('I_synaptic_default_init', 0 * default_current_unit_obj)
    updated_namespace.setdefault('I_stimulus_sum_default_init', 0 * default_current_unit_obj)
    updated_namespace.update(default_flags_init_ns) # Adds stfX_default_init =2.mV, u_init=None, C_m=100*b2.pF,
    ...
    base_eqs = """
    dv/dt = (k_val * (v - v_rest_iz_val) * (v - v_thresh_iz_val) - u + I_synaptic + I_stimulus_sum + I_tonic_val) / C_m_val : volt False
...
def IzhikevichNeuron(a=0.02/b2.ms, b=0.2*b2.nS, c=-65*b2.mV, d=2*b2.pA,
                     ...
                     k=0.7*b2.nS/b2.mV, v_rest_iz=-6euler'
    }
