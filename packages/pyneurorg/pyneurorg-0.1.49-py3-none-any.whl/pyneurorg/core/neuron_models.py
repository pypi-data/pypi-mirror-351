...
def _add_common_stim_syn_vars_and_flags(base_eqs_str: str, 
    ...
    Ensures I_synaptic and I_stimulus_sum are declared.
    ...
    # They are declared here to ensure they exist in the final model string.
    ...
    for i in range(num_stim_flags):
        ...
        flag_eqs_list.append(f"{flag_name} : boolean") # This line is syntactically correct
    ...
    full_eqs_str = "\n".join(filter(None, full_eqs_str_parts)) # filter(None,...) removes empty strings if any
    ...
    # setdefault only adds if key is not already present from base_namespace_dict
    ...
    updated_namespace.update(default_flags_init_ns) # Adds or overwrites stfX_default_init = False
    
    return full_eqs_str, updated_namespace


def LIFNeuron(tau_m=10*b2.ms, v_rest=0*b2.mV, v_reset=0*b2.mV,
              v_thresh=20*b2.mV, R```python
# src/pyneurorg/core/neuron_models.py
...
import brian2 as b2
from brian2.units.fundamentalunits import DIMENSIONLESS # For checking dimensionless quantities, e.g. phase
...
def _add_common_stim_syn_vars_and_flags(base_eqs_str: str, 
    ...
    base_eqs_str : str
        The base model equations string. This string should already utilize
        I_synaptic and I_stimulus_sum where appropriate (e.g., in a summed current term
        or directly in dv/dt if they represent the total external/synaptic current).
    base_namespace_dict : dict
        The base model namespace dictionary.
    num_stim_flags : int, optional
        Number of boolean stimulus target flags to create (stf0, stf1, ...).
...
def _add_common_stim_syn_vars_and_flags(base_eqs_str: str, 
    ...
    updated_namespace.setdefault('I_synaptic_default_init', 0 * default_current_unit_obj)
    updated_namespace.setdefault('I_stimulus_sum_default_init', 0 * default_current_unit_obj)
    updated_namespace.update(default_flags_init_ns) # Adds/overwrites stfX_default_init = False
...
def LIFNeuron(tau_m=10*b2.ms, v_rest=0*b2.mV, v_reset=0*b2.mV,
    ...
    # I_synaptic and I_stimulus_sum will be declared by the helper function.
    # This base_eqs string should use them.
    base_eqs = """
    dv/dt = (-(v - v_rest_val) + R_m_val * (I_synaptic + I_stimulus_sum + I_tonic_val)) / tau_m_val : volt (unless refractory)
_m=100*b2.Mohm, I_tonic=0*b2.nA,
              refractory_period=2*b2.ms, num_stim_flags=16):
    """Leaky Integrate-and-Fire (LIF) neuron model with stimulus capabilities."""
    base_eqs = """
    dv/dt = (-(v - v_rest_val) + R_m_val * (I_synaptic + I_stimulus_sum + I_tonic_val)) / tau_m_val : volt (unless refractory)
...
def LIFCalciumFluorNeuron(tau_m=10*b2.ms, v_rest=0*b2.mV, v_reset=0*b2.mV,
                          ...
                          tau_ca=50*b2.ms, ca_spike_increment=0.2, # Example Ca params (unitless increment here for Ca)
                          tau_f=100*b2.ms, k_f=0.5, # Example Fluor params
                          num_stim_flags=16):
    """LIF neuron with conceptual Calcium and Fluorescence dynamics."""
    base_eqs = """
    dv/dt = (-(v - v_rest_val) + R_m_val * (I_synaptic + I_stimulus_sum + I_tonic_val)) / tau_m_val : volt (unless refractory)
    dCa/dt = -Ca / tau_ca_val : 1 (unless refractory) # Calcium dynamics (Ca is unitless here)
    dF/dt = (k_f_val * Ca - F) / tau_f_val : 1 (unless refractory) # Fluorescence dynamics (F is unitless here)
    """
    base_namespace = {
        ...
        'Ca_default_init': 0.0, # Initial calcium concentration (unitless)
        'F_default_init': 0.0,   # Initial fluorescence (unitless)
        'ca_spike_increment_val': ca_spike_increment # []
    full_eqs_str_parts.extend(current_declarations) 
    full_eqs_str_parts.append(base_eqs_str)         
    if flag_eqs_list:
        full_eqs_str_parts.extend(flag_eqs_list)    
...
# Conceptual LIF neuron with added Calcium and Fluorescence dynamics
def LIFCalciumFluorNeuron(tau_m=10*b2.ms, v_rest=0*b2.mV, v_reset=0*b2.mV,
                          v_thresh=20*b2.mV, R_m=100*b2.Mohm, I_tonic=0*b2.nA,
                          refractory_period=2*b2.ms, 
                          tau_ca=50*b2.ms, ca_spike_increment=0.2, # Unitless Ca increment
                          tau_f=100*b2.ms, k_f=0.5, # Unitless k_f, F also unitless
                          num_stim_flags=16):
    """LIF neuron with conceptual Calcium and Fluorescence dynamics."""
    base_eqs = """
    dv/dt = (-(v - v_rest_val) + R_m_val * (I_synaptic + I_stimulus_sum + I_tonic_val)) / tau_m_val : volt (unless refractory)
    dCa/dt = -Ca / tau_ca_val : 1 (unless refractory) # Calcium dynamics (Ca is unitless here)
    dF/dt = (k_f_val * Ca - F) / tau_f_val : 1 (unless refractory) # Fluorescence dynamics (F is unitless)
    """
    base_namespace = {
        ...
        'Ca_default_init': 0.0, 
        'F_default_init': 0.0,
        # ca_spike_increment is used in reset, so it needs to be in the final namespace
    }
    full_eqs, full_namespace = _add_common_stim_syn_vars_and_flags(
        base_eqs, base_namespace, num_stim_flags, current_vars_units="amp"
    )
    
    # Add ca_spike_increment to the final namespace for use in the reset string
    full_namespace['ca_spike_increment_val'] = ca_spike_increment

    return {
        'model': full_eqs,
        'threshold': f'v > {v_thresh!r}',
        'reset': f'v = {v_reset!r}; Ca += ca_spike_increment_val', # Increment Ca on spike
        'refractory': refractory_period,
        'namespace': full_namespace,
        'method': 'euler' # Euler likely needed due to coupled Ca/F dynamics
    }