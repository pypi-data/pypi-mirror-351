...
class Simulator:
    ...
    def add_stimulus(self, electrode_id: int, stimulus_waveform: b2.TimedArray,
                     ...
                     flag_variable_template: str = "stim_target_flag{id}"): # CORRECTED DEFAULT
        """
        ...
        if current_flag_name not in target_ng.variables:
            raise AttributeError(
                ...
                f"'{current_flag_name}' defined in its equations. Ensure your neuron model "
                f"includes it (e.g., via 'num_stimulus_flags' in neuron_models.py using prefix '{flag_variable_template.split('{')[0]}')."
            )
