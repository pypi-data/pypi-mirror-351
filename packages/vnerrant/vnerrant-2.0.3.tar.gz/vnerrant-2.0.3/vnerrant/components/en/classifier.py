...
class Classifier(BaseClassifer):
    ...
    @staticmethod
    def get_pharse_repetition(toks: list[Token], ignore_trailing_punct: bool = True, case_insensitive: bool = True) -> str | None:

        list_toks = list(toks)
        if ignore_trailing_punct:
            # Remove trailing punctuation
            while list_toks and list_toks[-1].is_punct:
                list_toks.pop()
    ...
    def is_repetition(self, o_toks: list[Token], ignore_punctuation: bool = True) -> bool:
        """
        ...
        def remove_punctuation(text: str) -> str:
            ...
            """
            return "".join([c for c in text if c.isalnum() or c.isspace()]).strip().lower()
        ...
        while ref_cur_end_token > 0 and ref_cur_end_token >= cur_start_token and o_toks[0].doc[ref_cur_end_token - 1].is_punct:
            ref_cur_end_token -= 1
        ...
        if next_start_token >= len(o_toks[0].doc) or next_end_token >= len(o_toks[0].doc):
        ...
        if cur_text.lower() == prev_text.lower() or cur_text.lower() == next_text.lower():
            return True
        return False
    ...
    def _get_one_sided_type(self, toks: list[Token]) -> str:
        """
        ...
        if self.is_filler_word(toks):
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.FILLER_PAUSE

        if self.is_repetition(toks):
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.REPETITION
        if self.is_false_start(toks):
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.FALSE_START

        # Special cases
        if len(toks) == 1:
            # Possessive noun suffixes; e.g. ' -> 's
            if toks[0].tag_ == "POS":
                return ParentErrorType.NOUN + SeparatorTypes.COLON + ChildrenErrorType.POSSESSIVE
            ...
            if toks[0].lower_ == "to" and toks[0].pos == POS.PART and toks[0].dep_ != "prep":
                return ParentErrorType.VERB + SeparatorTypes.COLON + ChildrenErrorType.FORM
        ...
        if len(set(dep_list)) == 1 and dep_list[0] in dependency_resources.dep_map.keys():
    ...
    def _get_two_sided_type(self, o_toks: list[Token], c_toks: list[Token]) -> str:
        ...
        if len(o_toks) == len(c_toks) == 1:
            ...
            if (
                ...
                or (o_toks[0].lower_ in dependency_resources.conts and c_toks[0].lower_ in dependency_resources.aux_verbs)
                or (c_toks[0].lower_ in dependency_resources.conts and o_toks[0].lower_ in dependency_resources.aux_verbs)
            ) and o_pos == c_pos:
                # Both sides are contractions; e.g. "he's" -> "he is"
            ...
            if (
                o_toks[0].lower_ in dependency_resources.conts or c_toks[0].lower_ in dependency_resources.conts
            ) and o_pos == c_pos:  # TODO: why is this check here?
                return ParentErrorType.CONTRACTION
            ...
            if o_toks[0].lower_ in dependency_resources.aux_conts or c_toks[0].lower_ in dependency_resources.aux_conts:
                return ParentErrorType.VERB + SeparatorTypes.COLON + ChildrenErrorType.TENSE
            ...
            if o_toks[0].text.isalpha():
                ...
                if o_toks[0].text not in language_resources.spell and o_toks[0].lower_ not in language_resources.spell:
                    ...
                    if o_toks[0].lemma == c_toks[0].lemma:
                        ...
                        if o_pos == c_pos and o_pos[0] in {"NOUN", "VERB"}:
                            return o_pos[0] + SeparatorTypes.COLON + ChildrenErrorType.INFLECTION
            ...
            if (
                ...
                if o_pos == c_pos:
                    ...
                    if o_pos[0] == "ADJ":
                        return ParentErrorType.ADJECTIVE + SeparatorTypes.COLON + ChildrenErrorType.FORM
                    # Noun number
                    if o_pos[0] == "NOUN":
                        return ParentErrorType.NOUN + SeparatorTypes.COLON + ChildrenErrorType.NUMBER
                    # Verbs - various types
                    if o_pos[0] == "VERB":
                        ...
                        if o_toks[0].tag_ == "VBZ" or c_toks[0].tag_ == "VBZ":
                            return (
                                ParentErrorType.VERB + SeparatorTypes.COLON + ChildrenErrorType.SUBJECT_VERB_AGREEMENT
                            )
            ...
            if (
                language_resources.stemmer.stem(o_toks[0].text) == language_resources.stemmer.stem(c_toks[0].text)
                and o_pos[0] in pos_resources.open_pos2
                and c_pos[0] in pos_resources.open_pos2
            ):
                return ParentErrorType.MORPHOLOGY
            ...
            if set(o_pos + c_pos) == {"DET", "PRON"} or set((o_toks[0].pos_, c_toks[0].pos_)).issubset(
                {"DET", "PRON", "PROPN"}
            ):
            ...
            if o_toks[0].text.isalpha() and c_toks[0].text.isalpha():
                ...
                if len(o_toks[0].text) == 4:
                    ...
                    if len(c_toks[0].text) > 5 and str_sim > 0.5 and c_pos[0] not in pos_resources.rare_pos:
                        return c_pos[0]
                if len(o_toks[0].text) == 5:
                    ...
                    if len(c_toks[0].text) > 5 and c_pos[0] not in pos_resources.rare_pos:
                        return c_pos[0]
                ...
                if len(o_toks[0].text) > 5 and len(c_toks[0].text) > 5:
                    ...
                    if (
                        o_toks[0].text.startswith(c_toks[0].text) or c_toks[0].text.startswith(o_toks[0].text)
                    ) and str_sim >= 0.66:
                        return ParentErrorType.MORPHOLOGY
        ...
        if len(set(o_dep + c_dep)) == 1 and o_dep[0] in dependency_resources.dep_map.keys():
            return dependency_resources.dep_map[o_dep[0]]
        ...
        if (
            (o_toks[0].lower_ in {"most", "more"} or c_toks[0].lower_ in {"most", "more"})
        ...
        if {o_toks[0].lower_, c_toks[0].lower_}.issubset({"wild", "wildlife", "life", "lives"}):
            return ParentErrorType.ORTHOGRAPHY

        # Verb + preposition; e.g. look at -> see; see -> look at; see as -> look
        if list(set(o_pos + c_pos)) == ["VERB", "PREP"] and "VERB" in o_pos and "VERB" in c_pos:
            return ParentErrorType.VERB
    ...
    def _classify(self, edit: Edit) -> Edit:
        ...
        # Replacement and special cases
            ...
            # Replacement
                ...
                op = Operator.REPLACE
                cat = self._get_two_sided_type(edit.original.tokens, edit.corrected.tokens)
