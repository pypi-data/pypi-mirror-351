...
class Train_SpatialEx:
    def __init__(self,
                 ...
                 obs_path=os.path.join(Data_Dir, 'Human_Breast_Cancer_Rep1/cells.csv'),
                 img_path=os.path.join(Data_Dir,
                                       'Human_Breast_Cancer_Rep1/Xenium_FFPE_Human_Breast_Cancer_Rep1_he_image.ome.tif'),
                 transform_mtx_path=os.path.join(Data_Dir,
                                                 'Human_Breast_Cancer_Rep1/Xenium_FFPE_Human_Breast_Cancer_Rep1_he_imagealignment.csv'),
                 scale=0.363788,
                 cell_diameter=-1,
                 resolution=64,
                 impute_file_path=os.path.join(Data_Dir,
                                               'Human_Breast_Cancer_Rep2/cell_feature_matrix.h5'),
                 impute_obs_path=os.path.join(Data_Dir, 'Human_Breast_Cancer_Rep2/cells.csv'),
                 impute_img_path=os.path.join(Data_Dir,
                                              'Human_Breast_Cancer_Rep2/Xenium_FFPE_Human_Breast_Cancer_Rep2_he_image.ome.tif'),
                 impute_transform_mtx_path=os.path.join(Data_Dir,
                                                        'Human_Breast_Cancer_Rep2/Xenium_FFPE_Human_Breast_Cancer_Rep2_he_imagealignment.csv'),
                 scale_exp=False,
                 prune=10000,
                 num_classes=8,
                 ):
        ...
        self.transform_mtx_path = transform_mtx_path
        self.scale = scale
        self.cell_diameter = cell_diameter
        self.resolution = resolution
        self.impute_file_path = impute_file_path
        self.impute_obs_path = impute_obs_path
        self.impute_img_path = impute_img_path
        self.impute_transform_mtx_path = impute_transform_mtx_path
    ...
    def train(self):
        ...
        adata_raw = pp.Read_Xenium(self.file_path, self.obs_path)
        ...
        adata_raw = pp.Read_Xenium(self.impute_file_path, self.impute_obs_path)
        adata_raw = pp.Preprocess_adata(adata_raw, cell_mRNA_cutoff=0,
...
class Train_SpatialExP:
    def __init__(self,
                 ...
                 loss_fn="mse",
                 norm="batchnorm",
                 h5_path1=os.path.join(Data_Dir,
                                       'Human_Breast_Cancer_Rep1/cell_feature_matrix.h5'),
                 obs_path1=os.path.join(Data_Dir, "Human_Breast_Cancer_Rep1/cells.csv"),
                 h5_path2=os.path.join(Data_Dir,
                                       'Human_Breast_Cancer_Rep2/cell_feature_matrix.h5'),
                 obs_path2=os.path.join(Data_Dir, "Human_Breast_Cancer_Rep2/cells.csv"),
                 num_neighbors=7,
                 graph_kind='spatial',
                 scale=0.363788,
                 cell_diameter=-1,
        ...
        # 文件路径参数（保留原始路径拼接逻辑）
        self.h5_path1 = h5_path1
        self.obs_path1 = obs_path1
        self.h5_path2 = h5_path2
        self.obs_path2 = obs_path2
        ...
        self.selection = pd.read_csv(os.path.join(Data_Dir, 'Selection_by_name.csv'), index_col=0)
        self.panelA = self.selection.index[self.selection['slice1']].tolist()
        self.panelB = self.selection.index[self.selection['slice2']].tolist()
    ...
    def train(self):
        ...
        panel_1b = np.vstack(panel_1b)
        panel_1b = pd.DataFrame(panel_1b)
        panel_1b.columns = self.panelB
        panel_1b['obs_name'] = obs_list
        panel_1b = panel_1b.groupby('obs_name').mean()

        adata_raw = pp.Read_Xenium(self.h5_path1, self.obs_path1)
        adata_raw = pp.Preprocess_adata(adata_raw, cell_mRNA_cutoff=0, selected_genes=self.panelB,
                                        scale=self.scale_exp)  # 不筛除细胞， 构建slice1上的panelB的ground truth
        adata = adata_raw[panel_1b.index]
        graph = pp.Build_graph(adata.obsm['spatial'], graph_type='knn', weighted='gaussian', apply_normalize='row',
                               type='coo')
        ...
        panel_2a = np.vstack(panel_2a)
        panel_2a = pd.DataFrame(panel_2a)
        panel_2a.columns = self.panelA
        panel_2a['obs_name'] = obs_list
        panel_2a = panel_2a.groupby('obs_name').mean()

        adata_raw = pp.Read_Xenium(self.h5_path2, self.obs_path2)
        adata_raw = pp.Preprocess_adata(adata_raw, cell_mRNA_cutoff=0, selected_genes=self.panelA,
                                        scale=self.scale_exp)  # 不筛除细胞，构建slice2上的panelA的ground truth
        adata_slice2 = adata_raw[panel_2a.index]
        graph = pp.Build_graph(adata_slice2.obsm['spatial'], graph_type='knn', weighted='gaussian',
                               apply_normalize='row',
