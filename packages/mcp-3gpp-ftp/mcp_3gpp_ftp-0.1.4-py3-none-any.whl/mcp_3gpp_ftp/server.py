...
def filter_excel_columns_from_url(
    ...
    Returns:
        List[Dict[str, Any]]:
            A list of dicts (one per row) containing only the requested columns,
            and only rows matching all substring filters.

    Raises:
        ValueError:
            • If file_url is not HTTP(S),  
    ...
    # 5. Validate and subset output columns
