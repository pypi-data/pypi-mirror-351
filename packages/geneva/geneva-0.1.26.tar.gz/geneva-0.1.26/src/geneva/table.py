...
class TableReference:
    ...
    def open_checkpoint_store(self) -> CheckpointStore:
        ...
        try:
            return LanceCheckpointStore(self.db_uri + "/ckp")
        except Exception:
    ...
    def open_db(self) -> Connection:
        ...
        return connect(self.db_uri, checkpoint=self.open_checkpoint_store())
...
class Table(LanceTable):
    ...
    def add_columns(
        ...
        backfill : bool
            If set to true, will trigger backfill operaitons with this call
    ...
    def _add_virtual_columns(
        ...
        **kwargs,
        ...
        if kwargs.get("backfill", False):
            kwargs.pop("backfill")
            self.backfill(col_name, **kwargs)
    ...
    def backfill_async(
        ...
        **kwargs,
        ...
        if col_name not in self._ltbl.schema.names:
            raise ValueError(
    ...
    def backfill(
        ...
        **kwargs,
        ...
        if udf is None:
            ...
            udf = self._conn._packager.unmarshal(udf_spec)
        ...
        udf.input_columns = input_columns  # potentially override input_columns

        from geneva.runners.ray.pipeline import run_ray_add_column

        checkpoint_store = self._conn._checkpoint_store
        run_ray_add_column(
            self.get_reference(),
            input_columns,
            {col_name: udf},
    ...
    def search(
        ...
        if (
            query is not None
            or vector_column_name is not None
            or fts_columns is not None
        ):
            raise ValueError("Geneva search only implemented for plain queries")
