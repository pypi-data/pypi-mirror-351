...
logger = logging.getLogger("gc_metasploit")
...
async def _execute_module_rpc(
    ...
    try:
        ...
        logger.info(f"RPC execute() result for {full_module_path}: {exec_result} (type: {type(exec_result)})")

        # --- Process Execution Result ---
        if isinstance(exec_result, bool):
            # Special handling for boolean results
            logger.warning(f"Module {full_module_path} returned a boolean: {exec_result}")
            if exec_result is True:
                # Interpret True as likely success, but unknown job ID
                return {
                    "status": "success",
                    "message": f"{module_type.capitalize()} module {full_module_path} appears to have started successfully, but no job ID was returned.",
                ...
                # False typically indicates failure
                return {
                    "status": "error",
                    "message": f"{module_type.capitalize()} module {full_module_path} execution failed (returned False).",
                    "module": full_module_path
                }
        elif not isinstance(exec_result, dict):
            # Handle any other non-dictionary return types
        ...
        # Continue with dictionary result processing
...
app = FastAPI(
    ...
    version="1.6.3", # Updated version - sync execution is now default for better response handling
)
