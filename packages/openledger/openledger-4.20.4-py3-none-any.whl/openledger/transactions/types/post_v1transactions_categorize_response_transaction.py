# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ...core.serialization import FieldMetadata
from .post_v1transactions_categorize_response_transaction_metadata import (
    PostV1TransactionsCategorizeResponseTransactionMetadata,
)


class PostV1TransactionsCategorizeResponseTransaction(UniversalBaseModel):
    id: typing.Optional[str] = None
    metadata: typing.Optional[PostV1TransactionsCategorizeResponseTransactionMetadata] = None
    updated_at: typing_extensions.Annotated[typing.Optional[dt.datetime], FieldMetadata(alias="updatedAt")] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
