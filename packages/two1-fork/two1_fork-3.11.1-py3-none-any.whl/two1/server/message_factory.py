...
class SwirlMessageFactory:
    ...
    @staticmethod
    async def read_object_async(reader):
        head_buffer = await reader.read(2)
        if len(head_buffer) == 0:
            raise ConnectionError

        (size,) = struct.unpack(">H", head_buffer)
        pkt = await _read_exact_async(size, reader)
...
async def _read_exact_async(n, reader):
    buffer = await reader.read(n)
    n -= len(buffer)
    if n == 0:
        return buffer
