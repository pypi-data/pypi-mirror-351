...
def sell(ctx, **options):
    ...
    try:
        ...
        logger.info(
            click.style(
                "An unknown error occurred.  Please contact support@21.co.",
                fg="magenta",
            )
        )
        sys.exit()
...
@click.option("-w", "--wait-time", type=int, default=10)
...
def start(ctx, services, all, wait_time, no_vm, no_zt_dep, publishing_ip, assume_yes):
    ...
    if no_zt_dep:
        deps_list = [
            (name, installed) for name, installed in deps_list if name != "Docker"
        ]
    ...
    if not all_deps_installed:
        if assume_yes or click.confirm(
            ...
            logger.info(
                click.style(
                    "Installing missing dependencies.", fg=cli_helpers.TITLE_COLOR
                )
            )
    ...
    if isinstance(manager.machine, Two1MachineVirtual):
        logger.info(
            click.style("Checking virtual machine status.", fg=cli_helpers.TITLE_COLOR)
        )
        just_started = False
        ...
        if vm_status == VmState.NOEXIST:
            if assume_yes or click.confirm(
                ...
                try:
                    ...
                    cli_helpers.print_str(
                        "21 virtual machine", ["Created"], "TRUE", True
                    )

                except Two1MachineCreateException:
                    cli_helpers.print_str(
                        "21 virtual machine", ["Not created"], "FALSE", False
                    )
                vm_status = manager.status_machine()
            else:
                sys.exit()
        ...
        if vm_status != VmState.RUNNING:
            try:
                ...
                cli_helpers.print_str(
                    "21 virtual machine", ["Not started"], "FALSE", False
                )
                sys.exit()
            vm_status = manager.status_machine()
        ...
        if no_zt_dep:
            ...
            try:
                if not isinstance(manager.machine, Two1MachineVirtual):
                    ...
                    cli_helpers.start_long_running(
                        "Starting ZeroTier One", manager.start_networking
                    )
    ...
    if manager.get_market_address() == "":
        if no_zt_dep:
            ...
            if not isinstance(manager.machine, Two1MachineVirtual):
                ...
                cli_helpers.start_long_running(
                    "Connecting to 21mkt", manager.connect_market, ctx.obj["client"]
                )
    ...
    logger.info(
        click.style("Initializing micropayments server.", fg=cli_helpers.TITLE_COLOR)
    )
    ...
    if not all:
        logger.info(
            click.style(
                "Checking availability of selected services.",
        ...
        for available_selected_service in available_selected_services:
            cli_helpers.print_str(
                available_selected_service, ["Available"], "TRUE", True
            )

        for unavailable_selected_service in unavailable_selected_services:
            cli_helpers.print_str(
                unavailable_selected_service, ["Unavailable"], "False", False
            )
        ...
        if available_selected_services == set(services):
            ...
            raise click.ClickException(
                click.style(
                    "None of the services you've selected is available.", fg="magenta"
                )
    ...
    logger.info(
        click.style("Pulling images for selected services.", fg=cli_helpers.TITLE_COLOR)
    )
    ...
    for service_name in service_to_pull:
        ...
        def image_sucessfully_pulled_hook(image):
            ...
            cli_helpers.print_str(
                "%s -> %s" % (service_name, image), ["Pulled"], "TRUE", True
            )

        def image_failed_to_pull_hook(image):
            cli_helpers.print_str(
                "%s -> %s" % (service_name, image), ["Failed to pull"], "False", False
            )

        def image_is_local_hook(image):
            service_to_start.add(service_name)
            cli_helpers.print_str(
                "%s -> %s" % (service_name, image), ["Exists locally"], "TRUE", True
            )

        def image_is_malformed_hook(image):
            cli_helpers.print_str(
                "%s -> %s" % (service_name, image),
        ...
        manager.pull_image(
            image_for_service,
            image_sucessfully_pulled_hook,
            image_failed_to_pull_hook,
            image_is_local_hook,
    ...
    published_stats = cli_helpers.prompt_to_publish(
        started_services, manager, publishing_ip, assume_yes=assume_yes
    )
    for stat in published_stats:
        cli_helpers.print_str(stat[0], stat[2], "TRUE" if stat[1] else "FALSE", stat[1])
    ...
    logger.info(
        ...
        + click.style(
            "`21 sell status --detail`", bold=True, fg=cli_helpers.PROMPT_COLOR
        )
        + click.style(
            " to see your microservice balances.", fg=cli_helpers.PROMPT_COLOR
        )
    )
...
@click.argument("services", required=False, nargs=-1)
@click.option("-a", "--all", is_flag=True)
@click.option("--stop-vm", is_flag=True, default=False)
@click.option("--delete-vm", is_flag=True, default=False)
@click.pass_context
@decorators.catch_all
@decorators.capture_usage
def stop(ctx, services, all, stop_vm, delete_vm):
    ...
    try:
        dollars_per_sat = (
            cli_helpers.get_rest_client().quote_bitcoin_price(1).json()["price"]
        )
    except Exception:
    ...
    if (
        manager.status_machine() == VmState.NOEXIST
    ):  # docker isn't running under virtual machine
        if isinstance(manager.machine, Two1MachineVirtual):
            ...
            if stop_vm or delete_vm:
                logger.info(
                    click.style(
                        "Your services are running natively: "
                        "run `21 sell stop --all` to stop services",
                        fg="magenta",
                    )
                )
                sys.exit()
    ...
    if manager.status_machine() == VmState.STOPPED:  # docker-machine stopped
        if delete_vm:
            if not isinstance(manager.machine, Two1MachineVirtual):
                logger.info(
                    click.style(
                        "There are no VMs to stop or delete: "
                        "your services are running natively",
                        fg="magenta",
                    )
                )
            ...
            try:
                cli_helpers.start_long_running(
                    "Deleting virtual machine", manager.delete_machine
                )
                cli_helpers.print_str("Virtual machine", ["Deleted"], "TRUE", True)
            except Two1MachineDeleteException:
                cli_helpers.print_str(
                    "Virtual machine", ["Failed to delete"], "FALSE", False
                )
        else:
            if not stop_vm:
    ...
    if (
        manager.status_machine() == VmState.RUNNING
    ):  # docker is running under virtual machine and is up
        if all:
            try:
                valid_services = manager.get_running_services()
            except Exception:
        ...
        if stop_vm or delete_vm:
            try:
                cli_helpers.start_long_running(
                    "Stopping virtual machine", manager.stop_machine
                )
                cli_helpers.print_str("Virtual machine", ["Stopped"], "TRUE", True)
            except Two1MachineStopException:
                cli_helpers.print_str(
                    "Virtual machine", ["Failed to stop"], "FALSE", False
                )
                sys.exit()
...
def status(ctx, detail):
    ...
    def unknown_state_hook(service_name):
        cli_helpers.print_str(
            service_name.capitalize(), ["Unknown state"], "FALSE", False
        )

    if isinstance(manager.machine, Two1MachineVirtual):
        if not cli_helpers.vm_running_check(
            manager.status_machine() == VmState.RUNNING, log_not_running=True
        ):
            sys.exit()

    cli_helpers.zerotier_service_check(
        manager.status_networking(), log_not_running=True
    )
    ...
    try:
        ...
        for running_service in running_services:
            cli_helpers.print_str(
                running_service.capitalize(), ["Running"], "TRUE", True
            )
        for exited_service in exited_services:
            cli_helpers.print_str(
                exited_service.captitalize(), ["Exited"], "FALSE", False
            )
    except:
    ...
    example_usages = cli_helpers.get_example_usage(
        running_services,
    ...
    logger.info(
        ...
        + click.style("`21 sell list`", bold=True, fg=cli_helpers.PROMPT_COLOR)
        + click.style(
            " to see available microservices you can sell.", fg=cli_helpers.PROMPT_COLOR
        )
    )
...
def list_command(ctx):
    ...
    if len(available_21_services) > 0 or len(available_user_services) > 0:
        if len(available_21_services) > 0:
            ...
            logger.info(
                click.style("Official 21 Microservices", fg=cli_helpers.TITLE_COLOR)
            )
            ...
            logger.info(
                click.style(
                    "There are no official services available at this time.",
                    fg="magenta",
                )
            )
        ...
        if len(available_user_services) > 0:
            ...
            tips.append(
                click.style("run ", fg=cli_helpers.PROMPT_COLOR)
                + click.style(
                    "`21 sell add <service_name> <docker_image_name>`",
                    bold=True,
                    fg=cli_helpers.PROMPT_COLOR,
                ...
                + click.style(
                    " to make your own services available to sell.",
                    fg=cli_helpers.PROMPT_COLOR,
                )
            )
        tips.append(
            click.style("run ", fg=cli_helpers.PROMPT_COLOR)
            + click.style(
                "`21 sell start <services>`", bold=True, fg=cli_helpers.PROMPT_COLOR
            )
            + click.style(
                " to start selling an available microservice.",
                fg=cli_helpers.PROMPT_COLOR,
            )
        )
    else:
        logger.info(
            click.style("There are no services available at this time.", fg="magenta")
        )
        ...
        if len(tips) == 1:
            ...
            for idx, tip in enumerate(tips):
                if idx == 0:
                    logger.info(
                        click.style(
                            "\nTips: (%s) " % (idx + 1), fg=cli_helpers.PROMPT_COLOR
                        )
                        + tip
                    )
                else:
                    logger.info(
                        click.style(
                            "      (%s) " % (idx + 1), fg=cli_helpers.PROMPT_COLOR
                        )
                        + tip
                    )
...
def sweep(ctx):
    ...
    logger.info(
        click.style("Sweeping all service balances.", fg=cli_helpers.TITLE_COLOR)
    )

    provider = TwentyOneProvider()
    try:
        wallet = Two1Wallet.import_from_mnemonic(
            provider, manager.get_services_mnemonic()
        )
    except Exception:
        logger.info(
            click.style(
                "Error: unable to import wallet mnemonic.  Please check to make "
    ...
    if click.confirm(
        click.style(
            "Sweeping %s satoshis to your primary wallet. This will incur a "
        ...
        try:
            ...
            cli_helpers.print_str(
                "Sweep",
                [
                    "Wallet balance (%d satoshis) is less than the dust "
                    "limit. Not Sweeping." % total_value
                ],
                "FAILED",
                False,
            )
            ...
            cli_helpers.print_str(
                "Sweep",
                [
                    "Wallet balance (%d satoshis) would be below the "
                    "dust limit when fees are deducted. "
                    "Aborting sweep." % total_value
                ],
                "FAILED",
                False,
            )
            ...
            cli_helpers.print_str(
                "Sweep",
                [
                    "Swept %d satoshis, excluding %d satoshis of "
                    "fees" % (total_value - fee, fee)
                ],
                "SUCCESS",
                True,
            )
...
def upgrade_21_sell(ctx, services, all, wait_time, no_vm):
    ...
    cli_helpers.start_long_running("Resetting system", resetting_system)

    logger.info(
        click.style(
            "Your 21 VM has been deleted. Please run ", fg=cli_helpers.TITLE_COLOR
        )
        + click.style("21 sell start --all ", fg=cli_helpers.PROMPT_COLOR)
        + click.style(
            "to upgrade your 21 VM and start your " "bitcoin-payable microservices.",
            fg=cli_helpers.TITLE_COLOR,
        )
    )
    return
