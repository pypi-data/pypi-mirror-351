...
lc.EC_POINT_add.argtypes = [c_void_p] * 5
lc.EC_POINT_add.restype = c_int
lc.EC_POINT_make_affine.argtypes = [c_void_p, c_void_p, c_void_p]
lc.EC_POINT_make_affine.restype = c_int
lc.EC_POINT_free.argtypes = [c_void_p]
...
lc.ECDSA_do_verify.argtypes = [c_char_p, c_int, POINTER(OpenSSLSignature), c_void_p]
...
lc.BN_nnmod.argtypes = [c_void_p, c_void_p, c_void_p, c_void_p]
lc.BN_nnmod.restype = c_int
...
lc.EC_GROUP_get_cofactor.argtypes = [c_void_p, c_void_p, c_void_p]
lc.EC_GROUP_get_cofactor.restype = c_int

# Error handling functions
lc.ERR_clear_error.argtypes = []
lc.ERR_peek_error.argtypes = []
lc.ERR_peek_error.restype = c_long
lc.ERR_error_string.argtypes = [c_long, c_char_p]
lc.ERR_error_string.restype = c_char_p

# ERR_load_crypto_strings() was deprecated in OpenSSL 1.1.0 and removed in later versions
# In newer versions, error strings are loaded automatically
try:
    lc.ERR_load_crypto_strings()
except AttributeError:
    # Function doesn't exist in newer OpenSSL versions - this is expected
    pass

# Add ECDSA_SIG accessor functions for OpenSSL 3.x compatibility
lc.ECDSA_SIG_get0_r.argtypes = [POINTER(OpenSSLSignature)]
lc.ECDSA_SIG_get0_r.restype = c_void_p
lc.ECDSA_SIG_get0_s.argtypes = [POINTER(OpenSSLSignature)]
lc.ECDSA_SIG_get0_s.restype = c_void_p
lc.ECDSA_SIG_set0.argtypes = [POINTER(OpenSSLSignature), c_void_p, c_void_p]
lc.ECDSA_SIG_set0.restype = c_int
...
def sig_new_from_bytes(r_bytes, s_bytes):
    ...
    # Create BN objects for r and s
    r_bn = c_void_p(lc.BN_new())
    s_bn = c_void_p(lc.BN_new())

    r_buf = create_string_buffer(r_bytes)
    s_buf = create_string_buffer(s_bytes)
    lc.BN_bin2bn(r_buf, len(r_bytes), r_bn)
    lc.BN_bin2bn(s_buf, len(s_bytes), s_bn)

    # Use ECDSA_SIG_set0 for OpenSSL 3.x compatibility
    # This function takes ownership of the BN objects
    result = lc.ECDSA_SIG_set0(sig, r_bn, s_bn)
    if not result:
        # If setting failed, we need to free the BN objects ourselves
        lc.BN_free(r_bn)
        lc.BN_free(s_bn)
