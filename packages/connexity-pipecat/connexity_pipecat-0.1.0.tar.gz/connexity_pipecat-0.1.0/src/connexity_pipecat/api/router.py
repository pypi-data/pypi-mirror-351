...
def register_routes(app: FastAPI) -> FastAPI:
    ...
    for router_section in routers:
        prefix = router_section["prefix"] if router_section["prefix"] else ''
        tag = f"[Agent]"
        tag += [{prefix.replace('_', ' ').replace('/', '').title()}] if prefix else ''
    ...
    for ws_section in websockets:
        prefix = ws_section["prefix"] if ws_section["prefix"] else ''
