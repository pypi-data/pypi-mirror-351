...
INSPECTOR_DB_CHECK_INTERVAL = 240
...
CKEY_CORE_REPORTER_DEFAULTLOCALE = "default_reporting_locale"
"""Name of the configuration subkey key for ``default_reporting_locale`` configuration in ``core reporter`` configurations."""
CKEY_CORE_REPORTER_DEFAULTTIMEZONE = "default_reporting_timezone"
"""Name of the configuration subkey key for ``default_reporting_timezone`` configuration in ``core reporter`` configurations."""
...
REPORTING_MODES = (
    REPORTING_MODE_SUMMARY,
    REPORTING_MODE_EXTRA,
    REPORTING_MODE_BOTH,
    REPORTING_MODE_NONE,
)
...
REPORTING_TIMING_DEFAULT_INFO_PER = tr_("1day")
REPORTING_TIMING_DEFAULT_INFO_THR = tr_("4weeks")
REPORTING_TIMING_DEFAULT_INFO_REL = tr_("1week")
...
REPORTING_TIMING_TARGET_INFO_THR = tr_("none")
REPORTING_TIMING_TARGET_INFO_REL = tr_("none")
REPORTING_TIMING_TARGET_LOW_THR = tr_("none")
REPORTING_TIMING_TARGET_LOW_REL = tr_("none")
REPORTING_TIMING_TARGET_MEDIUM_THR = tr_("none")
REPORTING_TIMING_TARGET_MEDIUM_REL = tr_("none")
REPORTING_TIMING_TARGET_HIGH_THR = tr_("none")
REPORTING_TIMING_TARGET_HIGH_REL = tr_("none")
REPORTING_TIMING_TARGET_CRITICAL_THR = tr_("none")
REPORTING_TIMING_TARGET_CRITICAL_REL = tr_("none")
...
DFLT_REPORTING_MODE = REPORTING_MODE_EXTRA
...
REPORT_TYPE_TARGET = tr_("target")
REPORT_TYPES = (REPORT_TYPE_SUMMARY, REPORT_TYPE_EXTRA, REPORT_TYPE_TARGET)
...
##
## List of all possible fields in reports (additional information section)
##
REPORT_FIELDS_MAIN_NUMBER = [
    ("ConnCount", tr_("Connection count")),
    ("FlowCount", tr_("Flow count")),
    ("PacketCount", tr_("Packet count")),
    ("ByteCount", tr_("Byte count")),
    ("AvgPacketSize", tr_("Average packet size")),
    ("FlowCountDropped", tr_("Dropped flow count")),
    ("PacketCountDropped", tr_("Dropped packet count")),
    ("ByteCountDropped", tr_("Dropped byte count")),
]

REPORT_FIELDS_MAIN_LIST = [
    ("Ref", tr_("Reference")),
    ("UsernameAndPassword", tr_("Username, password")),
]

# These fields are just for report show view (and event class forms),
# and they are not aggregated, because they can be derived.
REPORT_FIELDS_MAIN_LIST_VIEW = REPORT_FIELDS_MAIN_LIST + [("protocols", tr_("Protocols"))]

## Possible fields in source and target sections.
## These constants are duplicated purely for translations.
REPORT_FIELDS_SOURCE_NUMBER = [
    ("ClockSkew", tr_("Source clock skew")),
    ("InFlowCount", tr_("Source InFlowCount")),
    ("OutFlowCount", tr_("Source OutFlowCount")),
    ("InPacketCount", tr_("Source InPacketCount")),
    ("OutPacketCount", tr_("Source OutPacketCount")),
    ("InByteCount", tr_("Source InByteCount")),
    ("OutByteCount", tr_("Source OutByteCount")),
]

REPORT_FIELDS_TARGET_NUMBER = [
    ("ClockSkew", tr_("Target clock skew")),
    ("InFlowCount", tr_("Target InFlowCount")),
    ("OutFlowCount", tr_("Target OutFlowCount")),
    ("InPacketCount", tr_("Target InPacketCount")),
    ("OutPacketCount", tr_("Target OutPacketCount")),
    ("InByteCount", tr_("Target InByteCount")),
    ("OutByteCount", tr_("Target OutByteCount")),
]

REPORT_FIELDS_SOURCE_LIST = [
    ("Port", tr_("Source port")),
    ("Hostname", tr_("Source hostname")),
    ("MAC", tr_("Source MAC")),
    ("Proto", tr_("Source protocol")),
    ("URL", tr_("Source URL")),
    ("Ref", tr_("Source reference")),
    ("Email", tr_("Source e-mail")),
    ("Router", tr_("Source router")),
    ("Netname", tr_("Source netname")),
    ("Interface", tr_("Source interface")),
    ("BitMask", tr_("Source bitmask")),
    ("services", tr_("Source service")),
    ("X509ExpiredTime", tr_("Source X509 expired at")),
    ("ips", tr_("Source IP addresses")),
]

# These fields are just for report show view (and event class forms),
# because they are aggregated in a different way (sooner) or not
# aggregated at all, but just counted from other fields (IP count).
REPORT_FIELDS_SOURCE_NUMBER_VIEW = REPORT_FIELDS_SOURCE_NUMBER + [("ip_count", tr_("Total source IP count"))]

REPORT_FIELDS_TARGET_LIST = [
    ("Port", tr_("Target port")),
    ("Hostname", tr_("Target hostname")),
    ("MAC", tr_("Target MAC")),
    ("Proto", tr_("Target protocol")),
    ("URL", tr_("Target URL")),
    ("Ref", tr_("Target reference")),
    ("Email", tr_("Target e-mail")),
    ("Router", tr_("Target router")),
    ("Netname", tr_("Target netname")),
    ("Interface", tr_("Target interface")),
    ("BitMask", tr_("Target bitmask")),
    ("services", tr_("Target service")),
    ("X509ExpiredTime", tr_("Target X509 expired at")),
    ("ips", tr_("Target IP addresses")),
]

# These fields are just for report show view (and event class forms),
# because they are aggregated in a different way (sooner) or not
# aggregated at all, but just counted from other fields (IP count).
REPORT_FIELDS_TARGET_LIST_VIEW = REPORT_FIELDS_TARGET_LIST + [("relevant_ips", tr_("Your target IPs"))]
REPORT_FIELDS_TARGET_NUMBER_VIEW = REPORT_FIELDS_TARGET_NUMBER + [("ip_count", tr_("Total target IP count"))]
