...
class TestMentatReportsUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        FilterModel(
            ...
            source_based=True,
            type="basic",
            filter='Node.Name == "org.example.kippo_honey"',
            description="DESC1",
            enabled=True,
        )
        FilterModel(
            ...
            source_based=True,
            type="basic",
            filter='Category == "Recon.Scanning"',
            description="DESC2",
            enabled=True,
        )
        global_filter = FilterModel(
            ...
            source_based=True,
            type="advanced",
            filter='Category == "Anomaly.Traffic"',
            description="DESC3",
            enabled=True,
        )
    ...
    def test_03_storage_thr_cache(self):
        ...
        relapses = self.stcache.relapses("test@domain.org", "low", ttltime + datetime.timedelta(seconds=50), False)
    ...
    def test_04_reporting_settings(self):
        ...
        filter_list = reporting_settings.setup_filters(filter_parser, filter_compiler, False)
        self.assertEqual(
            str(filter_list),
            "[(<Filter(name='FLT_GLOBAL')>, COMPBINOP(VARIABLE('Category') OP_EQ CONSTANT('Anomaly.Traffic'))), "
            "(<Filter(name='FLT1')>, COMPBINOP(VARIABLE('Node.Name') OP_EQ CONSTANT('org.example.kippo_honey'))), "
            "(<Filter(name='FLT2')>, COMPBINOP(VARIABLE('Category') OP_EQ CONSTANT('Recon.Scanning')))]",
        )
