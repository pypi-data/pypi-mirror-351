...
class DetailGridOptions(AgGridResourceBase):
    ...
    def _handle_nested_objects(self, return_dict: dict) -> dict:
        """Handle nested column definitions."""
        if "columnDefs" in return_dict and isinstance(return_dict["columnDefs"], list):
            # Process each column definition recursively
            formatted_columns = []
            for col_def in return_dict["columnDefs"]:
                if isinstance(col_def, dict):
                    formatted_columns.append(ColumnDef(**col_def).dict())
                else:
                    formatted_columns.append(col_def)
            return_dict["columnDefs"] = formatted_columns
        return return_dict
...
class DetailCellRendererParams(AgGridParamsBase):
    ...
    def _handle_nested_objects(self, return_dict: dict) -> dict:
        """Handle nested DetailGridOptions objects."""
        if "detailGridOptions" in return_dict and isinstance(
            return_dict["detailGridOptions"], dict
        ):
            return_dict["detailGridOptions"] = DetailGridOptions(
                **return_dict["detailGridOptions"]
            ).dict()
        return return_dict
