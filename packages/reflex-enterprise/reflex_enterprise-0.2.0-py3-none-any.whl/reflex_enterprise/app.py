...
from reflex_enterprise.config import ConfigEnterprise, environment
...
class AppEnterprise(App):
    ...
    def _check_login(self):
        ...
        if current_tier == "anonymous" and not environment.REFLEX_BACKEND_ONLY.get():
            raise RuntimeError(
                "`reflex-enterprise` is free to use but you must be logged in. Run `reflex login` or set REFLEX_ACCESS_TOKEN with your token."
            )
