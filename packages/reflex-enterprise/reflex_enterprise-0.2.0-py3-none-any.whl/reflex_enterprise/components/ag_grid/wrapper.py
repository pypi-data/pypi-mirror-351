...
def get_default_column_def(
    ...
    **cdef_kwargs,
    ...
    if rx.utils.types.is_optional(ftype):  # pyright: ignore [reportAttributeAccessIssue]
        ftype = rx.utils.types.args(ftype)[0]  # pyright: ignore [reportAttributeAccessIssue]
    ...
    if value_setter is not None:
        cdef.value_setter = rx.EventChain(
            ...
            invocation=rx.vars.function.ArgsFunctionOperation.create(
                args_names=["events"],
                return_expr=rx.Var(
                    "{queueEvents(events, socket); processEvent(socket)}"
                ),
            ),
        )
    return cdef
...
class AbstractWrapper(rx.ComponentState, Generic[DATASOURCE_PARAMS]):
    ...
    __data_route__: str = "/abstract-wrapper-data"
    __get_data_kwargs__: dict[str, Any] = {"state": lambda self: self.get_full_name()}
    ...
    def _add_data_route(cls):
        ...
        async def get_data(
            ...
            try:
                ...
                return []
            params = cls.__data_source_params_class__.from_request(request.query_params)  # pyright: ignore [reportArgumentType]
    ...
    def _set_datasource(self) -> EventSpec:
...
class ModelWrapperBase(
        ...
        with rx.session() as session:
            for item in session.exec(
                self._model_class.select().where(
                    self._model_class.id.in_([item.id for item in self._selected_items])  # pyright: ignore [reportAttributeAccessIssue, reportOptionalMemberAccess]
                )
            ).all():
                session.delete(item)
            session.commit()
    ...
    def create(cls, *children, model_class: Type[M], **props) -> rx.Component:
        ...
        if not comp.State:
            ...
            return comp
        comp.State._model_class = model_class  # pyright: ignore [reportAttributeAccessIssue, reportOptionalMemberAccess]
        frag = rx.fragment(
...
class ModelWrapper(ModelWrapperBase[DatasourceParams, M], Generic[M]):
    ...
    async def _get_data(
        ...
        if not await self._is_authorized(ModelWrapperActionType.SELECT, None):
            return []
        with rx.session() as session:
            return session.exec(  # pyright: ignore [reportReturnType]
                apply_sort_model(
                    ...
                    sort_model=params.sortModel or [],
                )
    ...
    def _default_grid_props(cls) -> dict[str, Any]:
        return {
            **super()._default_grid_props(),
            ...
            "get_row_id": FunctionStringVar("(params) => `${params.data.id}`"),
            "enterprise_modules": {
