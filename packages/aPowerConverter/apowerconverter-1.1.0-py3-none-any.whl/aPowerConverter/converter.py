...
def convert_adoc_to_docx(input_file, output_file=None, image_dir=None, dry_run=False):
    """
    Convert an AsciiDoc file to DOCX format.
    ...
    try:
        ...
        # Add resource path if image directory is specified
        if image_dir:
            pandoc_cmd.insert(-2, '--resource-path')
            pandoc_cmd.insert(-2, image_dir)
