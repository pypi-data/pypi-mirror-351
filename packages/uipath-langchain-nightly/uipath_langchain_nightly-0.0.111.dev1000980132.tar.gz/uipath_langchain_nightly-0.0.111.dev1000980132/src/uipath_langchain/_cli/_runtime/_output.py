            ...
            if hasattr(task, "interrupts") and task.interrupts:
                for interrupt in task.interrupts:
                    if isinstance(interrupt, Interrupt):
                        ...
                        instance._resume_trigger = (
                            await instance._hitl_processor.create_resume_trigger()
                        )
                        return instance
        ...
        try:
            ...
            async with (
                ...
                if default_escalation.enabled and isinstance(
                    self._hitl_processor.value, str
                ):
                    action = await default_escalation.create(self._hitl_processor.value)
                    if action:
                    ...
                    try:
                        self._resume_trigger = await self._hitl_processor.create_resume_trigger()
                        # if API trigger, override item_key and payload
                        if self._resume_trigger.api_resume:
                            self._resume_trigger.item_key = self._resume_trigger.api_resume.inbox_id
                            self._resume_trigger.payload = self._resume_trigger.api_resume.request
                    except Exception as e:
                        raise LangGraphRuntimeError(
                ...
                try:
                    logger.debug(f"ResumeTrigger: {self.resume_trigger.trigger_type} {self.resume_trigger.item_key}")
                    await cur.execute(
                        f"INSERT INTO {self.context.resume_triggers_table} (type, key, payload, folder_path, folder_key) VALUES (?, ?, ?, ?, ?)",
                        (
                            self.resume_trigger.trigger_type,
                            self.resume_trigger.item_key,
