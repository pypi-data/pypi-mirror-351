...
class LangGraphOutputProcessor:
    ...
    async def _save_resume_trigger(self) -> None:
        ...
        """
        if not self._resume_trigger or not self.context.memory:
            return
        ...
        try:
            ...
            async with (
                ...
                try:
                    logger.debug(f"ResumeTrigger: {self._resume_trigger.trigger_type} {self._resume_trigger.item_key}")
                    await cur.execute(
                        f"INSERT INTO {self.context.resume_triggers_table} (type, key, payload, folder_path, folder_key) VALUES (?, ?, ?, ?, ?)",
                        (
                            self._resume_trigger.trigger_type,
                            trigger_key,
                            self._resume_trigger.payload,
