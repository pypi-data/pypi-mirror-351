...
class LCVisWidget(anywidget.AnyWidget):
    ...
    buffered_workspace: dict | None = None
    ...
    def receive_widget_message(self, widget: Any, content: str, buffers: list[bytes]) -> None:
        if content == "screenshot taken":
            ...
            # Send the buffered workspace if we have it
            if self.buffered_workspace:
                self.send(
                    {
                        "cmd": self.buffered_workspace["cmd"],
                        "workspaceState": self.buffered_workspace["workspaceState"],
                        "isComparator": self.buffered_workspace["isComparator"],
                    },
                    self.buffered_workspace["buffers"],
                )
                self.buffered_workspace = None
