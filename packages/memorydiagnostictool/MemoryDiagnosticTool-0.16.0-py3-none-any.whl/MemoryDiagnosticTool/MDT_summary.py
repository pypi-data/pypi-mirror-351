...
from MDT.rrw_error_count_parser import rrw_error_count
from MDT.esid_complete_parser import check_esid_complete
from MDT.memtest_parser import check_memtest
from MDT.Trained_CSR_CSV import trained_csr_csv
from MDT.Trained_LOG_CSV import log_to_csv
from MDT.MR_readout_v1 import mr_readout
from MDT.dca_dcs_eye import scan_dcs_dca
from MDT.rd_wr_eye import Rd_Wr_Eye
from MDT.rd_wr_eye import Rd_Eye_Scan
from MDT.trained_phycsr_subset import phycsr_subset
from MDT.postcode import get_postcodefile
from MDT.MDT_html_template import parse_to_html
        ...
        if os.path.isdir(file_path):
            ...
            if decoded_logs:
                ...
                if not os.path.exists(mdteye):
                    os.mkdir(mdteye)
                try:
                    ordered_result, missing_print, analysis_result= log_to_csv(decoded_logs, jmp_logcsv, analysis_logcsv)
                except:
                    print("[LOG]: Log to csv fail")
                try:
                    scan_dcs_dca(decoded_logs, mdteye, dcsdca_stat_csv)
                except:
                    print("DCS DCA eye parsing error")
                try:
                    rd_wr = Rd_Wr_Eye(decoded_logs, mdteye)
                    rd_wr.main()
                except:
