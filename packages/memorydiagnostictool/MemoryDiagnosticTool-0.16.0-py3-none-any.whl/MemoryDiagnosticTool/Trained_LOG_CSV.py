...
def process_qcs_eye(prev3, prev2, prev1, curr, qcs_start):
    #pattern definition
    dev_pattern = re.compile(
        r"CHANNEL:\s*(\d+),\s*PHY:\s*(\d+),\s*PHYINIT:\s*<<---\s*Rank:\s*(\d+),\s*Ch:\s*(\d+)\s*--->>"
    )
    ...
    if match:
        ...
        if (ch, sub) in qcs_start:
            if qcs_start[ch, sub]== True:
                ...
                if match:
                    ...
                    if match:
                        ...
                        if eyepts=='Upper':
                            qcs_eye[int(ch), int(sub), int(r)]=[[i+int(delayoffset), val] for i, val in enumerate(data2) if val != 0]
                        else:
                            qcs_eye[int(ch), int(sub), int(r)]=[[i+int(delayoffset), val] for i, val in enumerate(data2) if val != 255]
                        match = data_pattern.search(curr)
                        if match:
                            eyepts, data= match.groups()
                            data2=[int(c) for c in data.split()]
                            if eyepts=='Upper':
                                qcs_eye[int(ch), int(sub), int(r)].extend([[i+int(delayoffset), val] for i, val in enumerate(data2) if val != 0])
                            else:
                                qcs_eye[int(ch), int(sub), int(r)].extend([[i+int(delayoffset), val] for i, val in enumerate(data2) if val != 255]) 
...
def process_scaneye(curr, start, mode):
    ln_sel_ref = {0 : [3,1,6,7], 1 : [0,2,4,5]}

    result = {}
    if mode=='base':
        #base pattern
        #text="CHANNEL: 6,  PHY: 0,  PHYINIT: BTFW: [DBV_EYES] [RDEYE Eye Scan] Rank = 0, Byte = 0, Lower Nibble, Lane = 0, Pi code start = 88"
        pattern=re.compile(
            r"CHANNEL:\s*(\d+),\s*PHY:\s*(\d+),\s*PHYINIT:\s*.*?Rank\s*=\s*(\d+),\s*Byte\s*=\s*(\d+),\s*(.*?)\s*Nibble,\s*Lane\s*=\s*(\d+),\s*Pi\s*code\s*start\s*=\s*(\d+)"
        )
        match = pattern.search(curr)
        if match:
            ch, sub, rank, db, nb, lane, base = match.groups()
            if nb=='Lower':
                pin = int(db)*8 + int(lane) 
            else:
                pin = int(db)*8 + 4 + int(lane)
            result[int(ch), int(sub), int(rank), pin] = int(base)   
        ...
        pattern=re.compile(
            r"CHANNEL:\s*(\d+),\s*PHY:\s*(\d+),\s*PHYINIT:\s*.*?Rank\s*=\s*(\d+),\s*Byte\s*=\s*(\d+),\s*lane_sel\s*=\s*(\d+),\s*Lane\s*=\s*(\d+),\s*Vref*.*?\s*=\s*(\d+)"
        )
        match = pattern.search(curr)
        if match:
            ch, sub, rank, db, lane_sel, lane, vrefbase = match.groups()
...
def process_read_vref_r1(prev, curr):
    header_pattern=re.compile(
        r"CHANNEL:\s*(\d+),\s*PHY:\s*(\d+),\s*PHYINIT:\s*BTFW:\s*\[DBV_INFO\]\s*\[RD TRAIN\]\s*<<\s*afe_IdacCtrl\s*>>"
    )
    subheader_pattern=re.compile(
        r"CHANNEL:\s*(\d+),\s*PHY:\s*(\d+),\s*PHYINIT:\s*BTFW:\s*\[DBV_INFO\]\s*\[RD TRAIN\]"
    )
...
def detect_decoded_log(filename, filename2=""):
    ...
    for idx, line in enumerate(combine_text):
        ...
        if qcs_start:
            ...
            qcs_eye_start[qcs_key] = qcs_start
        # Get QCS eye value
        temp_qcs= process_qcs_eye(previous_line[2], previous_line[1], previous_line[0], line, qcs_eye_start)
        if temp_qcs:
            for key in temp_qcs:
        ...
        if match:
            delay_base = match
        #get x-axis data
        match = process_scaneye(line, read_scan_start, 'delay')
        if match:
            _delay = match
        #get y-axis data
        match = process_scaneye(line, read_scan_start, 'vref')
        if match:
            _vref = match
            for key in _vref:
                new_key = (socket, iod, key[0], key[1], key[2], key[3])
                if new_key in read_eye_scan:
                    read_eye_scan[socket, iod, key[0], key[1], key[2], key[3]].append([_delay[key[0],key[1]], _vref[key]])
                else:
                    read_eye_scan[socket, iod, key[0], key[1], key[2], key[3]]=[[_delay[key[0],key[1]], _vref[key]]]
    ...
    try:
        for key, value in wlcoarse_value.items():
            ...
            if (key in wlfine_value) and (key in wlmr3_value):
                wl_result[key] = (value * 64) + wlfine_value[key] + (wlmr3_value[key] * 128)
            else:
...
def process_decodedlog(
    ...
    try:
        ...
        bn = file.split('.')[0]
        cs_html = os.path.join(mdteye_path, f'{bn}_cs.html')
        ...
        if qcs_eye:
            html_eye_plot(cs_html, qcs_eye, 'qcs', qcs_delay, qcs_vref)
        if qca_eye:
            html_eye_plot(qca_html, qca_eye, 'qca', qca_delay, qca_vref)            
        if read_eye_scan:
            html_eye_plot(readscan_html, read_eye_scan, 'readscan')
        if read_eye_even:
        ...
        if write_eye:
            ...
            for key, value in write_coarse_delay.items():
                soc, iod, ch, sub, r, dq = key
                if key in write_fine_delay:
                    write_eye_center[key] = (value* 64) + write_fine_delay[key]
    ...
    df_readvref["Param"] = "Read_Vref(Rdac)"
