...
from aim._core.error_handling import handle_exception

from .message_utils import raise_exception, pack_args, unpack_stream, unpack_args, encode_tree, decode_tree
...
class Client:
    ...
    def connect(self):
        ...
        if response.status_code == 200:
            response_json = response.json()
            self.user_id = response_json.get('user_id', None)
            return response
    ...
    def get_resource_handler(self, resource, resource_type, handler='', exclusive=False, args=()):
    ...
    def release_resource(self, queue_id, resource_handler):
        ...
        return response.json().get('is_last_ref', False)
