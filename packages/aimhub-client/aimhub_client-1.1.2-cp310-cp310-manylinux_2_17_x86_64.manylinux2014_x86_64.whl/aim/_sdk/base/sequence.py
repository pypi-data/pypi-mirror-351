...
from aim._sdk.query import SequenceQueryProxy, ContainerQueryProxy, RestrictedPythonQuery
...
if TYPE_CHECKING:
    ...
    from aim._sdk.base.collections import SequenceCollection
...
class _SequenceInfo:
    ...
    def preload(self):
        ...
        try:
            info = self._meta_tree[KeyNames.INFO_PREFIX]
            self.version = info['version']
            self.first_step = info['first_step']
            self.last_step = info['last_step']
            self.min_value = info.get('min_value', float('inf'))
            self.max_value = info.get('max_value', float('-inf'))
            self.max_record_len = info['max_record_len']
...
class Sequence(Generic[ItemType], ABCSequence):
    ...
    def __init__(self, container: 'Container', *, name: str, context: _ContextInfo):
        ...
        """
        self.storage_engine: StorageEngine = container.storage_engine
    ...
    def __storage_init__(self):
        ...
        self._type_tree = self._container_tree.subtree((KeyNames.SEQUENCE_TYPES_MAP, self._ctx_idx, self._name))
    ...
    def _init_mode(self):
        try:
            self._sequence_ref = self.storage_engine.resource_counter(
                exclusive=True, hash=self._container_hash, name=self._name, context=self._ctx_idx)
            self.readonly = False
        except Exception:
            logger.warning(f'Sequence {self} is in use. Opening in readonly mode.')
            self.readonly = True
    ...
    @property
    def max_record_len(self) -> int:
        self._info.preload()
        return self._info.max_record_len
    ...
    def track(self, value: Any, *, step: Optional[int] = None, **axis):
        ...
        Args:
            ...
            step (Optional[int], optional): The step at which the value should be tracked. If not provided,
        ...
        """
        if self.readonly:
            raise TypeError(f'Sequence {self} is in readonly mode. Tracking is not allowed.')
        value_type = type_utils.get_object_typename(value)
        value_len = type_utils.get_object_length(value)
        ...
        with self.storage_engine.write_batch(self._container_hash):
            if self._info.empty:
                ...
                self._tree[KeyNames.INFO_PREFIX, 'max_record_len'] = self._info.max_record_len = value_len
                ...
                self._tree[KeyNames.INFO_PREFIX, KeyNames.SEQUENCE_TYPE] = self._info.stype = sequence_type

                self._type_tree[KeyNames.VALUE_TYPE] = self._info.dtype = value_type
                self._type_tree[KeyNames.SEQUENCE_TYPE] = sequence_type
                ...
                for typename in sequence_type.split('->'):
                    ...
                    self._container_tree[KeyNames.SEQUENCE_TYPES, typename, self._ctx_idx, self.name] = 1

                self._tree['first_value'] = value
                self._tree['last_value'] = value
                self._tree['axis_last_values'] = axis
                self._info.axis_names = axis_names
                # min/max tracking is supported for metrics only
                if 'metric' in sequence_type.lower():
                    self._tree['min_value'] = self._info.min_value = value
                    self._tree['max_value'] = self._info.max_value = value
                self._info.empty = False

            # TODO: add better handling for metrics min/max tracking
            if 'metric' in self._info.stype.lower():
                if value > self._info.max_value:
                    self._tree[KeyNames.INFO_PREFIX, 'max_value'] = self._info.max_value = value
                if value < self._info.min_value:
                    self._tree[KeyNames.INFO_PREFIX, 'min_value'] = self._info.min_value = value
            ...
            if step < self._info.first_step:
                self._tree[KeyNames.INFO_PREFIX, 'first_step'] = self._info.first_step = step
                self._tree['first_value'] = value
            if value_len > self._info.max_record_len:
                self._tree[KeyNames.INFO_PREFIX, 'max_record_len'] = self._info.max_record_len = value_len
            if not type_utils.is_subtype(value_type, self._info.dtype):
                dtype = type_utils.get_common_typename((value_type, self._info.dtype))
                self._tree[KeyNames.INFO_PREFIX, KeyNames.VALUE_TYPE] = \
                    self._type_tree[KeyNames.VALUE_TYPE] = self._info.dtype = dtype
    ...
    def __iter__(self) -> Iterator[Tuple[int, Tuple[Any, ...]]]:
        ...
        """
        axis = self.AXIS_NAMES or self.axis_names
        data_iterator = zip(self.items(), zip(map(self.axis, axis)))
    ...
    def _check(self, query, query_cache, *, aliases=()) -> bool:
        ...
        proxy = SequenceQueryProxy(self.name, self._context_from_idx, self._ctx_idx, self._container_tree, query_cache[hash_])
        ...
        if self._container is not None:
            ...
            c_proxy = ContainerQueryProxy(type(self._container), hash_, self._container_tree, self._meta_tree, query_cache[hash_])
        else:
            from aim._sdk.base.container import Container
            container_alias_names = Container.default_aliases
            c_proxy = ContainerQueryProxy(Container, hash_, self._container_tree, self._meta_tree, query_cache[hash_])
