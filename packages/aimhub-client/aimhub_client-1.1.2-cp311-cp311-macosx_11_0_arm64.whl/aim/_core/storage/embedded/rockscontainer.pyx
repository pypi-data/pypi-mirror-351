...
from aim._core.storage.locking import NoopLock
from aim._core.storage.types import BLOB
from aim._core.storage.embedded.container import Container, ContainerKey, ContainerValue, ContainerItemsIterator
...
class RocksContainer(Container):
    ...
    def db(self) -> aimrocks.DB:
        ...
        if not self.read_only:
            ...
            self._lock = NoopLock(self._lock_path, timeout)
            self._lock.acquire()
            secondary_path = None
        else:
    ...
    def view(
        ...
        """
        return PrefixContainer(prefix=prefix, container=self)
...
class RocksContainerItemsIterator(ContainerItemsIterator):
    ...
    def seek(self, position: ContainerKey):
        self.it.seek(position)
