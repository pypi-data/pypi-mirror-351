...
MIMETYPES_TGZ = [("application/x-tar", "gzip")]
MIMETYPES_COMPRESSED = MIMETYPES_ZIP + MIMETYPES_TGZ
    ...
    if need_uncompress:
        ...
        with conf.get_temporary_directory() as tmp_uncompress_dir:
            if "file" in protocols:
                ...
                elif mtype in MIMETYPES_TGZ:
                    file = tarfile.open(path)
                    try:
                        # python >= 3.12
                        file.extractall(tmp_uncompress_dir, filter="data")
                    except TypeError:
                        # python < 3.12
                        raise NotImplementedError("Please pass to python >= 3.12")
                    file.close()
                else:
