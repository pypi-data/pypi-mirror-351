...
class FileBotAPI(commands.Bot):
    def __init__(self, *, intents: discord.Intents):
        ...
        self.download_queue = asyncio.Queue()  # Keep download queue
        self.download_semaphore = asyncio.Semaphore(3)
        self.delete_task_queue = asyncio.Queue()
        self.deletion_task = None
        self.user_uploading: Dict[int, List[str]] = {}  # {user_id: [filename1, filename2, ...]}
        self.user_downloading: Dict[int, str] = {}  # {user_id: filename}
        self.http_session = None
        self.log_prefix = "[FileBotAPI]"
        self.total_parts_cache: Dict[str, int] = {}  # Cache total parts for safe_send #ADDED THIS LINE
    ...
    async def download_filea(self, interaction: discord.Interaction, filename: str, database_file: str, download_folder: str):
        """Handles the download command using interaction.followup."""
        user_mention = interaction.user.mention
        if not filename:
            await interaction.followup.send(f"{user_mention}, please specify the filename to download.")
            return
        if not database_file:
            await interaction.followup.send(f"{user_mention}, please specify the database file to search in.")
            return
        if not download_folder:
            await interaction.followup.send(f"{user_mention}, please specify the download folder.")
            return
        ...
        if interaction.user.id in self.user_downloading and self.user_downloading[interaction.user.id] == filename:
            await interaction.followup.send(f"{user_mention}, you are already downloading '{filename}'. Please wait.")
            return

        if self.download_semaphore.locked():
            await interaction.followup.send(f"{user_mention}, server is busy with 3 download requests going at the same time(maximum)")
            self.log(f"Download limit reached. User {interaction.user.id} waiting.")
            return

        await interaction.followup.send(f"{user_mention}, download for '{filename}' started. I'll notify you when done.")
        ...
        try:
            async with self.download_semaphore:
                ...
                await self.download(self, await self.get_context(interaction), filename, database_file, download_folder)
                if os.path.exists(os.path.join(download_folder,filename)):
                    await interaction.followup.send(f"{user_mention}, '{filename}' found in '{download_folder}'.")
                self.log(f"Download request of '{filename}' by user {interaction.user.id} completed.")
                await interaction.followup.send(f"Download request of '{filename}' by {user_mention} completed.")
        except Exception as e:
            self.log(f"Error during download of '{filename}' by user {interaction.user.id}: {e}")
            self.log(traceback.format_exc())
            await interaction.followup.send(f"{user_mention}, error during download: {e}. See logs for details.")
        finally:
            if interaction.user.id in self.user_downloading and self.user_downloading[interaction.user.id] == filename:
    ...
    async def list_files_commanda(self, interaction: discord.Interaction, database_file: str):
        ...
        if not database_file:
            ...
            return
        await interaction.followup.send(f"{user_mention}, fetching the list of uploaded files. Please wait...")
        await self.list_files(await self.get_context(interaction), database_file) # Pass interaction

    async def delete_filea(self, interaction: discord.Interaction, filename: str, database_file: str):
        user_mention = interaction.user.mention
        if not filename:
            await interaction.followup.send(f"{user_mention}, please specify the filename to delete.")
            return
        if not database_file:
            await interaction.followup.send(f"{user_mention}, please specify the database file.")
            return
        await interaction.followup.send(f"{user_mention}, preparing to delete '{filename}'. This might take a moment.")
        await self.delete(self, await self.get_context(interaction), filename, database_file) # Pass interaction

    async def _start_upload_process(self, interaction: discord.Interaction, local_file_path: str, database_file: str,
                                    channel_id: int):
        """Starts the upload process after the initial interaction has been deferred."""
        user_mention = interaction.user.mention
        if not local_file_path:
            await interaction.followup.send(f"{user_mention}, please specify the local file path to upload.")
            return
        if not database_file:
            await interaction.followup.send(f"{user_mention}, please specify the database file to use.")
            return
        ...
        # Check if user is already uploading this file
        ...
        if self.upload_semaphore.locked():
            ...
            return
        self.log(f"before modification user_uploading is {self.user_uploading}")
        # Add filename to user's uploading list BEFORE the potentially long operation
        if interaction.user.id not in self.user_uploading:
            self.user_uploading[interaction.user.id] = [filename]  # Start a new list
        else:
            self.user_uploading[interaction.user.id].append(filename)  # Add to existing list
        self.log(f"after modification user_uploading is {self.user_uploading}")
        await interaction.followup.send(f"{user_mention}, upload for '{filename}' started. Please be patient.")
        try:
            async with self.upload_semaphore:
                self.log(
                    f"Starting upload processing for '{filename}' from '{local_file_path}' by user {interaction.user.id}. Channel ID: {channel_id}")
                await self._handle_upload_task(await self.get_context(interaction), local_file_path, database_file,
                                               channel_id)
                await interaction.followup.send(
                    f"{user_mention}, upload of '{filename}' to '{database_file}' complete.")
            ...
            self.log(traceback.format_exc())
            await interaction.followup.send(f"{user_mention}, error during upload: {e}. See logs for details.")
        finally:
            # Remove filename from user's uploading list
            if interaction.user.id in self.user_uploading:
                ...
                if not self.user_uploading[interaction.user.id]:
                    del self.user_uploading[interaction.user.id]  # Clean up empty list
    async def _handle_upload_task(self, ctx: commands.Context, local_file_path: str, DB_FILE: str, channel_id: int):
        """Handles the actual file upload process with detailed logging and error reporting."""
        filename = os.path.basename(local_file_path)
        user_mention = ctx.author.mention
        try:
            await self.upload_local_file(ctx, local_file_path, DB_FILE, channel_id)
        except FileNotFoundError:
            await ctx.send(
                f"{user_mention}, the file '{filename}' was not found during the upload task. Please ensure the file exists at the specified path.")
            ...
            self.log(traceback.format_exc())
            await ctx.send(
                f"{user_mention}, an error occurred during the upload of '{filename}': {e}. Please check the logs for more details.")
    ...
    def remove_rows_by_value(self, database_file, target_value):
        ...
        for r in range(total_rows):
            try:
                value = PDB.table_data.read(database_file, [0, 1, r])  # Read from column 1
                if value == target_value:
                    rows_to_delete.append(r)
            except IndexError:
                print(f"IndexError at row {r}.  This might indicate an issue with the database structure.")
                pass
        rows_to_delete = sorted(list(set(rows_to_delete)), reverse=True)  # Remove duplicates and sort
    ...
    async def safe_send(self, ctx: commands.Context, file_path: str, filename_to_send: str, offset: int, part_size: int, attempt: int, retries: int, part_number: int, *args, **kwargs) -> Optional[discord.Message]:
        print('safe_send hit')
        send_retries = 3  # Retries within safe_send
        send_delay = 2
        for send_attempt in range(send_retries):
            ...
            try:
                ...
                self.log(f"Send attempt {send_attempt + 1}/{send_retries} (overall attempt {attempt + 1}/{retries}), part {part_number}, file size: {file_size} bytes")
                if len(part_data) > 10:
                    self.log(
                    ...
                    self.log(f"Send attempt {send_attempt + 1}/{send_retries} (overall attempt {attempt + 1}/{retries}), part {part_number}, data: {part_data!r}")

                if not part_data and part_number < self.total_parts_cache.get(file_path, float('inf')):
                    self.log(f"Send attempt {send_attempt + 1}/{send_retries} (overall attempt {attempt + 1}/{retries}), part {part_number}, WARNING: Empty read, retrying send.")
                    await asyncio.sleep(0.3)
                    continue

                discord_file = discord.File(io.BytesIO(part_data), filename=filename_to_send)
                message = await ctx.send(*args, file=discord_file, **kwargs)
                self.log(f"Send attempt {send_attempt + 1}/{send_retries} (overall attempt {attempt + 1}/{retries}), part {part_number} successful, Message ID: {message.id if message else None}")
                return message
            except (aiohttp.ClientError, asyncio.TimeoutError, discord.errors.DiscordException) as e:
                self.log(f"Send attempt {send_attempt + 1}/{send_retries} (overall attempt {attempt + 1}/{retries}), part {part_number} failed: {e!r}")
                ...
                self.log(f"Unexpected error during send attempt {send_attempt + 1}/{send_retries} (overall attempt {attempt + 1}/{retries}), part {part_number}: {e!r}")
                return None
        return None

    async def send_with_retries(self, ctx: commands.Context, file_path: str, part_number: int, total_parts: int,
                                 part_size: int, filename_to_send: str) -> Optional[discord.Message]:
        retries = 5
        delay = 4
        offset = (part_number - 1) * part_size
        self.total_parts_cache[file_path] = total_parts # Cache total parts for safe_send

        for attempt in range(retries):
            result = await self.safe_send(ctx, file_path, filename_to_send, offset, part_size, attempt, retries, part_number)
            if result:
                ...
                if attempt < retries - 1:
                    ...
                    self.log(f"[ATTEMPT {attempt + 1}/{retries}] Max retries reached for part {part_number}, giving up.")
                    return None
        return None
    ...
    async def upload_local_file(self, ctx: commands.Context, local_file_path: str, DB_FILE: str, channel_id: int):
        filename = os.path.basename(local_file_path)
        user_mention = ctx.author.mention
        ...
        if await self.is_filename_in_db(DATABASE_FILE, filename):
            await ctx.send(f"{user_mention}, the file '{filename}' is already present in the database '{DB_FILE}'.")
            self.log(f">>> [upload_local_file] Aborted: '{filename}' already exists in '{DATABASE_FILE}'.")
            return

        # Ensure it's a list and append
        if ctx.author.id not in self.user_uploading:
            self.user_uploading[ctx.author.id] = [filename]
        elif filename not in self.user_uploading[ctx.author.id]:
            self.user_uploading[ctx.author.id].append(filename)

        self.log(f"Initiating upload of '{filename}' (size: {file_size} bytes).")
        await ctx.send(f"{user_mention}, preparing to upload '{filename}' (size: {file_size / (1024 * 1024):.2f} MB).")

        try:
            part_size = 10 * 1024 * 1024
            num_parts = (file_size + part_size - 1) // part_size
            await ctx.send(f"{user_mention}, this file will be uploaded in {num_parts} parts.")
            self.log(f"File '{filename}' will be uploaded in {num_parts} parts.")

            for i in range(1, num_parts + 1):  # Part numbers are 1-based
                part_filename = f'{filename}.part{i}'
                await ctx.send(f"{user_mention}, uploading part {i}/{num_parts}...")
                message = await self.send_with_retries(ctx, local_file_path, i, num_parts, part_size, part_filename)
                if message:
                    ...
                    await ctx.send(
                        f"{user_mention}, failed to send part {i}/{num_parts} of '{filename}'. Upload aborted. Cleaning up...")
                    self.log(f"Error: Failed to send part {i} of '{filename}'. Upload aborted.")
                    self.remove_rows_by_value(DATABASE_FILE, filename)
                    return
            await ctx.send(
                f"{user_mention}, all {num_parts} parts of '{filename}' uploaded successfully to '{DB_FILE}'.")
            self.log(f"All parts of '{filename}' uploaded and metadata stored successfully.")

        except FileNotFoundError:
            await ctx.send(f"{user_mention}, the file '{filename}' was not found during upload.")
            self.log(f"Error: File not found at '{local_file_path}' during upload.")
        except Exception as e:
            self.log(f"Error during upload of '{filename}': {e}")
            self.log(traceback.format_exc())
            await ctx.send(
                f"{user_mention}, an unexpected error occurred during the upload of '{filename}': {e}. Please check the logs for more details. Cleaning up...")
            self.remove_rows_by_value(DATABASE_FILE, filename)
        finally:
            # Remove filename from user's uploading list
            if ctx.author.id in self.user_uploading and filename in self.user_uploading[ctx.author.id]:
                self.user_uploading[ctx.author.id].remove(filename)
                if not self.user_uploading[ctx.author.id]:
                    del self.user_uploading[ctx.author.id]
    async def download(self, bot: "FileBotAPI", ctx: commands.Context, filename: str, DB_FILE: str,
                       DOWNLOAD_FOLDER: str):
        user_mention = ctx.author.mention
        message_retries = 5
        message_delay = 4
        ...
        try:
            ...
            DATABASE_FILE = os.path.abspath(os.path.normpath(DB_FILE))
            if not os.path.exists(DATABASE_FILE):
                await ctx.send(f"{user_mention}, the database file '{DB_FILE}' was not found.")
            ...
            if not os.path.exists(DOWNLOAD_FOLDER):
                try:
                    ...
                    await ctx.send(
                        f"{user_mention}, could not create the download folder '{DOWNLOAD_FOLDER}'.")
            ...
            try:
                ...
                if not parts_to_download or len(parts_to_download) != parts_to_download[0][1]:
                    await ctx.send(
                        f"{user_mention}, file '{filename}' not found or incomplete metadata in the database.")
                ...
                await ctx.send(f"{user_mention}, error finding file parts for '{filename}': {e}.")
            ...
            if not channel_id:
                await ctx.send(f"{user_mention}, could not determine the channel for '{filename}'.")
                self.log(f">>> [DOWNLOAD] ERROR: Could not determine channel ID for '{filename}'.")
                return

            channel = bot.get_channel(channel_id)
            if not channel:
                await ctx.send(f"{user_mention}, channel (ID: {channel_id}) not found for '{filename}'.")
                self.log(f">>> [DOWNLOAD] ERROR: Channel with ID {channel_id} not found.")
                return
            ...
            for part_number, _, message_id, _ in sorted(parts_to_download, key=lambda x: x[0]):
                await ctx.send(
                    f"{user_mention}, downloading part {part_number}/{total_expected_parts} of '{filename}'...")
                ...
                for attempt in range(message_retries):
                    ...
                    try:
                        ...
                        await ctx.send(f"{user_mention}, part {part_number} (ID: {message_id}) not found.")
                        self.log(f">>> [DOWNLOAD] ERROR: Part {part_number} (ID: {message_id}) not found.")
                        return
                    except discord.Forbidden:
                        await ctx.send(f"{user_mention}, no permission to access message (ID: {message_id}).")
                        ...
                        if attempt < message_retries - 1:
                            ...
                            await ctx.send(
                                f"{user_mention}, failed to fetch part {part_number} after {message_retries} retries.")
                            self.log(
                        ...
                        if attempt < message_retries - 1:
                            ...
                            await ctx.send(f"{user_mention}, critical error fetching part {part_number}.")
                            self.log(
                ...
                if message and message.attachments:
                    ...
                    for attempt in range(attachment_retries):
                        ...
                        try:
                            ...
                            if attempt < attachment_retries - 1:
                                ...
                                await ctx.send(
                                    f"{user_mention}, failed to download part {part_number} after {attachment_retries} retries.")
                                self.log(
                            ...
                            if attempt < attachment_retries - 1:
                                ...
                                await ctx.send(f"{user_mention}, network error downloading part {part_number}.")
                                self.log(
                            ...
                            if attempt < attachment_retries - 1:
                                ...
                                await ctx.send(f"{user_mention}, critical error downloading part {part_number}.")
                                self.log(
                    ...
                    if file_data:
                        ...
                        # This should be caught by the retry logic, but as a fallback
                        await ctx.send(f"{user_mention}, failed to retrieve data for part {part_number}.")
                        self.log(
                            f">>> [DOWNLOAD] ERROR: Failed to retrieve data for part {part_number} (ID: {message_id}).")
                        return
                else:
                    await ctx.send(
                        f"{user_mention}, part {part_number} (ID: {message_id}) has no attachment.")
                    self.log(
            ...
            try:
                ...
                await ctx.send(
                    f'{ctx.author.mention}, File "{filename}" downloaded successfully to "{DOWNLOAD_FOLDER}".')
            except OSError as e:
                await ctx.send(
                    f"{user_mention}, error writing the downloaded file '{filename}': {e}.")
                self.log(f">>> [DOWNLOAD] ERROR: OSError writing file '{filename}': {e}")
                self.log(traceback.format_exc())
                return
            except Exception as e:
                await ctx.send(
                    f"{user_mention}, unexpected error during download of '{filename}': {e}.")
            ...
            await ctx.send(
                f"{user_mention}, a general error occurred during download: {e}.")
            self.log(f">>> [DOWNLOAD] ERROR: General exception during download: {e}")
            self.log(traceback.format_exc())
            return

    async def delete(self, bot: "FileBotAPI", ctx: commands.Context, filename: str, DB_FILE: str):
        user_mention = ctx.author.mention
        await ctx.send(f"{user_mention}, Preparing to delete...")
        """Deletes file metadata from the database and attempts to delete the corresponding Discord messages."""
        await ctx.send(f"{ctx.author.mention}, delete request started for '{filename}'.")
        ...
        try:
            ...
            try:
                ...
                await ctx.send(f"{user_mention}, error reading the database to find '{filename}' for deletion: {e}")
                self.log(f">>> [DELETE] ERROR: Error reading database for '{filename}': {e}")
                self.log(traceback.format_exc())
                return

            if not rows_to_delete:
                await ctx.send(f'{user_mention}, file "{filename}" not found in the database.')
            ...
            try:
                ...
                await ctx.send(f"{user_mention}, error updating database after deleting '{filename}': {e}")
                self.log(f">>> [DELETE] ERROR: Error updating database: {e}")
                return
            except Exception as e:
                await ctx.send(f"{user_mention}, an unexpected error occurred while updating the database: {e}")
            ...
            await ctx.send(f"{user_mention}, an error occurred during the deletion process: {e}")
            self.log(f">>> [DELETE] ERROR: Discord API error during deletion: {e}")
            self.log(traceback.format_exc())
        except Exception as e:
            await ctx.send(f"{user_mention}, an unexpected error occurred during the deletion of '{filename}': {e}")
    ...
    async def _delete_messages(self, channel, message_ids):
        ...
        for i in range(0, len(messages_to_delete), BATCH_SIZE):
            ...
            try:
                ...
                if e.status == 429:
                    retry_after = getattr(e, 'retry_after', 5)  # Use getattr with a default value
                    self.log(
                    ...
                    return  # Exit the current batch processing
                else:
                    self.log(
    ...
    async def list_files(self, ctx: commands.Context, DB_FILE):
        user_mention = ctx.author.mention
        if self.upload_semaphore._value < 3:
            await ctx.send(f"{user_mention}, NOT ALLOWED TO LIST FILES WHILE SYSTEM IS UPLOADING FILES.")
            """Lists all unique filenames stored in the database."""
            self.log(f">>> [LIST FILES] , NOT ALLOWED TO LIST FILES WHILE SYSTEM IS UPLOADING FILES.")
            return
        await ctx.send(f"{user_mention}, Fetching file list...")
        """Lists all unique filenames stored in the database."""
        ...
        try:
            ...
            try:
                ...
                if not filenames:
                    await ctx.send(f'{user_mention}, no files have been uploaded yet.')
                    self.log(f">>> [LIST FILES] INFO: No files found in database: '{DATABASE_FILE}'.")
                    return
                await ctx.send(f'{user_mention}, uploaded files:\n{", ".join(sorted(list(filenames)))}')
                self.log(f">>> [LIST FILES] INFO: Listed {len(filenames)} files from database: '{DATABASE_FILE}'.")
            except Exception as e:
                await ctx.send(f"{user_mention}, an error occurred while reading the list of files: {e}")
            ...
            self.log(traceback.format_exc())
            await ctx.send(f"{user_mention}, an unexpected error occurred while listing files: {e}")
def startbot(BOT_TOKEN):
    ...
    async def listfiles_command(interaction: discord.Interaction, database_file: str):
        ...
        await bot.list_files(ctx, database_file)
    ...
    async def delete_command(interaction: discord.Interaction, filename: str, database_file: str):
        await interaction.response.defer(ephemeral=True)
        ctx = await bot.get_context(interaction)
        await bot.delete(bot, ctx, filename,database_file)
    bot.run(BOT_TOKEN)