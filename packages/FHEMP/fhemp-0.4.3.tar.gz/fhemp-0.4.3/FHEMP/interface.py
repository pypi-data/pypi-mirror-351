from .core import (
    generate_k_vector, generate_K_poly, generate_evaluation_key,
...
DEFAULT_DATA_DIR = os.path.join(os.path.expanduser("~"), "FHEMP_data")
...
def encrypt(message, filename, N, p, lam, psi, secret_key_file, vector_file):
    ...
    save_json(ciphertext, os.path.join(DEFAULT_CIPHER_DIR, f"{filename}.json"))
    return ciphertext
...
def operate(ciphertext_file1, operation, ciphertext_file2, p, name_evaluation_key_file, filename):
    ...
    if operation == "add":
        ...
        evaluation_key_file = os.path.join(DEFAULT_KEY_DIR, f"{name_evaluation_key_file}.json")
    ...
    save_json(result, os.path.join(DEFAULT_CIPHER_DIR, f"{filename}.json"))
    return result
