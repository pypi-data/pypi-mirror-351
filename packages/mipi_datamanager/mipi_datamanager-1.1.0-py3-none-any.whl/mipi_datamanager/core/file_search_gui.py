...
from textual.widgets import (
    Button, Digits, Footer, Header, Collapsible, Label, Markdown, DirectoryTree, Placeholder, \
    TabbedContent, TabPane, SelectionList, Input, TextArea, ListView, ListItem, Static, ContentSwitcher
)
...
from mipi_datamanager.core.file_search import File
...
logging.basicConfig(
    filename="fs_log.log",   # log file in your CWD
    filemode="w",                # overwrite on each run
    level=logging.DEBUG,         # capture everything ≥ DEBUG
    format="%(asctime)s %(levelname)-5s %(name)s: %(message)s",
    force=True                   # overwrite any existing config
)
logger = logging.getLogger("fs_log_logger")
...
class MenuButton(Button):
    ...
    def set_toggled(self):
        ...
        logger.info(f"Toggled classes: {self.classes}")
...
class ResultsViewer(HorizontalGroup):
    ...
    def _when_path_list_is_selected(self, event: ListViewPathsFS.Selected) -> None:
        ...
        logger.debug(f"setting file of ext: {lang}")
...
class FileSearchGui(App):
    ...
    def _set_live_path(self):
        ...
        if path:
            self.live_path = path
            logger.info(f"Live Path set: {self.live_path}")
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        """
        logger.info(f"running on_key: {event}")
        key = event.key
    ...
    def _export_results(self, event: Button.Pressed):
        ...
        if self.fs is None:
            return # dont do anything if filesearch has not been set.. # TODO change to files reactive element in Results Viewer
        if self.live_path is None:
            try:
                self._set_live_path()
            except:
                return # if set path doesnt work, end function
        df = self.fs.file_contents_dataframe()
        logger.info(f"attempting to export results to {self.live_path}")
        df.to_csv(self.live_path / f"file_search_results_{str(dt.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))}.csv", index=True)
    ...
    def _handle_menu_toggled(self, message:MenuButton.Toggled):
        ...
        logger.info(f"control: {message.control}")
    ...
    def on_button_pressed(self, event: Button.Pressed):
        ...
        logger.debug(f"exts: {exts}")
        logger.debug(f"ss: {content_includes}")
