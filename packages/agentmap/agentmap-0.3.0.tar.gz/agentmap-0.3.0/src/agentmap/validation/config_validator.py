# src/agentmap/validation/config_validator.py
"""
Configuration validator for AgentMap YAML config files.
"""
import hashlib
import os
import yaml
...
from agentmap.validation.models import ConfigModel
...
class ConfigValidator:
    ...
    def _validate_yaml_structure(self, config_data: Dict[str, Any], result: ValidationResult) -> None:
        """Validate basic YAML structure and format."""
        
        if not isinstance(config_data, dict):
            result.add_error("Config file must contain a YAML dictionary at the root level")
            return
        ...
        result.add_info(f"Found {len(sections)} configuration section(s): {', '.join(sections)}")
        
        # Check for common typos in section names
        expected_sections = {'paths', 'llm', 'memory', 'prompts', 'execution', 'tracing'}
        typo_suggestions = {
            'path': 'paths',
            'llms': 'llm', 
            'memories': 'memory',
    ...
    def _validate_schema(self, config_data: Dict[str, Any], result: ValidationResult) -> None:
    ...
    def _validate_paths(self, config_data: Dict[str, Any], result: ValidationResult) -> None:
        ...
        if csv_path:
            ...
            if not csv_file.exists():
                ...
                result.add_error(f"CSV path is not a file: {csv_path}", field="csv_path")
    ...
    def _validate_llm_configs(self, config_data: Dict[str, Any], result: ValidationResult) -> None:
        """Validate LLM provider configurations."""
        
        llm_config = config_data.get('llm', {})
        if not llm_config:
            result.add_info("No LLM configuration found - will use environment variables and defaults")
            return
        ...
        if configured_providers:
            result.add_info(f"LLM providers configured: {', '.join(configured_providers)}")
        else:
            result.add_info("No LLM providers explicitly configured - will use environment variables")
    
    def _validate_provider_config(self, provider: str, config: Dict[str, Any], result: ValidationResult) -> None:
        ...
        if not api_key and env_var:
            ...
            if not env_value:
                ...
                result.add_info(f"{provider} API key found in environment variable {env_var}")
        elif api_key:
