# src/agentmap/validation/csv_validator.py
"""
CSV validator for AgentMap workflow definitions.
"""
import hashlib
import pandas as pd
...
from agentmap.validation.errors import ValidationResult, ValidationLevel
from agentmap.validation.models import CSVRowModel
...
class CSVValidator:
    """
    Validator for AgentMap CSV workflow definitions.
    
    Validates structure, content, routing logic, and graph consistency.
    """
    ...
    def __init__(self):
        ...
        self.optional_columns = {
            'AgentType', 'Prompt', 'Description', 'Context',
            'Input_Fields', 'Output_Field', 
            'Edge', 'Success_Next', 'Failure_Next'
        }
            ...
            try:
                ...
                for col in df.columns:
                    ...
                    if pd.isna(value):
                        ...
                        row_dict[col] = str(value).strip() if str(value).strip() else None
                ...
                result.add_error(
                    f"Unexpected error validating row: {e}",
                    line_number=line_number
                )
    
    def _build_graphs_for_validation(self, df: pd.DataFrame, result: ValidationResult) -> Dict[str, Dict]:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                graph_name = str(row['GraphName']).strip() if pd.notna(row['GraphName']) else ''
                ...
                graphs[graph_name][node_name] = {
                    'line_number': line_number,
                    'agent_type': str(row.get('AgentType', '')).strip() if pd.notna(row.get('AgentType')) else '',
                    'edge': str(row.get('Edge', '')).strip() if pd.notna(row.get('Edge')) else '',
                    'success_next': str(row.get('Success_Next', '')).strip() if pd.notna(row.get('Success_Next')) else '',
                    'failure_next': str(row.get('Failure_Next', '')).strip() if pd.notna(row.get('Failure_Next')) else '',
                    'prompt': str(row.get('Prompt', '')).strip() if pd.notna(row.get('Prompt')) else ''
                }
                
            except Exception as e:
                result.add_error(f"Error processing row for graph validation: {e}", line_number=line_number)
    ...
    def _validate_graph_consistency(self, graphs: Dict[str, Dict], result: ValidationResult) -> None:
        ...
        for graph_name, nodes in graphs.items():
            ...
            for node_name, node_info in nodes.items():
                ...
                if node_info.get('edge') or node_info.get('success_next') or node_info.get('failure_next'):
    ...
    def _validate_routing_logic(self, df: pd.DataFrame, graphs: Dict[str, Dict], result: ValidationResult) -> None:
    ...
    def _validate_agent_types(self, df: pd.DataFrame, result: ValidationResult) -> None:
        ...
        if unique_agent_types:
            result.add_info(f"Found {len(unique_agent_types)} unique agent types: {', '.join(sorted(unique_agent_types))}")
