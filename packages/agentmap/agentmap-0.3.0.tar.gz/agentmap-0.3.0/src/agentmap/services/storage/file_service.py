...
class FileStorageService(BaseStorageService):
    ...
    def _resolve_file_path(self, collection: str, document_id: Optional[str] = None) -> Path:
        """
    ...
    def _is_text_file(self, file_path: str) -> bool:
        ...
        text_extensions = ['.txt', '.md', '.html', '.htm', '.csv', '.log', '.py', 
                          '.js', '.json', '.yaml', '.yml', '.xml', '.rst']
    ...
    def _is_binary_file(self, file_path: str) -> bool:
        ...
        ext = Path(file_path).suffix.lower()
        binary_extensions = ['.pdf', '.docx', '.doc', '.png', '.jpg', '.jpeg', 
                            '.gif', '.zip', '.tar', '.gz', '.exe', '.dll']
    ...
    def _filter_by_id(self, documents: List[Any], document_id: str) -> List[Any]:
        ...
        filtered = [doc for doc in documents if hasattr(doc, 'metadata') and 
                   doc.metadata.get('id') == document_id]
        ...
        try:
            ...
            try:
                ...
                if query:
                    ...
                    filter_query = {k: v for k, v in query.items() 
    ...
    def write(
        ...
        **kwargs
        ...
        try:
            ...
            if binary_mode or isinstance(content, bytes) or self._is_binary_file(str(file_path)):
                if not self.client["allow_binary"]:
                    return self._create_error_result(
                        "write",
    ...
    def get_file_metadata(self, collection: str, document_id: str, **kwargs) -> Dict[str, Any]:
        """
        ...
        **kwargs
        ...
        try:
            ...
            self._handle_error("copy", e, 
                             ...
                             target_collection=target_collection, target_id=target_id)
