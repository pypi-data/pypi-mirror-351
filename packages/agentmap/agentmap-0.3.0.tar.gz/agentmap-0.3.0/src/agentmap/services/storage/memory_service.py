...
class MemoryStorageService(BaseStorageService):
    ...
    def _initialize_client(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "max_documents_per_collection": self._config.get_option("max_documents_per_collection", 10000),
            "max_document_size": self._config.get_option("max_document_size", 1048576),  # 1MB
            ...
            "case_sensitive_collections": self._config.get_option("case_sensitive_collections", True),
            "persistence_file": self._config.get_option("persistence_file"),  # Optional file for persistence
        }
    ...
    def _perform_health_check(self) -> bool:
        ...
        try:
            ...
            if len(self._storage) > max_collections:
                self._logger.warning(f"Collection count ({len(self._storage)}) exceeds limit ({max_collections})")
                return False
            
            # Check document count limits per collection
            max_docs_per_collection = self.client.get("max_documents_per_collection", 10000)
            for collection_name, collection_data in self._storage.items():
                if len(collection_data) > max_docs_per_collection:
                    self._logger.warning(f"Collection '{collection_name}' document count ({len(collection_data)}) exceeds limit ({max_docs_per_collection})")
                    return False
    ...
    def _apply_query_filter(self, data: Dict[str, Any], query: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _update_metadata(self, collection: str, document_id: str, operation: str) -> None:
        """
            ...
            if mode == WriteMode.WRITE:
                ...
                if path:
                    ...
                    if document_id in collection_data:
                        ...
                        new_doc = {}
                        collection_data[document_id] = self._update_path(new_doc, path, data_to_store)
                        created_new = True
                else:
                    # Direct write
                ...
                if document_id not in collection_data:
                    ...
                    if isinstance(current_doc, list) and isinstance(data_to_store, list):
                        # Append lists
                    ...
                    elif isinstance(current_doc, dict) and isinstance(data_to_store, dict):
                        # Merge dictionaries
                        current_doc.update(data_to_store)
                    else:
        ...
        try:
            ...
            if query:
                ...
                for doc_id in deleted_ids:
                    ...
                    if collection in self._metadata and doc_id in self._metadata[collection]:
    ...
    def save_persistence(self) -> StorageResult:
        ...
        try:
            ...
            if not persistence_file:
                return self._create_error_result(
                    "save_persistence",
                    "No persistence file configured"
                )
