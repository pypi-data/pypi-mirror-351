...
class StorageServiceManager:
    ...
    def register_provider(
        self, 
        provider_name: str, 
        service_class: Type[BaseStorageService]
    ) -> None:
        """
    ...
    def _create_service_from_factory(self, provider_name: str) -> StorageService:
        ...
        try:
            ...
            self._logger.error(f"Failed to create storage service {provider_name} from factory: {e}")
            raise StorageServiceConfigurationError(
    ...
    def is_provider_available(self, provider_name: str) -> bool:
        ...
        """
        return (
            provider_name in self._service_classes or 
            provider_name in self._factories
        )
    ...
    def clear_cache(self, provider_name: Optional[str] = None) -> None:
        ...
        if provider_name:
            if provider_name in self._services:
                ...
                self._logger.info(f"Cleared cache for storage provider: {provider_name}")
        else:
