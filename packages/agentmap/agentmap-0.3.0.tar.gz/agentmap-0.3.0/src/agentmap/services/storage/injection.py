...
def inject_storage_services(
    ...
    try:
        ...
        if (not injected_services and 
            ...
            logger.debug(f"[ServiceInjection] Injected default storage service into {agent_name}")
            
        if injected_services:
            services_list = ', '.join(injected_services)
            logger.debug(f"[ServiceInjection] Successfully injected services into {agent_name}: {services_list}")
            
    except Exception as e:
        error_msg = f"Failed to inject storage services into agent '{agent_name}': {str(e)}"
