...
class NodeRegistryService:
    ...
    def build_registry(
        ...
        for node_name, node in graph_def.items():
            try:
                ...
                if not metadata.description and metadata.prompt:
                    ...
                    max_desc_len = self.configuration.get_value("node_registry.max_description_length", 100)
                ...
                self.logger.trace(f"[NodeRegistry] Added node '{node_name}' to registry")
                
            except Exception as e:
                self.logger.error(f"[NodeRegistry] Failed to process node '{node_name}': {e}")
        ...
        self.logger.info(f"[NodeRegistry] Built registry with {len(registry)} nodes for: {cache_key}")
        return registry
    ...
    def prepare_for_assembly(
        ...
        """
        self.logger.debug(f"[NodeRegistry] Preparing registry for assembly: {graph_name}")
    ...
    def verify_pre_compilation_injection(
        self, 
        assembler: Any  # GraphAssembler instance
    ) -> Dict[str, bool]:
        """
        ...
        if stats["orchestrators_found"] > 0:
            success_rate = stats["orchestrators_injected"] / stats["orchestrators_found"]
        
        result = {
            "has_orchestrators": stats["orchestrators_found"] > 0,
            "all_injected": stats["injection_failures"] == 0 and stats["orchestrators_injected"] == stats["orchestrators_found"],
        ...
        if result["has_orchestrators"]:
            if result["all_injected"]:
                self.logger.info(f"[NodeRegistry] ✅ All {stats['orchestrators_found']} orchestrators successfully injected")
            else:
                self.logger.warning(f"[NodeRegistry] ⚠️ Only {stats['orchestrators_injected']}/{stats['orchestrators_found']} orchestrators injected")
        else:
    ...
    def clear_cache(self, graph_name: Optional[str] = None) -> None:
        ...
        if graph_name:
            ...
            if cache_key in self._registry_cache:
                ...
                self.logger.debug(f"[NodeRegistry] Cleared cache for graph: {graph_name}")
        else:
    ...
    def _parse_node_context(self, context: Any) -> Dict[str, Any]:
        ...
        if isinstance(context, str):
            ...
            try:
                ...
                self.logger.debug(f"[NodeRegistry] Failed to parse context as key-value pairs: {e}")
