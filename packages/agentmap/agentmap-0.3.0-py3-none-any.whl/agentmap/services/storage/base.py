...
from agentmap.services.storage.types import (
    StorageResult, WriteMode, StorageConfig,
...
class BaseStorageService(StorageService, ABC):
    ...
    def _load_provider_config(self) -> StorageConfig:
        ...
        """
        try:
            config_data = self.configuration.get_value(f"storage.{self.provider_name}", {})
            if not config_data:
                # Try fallback configuration paths
                config_data = self.configuration.get_value(f"storage.providers.{self.provider_name}", {})
            ...
            self._logger.error(f"[{self.provider_name}] Failed to load configuration: {e}")
    ...
    def client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                self._logger.info(f"[{self.provider_name}] Client initialized successfully")
            except Exception as e:
                self._logger.error(f"[{self.provider_name}] Failed to initialize client: {e}")
    ...
    def batch_write(
        ...
        **kwargs
        ...
        """
        self._logger.debug(f"[{self.provider_name}] Performing batch write of {len(data)} items")
