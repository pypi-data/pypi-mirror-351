...
def extract_archive(
    ...
    Args:
        ...
        conflict_callback: Function to call when file conflicts occur
    """
    with TzstArchive(archive_path, "r") as archive:
        if flatten:
            ...
            # Global conflict resolution state
            overwrite_all = False
            skip_all = False
                ...
                if member.isfile():
                    ...
                    if (
                        ...
                        if conflict_callback:
                            ...
                            choice = conflict_callback(target_path, archived_info)
                            ...
                            if choice == "q":
                                ...
                                while target_path.exists():
                                    target_path = (
                                        extract_dir / f"{base_name}_{counter}{suffix}"
                                    )
                                    counter += 1
                            # 'y' falls through to extract normally
                    elif target_path.exists() and skip_all:
                        continue
                    elif target_path.exists() and auto_rename_all:
                        # Find a unique name
                        counter = 1
                        base_name = target_path.stem
                        suffix = target_path.suffix
                        while target_path.exists():
                            target_path = extract_dir / f"{base_name}_{counter}{suffix}"
                            counter += 1
            ...
            for member in member_list:
                ...
                if member.isfile():
                    ...
                    if (
                        ...
                        while target_path.exists():
                            target_path = (
                                target_path.parent / f"{base_name}_{counter}{suffix}"
                            )
                            counter += 1
