...
# Assuming prompt_text_ssis is defined in .prompts
from .prompts import prompt_text_ssis  # You must define this function separately
        ...
        try:
            ...
            for conn in root.findall(".//DTS:ConnectionManager", namespace):
                try:
                    ...
                    if conn_obj_data is not None:
                        conn_string = conn_obj_data.attrib.get("{www.microsoft.com/SqlServer/Dts}ConnectionString", "N/A")
                        ...
                        if conn_type == "FLATFILE":
                            ...
                            for col in conn_obj_data.findall(".//DTS:Column", namespace):
                                ...
                                if col_name:
                                    ...
                                    structured_data["issues"].append(f"Column in connection {conn_name} missing Name attribute")
                            conn_details["Columns"] = columns
                    summary.append(f"Connection: {conn_name} ({conn_type}, ConnectionString: {conn_details['ConnectionString']})")
            ...
            for var in root.findall(".//DTS:Variable", namespace):
                try:
                    ...
                    var_value = var_value_elem.text if var_value_elem is not None and hasattr(var_value_elem, 'text') else "N/A"
            ...
            for path in root.findall(".//DTS:Executables//path", namespace):
                try:
                    ...
                    if start_id not in component_ids or end_id not in component_ids:
                        structured_data["issues"].append(f"Path {path_name} references non-existent components: {start_id} -> {end_id}")
                except AttributeError as e:
...
class SSISEvaluator:
    ...
    def evaluate(self, questions: List[str], answer_path: str) -> Dict[str, any]:
        try:
            if not answer_path.endswith('.dtsx'):
                raise ValueError("File must be a .dtsx file")

            logger.debug("Reading .dtsx file: %s", answer_path)
            with open(answer_path, 'r', encoding='utf-8') as file:
                content = file.read()

            parsed_data = SSISAnswerParser.parse_single_file(content)
            if not isinstance(parsed_data, dict):
                raise TypeError(f"Expected dict from parse_single_file, got {type(parsed_data)}")
            ...
            if not answers:
                logger.warning("No answers parsed from .dtsx file")
                ...
                logger.debug("Expanded %d answers to %d for %d questions", len(parsed_data["text_answers"]), len(answers), len(questions))
            elif len(answers) > len(questions):
                # Truncate to match question count
                answers = answers[:len(questions)]
                logger.debug("Truncated %d answers to %d for %d questions", len(parsed_data["text_answers"]), len(answers), len(questions))
