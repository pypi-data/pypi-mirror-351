...
class LogTensor:
    ...
    def _process(self, name: str, resp: Union[Callable[[Any], Any], Any]):
        if callable(resp):
            try:
                response = resp()
            except Exception as e:
        ...
        self._print(msg)
        return dict(name=name, results=response, message=msg)

    def __call__(
        self,
        main_item: Union[Tensor, np.ndarray, Sequence[Number]],
        title: Optional[str] = None,
        loss_targets: Optional[Union[Tensor, np.ndarray, Sequence[Number]]] = None,
        *,
        ...
        **kwargs,
        ...
        main_item = self._setup_tensor(main_item)
        loss_targets = self._setup_tensor(loss_targets)
        if is_str(title):
        ...
        current_register.update(self._process("shape", main_item.shape))
        current_register.update(self._process("ndim", main_item.ndim))
        current_register.update(self._process("dtype", main_item.dtype))
        if log_device:
            current_register.update(self._process("device", main_item.device))

        if log_mean:
            fn = lambda: main_item.mean(
                dim=dim_mean,
            )
            current_register.update(self._process("mean", fn))

        if log_std:
            fn = lambda: main_item.std(dim=dim_std)
            current_register.update(self._process("std", fn))

        if external_logs:
            old_print = self.do_print
            self.do_print = do_print_external
            self._print("\n---[ External Logs ] ---")
            for log_fn, log_args in external_logs:
                if isinstance(log_args, Sequence) and not isinstance(log_args, str):
                    value = try_torch(log_fn, main_item, *log_args)
                elif isinstance(log_args, dict):
                    value = try_torch(log_fn, main_item, **log_args)
                elif log_args is None:
                    value = try_torch(log_fn, main_item)
                else:
                    value = try_torch(log_fn, main_item, log_args)
        ...
        if loss_targets is not None:
            losses = get_losses(main_item, loss_targets, False)
            started_ls = False
            if self.do_print:
        ...
        if log_tensor:
            current_register.update(self._process("Tensor", main_item))
