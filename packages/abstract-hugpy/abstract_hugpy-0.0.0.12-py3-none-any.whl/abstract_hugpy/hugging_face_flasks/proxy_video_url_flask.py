...
def get_from_local_host(endpoint,req):
    local_url = get_deepcoder_endpoint(endpoint)
    data = get_request_data(req)
    summary = postRequest(url=local_url,data=data)
    return summary
def get_deepcoder_endpoint(endpoint):
    if 'deepcoder_generate':
        port = get_env_key('DEEPCODER_FLASK_PORT')
    else:
        port = get_env_key('VIDEO_URL_FLASK_PORT')
