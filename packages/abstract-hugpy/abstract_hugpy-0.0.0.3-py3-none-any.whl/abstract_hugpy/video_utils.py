...
class VideoDirectoryManager(metaclass=SingletonMeta):
    ...
    def _init_data(self, url, video_id):
        dir_path = os.path.join(self.videos_directory, video_id)
        os.makedirs(dir_path, exist_ok=True)
        info_path = os.path.join(dir_path, 'video_info.json')
        video_info = VideoDownloader(url, download_directory=dir_path,download_video=False)
    ...
    def get_data(self, url=None, video_id=None):
        video_id = video_id or get_video_id(url)
        if video_id in self.url_data:
            return self.url_data[video_id]
        return self._init_data(url, video_id)

    def download_video(self, url):
        data = self.get_data(url)
        if not os.path.isfile(data['video_path']):
            video_info = VideoDownloader(url, download_directory=data['directory']).info
    ...
    def get_whisper(self, url):
        data = self.get_data(url)
        if not os.path.isfile(data['whisper_path']):
            audio = self.extract_audio(url)
...
def download_video(url): return video_mgr.download_video(url)
def extract_audio(url): return video_mgr.extract_audio(url)
def get_whisper(url): return video_mgr.get_whisper(url)
def get_video_whisper_text(url): return get_whisper(url).get('text')
def get_video_whisper_segments(url): return get_whisper(url).get('segments')
def get_metadata(url): return video_mgr.get_metadata(url)
def get_captions(url): return video_mgr.get_captions(url)
def get_video_directory(url): return video_mgr.get_data(url)['directory']
def get_video_path(url): return video_mgr.get_data(url)['video_path']
def get_audio_path(url): return video_mgr.get_data(url)['audio_path']
def get_srt_path(url): return video_mgr.get_data(url)['srt_path']
def get_metadata_path(url): return video_mgr.get_data(url)['metadata_path']
