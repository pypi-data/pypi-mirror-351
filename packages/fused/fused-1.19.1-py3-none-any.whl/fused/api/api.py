...
from fused.core._serialization import deserialize_parquet
...
class FusedAPI:
    ...
    def download_table_bbox(
        ...
        raise_for_status(r)
        return deserialize_parquet(r.content)
    ...
    def _get_udf_objects(
        ...
        *,
        ...
        udf_type_includes: dict[Literal["app", "udf"], list[str]] = {
            "app": ["app"],
            "udf": [
                "auto",
                "vector_tile",
        ...
        while has_content:
            ...
            if udfs_this_request:
                non_app_udfs = list(
                    udf
                    for udf in udfs_this_request
                    if udf["udf_type"] in udf_type_includes
                )
                udfs.extend(non_app_udfs)
            else:
                has_content = False
    ...
    def tail_logs(
        ...
        if job.status not in ["running", "pending"]:
            print(f"Job is not running ({job.job_status})")
            for message in r:
    ...
    def _list_efs(self, path: str, details: bool, client_id: str):
        prefix = "file:///mount"
    ...
    def download(
        ...
        *,
        ...
        if path.startswith("file:///mount"):
            path = path.removeprefix("file:///mount")
    ...
    def upload(
        self, path: str, data: bytes | BinaryIO, timeout: float | None = None
    ) -> None:
        """Upload a binary blob to a cloud location"""
        return self._upload_signed(path=path, data=data, timeout=timeout)
