...
class Output(BaseModel):
    ...
    sidecar_output: Optional[Union[bytes, Path]] = None
...
class PandasOutput(Output):
    ...
    def validate_data_with_schema(self, *, strict: bool = False):
        ...
        if not self.skip_fused_index_validation:
            assert INDEX_COLUMN_NAME in self.data.columns, (
        ...
        df_column_diff = {str(x) for x in self.data.columns}.difference(field_names)
        if df_column_diff:
            raise ValueError(
