...
from fused._options import StorageStr, get_cache_dir
...
class CacheLogEntry:
    ...
    @classmethod
    def byte_length(cls):
        ex_length = 26  # datetime.isoformat
        uuid = 36  # str(uuid4())
        status = 1  # char
...
def _serialize_function_defaults(func: Callable) -> bytes:
    ...
    if func.__kwdefaults__ is not None:
        for key, value in func.__kwdefaults__.items():
            parts.append(f"{key}{_hashify(value)}".encode())
...
def _get_lock(target: Path, lock_dir, new_cache_uuid):
    if is_pyodide():
        # Pyodide does not support os.link, so we use exclusive file creation
...
def wait_for_ready_result(
    ...
    *,
    ...
    decorator failed mid-execution). When this is used for reading the cache for the first time and no result is
    expected to exist, we do not want to wait. However, in the case where we know another invocation has acquired a
    lock and will write an entry, `wait_if_none` can be used during these situations to continue to wait.
    ...
    Args:
        ...
        wait_if_none (bool): Whether to wait if no cache entry is detected. Useful when the caller knows that an
            entry will soon be available.
    ...
    while True:
        if retries > max_retries:
            raise CacheResultError("Max retries reached")
        ...
        if cache_entry is None:
            ...
            logger.debug(
                f"Sleeping. No entry found yet. Retry [{retries}/{max_retries}]"
            )
...
def _cache(
    ...
    **kwargs,
    ...
    try:
        ...
        base_path = OPTIONS.cache_directory
        if storage != "auto":
            base_path = get_cache_dir(storage)
        ...
        for i, v in enumerate(args):
            ...
            if parsed_result.param_names[i] not in cache_key_exclude:
                id += "_" + _hashify(v)
        ...
        if not reset:
            try:
                return wait_for_ready_result(
                    fn_name, now, max_retries, wait_time_seconds, path_dir
                )
            except CacheResultError as e:
        ...
        # entry to complete.
        try:
            with _get_lock(path_dir / "index.log", locks_dir, new_cache_uuid):
