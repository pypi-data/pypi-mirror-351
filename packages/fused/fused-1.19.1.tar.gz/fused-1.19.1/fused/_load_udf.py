...
def load(url_or_udf: Union[str, Path], /, *, cache_key: Any = None) -> AnyBaseUdf:
    ...
    if isinstance(url_or_udf, str) and "\n" in url_or_udf:
        ...
        return load_udf_from_code(url_or_udf)

    path = Path(url_or_udf)

    if path.suffix == ".py":
        return load_udf_from_file(path)

    if path.suffix == ".zip":
        return load_udf_from_zip(path)

    if path.is_dir():  # Non-existant paths will be false
        return load_udf_from_directory(path)

    if isinstance(url_or_udf, str):
        if is_udf_token(url_or_udf):
            try:
                return load_udf_from_shared_token(url_or_udf)
