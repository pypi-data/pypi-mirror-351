...
from fused.core._impl._context_impl import context_in_batch
from fused.models.udf import GeoPandasUdfV2, MockUdfInput, Output


def _compile_and_get_local(udf: GeoPandasUdfV2) -> Callable:
    """
    ...
    def entrypoint_wrapper(*args, **kwargs):
        ...
        if isinstance(entrypoint, GeoPandasUdfV2):
...
def compile_udf_and_run_v3(code: GeoPandasUdfV2, input: MockUdfInput, output: Output):
    from fused._udf.state import (
        decorator_src_override_context,
        decorator_udf_override_context,
    )
    ...
    # Support returning a DataFrame directly from the UDF.
    if result is not None:
        output.data = result
    return result


def compile_udf_and_run_with_kwargs(udf: GeoPandasUdfV2, *args, **kwargs):
    ...
    compile_udf_and_run_v3(udf, input=input, output=output)
