...
StorageStr = Literal["auto", "mount", "local"]
...
def _cache_directory() -> Path:
    return get_cache_dir("auto")


def _data_directory() -> Path:
    return get_data_dir("auto")


def get_data_dir(storage: StorageStr = "auto") -> Path:
...
def get_cache_dir(storage: StorageStr = "auto") -> Path:
    base_path = get_writable_dir(storage)
    return base_path / "cached_data"


def get_writable_dir(storage: StorageStr = "auto") -> Path:
...
class Options(OptionsBaseModel):
    ...
    @classmethod
    def _validate_data_directory(cls, v):
        if is_path_writable(Path(v)):
            return v
        else:
...
def env(environment_name: Union[str, int]):
    ...
    # setting this will cause the global API to be reset
    setattr(options, "base_url", _env)
