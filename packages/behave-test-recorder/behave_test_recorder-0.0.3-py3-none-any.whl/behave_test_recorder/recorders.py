...
class BehaveTestRecorder(metaclass=SingletonMeta):
    ...
    def _write_request(
        ...
        unique_mixin = uuid.uuid4().hex[:6]
        path_postfix = request.path.replace('-', '_').split('/')[-1]
    ...
    def check_request(self, request) -> bool:
        ...
        is_request_appropriate = (
            ...
            (
                ...
                # TODO BOBUH-23698 /ria/gateway относится к проекту web_bb, нужно отделить.
                '/ria/gateway/' in request.path
            ) and
            not (
    ...
    def define_response_data(self, response, context):
        result_response_data = None

        if (
            response['content-type'] == 'application/json' or
            response['content-type'] == 'application/json; charset=utf-8' or
            self.project_downloads_url in response.content.decode('utf-8')
        ):
            result_response_data = response.content
        elif context:
            if 'window' in context:
                result_response_data = context['window']
            elif 'component' in context:
                result_response_data = context['component']
        else:
            try:
                json.loads(response.content)
            except ValueError:
                content_data = context_extractor.extract_data_with_regex(response.content)
                if content_data:
                    result_response_data = content_data
            else:
                result_response_data = response.content
