...
class ValidationTests(TestCase):
    ...
    def test_gates_success_unknown_type(self):
        ...
        self.assertIn("[type=enum, input_value='wrong', input_type=str]", str(e.exception))
    ...
    def test_parksettings_gates_no_empty_list(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("List should have at least 1 item after validation, not 0", str(e.exception))
    ...
    def test_parksettings_invalid_default_cost_entry_type(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("is not allowed as cost entry", str(e.exception))
    ...
    def test_cost_entry_invalid_type(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("[type=enum, input_value='static_cost123', input_type=str]", str(e.exception))
    ...
    def test_cost_entry_free_timerange(self):
        ...
        self.assertDictEqual(dict(entry_start=datetime(2021, 1, 1, 0, 0, 0)),
        ...
        with self.assertRaises(ValidationError) as e:
            parse_setting("common/privacy_settings",
                          dump_setting({"paid": {"hours": 10},
                                        ...
                                        "rejected": {"days": 7, "hours": 0, "minutes": 0}}))
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("Input should be greater than or equal to 0", str(e.exception))
    ...
    def test_cost_groups_vat_over_100(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("[type=less_than_equal, input_value=200, input_type=int]", str(e.exception))
    ...
    def test_timezone(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("Input should be a valid string", str(e.exception))
    ...
    def test_support_invalid_email(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertIn("String should have at least 3 characters", str(e.exception))
    ...
    def test_garage_name(self):
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertEqual(("technical_name",), e.exception.errors()[0]["loc"])
        self.assertIn("String should match pattern", str(e.exception))
        ...
        self.assertEqual(len(e.exception.errors()), 1)
        self.assertEqual(("slug",), e.exception.errors()[0]["loc"])
        self.assertIn("String should match pattern", str(e.exception))
    ...
    def test_enforcement(self):
        ...
        self.assertIn("Input should be greater than 0", str(e.exception))
        ...
        with self.assertRaises(ValidationError) as e:
            parse_setting("enforcement/basic_settings", setting_str)
        self.assertIn("Input should be less than or equal to 960", str(e.exception))
    ...
    def test_io_signals_correct_fields_gate_related(self):
        for t in [SignalType.permanently_closed, SignalType.shortterm_full, SignalType.open, SignalType.present_raw,
                  SignalType.present_decision, SignalType.traffic_light_green, SignalType.traffic_light_red]:
            setting_str = (
