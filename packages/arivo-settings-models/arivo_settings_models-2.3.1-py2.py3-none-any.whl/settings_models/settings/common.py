...
class CostEntryData(SettingsModel):
    ...
    source: Optional[str] = Field(None, description="Source of cost entry (usually service name)")
    source_id: Optional[str] = Field(None, description="Id to get information about cost entry in source service")
    idempotency_key: Optional[str] = Field(None, description="Legacy field for idempotency key")
...
class ParkingArea(SettingsModel):
    ...
    number_of_parking_spaces: Optional[int] = Field(None, description="Number of parking spaces in this area. None if "
                                                                      "not specified", ge=0)
