# coding: utf-8

# flake8: noqa

"""
    Arthur Scope

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 0.1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


__version__ = "0.1.0"

# import apis into sdk package
from scope_client.api_bindings.api.alert_rules_v1_api import AlertRulesV1Api
from scope_client.api_bindings.api.alerts_v1_api import AlertsV1Api
from scope_client.api_bindings.api.authorization_v1_api import AuthorizationV1Api
from scope_client.api_bindings.api.connector_schemas_v1_api import ConnectorSchemasV1Api
from scope_client.api_bindings.api.connectors_v1_api import ConnectorsV1Api
from scope_client.api_bindings.api.data_plane_associations_v1_api import DataPlaneAssociationsV1Api
from scope_client.api_bindings.api.data_planes_v1_api import DataPlanesV1Api
from scope_client.api_bindings.api.data_retrieval_v1_api import DataRetrievalV1Api
from scope_client.api_bindings.api.datasets_v1_api import DatasetsV1Api
from scope_client.api_bindings.api.feature_flags_v1_api import FeatureFlagsV1Api
from scope_client.api_bindings.api.groups_v1_api import GroupsV1Api
from scope_client.api_bindings.api.jobs_v1_api import JobsV1Api
from scope_client.api_bindings.api.metric_functions_v1_api import MetricFunctionsV1Api
from scope_client.api_bindings.api.metrics_v1_api import MetricsV1Api
from scope_client.api_bindings.api.models_v1_api import ModelsV1Api
from scope_client.api_bindings.api.o_auth_v1_api import OAuthV1Api
from scope_client.api_bindings.api.organizations_v1_api import OrganizationsV1Api
from scope_client.api_bindings.api.projects_v1_api import ProjectsV1Api
from scope_client.api_bindings.api.registration_v1_api import RegistrationV1Api
from scope_client.api_bindings.api.roles_v1_api import RolesV1Api
from scope_client.api_bindings.api.tasks_v1_api import TasksV1Api
from scope_client.api_bindings.api.upsolve_v1_api import UpsolveV1Api
from scope_client.api_bindings.api.users_v1_api import UsersV1Api
from scope_client.api_bindings.api.webhooks_v1_api import WebhooksV1Api
from scope_client.api_bindings.api.workspaces_v1_api import WorkspacesV1Api
from scope_client.api_bindings.api.default_api import DefaultApi

# import ApiClient
from scope_client.api_bindings.api_response import ApiResponse
from scope_client.api_bindings.api_client import ApiClient
from scope_client.api_bindings.configuration import Configuration
from scope_client.api_bindings.exceptions import OpenApiException
from scope_client.api_bindings.exceptions import ApiTypeError
from scope_client.api_bindings.exceptions import ApiValueError
from scope_client.api_bindings.exceptions import ApiKeyError
from scope_client.api_bindings.exceptions import ApiAttributeError
from scope_client.api_bindings.exceptions import ApiException

# import models into sdk package
from scope_client.api_bindings.models.aggregation_metric_type import AggregationMetricType
from scope_client.api_bindings.models.aggregation_spec import AggregationSpec
from scope_client.api_bindings.models.aggregation_spec_schema import AggregationSpecSchema
from scope_client.api_bindings.models.aggregation_spec_schema_init_args_inner import AggregationSpecSchemaInitArgsInner
from scope_client.api_bindings.models.alert import Alert
from scope_client.api_bindings.models.alert_bound import AlertBound
from scope_client.api_bindings.models.alert_check_job_spec import AlertCheckJobSpec
from scope_client.api_bindings.models.alert_rule import AlertRule
from scope_client.api_bindings.models.alert_rule_interval import AlertRuleInterval
from scope_client.api_bindings.models.alert_rule_notification_webhook import AlertRuleNotificationWebhook
from scope_client.api_bindings.models.alert_rule_sort import AlertRuleSort
from scope_client.api_bindings.models.alert_sort import AlertSort
from scope_client.api_bindings.models.alert_webhook_called import AlertWebhookCalled
from scope_client.api_bindings.models.available_dataset import AvailableDataset
from scope_client.api_bindings.models.available_datasets_sort import AvailableDatasetsSort
from scope_client.api_bindings.models.bad_request_error import BadRequestError
from scope_client.api_bindings.models.base_role import BaseRole
from scope_client.api_bindings.models.bound_member import BoundMember
from scope_client.api_bindings.models.bound_member_kind import BoundMemberKind
from scope_client.api_bindings.models.bound_resource import BoundResource
from scope_client.api_bindings.models.bound_resource_kind import BoundResourceKind
from scope_client.api_bindings.models.bound_role import BoundRole
from scope_client.api_bindings.models.config import Config
from scope_client.api_bindings.models.conflict_error import ConflictError
from scope_client.api_bindings.models.connector_check_job_spec import ConnectorCheckJobSpec
from scope_client.api_bindings.models.connector_check_outcome import ConnectorCheckOutcome
from scope_client.api_bindings.models.connector_check_result import ConnectorCheckResult
from scope_client.api_bindings.models.connector_field_data_type import ConnectorFieldDataType
from scope_client.api_bindings.models.connector_pagination_options import ConnectorPaginationOptions
from scope_client.api_bindings.models.connector_spec import ConnectorSpec
from scope_client.api_bindings.models.connector_spec_field import ConnectorSpecField
from scope_client.api_bindings.models.connector_spec_field_with_metadata import ConnectorSpecFieldWithMetadata
from scope_client.api_bindings.models.connector_spec_schema import ConnectorSpecSchema
from scope_client.api_bindings.models.connector_spec_schema_field import ConnectorSpecSchemaField
from scope_client.api_bindings.models.connector_type import ConnectorType
from scope_client.api_bindings.models.connectors_sort import ConnectorsSort
from scope_client.api_bindings.models.create_model_link_task_job_spec import CreateModelLinkTaskJobSpec
from scope_client.api_bindings.models.create_model_task_job_spec import CreateModelTaskJobSpec
from scope_client.api_bindings.models.created_alerts import CreatedAlerts
from scope_client.api_bindings.models.credentials import Credentials
from scope_client.api_bindings.models.d_type import DType
from scope_client.api_bindings.models.data_plane import DataPlane
from scope_client.api_bindings.models.data_plane_association import DataPlaneAssociation
from scope_client.api_bindings.models.data_plane_association_sort import DataPlaneAssociationSort
from scope_client.api_bindings.models.data_plane_capabilities import DataPlaneCapabilities
from scope_client.api_bindings.models.data_plane_sort import DataPlaneSort
from scope_client.api_bindings.models.data_result_filter import DataResultFilter
from scope_client.api_bindings.models.data_result_filter_op import DataResultFilterOp
from scope_client.api_bindings.models.data_retrieval_data import DataRetrievalData
from scope_client.api_bindings.models.data_retrieval_operation import DataRetrievalOperation
from scope_client.api_bindings.models.data_retrieval_status import DataRetrievalStatus
from scope_client.api_bindings.models.dataset import Dataset
from scope_client.api_bindings.models.dataset_column import DatasetColumn
from scope_client.api_bindings.models.dataset_connector import DatasetConnector
from scope_client.api_bindings.models.dataset_join_kind import DatasetJoinKind
from scope_client.api_bindings.models.dataset_join_spec import DatasetJoinSpec
from scope_client.api_bindings.models.dataset_list_type import DatasetListType
from scope_client.api_bindings.models.dataset_locator import DatasetLocator
from scope_client.api_bindings.models.dataset_locator_field import DatasetLocatorField
from scope_client.api_bindings.models.dataset_locator_field_data_type import DatasetLocatorFieldDataType
from scope_client.api_bindings.models.dataset_locator_schema import DatasetLocatorSchema
from scope_client.api_bindings.models.dataset_locator_schema_field import DatasetLocatorSchemaField
from scope_client.api_bindings.models.dataset_object_type import DatasetObjectType
from scope_client.api_bindings.models.dataset_reference import DatasetReference
from scope_client.api_bindings.models.dataset_scalar_type import DatasetScalarType
from scope_client.api_bindings.models.dataset_schema import DatasetSchema
from scope_client.api_bindings.models.datasets_sort import DatasetsSort
from scope_client.api_bindings.models.definition import Definition
from scope_client.api_bindings.models.delete_group_membership import DeleteGroupMembership
from scope_client.api_bindings.models.delete_model_task_job_spec import DeleteModelTaskJobSpec
from scope_client.api_bindings.models.dimension import Dimension
from scope_client.api_bindings.models.example_config import ExampleConfig
from scope_client.api_bindings.models.examples_config import ExamplesConfig
from scope_client.api_bindings.models.extended_role import ExtendedRole
from scope_client.api_bindings.models.fetch_data_job_spec import FetchDataJobSpec
from scope_client.api_bindings.models.fetch_model_task_job_spec import FetchModelTaskJobSpec
from scope_client.api_bindings.models.generate_metrics_spec_request import GenerateMetricsSpecRequest
from scope_client.api_bindings.models.group import Group
from scope_client.api_bindings.models.group_membership import GroupMembership
from scope_client.api_bindings.models.group_sort import GroupSort
from scope_client.api_bindings.models.http_validation_error import HTTPValidationError
from scope_client.api_bindings.models.health_status import HealthStatus
from scope_client.api_bindings.models.infinite_pagination import InfinitePagination
from scope_client.api_bindings.models.infinite_resource_list_alert import InfiniteResourceListAlert
from scope_client.api_bindings.models.infinite_resource_list_job import InfiniteResourceListJob
from scope_client.api_bindings.models.internal_server_error import InternalServerError
from scope_client.api_bindings.models.interval_unit import IntervalUnit
from scope_client.api_bindings.models.invite_new_user import InviteNewUser
from scope_client.api_bindings.models.items import Items
from scope_client.api_bindings.models.items1 import Items1
from scope_client.api_bindings.models.job import Job
from scope_client.api_bindings.models.job_dequeue_parameters import JobDequeueParameters
from scope_client.api_bindings.models.job_error import JobError
from scope_client.api_bindings.models.job_errors import JobErrors
from scope_client.api_bindings.models.job_kind import JobKind
from scope_client.api_bindings.models.job_log import JobLog
from scope_client.api_bindings.models.job_log_level import JobLogLevel
from scope_client.api_bindings.models.job_logs import JobLogs
from scope_client.api_bindings.models.job_priority import JobPriority
from scope_client.api_bindings.models.job_run import JobRun
from scope_client.api_bindings.models.job_spec import JobSpec
from scope_client.api_bindings.models.job_state import JobState
from scope_client.api_bindings.models.job_trigger import JobTrigger
from scope_client.api_bindings.models.jobs_batch import JobsBatch
from scope_client.api_bindings.models.jobs_sort import JobsSort
from scope_client.api_bindings.models.joined_dataset import JoinedDataset
from scope_client.api_bindings.models.keywords_config import KeywordsConfig
from scope_client.api_bindings.models.list_datasets_job_spec import ListDatasetsJobSpec
from scope_client.api_bindings.models.list_type import ListType
from scope_client.api_bindings.models.metrics_arg_spec import MetricsArgSpec
from scope_client.api_bindings.models.metrics_calculation_job_spec import MetricsCalculationJobSpec
from scope_client.api_bindings.models.metrics_column_list_parameter_schema import MetricsColumnListParameterSchema
from scope_client.api_bindings.models.metrics_column_parameter_schema import MetricsColumnParameterSchema
from scope_client.api_bindings.models.metrics_column_parameter_schema_allowed_column_types_inner import MetricsColumnParameterSchemaAllowedColumnTypesInner
from scope_client.api_bindings.models.metrics_dataset_parameter_schema import MetricsDatasetParameterSchema
from scope_client.api_bindings.models.metrics_literal_parameter_schema import MetricsLiteralParameterSchema
from scope_client.api_bindings.models.metrics_query_result import MetricsQueryResult
from scope_client.api_bindings.models.metrics_result_filter_op import MetricsResultFilterOp
from scope_client.api_bindings.models.metrics_upload import MetricsUpload
from scope_client.api_bindings.models.metrics_upload_metrics_inner import MetricsUploadMetricsInner
from scope_client.api_bindings.models.metrics_upload_result import MetricsUploadResult
from scope_client.api_bindings.models.metrics_version import MetricsVersion
from scope_client.api_bindings.models.metrics_versions_sort import MetricsVersionsSort
from scope_client.api_bindings.models.model import Model
from scope_client.api_bindings.models.model_metric_spec import ModelMetricSpec
from scope_client.api_bindings.models.model_metrics_schedule import ModelMetricsSchedule
from scope_client.api_bindings.models.model_problem_type import ModelProblemType
from scope_client.api_bindings.models.models_sort import ModelsSort
from scope_client.api_bindings.models.new_rule_request import NewRuleRequest
from scope_client.api_bindings.models.not_found_error import NotFoundError
from scope_client.api_bindings.models.numeric_metric import NumericMetric
from scope_client.api_bindings.models.numeric_point import NumericPoint
from scope_client.api_bindings.models.numeric_time_series import NumericTimeSeries
from scope_client.api_bindings.models.object_type import ObjectType
from scope_client.api_bindings.models.object_value import ObjectValue
from scope_client.api_bindings.models.organization import Organization
from scope_client.api_bindings.models.pii_config import PIIConfig
from scope_client.api_bindings.models.pagination import Pagination
from scope_client.api_bindings.models.patch_alert_rule import PatchAlertRule
from scope_client.api_bindings.models.patch_connector_spec import PatchConnectorSpec
from scope_client.api_bindings.models.patch_data_plane import PatchDataPlane
from scope_client.api_bindings.models.patch_data_plane_capabilities import PatchDataPlaneCapabilities
from scope_client.api_bindings.models.patch_dataset import PatchDataset
from scope_client.api_bindings.models.patch_dataset_locator import PatchDatasetLocator
from scope_client.api_bindings.models.patch_group import PatchGroup
from scope_client.api_bindings.models.patch_job import PatchJob
from scope_client.api_bindings.models.patch_model import PatchModel
from scope_client.api_bindings.models.patch_organization import PatchOrganization
from scope_client.api_bindings.models.patch_project import PatchProject
from scope_client.api_bindings.models.patch_service_account_user import PatchServiceAccountUser
from scope_client.api_bindings.models.patch_task_request import PatchTaskRequest
from scope_client.api_bindings.models.patch_user import PatchUser
from scope_client.api_bindings.models.patch_webhook import PatchWebhook
from scope_client.api_bindings.models.patch_workspace import PatchWorkspace
from scope_client.api_bindings.models.permission import Permission
from scope_client.api_bindings.models.permission_name import PermissionName
from scope_client.api_bindings.models.permission_request_item import PermissionRequestItem
from scope_client.api_bindings.models.permission_response_item import PermissionResponseItem
from scope_client.api_bindings.models.permissions_request import PermissionsRequest
from scope_client.api_bindings.models.permissions_response import PermissionsResponse
from scope_client.api_bindings.models.post_alert import PostAlert
from scope_client.api_bindings.models.post_alert_rule import PostAlertRule
from scope_client.api_bindings.models.post_alerts import PostAlerts
from scope_client.api_bindings.models.post_connector_spec import PostConnectorSpec
from scope_client.api_bindings.models.post_data_plane import PostDataPlane
from scope_client.api_bindings.models.post_data_plane_association import PostDataPlaneAssociation
from scope_client.api_bindings.models.post_data_retrieval_operation import PostDataRetrievalOperation
from scope_client.api_bindings.models.post_dataset import PostDataset
from scope_client.api_bindings.models.post_dataset_join_spec import PostDatasetJoinSpec
from scope_client.api_bindings.models.post_end_user import PostEndUser
from scope_client.api_bindings.models.post_global_role_binding import PostGlobalRoleBinding
from scope_client.api_bindings.models.post_group import PostGroup
from scope_client.api_bindings.models.post_group_membership import PostGroupMembership
from scope_client.api_bindings.models.post_job import PostJob
from scope_client.api_bindings.models.post_job_batch import PostJobBatch
from scope_client.api_bindings.models.post_job_kind import PostJobKind
from scope_client.api_bindings.models.post_job_spec import PostJobSpec
from scope_client.api_bindings.models.post_link_task_request import PostLinkTaskRequest
from scope_client.api_bindings.models.post_metrics_query import PostMetricsQuery
from scope_client.api_bindings.models.post_metrics_query_result_filter import PostMetricsQueryResultFilter
from scope_client.api_bindings.models.post_metrics_query_result_filter_and_group import PostMetricsQueryResultFilterAndGroup
from scope_client.api_bindings.models.post_metrics_query_result_filter_and_group_and_inner import PostMetricsQueryResultFilterAndGroupAndInner
from scope_client.api_bindings.models.post_metrics_query_result_filter_or_group import PostMetricsQueryResultFilterOrGroup
from scope_client.api_bindings.models.post_metrics_query_result_filter_or_group_or_inner import PostMetricsQueryResultFilterOrGroupOrInner
from scope_client.api_bindings.models.post_metrics_query_time_range import PostMetricsQueryTimeRange
from scope_client.api_bindings.models.post_metrics_versions import PostMetricsVersions
from scope_client.api_bindings.models.post_model import PostModel
from scope_client.api_bindings.models.post_project import PostProject
from scope_client.api_bindings.models.post_role_binding import PostRoleBinding
from scope_client.api_bindings.models.post_service_account import PostServiceAccount
from scope_client.api_bindings.models.post_task_request import PostTaskRequest
from scope_client.api_bindings.models.post_task_validation_api_key import PostTaskValidationAPIKey
from scope_client.api_bindings.models.post_upsolve_tenant import PostUpsolveTenant
from scope_client.api_bindings.models.post_webhook import PostWebhook
from scope_client.api_bindings.models.post_workspace import PostWorkspace
from scope_client.api_bindings.models.project import Project
from scope_client.api_bindings.models.project_bound_resource_metadata import ProjectBoundResourceMetadata
from scope_client.api_bindings.models.project_sort import ProjectSort
from scope_client.api_bindings.models.put_available_dataset import PutAvailableDataset
from scope_client.api_bindings.models.put_available_datasets import PutAvailableDatasets
from scope_client.api_bindings.models.put_dataset_schema import PutDatasetSchema
from scope_client.api_bindings.models.put_job_state import PutJobState
from scope_client.api_bindings.models.put_model_metric_spec import PutModelMetricSpec
from scope_client.api_bindings.models.put_model_metrics_schedule import PutModelMetricsSchedule
from scope_client.api_bindings.models.put_retrieved_data import PutRetrievedData
from scope_client.api_bindings.models.put_task_connection_info import PutTaskConnectionInfo
from scope_client.api_bindings.models.put_task_state_cache_request import PutTaskStateCacheRequest
from scope_client.api_bindings.models.regenerate_task_validation_key_job_spec import RegenerateTaskValidationKeyJobSpec
from scope_client.api_bindings.models.regex_config import RegexConfig
from scope_client.api_bindings.models.register_user import RegisterUser
from scope_client.api_bindings.models.resource_kind import ResourceKind
from scope_client.api_bindings.models.resource_list_aggregation_spec_schema import ResourceListAggregationSpecSchema
from scope_client.api_bindings.models.resource_list_alert_rule import ResourceListAlertRule
from scope_client.api_bindings.models.resource_list_available_dataset import ResourceListAvailableDataset
from scope_client.api_bindings.models.resource_list_connector_spec import ResourceListConnectorSpec
from scope_client.api_bindings.models.resource_list_connector_type import ResourceListConnectorType
from scope_client.api_bindings.models.resource_list_data_plane import ResourceListDataPlane
from scope_client.api_bindings.models.resource_list_data_plane_association import ResourceListDataPlaneAssociation
from scope_client.api_bindings.models.resource_list_dataset import ResourceListDataset
from scope_client.api_bindings.models.resource_list_group import ResourceListGroup
from scope_client.api_bindings.models.resource_list_group_membership import ResourceListGroupMembership
from scope_client.api_bindings.models.resource_list_job_error import ResourceListJobError
from scope_client.api_bindings.models.resource_list_job_log import ResourceListJobLog
from scope_client.api_bindings.models.resource_list_job_run import ResourceListJobRun
from scope_client.api_bindings.models.resource_list_metrics_version import ResourceListMetricsVersion
from scope_client.api_bindings.models.resource_list_model import ResourceListModel
from scope_client.api_bindings.models.resource_list_permission import ResourceListPermission
from scope_client.api_bindings.models.resource_list_project import ResourceListProject
from scope_client.api_bindings.models.resource_list_role import ResourceListRole
from scope_client.api_bindings.models.resource_list_role_binding import ResourceListRoleBinding
from scope_client.api_bindings.models.resource_list_user import ResourceListUser
from scope_client.api_bindings.models.resource_list_webhook import ResourceListWebhook
from scope_client.api_bindings.models.resource_list_workspace import ResourceListWorkspace
from scope_client.api_bindings.models.result_filter import ResultFilter
from scope_client.api_bindings.models.role import Role
from scope_client.api_bindings.models.role_binding import RoleBinding
from scope_client.api_bindings.models.role_sort import RoleSort
from scope_client.api_bindings.models.rule_response import RuleResponse
from scope_client.api_bindings.models.rule_response_config import RuleResponseConfig
from scope_client.api_bindings.models.rule_scope import RuleScope
from scope_client.api_bindings.models.rule_type import RuleType
from scope_client.api_bindings.models.scalar_type import ScalarType
from scope_client.api_bindings.models.schedule_jobs_job_spec import ScheduleJobsJobSpec
from scope_client.api_bindings.models.schema_inspection_job_spec import SchemaInspectionJobSpec
from scope_client.api_bindings.models.scope_schema_tag import ScopeSchemaTag
from scope_client.api_bindings.models.sensitive_data_plane import SensitiveDataPlane
from scope_client.api_bindings.models.sensitive_user import SensitiveUser
from scope_client.api_bindings.models.service_account_credentials import ServiceAccountCredentials
from scope_client.api_bindings.models.sketch_metric import SketchMetric
from scope_client.api_bindings.models.sketch_point import SketchPoint
from scope_client.api_bindings.models.sketch_time_series import SketchTimeSeries
from scope_client.api_bindings.models.sort_order import SortOrder
from scope_client.api_bindings.models.task_connection_info import TaskConnectionInfo
from scope_client.api_bindings.models.task_mutation_response import TaskMutationResponse
from scope_client.api_bindings.models.task_read_response import TaskReadResponse
from scope_client.api_bindings.models.task_response import TaskResponse
from scope_client.api_bindings.models.task_validation_api_key import TaskValidationAPIKey
from scope_client.api_bindings.models.task_validation_key_regeneration_response import TaskValidationKeyRegenerationResponse
from scope_client.api_bindings.models.toxicity_config import ToxicityConfig
from scope_client.api_bindings.models.update_model_task_rules_job_spec import UpdateModelTaskRulesJobSpec
from scope_client.api_bindings.models.upsolve_token import UpsolveToken
from scope_client.api_bindings.models.user import User
from scope_client.api_bindings.models.user_credentials import UserCredentials
from scope_client.api_bindings.models.user_service_account_credentials import UserServiceAccountCredentials
from scope_client.api_bindings.models.user_sort import UserSort
from scope_client.api_bindings.models.user_type import UserType
from scope_client.api_bindings.models.validation_error import ValidationError
from scope_client.api_bindings.models.validation_error_loc_inner import ValidationErrorLocInner
from scope_client.api_bindings.models.webhook import Webhook
from scope_client.api_bindings.models.webhook_response import WebhookResponse
from scope_client.api_bindings.models.webhook_result import WebhookResult
from scope_client.api_bindings.models.webhook_sort import WebhookSort
from scope_client.api_bindings.models.workspace import Workspace
from scope_client.api_bindings.models.workspace_sort import WorkspaceSort
