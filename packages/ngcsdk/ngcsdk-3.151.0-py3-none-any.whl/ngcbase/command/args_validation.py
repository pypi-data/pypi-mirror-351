...
def check_semver(value):  # noqa: D103
    try:
        semver.VersionInfo.parse(str(value)).to_dict()
    except ValueError:
        raise argparse.ArgumentTypeError("Must be a valid sementic version: '{}'.".format(value)) from None
    return value
...
def check_key_value_pattern(value: str, lower_bound_key_length: int = 1) -> str:
    ...
    pattern = rf"^[a-zA-Z\d_\.-]{{{lower_bound_key_length},63}}:.*"
    if not isinstance(value, str):
