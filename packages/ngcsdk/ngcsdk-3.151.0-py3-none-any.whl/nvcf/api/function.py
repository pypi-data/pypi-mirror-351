...
import json
import re
from typing import Any, Callable, Dict, Generator, Optional, TYPE_CHECKING, Union

from nvcf.api.authorization import FunctionAuthorizationAPI
from nvcf.api.deploy import DeployAPI
from nvcf.api.instance import InstanceAPI
...
from ngcbase.constants import PRODUCTION_ENV, SCOPED_KEY_PREFIX
from ngcbase.environ import NGC_CLI_API_KEY, NVCF_SAK
from ngcbase.errors import InvalidArgumentError, NgcException
from ngcbase.util.utils import extra_args, get_environ_tag, parse_key_value_pairs
...
class FunctionAPI:  # noqa: D101
    ...
    def create(
        ...
        *,
        ...
        if helm_chart and models and get_environ_tag() == PRODUCTION_ENV:
            raise InvalidArgumentError("Can not include helm chart with models")
        ...
        if logs_telemetry_id or metrics_telemetry_id:
            payload["telemetries"] = {"logsTelemetryId": logs_telemetry_id, "metricsTelemetryId": metrics_telemetry_id}
    ...
    def invoke(
        ...
        """  # noqa: D205
        keys = [starfleet_api_key, self.config.app_key, NVCF_SAK, NGC_CLI_API_KEY]
        for key in keys:
            if key and key.startswith(SCOPED_KEY_PREFIX):
                starfleet_api_key = key
                break
