...
from .ImageOwnerDetails import ImageOwnerDetails

# Unused import over optimization prevention
str(repr(PaginationInfo))
str(repr(ImageOwnerDetails))
...
class RepositoryTagsOwnersList(object):
    def __init__(self, propDict=None):
        fromDict = propDict is not None and isinstance(propDict, dict)
        self.imageOwnersList = (
            None if not fromDict or propDict.get(
                "imageOwnersList"
            ) is None
            else [ ImageOwnerDetails(i)
            for i in propDict["imageOwnersList"] ])
        self.paginationInfo = (
            None if not fromDict or propDict.get(
                "paginationInfo"
            ) is None
    ...
    @property
    def imageOwnersList(self):
        """list: list of image owner details in given repository"""
        return self._imageOwnersList

    @imageOwnersList.setter
    def imageOwnersList(self, value):
        if value is not None and not isinstance(value, list):
            value = list(value)
        if (value is None) or isinstance(value, list):
            self._imageOwnersList = value
        else:
            raise TypeError(
                'Property "imageOwnersList" of type "list<ImageOwnerDetails>"'
                ' set to wrong type'
            )
    ...
    def toDict(self):
        result = {}
        if self._imageOwnersList:
            result["imageOwnersList"] = [
                i.toDict() for i in self._imageOwnersList 
            ] if self._imageOwnersList else []
    ...
    def isValid(self):
        if (self._imageOwnersList is not None):
            for obj in self._imageOwnersList:
