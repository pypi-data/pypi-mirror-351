...
class ImagePrinter(NVPrettyPrint):
    ...
    def print_image_details(  # noqa: D102
        self, repo_details, version_details, scan_details, arch_details, show_layers=False, show_scan=False
    ):
        if self.format_type == "json":
                ...
                GeneralWrapper.from_dict(
                    {
                        ...
                        **version_details.toDict(),
                        ...
                        "architectureDetails": arch_details,
                    }
                )
            )
        else:
            tag_info_list = [img for img in repo_details.images if img.tag == version_details.tag]
            if not tag_info_list:
                self.print_error(f"No information for tag '{version_details.tag}'.")
                return
            tag_info = tag_info_list[0]
            # This can be None, so default to displaying False
            tag_signed = tag_info.isSigned or False
            ...
            for arch in arch_details:
                ...
                tbl.add_label_line("Image Size", str(human_size(arch["compressedSize"])), level=1)
            ...
            tbl.add_label_line("Last Updated", format_list_view_date(tag_info.updatedDate))
            tbl.add_label_line("Malware Scan Status", tag_info.malwareScanStatus)
            tbl.add_label_line("Malware Scan Date", tag_info.malwareScanDate)
    ...
    def print_image_list(self, image_list, columns=None):
        ...
        if self.format_type == "json":
            ...
            if not columns:
                columns = [("tag", "Tag"), ("updated", "Updated Date"), ("size", "Image Size"), ("signed", "Signed?")]
