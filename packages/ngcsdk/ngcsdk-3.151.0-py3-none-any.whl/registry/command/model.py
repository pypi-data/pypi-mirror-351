...
class ModelSubCommand(RegistryCommand):  # noqa: D101
    ...
    def list(self, args):
        ...
        model_list = self.model_api.list(
            args.target, mrt.org, mrt.team, args.sort, args.access_type, product_names_args, args.signed
        )
    ...
    def download_version(self, args):
        ...
        license_terms = self.model_api.download_version(
            target=args.target, destination=args.dest, file_patterns=args.file, exclude_patterns=args.exclude
        )
        if license_terms:
            self.model_printer.print_ok("This model has the following license(s):")
            for license in license_terms:
                self.model_printer.print_ok(
    ...
    def upload_version(self, args):
        ...
        # print statement will add additional output for progress_bar,
        # thus print statement after progress_bar close
        ver_resp = self.model_api.info(args.target)
        self.transfer_printer.print_async_upload_transfer_summary(
            "model", *upload_res, version_status=ver_resp.modelVersion.status
        )
    ...
    def update_license_terms(self, args):  # noqa: D102
        license_terms_specs = validate_parse_license_terms(args)
        self.model_api.update_license_terms(args.target, license_terms_specs, args.clear)
        if bool(args.clear):
            self.model_printer.print_ok("Successfully cleared license terms for '{}'.".format(args.target))
        else:
            self.model_printer.print_ok("Successfully updated license terms for '{}'.".format(args.target))
