...
class TimeSeriesClient:
    ...
    def _query_api_v2(self, object_ids, metric, frequency, start_date, end_date):
        """
        Query data using the new API (without pagination).

        Args:
            object_ids (dict): Object IDs grouped by market.
            metric (list[str] | dict): Metrics to query.
...
def _extract_metrics_from_query(metric_query, available_df):
    ...
    metric_names = list(set(metric_names)) + ["Date", "ObjectId"]
