...
class ExecutableDirectory:
    ...
    def __init__(self, dirname, runsh="run.sh", _execution_resources=None) -> None:
    ...
    def run(self, queue=None, **kwargs):
        ...
        if queue is None:
            ...
            subprocess.call(["bash", self.runsh], cwd=self.dirname)
        else:
    ...
    def pmws(
        ...
        with open(os.path.join(self.dirname, ".manifest.json"), "r") as f:
...
def slurm_mps(exec_dirs, **kwargs):
    ...
    sl.submit([ed.dirname for ed in exec_dirs], nvidia_mps=True)
    return sl
    ...
    for version in app_versions:
        ...
        if (
            ...
            try:
                ...
                logger.error(
                    f"Failed to parse manifest for app {manifest['name']} version {version} due to error:\n{traceback.format_exc()}"
                )
                continue
        ...
        app_info[version] = {
            ...
            "main_function": {
                "args": [],
                "docs": ["Main function not implemented. See other methods!"],
            },
            "tests": {},
            "new_mode": new_mode,
        }
        try:
            for func in manifest["functions"]:
                func = deepcopy(func)
                func["name"] = func["function"].split(".")[-1]
                func["args"] = _args_from_manifest(func["params"])
                func["docs"] = _docs_from_manifest(
                    func, appname, app_versions[version]["appdir"]
                )
                app_info[version]["functions"].append(func)
                if func["name"] == "main":
                    app_info[version]["main_function"] = func

                if "tests" in func:
                    for test in func["tests"]:
                        app_info[version]["tests"][test] = func["tests"][test]
                        app_info[version]["tests"][test]["function"] = func["name"]
        except Exception:
            import traceback
...
def _set_root(root_dir):
    ...
    if os.path.exists(dsdir) and os.path.exists(dsjson):
        ...
        playmolecule.datasets = _Artifacts(manifest, files)
