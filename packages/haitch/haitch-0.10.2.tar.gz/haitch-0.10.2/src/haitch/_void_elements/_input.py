...
def input(
    ...
    **attrs: Unpack[InputAttrs],
    ...
    The input element enables interactive controls for web-based forms in order to
...
class InputAttrs(GlobalAttrs):
    ...
    This value is submitted with the form as part of a name/value pair.
    ...
    included in the submitted data if they are currently checked. If they are, the
    name and the value(s) of the checked controls are submitted.

    For example, if a checkbox whose name is fruit has a value of cherry, and the
    checkbox is checked, the form data submitted will include fruit=cherry. If the
    checkbox isn't active, it isn't listed in the form data at all. The default value
    ...
    Note: Although not required by the specification, Firefox will by default persist
    the dynamic disabled state of an `<input>` across page loads. Use the autocomplete
    ...
    Note: In addition to the actual names of tabs, windows, or inline frames, there
    are a few special keywords that can be used: `_self`, `_blank`, `_parent`, `_top`.
    ...
    Valid types: all except `hidden`, `password`, `checkbox`, `radio` and buttons
    ...
    Valid types: `date`, `month`, `week`, `time`, `datetime-local`, `number`, `range`
    ...
    """Provides a brief hint to the user as to what kind of info is expected in the field.

    Note: The placeholder attribute is not as semantically useful as other ways to
    explain your form, and can cause unexpected technical issues with your content.

    Valid types: `text`, `search`, `url`, `tel`, `email`, `password` and `number`
    ...
    Valid types: all except `hidden`, `range`, `color`, `checkbox`, `radio` and buttons
    ...
    Valid types: `date`, `month`, `week`, `time`, `datetime-local`, `number` and `range`
    ...
    Note: The value attribute is always optional, though should be considered mandatory
