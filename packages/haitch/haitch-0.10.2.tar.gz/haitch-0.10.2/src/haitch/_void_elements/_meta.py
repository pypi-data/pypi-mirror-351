...
def meta(
    ...
    **attrs: Unpack[MetaAttrs],
    ...
    """Represents metadata that cannot be represented by other meta-related elements.

    Other meta-related elements: `<base>`, `<link>`, `<script>`, `<style>`, `<title>`

    Note: the attribute `name` has a specific meaning for the `<meta>` element, and
    the `itemprop` attribute must not be set on the same `<meta>` element that has
...
class MetaAttrs(GlobalAttrs):
    ...
    the string "utf-8", because UTF-8 is the only valid encoding for HTML5 documents.
    `<meta>` elements which declare a character encoding must be located entirely
    ...
    The attribute is named `http-equiv(alent)` because all the allowed values are names
    of particular HTTP headers.
    ...
    Often used together with `content` where the `name` attribute gives the metadata
    name, and the `content` attributes gives the value.
