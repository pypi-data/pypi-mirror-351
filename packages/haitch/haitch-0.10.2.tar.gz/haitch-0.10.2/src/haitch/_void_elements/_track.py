...
def track(
    ...
    **attrs: Unpack[TrackAttrs],
    ...
    """Embeds timed text tracks, for example to automatically handle subtitles.

    The tracks are formatted in Web Video Text Tracks (WebVTT) format (`.vtt` files).
...
class TrackAttrs(GlobalAttrs):
    ...
    Note: must be a valid URL. This attribute must be specified and its URL value must
    have the same origin as the document - unless the `<audio>` or `<video>` parent
    ...
    If omitted, the default kind is `subtitles`. If the value is invalid, `metadata`
    will be used.
    ...
    """A user-readable title used by the browser when listing available text tracks.
