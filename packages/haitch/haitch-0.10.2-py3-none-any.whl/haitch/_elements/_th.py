...
def th(
    ...
    **attrs: Unpack[ThAttrs],
    ...
    The exact nature of this group is defined by the `scope` and `headers` attribute.
...
class ThAttrs(GlobalAttrs):
    ...
    It is provided as an alternative label to use for the header cell when referencing
    the cell in other contexts. Some user-agents, such as speech readers, may present
    this description before the content itself.
    ...
    If not provided, the default value is `1`. if its value is set to 0, the header
    cell will extends to the end of the table grouping section (<thead>, <tbody>,
    <tfoot>, even if implicitly defined), that the <th> belongs to. Values higher
    than 65534 are clipped at 65534.
    ...
    `rowgroup`, or `colgroup`, then browsers automatically select the set of cells
    to which the header cell applies.
