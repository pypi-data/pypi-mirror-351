...
def style(
    ...
    **attrs: Unpack[StyleAttrs],
    ...
    In general, it is better to put your styles in external stylesheets and apply
    them using `<link>` elements.

    Note: if you include multiple `<style>` and `<link>` elements in your document, they
    will be applied to the DOM in the order they are included in the document — make
    sure you include them in the correct order, to avoid unexpected cascade issues.
