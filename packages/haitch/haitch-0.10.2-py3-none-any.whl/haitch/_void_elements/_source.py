...
def source(
    ...
    **attrs: Unpack[SourceAttrs],
    ...
    This element is commonly used to offer the same media content in multiple file
...
class SourceAttrs(GlobalAttrs):
    ...
    Note: required for `<audio>` or `<video>`, but not allowed if parent is `<picture>`.
    ...
    Note: required if the parent of `<source>` is `<picture>`, but not allowed if parent
    is `<audio>` or `<video>`.
    ...
    """Specifies a list of source sizes describing the final rendered width of the image.
