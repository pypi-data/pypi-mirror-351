...
class RS485_slave_server_class:
    def __init__(self,hw_channal=None,baudrate=None,timeout=1):
        
        self.hw_channal=hw_channal
        self.baudrate=baudrate
        self.slave_dict={}
        self.slave_states={}
        self.active_slave_dict={}
        self.server=None
    ...
    def add_slave(self,slave_id,slave_context):
        self.slave_dict[slave_id] = slave_context
        self.slave_states[slave_id] = True # True means slave is active, False means slave is inactive
        ###all slaves are active by default

    ### function to update slaves context, to be run every time coltext is updated
    def run_server_function(self):
        while(1):
            if(self.stop_server_flag == 0):
                """Update the state of slaves in the server"""
                self.active_slave_dict={}

                # Update the active slave dictionary based on the states of slaves
                for s_id in self.slave_dict.keys():
                    if self.slave_states[s_id]:
                        self.active_slave_dict[s_id] = self.slave_dict[s_id]
                    
                
                # Create a Modbus server context with the active slaves            
                self.server= StartSerialServer(context=ModbusServerContext(slaves=self.slave_dict,single=False),
                                                defer_start=False,framer=ModbusRtuFramer, port=self.hw_channal,baudrate=self.baudrate,
                                                timeout=self.timeout)
            
            sleep(0.1) # sleep for 100ms to avoid busy waiting
    
    server_thread = Thread(target=run_server_function, daemon=True)


    def set_slave_state(self,slave_id,state):
        """Set the state of a slave"""
        if slave_id in self.slave_states.keys():
            self.slave_states[slave_id] = state            
        else:
            raise ValueError(f"Slave ID {slave_id} does not exist.")     
        self.start_server() # restart the server to apply the changes in slave states
           
    def start_server(self):
        """Start the Modbus server"""
        self.stop_server_flag=1
        sleep(1) # wait for 1 second to ensure the server is stopped before starting it again
        ...
        self.stop_server_flag=0
        try:
            self.server_thread = Thread(target=self.run_server_function, daemon=True)
    ...
    def start_all_slaves(self):
        """Start all slaves in the server"""
        for slave_id in self.slave_dict.keys():
            self.slave_states[slave_id] = True
        self.start_server() # restart the server to apply the changes in slave states
        
    
    def stop_all_slaves(self):
        """Stop all slaves in the server"""
        for slave_id in self.slave_dict.keys():
            self.slave_states[slave_id] = False
        self.start_server() # restart the server to apply the changes in slave states
...
def ServerStop():  # pylint: disable=invalid-name
    """Terminate server."""
    print("in stop server function")
    while(1):
        # print(_serverList._servers)
        if my_job := _serverList.get_server():
            if my_job.loop.is_running():
                asyncio.run_coroutine_threadsafe(my_job.async_await_stop(), my_job.loop)
                sleep(0.1)
                # print("Stopping server: "+str(_serverList._servers[-1]))
            else:
                print("Stopped server: "+str(_serverList._servers[-1]))
                _serverList._servers.pop()
        else:
