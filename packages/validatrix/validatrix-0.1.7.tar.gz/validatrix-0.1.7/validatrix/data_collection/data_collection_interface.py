...
class DataCollectionInterface:
    ...
    def register_for_data_collection(self,data_dictionary,csv_log_flag=False):
        for c in data_dictionary.keys():
            self.all_data_dict[c] = -1
            if(csv_log_flag):
                self.csv_log_column_list.append(c)

    ### funcion to update global data dictionary from any logger module
    def update_data(self,input_data_dict):
        with self.data_lock:
            for k in input_data_dict.keys():
                if(k in self.all_data_dict.keys()):
                    self.all_data_dict[k]=input_data_dict[k]
