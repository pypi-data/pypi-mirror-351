...
class AutomatedTestInterface:
    ...
    def add_to_test_log(self, log_message):
        print("Adding to test log:" + str(log_message))
        if self.data_interface_obj:
            self.data_interface_obj.print_with_log(log_message)
        else:
            print("No data interface object found. Logging to console only.")
        ctime = datetime.datetime.now()
        dt_str = ctime.strftime("%Y-%m-%d_%H:%M:%S.%f,")
        in_str = str(log_message)
        self.test_log += f"{dt_str} {in_str}\n"
