...
class ServiceDiscovery:
        ...
        try:
            ...
            for service_name in services:
                ...
                for node in service_nodes:
                    ...
                    # Build the URI based on service type
                    address = node.get("ServiceAddress", "")
                    port = node.get("ServicePort", 0)

                    if address and port:
                        # For services, don't include base route
                        if "SERVICE" in tags:
                            uri = f"http://{address}:{port}"
                        # For workers and indexers, include base route
                        elif ServiceType.WORKER.value in tags or ServiceType.INDEXER.value in tags:
    ...
    async def get_services(self) -> dict[str, list[str]]:
        ...
        Returns:
            ...
            Includes all service types (SERVICE, WORKER, INDEXER).
            SERVICE types return base URIs without routes.
            WORKER/INDEXER types include their base routes.
        """
