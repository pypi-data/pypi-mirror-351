...
To run:
    ...
    # Run the application
    uvicorn integration-test-server.main:app --reload
"""
...
async def self_discover():
    ...
    try:
        ...
        # Check which services are discoverable
        # Note: With the new design, the application is registered as a single service
        # using the SERVICE_NAME config, not individual services
        discovered = {}

        # Also check workers/indexers which are still registered via decorators
        worker_indexer_services = ["pdf-processor", "document-indexer"]

        for service_name in list(all_services.keys()) + worker_indexer_services:
            uri = await discovery.get_service_uri(service_name)
            if uri or service_name in all_services:
