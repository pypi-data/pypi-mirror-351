...
async def create_consul_lifespan(
    ...
    - Application service registration if service_name is configured
    - Worker/Indexer registration based on decorators
    ...
    # Track what we registered for cleanup
    app_registration: ApplicationRegistration | None = None
    app_name: str | None = None

    # Handle automatic application registration if service_name is configured
    if config.service_name:
        try:
            app_registration = ApplicationRegistration(config)
            await app_registration.register_application(config.service_name)
            app_name = config.service_name
            logger.info(f"Automatically registered application '{config.service_name}' with Consul")
        except Exception as e:
            logger.error(f"Error during application registration: {e}", exc_info=True)
            # Don't fail startup due to registration errors
    elif config.enable_registration:
        logger.warning(
            "Service registration is enabled but no service name is configured. "
            "To register this application, add 'SERVICE_NAME' to your configuration. "
            "If you only need workers/indexers, you can ignore this warning."
        )

    # Handle worker/indexer registrations
    registration_service = ConsulRegistrationService(config)

    try:
        await registration_service.register_services()
    except Exception as e:
        logger.error(f"Error during worker/indexer registration: {e}", exc_info=True)
        # Don't fail startup due to registration errors
    ...
    try:
        ...
        # Deregister application if we registered one
        if app_registration and app_name:
            try:
                await app_registration.deregister_application(app_name)
                logger.info(f"Deregistered application '{app_name}' from Consul")
            except Exception as e:
                logger.error(f"Error during application deregistration: {e}", exc_info=True)
            finally:
                await app_registration.close()

        # Deregister workers/indexers
        try:
            await registration_service.deregister_services()
        except Exception as e:
            logger.error(f"Error during worker/indexer deregistration: {e}", exc_info=True)
