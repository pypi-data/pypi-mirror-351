...
class TestServiceDiscovery:
    ...
    @patch("service_discovery.service_discovery.Consul")
    async def test_get_services_all_types(self, mock_consul_class):
        """Test getting services with different types (SERVICE, WORKER, INDEXER)."""
        # Mock Consul client
        mock_consul = AsyncMock()
        mock_consul_class.return_value = mock_consul

        # Mock service list
        mock_consul.catalog.services = AsyncMock(
            return_value=(None, {"user-api": [], "worker-service": [], "indexer-service": []})
        )
        ...
        user_api_nodes = [
            {
                ...
                "ServicePort": 8080,
                "ServiceTags": ["SERVICE"],
                "ServiceMeta": {},  # No base_route for SERVICE type
            },
            {
        ...
        indexer_nodes = [
            {
                ...
                "ServicePort": 8082,
                "ServiceTags": [ServiceType.INDEXER.value],
                "ServiceMeta": {"base_route": "/api/indexers/v1"},
            }
        ]
        ...
        # Should include SERVICE type without base route
        assert "user-api" in services
        assert len(services["user-api"]) == 2
        assert "http://10.0.0.1:8080" in services["user-api"]
        assert "http://10.0.0.2:8080" in services["user-api"]

        # Should include WORKER type with base route
        assert "worker-service" in services
        assert "http://10.0.0.3:8081/api/workers/v1" in services["worker-service"]

        # Should include INDEXER type with base route
        assert "indexer-service" in services
        assert "http://10.0.0.4:8082/api/indexers/v1" in services["indexer-service"]
    ...
    async def test_get_service_uri_single(self, mock_consul_class):
        ...
        assert uri == "http://10.0.0.1:8080"  # SERVICE types don't include base route
    ...
    async def test_get_all_service_uris(self, mock_consul_class):
        ...
        assert len(uris) == 2
        assert "http://10.0.0.1:8080" in uris  # SERVICE types don't include base route
        assert "http://10.0.0.2:8080" in uris  # SERVICE types don't include base route
