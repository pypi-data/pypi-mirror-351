...
class FeaturesSchema:
    ...
    def from_proto(proto: pb.FeaturesSchema) -> "FeaturesSchema":
        ...
        feature_cols = [
            FeatureColumn.from_proto(feature_col)
            for feature_col in proto.feature_cols
        ]
    ...
    def from_proto(source_primary_data):
        ...
        if primary_type == LocationType.TABLE:
            ...
            if source_primary_data.catalog.catalog_type == pb.GLUE:
                ...
                raise Exception(f"Unknown catalog type '{source_primary_data.catalog.catalog_type}'")
        elif primary_type == LocationType.KAFKA:
            location = KafkaTopic.from_proto(source_primary_data.kafka)
        else:
...
class SQLTransformation(Transformation):
    ...
    streaming_inputs: list = field(default_factory=list)
    ...
    def to_proto(self) -> pb.Transformation:
        ...
        for i, inp in enumerate(self.streaming_inputs):
            if hasattr(inp, "name_variant"):
                self.streaming_inputs[i] = inp.name_variant()
        ...
        streaming_nvs = []
        for inp in self.streaming_inputs:
            streaming_nvs.append(pb.NameVariant(name=inp[0], variant=inp[1]))
        ...
        transformation = pb.Transformation(
            SQLTransformation=pb.SQLTransformation(
                ...
                streaming_sources=streaming_nvs,
...
class FeatureView:
    name: str
    provider: str
    features: Optional[List[Tuple[str, str]]] = field(default_factory=list)
    entity: str = None
    ...
    @staticmethod
    def get(stub, name: str) -> "FeatureView":
    ...
    def _create(
        self, stub, engine: str, use_super_admin_delete_override: bool
    ) -> "FeatureView":
        fv = pb.FeatureView(
            name=self.name,
            provider=self.provider,
            features=(
                [
                    ...
                    for name, variant in self.features
                ]
                if len(self.features) > 0
                else []
            ),
