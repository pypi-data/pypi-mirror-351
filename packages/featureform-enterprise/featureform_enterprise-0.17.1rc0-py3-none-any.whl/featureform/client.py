...
from .proto import metadata_pb2 as pb, serving_pb2 as pb_serving
...
DEFAULT_MATERIALIZATION_ENGINE = "K8S"
...
class Client(ResourceClient, ServingClient):
    ...
    **Using the Client:**
    ...
    def get_kafka(self, name):
        """
        Get a Kafka provider. The returned object can be used to register additional Kafka resources.

        **Examples**:

        ``` py
        kafka = client.get_kafka("kafka")
        kafka_stream = spark.register_kafka_topic(
            name="kafka_stream",
            topic="transactions",
            description="Transaction stream from Kafka topic",
        )
        ```

        Args:
            name (str): Name of the Kafka provider to be retrieved.

        Returns:
            OfflineKafkaProvider: A provider for managing Kafka-related resources.
        """
        provider = self.__get_provider(name)
        config_bytes = provider.serialized_config
        spark_config = KafkaConfig.deserialize(config_bytes)
    ...
    def update_materialization(
        self, name: str, variant: str, port: Optional[str] = None
    ):
        parsedHost = urlparse(
            f"{'http://' if self._insecure else 'https://'}{self._host}"
        )
        url = f"{parsedHost.scheme}://{parsedHost.hostname}/data/materialization"
        res = requests.post(url, json={"name": name, "variant": variant})
        if res.status_code != 200:
            raise Exception(f"Failed to update materialization: {res.text}")

    def materialize_feature_view(
        self,
        view_name: str,
        inference_store: Union[str, OnlineProvider],
        features: Optional[List[Union[str, tuple, FeatureColumnResource]]] = None,
        description: Optional[str] = None,
        table_opts=None,
        materialization_opts=None,
        materialization_engine: Union[
            str, OfflineSparkProvider
        ] = DEFAULT_MATERIALIZATION_ENGINE,
        owner: Optional[Union[str, User]] = None,
        use_super_admin_delete_override: bool = False,
    ):
        """
        Materialize feature view to the inference store.
        ...
        Args:
            ...
            materialization_opts: Additional options for the materialization process.
        ...
        **Example:**
        ```py
        feature_view = client.materialize_feature_view(
            view_name="my_view",
        ...
        if isinstance(owner, User):
            ...
            owner = global_registrar.must_get_default_owner()

        feature_tuples = []
        for feature in features:
            if isinstance(feature, FeatureColumnResource):
                feature_tuples.append((feature.name, feature.variant))
            elif isinstance(feature, tuple):
                feature_tuples.append(feature)
            else:
                raise ValueError("Invalid feature type.")
        unique_features = {(name, variant): None for name, variant in feature_tuples}

        feature_view = FeatureView(
            name=view_name,
            provider=provider,
            features=[(n, v) for n, v in unique_features.keys()],
            description=description,
            owner=owner,
        )
        ...
        return feature_view._create(self._stub, engine, use_super_admin_delete_override)
    ...
    def get_feature_view(self, name):
        ...
        Returns:
            FeatureView: The feature view.
        """
        return FeatureView.get(self._stub, name)

    def serve_feature_view(
        self,
        view: str,
        entity_ids: List[str],
        filter_features: Optional[List[tuple]] = None,
    ):
        """
        Serve a feature view.

        Args:
            view (str): The name of the feature view to serve.
            entity_ids (List[str]): The entity IDs to serve.
            filter_features (Optional[List[tuple]]): A list of features to filter.

        Returns:
            FeatureView: The served feature view.
        """
        if not isinstance(view, str) or not view:
            raise ValueError("View name must be a non-empty string.")
        if len(entity_ids) == 0:
            raise ValueError("Entity IDs cannot be empty.")
        filter_feature_protos = []
        if filter_features:
            for feature in filter_features:
                if not isinstance(feature, tuple) or len(feature) != 2:
                    raise ValueError(
                        "Filter features must be tuples of (name, variant)."
                    )
                name, variant = feature
                filter_feature_protos.append(
                    pb_serving.FeatureID(name=name, version=variant)
                )

        req = pb_serving.FeatureViewServeRequest(
            view_name=view,
            entity_ids=entity_ids,
            filter_features=filter_feature_protos or [],
        )

        return self.impl._stub.ServeFeatureView(req)
