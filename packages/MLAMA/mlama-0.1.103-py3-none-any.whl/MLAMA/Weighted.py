...
def find_weighted_predictions(data, stan_optimal_weights, observed_column_name, models, metrics=["MAPE", "MSE"]):
    ...
    # for metric in metrics:
    #     theD_long['Algorithm'] = theD_long['Algorithm'].str.replace(f'_{metric}', '')

    # # Group data and calculate mean metric values by algorithm and adaptability
    # theDD = theD_long.groupby(['Algorithm', 'Adaptability']).agg({'Value': 'mean'}).reset_index()
