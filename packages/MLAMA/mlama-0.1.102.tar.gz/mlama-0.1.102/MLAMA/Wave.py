...
class Wave:
  ...
  def __init__(self, waveID, startDate, endDate,
    ...
    if trend_adjustment_steps is not None:
        self.max_shift = max(trend_adjustment_steps)
        self.sliding_room = datetime.timedelta(weeks=self.max_shift)
    else:
        self.max_shift = 0
        self.sliding_room = datetime.timedelta(0)
  ...
  def add_one_week(self, week_data):#for adding future  data to the current data
    ...
    Returns:
      self/nothing
    """
    # s = datetime.datetime.strptime(self.testStartDate,"%Y-%m-%d")##test start date shifted
  ...
  def get_wave_dates_with_shift_delay(self, shift, wave_start_shift_matrix):#JMM definition. get_wave_dates_with_shift_delay
    ...
    s = datetime.datetime.strptime(self.startDate,"%Y-%m-%d")#start fixed, irrespective of shift
    ...
    #start = datetime.datetime.strptime(self.trainStartDate,"%Y-%m-%d")#train start fixed, irrespective of shift
    #start_str = start.strftime("%Y-%m-%d")
    ...
    #s = datetime.datetime.strptime(self.testStartDate,"%Y-%m-%d")#test start date shifted
    w = datetime.timedelta(weeks=shift+1)
    start = e + w #test start date shifted
    start_str = start.strftime("%Y-%m-%d")
