...
def _get_nbconvert_path() -> Path:
    import shutil

    cmd = "jupyter-nbconvert"
    maybe_path = shutil.which(cmd)
    if maybe_path is None:
        raise FileNotFoundError(
            f"Command '{cmd}' not found in PATH. "
            "Please install nbconvert or ensure it is available in your PATH."
        )
    else:
        return Path(maybe_path).resolve(strict=True)
...
def _main(
    ...
    *,
    ...
    if cli:
        # fmt: off
        cmd = [
            _get_nbconvert_path().as_posix(), str(p0),
            "--to", "html",
            "--theme", theme,
            "--template", template,
            "--output", p.as_posix(),
...
def _version_callback(value: bool) -> None:
    if value:
        ...
        try:
            ...
            typer.echo(f"- nbconvert {nbconvert.__version__} (library)")

        import subprocess

        try:
            nbconvert_path = _get_nbconvert_path()
            cp = subprocess.run(
                [nbconvert_path.as_posix(), "--version"],
                text=True,
                capture_output=True,
            )
            ...
            typer.echo(f"- nbconvert {cp.stdout.strip()} (CLI) at {nbconvert_path.as_posix()}")
