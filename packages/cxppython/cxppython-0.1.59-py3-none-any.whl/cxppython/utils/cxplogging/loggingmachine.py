...
class LoggingMachine(StateMachine, Logger):
    ...
    def _extract_logging_config(self, config: "Config") -> dict:
        """Extract cxplogging's config from bittensor config

        Args:
            config (bittensor.core.config.Config): Bittensor config instance.

        Returns:
            (dict): cxplogging's config from Bittensor config or Bittensor config.
        """
    ...
    def enable_third_party_loggers(self):
        ...
        for logger in all_loggers():
            if logger.name in self._primary_loggers:
                continue
            print(f"enable_third_party_loggers:{logger.name}")
            queue_handler = QueueHandler(self._queue)
            logger.addHandler(queue_handler)
            logger.setLevel(self._logger.level)

    def enable_third_party_loggers_by_name(self,logger_name: List[str]):
        """Enables logging for third-party loggers by adding a queue handler to each."""
        for logger in all_loggers():
            if logger.name in logger_name:
                queue_handler = QueueHandler(self._queue)
                logger.addHandler(queue_handler)
                logger.setLevel(self._logger.level)
    ...
    def close_logging_by_default(self,logging):
        logging.getLogger().handlers.clear()

    def close_logging_by_name(self,logging,name):
    ...
    def add_args(cls, parser: argparse.ArgumentParser, prefix: str = None):
        ...
        try:
            default_logging_debug = os.getenv("CC_LOGGING_DEBUG") or False
            default_logging_info = os.getenv("CC_LOGGING_INFO") or False
            default_logging_trace = os.getenv("CC_LOGGING_TRACE") or False
            default_logging_record_log = os.getenv("CC_LOGGING_RECORD_LOG") or False
            default_logging_logging_dir = os.getenv(
                "CC_LOGGING_LOGGING_DIR"
            ) or os.path.join("~", ".cxppython", "logs")
            parser.add_argument(
                "--" + prefix_str + "logging.debug",
                action="store_true",
                help="""Turn on cxppython debugging information""",
                default=default_logging_debug,
            )
            parser.add_argument(
                "--" + prefix_str + "logging.trace",
                action="store_true",
                help="""Turn on cxppython trace level information""",
                default=default_logging_trace,
            )
            parser.add_argument(
                "--" + prefix_str + "logging.info",
                action="store_true",
                help="""Turn on cxppython info level information""",
