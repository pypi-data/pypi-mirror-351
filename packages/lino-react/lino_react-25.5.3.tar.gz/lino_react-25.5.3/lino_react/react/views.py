...
from lino.utils.html import assert_safe, Grouper
...
from lino.core.utils import navinfo, PhantomRow
...
TILE_TEMPLATE = mark_safe("""
<div style="margin: 20px;" class="p-card"><div class="p-card-body"><div class="p-card-content">
{chunk}
</div></div></div>
""")
    ...
    def get(self, request, app_label=None, actor=None):
        ...
        if fmt == constants.URL_FORMAT_JSON:
            ...
            if display_mode == constants.DISPLAY_MODE_STORY:
                ...
                html_text = mark_safe("")
                for obj in ar.sliced_data_iterator:
                    chunk = obj.as_tile(ar)
                    assert_safe(chunk)
                    html_text += TILE_TEMPLATE.format(chunk=chunk)
    ...
    def get(self, request):
        ...
        def getit():
            ...
            user_settings = dict(
                ...
                lang=u.language if not_anon else get_language(),
                site_data=settings.SITE.build_site_cache_url(
