'''Tests for time_since_received_from.py'''
...
def test_time_since_received_from() -> None:
    '''Test the time_since_sent_specific decider.'''
    distributed = DistributedConfig(
        clause={
            'decider': {
                'time_since_received_from': {
                    'time_seconds': 1.0,
                }
            },
        }
    ...
    decider: Decider = catalog.lookup_by_name('time_since_received_from')(
        config=distributed.decider.time_since_received_from)  # type: ignore[attr-defined] # pylint: disable=no-member
    ...
    neighbors[neighbor_id2].current_state = my_meta.current_state
    neighbors[neighbor_id3].current_state = my_meta.current_state
    # We should not send immediately after receiving...
