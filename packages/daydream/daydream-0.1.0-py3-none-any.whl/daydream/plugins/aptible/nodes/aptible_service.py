...
from daydream.knowledge import HasLogs, HasMetrics
from daydream.lib.log_anomalies import LogLine
from daydream.models import Metric
from daydream.plugins.aptible.logs import get_logs_for_time_range
...
class AptibleService(AptibleNode, HasLogs, HasMetrics):
    ...
    async def get_metric(
        ...
        metrics_data_by_container_id = {}
        for container in active_containers:
            container_metric = await container.get_metric(
                metric_name, time_range_start, time_range_end
            )

            if not isinstance(container_metric, Metric):
                continue

            metrics_data_by_container_id[container.raw_data["id"]] = container_metric.data

        return Metric(
            metric_name=metric_name,
            data=metrics_data_by_container_id,
        )
    ...
    async def get_logs(
        ...
        stack_name = await self._get_stack_name()
        if not stack_name:
            print(f"No stack name found for app {self.nid}. Unable to retrieve logs.")
            return []

        return await get_logs_for_time_range(
            stack=stack_name,
            node=self,
            containers=active_containers,
            start_dt=time_range_start,
            end_dt=time_range_end,
        )
