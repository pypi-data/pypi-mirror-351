...
from daydream.plugins.aptible.client import AptibleMetricsClient
...
class AptibleContainer(AptibleNode, HasMetrics, HasLogs):
    ...
    async def get_metric(
        ...
        metrics_data = await metrics_client.get_metrics(
            [container_id], metric_name, time_range_start, time_range_end
        )

        if not metrics_data:
            return "No metrics data found for the time range."
