...
from daydream.utils import generate_contrasting_colors, print, strip_secrets
...
class Graph:
    ...
    def digraph(self) -> nx.DiGraph:
        if not self._loaded:
            ...
            for _nid, node in self._digraph.nodes(data=True):
                ...
                node["data"] = NODE_REGISTRY[node_key].model_construct(
                    **node_data,
                    _graph=self,
                )
    ...
    async def get_node(self, nid: str) -> Node:
        ...
        if not node:
            raise LookupError(f"Node {nid} not found in graph")
    ...
    async def infer_edges(self) -> None:
        ...
        async for node in self.iter_nodes():
            for ref in await node.get_reference_identifiers():
                ...
                print(f"Inferred edge between {node.nid} and {related_node.nid}")
    ...
    async def save(self, path: Path | str) -> None:
        ...
        async with self._lock:
            ...
            cleaned_data = strip_secrets(data)
            path = Path(path)
            path.write_bytes(
                to_json(
                    {
