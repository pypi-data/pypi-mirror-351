...
class ConjugationFeatures(DataclassFeatureSet):
    """Características de conjugação verbal.
...
class Conjugation(enum.Enum):
    ...
    def from_verb(regular_verb_root: str) -> "Conjugation":
        ...
        :param regular_verb_root: verbo em sua forma
            normal (e.g., cantar, ler).

        :return: conjugação desse verbo regular (1ª, 2ª ou 3ª).
        """
    ...
    def from_desinencia(desinencia: str) -> "Conjugation":
        ...
        :param desinencia: desinência do verbo (e.g., ER, AR, IR).

        :return: Conjugação para essa desinência (1ª, 2ª ou 3ª).
        """
        try:
...
class ConjugationExtractor(FeatureExtractor):
    """Extrator de características de conjugação.

    :param nlp: modelo do spaCy para ser utilizado.
