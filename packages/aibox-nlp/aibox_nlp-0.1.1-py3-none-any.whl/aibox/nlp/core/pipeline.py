...
class Pipeline:
    ...
    def predict(self, X: TextArrayLike, **kwargs) -> np.ndarray:
        ...
        :param `**kwargs`: configurações estras para o estimador.
    ...
    def _batch_vectorize(self, X: TextArrayLike) -> list[np.ndarray]:
        """Método auxiliar que aplica uma estratégia
        de vetorização em batch.

        :param X: array-like de strings com formato (n_samples, <any>).

        :return: textos em sua versão vetorizada.
        """
        return [
