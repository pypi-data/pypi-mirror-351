...
class LSTMEstimator(Estimator):
    ...
    :param hidden_size: número de features no
        estado escondido h.
    :param num_layers: quantidade de camadas
        recorrentes (stacked LSTM).
    :param kind: se é uma rede de classificação
        ou regressão.
    :param random_state: seed.
    :param epochs: quantidade de épocas a ser utilizadas
        no momento do treinamento.
    :param bias: se a camada LSTM deve possuir bias
        ou não.
    :param dropout_prob: probabilidade de dropout. Se não
        zero, introduz uma camada de dropout para cada
        saída da LSTM.
    :param bidirectional: se a LSTM deve ser bidirecional.
    :param proj_size: se maior que 0, a LSTM utiliza
        projeções com esse tamanho.
    ...
    O modelo é construído na primeira chamada ao  método :py:meth:`fit`. A arquitetura consiste em:
    ...
    Esse estimador também permite que o shape de entrada
    (para os métodos :py:meth:`fit` e  :py:meth:`predict`) seja
    (`n_samples`, `variable_seq_size`, `n_features`).
    """
    ...
    def fit(self, X: ArrayLike, y: ArrayLike, **kwargs) -> None:
        ...
        assert len(X.size()) == 3, "Shape != (batch, sequence, features)"
    ...
    def _maybe_padded_sequence(self, X: ArrayLike) -> torch.Tensor:
        # Convertendo as características para um tensor
        maybe_variable_length = []
        for x in X:
            tensor = torch.tensor(x, device=self._device, dtype=self._dtype)

            # Talvez não seja uma sequência?
            if len(tensor.shape) < 2:
                # Shape entrada: (n_features,)
                # Shape de saída: (1, n_features)
                tensor = torch.unsqueeze(tensor, dim=0)

            maybe_variable_length.append(tensor)
