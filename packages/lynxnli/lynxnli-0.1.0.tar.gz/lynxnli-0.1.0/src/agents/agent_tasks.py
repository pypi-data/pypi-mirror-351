...
class TasksAgent(BaseAgent):
    ...
    def prompt(self, query:str) -> str:
        ...
        prompt =  f"""You are a LLM agent. You receive this request for 
        this task: {query}. Your job is find the most suiable tool
        from all possible tools and give the name and instructions of the 
        best tool back as a Tool object.
        {self.base_prompt}"""
    ...
    def validation(self, task:Task) -> bool:
        ...
        Only return False if this solution contains any error massage,
        otherwise return True in string."""
    ...
    def generate(self, agentState: AgentState) -> AgentState:
        # Initialize the tool executor to access available tools
        ...
        for index, task in enumerate(agentState["tasks"]): 
            ...
            tool_description = tool_executor.get_tool_description(task["description"])
            ...
            if tool["name"] not in tool_executor.get_tool_dict():
            ...
            tool_config = tool_executor.registry.get(tool["name"], {})
            ...
            executor = tool_executor.registry[tool["name"]]['executor']
