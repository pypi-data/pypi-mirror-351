...
if (ispin == 1):
   ...
   for i in range(nk):
       ...
       for j in range(nb):
           ...
           if (maior_n2 > menor_n2):
              GAP = (maior_n2 - menor_n2)
           if (maior_n2 < menor_n2):
              GAP = (menor_n2 - maior_n2)
              GAP = 0.0
   ...
   if (n_procar == 1):
      ...
      #---------------------------------------------------
      inform.write(f'Valence band maximum (VBM)    = {menor_n2} eV  -  k-point {kp1} \n')
      inform.write(f'Conduction band minimum (CBM) = {maior_n2} eV  -  k-point {kp2} \n')
      #---------------------------------------------------------------
      if (maior_n2 > menor_n2):
...
if (ispin == 2):
   for nspin in range(1,(2+1)):
      ...
      if (nspin == 1):
         VTemp = outcar.readline()
         VTemp = outcar.readline().split()
         if ((len(VTemp) >= 3) and (VTemp[0] == 'Fermi')): Efermi = float(VTemp[2])
         VTemp = outcar.readline()
         VTemp = outcar.readline()
         VTemp = outcar.readline()
         #------------------------------------------------------------------------------
