...
##  GemeindeInstall.sh
##  
##  Diese Datei wird dem User gegeben, damit er damit die Gemeinde-GUI auf einem Linux-Rechner
##  einfach installieren kann.
##
##  Als Anleitung werden ihm folgende Schritte nahegelegt:
##      1. Ordner/Verzeichnis für die GUI anlegen.
##      2. Das Skript GemeindeInstall.sh in dieses Verzeichnis speichern.
...
##                    d) Dort das Skript ausführen (./GemeindeInstall.sh)
...
chmod a+x GemeindeStart.sh GemeindeUpgrade.sh
