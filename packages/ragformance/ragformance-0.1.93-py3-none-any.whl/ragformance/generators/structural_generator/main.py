...
def get_section_size_size_comparison_questions(manual: Manual) -> list[dict[str, Any]]:
    ...
    id_longest_section = np.argmax(sections_length)
    ...
    number_sections_with_same_nb_docs_template = (
        "how many sections have {n} number of pages?"
    )
    ...
    for i in range(len(manual.sections[id_longest_section].pages_number)) + 1:
        id_sections_length_i = list(np.where(np.array(sections_length) == i)[0])
        ...
        query_set = format_query(
            ...
            ref_answer="\n".join(
                [
                    "-" + section.title
                    for section in np.array(manual.sections)[id_sections_length_i]
                ]
            ),
        )
...
def get_section_subsection_questions(manual: Manual) -> list[dict[str, Any]]:
    subsections_counts = [len(section.subsections) for section in manual.sections]
    if not subsections_counts:
        return []
    max_subsections = max(subsections_counts)
    idx_max = subsections_counts.index(max_subsections)
    question_max = "What is the title of the section the highest number of subsections?"
    qa_max = format_query(
        text=question_max,
        references=[
            {"corpus_id": manual.pages[page_number - 1]._id, "score": 1}
            for page_number in manual.sections[idx_max].pages_number
        ],
        ref_answer=manual.sections[idx_max].title.strip("#"),
    )

    questions_count = []
    unique_counts = sorted(set(subsections_counts))
    template_count = "how many sections have {n} number of subsections ?"
    for n in unique_counts:
        idx_sections = [i for i, count in enumerate(subsections_counts) if count == n]
        refs = [
            {"corpus_id": manual.pages[page_number - 1]._id, "score": 1}
            for i in idx_sections
            for page_number in manual.sections[i].pages_number
        ]
        qa = format_query(
            text=template_count.format(n=n),
            references=refs,
            ref_answer=str(len(idx_sections)),
        )
        questions_count.append(qa)
    return [qa_max] + questions_count
...
def run(folder_path: str, file_name: str = None, output_path: str = "output") -> None:
    ...
    if file_name is not None:
        ...
        queries += get_section_subsection_questions(manual)
    else:
