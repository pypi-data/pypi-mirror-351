...
class UnifiedAIProvider(BaseAIProvider):
    ...
    def __init__(self, config: AIConfig):
        ...
        self.max_tokens = 12000  # Always use 12000 tokens for responses
        self.timeout = 10
    ...
    def _generate_openai(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            # Handle potential list or None response from API
            content = response.choices[0].message.content
            if content is None:
                return None
            elif isinstance(content, list):
                # If content is a list, join it or take first element
                return (content[0] if content else "").strip() if isinstance(content[0], str) else str(content[0]).strip()
            else:
    ...
    def _try_ollama_api(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            # Calculate dynamic num_ctx based on prompt size
            prompt_tokens = self.token_manager.count_tokens(prompt)
            num_ctx = self.token_manager.calculate_num_ctx(prompt_tokens)
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "options": {
                        ...
                        "num_ctx": num_ctx,  # Dynamic context window based on prompt + 1000
                        "stop": ["\n\n", "```", "</commit>"]
                    }
                },
                timeout=30
    ...
    def _generate_grouping_openai_with_tokens(self, prompt: str, num_predict: int) -> Optional[str]:
        ...
        try:
            ...
            response = self._client.chat.completions.create(
                ...
                temperature=0.1,  # Lower temperature for more consistent JSON
                max_tokens=min(num_predict, 12000)  # Use full 12000 token limit
            )
