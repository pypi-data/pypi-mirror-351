    ...
    def run(self, args: Optional[list] = None):
        ...
        try:
            ...
            try:
                ...
                # Execute the actual squash
                generator = RebaseScriptGenerator()
                success, result = generator.execute_squash(groups)
                
                if not success:
                    raise Exception(f"Rebase failed: {result}")
                
                print_success("\n🎉 Successfully squashed commits!")
                print_colored(f"  Backup branch created: {result}", "gray")
