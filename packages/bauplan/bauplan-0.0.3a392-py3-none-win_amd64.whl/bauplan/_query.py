    ...
    def query_to_flight_stream(
        ...
        for log in log_stream:
            ...
            if event_type == 'job_completion':
                outcome_type = ev.job_completion.outcome.WhichOneof('outcome')
                if outcome_type == 'failure':
                    raise exceptions.BauplanQueryError(ev.job_completion.outcome.failure.error_message)
                if outcome_type == 'rejected':
                    raise exceptions.BauplanQueryError(
                        f'Query rejected: {ev.job_completion.outcome.rejected.reason}'
                    )
