    ...
    def evaluate(self, *args):
        try:
            ...
            six.reraise(
                ...
                TemplateExecutionError(self, exc_info),
                exc_info[2],
            )
...
class StringLiteral(_Element):
    ...
    def parse(self):
        ...
        # note: disabling the unescape logic here, for AWS parity (covered by tests) - TODO remove?
...
class NameOrCall(_Element):
    ...
    def parse(self):
        ...
        if not is_valid_vtl_identifier(self.name):
            raise NoMatch(f"Invalid VTL identifier '{self.name}'")
        try:
    ...
    def calculate(self, current_object, loader, top_namespace):
        ...
        if isinstance(result, _FunctionDefinition):
            ...
            if isinstance(result, DefineDefinition) and self.parameters and not params:
                # AWS parity: empty param lists printed verbatim (see `test_use_define_with_no_parameters`)
                result_value += self.parameters.my_text()
            return result_value
...
class FormalReference(_Element):
    ...
    def parse(self):
        ...
        # note: setting silent=True here for AWS parity (e.g., replacing $foo with empty string)
        self.silent = True
        try:
            self.expression = self.next_element(VariableExpression)
            self.calculate = self.expression.calculate
    ...
    def evaluate_raw(self, stream, namespace, loader):
        ...
        if is_string(value):
            ...
            stream.write(value_to_string(value))
...
class UnaryOperatorValue(_Element):
    ...
    def calculate(self, namespace, loader):
        calculated_value = self.value.calculate(namespace, loader)
        if calculated_value is None:
            return False
        return self.op(calculated_value)
...
class Expression(_Element):
    ...
    def calculate(self, namespace, loader):
        ...
        if len(valuestack) != 1:
            LOG.warning(
                "evaluation of expression in Condition.calculate "
...
class Condition(_Element):
    def parse(self):
        self.expression = self.next_element(ParenthesizedExpression)
        self.optional_match(WHITESPACE_TO_END_OF_LINE)

    def calculate(self, namespace, loader):
        result = self.expression.calculate(namespace, loader)
        if result is None:
            return True
        return bool(result)
