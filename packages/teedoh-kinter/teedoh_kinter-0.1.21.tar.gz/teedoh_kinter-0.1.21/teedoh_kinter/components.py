    ...
    def pack(self, just_children=False, grid=None):
        if self.grid_children:
            if self.max_columns is not None:
                ...
                self.total_rows = math.ciel(len(self.components) \ self.max_columns)
            ...
            if (
                ...
                if max_columns is not None:
                    for i in range(max_columns):
                        self.tkinter.columnconfigure(i, 1)
                else:
                    self.tkinter.columnconfigure(0, 1)
            if (
                self.fill['fill'] == 'both'
                or self.fill['fill'] == 'y'
            ):
                for i in range(self.total_rows):
                    self.tkinter.rowconfigure(i, 1)
