...
class THS:
    ...
    def security_bars(self, ths_code: str, start: int, end: int, adjust: str, interval: int) -> Response:
        ...
        """
        ths_code = ths_code.upper()
        if len(ths_code) != 10 or not ths_code.startswith(('USHA', 'USZA', 'USHP', 'USHT', 'USZP', 'USZT')):
            raise InvalidCodeError("Code must be 10 characters long and start with 'USHA' or 'USZA'")
    ...
    def stock_market_data(self, ths_code: Any) -> Response:
        ...
        if isinstance(ths_code, str):
            ...
            raise ValueError("ths_code 必须是str或者list")

        for code in ths_code:
            code = code.upper()
            if len(code) != 10 or not code.startswith(('USHA', 'USZA', 'USHP', 'USHT', 'USZP', 'USZT')):
                raise InvalidCodeError("必须10个字符组成代码，开头比如 'USHA' or 'USZA'")

        markets = {code[:4] for code in ths_code}
        if len(markets) > 1:
            raise ValueError("一次性查询多支股票必须市场代码相同")
