...
class HQ:
    ...
    def _initialize_library(self) -> None:
        ...
        try:
            ...
            raise THSAPIError(f"加载库 {self.__lib_path} 失败: {e}")
...
class THS:
    ...
    def query_data(self, req: str, query_type: str = "zhu", output_buffer: c.c_char_p = None,
        ...
        while attempt < max_attempts:
            ...
            if result_code == 0:
                ...
                logger.debug(f"查询执行: {req}, 类型: {query_type}")
                return response
            elif result_code == -1:
    ...
    def get_block_components(self, block_code: str) -> Response:
        ...
        """
        if not block_code:
            return error_response("Block code must be provided")
        req = (f"id=7&instance={self.share_instance}&zipversion={self.zip_version}"
    ...
    def get_transaction_data(self, ths_code: str, start: int, end: int) -> Response:
        ...
        if len(ths_code) != 10 or not any(ths_code.upper().startswith(market) for market in MARKETS):
            return error_response("必须10个字符组成代码，开头比如 'USHA'/'USZA'")
        if start >= end:
            return error_response("Start timestamp must be less than end timestamp")
