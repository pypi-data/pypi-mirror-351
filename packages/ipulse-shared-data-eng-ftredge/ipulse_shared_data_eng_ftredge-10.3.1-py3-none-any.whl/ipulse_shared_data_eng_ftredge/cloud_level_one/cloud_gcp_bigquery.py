...
def _handle_query_job_result(query_job: Union[bigquery.LoadJob, bigquery.QueryJob, bigquery.CopyJob, bigquery.ExtractJob],
    ...
    if pipelinemon:
        # Use q_unit=DataResource.DB_RECORD for proper record type indication
        pipelinemon.add_log(StructLog(
            ...
            q_unit=DataResource.DB_RECORD,  # Indicate the unit is database records
            ...
            if replace_if_exists:
                ...
                if pipelinemon:
                    ...
                    pipelinemon.add_log(StructLog(
                        ...
                        q_unit=DataResource.DB_RECORD  # Track by records, not tables
                    ))
            else:
                ...
                if pipelinemon:
                    pipelinemon.add_log(StructLog(
                        ...
                        q_unit=DataResource.DB_BIGQUERY_TABLE  # Table operation, not record operation
                    ))
...
def load_from_json_bigquery_table_extended(
    ...
    truncate_table_if_exists: bool = False,
    temp: bool = False,
    bigquery_client: Optional[bigquery.Client] = None,
    ...
    Args:
        ...
        truncate_table_if_exists: Whether to truncate table if it exosts, otherwise it will append (False to error)
        temp: Whether this is a temporary table
        bigquery_client: Optional pre-initialized BigQuery client
    ...
    try:
        ...
        result.add_metadata(
            ...
            overwrite_data_if_exists=truncate_table_if_exists,
            temp=temp
        )
        ...
        if table_exists:
            if truncate_table_if_exists:
                job_config.write_disposition = bigquery.WriteDisposition.WRITE_TRUNCATE
                result.add_state("CHOOSING OPTION To TRUNCATE EXISTING TABLE")
            else:
...
def merge_into_bigquery_table_extended(
    ...
    result.add_metadata(
        ...
        merge_type=merge_type.name if hasattr(merge_type, 'name') else str(merge_type),
        ...
        if records_to_be_merged_count == 0:
            ...
            if pipelinemon:
                pipelinemon.add_log(StructLog(
                    ...
                    q_unit=DataResource.DB_RECORD  # Use proper unit for records
                ))
...
def get_bigquery_table_schema(
    ...
    Returns:
        FunctionResult with table schema as a list of SchemaField objects
    """
    function_name = inspect.currentframe().f_code.co_name
    result = FunctionResult(function_name)
    result.add_metadata(
    ...
    try:
        ...
        # Get table
        table = bigquery_client.get_table(table_ref)
        result.data = table.schema
        result.add_state("SCHEMA_FETCHED")
