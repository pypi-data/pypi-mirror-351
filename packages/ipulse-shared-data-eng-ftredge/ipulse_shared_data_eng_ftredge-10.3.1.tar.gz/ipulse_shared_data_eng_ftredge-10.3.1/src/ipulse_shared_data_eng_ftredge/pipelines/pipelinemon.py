...
class Pipelinemon:
    ...
    def set_early_stop(self, reason: str, e: Exception = None):
    ...
    destinations: Optional[Union[Resource, List[Resource]]] = None  # New parameter
        ...
        if progress_statuses is not None:
            if isinstance(progress_statuses, (set, frozenset)):
                allowed_status_names = {ps.name for ps in progress_statuses if isinstance(ps, ProgressStatus)}
            elif isinstance(progress_statuses, list):
                allowed_status_names = {ps.name for ps in progress_statuses if isinstance(ps, ProgressStatus)}
            elif isinstance(progress_statuses, ProgressStatus):
                allowed_status_names = {progress_statuses.name}

        # Convert resource to set of names if needed
        allowed_resource_names = None
        if resources is not None:
            if isinstance(resources, list):
                allowed_resource_names = {res.name for res in resources if isinstance(res, Resource)}
            elif isinstance(resources, Resource):
                allowed_resource_names = {resources.name}

        # Convert sources to set of names if needed
        allowed_source_names = None
        if sources is not None:
            if isinstance(sources, list):
                allowed_source_names = {src.name for src in sources if isinstance(src, Resource)}
            elif isinstance(sources, Resource):
                allowed_source_names = {sources.name}

        # Convert destinations to set of names if needed
        allowed_destination_names = None
        if destinations is not None:
            if isinstance(destinations, list):
                allowed_destination_names = {dst.name for dst in destinations if isinstance(dst, Resource)}
            elif isinstance(destinations, Resource):
                allowed_destination_names = {destinations.name}
    ...
    def generate_execution_summary(self, countable_subj_name: str, total_countables: int, subj_resource:Resource=AbstractResource.PIPELINE_SUBJECT_SEQUENCE, final_level:Optional[LogLevel]=None) -> str:
        ...
        for event_tuple, count in self._by_event_count.items():
            ...
            for item in event_tuple:
                if item == subj_resource.name:
                    found_resource = True
                elif isinstance(item, str) and any(item == status.name for status in ProgressStatus):
                    found_status = item
        ...
        success = sum(event_counts[status.name] for status in ProgressStatus.success_statuses())
        done_with_notices = event_counts[ProgressStatus.DONE_WITH_NOTICES.name]
        done_with_warnings = event_counts[ProgressStatus.DONE_WITH_WARNINGS.name]
        failures = sum(event_counts[status.name] for status in ProgressStatus.failure_statuses())
        unfinished = event_counts[ProgressStatus.UNFINISHED.name]
        pending = sum(event_counts[status.name] for status in ProgressStatus.pending_statuses())
        skipped = sum(event_counts[status.name] for status in ProgressStatus.skipped_statuses())
        ...
        - FINISHED_WITH_ISSUES {subj_resource.name.lower()}(s): [{failures}/{total_countables}] {countable_subj_name}(s)
