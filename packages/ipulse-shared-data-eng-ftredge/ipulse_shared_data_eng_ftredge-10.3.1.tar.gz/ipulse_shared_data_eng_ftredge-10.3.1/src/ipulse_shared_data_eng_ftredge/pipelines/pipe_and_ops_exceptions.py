...
class PipelineDynamicIteratorEarlyTermination(Exception):
    """
    Exception raised for controlled iterator termination.
    ...
    def __init__(
        ...
        super().__init__(f"Pipeline Dynamic Iterator Early Termination raised in CONTEXT '{context}' : STEP '{step.name}': {reason}")
...
class PipelineSequenceTerminationError(Exception):
    ...
    def __init__(
        ...
        step.final(force_status=ProgressStatus.FAILED)
        self.already_logged = already_logged
        super().__init__(f"Exception in context {context} : STEP '{step.name}': {reason} . - Sequence Early Termination")
