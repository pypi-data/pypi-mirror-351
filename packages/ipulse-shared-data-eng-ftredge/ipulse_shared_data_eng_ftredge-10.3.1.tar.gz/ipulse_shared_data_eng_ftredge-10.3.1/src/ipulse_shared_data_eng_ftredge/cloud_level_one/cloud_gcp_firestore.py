...
from ipulse_shared_base_ftredge import (
    LogLevel, DataResource, Action, ProgressStatus, StructLog, log_info, log_warning, log_by_lvl
)
...
def _sanitize_firestore_data(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Sanitize data before sending to Firestore to prevent common errors.
    ...
    # Handle special values and nested structures
    for key, value in data.items():
       ...
       # Handle None values
        if value is None:
            sanitized[key] = None  # Preserve None values
        # Handle nested dictionaries recursively
        elif isinstance(value, dict):
            sanitized[key] = _sanitize_firestore_data(value)
        # Handle list/array values
        elif isinstance(value, list):
            sanitized_list = []
            for item in value:
                if isinstance(item, dict):
                    sanitized_list.append(_sanitize_firestore_data(item))
                else:
                    sanitized_list.append(item)
            sanitized[key] = sanitized_list
        # Handle datetime objects - KEEP AS DATETIME, don't convert to string
        elif isinstance(value, (datetime, date)):
            sanitized[key] = value  # Keep as native datetime for Firestore to handle
...
def create_or_merge_firestore_document_extended(
    ...
    try:
        ...
        try:
            sanitized_data = _sanitize_firestore_data(processed_data)
        ...
        result.add_state(f"DOCUMENT_{action.name}_COMPLETED")
        ...
        try:
            ...
            log_by_lvl(
                info_msg=f"Firestore {action.name} document {document_id} in collection {collection} completed",
