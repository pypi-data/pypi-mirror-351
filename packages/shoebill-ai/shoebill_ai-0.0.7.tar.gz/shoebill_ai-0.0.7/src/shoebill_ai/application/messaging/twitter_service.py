...
class TwitterService:
    ...
    async def get_home_timeline(self) -> list[dict[str,Any]]:
        request = TwitterHomeTimelineRequestMessage.create_message(
            ...
            max_results=100)
        tweets_message = await self.nats_publisher_adapter.request(request, timeout=5.0)
    ...
    async def quote_retweet(self, tweet_id: str, text: str):
        request = TwitterQuoteRetweetRequestMessage.create_message(tweet_id, text)
        success = await self.nats_publisher_adapter.request(request, timeout=5.0)
    ...
    async def reply_to_tweet(self, tweet_id: str, text: str):
    ...
    async def get_tweet_details(self, tweet_id: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            tweet_message = await self.nats_publisher_adapter.request(request, timeout=10.0)
    ...
    async def get_user_tweets(self, user_id: str, max_results: int = 5):
        request = TwitterUserTweetsRequestMessage.create_message(user_id, max_results)
        response = await self.nats_publisher_adapter.request(request, timeout=5.0)
        tweets_response = TwitterUserTweetsResponseMessage.from_hai_message(response)
        return tweets_response.tweets

    async def get_twitter_users_metadata(self, twitter_screen_names: List[str]) -> List[dict[str, str]]:
    ...
    async def get_twitter_user_metadata(self, twitter_screen_name: str) -> Optional[dict[str, str]]:
    ...
    async def follow_user(self, user_id: str):
        ...
        await self.nats_publisher_adapter.request(request)

    async def search_tweets(self, query: str, max_results: int = 10, min_views: int = 0):
