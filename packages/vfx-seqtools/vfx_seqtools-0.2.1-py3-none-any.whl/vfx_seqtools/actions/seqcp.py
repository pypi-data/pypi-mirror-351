...
def do_action(tupl: tuple) -> None:
    ...
    if is_dryrun:
        logger.info(f"dry-run: COPY {substituted_src} {substituted_dst}")
    else:
        if be_verbose:
            logger.info(f"COPY: {substituted_src} {substituted_dst}")
        try:
            if be_interactive:
                confirm = input(f"copy {substituted_src} -> {substituted_dst}? (y/n): ")
                if confirm.lower() != "y":
                    return
            shutil.copy2(substituted_src, substituted_dst)
        except Exception as e:
            if be_strict:
                raise
            logger.warning(
                f"Error copying file: {substituted_src} -> {substituted_dst}, skipping it. Use --strict to stop on errors. {e}"
            )
...
def seqcp(
    ...
    'seqcp file.####.exr newfile.####.exr -f 1-10' - copy frames 1-10 of file.####.exr to newfile, with 4-padded numbering like '0001'.

    frame offsets using '+' and '-' are supported.

    'seqcp file.@.exr newfile.@+10.exr -f 1-10' - copy frames 1-10 of file.@.exr to newfile, with a +10 frame number offset.
    """
    if frame_seq:
