...
def main(stdscr):
    ...
    while True:
        ...
        stdscr.addstr(0, 0, repr(curses.keyname(key)))
        stdscr.refresh()
        ...
        # --- Shift+Arrow selection ---
        elif key in (getattr(curses, "KEY_SLEFT", -1), 393):  # Shift+Left
            if not sel_mode:
                sel_mode = True
                sel_start = (y, x)
            # Move left
            if x > 0:
                ...
                x = len(text[y])
            sel_end = (y, x)
        elif key in (getattr(curses, "KEY_SRIGHT", -1), 402):  # Shift+Right
            if not sel_mode:
                sel_mode = True
                sel_start = (y, x)
            # Move right
            if x < len(text[y]):
                ...
                x = 0
            sel_end = (y, x)
        elif key in (getattr(curses, "KEY_SUP", -1), 391):  # Shift+Up
            if not sel_mode:
                sel_mode = True
                sel_start = (y, x)
            if y > 0:
                ...
                x = min(x, len(text[y]))
            sel_end = (y, x)
        elif key in (getattr(curses, "KEY_SDOWN", -1), 402):  # Shift+Down
            if not sel_mode:
                sel_mode = True
                sel_start = (y, x)
            if y < len(text) - 1:
