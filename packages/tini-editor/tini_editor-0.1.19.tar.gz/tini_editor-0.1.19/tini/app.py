...
class TiniEditor(App):
    ...
    BINDINGS = [
        ...
        Binding("tab", "insert_tab", "Insert Tab"),  # Add Tab binding
    ]
    ...
    def perform_goto(self, line_number):
        ...
        try:
            ...
            if 0 <= line_number < len(lines):
                ...
                self.text_area.cursor_position = index
                self.text_area.focus()  # Ensure the text area is focused after moving the cursor
            else:
    ...
    def action_insert_tab(self):
        # Insert a tab character at the current cursor position in the text area
        ta = self.text_area
        text = ta.text
        pos = ta.cursor_position
        ta.text = text[:pos] + "\t" + text[pos:]
        ta.cursor_position = pos + 1
