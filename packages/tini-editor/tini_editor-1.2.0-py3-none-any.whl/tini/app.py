...
def main(stdscr):
    ...
    while True:
        ...
        if key == 17:  # Ctrl+Q
            ...
            if x > 0:
                # Ctrl+Left: skip to previous word boundary
                if curses.keyname(key) == b'^[[1;5D' or (hasattr(curses, 'KEY_SLEFT') and key == curses.KEY_SLEFT) or (hasattr(curses, 'KEY_CTL_LEFT') and key == curses.KEY_CTL_LEFT):
                    # Move left to the previous word boundary
                    while x > 0 and text[y][x-1] in " \t.,;:!?()[]{}\"'":
                        x -= 1
                    while x > 0 and text[y][x-1] not in " \t.,;:!?()[]{}\"'":
                        x -= 1
                else:
                    x -= 1
            elif y > 0:
            ...
            if x < len(text[y]):
                # Ctrl+Right: skip to next word boundary
                if curses.keyname(key) == b'^[[1;5C' or (hasattr(curses, 'KEY_SRIGHT') and key == curses.KEY_SRIGHT) or (hasattr(curses, 'KEY_CTL_RIGHT') and key == curses.KEY_CTL_RIGHT):
                    # Move right to the next word boundary
                    while x < len(text[y]) and text[y][x] not in " \t.,;:!?()[]{}\"'":
                        x += 1
                    while x < len(text[y]) and text[y][x] in " \t.,;:!?()[]{}\"'":
                        x += 1
                else:
                    x += 1
            elif y < len(text) - 1:
