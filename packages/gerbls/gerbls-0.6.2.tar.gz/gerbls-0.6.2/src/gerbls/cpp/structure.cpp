...
std::unique_ptr<bool[]> DataContainer::find_flares(const double* mag0) {
    ...
    for (size_t i = 0; i < size; i++) {
        j = 0; 
        while ((i + j < size) && (log10(mag[i+j] / mag0[i+j]) >= 2 * LOG10E * err[i+j] / mag[i+j])) 
            j++;
        if (j >= 4) {
            k = i;
            while ((k > 0) && (mag[k-1] > mag0[k-1])) 
                k--;
            while ((i + j < size) && (mag[i+j] > mag0[i+j]))
                j++;
            for ( ; k < i + j; k++)
                mask[k] = true;
            i += j;
...
std::unique_ptr<DataContainer> DataContainer::phase_folded(double P_rot, double t_extend, int* save_indices) {
    ...
    std::sort(indices, indices + size,
              [&phases](int j, int k) {return (phases[j] < phases[k]);});
    
    // Number of data points to extend
    size_t j = 0, k = 0;
    while ((j < size) && (phases[indices[j]] < t_extend)) 
        j++;
    while ((k < size) && (phases[indices[size-k-1]] > P_rot - t_extend))
        k++;
...
void DataContainer::read_from_file(std::string filename, const std::vector<int>* sectors) {
    ...
    for (size_t i = 0; i < N_lines; ) {
        ...
        if (fin.eof()) {
            std::cout << "Something went wrong: end of data file reached before all data read in.\n";
            break;
        }
...
std::vector<double> DataContainer::running_median_eval(double hw, double* rjd_eval, size_t N_eval) {
    ...
    for (size_t k = 0; k < N_eval; k++) {
        while ((i < size) && (rjd[i] < rjd_eval[k] - hw))
            i++;
        while ((j < size) && (rjd[j] < rjd_eval[k] + hw))
            j++;
        if (i < j)
...
std::vector<double> DataContainer::running_median_per(double hw, double P_rot) {
    ...
    while (data->rjd[N_extend] < 0)
        N_extend++;
    std::vector<double> med_per = data->running_median(hw);
    for (size_t i = 0; i < size; i++)
        med[indices[i]] = med_per[i+N_extend];
    
    return med;

}

void DataContainer::set(double* _rjd, double* _mag, double* _err, size_t _size) {

    set(nullptr, _rjd, _mag, _err, _size);
    
}
void DataContainer::set(int* _sec, double* _rjd, double* _mag, double* _err, size_t _size) {
...
void DataContainer::store(double* _rjd, double* _mag, double* _err, size_t _size) {

    store(nullptr, _rjd, _mag, _err, _size);
    
}
// Allocate memory and copy a dataset
void DataContainer::store(int* _sec, double* _rjd, double* _mag, double* _err, size_t _size) {
    ...
    if (_sec == nullptr)
        ...
        std::copy_n(_sec, _size, sec);
    std::copy_n(_rjd, _size, rjd);
    std::copy_n(_mag, _size, mag);
    std::copy_n(_err, _size, err);
    
}

// Equivalent to store but only stores members where mask evaluates to false
void DataContainer::store_unmasked(double* _rjd, double* _mag, double* _err, bool* mask, size_t _size) {

    store_unmasked(nullptr, _rjd, _mag, _err, mask, _size);

}
void DataContainer::store_unmasked(int* _sec, double* _rjd, double* _mag, double* _err, bool* mask, size_t _size) {
