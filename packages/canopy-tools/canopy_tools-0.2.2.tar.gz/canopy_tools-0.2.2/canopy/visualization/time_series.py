...
def apply_legend_style(plot, legend_style, max_labels_per_col, move_legend):
    ...
    if legend_style == 'default':
        ...
        plot.legend(handlelength=0, handletextpad=0, labelcolor='linecolor', loc='best', frameon=False, fontsize=14, ncols=ncols)
        ...
        raise ValueError("Invalid legend_style. Choose from 'default', 'highlighted', or 'end-of-line'.")
    
    if move_legend is True:
            sns.move_legend(plot, "center left", bbox_to_anchor=(1, 0.5), ncols=ncols)

def plot_time_series(fig, ax, time_series, layers, reverse_hue_style, field_labels, colors, move_legend, legend_style, max_labels_per_col, rolling_size, no_style, **kwargs):
    """
    ...
    if len(time_series) > 1:
        ...
        time_series = combined_ts.reset_index().melt(id_vars=['time', 'DataFrame'], value_vars=layers, var_name='Column', value_name='Value')
        ...
        if isinstance(layers, str) or (isinstance(layers, list) and len(layers) == 1):  # Only colours if only one layer
            ...
            if reverse_hue_style is True:
                ax = sns.lineplot(data=time_series, x='time', y='Value', palette=colors, hue='Column', style=(None if no_style == True else 'DataFrame'), legend=bool(legend_style))
            else:
                ax = sns.lineplot(data=time_series, x='time', y='Value', palette=colors, hue='DataFrame', style='Column', legend=bool(legend_style))
...
def plot_stacked_time_series(ax, time_series, layers, colors, max_labels_per_col):
    """
...
def make_time_series(fields: cp.Field | List[cp.Field], output_file: Optional[str] = None, layers: Optional[List[str] | str] = None, redop: Optional[str] = 'av',
                     make_diff: Optional[bool] = False, yaxis_label: Optional[str] = None, field_labels: Optional[List[str]] = None, 
                     unit: Optional[str] = None, title: Optional[str] = None, palette: Optional[str] = None,
                     custom_palette: Optional[str] = None, move_legend: Optional[bool] = False, legend_style: Optional[str] = 'default',
                     reverse_hue_style: Optional[bool] = False, max_labels_per_col: Optional[int] = 15, baseline: Optional[bool] = False,
                     rolling_size: Optional[int] = None, stacked: Optional[bool] = False, relative: Optional[bool] = False,
                     dark_mode: Optional[bool] = False, transparent: Optional[bool] = False, no_style: Optional[bool] = False, 
                     x_fig: Optional[float] = 10, y_fig: Optional[float] = 10, return_fig: Optional[bool] = False, **kwargs):
    """
    ...
    legend_style : str, optional
        Style of the legend ('default', 'highlighted', 'end-of-line' or False).
    ...
    **kwargs
    ...
    fields_red = []
    for field in fields:
        fields_red.append(field.red_space(redop))
    ...
    if len(time_series) > 1:
        if legend_style != 'default' and no_style == False and len(layers)>1:
    ...
    if relative is True:
        ...
        if unit != "%":
            ...
            unit = "%"
        # Set y-axis limits to 0-100 for relative values
        ax.set_ylim(0,100)
    ...
    # Plot
    if stacked is False:
        fig, ax = plot_time_series(fig, ax, time_series, layers, reverse_hue_style, field_labels, colors, move_legend, legend_style, max_labels_per_col, rolling_size, no_style, **kwargs)
    else:
        ax = plot_stacked_time_series(ax, time_series, layers, colors, max_labels_per_col)
