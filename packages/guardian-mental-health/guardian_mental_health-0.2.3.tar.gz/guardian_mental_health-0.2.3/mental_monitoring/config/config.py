...
# Try to load environment variables for sensitive configuration
try:
...
MODEL_CONFIG = {
    ...
    "saved_model_path": os.path.join(MODEL_DIR, "checkpoints", "saved_model_fixed.pt"),  # Using fixed model
    "original_model_path": os.path.join(MODEL_DIR, "checkpoints", "saved_model.pt"),  # Original path for reference
    "default_checkpoint_dir": os.path.join(MODEL_DIR, "checkpoints")
}

# Discord bot configuration
DISCORD_CONFIG = {
    # Token should ONLY be set via environment variable for security
    "token": os.environ.get("DISCORD_BOT_TOKEN", None),  # Never hardcode tokens
    "command_prefix": "!",
    # Try to get parent channel ID from environment variable
    "parent_channel_id": os.environ.get("GUARDIAN_PARENT_CHANNEL", None),
    "risk_thresholds": {
...
DASHBOARD_CONFIG = {
    "port": int(os.environ.get("GUARDIAN_DASHBOARD_PORT", "8501")),
    "theme": "light",
    "data_path": os.path.join(DATA_DIR, "message_history.json"),
    "model_path": os.environ.get("GUARDIAN_MODEL_PATH", MODEL_CONFIG["saved_model_path"]),
...
logger.info("Guardian configuration loaded")
logger.info(f"Using model path: {MODEL_CONFIG['saved_model_path']}")
if DISCORD_CONFIG['token'] != "YOUR_DISCORD_BOT_TOKEN":
    logger.info("Discord bot token configured")
else:
    logger.warning("Discord bot token not set. Set DISCORD_BOT_TOKEN environment variable or update config.py")
