...
class ReplicateInput(BaseModel):
    """Input schema for Pixverse v4.5 Replicate model."""
    prompt: str = Field(..., description="Text prompt for video generation")
    image: Optional[HttpUrl | str] = Field(None, description="Image to use for the first frame of the video")
    last_frame_image: Optional[HttpUrl | str] = Field(None, description="Use to generate a video that transitions from the first image to the last image. Must be used with image.")
...
class ReplicateOutput(str):
    """Output schema for Pixverse v4.5 Replicate model - returns a URL string."""
    pass 