...
from .schema import RecraftV3TextToImageInput, RecraftV3ImageToImageInput


class FalTextToImageAdapter(IFalAdapter):
    """Adapter for Recraft V3 text-to-image generation using FAL."""
    ...
    def convert_input(self, data) -> dict:
        ...
        fal_input = RecraftV3TextToImageInput(
...
class FalImageToImageAdapter(IFalAdapter):
    """Adapter for Recraft V3 image-to-image generation using FAL."""
    
    def convert_input(self, data) -> dict:
        """Convert from Sunra's ImageToImageInput to FAL's input format."""
        input_model = ImageToImageInput.model_validate(data)
        
        # Create FAL input instance
        fal_input = RecraftV3ImageToImageInput(
            prompt=input_model.prompt,
            image_url=input_model.image,
            strength=input_model.image_strength,
            style=input_model.style,
            style_id=input_model.style_id,
        )
    ...
    def get_request_url(self) -> str:
        return "https://queue.fal.run/fal-ai/recraft/v3/image-to-image"
