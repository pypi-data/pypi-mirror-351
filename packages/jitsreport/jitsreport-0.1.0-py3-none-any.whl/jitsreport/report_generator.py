...
def generate_extreme_values_section(df, col):
    ...
    top_max_num = value_counts.nlargest(10, 'Value').round(2)
    top_min_num = value_counts.nsmallest(10, 'Value').round(2)
    ...
    top_max_freq = df[col].value_counts().nlargest(10).reset_index()
    top_min_freq = df[col].value_counts().nsmallest(10).reset_index()
    ...
    fig_num_max = px.pie(top_max_num, values='Count', names='Value', 
                        title=f'Top 10 <b>Numerical</b> Maximums: {col}',
                        color_discrete_sequence=px.colors.sequential.Reds_r)
    
    fig_num_min = px.pie(top_min_num, values='Count', names='Value', 
                        title=f'Top 10 <b>Numerical</b> Minimums: {col}',
                        color_discrete_sequence=px.colors.sequential.Blues_r)

    fig_freq_max = px.pie(top_max_freq, values='Count', names='Value', 
                         title=f'Top 10 <b>Most Frequent</b> Values: {col}',
                         color_discrete_sequence=px.colors.sequential.Oranges_r)

    fig_freq_min = px.pie(top_min_freq, values='Count', names='Value', 
                         title=f'Top 10 <b>Least Frequent</b> Values: {col}',
                         color_discrete_sequence=px.colors.sequential.Greens_r)

    # Update layout for all figures
    for fig in [fig_num_max, fig_num_min, fig_freq_max, fig_freq_min]:
        fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', 
                         width=350, height=350, showlegend=False)

    # Create HTML section with clear headings
    extreme_html = f"""
    <div style="margin: 20px 0; page-break-inside: avoid;">
        <h3>Extended Extreme Values Analysis: {col}</h3>
        
        <div style="display: flex; gap: 20px; flex-wrap: wrap;">
            <!-- Numerical Extremes -->
            <div style="flex: 1; min-width: 350px;">
                {fig_num_max.to_html(full_html=False)}
                <div style="margin-top: 10px;">
            ...
            <div style="flex: 1; min-width: 350px;">
                {fig_num_min.to_html(full_html=False)}
                <div style="margin-top: 10px;">
        ...
        <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-top: 30px;">
            <!-- Frequency Extremes -->
            <div style="flex: 1; min-width: 350px;">
                {fig_freq_max.to_html(full_html=False)}
                <div style="margin-top: 10px;">
            ...
            <div style="flex: 1; min-width: 350px;">
                {fig_freq_min.to_html(full_html=False)}
                <div style="margin-top: 10px;">
...
def generate_report_from_excel(excel_path, output_html="report.html", output_pdf="report.pdf"):
    ...
    <div style="page-break-before: always; margin-top: 40px;">
        ...
        <div style="text-align: center; margin: 40px 0; color: #666; font-size: 0.9em; border-top: 1px solid #ddd; padding-top: 20px;">
            © {pd.Timestamp.now().year} Surajit Das (mr.surajitdas@gmail.com) Analytics. All rights reserved.
        </div>
    </div>
    """
