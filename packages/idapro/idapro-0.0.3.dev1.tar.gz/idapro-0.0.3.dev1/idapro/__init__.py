...
def open_database(file_name:str, run_auto_analysis: bool, args: str = None)->int:
    """Opens the database specified by the file_name argument.
    If run_auto_analysis is set to True, the auto-analysis will start and wait for its completion.
    Optionally, additional arguments can be provided via args parameter, which follow the same format as IDA's command-line arguments.
    NOTE: All library functions must be called from the same thread that initialized the library.
    The library is single-threaded, and performing database operations from a different thread may result in undefined behavior."""
    return libida.open_database(file_name.encode(), run_auto_analysis, args.encode() if args is not None else None)
