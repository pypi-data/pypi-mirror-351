...
class ProjectService(AbstractService):
    ...
    def project_run_task(
        ...
        if not files_to_add and has_changes:
            print(f"[{ColorScheme.USEFUL_INFO.value}]Uncommitted changes detected, but the command is being run without adding these files. If you want to include these changes in the task, use the -fta flag (e.g., -fta .)[{ColorScheme.USEFUL_INFO.value}]")
    ...
    def project_run_inference_simulator(
        ...
        if not files_to_add and has_changes:
            print(f"[{ColorScheme.USEFUL_INFO.value}]Uncommitted changes detected, but the command is being run without adding these files. If you want to include these changes in the inference simulator, use the -fta flag (e.g., -fta .)[{ColorScheme.USEFUL_INFO.value}]")
    ...
    def checkout_project(
        ...
        if not files_to_add and has_changes:
            print(f"[{ColorScheme.WARNING.value}][WARNING] Uncommitted changes detected, cannot checkout with uncommitted changes. You should use the -fta flag (e.g., -fta .)[{ColorScheme.WARNING.value}]")
            raise typer.Exit(1)
