...
class DeepST_model(nn.Module):
    def __init__(
        ...
        for i, hidden_size in enumerate(linear_encoder_hidden):
            self.encoder.add_module(
                f"encoder_L{i}", self._build_network(current_dim, hidden_size)
            )
        ...
        for i, hidden_size in enumerate(linear_decoder_hidden):
            self.decoder.add_module(
                f"decoder_L{i}", self._build_network(current_dim, hidden_size)
            )
            current_dim = hidden_size

        # Final decoder layer
        self.decoder.add_module("decoder_out", nn.Linear(current_dim, self.input_dim))
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> tuple:
        ...
        q = 1.0 / (
            (
                1.0
                + torch.sum(torch.pow(z.unsqueeze(1) - self.cluster_layer, 2), 2)
                / self.alpha
            )
            + 1e-8
        )
