        ...
        if batch_key and batch_key in adata.obs.columns:
            ...
            for b in adata.obs[batch_key].unique():
                ...
                adj_2d = distance.cdist(
                    sub.obsm["spatial"], sub.obsm["spatial"], "euclidean"
                )
                refined = refine(
                    sub.obs_names.tolist(), sub.obs[key_added].tolist(), adj_2d, shape
                )
            ...
            refined = refine(
                adata.obs_names.tolist(), adata.obs[key_added].tolist(), adj_2d, shape
            )
