...
async def create_video_by_podcast(
    podcast_html_url: Optional[str] = None,
    async_mode: bool = False,
    output_path: str = DEFAULT_OUTPUT_PATH
):
    # API key validation
    ...
    # Parameter validation
    if not podcast_html_url:
        return {"error": "podcast_html_url must be provided"}
    
    # Prepare payload
    payload = {
        "podcast_url": podcast_html_url
    }
    
    # Create the video from podcast
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{api_base_url}/video/create_by_podcast",
                headers=get_auth_headers(),
                json=payload,
                timeout=30
            )
