...
class FacebookKeywordCollector:
    """
    A class to collect Facebook posts by keyword using cookie authentication.
    """

    def __init__(self, cookie, max_post_by_keyword=100, max_keyword_post_retry=3):
        """
    ...
    def _search_posts(self, keyword, max_posts=100):
        """
        Search for posts using keyword
    ...
    def _parse_performance_log(self, log_text):
        """
        Parse performance log text to extract post data.

        Args:
            log_text (str): The performance log text
        ...
        try:
            ...
            if 'data' in data and 'serpResponse' in data['data']:
                return data
        except Exception as e:
    ...
    def _get_response_body(self, response_text):
        ...
        data = data.get('serpResponse') if data.get('serpResponse') is not None else {}
        data = data.get('results') if data.get('results') is not None else {}
        data = data.get('edges') if data.get('edges') is not None else []

        if len(data) > 0:
            for item in data:
                item = item.get('relay_rendering_strategy') if item.get('relay_rendering_strategy') else {}
                item = item.get('view_model') if item.get('view_model') else {}
                item = item.get('click_model') if item.get('click_model') else None
    ...
    @staticmethod
    def _get_next_cursor_by_response_fb(body) -> dict:
        try:
            body = json.loads(body)
            resource_response_dict = _safe_get(body, 'data', 'serpResponse', 'results', 'page_info')
            cursor = {
