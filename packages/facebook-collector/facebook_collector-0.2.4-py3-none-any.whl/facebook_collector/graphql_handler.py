...
class FacebookGraphQLCollector:
    ...
    def scroll_post_detail(self, duration=2):
        """
        Scroll within the post detail popup to load more comments
        :param duration: Time to wait after scrolling in seconds
        :return: True if popup was scrolled, False otherwise
        """
        try:
            # Wait for popup to appear
            time.sleep(3)
            
            # Find the popup using the specific selector
            popup_selector = "#mount_0_0_UY > div > div:nth-child(1) > div > div.__fb-light-mode.x1n2onr6.x1vjfegm > div > div > div.x78zum5.xdt5ytf.x1iyjqo2 > div > div > div > div"
            
            try:
                popup = self.driver.find_element(By.CSS_SELECTOR, popup_selector)
                print("Found popup with specific selector")
            except:
                print("Could not find popup with specific selector")
                return False
                
            # Get current scroll position
            last_scroll = self.driver.execute_script("return arguments[0].scrollHeight", popup)
            print(f"Current scroll height: {last_scroll}")
            
            # Scroll within popup using scrollHeight
            ...
            new_scroll = self.driver.execute_script("return arguments[0].scrollHeight", popup)
            print(f"New scroll height: {new_scroll}")
            
            return new_scroll != last_scroll
        except Exception as e:
