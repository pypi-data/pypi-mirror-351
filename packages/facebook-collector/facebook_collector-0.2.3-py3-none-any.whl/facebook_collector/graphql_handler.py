...
class FacebookGraphQLCollector:
    ...
    def scroll_post_detail(self, duration=2):
        ...
        try:
            ...
            last_scroll = self.driver.execute_script("return arguments[0].scrollTop", popup)
            print(f"Current scroll position: {last_scroll}")
            
            # Scroll within popup
            self.driver.execute_script("""
                arguments[0].scrollTo({
                    top: arguments[0].scrollTop + 1000,
                    behavior: 'smooth'
                });
            """, popup)
            ...
            new_scroll = self.driver.execute_script("return arguments[0].scrollTop", popup)
            print(f"New scroll position: {new_scroll}")
