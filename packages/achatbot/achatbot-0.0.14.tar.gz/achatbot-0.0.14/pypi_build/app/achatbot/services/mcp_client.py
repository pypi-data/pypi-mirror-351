...
from achatbot.schemas.function_schema import FunctionSchema
from achatbot.schemas.tools_schema import ToolsSchema
from achatbot.common.event import EventHandlerManager
...
class MCPClient(EventHandlerManager):
    ...
    async def _sse_register_tools(self, llm: LLMProcessor) -> ToolsSchema:
        ...
        logging.debug("Starting registration of mcp.run tools")
    ...
    async def _stdio_register_tools(self, llm: LLMProcessor) -> ToolsSchema:
        """Register all available mcp.run tools with the LLM service.
        Args:
            llm: The Pipecat LLM service to register tools with
        Returns:
            ...
            if hasattr(results, "content") and results.content:
                for i, content in enumerate(results.content):
                    if hasattr(content, "text") and content.text:
                        ...
                        pass
                logging.info(f"Tool '{function_name}' completed successfully")
                logging.debug(f"Final response: {response}")
            else:
                logging.error(f"Error getting content from {function_name} results.")
