...
def waitFiles(pattern,timeout=180,interval=5) -> Optional[List[str]]:
    logger.info(f"_polling_wait {pattern} waiting {timeout} ")
...
def waitGlob(pattern,timeout=180,interval=5) -> Optional[List[str]]:
    logger.info(f"_polling_wait {pattern} waiting {timeout} ")
    for _ in range(timeout // interval):
        if files := glob.glob(pattern):
            logger.info(f"_polling_wait {files[0]} waited ")
...
"""
waitGlob(pattern,timeout=180,interval=5)
"""
