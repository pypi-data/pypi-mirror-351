"""
Imandra Universe Agent

Example:
    $ pip install 'imandra[universe]'
    $ export IMANDRA_API_KEY=<redacted>
    $ python
    >>> from imandra.u.agents import code_logician
    >>> g = code_logician.get_remote_graph()
    >>> code_logician.create_thread_sync(g)
    >>> response = g.invoke({'src_lang': 'python', 'src_code': src_code})
"""
