...
from hestia_earth.validation.utils import (
    _filter_list_errors, get_term_lookup_value, is_permanent_crop, blank_node_properties_group, get_lookup_value
)
...
def validate_waterRegime_rice_products(cycle: dict, list_key: str = 'practices'):
    ...
    def validate(values: tuple):
        ...
        allowed_product_ids = (get_term_lookup_value(term, 'allowedRiceTermIds') or '').split(';')
...
def validate_croppingDuration_riceGrainInHuskFlooded(cycle: dict, list_key: str = 'practices'):
    ...
    min_value = safe_parse_float(
        get_table_value(lookup, 'termid', country_id, column_name('Rice_croppingDuration_days_min')),
        None
    )
    max_value = safe_parse_float(
        get_table_value(lookup, 'termid', country_id, column_name('Rice_croppingDuration_days_max')),
        None
    )
...
def _practice_is_tillage(practice: dict):
    ...
    return True if term_type == TermTermType.OPERATION.value and get_term_lookup_value(term, 'isTillage') else False
...
def validate_processing_operation(cycle: dict, site: dict, list_key: str = 'practices'):
    has_processing_operation = any([
        not not get_term_lookup_value(p.get('term', {}), 'isProcessingOperation')
        for p in cycle.get(list_key, []) if all([
            p.get('primary', False),
            p.get('term', {}).get('termType') == TermTermType.OPERATION.value
        ])
    ])
    return site.get('siteType') != SiteSiteType.AGRI_FOOD_PROCESSOR.value or has_processing_operation or {
        'level': 'error',
        'dataPath': f".{list_key}",
