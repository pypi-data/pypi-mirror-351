...
class Agents:
    def __init__(self, api_key: Optional[str] = None, base_url = "", prompts: Prompts = None, x_trace_id: Optional[str] = None):
    ...
    def create(
        ...
        if type(agent.instructions) == str:
            instructions = PromptRequest(name = agent.name + ' prompt', content=agent.instructions, description = 'prompt for ' + agent.name)
        ...
        try:
            ...
            debug_print(f'tried to update but failed with error {e}, creating new prompt')
    ...
    def run(self, agent: Agent, message_history: List, context: dict = {}, debug: bool = False, max_turns: int = 1000, execute_tools: bool = True, response_schema: dict = None, environment: str = "dev"):
        res = None
        # Send function inputs
        try:   
            response = self._make_request("POST", f"/shared/agents/run/{agent.id}", 
                {
