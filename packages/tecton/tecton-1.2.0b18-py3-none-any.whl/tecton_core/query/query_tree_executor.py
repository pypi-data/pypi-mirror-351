...
def rewrite_offline_scan_node(
    ...
    for _, staging_node in staging_nodes.items():
        ...
        elif offline_scan_node.feature_definition_wrapper.has_delta_offline_store:
...
def check_partitioning_compatibility(plan: NodeRef) -> None:
    ...
    for join_node in get_all_join_nodes(plan):
        ...
        if (
            isinstance(repartition_side_input_node, StagingNode)
            and repartition_side_input_node.dialect == Dialect.DUCKDB
        ):
...
def _execute_stage(
    ...
    if isinstance(compute, ArrowCompute):
        ...
        reader = compute.run(
            output_node_ref, input_nodes_ref, input_data=inputs, context=context, partition_selector=partition_selector
        )
    elif isinstance(compute, SQLCompute):
        with compute:
            for input_node_ref, input_table in zip(input_nodes_ref, inputs):
...
def convert_plan_to_ray_dag(node_ref: NodeRef, context: ExecutionContext, **kwargs: Any) -> RayDataset:
    assert isinstance(node_ref.node, StagingNode)
    ...
    if not child_ray_datasets:
        ...
        def _partition_generator(partition_selector: PartitionSelector) -> pyarrow.Table:
            ...
            if conf.get_bool("DUCKDB_DEBUG"):
                logger.warning(f"Partition generation: {partition_selector} took {time.time() - start} secs")
            return out
        ...
        def _map_fn(partition_selector: PartitionSelector, input_table: pyarrow.Table) -> pyarrow.Table:
            ...
            if conf.get_bool("DUCKDB_DEBUG"):
                logger.warning(f"MapFn: {partition_selector} took {time.time() - start} secs")
            return out
        ...
        def _co_group_fn(partition_selector: PartitionSelector, *input_tables: Tuple[pyarrow.Table]) -> pyarrow.Table:
            ...
            if conf.get_bool("DUCKDB_DEBUG"):
                logger.warning(f"CoGroup: {partition_selector} took {time.time() - start} secs")
            return out
