...
from ..core.utils import is_pkg_beta, pip_update_package
...
class Base(Controller):
    ...
    @ex(help="Updates gcli to the latest version",
        arguments=[
            (['--stable'], {'action': 'store_true',  'dest': 'stable'}),
    ...
    def update(self):
        ...
        is_in_beta_version = bool(is_pkg_beta(package_name))
        use_unstable = self.app.pargs.unstable
        use_stable = self.app.pargs.stable
        
        use_test_pypi = False
        if use_unstable or is_in_beta_version:
            use_test_pypi = use_unstable or is_in_beta_version
        
        if use_stable:
            use_test_pypi = False

        pip_update_package(package_name, ask_force_update=True, use_test_pypi=use_test_pypi)