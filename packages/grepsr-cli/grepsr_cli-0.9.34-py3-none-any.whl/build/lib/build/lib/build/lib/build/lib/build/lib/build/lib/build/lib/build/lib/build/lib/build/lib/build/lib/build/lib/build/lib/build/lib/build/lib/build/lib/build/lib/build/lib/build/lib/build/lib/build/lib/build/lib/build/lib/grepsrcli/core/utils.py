...
def cmd_shell_exec(cmd, working_dir=None, timeout=100):
    return subprocess.run(cmd, shell=True, timeout=timeout, cwd=working_dir or None, capture_output=True, text=True)
...
def pip_update_package(package_name, ask_force_update=False, force_update=False):
    prefix_cmd = f'{sys.executable} -m'
    proc = cmd_shell_exec(f'{prefix_cmd} pip install {package_name} --upgrade')
    if proc.returncode != 0:
        if (ask_force_update or force_update) and '--break-system-packages' in proc.stderr:
            if force_update or ask_user_input_YN(f'{package_name} update unsuccessful. Do you want to force update using `--break-system-packages`?') == 'Y':
                proc = cmd_shell_exec(f'{prefix_cmd} pip install {package_name} --break-system-packages --upgrade')
    if proc.returncode == 0:
        stdout = proc.stdout
        if f'Successfully installed {package_name}' in stdout:
            print(f'{package_name} updated successfully.', flush=True)
        else:
            print(f'{package_name} is already at the latest version.', flush=True)
    else:
...
def is_package_installed_locally(package_name: str):
    ...
    for module in modules:
        if module['name'] == package_name:
            if module['editable_project_location']:
                return True
...
def try_updating_itself():
    ...
    if is_package_installed_locally(package_name):
        # dont
        return
    ...
    if (not file_contents) or curr_timestamp - stored_timestamp > 86_400:
        print('\nTrying to update gcli...')
        pip_update_package(package_name, force_update=True)
