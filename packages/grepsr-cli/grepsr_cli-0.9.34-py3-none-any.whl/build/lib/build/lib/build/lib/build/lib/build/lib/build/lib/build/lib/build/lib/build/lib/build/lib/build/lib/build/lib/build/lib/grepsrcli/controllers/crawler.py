...
class Crawler(Controller):
    ...
    def create(self):
        ...
        if self.app.pargs.init:
            ...
            if lang_type in NODE_LANGUAGE_TYPES:
                ...
                if lang_type == LanguageType.JAVASCRIPT:
                ...
                Log.info('Plugin created at {}'.format(plugin_path))
                return
    ...
    def deploy(self):
        ...
        try:
            if self.config.get('deploy', {}).get('verbose') == True or self.app.pargs.verbose == True:
                verbose_mode = True
        except:
        ...
        for plugin_name in plugin_names:
            ...
            try:
                ...
                try:
                    proc = cmd_shell_exec(cmd, working_dir=root_repo_dir, check=False) # check false so that we can show the exact error to the user.
                except subprocess.TimeoutExpired:
                    self.app.log.error(f'timed out on syncing repo for {plugin_name}')
                    continue
                if proc.returncode != 0:
                    self.app.log.error(f'error when syncing repo for {plugin_name}')
                ...
                for _ in range(5):
                    ...
                    proc = cmd_shell_exec(cmd, working_dir=root_repo_dir, check=True)
                    
                    # git push portion
                    push_successful = False
                    should_try_with_rebase = False
                    if verbose_mode:
                        cmd = verbosify(push_cmds)
                    else:
                        cmd = return_cmd_with_progress(push_cmds, toolbar_width)
                    try:
                        proc = cmd_shell_exec(cmd, working_dir=root_repo_dir, timeout=300)
