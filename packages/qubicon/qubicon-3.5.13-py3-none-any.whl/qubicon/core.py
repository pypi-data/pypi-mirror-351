...
class QubiconCore:
    ...
    def get_process_list_state(
        ...
        """
        Fetch minimum/partial status data for processes with filtering options.

        Args:
            archived_or_will_be_archived: Filter for archived processes (default: False)
            sort: Sorting criteria (default: "lastUsageDate,desc")
            part_of_experiment: Filter for processes part of an experiment (default: False)
            statuses: List of statuses to filter by (default: ["COMPLETED", "FAILED", "READY", "RUNNING", "WARMING_UP"])
        ...
        try:
            ...
            params = {
                "archivedOrWillBeArchived": str(archived_or_will_be_archived).lower(),
                "sort": sort,
                "partOfExperiment": str(part_of_experiment).lower(),
                "statuses": ",".join(statuses),
                "page": page,
                "size": size,
            }
