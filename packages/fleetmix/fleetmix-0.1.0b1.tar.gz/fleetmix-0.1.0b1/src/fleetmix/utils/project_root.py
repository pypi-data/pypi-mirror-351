...
def get_project_root() -> Path:
    ...
    if env_root_str:
        ...
        if (env_root / "pyproject.toml").exists() or \
           (env_root / ".git").is_dir() or \
           (env_root / "src" / "fleetmix").is_dir():
    ...
    for _ in range(5): # Limit search depth (src/fleetmix/utils -> src/fleetmix -> src -> project_root)
