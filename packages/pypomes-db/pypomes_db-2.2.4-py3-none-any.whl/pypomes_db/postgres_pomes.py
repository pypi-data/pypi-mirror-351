...
def bulk_execute(errors: list[str],
    ...
    *DELETE* operations require a special syntax using a *IN* clause:
    ...
    For *INSERT* operations, the *VALUES* clause must be simply *VALUES %s*:
        INSERT INTO my_schema.my_table (id, v1, v2) VALUES %s
    ...
    *UPDATE* operations require a special syntax, with *VALUES %s* combined with a *FROM* clause:
    ...
    data type mismatch. The solution is to explicitly cast the value of the column in the corresponding
    query clauses.

    For illustration purposes, here are examples of the modified query statements:
        For *INSERTs*:
            INSERT INTO my_schema.my_table (id::int4, v1::numeric, v2::timestamp) VALUES %s
        For *UPDATEs*:
...
def tipify_columns(errors: list[str] | None,
                   stmt: str,
                   nullable_only: bool,
                   conn: connection,
                   logger: Logger) -> str:
    """
    Modify the bulk *INSERT* or *UPDATE* statements given in *stmt* by adding the appropriate data types.

    As an illustration, the original query statements
      - INSERT INTO my_schema.my_table (v1, v2, ...) VALUES %s
      - UPDATE my_schema.my_table SET v1 = data.v1, v2 = data.v2, ...
        FROM (VALUES %s) AS data (id, v1, v2) WHERE my_schema.my_table.id = data.id

    would result, respectively, in the statements
      - INSERT INTO my_schema.my_table (v1::int4, v2::timestamp, ...) VALUES %s
      - UPDATE my_schema.my_table SET v1 = data.v1::int4, v2 = data.v2::timestamp, ...
        FROM (VALUES %s) AS data (id, v1, v2) WHERE my_schema.my_table.id = data.id

    :param errors: incidental error messages
    :param stmt: the bulk *INSERT* or *UPDATE* statement
    :param nullable_only: whether to consider nullable columns, only
    :param conn: the connection to use
    :param logger: optional logger
    ...
    is_insert: bool = stmt.startswith("INSERT INTO ")
    if is_insert:
        table_name: str = stmt[12:stmt.find(" (")]
    else:
        table_name: str = stmt[7:stmt.find(" SET")]
    table_schema: str
