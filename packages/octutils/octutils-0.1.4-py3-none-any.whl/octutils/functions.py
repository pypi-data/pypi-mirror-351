    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_input):
            try:
                ...
                print(f"Error processing {future_to_input[future]}: {e}")     
        ...
        with ThreadPoolExecutor(max_workers=7) as executor:
            ...
            for i, future in enumerate(futures):
                try:
                    ...
                    print(f"Error in {functions[i].__name__}: {e}")
                    results.append(None) 
...
def get_all_dataset_lineage():
    datasetlist_table = spark.sql("select a.* from datasetlist a left join oct_errors_parameters b on b.Item_ID = a.ID where b.Item_ID is null")
    spark_tables_list = []
    ...
    spark_errors.write.format("delta").mode("append").saveAsTable("oct_errors")
...
def save_measures():
    ...
    AS SELECT om.*, mf.Definition,mf.Feedback
...
def upsert_dataset_lineage(results):
    error_schema = StructType([
                    StructField("Workspace_ID", StringType(), True),
                    StructField("Item_Type", StringType(), True),
                    StructField("Item_ID", StringType(), True),
...
def encapsulate_dataset(WorkspaceID,DatasetID):
    ...
    upsert_msg = upsert_dataset_lineage(results)
    return results
...
def run_parallel_tasks_oct(WorkspaceID,DatasetID,LakehouseWorkspaceID,LakehouseID,Alias,ModifiedTime):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [
            ...
            executor.submit(final_shortcuts,WorkspaceID = LakehouseWorkspaceID,LakehouseID = LakehouseID)
        ]
        # Optionally wait for all futures to complete and collect results or handle exceptions
        for future in futures:
            future.result()
