...
class ToolService:
    ...
    async def a_serve(self):
        ...
        try:
            ...
            if self.nats_max_payload:
                separator = "=" * 50
                logger.info(f"\n{separator}\nSetting NATS max payload to {self.nats_max_payload} bytes for tool '{self.tool.name}'\n{separator}")
            ...
            logger.info(f"Received keyboard interrupt. Shutting down tool '{self.tool.name}' gracefully...")
            self._running = False
            ...
            try:
                if self.nc:
                    logger.info(f"Draining NATS connection for tool '{self.tool.name}'...")
                    await self.nc.drain()
                    self.nc = None
                    logger.info(f"NATS connection drained for tool '{self.tool.name}'")
            except Exception as e:
    ...
    def _setup_signal_handlers(self):
        ...
        def signal_handler():
            logger.info(f"Received termination signal. Initiating graceful shutdown for tool '{self.tool.name}'...")
            self._running = False
    ...
    async def subscribe(self):
        ...
        if sub:
            logger.info(f"Tool '{self.tool.name}' subscribed")
        while True:
            await asyncio.sleep(1)
