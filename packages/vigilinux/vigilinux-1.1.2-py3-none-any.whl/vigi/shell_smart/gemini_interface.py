...
class AICommunicationGateway:
    ...
    def request_output_summary_from_ai(
        ...
        self.terminal_display.log(f"[AI Gateway] Summary Prompt:\n{summary_prompt_text}") # Log the prompt for debugging
        ...
        self.terminal_display.log(f"[AI Gateway] Requesting summary for command: {executed_command} using {ai_engine_id_for_summary}")
        ...
        with self.terminal_display.status("[bold yellow]Vigi is thinking (analyzing output)...[/bold yellow]", spinner="dots8"):
            try:
                ...
                if response.parts:
                    ...
                    self.terminal_display.log(f"[AI Gateway] Summary received: {summary_text[:150].replace(chr(10), ' ')}...")
                    ...
                    self.terminal_display.log(f"[AI Gateway] [bold red]{block_msg}[/bold red]")
                    return f"Error: AI summary blocked. ({block_msg})"
                else:
                    self.terminal_display.log("[AI Gateway] [bold red]AI returned an empty response structure for summary.[/bold red]")
                ...
                self.terminal_display.log(f"[AI Gateway] [bold red]{error_msg}[/bold red]")
