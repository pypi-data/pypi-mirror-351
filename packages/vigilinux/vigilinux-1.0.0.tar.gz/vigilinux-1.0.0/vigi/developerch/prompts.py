...
Provide ONLY a JSON object with filenames as keys and new file contents as values.
Example: {{"file1.js": "new code"}}
"""
...
async def generate_modification(context: Dict[str, Any], 
    ...
    try:
        ...
        match = re.search(r'```(?:json)?\s*(\{.*?\})\s*```', json_response_str, re.DOTALL)
        if match:
            ...
            json_str_cleaned = json_response_str

        return json.loads(json_str_cleaned)
    except json.JSONDecodeError as e:
        logger.error(f"Failed to decode JSON from modification response: {json_response_str}. Error: {e}")
        return {"error": "Failed to parse LLM JSON output for modifications."}
    except Exception as e:
