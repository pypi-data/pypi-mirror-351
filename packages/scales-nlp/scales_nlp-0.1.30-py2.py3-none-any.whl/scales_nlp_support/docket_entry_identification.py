...
from scales_nlp_support import court_functions as cfunc
...
def assign_entry_to_judge(dentry, judge_ents, dentry_index=None):
    ...
    import sys
    sys.path.append('..')
    import scales_nlp_support.entity_functions as efunc
    import scales_nlp_support.language_tools as langtools
    ...
    if len( list(set(judge_lnames)) ) > 1:
        ...
        if 'assign' in dentry.lower():
            ...
            return efunc.clean_judge_name(judge_ents[min_index].text, punc=False, lower=False, suffix=False, prefix=False)
        #Case is referred
        ...
        #Multiple judge names....what to do..base it off the text
            ...
            if dentry.lower().count('before')>1 and 'signed' in dentry.lower():
                ...
                #Adding judges to the docket, no way to know who's in charge
                    ...
                    return ';;;'.join([efunc.clean_judge_name(x.text, punc=False,lower=False,suffix=False,prefix=False) for x in judge_ents])
        ...
        #Judge name found
        if judge_ents:
            return efunc.clean_judge_name(judge_ents[0].text, punc=False, lower=False, suffix=False, prefix=False)
        #No judge name found
        else:
            return ''
...
def identify_judge_entriesv2(jfhandle = None, docket = None):
    ...
    jnlp = spacy.load(settings.PROJECT_ROOT / 'code' / 'scales_nlp_support' / 'language_models' / 'judgemodel/')
