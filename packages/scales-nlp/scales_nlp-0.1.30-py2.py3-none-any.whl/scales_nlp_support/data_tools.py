...
from scales_nlp_support import bundler as bundler
from scales_nlp_support import docket_entry_identification as dei
from scales_nlp_support import entity_functions as efunc
from scales_nlp_support import fhandle_tools as ftools
from scales_nlp_support import lexicon
from scales_nlp_support import party_classification as pc
from scales_nlp_support import party_tagging as pt
from scales_nlp_support import settings
from scales_nlp_support.core import std_path
from scales_nlp_support import redact_clayton_regex
...
def remove_sensitive_info(docket):
    ...
    a_number_re = re.compile(r"(?i)([^0-9a-z])(a ?#? ?[0-9\- ]{8,11})([^0-9])")
...
def redact_private_individual_names_clayton(jdata, hdata, elective_nlp=None):
    '''
    Given html & json representations of a case, return versions of both with private individuals' names redacted
    '''

    def _clayton_redaction_tree(party, party_orig, party_composite, extra_info, nos_subtype,
                            label_names, label_names_ei, label_names_non_ei, elective_nlp, custom_data):

        _clayton_non_redact_words = ['apartments', 'apts', 'management', 'property', 'properties', 'finance',
                                     'residential', 'realty', 'townhouses', 'townhomes', 'homes', 'assignee',
                                     'administrator', 'administratrix', 'executor', 'official']
        _clayton_redact_words = ['individually']
        ...
        def _check_for_non_census_name(custom_data, party_composite):
            return (custom_data.get('party_type')=='defendant' and any((
                x in (custom_data.get('suit_types') or []) for x in (
                'Patho', 'Divorce', 'Abandoned Motor Vehicles')))) and ' '.join(
                party_composite.strip().replace('\n',' ').split()).count(' ') in range(1,4)

        # clayton-specific interventions
        if any((f' {x} ' in party for x in _clayton_non_redact_words)):
            return ('no_redact', '_clayton_non_redact_words')
        if any((f' {x} ' in party for x in _clayton_redact_words)):
            return ('redact', '_clayton_redact_words')

        # rules for parties with at least one label
        if label_names:

            # co, crim, role, gov/org/group, person
            if 'co' in label_names:
                return ('no_redact', '"co" label')
            if 'civil' not in (custom_data.get('court') or 'civil') and custom_data[
                'action_type'] not in crim_actions_needing_classification and party_composite != ' state of georgia ':
                return ('redact', 'criminal party')
            if 'role' in label_names and not any((
                ' '+x+' ' in party_composite for x in lexicon.nonperson_words_role_superseding)):
            ...
            if (any((x in label_names for x in ('unknown', 'other'))) or any((
                x in party_composite for x in ('$', '_')))):
                return ('no_redact', '"unknown," "other," "$", "_"')
            if (' doe ' in party_composite and any((f' {x} ' in party_composite for x in (
                'john', 'jane', 'johnjane')))):
                return ('no_redact', '"doe"')

            # for all others (mostly person-eng conflicts), classify as individual if person labels are plurality
            label_counts = Counter(label_names)
            mode_label = max(label_names, key=lambda label: label_counts[label])
            is_plurality = lambda label: mode_label==label and not any((
                x!=label and label_counts[x]==label_counts[label] for x in label_names))
            if is_plurality('person'):
                return ('redact', 'plurality of "person" labels')

            # fallback case
            else:
                if _check_for_non_census_name(custom_data, party_composite):
                    return ('redact', 'non-census name (has labels)')
                return _classify_with_nlp(party_orig, 'spacy (has labels but fallback case)')
        ...
        # rules for labelless parties
            ...
            if 'civil' not in (custom_data.get('court') or 'civil') and custom_data[
                ...
                return ('redact', 'criminal party')
            if _check_for_non_census_name(custom_data, party_composite):
                return ('redact', 'non-census name (no labels)')
            else:
                return _classify_with_nlp(party_composite, 'spacy (no labels)')

    def _clean_eviction_name(name, custom_data):
        name = re.sub(''.join(('(?i)(?:and )?(?:all )?(?:other )?(?:other ?s|',
                               '(?:(?<= )|(?<=^))o ?cc ?u ?pa ?n ?t ?s?)(?= |$)(?! of)|',
                               'and all other( |$)|et ?al$')), '', name)
        or_clauses = name.lower().split(' or ')
        if len(or_clauses)>1: # "any unknown estate" probably not eviction, but throwing it in here for convenience
            if re.match('(?:tenant|occupant|all other|any unknown estate)', or_clauses[1]):
                name = name[:len(or_clauses[0])]
        name = name.strip()
        return name

    def _clean_misc(name, custom_data):
        name = re.sub('(?i) aka$', '', name)
        name = re.sub('(?i)(?:2|seco)nd or[a-z ]+al', '', name).strip()
        return name

    def _handle_numbers(name, custom_data):
        third_party_re = '(?i)3rd (?:party|pty).*$'
        number_exclusions = ['acura', 'amc', 'buick', 'caddilac', 'cadellac', 'cadillac', 'cherolet', 'chev',
                             'chevrolet', 'chevy', 'chrysler', 'crysler', 'daihatsu', 'datsun', 'dodge', 'eagle',
                             'ford', 'geo', 'gmc', 'heileg', 'heilig', 'honda', 'hyundai', 'hyundia', 'isuzu',
                             'jeep', 'lincoln', 'mazda', 'mercury', 'mitsubishi', 'nissan', 'olds', 'oldsmobile',
                             'plymouth', 'pontiac', 'renault', 'subaru', 'suzuki', 'toyota', 'volks', 'volkswagen',
                             'volkswagon', 'volkwagen', 'volvo', 'vw', 'yamaha', 'yugo'] # namelike (cars+'heilig')
        if any((x.isnumeric() for x in name)):

            # numbers in names (prisoner numbers, apartment numbers, etc)
            number_re = re.match(''.join(('(?i)(?:(?:[a-z]+ +)+?(?:', # capture grps mutually exclusive bc big ORs
                                          '[a-z]+ +((?:\d+ )?ef ?\d+(?: [a-z]\d*)?(?: \d+)?)(?: [a-z]+)*|',
                                          '(le ?\d+)(?: [a-z]+)*|',
                                          '[a-z]+ +((?:[a-z]{1,2} ?|gdc|apt (?:[a-z] ?)?(?:\d+ )?)?\d+[a-z]?))$|',
                                          '[a-z]{2,}([0-9]{2,}) [a-z]+)')), name)
            if number_re and not any((f' {x} ' in f' {name.lower()} ' for x in number_exclusions)):
                substring_to_remove = list(filter(None, number_re.groups()))[0] # ok to just take [0] bc mutu excl
                return name.replace(substring_to_remove, '').strip()

            # e.g. 'J0NES KATRINA', 'ADDIS ME3SKEREM ET AL'
            elif re.match('(?i)[a-z0 ]+$', name) and all((x.replace('0','') for x in name.split())):
                return name.strip('0').replace('0','O')
            elif re.match('(?i)([a-z ]*(?:e3|3e)[a-z ]*)+$', name) and all((
                x.lower().replace('e3','').replace('3e','') for x in name.split())):
                return name.replace('e3','').replace('3e','')

            # handle '2nd', '3rd', etc
            elif re.search(third_party_re, name):
                return re.sub(third_party_re, '', name).strip()
            elif re.search('(?i)(?<!^)2nd|3rd|4th', name) and name.count(' ')<4:
                for regex,replacement in (('(?i)2nd', 'II'), ('(?i)3rd', 'III'), ('(?i)4th', 'IV')):
                    return re.sub(regex, replacement, name)

            else:
                return pc.preproc_encode(name, 'no_redact', '_handle_numbers')

        else:
            return name

    def _handle_patho_error(name, custom_data): #errs ever so slightly towards over-redaction, but still worthwhile
        if 'Patho' in (custom_data.get('suit_types') or []) and custom_data['party_type']=='defendant':
            name = name.replace('  ', ' & ')
            if re.match('(?i).{30} .+', name) and any((x in name.lower() for x in (' & ', ' and '))):
                name = ''.join(name.rsplit(' ', 1))
                return pc.preproc_encode(name, 'redact', '_handle_patho_error')
        return name

    def _make_pc_call(name, custom_data):
        return pc.classify_party(name, None, None, elective_nlp=elective_nlp,
                                 custom_classifier = _clayton_redaction_tree,
                                 custom_multient_handler = pc.name_redaction_multient_handler,
                                 custom_listname_mappings={'unnamed_individual_words':'other',
                                                           'family_words':'person',
                                                           'role_words_non_professional':'person'},
                                 custom_preprocessing_functions=[_handle_patho_error,
                                                                 _clean_eviction_name,
                                                                 _clean_misc,
                                                                 _handle_numbers],
                                 custom_data=custom_data)

    def _make_custom_data(jdata, party=None, action_type=None):
        return {'court': jdata['court'],
                'party_type': party['party_type'] if party else None,
                'action_type': action_type,
                'suit_types': list(chain.from_iterable([x.get('suit_types') or [] for x in jdata['parties']]))}

    def _clean_party(party_orig):
        party = ' '.join(party_orig.lower().strip().split())
        for string in [',', '.', "'", '-', '/', ' &', ' etal', ' et al', ' aka', ' dba',
                       ' and all others', ' all other occupants']:
            party = party.replace(string, '')
        return party

    def _permute_party(party_orig, redact=False, custom_data={}):
        party = _clean_party(party_orig)
        if len(party.split())>6: # speed degrades above 6; luckily, such priv-indiv parties seem to be outliers
            return set([party])
        if redact or _make_pc_call(party, custom_data)[0] == 'redact':
            permutations = set([' '.join(x) for x in itertools.permutations(party.split())])
        else:
            permutations = set([party])
        if party!='state of georgia':
            for i in range(0, party.count(' ')+1):
                if len(party.split()[i])>1:
                    permutations.add(party.split()[i])
                sub_party = ' '.join(party.split(' ')[:i]+party.split(' ')[i+1:])
                permutations |= set([' '.join(x) for x in itertools.permutations(sub_party.split())])
    #         if party.count(' ')>1: # no longer needed due to auto-redaction of criminal parties
    #             for i in range(len(party.rsplit(' ',1)[0])+2, len(party)):
    #                 permutations.add(party[:i])
        return permutations

    def _hash_and_format_text(text):
        hashed = hashlib.md5(text.encode('utf-8')).hexdigest().upper()
        return {'text_long': f'SCALES-Party-Hash-{hashed}', 'text_short': f'SCALES-{hashed[:7]}',
                'html_long': f'<span style="color:purple">SCALES-Party-Hash-{hashed}</span>',
                'html_short': f'<span style="color:purple">SCALES-{hashed[:7]}</span>'}

    action_to_regex = redact_clayton_regex.action_to_regex.copy()
    findall_actions = ['general', 'Case Filing', 'Case Outcome', 'Event Outcome']
    actions_with_repeating_capture_groups = ['FiFa Issued', 'Fifa issued', 'Alias Fifa', 'Answer', 'Judgment',
                                             'Party Inactive', 'Proposed Order', 'Jury Demand', 'Stipulation',
                                             'Off Code Change', 'Entry of Appear']
    crim_actions_needing_classification = ['Arr War Probter', 'Dem for Discov', 'Dem JD Hearing', 'Image Document',
                                           'Motion', 'Motion filed', 'Order filed', 'Pet/Ord Adj Glt', 'Transcript']
    actions_not_needing_general_regex=[ # don't remove actions w/o adding to above list, or they'll auto-redact in crim
        'Amended Acc', 'Conv Dkt Text', 'Image Copy', 'Judge Reassign', 'Miscellaneous', 'Ord Jment Costs',
        'Ord Modify Sen', 'Ord No Adj Glt', 'Order for Bond', 'Pet for Vop', 'Prob Modified', 'Recusal',
        'Sent Modified', 'Viol of Prob']
    header_delim, cell_delim, docketline_delim = '<tr class="body11">', '<td>', '<span class="body11">'
    null_char = '∅' # used in actions_with_repeating_capture_groups block
    
    
    # start w/ the json data, which is easier to work with; do classification first (we'll do redaction throughout)
    hdata, jdata = copy.deepcopy(hdata), copy.deepcopy(jdata)
    names_to_spans = {} # also contains hash strings, but keeping this name for brevity
    num_extra_rows = 0 # hack due to the fact that some non-party chunks use header_delim
    for i,party in enumerate(jdata['parties']):
        name = party['name']
        custom_data = _make_custom_data(jdata, party=party)
        if _make_pc_call(name, custom_data)[0] == 'redact':
            hashes = _hash_and_format_text(name)
            names_to_spans[_clean_party(name)] = {'header_ind': i+num_extra_rows,
                                                  'header_name': name,
                                                  'permutations': _permute_party(_clean_party(name), redact=True),
                                                  'spans': [],
                                                  'hashes': hashes}
            
            # redact from header & entity info
            party['name'] = hashes['text_long']
            raw_info = party['entity_info'].get('raw_info')
            if raw_info:
                if raw_info.split('\n')[0]!=name:
                    raise Exception("assert raw_info.split('\\n')[0]!=party['name']") # this shouldn't happen
                party['entity_info']['raw_info'] = hashes['text_long'] # remove address info as well as name
            jdata['parties'][i] = party
        num_extra_rows += len(party.get('suit_types') or []) + len(party.get('pending_counts') or []) + max(
            len(party.get('counsel') or [])-1, 0)
           
        
    # do regex tagging
    docket_orig = copy.deepcopy(jdata['docket'])
    for i,entry in enumerate(jdata['docket']):
        text, action, action_orig = entry['docket_text'], entry['docket_action'], entry['docket_action']
        text = re.sub('\n\n+', '\n', text) # weird bug in magistrate;;0:12-co-00776
        if action not in action_to_regex and action not in actions_not_needing_general_regex:
            action = 'general'
        if action in findall_actions:
            matches = list(re.finditer(action_to_regex[action], text))
        else:
            match = re.match(action_to_regex.get(action) or null_char, text)
            matches = [match] if match else []
        if not any((x for x in matches)) and action!='general' and action not in actions_not_needing_general_regex:
            matches = list(re.finditer(action_to_regex['general'], text)) # try gen on known actions w/o matches
            
        # handle repeating capture groups
        if matches and action in actions_with_repeating_capture_groups:
            match = matches[0] # ugly but effective (actions_with_r_c_g and findall_actions are mutually exclusive)
            matches, text_edited = [], text
            regex = action_to_regex[action].replace('[^', f'[^{null_char}').replace('.+', f'[^\n{null_char}]+')
            while match and [x for x in match.groups() if x and x!=' ' and x.replace(null_char,'')]:
                matches.append(match)
                for j in range(1, len(match.groups())+1):
                    if match.group(j):
                        start,end = match.span(j)
                        text_edited = text_edited[:start] + null_char*len(match.group(j)) + text_edited[end:]
                match = re.match(regex, text_edited)
                
        # do permutation tagging
        inds_to_ignore, permu_all = set(), []
        for match in matches:
            for j in range(1, len(match.groups())+1):
                if match.group(j):
                    for ind in range(match.start(j), match.end(j)):
                        inds_to_ignore.add(ind)
        for name,party_dict in names_to_spans.items():
            permu_all += [(name,x) for x in party_dict['permutations']]
        permu_all = sorted(permu_all, reverse=True, key=lambda x: len(x[1]))
        for name,permu in permu_all:
            permu = '[,\.]? '.join(permu.split())
            for match in list(re.finditer(f'(?i)(?:(?<=[ /-]|\n)|(?<=^)){permu}(?=[ ,;/\.\'-]|s |\n|$)', text)):
                if match.start() not in inds_to_ignore:
                    matches.append((match.span(), name))
                    for ind in range(match.start(), match.end()):
                        inds_to_ignore.add(ind)

        # iterate through matches, skipping any that aren't in the header and don't seem to be private individuals
        if matches:
            json_spans = []
            for match in matches:
                if type(match)==tuple: # permutation matches
                    json_spans.append(match)
                else:
                    for j in range(1, len(match.groups())+1):
                        if match.group(j):
                            json_spans.append((match.span(j), match.group(j)))
            json_spans = sorted(json_spans, key=lambda x: x[0][0], reverse=True)
            for matched_span,matched_name in json_spans:
                cleaned_name = _clean_party(matched_name)
                if cleaned_name not in names_to_spans:
                    matched_permu = [k for k,v in names_to_spans.items() if cleaned_name in v['permutations']]
                    if matched_permu:
                        cleaned_name = matched_permu[0] # as long as we redact, it's ok to hackily do [0]
                    elif matched_name in [x['name'] for x in jdata['parties']]: # we already ok'd not redacting
                        continue
                    elif _make_pc_call(matched_name, _make_custom_data(
                        jdata, action_type=action_orig))[0] != 'redact':
                        continue
                    else:
                        names_to_spans[cleaned_name] = {'header_ind': None,
                                                        'header_name': None,
                                                        'permutations': set(),
                                                        'spans': [],
                                                        'hashes': _hash_and_format_text(matched_name)}

                # calculate the span(s), accounting for various edge cases, and update the spans dictionary
                _newline_offset = lambda text: text.count('\n')
                _char_code_offset = lambda text: text.count('&')*4 + text.count('>')*3 + text.count('<')*3
                start, end, text_for_span, spans = *matched_span, text, []
                entry_index_with_offset = i + _newline_offset(''.join([x[
                    'docket_text'] for x in docket_orig[:i]]))
                
                if '\n' in text_for_span[:start]:
                    entry_index_with_offset += text_for_span[:start].count('\n')
                    last_newline = text_for_span[:start].rindex('\n')
                    start, end = start-last_newline-1, end-last_newline-1
                    text_for_span = text_for_span[last_newline+1:]
                if '\n' in text_for_span[start:end]:
                    old_lines = []
                    for k,line in enumerate(text_for_span[start:end].split('\n')):
                        spans.append((entry_index_with_offset+k,
                                      (start if k==0 else 0) + _char_code_offset(
                                          text_for_span[:start]+''.join(old_lines)),
                                      (start if k==0 else 0) + len(line) + _char_code_offset(
                                          text_for_span[:start]+''.join(old_lines)+line)))
                        old_lines.append(line)
                else:
                    spans.append((entry_index_with_offset,
                                  start + _char_code_offset(text_for_span[:start]),
                                  end + _char_code_offset(text_for_span[:end])))
                names_to_spans[cleaned_name]['spans'] += spans

                # redact from docket
                text = text[:matched_span[0]] + names_to_spans[cleaned_name]['hashes'][
                    ('text_short' if names_to_spans[cleaned_name][
                        'header_ind'] is not None else 'text_long')] + text[matched_span[1]:]
                entry['docket_text'] = text
                jdata['docket'][i] = entry
                        
    
    # translate json work to html; classification/tagging done, so just do redaction (start with docket)
    text_split, chunks_docket, queue = hdata.split(docketline_delim), [], [] # assume most cases need chunks_docket
    docket_actions = [x.split('<')[0].strip() for x in text_split[:-1][::-4]]
    for i,chunk in enumerate(text_split[::-1]):
        if i%4==0:
            queue = [(len(text_split)-i-1, chunk)] + queue
        if (i%4==0 and docket_actions[int(i/4)]) or i==len(text_split)-1:
            chunks_docket += queue
            queue = []
            
    header_inds_to_redact = [] # save these for later
    for v in names_to_spans.values():
        header_ind, header_name, spans, hashes = v['header_ind'], v['header_name'], v['spans'], v['hashes']
        if header_ind is not None:
            header_inds_to_redact.append((header_ind, header_name, hashes['html_long']))
        for span in spans:
            j,start,end = span
            i,chunk = chunks_docket[j]
            chunks_docket[j] = (i, chunk[:start+1]+hashes[ # add 1 so as not to mutate chunk via strip()
                ('html_short' if header_ind is not None else 'html_long')]+chunk[end+1:])
            
    chunks_docket = sorted(chunks_docket, key=lambda x: x[0])
    mod_value = chunks_docket[0][0]%4
    chunks_docket_redacted = []
    for i in range(0, len(text_split)):
        chunks_docket_redacted.append(chunks_docket[int(i/4)][1] if i%4==mod_value else text_split[i])
    hdata = docketline_delim.join(chunks_docket_redacted)
        
    # redact header & entity info
    tables = hdata.split('</table>')
    chunks_header = tables[1].split(header_delim) # assume most cases need chunks_header
    for i,name,hash_string in header_inds_to_redact:
        chunk = chunks_header[i+1]
        cells = chunk.split(cell_delim)
        cells_redacted = [(hash_string+'</td>' if name in x else x) for x in cells] 
        chunks_header[i+1] = cell_delim.join(cells_redacted)
    tables[1] = header_delim.join(chunks_header)
    hdata = '</table>'.join(tables)
...
def load_recap_dump(indir):
    ...
    import pandas as pd
    sys.path.append('..')
    import scales_nlp_support.fhandle_tools as ftools
...
def convert_filepaths_list(infile=None, outfile=None, file_iter=None, nrows=None):
    ...
    dmap = {
        ...
        'judge': lambda c,f: efunc.clean_judge_name(c.get('judge')),
    ...
    # Judge matching
#     jmap = efunc.unique_judge_mapping(df.judge.unique())
