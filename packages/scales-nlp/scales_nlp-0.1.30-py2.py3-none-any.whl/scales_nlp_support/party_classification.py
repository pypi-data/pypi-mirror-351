...
preproc_encoded_flag = '***pclass and pdetails encoded into this "party name"***'
preproc_encoded_delim = '|||'
...
def preproc_encode(name, pclass, pdetails):
    return preproc_encoded_delim.join((preproc_encoded_flag, name, pclass, pdetails))

def _preproc_decode(encoded):
    if preproc_encoded_flag in encoded:
        return encoded.split(preproc_encoded_flag+preproc_encoded_delim)[1].split(preproc_encoded_delim)
    else:
...
def _apply_second_order_labeling_procedures(lex_labels, party, listname_mappings):
    ...
    for num_re in ('[^0-9]\d{5} \d{3}[^0-9]','(?i)([^0-9a-z])a ?#? ?[0-9\- ]{8,11}([^0-9])'):#latter via scales_nlp_support.data_tools.remove_sensitive_info
...
def classify_party(party, extra_info, nos_subtype, elective_nlp=None, include_labels_in_results=False, custom_data={},
                   custom_classifier=None, custom_multient_handler=None, custom_listname_mappings=None, custom_preprocessing_functions=[]):
    ...
    # do some preprocessing (empty parties, dba words, multi-entity parties), and discard irrelevant text if there's only one entity of interest
    party_orig = party
    ...
    if any((re.search(f'(?i)\\b{x}\\b'.replace('/','\/'), party_composite) for x in lexicon.dba_words)):
        return _get_return_values('other', 'standard')
    for preproc_func in custom_preprocessing_functions:
        party_composite, pclass, pdetails = _preproc_decode(preproc_func(party_composite, custom_data))
        if pclass:
            return _get_return_values(pclass, pdetails)

    # handle multi-entities
