...
from scales_nlp_support import court_functions as cf
from scales_nlp_support import data_tools as dtools
from scales_nlp_support import fhandle_tools as ftools

JUDGE_NAN_NAMES = {'not_found': 'XXNaNXX', 'committee': 'XXNon-IndividualXX'}
...
def load_entity_data(ucid, collection, as_df=True):
    '''
    Load entity data from relevant .jsonl files in the directory associated with the specified collection

    Inputs:
        - ucid (str or iterable): can be a single ucid (str) or any iterable (list / pd.Series)
        - collection (str): the name of the entity collection to load
        - as_df (bool): if true returns as type pd.DataFrame, otherwise list of dicts

    Output:
        (pd.DataFrame or list of dicts) data for the given ucid(s)
    '''

    ucids = [ucid] if type(ucid) is str else ucid
    rows = []

    for ucid in ucids:
        fname = ftools.build_entity_filename_from_ucid(ucid, settings.ENTITY_PATH_MAP[collection])
        results = []
        if fname.exists():
...
def clean_judge_name_field(name_field):
    '''
...
def clean_judge_name(judge_name, punc=True, lower=True, suffix=True, prefix=True):
    '''
...
def unique_judge_mapping(judge_list, cleaned=True):
    '''
    ...
    judges_list_clean = judge_list if cleaned else [clean_judge_name(x) for x in judge_list]
...
def identify_judge_last_name_clean(name):
    cname = clean_judge_name(name)
...
def judge_name_similarity_matcher(name, jdf):
    '''
    inputs:
    ...
    import scales_nlp_support.text_functions as tfunc
    from collections import Counter
    #Clean our name
    cname = clean_judge_name(name)
    source_parts = cname.split(' ')
    #Create the full judge name and clean
    fullnames = [x.strip().split(' ') for x in jdf.FullName.apply(clean_judge_name)]
    firstnames = [x.strip() for x in jdf['First Name'].apply(clean_judge_name)]
    for i, fname in enumerate(firstnames):
        # If first is just an initial, replace with middle name
        if len(fname) == 1:
            firstnames[i] = clean_judge_name(jdf.iloc[i]['Middle Name'])

    lastnames = [x.strip() for x in jdf['Last Name'].apply(clean_judge_name)]
    overlaps = []
...
def filter_judges_by_court(df, court, date=None):
    '''
...
def find_district_judge(name, court, jdf, courtdf, date = None):
    ...
    last_name = identify_judge_last_name_clean(name)
    ...
    if len(jdf_matches) == 1:
        return judge_name_similarity_matcher(name, jdf_matches)
    # No match
    ...
    # Multiple matches
        ...
        # Filter by court
        jdf_matches = filter_judges_by_court(jdf_matches, court, date)
        ...
        # No match
            ...
            return  judge_name_similarity_matcher(name, filter_by_court(jdf, court, date))
...
def is_senior_judge(name, court, date=None):
    '''
