...
CircleCenterRadius = Tuple[vec.FloatVec, float]
...
def point_at_angle(center: vec.FloatVec,
                   radius: float,
                   radians: float = None,
...
def rotated_point(point: vec.FloatVec,
...
def circles_from_2tanline_and_tanpoint(
    ...
    pt_on_line1 = line.point_is_on_line_2d(tan_point, linepts=line.points_on_a_slope_intercept(tan_line1))
    pt_on_line2 = line.point_is_on_line_2d(tan_point, linepts=line.points_on_a_slope_intercept(tan_line2))
...
if __name__ == "__main__":
    ...
    def _circ_option_1(pt, v, desired_pt, desired_v) -> CircleCenterRadius:
        ...
        omega = line.rads_between_lines(line.slope_intercept_of_line(line1), line.slope_intercept_of_line(line2))
        print(f"Omega: {omega}")
    ...
    def test_3():
        ...
        tan_line1 = line.slope_intercept_of_line(line=(desired_pt, vec.add_vectors([desired_pt, desired_v])))
        tan_line2 = line.slope_intercept_of_line(line=(pt, vec.add_vectors([pt, v])))
        ...
        pt_on_line1 = line.point_is_on_line_2d(desired_pt,
                                               linepts=(desired_pt, vec.add_vectors([desired_pt, desired_v])))
        pt_on_line2 = line.point_is_on_line_2d(desired_pt, linepts=(pt, vec.add_vectors([pt, v])))
