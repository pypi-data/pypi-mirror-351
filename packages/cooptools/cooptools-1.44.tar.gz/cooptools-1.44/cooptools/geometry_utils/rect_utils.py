...
def rect_corners(rect: Rect, cardinality: CardinalPosition = CardinalPosition.BOTTOM_LEFT) -> Dict[CardinalPosition, Tuple[float, float]]:
    x, y, w, h = rect
...
def rect_contains_point(rect: Rect, pt: Tuple[float, float]) -> bool:
    x, y, w, h = rect
...
def bounding_circle_radius(rect: Rect) -> float:
    x, y, w, h = rect
...
def bounding_rect(pts: vec.IterVec,
                  buffer: float | vec.FloatVec = None) -> Rect:
