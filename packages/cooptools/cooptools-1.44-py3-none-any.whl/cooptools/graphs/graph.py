...
class Graph(object):
    ...
    def astar(self,
        ...
        if results is None:
            ...
            return AStarResults(None, None, steps, source=start, dest=end)
        else:
            ''' Log Path found '''
    ...
    def SectorGraph(self) -> Self:
        ...
        graph_dict = {Node(name=k,
                           pos=rect.rect_center(self._sec_tree.Sectors[k])):
                          [Node(name=x, pos=rect.rect_center(self._sec_tree.Sectors[x])) for x in v] for k, v in g_dic_ret.items()}
