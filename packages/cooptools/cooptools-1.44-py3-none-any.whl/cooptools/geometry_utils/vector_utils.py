from typing import Tuple, Optional, List, Iterable, Callable, Dict
import math
import numpy as np
...
def describe2(pts: IterVec) -> Tuple[FloatVec, FloatVec, FloatVec]:
    ...
    mins = tuple(
        min(pt[ii] for pt in pts) for ii in range(len(pts[0]))
    )

    maxs = tuple(
        max(pt[ii] for pt in pts) for ii in range(len(pts[0]))
    )
    avgs = avg_of_points(pts)
...
def scaled_to_length(a: FloatVec,
                     ...
                     preserved_idxs: Iterable[int] = None) -> FloatVec:
...
def project_onto(a: FloatVec,
                 ...
                 allow_diff_lengths: bool = False) -> FloatVec:
    if origin is None:
...
def inversion(vec: FloatVec, inversion_idxs: Iterable[int]) -> FloatVec:
    ret = []
...
def avg_of_points(
    ...
    avgs = [
        sum(x[ii] for x in points) / len(points) for ii in range(max_len)
    ]
...
def bisecting_vector_2d(a: FloatVec, b: FloatVec, unit: bool=False) -> FloatVec:
    len_a = vector_len(a)
    len_b = vector_len(b)
...
def threedim_from_delim_string(coords_str: str, delimiter: str = ',') -> FloatVec:
