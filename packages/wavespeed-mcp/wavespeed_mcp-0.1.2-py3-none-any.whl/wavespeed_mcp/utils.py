...
def build_output_file(
    ...
    output_file_name = f"{tool}_{short_desc}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{extension}"
...
def get_image_as_base64(url: str, timeout: int = 30) -> tuple[str, str]:
    ...
    try:
        ...
        raise WavespeedMcpError(f"Timeout downloading image from URL (after {timeout}s)")
...
def validate_loras(loras: Optional[List[Dict[str, Union[str, float]]]]) -> List[Dict[str, Union[str, float]]]:
