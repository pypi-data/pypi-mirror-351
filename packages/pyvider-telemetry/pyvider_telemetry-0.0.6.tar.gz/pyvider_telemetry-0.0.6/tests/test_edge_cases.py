...
def test_invalid_environment_variables_handling() -> None:
    """Tests handling of invalid environment variables."""
    invalid_env_cases = [
        ("PYVIDER_LOG_LEVEL", "INVALID_LEVEL"),
        ("PYVIDER_LOG_CONSOLE_FORMATTER", "invalid_formatter"),
        ("PYVIDER_LOG_LOGGER_NAME_EMOJI_ENABLED", "maybe"),
        ("PYVIDER_LOG_DAS_EMOJI_ENABLED", "sometimes"),
        ("PYVIDER_LOG_OMIT_TIMESTAMP", "perhaps"),
        ("PYVIDER_TELEMETRY_DISABLED", "kinda"),
        ("PYVIDER_LOG_MODULE_LEVELS", "invalid:format:here,also:bad"),
    ]

    original_stderr = sys.stderr
    captured_warnings = io.StringIO()

    for env_var, invalid_value in invalid_env_cases:
        # Capture warnings during config loading
        ...
        with patch.dict(os.environ, {env_var: invalid_value}):
            try:
                ...
                if env_var == "PYVIDER_LOG_LEVEL":
                    assert config.logging.default_level == "DEBUG"  # Fallback
                elif env_var == "PYVIDER_LOG_CONSOLE_FORMATTER":
                    assert config.logging.console_formatter == "key_value"  # Fallback

            except Exception as e: # pragma: no cover
                pytest.fail(f"Configuration loading failed with {env_var}={invalid_value}: {e}")
            finally:
                # Reset stderr for each case to isolate warning capture
                sys.stderr = original_stderr
                captured_warnings.seek(0)
                captured_warnings.truncate(0)


    # Check that warnings were issued (this part needs to be inside the loop or rethought)
    # For simplicity, we'll assume the original test's intent was to check if *any* warning was produced.
    # A more robust test would check specific warnings for specific invalid inputs.
    # The current structure with a single captured_warnings outside the loop will accumulate.
    # Let's re-check the logic for warning capture.
    # The original test had sys.stderr = original_stderr *after* the loop.
    # This means captured_warnings would contain all warnings.

    # Re-evaluating the warning check:
    # The original test structure for checking warnings was:
    # sys.stderr = captured_warnings (before loop)
    # ... loop ...
    # sys.stderr = original_stderr (after loop)
    # warning_output = captured_warnings.getvalue()
    # assert "Invalid" in warning_output or "Warning" in warning_output

    # Let's refine the warning check to be more specific if possible, or ensure it works as intended.
    # The current fix moves stderr restoration inside the loop, so we need to check warnings per case or accumulate.
    # For now, let's assume the goal is that *some* warning is printed for *some* invalid input.
    # A better test would be to check specific warnings for specific inputs.

    # To make the test pass with the current structure, we need to check warnings after each case or accumulate.
    # Let's try to check if any warning was printed during the whole process.
    all_warnings_output = ""
    sys.stderr = captured_warnings # Capture all warnings from the loop
    for env_var, invalid_value in invalid_env_cases:
        with patch.dict(os.environ, {env_var: invalid_value}):
            config = TelemetryConfig.from_env() # This will print to the captured_warnings
            # Basic assertions on config structure
            assert config is not None
            assert isinstance(config.logging, LoggingConfig)
    sys.stderr = original_stderr # Restore stderr
    all_warnings_output = captured_warnings.getvalue()
    assert "Invalid" in all_warnings_output or "Warning" in all_warnings_output, \
        f"Expected warnings for invalid env vars, but got: {all_warnings_output}"
