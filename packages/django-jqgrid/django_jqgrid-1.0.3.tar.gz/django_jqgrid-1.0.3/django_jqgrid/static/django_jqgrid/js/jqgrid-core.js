...
window.jqGridConfig = window.jqGridConfig || {
    ...
    rowRenderers: {
        // Default notification row renderer
        ...
        // Default notice widget row renderer
        ...
        // Default contacts row renderer
        lead_contacts_jqGrid: function(cellValue, options, rowObject) {
            const formatLastSeen = lastSeen => {
                // Use the configurable timeSince function
                return window.jqGridConfig.timeSince(lastSeen);
            };
...
function initializeGridForTable(tableInstance) {
    ...
    return $.ajax({
        ...
        success: function(response) {
            ...
            // Initialize import/export if the import-export module is available
            if (window.importExportUtils && typeof window.importExportUtils.initializeImportExport === 'function') {
                window.importExportUtils.initializeImportExport(tableInstance, {
                    import_config: {
                ...
                if (selectedIds && selectedIds.length > 0) {
                    if (action.id === 'bulk-update') {
                        ...
                        if (typeof openFormCard === 'function') {
                            // Get selection scope (specific or all)
                            const scope = $(`input[type='radio'][name='${tableInstance.id}_bulk_selection_choice']:checked`).val();
                            const selectedIds = tableInstance.$grid.jqGrid('getGridParam', 'selarrrow');
                            const filters = tableInstance.$grid.jqGrid('getGridParam', 'postData').filters;
                            
                            // Pass selected IDs and filters to campaign form
                            openFormCard({
                                url:'/crud/sales/campaign/', 
                                mode:'modal',
                                data: {
                            ...
                            utils.notify('error', 'openFormCard function is not defined', tableInstance);
                        }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (data) {
                ...
                // Initialize import/export configuration if available and the module exists
                if ((data.import_config || data.export_config) && typeof window.importExportUtils !== 'undefined') {
                    window.importExportUtils.initializeImportExport(tableInstance, data);
                }
...
window.importExportUtils = window.importExportUtils || {
     ...
     */
    initializeImportExport: function(tableInstance, data) {
        ...
        // Check if custom implementations exist
        if (typeof this.setupImportUI === 'function' && data.import_config) {
            this.setupImportUI(tableInstance.appName, tableInstance.tableName, tableInstance.importConfig);
        }
        
        if (typeof this.setupExportUI === 'function' && data.export_config) {
            this.setupExportUI(tableInstance.appName, tableInstance.tableName, tableInstance.exportConfig);
        }
    },
    
    // Placeholder methods that can be overridden by the user
    setupImportUI: null,
    setupExportUI: null
};
