...
var JqGridConfig = {
    ...
    defaultButtons: {
        ...
        export: {
            ...
            action: function(tableInstance) {
                exportToFormat(tableInstance, 'csv');
            }
        },
        import: {
            id: 'importData',
...
var JqGridUtils = {
    // Export grid data to different formats
    exportToFormat: function(tableInstance, format) {
        var grid = tableInstance.$grid;
        var colModel = grid.jqGrid('getGridParam', 'colModel');
        var data = grid.jqGrid('getGridParam', 'data');
        
        var filename = tableInstance.tableName + '_export_' + new Date().toISOString().slice(0,10);
        
        if (format === 'csv') {
            this.exportToCsv(grid, colModel, data, filename + '.csv');
        } else if (format === 'excel') {
            this.exportToExcel(grid, colModel, data, filename + '.xlsx');
        } else if (format === 'json') {
            this.exportToJson(grid, colModel, data, filename + '.json');
        }
    },
    
    // CSV export
    exportToCsv: function(grid, colModel, data, filename) {
        var csvContent = '';
        
        // Add header row
        var headerRow = [];
        for (var i = 0; i < colModel.length; i++) {
            if (!colModel[i].hidden && colModel[i].name !== 'cb' && colModel[i].name !== 'actions') {
                headerRow.push('"' + (colModel[i].label || colModel[i].name) + '"');
            }
        }
        ...
        for (var j = 0; j < data.length; j++) {
            ...
            for (var k = 0; k < colModel.length; k++) {
                if (!colModel[k].hidden && colModel[k].name !== 'cb' && colModel[k].name !== 'actions') {
                    var cellValue = data[j][colModel[k].name];
                    if (cellValue !== null && cellValue !== undefined) {
                        // Escape double quotes and wrap in quotes
                        dataRow.push('"' + String(cellValue).replace(/"/g, '""') + '"');
                    } else {
                        dataRow.push('""');
                    }
                }
