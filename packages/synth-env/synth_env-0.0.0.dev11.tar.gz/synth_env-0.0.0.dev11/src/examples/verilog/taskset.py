...
async def create_verilog_taskset(max_instances: int = 10) -> TaskInstanceSet:
    ...
    # Determine dataset size - if max_instances is None, use full dataset
    ...
    # Create split info - 60% train, 20% validation, 20% test
    total_instances = len(instances)
    train_split = int(0.6 * total_instances)
    val_split = int(0.8 * total_instances)  # 60% + 20% = 80%
    
    train_ids = {inst.id for inst in instances[:train_split]}
    val_ids = {inst.id for inst in instances[train_split:val_split]}
...
module adder4_tb;
    ...
    return VerilogTaskInstance(
        ...
        initial_engine_snapshot=None,
        pristine_dir=str(pristine_dir),
        snapshot_dir=(lambda: (_temp_dirs.append(d := tempfile.mkdtemp(prefix=f"verilog_snapshot_{instance_id}_")), d)[1])()
    )
