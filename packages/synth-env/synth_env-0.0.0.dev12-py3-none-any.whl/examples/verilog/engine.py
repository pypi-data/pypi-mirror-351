...
class VerilogPublicState:
    ...
    last_custom_test_output: Optional[Dict[str, Any]] = None
...
class VerilogEngine(StatefulEngine):
    ...
    def __init__(self, task_instance: TaskInstance):
        ...
        # Track last compile/simulate/custom test outputs
        self._last_compile_output: Optional[str] = None
        self._last_simulate_output: Optional[str] = None
        self._last_custom_test_output: Optional[Dict[str, Any]] = None
    ...
    async def _reset_engine(self, *, seed: Optional[int] = None) -> Tuple[VerilogPrivateState, VerilogPublicState]:
        ...
        self._last_custom_test_output = None
        ...
        pub = VerilogPublicState(
            ...
            last_custom_test_output=self._last_custom_test_output
        )
    ...
    async def _step_engine(self, action_result: Dict[str, Any]) -> Tuple[VerilogPrivateState, VerilogPublicState]:
        ...
        # Update last outputs if this is a compile, simulate, or custom test action
        ...
        elif action_result.get("type") == "run_custom_test":
            self._last_custom_test_output = action_result
        ...
        reward_from_stack = await self.reward_stack.step_reward(
            state=current_pub_state, 
            action=self._current_action_for_reward
        )
        self._current_action_for_reward = None
    ...
    async def compile(self, sources: Optional[list] = None, testbench: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def simulate(self, binary: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Handle binary path properly - if relative, make it relative to build_dir
        if binary is None:
            bin_path = self.build_dir / "a.out"
        else:
            # If binary is just a filename (like "a.out"), assume it's in build_dir
            if "/" not in binary and "\\" not in binary:
                bin_path = self.build_dir / binary
            else:
                bin_path = Path(binary)
        
        try:
            # Fix: Run vvp with explicit cwd to ensure proper subprocess output capture
            # The bug was that vvp sometimes produces empty stdout when run with absolute paths
    ...
    async def run_custom_test(self, test_file: str, design_files: Optional[list] = None) -> Dict[str, Any]:
        """Compile and run agent-written test against design files."""
        if not self.snapshot_dir or not self.build_dir:
            return {"ok": False, "error": "Directories not initialized"}
            
        # Validate test file exists
        test_path = self.snapshot_dir / test_file
        if not test_path.exists():
            return {"ok": False, "error": f"Test file {test_file} not found"}
            
        # Get design files - default to all .v files except testbenches if not specified
        if design_files is None:
            design_files = []
            for p in self.snapshot_dir.glob("*.v"):
                # Skip files that look like testbenches
                filename = p.name.lower()
                if not (filename.endswith("_tb.v") or filename.endswith("_test.v") or "testbench" in filename):
                    design_files.append(str(p.relative_to(self.snapshot_dir)))
        
        # Create paths for design files and validate they exist
        design_paths = []
        for design_file in design_files:
            design_path = self.snapshot_dir / design_file
            if design_path.exists():
                design_paths.append(design_path)
            else:
                return {"ok": False, "error": f"Design file {design_file} not found"}
        
        # Create custom test binary name
        test_name = test_path.stem
        custom_binary = self.build_dir / f"{test_name}_test.out"
        
        # Compile test with design files
        cmd = ["iverilog", "-g2012", "-o", str(custom_binary), str(test_path)] + [str(p) for p in design_paths]
        
        try:
            compile_proc = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
            
            if compile_proc.returncode != 0:
                return {
                    "ok": False,
                    "error": "Custom test compilation failed",
            ...
            # Fix: Use cwd to ensure proper subprocess output capture
            sim_proc = subprocess.run(
                ["vvp", custom_binary.name], 
                cwd=str(custom_binary.parent),
                capture_output=True, 
                text=True, 
                timeout=30
            )
            ...
            passed = (
                "ALL_TESTS_PASSED" in stdout or
                ("Mismatches: 0 " in stdout and "samples" in stdout) or
                ("no mismatches" in stdout.lower() and "errors" not in stdout.lower()) or
                sim_proc.returncode == 0  # Also consider clean exit as success
            )
            ...
            return {
                ...
                "passed": passed,
                "binary": str(custom_binary)
            }
            
        except subprocess.TimeoutExpired:
            return {"ok": False, "error": "Custom test timeout", "type": "run_custom_test"}
        except Exception as e:
            return {"ok": False, "error": str(e), "type": "run_custom_test"}
