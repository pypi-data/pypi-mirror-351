...
class RunCustomTestArgs(BaseModel):
    test_file: str = Field(description="Path to the custom test file")
    design_files: Optional[List[str]] = Field(None, description="List of design files to test against")
    reasoning: str = Field(description="Reasoning for running custom test")
...
class RunCustomTest(EnvToolCall):
    def __init__(self, test_file: str, design_files: Optional[List[str]] = None):
        super().__init__(tool="run_custom_test", args={"test_file": test_file, "design_files": design_files})
...
def format_obs_for_llm(obs: Dict[str, Any]) -> str:
    ...
    custom_test_status = obs.get("custom_test_status", "")
    if custom_test_status:
        status_info += f"Custom test status: {custom_test_status}\n"
    else:
        status_info += "Custom test status: No custom tests run\n"
        ...
        self.tools = [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "parameters": RunCustomTestArgs.model_json_schema(),
                },
            },
            {
                "type": "function",
...
- run_custom_test: Write and run your own tests against design files
- submit: Submit solution when complete
- terminate: End if task complete or cannot proceed

Note: You can write custom testbenches to verify your design logic before running the official testbench.
Always use the tools available. Include reasoning in your tool calls."""
        ...
        try:
            ...
            if hasattr(tool_call, 'function'):
                ...
                return {"action": "terminate", "args": {"reason": f"Unexpected tool call format: {type(tool_call)}"}}
...
async def run_verilog_episode(task_instance: VerilogTaskInstance, model_name: str, debug: bool = False) -> bool:
    ...
    try:
        ...
        for turn in range(agent.max_turns):
            ...
            elif action_name == "run_custom_test":
                tool_call = RunCustomTest(action_args["test_file"], action_args.get("design_files"))
...
async def eval_verilog_react(
    ...
    # Use tqdm to track task completion, not instance completion
    ...
    for task_instance in tqdm(taskset.instances, desc="Tasks", unit="task"):
        ...
        task_coroutines = [
            run_verilog_episode(task_instance, model_name, debug=debug_mode)
            for _ in range(n_instances)
        ]
        
        # Run all instances in parallel without progress tracking
...
async def run_parallel_evaluation(
    ...
    print(f"Starting parallel evaluation for {len(models_to_test)} models...")
...
if __name__ == "__main__":
    ...
    asyncio.run(
        run_parallel_evaluation(
            models_to_test=["gpt-4.1-mini"],  # , "gpt-4.1-mini", "gpt-4.1"
            n_instances=10,
        )
    )