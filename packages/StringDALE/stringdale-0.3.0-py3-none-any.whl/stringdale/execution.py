...
# %% auto 0
__all__ = ['logger', 'decision_logger', 'flow_logger', 'simplify_output', 'TruncatedPrettyPrinter', 'Trace', 'NodeState',
...
def merge_pydantic_models(m1, m2):
    merged_dict = always_merger.merge(m1.model_dump(exclude_unset=True),m2.model_dump(exclude_unset=True))
    return m1.model_validate(merged_dict)
...
def load_external_state(self:Diagram,outside_state=None):
    ...
    if not isinstance(outside_state,(dict,BaseModel)):
        raise ValueError(f"State must be a dict or BaseModel or None, got type{type(outside_state)} for {outside_state}")


    state_class = self.schema.state_class    
    init_state = self.state is None and isinstance(outside_state,dict)
    assign_state = self.state is None and isinstance(outside_state,BaseModel)
    add_to_state = self.state is not None and isinstance(outside_state,dict)
    merge_state = self.state is not None and isinstance(outside_state,BaseModel)

    if init_state:
        return state_class(**outside_state)
    elif assign_state:
        ...
        raise ValueError(f"Shouldnt get here")
