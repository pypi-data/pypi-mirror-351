...
def format_dialogs(dialog_record, keyword="", history_action_list=None):
    ...
    if len(dialog_record) > 1:
        use_history_action = True if "action" in dialog_record[1].keys() else False
...
def init_template(faq_answers, kn_answer_action, current_round, action2comb_map, logger):
    ...
    if len(valid_knowledge) > 0:
        ...
        for action in valid_kn_action[0].split("、"):  # ["A、B"]
            if "-" not in action:
                if "介绍" in action:
                    template_types.append("答疑")
                elif "套电" in action or "套联" in action:
...
def repl(text, prompt, intent, domain):
    ...
    if domain == "hair":
        text = str(text).replace("新生植发", "雍禾植发").replace("碧莲盛", "雍禾").replace(
            "雍禾是国企公立医院", "雍禾是上市公司")
    elif domain == "dentistry":
        text = str(text).replace("布洛芬", "药").replace("甲硝锉", "药").replace("甲肖锉", "药").replace(
            "头孢克肟", "消炎药").replace("头孢拉定", "消炎药").replace("头孢", "消炎药").replace(
            "阿莫西林", "消炎药").replace("甲硝唑", "药").replace("牙博士", "")
    if re.search(r"去.{0,3}医院", text):
        pos = re.search(r"去.{0,3}医院", text).span()[0]
        text = text[:pos] + "来" + text[pos + 1:]
    if "不要来" not in text and "qq" not in text:
        text = text.replace("不要来", "来")
    text = text.replace("【百度AI机器人】", "")
    text = text.replace("没有办法做", "比较难").replace("彻底", "").replace("邮箱", "联系方式")
    if "价格" in prompt or "活动" in prompt or "价格" in intent:
        text_temp = copy.copy(text)
        text = re.sub("([1-9]\d*)", "**", text_temp)
        if contains(items=["**根", "**."], targets=text):  # 如果是几根头发这种情况，就撤回数字替换的逻辑
            text = text_temp
    text = re.sub("(1[3-9]\d{9}|1[3-9]\d{1}[-.,]{0,1}\d{4}[-.,]{0,1}\d{4})", "[[KICP_226_customerMobile]]", text)
    text = re.sub("微信是([a-zA-Z]{1}[-_a-zA-Z0-9]{5,19})", "微信是 [[KICP_226_customerWeixin]]", text)
    text = text.replace("<mobile>", "[[KICP_226_customerWeixin]]")
    if re.search("微信是(:|：|)[\[【]{0,2}([a-zA-Z]{1}[-_a-zA-Z0-9]{5,19})", text) is None and \
            re.search("微信是(:|：|)", text) and re.search("微信是(:|：|)(多少|？|什么)", text) is None:
        text = re.sub("微信是(:|：|)", "微信是 [[KICP_226_customerWeixin]]", text)
    text = str(text).replace("<|im_start|>", "").replace("<|im_end|>", "").replace(
        "<|endoftext|>", "").replace("</s>", "").replace("<unk>", "").replace(" ", "").replace(
        "&nbsp;", "").replace("u0026", "").replace("<SEP>", "<sep>").replace("\xa0", "").replace("xa0", "").replace(
        "短信", "电话")
    if "user" in text:
