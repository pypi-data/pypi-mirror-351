...
5.回复的文本与对话历史中最近的访客说的话是对同一个主题进行的讨论，答疑需要以访客上文意图+病种主题相结合去回答，不能偏题，并且逻辑关系越好，则认为回复越好，否则存在重复问类似问题、逻辑矛盾、答非所问的情况都是不好。
6.回复的文本如果太短，或者意思表达不充分，则认为该回复不好。
7.回复的文本对于价格、优惠、地址、医院名称、药品等信息模糊的描述相较于具体的描述更好，也就是回复中尽量不要出现类似“250元”、“九折”、“厦门中诺口腔”这样的具体的个性化信息。
8.回复的文本准确的知识越充分并且知识一定要逻辑合理，则认为回复越好。
9.回复的文本与对话历史的衔接，答疑的关联性，问诊的逻辑承接，套电的关联性越好，则认为回复越好。
10.回复的文本中包含有助于推进对对话的问诊或者合适套电时机就进行套电会更好。
11.回复的文本表达相对完整的，字符数不少于15，并且不超过80的，则认为回复更合理一些。
12.上一轮访客对客服的询问联系方式的话，进行”好的“、”行“这样有留联意愿的表达时，则回复的文本中包含询问联系方式或者微信的表达更好。
13.如果对话历史中给访客已经给了联系方式（包含电话/微信），则回复的文本中包含确认联系方式、继续确认访客基本信息或者预约相关的话术更好。
14.回复如果是询问联系方式的意思，那么与访客最关心的问题关联性越好，并且能具备对访客的好处，则认为该回复越好，一定不能只是”方便加个微信吗“这样的话。
15.排序需要分开对回复文本中的问诊、答疑、套电等分句分别进行评估，只要有其中一个分句不好，那整个回复就算不好。
16.回复的文本只要存在缺陷就一定不能在输出排序中排在第一位，确保排序第一的回复文本即便没有突出的优点也必须是没有任何缺点的。
17.如果回复的候选集合只有一个样本，那就直接输出序号1。
18.确保排序从好到不好的顺序是从左到右。
