...
from fleet_control.utils.python_utils import ColorFormatter, load_json, converter, select_option, COLORS, convert_values_to_strings
...
def cli(log_format, silent, minimal_logs, log_level):
    ...
    if silent:
        ...
        console_handler.setFormatter(ColorFormatter(fmt="{message}"))
    else:
        ...
        if log_format:
            console_handler.setFormatter(ColorFormatter(fmt=log_format))
...
def _change(variables, targets: tuple, file):
    """
    ...
    change(variables, set(targets_set))
...
@click.option("--output-yaml", "-y", is_flag=True, help="Output the result in YAML format instead of JSON")  # Add this flag
def _get(target_id: str, variable_name: str, output: str, custom: bool, all_vars: bool, output_yaml: bool):
    """
    ...
    Example with YAML output: fct get --yaml 4X001 VAR_NAME
    """
    ...
    if isinstance(result, str):
        ...
        if output_yaml:  # Handle YAML output
...
@click.argument("targets", type=str, nargs=-1)
@click.option("--file", type=click.Path(exists=True, readable=True), help="Path to a file containing variables.")
def _delete(variables, targets: tuple, file):
    """
