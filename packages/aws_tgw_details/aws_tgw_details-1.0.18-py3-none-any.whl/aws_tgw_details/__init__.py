...
def get_tgw_attachments(
    ...
    if tgw_attachment_id is not None:
        if log_level < 30:
...
def get_tgw_vpn_connections(
    client,
    tgw_id: str,
    tgw_vpn_connection_id: Optional[str],
    log_level: int
) -> list:
...
def login_to_aws(
    ...
    aws_session_token = os.environ.get("TGW_AWS_SESSION_TOKEN")
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        if tgw_vpn_connection_id is not None:
        ...
        if (
            tgw_id is not None
            and cgw_id is None
            and tgw_rt_id is None
            and tgw_attachment_id is None
            and tgw_vpn_connection_id is None
        ):
            tgws = get_tgws(
                client,
                tgw_id,
