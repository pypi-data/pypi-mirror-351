...
class bstack1111l11l11_opy_:
    ...
    bstack1111l1111l_opy_: Union[None, Callable]
    def __init__(self, timeout=1, bstack1111l11111_opy_=1, bstack11111lllll_opy_=None, bstack1111l1111l_opy_=None):
    ...
    def enqueue(self, job: Callable):
        if not callable(job):
            raise ValueError(bstack1ll_opy_ (u"ࠢࡪࡰࡹࡥࡱ࡯ࡤࠡ࡬ࡲࡦ࠿ࠦࠢူ") + type(job))
        self.queue.put(job)
    def start(self):
        if self.threads:
            return
        self.threads = [threading.Thread(target=self.worker, daemon=True) for _ in range(self.bstack1111l11111_opy_)]
            ...
            try:
                ...
                try:
                    ...
                    if callable(self.bstack11111lllll_opy_):
                        self.bstack11111lllll_opy_(e, job)
                finally:
                    self.queue.task_done()
            except queue.Empty:
                pass
            except Exception as e:
                if callable(self.bstack1111l1111l_opy_):
                    self.bstack1111l1111l_opy_(e)