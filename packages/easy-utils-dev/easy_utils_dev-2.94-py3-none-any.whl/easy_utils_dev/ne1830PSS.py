...
class PSS1830 :
    ...
    def ssh_execute(self , ssh ,command ,  merge_output=False , hide_output=False , supress_error=True) :
    ...
    def newUserPwdReset(self):
        ...
        if 'Error:' in d:
            ...
            raise exceptions.PssPasswordChangeError(f"Password Change Failed due to Error: {d}")
        return d
    ...
    def activateGmreRelease(self , release ) :
        ...
        if  release.count('.') < 2 :
            ...
            raise Exception('Wrong GMRE release format.')
    ...
    def _connect(self , mode='cli' , neip=None , port=22 , rootpw='QUx1MTIj' , tryNumber=0) :
        self.client = self.createClient()
        if self.sim == True :
            port = 22 
        elif self.sim == False :
            port = 5122
        # connect to node if no jump server.
    ...
    def connect(self , mode='cli' , neip=None , port=22  , user=None , pw=None , rootpw=None,jumpserver = None , resetRequired = False, tmpPassword = None , maxReconnectAttemp=40,auto_disable_paging=True, return_channel=False) :
        if not user :
            user = 'admin'
        if not pw :
            pw = 'YWRtaW4='
        ...
        maxAttemps=maxReconnectAttemp
        for i in range(maxAttemps) :
            self.logger.info(f"try {i+1}/{maxAttemps} to connect to {neip} ...")
            self.client , error , status = self._connect(mode  , neip , port , rootpw ,tryNumber=i)
            if error == "AUTH_FAIL" :
                raise Exception(f"Authentication failure at {self.neip}")
            self.logger.debug(f"\n\tneip={neip}\n\tself.client={self.client} \n\terror='{error}' \n\tconnectionSuccess={status}")
            if mode in ['cli' , 'ssh'] :
                status_bool , neVersionResponse = self.get_neversion(supress_error=True)
                if (status and not status_bool) or (not status) :
                    if i == maxAttemps - 1 :
                        raise exceptions.NetworkElementConnectionFailure(f'{error} @{neip}')
                    self.logger.debug(f"try to connect again. pipe maybe broken", source=neip)
                    if status :
                        self.close_ssh(self.client)
            else :
                status_bool=True
            ...
            if status_bool and status :
                self.logger.debug(f'connection status and pip is not broken.. continue login process ..')
                break  
            sleep(3)
        self.connected=True
        self.logger.debug(f'''\n
    ...
    def wait_result(self , command,expect,expectError=None,errorMessage=None,autoError = True,return_untouched_buffer=False,expects=[]) :
        ...
        i=0
        self.logger.debug(f"""wait_results params:-
                command={command}
                expect={expect}
                expects={expects}
        ...
        while True:
            ...
            try :
                ...
                new_data = None
            self.logger.debug(f"""WAIT RESULT : full data -> new_data [before splitlines]:- {new_data}""")
            if new_data :
                data += new_data   
                self.full_buffer += new_data   
                self.logger.debug(f'new data entered if-s-a23')
                self.logger.debug(f'full buffer data after appending new data START==>||{data}||<==END')
                self.logger.debug(f"checking if any of the next statements are in the data ..\n\r expect={str(expect)} or {self.prompt} in the data ...")
                if expect :
                ...
                if len(expects) != 0  :
                    for elem in expects :
                        ...
                        if find3 != -1 :
                            self.logger.debug(f'expects are provided. found statemnt will be used as expect. catchedExpectedFromExepcts={elem}')
                ...
                if find1 != -1 or find2 != -1 or find3 != -1:   
                    self.logger.debug(f'new data entered if-s-a24')
                ...
                if i >= 10 :
                    ...
                    raise Exception(f'wait result in buffer timedout in node {self.neip}')
    ...
    def get_xcs(self) :
        ...
        for _ , line in enumerate(xcs) : 
            if "A-End" in line or 'OCH Trail' in line or "Admin Oper" in line or "entries" in line or " "*20 in line or "------------" in line : 
                continue
            details = line.split('  ')
