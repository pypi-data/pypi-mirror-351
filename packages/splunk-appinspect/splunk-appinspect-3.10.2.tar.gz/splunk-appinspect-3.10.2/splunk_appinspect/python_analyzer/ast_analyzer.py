...
from typing import TYPE_CHECKING, Any, Generator, Iterable, Optional, Type

# self defined modules
from splunk_appinspect.python_analyzer import ast_types, utilities
from splunk_appinspect.python_analyzer.ast_info_query import AstInfoQuery
...
class AstAnalyzer:
    ...
    def __init__(
        self,
        python_file_path: Optional[Path] = None,
        python_code_string: Optional[str] = None,
        context: Optional[ast_types.AstContext] = None,
        module_manager: Optional[object] = None,
    ):
        """
        Args:
            python_file_path: code file path.
            python_code_string: raw code string.
            context: AST context when starting traversal.

        """

        # python file path
        self._original_file_path = python_file_path
        self._module_name: Optional[Path] = Path(python_file_path).stem if python_file_path is not None else None
        # module usage
        self._module_imported_usage: dict[str, set[ast.AST]] = {}
        self._module_variable_usage: dict[str, set[ast.AST]] = {}
        self._module_function_call_usage: dict[str, set[ast.Call]] = {}
        self._module_function_call_return_value_usage: dict[str, set[ast.AST]] = {}
        # other possible usage
        self._function_call_usage: dict[str, set[ast.AST]] = {}
        self._variable_usage: dict[str, set[ast.AST]] = {}
        self._exec_usage: set[ast.AST] = set()
        self._literal_string_usage: dict[str, set[ast.AST]] = {}
        # root ast node
        self._root_ast_node: Optional[ModuleType] = None
        # python code lines
        self._python_code_lines: Optional[list[str]] = None
        # context dict, map AST node to context
        self._context_dict: dict[ast.AST, ast_types.AstContext] = {}
        # global context dict, map AST node to global context
        self._global_context_dict: dict[ast.AST, dict[str, ast_types.Copyable]] = {}
        # function vis set, used to avoid recursive function call
        self._function_visit_set = set()
        # exposed modules
        self._exposed_module_set: set[str] = set()
        # store all ast nodes' parent ast node
        self._parent_ast_node_dict: dict[ast.AST, ast.AST] = {}
        # store all ast nodes' children ast nodes
        self._children_ast_node_dict = {}
        # subtree types dict
        self._subtree_types_dict: dict[ast.AST, set[Type[ast.AST]]] = {}
        # cache variable parse result
        self._ast_node_to_variable: dict[ast.AST, ast_types.AstVariable] = {}
        # cache call nodes' module usage
        self._call_node_to_modules: dict[ast.expr, set[str]] = {}

        # Only store context for exec now, It will be used by re-parsing
        self._snapshot_ast_types: type = type("Undefined", (), {})
    ...
    def get_module_usage(self, module: str, lineno_only: bool = False, with_context: bool = False) -> list[ast.AST]:
        """
        ...
        return self._filter_result_list(list(result), lineno_only=lineno_only, with_context=with_context)

    def get_module_all_possible_usage(
        self, module: str, lineno_only: bool = False, with_context: bool = False
    ) -> list[ast.AST]:
        """
        ...
        for module_name, usage_set in iter(self._module_function_call_usage.items()):
            if self._is_prefix_module(module, module_name):
                result = result | usage_set
        for module_name, usage_set in iter(self._module_function_call_return_value_usage.items()):
            if self._is_prefix_module(module, module_name):
                result = result | usage_set
    ...
    def get_module_function_call_usage(
        ...
        function_args: Optional[dict[int, dict[str, list[str] | bool]]] = None,
        function_keywords: Optional[dict[str, dict[str, list[str] | bool]]] = None,
        lineno_only: bool = False,
        with_context: bool = False,
        fuzzy: bool = False,
    ) -> list[ast.Call]:
        """
        Args:
            fuzzy: if fuzzy collect all possible function calls related to module_name and function_name else
                find exactly function call usage.
            function_args::
                {
                    index: {
                            'arg_values': any iterator collection here,
                            'required': True
                    }
                }
        ...
        function_nodes: set[ast.Call] = set()
        if module_name in self._module_function_call_usage:
            ...
            for function_node in self._module_function_call_usage[module_name]:
                ...
                if (isinstance(function_node.func, ast.Attribute) and function_node.func.attr == function_name) or (
                    isinstance(function_node.func, ast.Name) and function_node.func.id == function_name
                ):
                    function_nodes.add(function_node)
        module_string = (
            module_name + "." + function_name if (function_name is not None and function_name != "") else module_name
        )
        ...
        if not fuzzy:
            ...
            def function_filter(function_node: ast.Call) -> bool:
                ...
                for module_name in module_names:
                    ...
                    if module_name.startswith(module_string) and module_name != module_string:
                        return False
                return True

            function_nodes: list[ast.Call] = [
                function_node for function_node in function_nodes if function_filter(function_node)
            ]

        result_list = []
        for function_node in function_nodes:
            if self._check_function_args(function_node, function_args, function_keywords):
                result_list.append(function_node)
        return self._filter_result_list(result_list, lineno_only=lineno_only, with_context=with_context)

    def get_literal_string_usage(
        self, search_pattern: str = ".*", flags: int = 0, lineno_only: bool = False, with_context: bool = False
    ) -> list[ast.AST]:
    ...
    def get_code_piece_usage(
        self, code_piece: str | bytes, lineno_only: bool = False, with_context: bool = False
    ) -> list[ast.AST]:
        """
        ...
        """
        result_list = []
        code_ast_root = ast.parse(code_piece, filename=self._original_file_path or AstAnalyzer.UNKNOWN_FILENAME)
        # only handle one module subtree
    ...
    def get_variable_distribution(
        self, variable_name: str, lineno_only: bool = False, with_context: bool = False
    ) -> list[ast.AST]:
        """Return all possible variable usages, return ast nodes by default."""
        result_list = list(self._variable_usage[variable_name]) if variable_name in self._variable_usage else []
        return self._filter_result_list(result_list, lineno_only=lineno_only, with_context=with_context)

    def get_variable_details(self, ast_node: ast.AST) -> ast_types.AstVariable:
        ...
        if isinstance(ast_node, ast.Str):
            return ast_types.AstVariable(ast_node, {ast_types.AstVariable.STRING_TYPE}, ast_node.s)
        ...
        if isinstance(ast_node, ast.JoinedStr):
            ...
            return ast_types.AstVariable(ast_node, {ast_types.AstVariable.STRING_TYPE}, joined_str)

        if isinstance(ast_node, ast.Num):
            return ast_types.AstVariable(ast_node, {ast_types.AstVariable.NUMBER_TYPE}, ast_node.n)
    ...
    def get_parent_ast_node(self, ast_node: ast.AST) -> Optional[ast.AST]:
        """
        Returns:
            ast_node's parent ast node in ast tree, None if no parent exists.

        """
        return self._parent_ast_node_dict.get(ast_node, None)

    def get_ast_types_in_subtree(self, ast_node: ast.AST) -> set[Type[ast.AST]]:
        """Return all ast types included in ast_node's subtree."""
        return self._subtree_types_dict.get(ast_node, set())

    def get_ast_nodes_lca(self, ast_nodes: list[ast.AST], target_type: type[ast.AST]) -> Optional[ast.AST]:
        """If ast_nodes share same ancestor with given ast node type, return this AST-nodes. Otherwise, return None."""
    ...
    def is_in_same_code_block(self, nodes: Iterable[ast.AST]) -> bool:
    ...
    def get_code_block(self, node: ast.AST) -> Optional[ast.AST]:
        """
        Returns:
             Current node's code block node.
    ...
    def get_call_node_modules(self, call_node: ast.expr) -> set[str]:
        """
        Returns:
            All possible modules belong to this call node in run time

        """
        return self._call_node_to_modules.get(call_node, set())

    def query(self) -> AstInfoQuery:
        # share necessary data with query, since we want to save space usage
        return AstInfoQuery(self._root_ast_node, self._parent_ast_node_dict, self._subtree_types_dict)

    @property
    def exec_usage(self):
        """Exec AST nodes and corresponding context are returned. They could be used by ast_analyzer again."""
        return self._exec_usage

    @property
    def function_call_usage(self) -> dict[str, set[ast.AST]]:
        """All normal function call usages."""
        return self._function_call_usage

    @property
    def literal_string_usage(self) -> dict[str, set[ast.AST]]:
        """All literal string usages."""
        return self._literal_string_usage

    @property
    def variable_usage(self) -> dict[str, set[ast.AST]]:
        """All variables usages."""
        return self._variable_usage

    @property
    def context_dict(self) -> dict[ast.AST, ast_types.AstContext]:
        """Context dict for all AST node, node -> context."""
        return self._context_dict

    @property
    def global_context_dict(self) -> dict[ast.AST, dict[str, ast_types.Copyable]]:
        """
    ...
    @property
    def exposed_module_set(self) -> set[str]:
    ...
    @property
    def root_ast_node(self) -> Optional[ModuleType]:
        return self._root_ast_node

    @property
    def python_code_lines(self) -> Optional[list[str]]:
        return self._python_code_lines

    @property
    def module_manager(self) -> Optional[object]:
    ...
    @content_hash.setter
    def content_hash(self, content_hash_value: str) -> str:
    ...
    def _parse_file(
        ...
        try:
            with open(python_file_path, "r", encoding="utf-8-sig") as file:
    ...
    def _parse_code(
        ...
        self._root_ast_node = ast.parse(
            python_code_string, filename=self._original_file_path or AstAnalyzer.UNKNOWN_FILENAME
        )
        init_context = ast_types.AstContext(0, self._root_ast_node) if context is None else context
    ...
    def _prev_traverse(self) -> None:
        ...
        builtin_functions = [
            ...
            if function_name.startswith("AstBuiltin") and function_name != "AstBuiltinFunction"
        ]
        self._builtin_map = {
        ...
        for builtin_function in builtin_functions:
            self._global_map["__builtins__"].global_map[builtin_function.name] = builtin_function

        self.gather_metadata(self._root_ast_node)

    def _post_traverse(self) -> None:
    ...
    def _traverse_ast(self, ast_root: ast.AST, context: ast_types.AstContext) -> None:  # noqa: C901
        ...
        while stack:
            ...
            if isinstance(node, ast.ClassDef):
                ...
                class_context.namespace = (
                    node.name if class_context.namespace == "" else class_context.namespace + "." + node.name
                )
            ...
            # enter function block or async function block
                ...
                function_context.namespace = (
                    node.name if function_context.namespace == "" else function_context.namespace + "." + node.name
                )
                ...
                for arg in list(filter(lambda arg: isinstance(arg, ast.arg), args)):
                    function_context.variable_map[arg.arg] = ast_types.AstVariable(arg, set(), None)
                for arg, default_arg in list(
                    ...
                    default_variable = self._get_variable_from_node(default_arg, context)
                ...
                if isinstance(context.root_node, ast.Module):
                    self._global_map[function_context.namespace] = ast_types.AstFunction(
                ...
                # function in function
                    ...
                    context.variable_map[node.name] = ast_types.AstVariable(
                        ...
                        ast_types.AstFunction(node.name, node, ast_types.AstVariable(None, set(), None)),
                        node.name,
                    )
            ...
            # enter exception handler block
                ...
                # just an isolated context when meeting exception handler
                ...
                if node_name_string and node_type_string:
                    exception_context.variable_map[node_name_string] = ast_types.AstVariable(node, {node_type_string})
                self._traverse_ast(node.body, exception_context)
            else:
                if isinstance(node, list):
                    if isinstance(context.root_node, ast.ClassDef):
                        function_nodes = list(filter(lambda node: isinstance(node, ast.FunctionDef), node))
                        other_nodes = list(filter(lambda node: not isinstance(node, ast.FunctionDef), node))
                        ...
                        if context.parent_context is not None:
                            if isinstance(context.parent_context.root_node, ast.Module):
                                self._global_map[context.namespace] = ast_types.AstClass(
                                ...
                                context.parent_context.variable_map[context.root_node.name] = ast_types.AstVariable(
                                    context.root_node,
                                ...
                                ast_class = context.parent_context.variable_map[context.root_node.name].variable_value
                        ...
                        if ast_class:
                            ...
                            for base_class in context.root_node.bases:
                                ...
                                self._gather_variable_names_in_ast_tree(base_class, context)
                                self._traverse_ast(base_class, context)

                                base_class_obj = self._get_variable_from_node(base_class, context)
                                if ast_types.AstVariable.is_class_instance(base_class_obj):
                                ...
                                if isinstance(base_class_obj, ast_types.AstClass):
                                    ...
                                    for variable_name, variable in iter(
                                        ...
                                        if variable_name not in ast_class.class_context.variable_map:
                                            ast_class.class_context.variable_map[variable_name] = variable
                                    # copy function if not exists
                                    for function_name, function_variable in iter(base_class_obj.function_dict.items()):
                                        if function_name not in ast_class.function_dict:
                                            ast_class.function_dict[function_name] = function_variable
                                    self._update_inverted_index_dict_with_key_set(
                                ...
                                # if class_name is not user-defined class, regard it as a module and add it to global map
                                    ...
                                    all_modules = self._get_modules_used_in_ast_tree(base_class, context)
                                    # if modules could be found in base_class
                                    if all_modules:
                                        self._gather_module_variable_usage_in_ast_tree(base_class, context)
                                        ast_class.modules |= all_modules
                                    else:
                                        nodes, strings = [], []
                                        has_func = set()
                                        current_node = base_class
                                        while hasattr(current_node, "value") or hasattr(current_node, "func"):
                                            if isinstance(current_node, (ast.Name, ast.Attribute)):
                                                nodes.append(current_node)
                                                strings.append(utilities.get_node_name_or_attr(current_node))
                                        ...
                                        if isinstance(current_node, ast.Name):
                                            ...
                                            for index in range(len(nodes)):
                                                module_string = ".".join(strings[: index + 1])
                                                self._update_inverted_index_dict(
                                                    module_string,
                                                    nodes[index],
                                                    self._module_variable_usage,
                            ...
                            for child_node in other_functions:
                                self._traverse_ast(child_node, ast_class.class_context.copy())
                    else:
                        # if it is not in class, visit all nodes one by one
                        # it could be in an if block, a module block
                        for child_node in node:
                            self._traverse_ast(child_node, context)
                else:
                    # share the same context
                    self._visit_node(node, context)
                    stack.extend(list(reversed([child_node for child_node in self.iter_child_nodes(node)])))

            if isinstance(node, ast.AST):
                self._post_visit(node, context)

    def _prev_visit(self, node: ast.AST, context: ast_types.AstContext) -> None:
    ...
    def _post_visit(self, node: ast.AST, context: ast_types.AstContext) -> None:
        pass

    def _visit_constant_node(self, node: ast.Constant, context: ast_types.AstContext) -> None:
    ...
    def _visit_node(self, node: ast.AST, context: ast_types.AstContext) -> None:
        node_class = node.__class__.__name__.lower()
        method_name = f"_visit_{node_class}_node"
        visitor = getattr(self, method_name, self._generic_visit_node)
        visitor(node, context)

    def _generic_visit_node(self, node: ast.AST, context: ast_types.AstContext) -> None:
        if not issubclass(node.__class__, ast.AST):
            raise Exception(f"unexpected node type, {type(node)}")

    # related to module import
    def _visit_import_node(self, node: ast.AST, context: ast_types.AstContext) -> None:
        """
        1. `import a`
        ...
        for alias_node in node.names:
            ...
            if alias_node.asname is not None:
                self._collect_modules_import_usage(alias_node.name, node, context, alias_node.asname)
            else:
                # import a
                self._collect_modules_import_usage(alias_node.name, node, context)

    def _visit_importfrom_node(self, node: ast.AST, context: ast_types.AstContext) -> None:
        """
        1. `from a import b, c`
        ...
        module_name = utilities.get_from_import_module_name(node, self._python_code_lines)
        # from . import b as c
        for module in node.names:
            ...
            if module.asname is not None:
                self._collect_modules_import_usage(module_string, node, context, alias_name=module.asname)
            else:
                # from x import a
                self._collect_modules_import_usage(module_string, node, context)

    def _collect_modules_import_usage(
        self, module_string: str, node: ast.AST, context: ast_types.AstContext, alias_name: Optional[str] = None
    ) -> None:  # noqa: C901
        # from xx import xx
        ...
        if not is_import_from and module_string == "__builtin__":
            ...
            if self._is_in_global_context(context):
                ...
                for function_name, builtin_function in iter(self._builtin_map.items()):
                    self._global_map["__builtin__"].global_map[function_name] = builtin_function
            else:
                # it is in local context
                context.variable_map["__builtin__"] = ast_types.AstModule("__builtin__")
                for function_name, builtin_function in iter(self._builtin_map.items()):
                    context.variable_map["__builtin__"].global_map[function_name] = builtin_function
            return
        ...
        if not is_import_from and module_string == "builtins":
            ...
            if self._is_in_global_context(context):
                ...
                for function_name, builtin_function in iter(self._builtin_map.items()):
                    self._global_map["builtins"].global_map[function_name] = builtin_function
            else:
                # it is in local context
                context.variable_map["builtins"] = ast_types.AstModule("builtins")
                for function_name, builtin_function in iter(self._builtin_map.items()):
                    context.variable_map["builtins"].global_map[function_name] = builtin_function
            return
        ...
        if self._module_manager:
            ...
            if alias_name:
                ...
                if modules:
                    assert len(modules) == 1, "load more than one modules when alias name is exists"
                    module = modules[0].copy()
                    self._store_attribute_in_current_context(alias_name, module, node, context)
                ...
                if is_import_from:
                    ...
                    if modules:
                        for module in modules:
                            ...
                            self._store_attribute_in_current_context(copy_module.name, copy_module, node, context)
                            self._collect_module_usage_in_external_module(copy_module, node)
                        return
                else:
                    # import a.b, import a
                    ...
                    if modules:
                        assert len(modules) == 1, "load more than one modules when using `import statement`"
                        module = modules[0].copy()
                        self._store_attribute_in_current_context(base_module_name, module, node, context)
                        self._collect_module_usage_in_external_module(module, node)
                        return
        ...
        if self._is_in_global_context(context):
            if (
                ...
                self._global_map[key] = ast_types.AstVariable(node, module_names, None, key)
        else:
            if (
                context.context_mode == ast_types.AstContext.COLLECT_MODE
                ...
                context.variable_map[key] = ast_types.AstVariable(node, module_names, None, key)

        self._update_inverted_index_dict_with_key_set(module_names, node, self._module_imported_usage)

    def _visit_call_node(self, node: ast.Call, context: ast_types.AstContext) -> None:
        ...
        self._update_inverted_index_dict_with_key_set(modules, node, self._module_function_call_return_value_usage)
        # now `a()()` is supported
        if isinstance(node.func, (ast.Attribute, ast.Name, ast.Call)):
            module_names = self._get_modules_used_in_ast_name_and_attribute_chain(node.func, context)
            self._update_inverted_index_dict_with_key_set(module_names, node, self._module_function_call_usage)
            ...
            if not module_names and isinstance(node.func, (ast.Attribute, ast.Name)):
                function_call = node.func.attr if isinstance(node.func, ast.Attribute) else node.func.id
                self._update_inverted_index_dict(function_call, node, self._function_call_usage)

    def _visit_assign_node(self, node: ast.Assign, context: ast_types.AstContext) -> None:
        """
    ...
    def _visit_with_node(self, node: ast.With, context: ast_types.AstContext) -> None:
        for item in node.items:
            self._handle_with_item(item, context)

    def _handle_with_item(self, node: ast.withitem, context: ast_types.AstContext) -> None:
        if node.optional_vars is not None and node.context_expr is not None:
            if isinstance(node.optional_vars, ast.Name):
                ...
                self._update_inverted_index_dict(node.optional_vars.id, node.optional_vars, self._variable_usage)
                self._ast_node_to_variable[node.optional_vars] = self._get_variable_from_node(
                    node.context_expr, context
                )
            elif isinstance(node.optional_vars, ast.Tuple):
                for target_node in filter(lambda node: isinstance(node, ast.Name), node.optional_vars.elts):
                    variable = self._get_variable_from_node(node.context_expr, context)
                    context.variable_map[target_node.id] = variable
                    self._update_inverted_index_dict(target_node.id, target_node, self._variable_usage)
                    self._ast_node_to_variable[target_node] = variable

    def _visit_set_node(self, node: ast.Set, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_list_node(self, node: ast.List, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_dict_node(self, node: ast.Dict, context: ast_types.AstContext) -> None:
        # gather information from value nodes only
        for value_node in node.values:
            self._gather_info_in_common_ast_tree(value_node, context)

    def _visit_exec_node(self, node, context: ast_types.AstContext) -> None:
        self._exec_usage.add(node)

    def _visit_print_node(self, node, context: ast_types.AstContext) -> None:
    ...
    def _visit_subscript_node(self, node: ast.Subscript, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_lambda_node(self, node: ast.Lambda, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_eq_node(self, node: ast.Eq, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_compare_node(self, node: ast.Compare, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_boolop_node(self, node: ast.boolop, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_unaryop_node(self, node: ast.unaryop, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node, context)

    def _visit_yield_node(self, node: ast.Yield, context: ast_types.AstContext) -> None:
        """yield xxx"""
        self._visit_return_node(node, context)

    def _visit_return_node(self, node: ast.Return, context: ast_types.AstContext) -> None:
        # return xxx
            ...
            if isinstance(context.root_node, ast.FunctionDef):
                ...
                if (context.parent_context is not None) and (
                    ...
                    if (
                        ...
                        and ast_types.AstVariable.is_class_instance(class_context.variable_map[class_name])
                        and function_name in class_context.variable_map[class_name].variable_value.function_dict
                    ):
                        function = class_context.variable_map[class_name].variable_value.function_dict[function_name]
                        temp_variable_type_set = function.return_value.variable_type_set - set(
                            ast_types.AstVariable.SPECIAL_TYPES
                        )
                        function.return_value = ast_variable
                        function.return_value.variable_type_set |= temp_variable_type_set
                    else:
                        if class_name in self._global_map and isinstance(
                            ...
                            if function_name in class_obj.function_dict:
                                temp_variable_type_set = class_obj.function_dict[
                                    function_name
                                ].return_value.variable_type_set - set(ast_types.AstVariable.SPECIAL_TYPES)
                                class_obj.function_dict[function_name].return_value = ast_variable
                                class_obj.function_dict[
                                    function_name
                                ].return_value.variable_type_set |= temp_variable_type_set
                else:
                    function_name = context.root_node.name
                    if context.parent_context is not None and isinstance(context.parent_context.root_node, ast.Module):
                        if function_name in self._global_map and isinstance(
                            ...
                            self._global_map[function_name].return_value.variable_type_set |= temp_variable_type_set
                    elif (
                        context.parent_context is not None
                        and function_name in context.parent_context.variable_map
                        and ast_types.AstVariable.is_function(context.parent_context.variable_map[function_name])
                    ):
                        temp_variable_type_set = context.parent_context.variable_map[
                            function_name
                        ].variable_value.return_value.variable_type_set - set(ast_types.AstVariable.SPECIAL_TYPES)
                        context.parent_context.variable_map[function_name].variable_value.return_value = ast_variable
                        context.parent_context.variable_map[
                            function_name
                        ].variable_value.return_value.variable_type_set |= temp_variable_type_set

    def _visit_delete_node(self, node: ast.Delete, context: ast_types.AstContext) -> None:
    ...
    def _visit_global_node(self, node: ast.Global, context: ast_types.AstContext) -> None:
    ...
    def _visit_for_node(self, node: ast.For, context: ast_types.AstContext) -> None:
        # it is another type of assignation
        self._visit_variable_node_and_value_node(node.target, node.iter, context)

    def _visit_str_node(self, node: ast.Str, context: ast_types.AstContext) -> None:
        # TODO Possible encoding issue is as follow, but it is not a blocking issue
        # `UnicodeWarning: Unicode equal comparison failed to convert both arguments to Unicode - interpreting them as being unequal`
        self._collect_literal_string_usage(node, context)
        self._collect_sensitive_string_usage(node, context)

    def _visit_name_node(self, node: ast.Name, context: ast_types.AstContext) -> None:
        self._collect_literal_string_usage(node, context)
        self._collect_sensitive_string_usage(node, context)

    def _visit_attribute_node(self, node: ast.Attribute, context: ast_types.AstContext) -> None:
        self._collect_literal_string_usage(node, context)
        self._collect_sensitive_string_usage(node, context)

    def _visit_binop_node(self, node: ast.BinOp, context: ast_types.AstContext) -> None:
        self._collect_literal_string_usage(node, context)
        self._collect_sensitive_string_usage(node, context)
    ...
    def _visit_await_node(self, node: ast.Await, context: ast_types.AstContext) -> None:
        self._gather_info_in_common_ast_tree(node.value, context)

    # helper functions

    def _gather_info_in_common_ast_tree(self, node: ast.AST, context: ast_types.AstContext) -> None:
    ...
    def _gather_module_variable_usage_in_ast_tree(self, node: ast.Module, context: ast_types.AstContext) -> None:
        for ast_node in self.walk(node):
            # only watch name and attribute ast nodes
            if isinstance(ast_node, (ast.Name, ast.Attribute)):
                module_names = self._get_modules_used_in_ast_name_and_attribute_chain(ast_node, context)
                self._update_inverted_index_dict_with_key_set(module_names, ast_node, self._module_variable_usage)

    def _visit_variable_node_and_value_node(
        self, variable_node: ast.expr, value_node: ast.expr, context: ast_types.AstContext
    ) -> None:
        if isinstance(variable_node, (ast.Name, ast.Attribute)):
            ...
            if isinstance(value_node, ast.Tuple):
                if len(variable_node.elts) == len(value_node.elts):
                    for variable_node_next, value_node_next in zip(variable_node.elts, value_node.elts):
                        self._visit_variable_node_and_value_node(variable_node_next, value_node_next, context)
            else:
                # flow data to all possible tuple elements
                for element in variable_node.elts:
                    self._visit_variable_node_and_value_node(element, value_node, context)
        else:
    ...
    def _assign_variable(self, variable_node: ast.expr, value_node: ast.expr, context: ast_types.AstContext) -> None:
        # check if node is class variable based on variable name
        variable_node_name, is_class_variable = self._get_variable_node_name(variable_node, context)
        ...
        if variable_node_name is not None:
            ...
            for index, name in enumerate(variable_node_name_list):
                ...
                if index == len(variable_node_name_list) - 1:
                    ...
                    if context.context_mode == ast_types.AstContext.COLLECT_MODE and name in current_variable_map:
                        ...
                        # it is a variable assignation, some extra works included here
                            ...
                            current_node.variable_value_node = target_variable.variable_value_node
                            temp_variable_type_set = current_node.variable_type_set - set(
                                ast_types.AstVariable.SPECIAL_TYPES
                            )
                            current_node.variable_type_set = temp_variable_type_set | target_variable.variable_type_set
                    else:
                ...
                # name in call chain
                    ...
                    if name in current_variable_map:
                        current_variable_map = self._get_variable_namespace(current_variable_map[name])
    ...
    def _get_variable_from_node(
        self, node: ast.AST, context: ast_types.AstContext
    ) -> ast_types.AstVariable:  # noqa: C901
        # special check for primitive data type, eg: `str` and `int`
        if isinstance(node, ast.Str):
            return ast_types.AstVariable(node, {ast_types.AstVariable.STRING_TYPE}, node.s)

        if isinstance(node, ast.Num):
            return ast_types.AstVariable(node, {ast_types.AstVariable.NUMBER_TYPE}, node.n)
        ...
        if isinstance(node, ast.BinOp) and isinstance(node.op, ast.Add):
            ...
            if ast_types.AstVariable.is_string(left_variable) and ast_types.AstVariable.is_string(right_variable):
                string_result = left_variable.variable_value + right_variable.variable_value
                return ast_types.AstVariable(node, {ast_types.AstVariable.STRING_TYPE}, string_result)
        ...
        if isinstance(node, ast.Lambda):
            ...
            for arg, default_arg in list(
                ...
                lambda_context.variable_map[arg.arg] = self._get_variable_from_node(default_arg, context)
            ...
            if self._access_class_attribute_with_self(node_name_list, context):
                ...
                if node_list:
                    ...
                    if start_name in context.parent_context.variable_map:
                        current_ast_variable = context.parent_context.variable_map[start_name]
                    ...
                    # function call
                        ...
                        and start_name in self._global_map[context.parent_context.namespace].function_dict
                    ):
                        current_ast_variable = self._global_map[context.parent_context.namespace].function_dict[
                            start_name
                        ]
                        could_start_parse = True
                    # nested class function call
                        ...
                        if context.parent_context.root_node.name in outer_context.variable_map:
                            class_obj = outer_context.variable_map[context.parent_context.root_node.name]
                            if ast_types.AstVariable.is_class_instance(class_obj):
                                ...
                                if start_name in class_obj.class_context.variable_map:
                                    current_ast_variable = class_obj.class_context.variable_map[start_name]
                                    could_start_parse = True
                                elif start_name in class_obj.function_dict:
                                    current_ast_variable = class_obj.function_dict[start_name]
                                    could_start_parse = True
                    ...
                    # self node only
                    if context.parent_context.namespace in self._global_map and isinstance(
                        self._global_map[context.parent_context.namespace],
                        ast_types.AstClass,
                    ):
                        return ast_types.AstVariable(
        ...
        if isinstance(current_node, ast.Call):
            modules = self._get_modules_used_in_ast_name_and_attribute_chain(current_node, context)
        else:
    ...
    def _parse_call_chain(  # noqa: C901
        ...
        current_ast_variable: ast_types.AstVariable,
        current_node: ast.AST,
        context: ast_types.AstContext,
        node_to_call_dict: dict[ast.AST, ast.Call],
    ) -> Optional[ast_types.AstVariable]:
        ...
        for node in node_list:
            ...
            if isinstance(current_ast_variable, ast_types.AstClass):
                ...
                if current_node_name in current_ast_variable.class_context.variable_map:
                    current_ast_variable = current_ast_variable.class_context.variable_map[current_node_name]
                # function call
                elif current_node_name in current_ast_variable.function_dict:
                    current_ast_variable = current_ast_variable.function_dict[current_node_name]
                else:
                    parse_succeed = False
                    break
            # class variable
                ...
                if current_node_name in ast_class.class_context.variable_map:
                    current_ast_variable = ast_class.class_context.variable_map[current_node_name]
                # function call
            ...
            # module
                ...
                if current_node_name in current_ast_variable.global_map:
                    current_ast_variable = current_ast_variable.global_map[current_node_name]
                else:
                    parse_succeed = False
                    break
            # module variable
                ...
                if current_node_name in current_ast_variable.variable_value.global_map:
                    current_ast_variable = current_ast_variable.variable_value.global_map[current_node_name]
                else:
                    parse_succeed = False
                    break
            else:
            ...
            current_ast_variable = self._propagate_ast_variable(current_ast_variable, node, node_to_call_dict, context)

        # if we parse all nodes succeeded
        if parse_succeed:
            last_call_node = node_to_call_dict[current_node] if current_node in node_to_call_dict else current_node
            # special check for AstClass, instantiate a new namespace
            if isinstance(current_ast_variable, ast_types.AstClass) or ast_types.AstVariable.is_class_instance(
                current_ast_variable
            ):
                ...
                if node_to_call_dict.get(current_node, None) is not None:
                    ...
                    # don't need to copy here, since it is an alias of current class
                    ast_class = current_ast_variable
                ast_class.class_context.context_mode = ast_types.AstContext.SIMULATE_MODE
                ...
                return ast_types.AstVariable(last_call_node, variable_type_set, ast_class)
    ...
    def _gather_variable_names_in_ast_tree(self, root_node: ast.expr, context: ast_types.AstContext) -> None:
        for ast_node in self.walk(root_node):
            # eg: a.func(), a.func is not acceptable
            # a.func(x, y), x and y should be acceptable
            if (not isinstance(self._parent_ast_node_dict.get(ast_node, None), ast.Call)) or self._parent_ast_node_dict[
                ast_node
            ].func != ast_node:
                variable_name = utilities.get_name_from_attribute_chain(ast_node)
                if variable_name:
                    self._update_inverted_index_dict(variable_name, ast_node, self._variable_usage)
                    variable = self._get_variable_from_node(ast_node, context)
                    self._ast_node_to_variable[ast_node] = variable

    def _get_variable_node_name(self, node: ast.expr, context: ast_types.AstContext) -> tuple[str, bool]:
    ...
    def _get_modules_used_in_ast_name_and_attribute_chain(
        self, node: ast.AST, context: ast_types.AstContext
    ) -> set[str]:  # noqa: C901
        """
        ...
        if isinstance(node, ast.Name):
            ...
            if self._access_class_attribute_with_self(node_name_list, context):
                ...
                if not could_start_parse:
                    ...
                    if context.parent_context.namespace in self._global_map and isinstance(
                        ...
                        result_set |= self._global_map[context.parent_context.namespace].modules
                        could_start_parse = True
                        parse_start_index = 0
                    else:
                        if context.parent_context.parent_context is not None:
            ...
            if could_start_parse:
                ...
                for i in range(parse_start_index, len(node_name_list)):
                    ...
                    if isinstance(current_ast_variable, ast_types.AstClass):
                        if node_name in current_ast_variable.class_context.variable_map:
                            current_ast_variable = current_ast_variable.class_context.variable_map[node_name]
                        elif node_name in current_ast_variable.function_dict:
                            current_ast_variable = current_ast_variable.function_dict[node_name]
                        else:
                            break
                    # class instance variable
                    elif ast_types.AstVariable.is_class_instance(current_ast_variable):
                        if node_name in current_ast_variable.variable_value.class_context.variable_map:
                            current_ast_variable = current_ast_variable.variable_value.class_context.variable_map[
                                node_name
                            ]
                        elif node_name in current_ast_variable.variable_value.function_dict:
                            current_ast_variable = current_ast_variable.variable_value.function_dict[node_name]
                        else:
                            break
                    # module definition
                    elif isinstance(current_ast_variable, ast_types.AstModule):
                        if node_name in current_ast_variable.global_map:
                            current_ast_variable = current_ast_variable.global_map[node_name]
                        else:
                            break
                    # module variable
                    elif ast_types.AstVariable.is_module(current_ast_variable):
                ...
                if isinstance(current_ast_variable, ast_types.AstClass):
                    ...
                    result_set |= {
                        ast_types.AstVariable.BUILTIN_TYPE,
                        ast_types.AstVariable.BUILTIN_TYPE + "." + current_ast_variable.name,
                    }
                    candidate_prefixes |= {
                        ast_types.AstVariable.BUILTIN_TYPE,
                        ast_types.AstVariable.BUILTIN_TYPE + "." + current_ast_variable.name,
                ...
                # builtin is an exception(used by python check), so retain this type
                candidate_prefixes -= set(ast_types.AstVariable.SPECIAL_TYPES) - {ast_types.AstVariable.BUILTIN_TYPE}
    ...
    def _get_modules_used_in_ast_tree(self, tree_node: ast.AST, context: ast_types.AstContext) -> set[str]:
        result_set = set()
        stack = deque([tree_node])
        while stack:
            node = stack.pop()
            module_names = self._get_modules_used_in_ast_name_and_attribute_chain(node, context)
    ...
    def _check_function_args(
        self,
        node: ast.Call,
        function_args: dict[int, dict[str, list[str] | bool]],
        function_keywords: dict[str, dict[str, list[str] | bool]],
    ) -> bool:
        # check function args
            ...
            for keyword_name, keyword_value_dict in iter(function_keywords.items()):
                if keyword_name in keyword_arg_dict:
                    if not self._contains_same_ast_tree_in_node_set(
                        ...
                        self._get_expr_ast_node_set(keyword_value_dict.get("arg_values", [])),
                    ):
                        return False
                else:
    ...
    def _contains_same_ast_tree_in_node_set(self, node1: ast.expr, node_set: set[ast.expr]) -> bool:
        for node in node_set:
            if utilities.is_same_ast_tree(node1, node, node1_extra=False):
                return True
        return False

    def _get_expr_ast_node_set(self, ast_strings: list[str]) -> set[ast.expr]:
        node_set = set()
    ...
    def _get_expr_ast_node(self, ast_string: str) -> Optional[ast.expr]:
        try:
            ast_tree = ast.parse(ast_string, filename=self._original_file_path or AstAnalyzer.UNKNOWN_FILENAME)
    ...
    def _is_prefix_module(self, prefix_module: str, module: str) -> bool:
    ...
    def _get_parent_chain_length(self, node: ast.AST) -> int:
        length = 0
    ...
    def _collect_sensitive_string_usage(self, node: ast.AST, context: ast_types.AstContext) -> None:
        ...
        if ast_types.AstVariable.is_string(variable):
            ...
            if string in self._module_imported_usage:
                self._update_inverted_index_dict(string, node, self._module_variable_usage)

    def _collect_literal_string_usage(self, node: ast.AST, context: ast_types.AstContext) -> None:
        variable = self._get_variable_from_node(node, context)
        if ast_types.AstVariable.is_string(variable):
            string = variable.variable_value
            self._update_inverted_index_dict(string, node, self._literal_string_usage)

    def _filter_result_list(self, result_list: list[ast.AST], lineno_only: bool, with_context: bool) -> list[ast.AST]:
    ...
    def _global_map_snapshot(self) -> dict[str, ast_types.Copyable]:
        new_dict = {}
    ...
    def _local_context_snapshot(self, context: ast_types.AstContext) -> ast_types.AstContext:
        new_context = ast_types.AstContext(context.level, context.root_node, context.parent_context)
    ...
    def _access_class_attribute_with_self(self, name_list: list[str], context: ast_types.AstContext) -> bool:
        if name_list:
            return name_list[0] == "self" and self._is_in_class_method(context)

        return False

    def _is_in_class(self, context: ast_types.AstContext) -> bool:
        return isinstance(context.root_node, ast.ClassDef)

    def _is_in_class_method(self, context: ast_types.AstContext) -> bool:
    ...
    def _is_in_global_context(self, context: ast_types.AstContext) -> bool:
        return isinstance(context.root_node, ast.Module)

    def _get_variable_namespace(
        self, ast_variable: ast_types.AstModule | ast_types.AstClass | ast_types.AstVariable
    ) -> Optional[dict[str, ast_types.AstVariable]]:
    ...
    def _store_attribute_in_current_context(
        self, attribute_name: str, attribute: ast_types.AstModule, ast_node: ast.AST, context: ast_types.AstContext
    ) -> None:
        # global namespace
        if self._is_in_global_context(context):
    ...
    def _collect_module_usage_in_external_module(self, module: ast_types.AstVariable, ast_node: ast.AST):
        ...
        if isinstance(module, ast_types.AstModule):
            ...
            for i in range(len(names)):
                name_prefix = ".".join(names[: i + 1])
                self._update_inverted_index_dict(name_prefix, ast_node, self._module_imported_usage)

    def _update_inverted_index_dict(
        self, key: str, node: ast.AST, inverted_index_dict: dict[str, set[ast.AST]]
    ) -> None:
    ...
    def _update_inverted_index_dict_with_key_set(
        self, key_set: set[str], node: ast.AST, inverted_index_dict: dict[str, set[ast.AST]]
    ) -> None:
        for key in key_set:
            self._update_inverted_index_dict(key, node, inverted_index_dict)

    def gather_metadata(self, root_node: ast.AST) -> None:
        """Gather node's metadata via RPO traverse"""
        types_set = {}

        def _recur(root_node: ast.AST) -> None:
            ...
            for ch_node in ast.iter_child_nodes(root_node):
                ...
                ch_subtypes = (
                    {type(ch_node)} if ch_node not in self._subtree_types_dict else self._subtree_types_dict[ch_node]
                )
    ...
    def iter_child_nodes(self, root_node: ast.AST) -> Generator[ast.AST, Any, None]:
    ...
    def walk(self, root_node: ast.AST) -> Generator[ast.AST, Any, None]:
        q = deque([root_node])
        while q:
            n = q.popleft()
    ...
    def _propagate_ast_variable(
        self,
        current_ast_variable: ast_types.Copyable,
        ast_node: ast.AST,
        node_to_call_dict: dict[ast.AST, ast.Call],
        context: ast_types.AstContext,
    ) -> Optional[ast_types.AstVariable]:
    ...
    def _is_propagate_variable(self, current_ast_variable: ast_types.Copyable) -> bool:
    ...
    def _simulate_function_call(
        self, current_ast_variable: ast_types.Copyable, call_node: ast.Call, context: ast_types.AstContext
    ) -> "AstVariableValue":
        ...
        if isinstance(
            ...
            return current_ast_variable.action(
                ...
                {keyword.arg: self._get_variable_from_node(keyword.value, context) for keyword in call_node.keywords},
                context,
            )

        raise Exception(f"Illegal AST variable `{type(current_ast_variable)}` to call")
