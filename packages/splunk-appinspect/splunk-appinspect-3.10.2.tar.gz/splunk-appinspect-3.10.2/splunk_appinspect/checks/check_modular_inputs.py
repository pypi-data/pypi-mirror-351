...
from splunk_appinspect.constants import MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION, PYTHON_LATEST_VERSION, Tags
    ...
    def __init__(self) -> None:
        super().__init__(
            config=CheckConfig(
                ...
                f" set the python.version to one of these: {MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION} as required.",
        ...
        for modular_input in modular_inputs.get_modular_inputs():
            ...
            if input_python_version is not None:
                if (
                    ...
                    and input_python_version.value not in MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION
                ):
                    yield FailMessage(
                        f"Modular input `{modular_input.name}` specifies `{input_python_version.value}` "
                        f"for `python.version`, which should be set to one of these: `{MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION}` as required.",
                        file_name=input_python_version.get_relative_path(),
                        line_number=input_python_version.get_line_number(),
                        remediation=f"Set `python.version` to one of these: `{MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION}` as required.",
                    )
                ...
                if (
                    ...
                    and global_default_python.value not in MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION
                ):
                    yield FailMessage(
                        ...
                        f"which should be set to one of these: `{MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION}` as required.",
                ...
                yield FailMessage(
                    ...
                    remediation=f"Add `inputs.conf` and set `python.version` to one of these: "
                    f"`{MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION}` as required in a "
