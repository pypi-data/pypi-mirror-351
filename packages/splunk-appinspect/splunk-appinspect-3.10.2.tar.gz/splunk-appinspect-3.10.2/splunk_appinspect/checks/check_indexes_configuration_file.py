    ...
    def __init__(self) -> None:
        super().__init__(
            config=CheckConfig(
                ...
                f"the python.version to `{PYTHON_3_VERSION}`.",
                depends_on_config=("indexes",),
                tags=(
                    Tags.SPLUNK_APPINSPECT,
                    Tags.CLOUD,
    ...
    def check_config(self, app: "App", config: "ConfigurationProxy") -> Generator[CheckMessage, Any, None]:
        for section in config["indexes"].sections():
            if section.has_option("coldToFrozenScript"):
                if not section.has_option("python.version") or section.get_option("python.version").value not in [
                    PYTHON_3_VERSION,
                    PYTHON_LATEST_VERSION,
                ]:
                    yield FailMessage(
                        f"The python.version of coldToFrozenScript should be explicitly set to "
                        f"`{PYTHON_3_VERSION}`. ",
                        file_name=config["indexes"].get_relative_path(),
