    ...
    def __init__(self) -> None:
        super().__init__(
            config=CheckConfig(
                ...
                description=f"Check that `python.version` is set to one of this : {MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION} as required, for executables in restmap.conf.",
                depends_on_config=("restmap",),
                tags=(
                    Tags.SPLUNK_APPINSPECT,
                    Tags.CLOUD,
    ...
    def check_config(self, app: "App", config: "ConfigurationProxy") -> Generator[CheckMessage, Any, None]:
        ...
        for handler in handler_list:
            ...
            if (
                ...
                and handler.python_version not in MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION
            ):
                yield FailMessage(
                    f"The handler of stanza [{handler.name}] should have `python.version` "
                    f"property set to one of this: {MODULAR_INPUT_REST_ENDPOINT_PYTHON_3_VERSION} as required.",
                    file_name=config["restmap"].get_relative_path(),
                    line_number=config["restmap"][handler.name].get_line_number(),
                )
