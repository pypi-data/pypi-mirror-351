...
from .runtime.src.fsmnvad.Speech2VadSegmentOffline import E2EVadModel
...
from .runtime.src.utils.WavFrontend import WavFrontend, WavFrontendOnline
...
class FSMNVad(object):
    def __init__(
        self, config_path=root_dir / "config/config.yaml", level="info", online=False
    ):
        ...
        if online:
            ...
            self.config["FSMN"]["model_path"] = self.config["FSMN"][
                "offline_model_path"
            ]
    ...
    def segments_offline(self, waveform_path: Union[str, Path]):
        ...
        if os.path.isfile(waveform_path):
            ...
            waveform, _sample_rate = librosa.load(waveform_path, sr=None)
        else:
            raise FileNotFoundError(str(Path))

        if _sample_rate != 16000:
            # resample to 16000
            waveform = librosa.resample(waveform, orig_sr=_sample_rate, target_sr=16000)
...
class FSMNVadOnline(FSMNVad):
    ...
    def segments_online(
        ...
        segments_part, in_cache = self.vad.infer_online(
            feats, waveform, self.prepare_cache(in_cache), is_final=is_final
        )
