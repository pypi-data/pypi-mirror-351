...
def read_and_config_file(input_path, decode=0):
    ...
    if decode:
        if os.path.isdir(input_path):
            processed_list = librosa.util.find_files(
                input_path, ext="wav"
            )  # Look for WAV files
            if len(processed_list) == 0:
                processed_list = librosa.util.find_files(
                    input_path, ext="flac"
                )  # Fallback to FLAC files
        else:
            ...
            with open(input_path) as fid:
                for line in fid:
                    ...
                    processed_list.append(
                        path_s[0]
                    )  # Append the first part (input path)
...
def reload_for_eval(model, checkpoint_dir, use_cuda):
    ...
    with open(name, "r") as f:
        model_name = (
            f.readline().strip()
        )  # Read the model name from the checkpoint file
...
def batch_pesq(clean, noisy):
    ...
    return torch.FloatTensor(pesq_score).to(
        "cuda"
    )  # Return normalized scores as a tensor
...
def power_uncompress(real, imag):
    ...
    mag = mag ** (1.0 / 0.3)  # Uncompress magnitude by raising to the power of 1/0.3
    real_uncompress = mag * torch.cos(phase)  # Reconstruct real part
    imag_uncompress = mag * torch.sin(phase)  # Reconstruct imaginary part
    return torch.stack(
        [real_uncompress, imag_uncompress], -1
    )  # Stack uncompressed parts
...
def compute_fbank(audio_in, args):
    ...
    """
    frame_length = (
        args.win_len / args.sampling_rate * 1000
    )  # Frame length in milliseconds
    frame_shift = (
        args.win_inc / args.sampling_rate * 1000
