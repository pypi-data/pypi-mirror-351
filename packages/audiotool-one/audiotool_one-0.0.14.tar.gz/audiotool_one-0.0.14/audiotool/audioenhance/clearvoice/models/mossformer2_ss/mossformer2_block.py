...
class ScaledSinuEmbedding(nn.Module):
    ...
    def __init__(self, dim):
        ...
        )  # Inverse frequency for sinusoidal embeddings
...
class OffsetScale(nn.Module):
    ...
    def __init__(self, dim, heads=1):
        ...
        nn.init.normal_(
            self.gamma, std=0.02
        )  # Initialize gamma with a small standard deviation
...
class FFM(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, norm_klass=nn.LayerNorm, dropout=0.1):
        ...
        self.mdl = nn.Sequential(
            ...
            nn.Linear(
                dim_in, dim_out
            ),  # Linear transformation to project input to output dimensionality
...
class FLASH_ShareA_FFConvM(nn.Module):
    ...
    def forward(self, x, *, mask=None):
        ...
        if self.shift_tokens:
            ...
            x_shift = F.pad(x_shift, (0, 0, 1, -1), value=0.0)  # Shift tokens
...
class Gated_FSMN_Block_Dilated(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = nn.Conv1d(
            inner_channels, dim, kernel_size=1
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        seq_out = self.gated_fsmn(norm1.transpose(2, 1))
        norm2 = self.norm2(
            seq_out.transpose(2, 1)
        )  # Apply normalization after gated FSMN
