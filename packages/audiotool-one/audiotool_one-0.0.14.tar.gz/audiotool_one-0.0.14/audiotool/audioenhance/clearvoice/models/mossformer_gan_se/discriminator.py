...
def segment_specs(y, seg_length=15, seg_hop=4, max_length=None):
    ...
    for b in range(B):
        ...
        x = (
            x.transpose(1, 0)[idx3, :].unsqueeze(1).transpose(3, 2)
        )  # Rearrange dimensions for CNN input
        ...
        if max_length is not None:
            if max_length < n_wins:
                raise ValueError(
                    "n_wins {} > max_length {}. Increase max window length max_segments!".format(
                        n_wins, max_length
                    )
                )
            x_padded = torch.zeros(
                (max_length, x.shape[1], x.shape[2], x.shape[3])
...
class AdaptCNN(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            self.input_channels, self.c_out_1, self.kernel_size, padding=self.cnn_pad
        )
        ...
        self.conv6 = nn.Conv2d(
            self.conv5.out_channels, self.c_out_3, self.kernel_size_last, padding=(1, 0)
        )
    ...
    def forward(self, x):
        ...
        x = F.adaptive_max_pool2d(
            x, output_size=(self.pool_1)
        )  # Adaptive pooling after conv1

        x = F.relu(self.bn2(self.conv2(x)))  # Second convolutional layer
        x = F.adaptive_max_pool2d(
            x, output_size=(self.pool_2)
        )  # Adaptive pooling after conv2

        x = self.dropout(x)  # Apply dropout
        x = F.relu(self.bn3(self.conv3(x)))  # Third convolutional layer
        x = F.adaptive_max_pool2d(
            x, output_size=(self.pool_3)
        )  # Adaptive pooling after conv3

        x = self.dropout(x)  # Apply dropout
        x = F.relu(self.bn4(self.conv4(x)))  # Fourth convolutional layer
        x = F.adaptive_max_pool2d(
            x, output_size=(self.pool_4)
        )  # Adaptive pooling after conv4

        x = self.dropout(x)  # Apply dropout
        x = F.relu(self.bn5(self.conv5(x)))  # Fifth convolutional layer
        x = F.adaptive_max_pool2d(
            x, output_size=(self.pool_5)
        )  # Adaptive pooling after conv5
