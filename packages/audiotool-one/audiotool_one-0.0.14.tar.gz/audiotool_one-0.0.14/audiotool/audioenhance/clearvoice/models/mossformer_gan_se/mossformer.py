...
class ScaledSinuEmbedding(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.register_buffer(
            "inv_freq", inv_freq
        )  # Store frequency values for sine and cosine
...
class T5RelativePositionBias(nn.Module):
    ...
    def __init__(self, scale, causal=False, num_buckets=32, max_distance=128):
        ...
        self.relative_attention_bias = nn.Embedding(
            num_buckets, 1
        )  # Bias embedding for relative positions
...
class RelativePosition(nn.Module):
    ...
    def forward(self, x):
        ...
        distance_mat = (
            range_vec_k[None, :] - range_vec_q[:, None]
        ...
        embeddings = self.embeddings_table[
            final_mat
        ]  # Get embeddings based on distances
...
class OffsetScale(nn.Module):
    ...
    def forward(self, x):
        ...
        return out.unbind(
            dim=-2
        )  # Return the result unbound along the last head dimension
...
class MossFormer(nn.Module):
    ...
    def forward(self, x, *, mask=None):
        ...
        if self.shift_tokens:
            ...
            x_shift = F.pad(x_shift, (0, 0, 1, -1), value=0.0)  # Pad to maintain shape
    ...
    def cal_attention(self, x, quad_q, lin_q, quad_k, lin_k, v, u, B, mask=None):
        ...
        sim_c = (
            einsum("... i d, ... j d -> ... i j", quad_q_c, quad_k_c)
            / quad_q_c.shape[-2]
        )
        ...
        attn_c = attn_c.masked_fill(mask_c, 0.0)  # Mask diagonal for attention

        if exists(mask):
            attn = attn.masked_fill(~mask, 0.0)  # Apply the mask to the main attention

        if self.causal:
            # Create a causal mask for the attention
            causal_mask = torch.ones((g, g), dtype=torch.bool, device=device).triu(1)
            attn = attn.masked_fill(causal_mask, 0.0)  # Apply causal mask
