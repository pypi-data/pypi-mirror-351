...
class BsRoformer_Loader:
    ...
    def get_model_from_config(self):
        ...
        model = BSRoformer(**dict(config))

        return model

    def demix_track(self, mix, device):
        C = 352800
        # num_overlap
        N = 1
        fade_size = C // 10
    ...
    def run_folder(self, input, vocal_root, others_root, format):
        ...
        if format in ["wav", "flac"]:
            sf.write(
                "{}/{}_{}.{}".format(
                    vocal_root, os.path.basename(path)[:-4], "vocals", format
                ),
                estimates,
                sr,
            )
            sf.write(
                "{}/{}_{}.{}".format(
                    others_root, os.path.basename(path)[:-4], "instrumental", format
                ),
                mix_orig.T - estimates,
                sr,
            )
        else:
            path_vocal = "%s/%s_vocals.wav" % (vocal_root, os.path.basename(path)[:-4])
            path_other = "%s/%s_instrumental.wav" % (
                others_root,
