...
class XbsUiOperate:
        ...
        if image_path:
            ...
            for i in range(timeout):
                ...
                if widget_info:
                    return True
                sleep(1)
        print(f"未找到元素 text:{text} index:{index} resourceId:{resourceId} class:{class_} image_path: {image_path}")
        return False
    ...
    def input(self, text=None, index=None, resourceId=None, class_=None, image_path=None, content=None,threshold=0.98):
        if image_path:
            ...
            os.system(f"adb -s {self.dev} shell input text {content}")
        return True
    ...
    def app_start(self, package_name):
        ...
        try:
            ...
            print(f"打开应用{package_name}失败: {e}")
            return False
        return True
    ...
    def install_app(self, apk_file):
        if not os.path.exists(apk_file):
            print(f"{self.dev} 文件不存在： {apk_file}")
            return False
