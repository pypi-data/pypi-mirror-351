...
def collect_referenced_models(model: type[BaseModel]) -> set[type[BaseModel]]:
    ...
    def _collect(model_cls):
        ...
        for field in model_cls.model_fields.values():
            ...
            # Handle Optional[Model], Union[Model, None], and Union with list[Model]
            if origin is Union:
                for arg in args:
                    if arg is type(None):
                        continue
                    ...
                    if inspect.isclass(arg) and issubclass(arg, BaseModel):
                        _collect(arg)
                    elif arg_origin in (list, tuple, set) and arg_args:
                        for sub_arg in arg_args:
                            if inspect.isclass(sub_arg) and issubclass(sub_arg, BaseModel):
                                _collect(sub_arg)
...
def flatten_schema(model: type[BaseModel]) -> dict:
    ...
    # Prende tutti i modelli referenziati ricorsivamente
    ref_models = {cls.__name__: cls for cls in collect_referenced_models(model)}
    ...
    # --- Gestisci ricorsivamente le proprietà 'anyOf' in tutto lo schema ---
    def _handle_anyof_recursive(node):
        if isinstance(node, dict):
            # Se è una proprietà con anyOf, applica la logica
            if "anyOf" in node and isinstance(node["anyOf"], list) and node["anyOf"]:
                first_type_schema = node["anyOf"][0]
                # Unisci il primo tipo con i dati originali, senza sovrascrivere le chiavi già presenti
                merged_schema = {**first_type_schema, **{k: v for k, v in node.items() if k != "anyOf"}}
                node.clear()
                node.update(merged_schema)
            # Ricorri su tutte le chiavi
            for k, v in list(node.items()):
                # Se la chiave è 'properties', aggiorna anche la lista required
                ...
                _handle_anyof_recursive(v)
        elif isinstance(node, list):
            for item in node:
                _handle_anyof_recursive(item)

    _handle_anyof_recursive(flat_schema)

    # Aggiunto: Gestisci ricorsivamente le proprietà che sono array di riferimenti
    def _handle_anyof_and_arrays_recursive(node):
        if isinstance(node, dict):
            # Handle anyOf as before
            if "anyOf" in node and isinstance(node["anyOf"], list) and node["anyOf"]:
                first_type_schema = node["anyOf"][0]
                merged_schema = {**first_type_schema, **{k: v for k, v in node.items() if k != "anyOf"}}
                node.clear()
            ...
            if node.get("type") == "array" and "items" in node:
                ...
                if isinstance(items, dict) and "$ref" in items:
                    ...
                    if ref_path.startswith("#/$defs/"):
                        ref_name = ref_path.split("/")[-1]
                        if ref_name in ref_schemas:
                            node["items"] = _resolve_refs(ref_schemas[ref_name])
            # Recurse
            for k, v in list(node.items()):
                if k == "properties" and isinstance(v, dict):
                    required = set(node.get("required", []))
                    for prop, prop_schema in v.items():
                        if "anyOf" in prop_schema and isinstance(prop_schema["anyOf"], list) and prop_schema["anyOf"]:
                            required.discard(prop)
                    node["required"] = list(required)
                _handle_anyof_and_arrays_recursive(v)
        elif isinstance(node, list):
            for item in node:
                _handle_anyof_and_arrays_recursive(item)

    _handle_anyof_and_arrays_recursive(flat_schema)
