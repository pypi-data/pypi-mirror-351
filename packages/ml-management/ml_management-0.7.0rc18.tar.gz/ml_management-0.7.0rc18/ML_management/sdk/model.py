...
from ML_management.sdk.sdk import _to_datetime
...
def set_model_visibility(name: str, visibility: VisibilityOptions) -> ModelInfo:
    """
    ...
    """
    op = Operation(schema.Mutation)
    set_visibility = op.update_model(name=name, update_model_form=UpdateObjectForm(new_visibility=visibility.name))
    set_visibility.name()
...
def set_model_version_visibility(name: str, version: int, visibility: VisibilityOptions) -> ModelVersionInfo:
    """
    ...
    set_visibility = op.update_model_version(
        model_version=choice, update_model_version_form=UpdateObjectForm(new_visibility=visibility.name)
    )
    set_visibility.name()
