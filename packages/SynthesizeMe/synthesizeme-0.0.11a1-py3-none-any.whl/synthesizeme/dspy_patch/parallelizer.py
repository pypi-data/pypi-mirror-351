...
class ParallelExecutor:
    def __init__(
        ...
        """
        Offers isolation between the tasks (dspy.settings) irrespective of whether
        num_threads == 1 or > 1.  Also handles straggler timeouts.
        """
        ...
        self.provide_traceback = (
            provide_traceback
            if provide_traceback is not None
            else settings.provide_traceback
        )
    ...
    def _execute_parallel(self, function, data):
        ...
        def worker(parent_overrides, submission_id, index, item):
            ...
            if parent_overrides.get("usage_tracker"):
                # Usage tracker needs to be deep-copied across threads
        ...
        try:
            with interrupt_manager():
                ...
                # Create the progress bar only if it will be shown
                pbar = None
    ...
    def _update_progress(self, pbar, nresults, ntotal):
        if pbar is None:  # progress bar suppressed
            return
        if self.compare_results:
