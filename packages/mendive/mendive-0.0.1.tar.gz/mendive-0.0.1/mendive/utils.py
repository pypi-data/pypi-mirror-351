...
def string_simple_format(is_free):
  """
  Returns a string indicating whether a graph is claw-free.

  Args:
    is_free: A Boolean value, True if the graph is claw-free, False otherwise.
  Returns:
    - "Claw Free" if claw is True, "Claw Found" otherwise.
  """
  return "Claw Free" if is_free  else "Claw Found"

def string_complex_format(result, count_result=False):
  """
  Returns a string indicating whether the graph is claw-free.
  
  Args:
    result: None if the graph is claw-free, the claw vertices otherwise.
    count_result: Count the number of claws found (default is False).

  Returns:
    - "Claw Free" if claw is None, "Claw{s} Found {a, b, c}, ...." otherwise.
  """
  if result:
    if count_result:
