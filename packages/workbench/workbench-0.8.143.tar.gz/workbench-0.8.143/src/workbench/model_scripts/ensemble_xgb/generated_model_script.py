...
TEMPLATE_PARAMS = {
    ...
    "model_metrics_s3_path": "s3://sandbox-sageworks-artifacts/models/training/aqsol-ensemble",
...
if __name__ == "__main__":
    ...
    # Train one model with ALL the data
    params = {"objective": "reg:squarederror"}
    ...
    # Add the 'All Model' predictions to the DataFrame
    result_df["prediction"] = full_model.predict(X)
...
def predict_fn(df, models) -> pd.DataFrame:
    ...
    df["p_mean"] = df[[name for name in df.columns if name.startswith("m_")]].mean(axis=1)
    ...
    df["p_stddev"] = df[[name for name in df.columns if name.startswith("m_")]].std(axis=1)
