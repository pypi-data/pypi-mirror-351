...
TEMPLATE_PARAMS = {
    ...
    "feature_list": ['sd', 'ocurrences', 'molwt', 'mollogp', 'molmr', 'heavyatomcount', 'numhacceptors', 'numhdonors', 'numheteroatoms', 'numrotatablebonds', 'numvalenceelectrons', 'numaromaticrings', 'numsaturatedrings', 'numaliphaticrings', 'ringcount', 'tpsa', 'labuteasa', 'balabanj', 'bertzct'],
    "scikit_model_class": BayesianRidge,
    "model_metrics_s3_path": "s3://sandbox-sageworks-artifacts/models/aqsol-bayesian-reg/training",
    "train_all_data": True
}
...
if __name__ == "__main__":
    ...
    if model_type in ["classifier", "regressor"]:
        ...
        # Make predictions and pull out the standard deviation
        y_mean, y_std = model.predict(X_val, return_std=True)

        # Add predictions to the validation DataFrame
        df_val["prediction"] = y_mean
        df_val["prediction_std"] = y_std

        # Save the validation predictions to S3
        output_columns = [target, "prediction", "prediction_std"]
