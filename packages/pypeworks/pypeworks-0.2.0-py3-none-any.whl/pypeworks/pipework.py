...
class Pipework(Node[Iterator[T] | T], Generic[T, R]):
    ...
    def __init__(
        ...
        **nodes       : dict[str, Node]
        ...
        if self.nodes_by_name.get("enter", None) is None:
            self.nodes_by_name["enter"] = Node(lambda x: x)
        ...
        # Node
        super().__init__(callable = self.__call__, join = False)
    ...
    def __call__(self, inputs : Iterator[T] | T = None) -> Iterator[R]:
        ...
        # Determine how the queue should be populated, based on whether an iterable or plain object
        # was passed.
                
        # Iterable
        if hasattr(inputs, "__iter__") and not hasattr(inputs, "__len__"):
                
            # Input is fed asynchronyously as to immediately take advantage of the pipework's
            ...
            def populate():
                ...
                """
                Asynchronously fills the priority queue with data from the given iterable.
                """
        ...
        # Plain object
            ...
            push_object(inputs, enter_id, True)
        ...
        with multiprocessing.pool.ThreadPool(processes = self.processes) as pool:
            ...
            while True:
                ...
                if node_id == exit_id:
                    ...
                    # Yield result
                    yield obj if not isinstance(obj, Args) else obj.kwargs
                ...
                pool.apply_async(
                    ...
                    callback = (
                        get_callback(
                            ...
                            return_hints = (
                                node.returns if isinstance(node.returns, _ArgsAlias) 
                                else None
                            )
                        )
                    ), 
