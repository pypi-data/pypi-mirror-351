...
Date         : 2024-05-15 15:15:26 +0200
LastEditors  : Jie Wu j.wu@cern.ch
LastEditTime : 2024-12-06 14:17:35 +0100
FilePath     : PIDCorr.py
Description  : 
...
def PIDCorr(input_file, input_tree_name, output_file, data_set, mode, tracks_file, tmp1, tmp2, simversion):
    ## START OF CONFIG
    ...
    # Simulation version
    year = data_set.split("_")[1]

    def get_RunTime_simversion(year, simversion, track):
        if year in {'2015', '2016', '2017', '2018'}:
            simversion = 'run2'
        elif year in {'2011', '2012'}:
            if 'sim08' in simversion.lower():
                if 'mu' in track.lower():
                    print(f'PIDCorr::WARNING:: The PIDCorr files for {track} does not exist in sim08 ({year}), using sim09 instead')
                    simversion = 'sim09'
                else:
                    simversion = 'sim08'
            else:
            ...
            print(f'ERROR::PIDCorr.py: Year {year} not supported')
            exit(1)
    ...
    for input_file, output_file, dataset in files:
        ...
        for track, subst in tracks.items():
            for var, config in subst.items():
                ...
                command += " -S %s" % get_RunTime_simversion(year, simversion, track)
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--simversion', help='Simulation version, sim08, sim09, run2, etc.')
    return parser
