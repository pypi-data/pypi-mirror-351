...
LastEditTime : 2025-05-18 07:10:47 +0200
...
from typing import TypedDict, Literal, Optional, Sequence, Union, TypeVar, List, NamedTuple
    ...
    def __post_init__(self):
        ...
        for f in file_path_list:
            if not os.path.exists(f):
                raise ValueError(f"File not found: {f} in {self.file_path}")
...
def load_data_from_root(datasets: list[DatasetConfig], cmp_vars: list[dict]):
    for i_dataset, dset in enumerate(datasets):
        # Variables to be loaded (includes both comparison variables and weights)
        _vars_to_load = list(cmp_vars[i_dataset].values())
        if dset.weight_expr:
            _vars_to_load.append(dset.weight_expr)

        # Load variables into a pandas DataFrame
        variables_pd = load_variables_to_pd_by_uproot(
            input_file_path=dset.file_path,
            input_tree_name=dset.tree_name,
            variables=_vars_to_load,
            selection=dset.selection,
            library='pd',
        ...
        # Create new variables based on expressions
        ...
        for var_title in cmp_vars[i_dataset]:
            ...
            #     rprint(f"INFO::load_data_from_root: Converting the column '{var_title}' from DataFrame to Series")
        ...
        # 2. For weights
        if dset.weight_expr:
            weight_name = "__weight_for_distribution_comparison__"
            weight_pd = create_variables_in_pd(variables_pd, {weight_name: dset.weight_expr})[weight_name]
        else:
            weight_pd = np.ones(len(variables_pd))

        # 3. Store data and weights in the dataset config
        dset.data = variables_pd
        dset.weights = weight_pd

    return datasets


def getRanges(datas: list[np.array], percentRange: str = "auto") -> list:
    """
    Get ranges of list of datasets
    datas: [list] a list of data given in numpy format.
    percentRange: [str] the range to be used for the plot, can be "auto" or a string of two numbers separated by a comma, e.g. "0,100"
    """
        ...
        try:
            if ext.lower() == '.png':
                ...
                try:
                    ...
                    logger.info(f"Collect_all_plots_in_one_canvas Converting PDF '{fig_path}' to image...")
...
def draw_distributions(
    ...
    # Convert string representations to DatasetConfig objects
    datasets_config = []
    for ds in dataset:
        if isinstance(ds, str):
            try:
                datasets_config.append(DatasetConfig.from_string(ds))
            except ValueError as e:
                # Provide more context in the error message
                raise ValueError(f"Failed to parse dataset '{ds}': {e}")
        else:
            datasets_config.append(ds)
    ...
    logger.info(f"Compare_variables: {compare_variables}")
    ...
    if len(compare_variables) == 1:
        ...
        raise ValueError(f"{colorama.Fore.RED}When specifying different variables for each dataset, the number of compare_variables must match the number of datasets.{colorama.Style.RESET_ALL}")

    # Read variables from YAML or create variable mappings
    cmp_vars: list[dict[str, str]] = []
    ...
    # Load data from ROOT files
    load_data_from_root(datasets_config, cmp_vars)
    ...
    var_titles: list[str] = list(cmp_vars[0].keys())
    ...
    for var_title in var_titles:
        # Collect data for the current variable from all datasets
        datas: list[np.array] = [dset.data[var_title] for dset in datasets_config]
        weights: list[np.array] = [dset.weights for dset in datasets_config]
        labels: list[str] = [dset.label for dset in datasets_config]

        # Plot the variable comparison
    ...
    # Create combined canvas with all plots
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="Dataset to be compared in format: 'path_to_file,tree_name,weight_expr,selection,label'. "
        "weight_expr can be 'none' if not applicable. "
        "selection must be enclosed in parentheses, e.g., '(pt>20)'. "
        "For complex selections, each condition must be parenthesized: '((pt>20) && (eta<2.5))'.",
    )
