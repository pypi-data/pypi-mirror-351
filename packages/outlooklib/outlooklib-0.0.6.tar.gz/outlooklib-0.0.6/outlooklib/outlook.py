...
Microsoft Documentation about this API:
...
class Outlook(object):
    ...
    def _handle_response(self, response: requests.Response, model: Type[BaseModel], rtype: str = "scalar") -> BaseModel | list[BaseModel]:
        ...
        into a Pydantic BaseModel instance or a list of instances, depending on the specified response type.
    ...
    def change_client_email(self, email: str) -> None:
        ...
        This method updates the email address of the client that the Outlook instance will interact with
    ...
    def change_folder(self, id: str) -> None:
        ...
        Args:
            id (str): The name or ID of the folder to be accessed. Refer to the Microsoft Graph API documentation for valid folder names and IDs.
    ...
    def list_folders(self, save_as: str | None = None) -> Response:
        ...
        This method fetches the mail folders for the authenticated user's email account.
        The results can be saved to a JSON file if the `save_as` parameter is provided.

        Args:
            save_as (str, optional): The file path where the JSON response will be saved. If None, the response will not be saved.
    ...
    def list_messages(self, filter: str, save_as: str | None = None) -> Response:
        ...
        class DataStructure(BaseModel):
            ...
            # flag_status: str = Field(None)

            # @validator("flag_status", always=True)
            # def set_flag_status(cls, v, values):
            #     if values.get("flag") is not None:
            #         return values["flag"]["flagStatus"]
            #     return None
    ...
    def move_message(self, id: str, to: str, save_as: str | None = None) -> Response:
        ...
        This method relocates an email message identified by its unique ID from the current folder
        to a destination folder specified by its ID.
        ...
        # Request query
        url_query = fr"https://{api_domain}/{api_version}/users/{client_email}/mailFolders/{client_folder}/messages/{id}/move"
    ...
    def download_message_attachment(self, id: str, path: str, index: bool = False) -> Response:
        ...
        Args:
            ...
            index (bool, optional): If True, an index is added to the file name to prevent overwriting. Defaults to False.
