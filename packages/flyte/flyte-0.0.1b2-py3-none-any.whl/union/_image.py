...
def _ensure_tuple(val: Union[T, List[T], Tuple[T, ...]]) -> Tuple[T, ...]:
    """
...
class PipPackages(Layer):
    ...
    extra_index_urls: Optional[Tuple[str]] = None
    pre: bool = False
...
class Image:
    ...
    platform: Optional[Tuple[Architecture, ...]] = field(default=("linux/amd64",))
    ...
    @classmethod
    def _get_default_image_for(cls, python_version: Tuple[int, int], union_version: Optional[str] = None) -> Image:
        ...
        preset_tag = None
        if union_version:
            preset_tag = union_version if union_version.startswith("v") else f"v{union_version}"
        ...
        labels_and_user = _DockerLines(
            (
                ...
                "RUN useradd --create-home --shell /bin/bash flytekit && chown -R flytekit /root && chown -R flytekit /home",  # noqa: E501
                "WORKDIR /root",
            )
        )
        ...
        image = image.with_apt_packages("build-essential")

        base_packages = ["kubernetes", "msgpack", "mashumaro"]

        # Add in union library
        if union_version:
            base_packages.append(f"union=={union_version}")
            image = image.with_pip_packages(base_packages)
        else:
            from union._version import __version__

            if cls._is_editable_install() or (__version__ and "dev" in __version__):
                image = image.with_pip_packages(base_packages)
                image = image.with_local_v2()
            else:
                base_packages.append(f"union=={__version__}")
                image = image.with_pip_packages(base_packages)
    ...
    def auto(
        cls,
        python_version: Optional[Tuple[int, int]] = None,
        union_version: Optional[str] = None,
        ...
        :param union_version: Union version to use
        :param registry: Registry to use for the image
        ...
        base_image = cls._get_default_image_for(python_version=python_version, union_version=union_version)
    ...
    def with_pip_packages(
        ...
        if packages:
            packages = _ensure_tuple(packages)
        if extra_index_urls:
            extra_index_urls = _ensure_tuple(extra_index_urls)
    ...
    def with_commands(self, commands: List[str]) -> Image:
        ...
        :return: Image
        """
        commands = _ensure_tuple(commands)
        new_image = self.clone(addl_layer=Commands(commands=commands))
        return new_image
    ...
    def __img_str__(self) -> str:
        """
        For current image only, print all the details if they are not None
        """
        details = []
        if self.base_image:
        ...
        if self.registry:
            details.append(f"Registry: {self.registry.url}")
        if self.name:
