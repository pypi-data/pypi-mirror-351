...
class QuarkRemoteDriver(pydantic.BaseModel, metaclass=abc.ABCMeta):
    ...
    async def get_registry_item(self) -> QuarkRegistryItem:
        ...
        res = await api_client.registry.quark.retrieve(name=name, cat=cat)
    ...
    async def read_output_dataset(self) -> Union[pa.Table, None]:
        ...
        record_batches_raw = await api_client.dataset.retrieve_arrow(self._history.output["dataset_uuid"])
        await api_client.close()

        return pa.ipc.open_stream(record_batches_raw).read_all()
