...
class _BaseEvaluationConfig:
    ...
    custom_metrics: List[agent_custom_metrics.CustomMetric] = dataclasses.field(
...
class GlobalEvaluationConfig(_BaseEvaluationConfig):
    ...
    global_assessment_configs: List[assessment_config.AssessmentConfig] = (
        dataclasses.field(default_factory=list)
    )
    per_item_assessments: dict[
        str,
        list[
            Union[assessment_config.AssessmentConfig, agent_custom_metrics.CustomMetric]
        ],
    ] = dataclasses.field(
    ...
    def _from_dict(cls, config_dict: Mapping[str, Any]):
        ...
        try:
            ...
            if (
                ...
                and assessment_config.GLOBAL_GUIDELINE_ADHERENCE.assessment_name
                not in builtin_assessment_configs
            ):
                builtin_assessment_configs.append(
                    assessment_config.GLOBAL_GUIDELINE_ADHERENCE.assessment_name
                )
        ...
        custom_metrics = [
            ...
            if isinstance(metric, agent_custom_metrics.CustomMetric)
        ]
    ...
    def from_mlflow_evaluate_args(
        ...
        if EVALUATOR_CONFIG__GLOBAL_GUIDELINES_KEY in evaluator_config:
            ...
            if not input_output_utils.is_valid_guidelines_mapping(
                global_guidelines_mapping
            ):
                raise error_utils.ValidationError(
            ...
            input_output_utils.check_guidelines_mapping_exceeds_limit(
                global_guidelines_mapping
            )
