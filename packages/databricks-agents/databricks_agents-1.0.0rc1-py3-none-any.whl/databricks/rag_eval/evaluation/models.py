...
def invoke_model(
    ...
    # === Extract the trace from the model output ===
    ...
    # Last resort: just forge a trace.
    if model_result.trace is None:
    ...
    model_result.retrieval_context = traces.extract_retrieval_context_from_trace(
        model_result.trace
    )
...
def _is_agent_endpoint(model: Any) -> bool:
    ...
    try:
        ...
        models = endpoint_metadata.config.get(
            "served_models"
        ) or endpoint_metadata.config.get("served_entities")
        if not models:
            return False
