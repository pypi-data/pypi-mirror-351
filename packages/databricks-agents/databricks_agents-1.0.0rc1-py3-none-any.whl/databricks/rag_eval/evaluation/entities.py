...
class EvalItem:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "EvalItem":
        ...
        custom_expected = input_output_utils.normalize_to_dictionary(
            deepcopy(data.get(schemas.CUSTOM_EXPECTED_COL))
        )
        expectations = input_output_utils.normalize_to_dictionary(
        ...
        guidelines_context = (
            guidelines_context
            if not input_output_utils.is_none_or_nan(guidelines_context)
            else None
        )
...
class PerRequestAssessmentResult(AssessmentResult):
    ...
    def to_mlflow_assessment(
        ...
        source = mlflow_entities.AssessmentSource(
            ...
            source_id=self.assessment_source.source_id
            or AssessmentSource.builtin().source_id,
        )
        ...
        for position, rating in self.positional_rating.items():
            ...
            assessments.append(
                mlflow_entities.Feedback(
                    ...
                    source=source,
                    rationale=rating.rationale,
                    metadata={_SPAN_OUTPUT_KEY: str(position)},
                    **value_kwarg,
                )
            )
        return assessments
...
class MetricResult:
    ...
    def to_mlflow_assessment(
        ...
        :param assessment_name: The name of the assessment
        :param trace_id: The trace ID of the trace associated
        :param span_id: The span ID of the span associated
        :return: MLflow Assessment object
        """
        if trace_id is not None:
...
class EvalResult:
    ...
    eval_item: EvalItem
    _assessments: List[mlflow_entities.Assessment] = dataclasses.field(
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        for assessment in self.assessment_results:
            if isinstance(assessment, PerRequestAssessmentResult):
                converted_assessment_results.append(
                    assessment.to_mlflow_assessment(
        ...
        expectations = {
            key: value
            for key, value in eval_item_dict.items()
            if key in _EXPECTATION_FIELDS
        }
        ...
        return (
            self._assessments
            + converted_assessment_results
            + converted_metric_results
    ...
    def get_assessment_results_dict(self) -> Dict[str, schemas.ASSESSMENT_RESULT_TYPE]:
        ...
        for metric in self.metric_results:
            ...
            if metric.metric_value.feedback.value is not None:
                assessments[f"{metric.metric_value.name}/value"] = (
                    metric.metric_value.feedback.value
                )

            if metric.metric_value.rationale is not None:
                assessments[f"{metric.metric_value.name}/rationale"] = (
            ...
            if metric_error is not None and metric_error.error_message is not None:
                assessments[f"{metric.metric_value.name}/error_message"] = (
                    metric_error.error_message
                )

            if metric_error is not None and metric_error.error_code is not None:
                assessments[f"{metric.metric_value.name}/error_code"] = (
                    metric_error.error_code
                )
        return assessments
