...
def _validate_custom_assessment_name(*, metric_name: str, assessment_name: Any):
...
def _get_custom_assessment_name(
    assessment: mlflow_entities.Assessment, metric_name: str
) -> str:
    ...
    """
    return assessment.name if assessment.name is not None else metric_name
...
def _normalize_custom_assessment(
    ...
    assessment = mlflow_entities.Feedback(
...
def metric(
    ...
    *,
    ...
    def decorator(fn, *, _name=name, _aggregations=aggregations):
        ...
        supported_aggregations = list(
            aggregation_utils._AGGREGATION_TO_AGGREGATE_FUNCTION.keys()
        )
        for aggregation in _aggregations or []:
            if (
                isinstance(aggregation, str)
                and aggregation not in supported_aggregations
            ):
                raise error_utils.ValidationError(
                    f"Invalid aggregation: {aggregation}. Supported aggregations: {supported_aggregations}.",
                )
        return functools.wraps(fn)(
            CustomMetric(name=metric_name, eval_fn=fn, aggregations=_aggregations)
        )
...
def _create_metric_from_function_body(
    metric_name: str, function_body: str
) -> CustomMetric:
    ...
    def wrapped_fn(**kwargs):
        ...
        indented_body = "\n".join(
            f"{indent_unit*indent_length}{line}" for line in func_lines
        )
        ...
        cm_kwargs = {
            ...
            schemas.EXPECTED_RETRIEVED_CONTEXT_COL: kwargs.get(
                schemas.EXPECTED_RETRIEVED_CONTEXT_COL
            ),
