...
class TokenCountMetric(Metric):
    """
    TokenCountMetric computes the token count metrics for an eval item.
    """
    ...
    def run(
        ...
        *,
        ...
        token_count = traces.compute_total_token_count(eval_item.trace)
        if (
            token_count.input_token_count is None
            and token_count.output_token_count is None
        ):
            return []

        return [
            entities.MetricResult.make_legacy_metric(
                metric_name=schemas.TOTAL_TOKEN_COUNT_COL,
                metric_value=token_count.total_token_count,
            ),
            entities.MetricResult.make_legacy_metric(
                metric_name=schemas.TOTAL_INPUT_TOKEN_COUNT_COL,
                metric_value=token_count.input_token_count,
            ),
            entities.MetricResult.make_legacy_metric(
                metric_name=schemas.TOTAL_OUTPUT_TOKEN_COUNT_COL,
                metric_value=token_count.output_token_count,
            ),
        ]
    ...
    def run(
        ...
        *,
        ...
        if (
            ...
            or eval_item.trace.info.execution_duration is None
        ):
            return []
        return [
            entities.MetricResult.make_legacy_metric(
...
class GroundTruthRetrievalMetric(Metric):
    ...
    def run(
        ...
        *,
        ...
        if (
            not eval_item
            or not eval_item.retrieval_context
            or not eval_item.ground_truth_retrieval_context
        ):
            return results
...
class GroundTruthDocumentRatingMetric(Metric):
    """
    Compute the ground truth document ratings for an eval item.
    ...
    def run(
        ...
        *,
        ...
        return [
            entities.MetricResult.make_legacy_metric(
                metric_name=schemas.GROUND_TRUTH_DOCUMENT_RATING_COL,
                metric_value=categorical_ratings,
            )
        ]
...
TOKEN_COUNT_METRIC = TokenCountMetric()
LATENCY_METRIC = LatencyMetric()
GROUND_TRUTH_RETRIEVAL_METRIC = GroundTruthRetrievalMetric()
GROUND_TRUTH_DOCUMENT_RATING_METRIC = GroundTruthDocumentRatingMetric()
...
BUILT_IN_METRICS = [
    ...
    GROUND_TRUTH_DOCUMENT_RATING_METRIC,
