...
class LabelSchema:
    ...
    input: Union[
        InputCategorical, InputCategoricalList, InputText, InputTextList, InputNumeric
    ]
...
class LabelingSession:
    ...
    def add_dataset(
        ...
        if not self.agent:
            raise ValueError(
                "This session has no agent assigned. An agent is required when adding "
                "dataset records in order to generate responses"
            )
        ...
        if record_ids:
            ...
            ), "`record_ids` must be a list of strings"
        ...
        dataset_record_ids = set(record_ids or (r.dataset_record_id for r in records))
        ...
        existing_record_ids = set(
            [
                ...
                for item in items
                if item.source
                and item.source.dataset_record
                and item.source.dataset_record.dataset_record_id
            ]
        )

        # Add the records that are not already in the session.
        record_ids_to_add = dataset_record_ids - existing_record_ids
    ...
    def add_traces(
        ...
        traces = [
            serialization_utils.deserialize_trace(t) if isinstance(t, str) else t
            for t in traces
        ]
        ...
        for trace in traces:
            if not trace:
                raise ValueError(
                    "trace can not be None. "
                    "Must be `mlflow.entities.Trace` or its json string representation."
                )

            # Always copy the trace to this experiment and run.
            trace = log_trace_to_experiment(
                trace, self.experiment_id, self.mlflow_run_id
            )
    ...
    def sync_expectations(self, to_dataset: str) -> None:
        ...
        for item in items:
            for chat_round in item.chat_rounds:
                ...
                if not trace:
                    print(
                        f"Trace {chat_round.trace_id} not found in the databricks server."
                    )
                    continue
        ...
        datasets.get_dataset(to_dataset).insert(records_to_insert)
    ...
    def set_assigned_users(self, assigned_users: list[str]) -> "LabelingSession":
        ...
        client = _get_client()
        if self.agent:
            review_app = client.list_review_apps(
                filter=f"experiment_id={self.experiment_id}"
            )[0]
            agent_config = next(
                a for a in review_app.agents if a.agent_name == self.agent
            )
            if agent_config.model_serving_endpoint:
                add_users_to_serving_endpoint(
                    assigned_users, agent_config.model_serving_endpoint
                )
...
class ReviewApp:
    ...
    def add_agent(
        ...
        self.agents.append(
            Agent(agent_name=agent_name, model_serving_endpoint=model_serving_endpoint)
        )
    ...
    def remove_agent(self, agent_name: str) -> "ReviewApp":
        ...
        agent_to_remove = next(
            (x for x in self.agents if x.agent_name == agent_name), None
        )
    ...
    def delete_label_schema(self, label_schema_name: str) -> "ReviewApp":
        ...
        schema_to_remove = next(
            (
                label_schema
                for label_schema in self.label_schemas
                if label_schema.name == label_schema_name
            ),
            None,
        )
        ...
        *,
        ...
        for schema_name in label_schemas:
            if schema_name not in valid_schema_names:
                raise ValueError(
                    f"Label schema {schema_name} not found. "
                    "Please create it first via `create_label_schema`."
                )
    ...
    def delete_labeling_session(self, labeling_session: LabelingSession) -> "ReviewApp":
        ...
        return _get_client().delete_labeling_session(
            self.review_app_id, labeling_session.labeling_session_id
        )
