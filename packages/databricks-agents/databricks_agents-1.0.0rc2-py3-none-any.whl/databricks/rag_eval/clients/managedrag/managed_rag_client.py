...
def _result_contains_error_code(
    ...
    if isinstance(assessment_result, entities.PerRequestAssessmentResult):
        return assessment_result.rating.error_code is not None and (
            assessment_result.rating.error_code in error_codes
        )
...
class ManagedRagClient(databricks_api_client.DatabricksAPIClient):
    ...
    def get_assessment(
        ...
        """
        try:
            return self._get_assessment(eval_item, config)
    ...
    @tenacity.retry(
        ...
        stop=tenacity.stop_after_attempt(
            env_vars.RAG_EVAL_LLM_JUDGE_MAX_JUDGE_ERROR_RETRIES.get()
        ),
        reraise=False,
    )
    def _get_assessment(
        ...
        request_json = self.proto_serde.construct_assessment_request_json(
            eval_item, assessment_name, config.domain_instructions
        )
