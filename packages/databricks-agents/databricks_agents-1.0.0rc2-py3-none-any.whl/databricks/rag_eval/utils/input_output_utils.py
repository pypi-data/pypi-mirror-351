...
def parse_variant_data(data):
    ...
    it will be converted to a dict (and fail if the data is not json serializable).
    """
    try:
...
def request_to_string(data: ModelInput) -> str:
    ...
    if _QUERY in data:
        ...
        return (
            ...
            else json.dumps(content, default=lambda o: o.__dict__)
        )
...
def is_valid_guidelines_iterable(guidelines: Any) -> bool:
    ...
    return isinstance(guidelines, (list, np.ndarray)) and all(
...
def is_valid_guidelines_mapping(guidelines: Any) -> bool:
    ...
    return isinstance(guidelines, dict) and all(
        isinstance(guidelines_group, list)
        and all(isinstance(guideline, str) for guideline in guidelines_group)
