    ...
    if model:
        ...
        try:
            ...
            if (
                ...
                and _should_clone_trace(eval_item.trace, experiment_id, run_id)
            ):
                prepared_trace = trace_utils.clone_trace_to_reupload(eval_item.trace)
                cloned_trace = trace_utils.inject_experiment_run_id_to_trace(
...
def _should_clone_trace(
    trace: Optional[mlflow_entities.Trace], experiment_id: str, run_id: str
) -> bool:
    """
    ...
    # If the trace is from a different experiment or the same experiment but a different run,
    # we need to clone the trace
    clone_trace = not is_trace_from_same_exp or (
        trace_run_id is not None and trace_run_id != run_id
    )
    return clone_trace
...
def _create_minimal_trace(
    ...
    if eval_item.retrieval_context is not None:
        ...
        eval_item.retrieval_context.span_id = traces._get_top_level_retrieval_spans(
            trace
        )[0].span_id
        return trace
    else:
        return trace_utils.create_minimal_trace(
            eval_item.raw_request, eval_item.raw_response
        )
