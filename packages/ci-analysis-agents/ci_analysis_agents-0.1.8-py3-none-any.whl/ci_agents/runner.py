...
from ci_agents.log_config import configure_root_logger, logger
...
async def analyze_failure(analysis_context: AnalysisContext) -> AnalysisResult:
    ...
    logger.info(f"Last agent {type(last_agent_response.failure_type)}")
    logger.info(f'Last agent response: {json.dumps(last_agent_response.model_dump(), indent=2, ensure_ascii=False) if last_agent_response else "No response from agent"}')
