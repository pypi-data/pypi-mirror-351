...
class OTFS(MatlabFuncHelper):
    ...
    def setChannel(self, in1, in2, in3, *, force_frac=False, isAWGN=False, isRician=False, isUniform=False, isSameDD=False):
        ...
        if in1.ndim > 0 and in2.ndim > 0 and in3.ndim > 0:
            ...
            if force_frac:
                ...
                if isSameDD:
                    frac_range_max_pos = np.tile(np.random.rand(p)*(kmax_frac+0.5)-0.5, (self.batch_size, 1));
    ...
    '''
    pass the channel
    @No: noise power (a scalar) or a given noise vector, [1], [(batch_size), 1], [batch_size, n]
    '''
    def passChannel(self, No):
        ...
        if No.ndim == 0 or self.batch_size is not self.BATCH_SIZE_NO and No.ndim == 1 and No.shape[0] == self.batch_size: 
            pass
        ...
        if No.ndim == 0:
            ...
            raise Exception("The noise is not added.")
