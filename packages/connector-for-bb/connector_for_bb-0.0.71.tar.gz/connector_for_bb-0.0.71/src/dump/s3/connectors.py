...
class S3UploadDF(S3Upload):
    def __init__(
        ...
        **kwargs,
        ...
        if self.df_compression:
            ...
            self._df_file_system = TemporaryFileSystem(file_format=tmp_file_format)
        else:
            self._df_file_system = TemporaryFileSystem(file_format="csv")
    ...
    def upload_to_s3_df(self, df: PandasDataFrame, file_name: str):
        save_path = self._df_file_system.save_path()
...
class S3Download(S3Client):
    ...
    def __validate_file_system_by_filename(self, download_file_name: str):
        ...
        if extension:
            self.file_system.file_format = extension
