...
class OpenAIDataExtractor:
        ...
        try:
            ...
            if IS_OCR_EXIST == 'false':
                ...
                if '.PDF' in self.fileExtension_lower or '.pdf' in self.fileExtension_lower or '.png' in self.fileExtension_lower:
                    ...
                    self.remove_detected_watermark(file_path)
                    ...
                    if 'PP' == proc_mtd_value[0] :
                        ...
                        logger.log("\nCASE PPH\n")
                        finalResult=self.process_image(file_path,proc_api_key,ai_proc_templ, ai_proc_variables,Model_Name)
            ...
            if ai_proc_templ:
                if 'PPH' != proc_mtd_value[0]: 
                    ...
                    #     finalResult = self.extractdatausing_turbo(proc_api_key = proc_api_key, ai_proc_templ=ai_proc_templ,ai_proc_variables=ai_proc_variables, OCR_Text = OCR_Text,userId = self.userId)
                        ...
                        finalResult = self.extractdatausing_turbo(proc_api_key,ai_proc_templ,ai_proc_variables,OCR_Text,Model_Name,geminiAI_APIKey,self.userId,localAIURL,proc_mtd_value)
        ...
        try:
            ...
            if ai_proc_templ:
                ...
                if ent_name.strip() and ((isinstance(ent_code, str) and ent_code.strip()) or isinstance(ent_code, int)):  
                    ...
                    if template_Keyword_1 not in OCR_Text:
                        ...
                        return final_result
                    elif template_Keyword_2 not in OCR_Text:
                        ...
                        final_result['error'] = errorjson
                        return final_result
                    elif ent_name in OCR_Text:
                        logger.log(f'\n[ Template creation Start time  305  :          {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}]', "0")
                        templatecreation = GenerateExtractTemplate()
                        templatecreation.generateHeaderTemplate(ymlfilepath_final,ent_name,ent_code,ent_type,ai_proc_variables,OCR_Text,template_Keyword_1,template_Keyword_2)
                        templatecreation.generateOCRTextFile(ocrfilepath_final,ent_name,ent_code,ent_type,ai_proc_variables,OCR_Text,template_Keyword_1,template_Keyword_2)
    ...
    def extractdatausing_turbo(self,proc_api_key, ai_proc_templ,ai_proc_variables,OCR_Text,Model_Name,geminiAI_APIKey,userId,localAIURL,proc_mtd_value):
        logger.log(f"\nai_proc_variables::::\n {ai_proc_variables}\n{type(ai_proc_variables)}","0")
        logger.log(f"\nai_proc_templ::::\n {ai_proc_templ}\n{type(ai_proc_templ)}","0")
        logger.log(f"\nai_OCR_Text::::  512\n {OCR_Text}\n{type(OCR_Text)}","0")
        OCR_List            = []
        ...
        if page_wise == "No" or self.fileExtension_lower == 'csv' or self.fileExtension_lower == 'xls' or self.fileExtension_lower == 'xlsx':
            ...
            if Model_Name  == "OpenAI":
                openAIResponseStr = self.call_GPT_Service_OpenAI(OCR_Text_withInstruction, proc_api_key, userId,proc_mtd_value)
        ...
        for i in range(len(ai_proc_templ_updated)):
            ...
            if Model_Name  == "OpenAI":
                AI_model_result = self.call_GPT_Service_OpenAI(ai_proc_templ_updated[i], proc_api_key, userId,proc_mtd_value)
    ...
    def call_GPT_Service_OpenAI(self, text, proc_api_key, userId, proc_mtd_value, token_limit = 4096, max_response_tokens = 1800):
    ...
    def find_watermark(self, pdf_path):
        doc = fitz.open(pdf_path)
        large_texts = defaultdict(int)
        for page in doc:
                ...
                if block['type'] == 0:
                    for line in block['lines']:
                        for span in line['spans']:
                            ...
                            if size > 80 and len(text) > 2:
                                large_texts[text.lower()] += 1  # lowercase for consistency
        if not large_texts:
            logger.log("No Watermark found.")
            return None
        return max(large_texts, key=large_texts.get)
    
    def remove_detected_watermark(self, file_path):
        watermark_text = self.find_watermark(file_path)
        if not watermark_text:
            logger.log("No watermark detected. Exiting.")
            return

        logger.log(f"Detected watermark: '{watermark_text}'")
        ...
        logger.log(f"Watermark removed and file replaced: {file_path}")