...
from fit_better.data.preprocessing import train_test_split_with_indices
...
def load_and_match_data(
    features_file, target_file, max_samples=None, random_state=42, has_header=False
):
    """
    ...
    has_header (bool): Whether CSV files have headers (default: False)

    Returns:
    tuple: (X, y, ids, target_col_name)
    """
    print(f"Loading data from {features_file} and {target_file}...")
    print(f"Assuming headers: {'Yes' if has_header else 'No'}")

    # Use fit_better's match_xy_by_key with appropriate settings for header/headerless CSVs
    print("Using fit_better's match_xy_by_key function...")

    start_time = time.time()
    try:
        if has_header:
            # Peek at headers to understand structure
            features_sample = pd.read_csv(features_file, nrows=1)
            target_sample = pd.read_csv(target_file, nrows=1)

            id_col = features_sample.columns[0]
            target_id_col = target_sample.columns[0]
            target_col_name = (
                target_sample.columns[1]
                if len(target_sample.columns) > 1
                else target_sample.columns[0]
            )
            feature_columns = [col for col in features_sample.columns if col != id_col]
            ...
            # For headerless CSVs, use column indices
            # Assume first column is ID, remaining columns in features file are features
            # Assume first column is ID, second column in target file is target value
            X, y = match_xy_by_key(
                X_path=features_file,
                y_path=target_file,
                x_key_column=0,  # First column as ID
                y_key_column=0,  # First column as ID
                x_value_columns=None,  # Will use all columns except the key column
                y_value_column=1,  # Second column as target value
            )
            target_col_name = "target"  # Generic name since we don't have headers
        elapsed_time = time.time() - start_time
        print(f"Data matching completed in {elapsed_time:.2f} seconds.")
        print(f"Successfully matched {len(X)} samples with {X.shape[1]} features")

    except Exception as e:
        print(f"Error with column assumptions: {e}")
        print("Trying to infer structure from first few rows...")

        # Fallback: peek at the files to understand structure
        features_sample = pd.read_csv(features_file, nrows=5, header=None)
        target_sample = pd.read_csv(target_file, nrows=5, header=None)

        print(
            f"Features file structure - Columns: {features_sample.shape[1]}, Sample shape: {features_sample.shape}"
        )
        print(
            f"Target file structure - Columns: {target_sample.shape[1]}, Sample shape: {target_sample.shape}"
        )
        print("First few rows of features file:")
        print(features_sample.head(2))
        print("First few rows of target file:")
        print(target_sample.head(2))

        # Try again with adjusted parameters based on file structure
        if target_sample.shape[1] == 2:
            # Standard case: ID, Target
            x_value_cols = list(
                range(1, features_sample.shape[1])
            )  # All columns except first
            y_value_col = 1
        else:
            # Non-standard case: might be target only or different structure
            x_value_cols = (
                list(range(1, features_sample.shape[1]))
                if features_sample.shape[1] > 1
                else [0]
            )
            y_value_col = 1 if target_sample.shape[1] > 1 else 0

        print(f"Using feature columns: {x_value_cols}, target column: {y_value_col}")
        ...
        X, y = match_xy_by_key(
            ...
            x_key_column=0,
            y_key_column=0,
            x_value_columns=x_value_cols,
            y_value_column=y_value_col,
        )
        elapsed_time = time.time() - start_time
        print(f"Data matching completed in {elapsed_time:.2f} seconds.")
        print(f"Successfully matched {len(X)} samples with {X.shape[1]} features")
        target_col_name = "target"  # Generic name for fallback case

    # Handle large datasets with sampling if max_samples is specified
    if max_samples is not None and len(X) > max_samples:
        print(
            f"Dataset has {len(X)} samples. Sampling {max_samples} samples for training..."
        )
        np.random.seed(random_state)
        sample_indices = np.random.choice(len(X), size=max_samples, replace=False)
        X = X[sample_indices]
        y = y[sample_indices]
        print(f"Sampling completed. Using {len(X)} samples for training.")

    # Create a simple range for IDs since we don't need the actual IDs for modeling
...
def train_models(
    ...
    max_samples (int, optional): Maximum number of samples to use (for large datasets)
    has_header (bool, optional): Whether CSV files have headers (default: False)
    ...
    # Load and match data
    X, y, ids, target_col_name = load_and_match_data(
        features_file, target_file, max_samples, random_state, has_header
    )
...
Examples:
  ...
  python kmeans_mlp.py train --features data/train_features.csv --target data/train_target.csv --output-dir model/ --n-clusters 5 --regressor random_forest --partition-mode kmeans_plus_plus --n-jobs 4
    ...
    train_parser.add_argument(
        "--has-header",
        action="store_true",
        help="Whether CSV files have headers (default: no headers assumed)",
    )
