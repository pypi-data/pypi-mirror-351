...
X_aligned, y_aligned, matched_keys = match_xy_by_key(
    X_path="X_data.csv",
    y_path="y_data.csv",
...
def load_data_from_files(
    ...
    try:
        ...
        if match_by_key:
            ...
            X_train, y_train, train_keys = match_xy_by_key(
                X_train_path,
                y_train_path,
            ...
            X_test, y_test, test_keys = match_xy_by_key(
                X_test_path,
                y_test_path,
                x_key_column=x_key_column,
                y_key_column=y_key_column,
            ...
            # For y data, if it has multiple columns, extract only the target column
            # (assume the last column is the target if y_value_column is not specified)
            if y_train.ndim > 1 and y_train.shape[1] > 1:
                if y_value_column is not None:
                    # Extract specified target column
                    if isinstance(y_value_column, str):
                        # Need to get headers to find column index
                        if y_train_path.endswith((".csv", ".txt")):
                            import csv
                            ...
                            with open(y_train_path, "r") as f:
                                ...
                                if header == "infer":
                                    headers = next(reader)
                                    if y_value_column in headers:
                                        col_idx = headers.index(y_value_column)
                                        y_train = y_train[:, col_idx]
                                    else:
                                        raise ValueError(
                                            f"Column '{y_value_column}' not found in {y_train_file}"
                                        )
                    elif isinstance(y_value_column, int):
                        if y_value_column < y_train.shape[1]:
                            y_train = y_train[:, y_value_column]
                        else:
                            raise ValueError(
                                f"Column index {y_value_column} out of bounds in {y_train_file}"
                            )
                else:
                    # Default: use the last column as target
                    y_train = y_train[:, -1]
            ...
            if y_test.ndim > 1 and y_test.shape[1] > 1:
                if y_value_column is not None:
                    ...
                    if isinstance(y_value_column, str):
                        # Need to get headers to find column index
                        if y_test_path.endswith((".csv", ".txt")):
                            import csv
                            ...
                            with open(y_test_path, "r") as f:
                                ...
                                if header == "infer":
                                    ...
                                    if y_value_column in headers:
                                        ...
                                        raise ValueError(
                                            f"Column '{y_value_column}' not found in {y_test_file}"
                                        )
                    elif isinstance(y_value_column, int):
                        if y_value_column < y_test.shape[1]:
                            y_test = y_test[:, y_value_column]
                        else:
                            raise ValueError(
                                f"Column index {y_value_column} out of bounds in {y_test_file}"
                            )
                else:
                    # Default: use the last column as target
                    y_test = y_test[:, -1]
...
def match_xy_by_key(
    ...
    Returns:
        X, y, keys arrays with matching keys
        - X: Feature data with matched rows
        - y: Target data with matched rows
        - keys: Matched key values that link X and y rows
    ...
    try:
        ...
        # Extract original string keys before any numeric conversion affects them
        x_keys_original = [row[x_key_idx] for row in x_data]
        y_keys_original = [row[y_key_idx] for row in y_data]
        ...
        # Extract keys from both arrays using original string values
        x_keys = x_keys_original
        y_keys = y_keys_original

        logger.info(
            f"Starting key matching for {len(x_keys)} X keys and {len(y_keys)} Y keys..."
        )
        match_start_time = time.time()

        # Find matches between keys using efficient hash table approach (O(n) instead of O(n²))
        # Build hash table for y_keys with their indices
        y_key_to_indices = {}
        for j, y_key in enumerate(y_keys):
            if y_key not in y_key_to_indices:
                y_key_to_indices[y_key] = []
            y_key_to_indices[y_key].append(j)

        # Find matches by looking up x_keys in the hash table
        matched_rows = []
        for i, x_key in enumerate(x_keys):
            if x_key in y_key_to_indices:
                # Handle case where there might be duplicate keys in y
                for j in y_key_to_indices[x_key]:
        ...
        logger.info(
            f"Key matching completed in {match_elapsed_time:.3f} seconds using hash table approach"
        )
        ...
        keys_matched = np.array([key for _, _, key in matched_rows])

        # Convert to appropriate numeric types to avoid string conversion issues
        try:
            X_matched = X_matched.astype(float)
        except ValueError:
            # If conversion to float fails, try to convert individual columns
            X_converted = []
            for col_idx in range(X_matched.shape[1]):
                try:
                    col_data = X_matched[:, col_idx].astype(float)
                    X_converted.append(col_data)
                except ValueError:
                    # Keep as string if can't convert to numeric
                    X_converted.append(X_matched[:, col_idx])
            X_matched = np.column_stack(X_converted)

        try:
            y_matched = y_matched.astype(float)
        except ValueError:
            # Keep y as is if it can't be converted to float
            pass
        ...
        return X_matched, y_matched, keys_matched
