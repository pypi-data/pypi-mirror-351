...
# Type aliases для гибкости типов
LogLevelType = Union['LogLevel', int, str]
...
class FncLogger:
    ...
    def __init__(
            ...
            console_level: Optional[LogLevelType] = None,
            file_level: Optional[LogLevelType] = None,
            log_dir: Optional[PathType] = None,
        ...
        Args:
            ...
            level: Базовый уровень логирования (LogLevel, int или str)
        ...
        # Нормализуем уровни логирования
        self.base_level = self._normalize_level(level)
        self.console_level = self._normalize_level(console_level) or self.base_level
        self.file_level = self._normalize_level(file_level) or self.base_level
    ...
    def _normalize_level(self, level: Optional[LogLevelType]) -> Optional[LogLevel]:
        """Конвертирует различные типы уровней в LogLevel"""
        if level is None:
            return None
        elif isinstance(level, LogLevel):
            return level
        elif isinstance(level, int):
            # Конвертируем int в LogLevel
            for log_level in LogLevel:
                if log_level.value == level:
                    return log_level
            raise ValueError(f"Неизвестный уровень логирования: {level}")
        elif isinstance(level, str):
            # Конвертируем строку в LogLevel
            try:
                return LogLevel[level.upper()]
            except KeyError:
                raise ValueError(f"Неизвестный уровень логирования: {level}")
        else:
            raise TypeError(f"Неподдерживаемый тип уровня: {type(level)}")
    ...
    def _setup_formatters(self, date_format: str, custom_format: Optional[str]):
        ...
        # Rich форматтер (всегда доступен)
    ...
    def critical(self, message: str, extra: Optional[Dict[str, Any]] = None,
                 exc_info: bool = False, **kwargs) -> None:  # Изменено на False по умолчанию
        """Критическая ошибка"""
    ...
    def set_level(self, level: LogLevelType) -> 'FncLogger':
        """Изменение уровня логирования"""
        normalized_level = self._normalize_level(level)
        if normalized_level:
            self.logger.setLevel(normalized_level.value)
        return self

    def set_console_level(self, level: LogLevelType) -> 'FncLogger':
        """Изменение уровня для консольного вывода"""
        normalized_level = self._normalize_level(level)
        if normalized_level:
            self.console_level = normalized_level
            for handler in self.logger.handlers:
                if isinstance(handler, (logging.StreamHandler, type(None))):
                    if not isinstance(handler, logging.FileHandler):
                        handler.setLevel(normalized_level.value)
        return self

    def set_file_level(self, level: LogLevelType) -> 'FncLogger':
        """Изменение уровня для файлового вывода"""
        normalized_level = self._normalize_level(level)
        if normalized_level:
            self.file_level = normalized_level
            for handler in self.logger.handlers:
                if isinstance(handler, (RotatingFileHandler,)):
                    handler.setLevel(normalized_level.value)
        return self
...
def setup_basic_logger(name: str = "app", level: LogLevelType = "INFO") -> FncLogger:
