...
class Raster2STAC:
    ...
    @property
    def data(self):
        """Lazy-loading property that handles both paths and xarray objects"""
        if self._data is None:
            if isinstance(self._input_source, (xr.Dataset, xr.DataArray)):
                self._data = self._input_source
            elif isinstance(self._input_source, (str, Path)):
                path_str = str(self._input_source)
                if path_str.endswith(".zarr"):
                    if path_str.startswith("s3://"):
                        self._data = xr.open_zarr(
                            path_str,
                            storage_options={
                                "client_kwargs": {
                        ...
                        self._data = xr.open_zarr(path_str)
                else:
                    self._data = xr.open_dataset(path_str)

            elif isinstance(self._input_source, list):
                # Allow a list of file paths
                self._data = self._input_source
    ...
    def generate_zarr_stac(self, item_id=None):
        ...
        # Reads data from the property and setter
        if not isinstance(self.data, (xr.Dataset, xr.DataArray)):
            raise ValueError("Input must be convertible to xarray format")

        # Saving a copy to the data_ds for later extrction while reading bands individually
        if isinstance(self.data, xr.Dataset):
            # Use property setter to maintain encapsulation
            self.data_ds = self.data.copy(deep=True)
            self.data = self.data.to_dataarray(dim="bands")

        # 3. Get Dimensions from property
        self.X_DIM = self.data.openeo.x_dim
        self.Y_DIM = self.data.openeo.y_dim
        self.T_DIM = self.data.openeo.temporal_dims[0]
        self.B_DIM = self.data.openeo.band_dims[0]
        ...
        is_zarr_input = False
        if isinstance(self._input_source, str) and self._input_source.endswith(
            ".zarr"
        ):
            is_zarr_input = True
        elif (
            isinstance(self._input_source, Path)
            and self._input_source.suffix == ".zarr"
        ):
            is_zarr_input = True
        ...
        if is_zarr_input:
            ...
            if self.s3_upload:
                ...
                if isinstance(self._input_source, (str, Path)):
                    ...
                    raise ValueError("Invalid input source for Zarr input.")
        else:
            ...
            if self.s3_upload:
                ...
                self.data_ds.to_zarr(store, compute=True, mode="w")
                link_path = self._get_s3_link_path(item_id)
            else:
        ...
        proj_info = {
            f"proj:{name}": value
            for name, value in rioxarray_get_projection_info(self.data).items()
        }
