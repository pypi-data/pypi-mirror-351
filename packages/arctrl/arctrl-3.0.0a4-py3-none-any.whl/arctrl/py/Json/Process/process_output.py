...
def _arrow2669(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2670(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2671(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2669, ROCrate_decoder_1), map(_arrow2670, ROCrate_decoder_2), map(_arrow2671, ROCrate_decoder_3)]))
...
def _arrow2673(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2674(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2675(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2673, ISAJson_decoder_1), map(_arrow2674, ISAJson_decoder_2), map(_arrow2675, ISAJson_decoder_3)]))
