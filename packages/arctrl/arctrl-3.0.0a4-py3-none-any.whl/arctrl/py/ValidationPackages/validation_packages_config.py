...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3333(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3332(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3331(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3330(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3329(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow3328(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow3328))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3329))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3330))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3331))

            return append(singleton("{"), delay(_arrow3332))

        return join("\n", to_list(delay(_arrow3333)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3336(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3335(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3335, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3336))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3337
