...
class MaterialAttributeValue(Record):
    ...
    def PrintCompact(self, __unit: None=None) -> str:
        ...
        def _arrow570(__unit: None=None) -> str:
            value_2: str = value
            return value_2

        def _arrow571(__unit: None=None) -> str:
            category_2: str = category
            return (category_2 + ":") + "No Value"

        def _arrow572(__unit: None=None) -> str:
            category_1: str = category
            value_1: str = value
            return (category_1 + ":") + value_1

        return ("" if (value is None) else _arrow570()) if (category is None) else (_arrow571() if (value is None) else _arrow572())
...
MaterialAttributeValue_reflection = _expr573
...
def MaterialAttributeValue__SetCategory_ZDED3A0F(this: MaterialAttributeValue, c: OntologyAnnotation) -> MaterialAttributeValue:
    def _arrow574(__unit: None=None, this: Any=this, c: Any=c) -> MaterialAttribute | None:
    ...
    return MaterialAttributeValue(this.ID, _arrow574(), this.Value, this.Unit)
