...
from ...fable_modules.fable_library.seq import (exists, to_array, delay, collect, singleton, empty, fold)
...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow631(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow630(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow630, this.Cells)

        arr: Array[OntologyAnnotation] = to_array(delay(_arrow631))
...
CompositeColumn_reflection = _expr632
