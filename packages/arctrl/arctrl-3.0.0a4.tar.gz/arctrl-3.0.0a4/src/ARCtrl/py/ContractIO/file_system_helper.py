...
def directory_exists_async(path: str) -> Async[bool]:
    def _arrow3585(__unit: None=None, path: Any=path) -> Async[bool]:
        return singleton.Return(Path(path).is_dir())

    return singleton.Delay(_arrow3585)


def create_directory_async(path: str) -> Async[None]:
    def _arrow3586(__unit: None=None, path: Any=path) -> Async[None]:
        Path(path).mkdir(parents=True, exist_ok=True)
        return singleton.Zero()

    return singleton.Delay(_arrow3586)


def ensure_directory_async(path: str) -> Async[None]:
    def _arrow3588(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3587(_arg: bool) -> Async[None]:
            return singleton.ReturnFrom(create_directory_async(path)) if (not _arg) else singleton.Zero()

        return singleton.Bind(directory_exists_async(path), _arrow3587)

    return singleton.Delay(_arrow3588)


def ensure_directory_of_file_async(file_path: str) -> Async[None]:
    def _arrow3589(__unit: None=None, file_path: Any=file_path) -> Async[None]:
        return singleton.ReturnFrom(ensure_directory_async(Path(file_path).parent))

    return singleton.Delay(_arrow3589)


def file_exists_async(path: str) -> Async[bool]:
    def _arrow3590(__unit: None=None, path: Any=path) -> Async[bool]:
        return singleton.Return(Path(path).is_file())

    return singleton.Delay(_arrow3590)


def read_file_text_async(path: str) -> Async[str]:
    def _arrow3592(__unit: None=None, path: Any=path) -> Async[str]:
        def _arrow3591(__unit: None=None) -> str:
            with open(path, 'r', encoding='utf-8') as f: return f.read()

        return singleton.Return(_arrow3591())

    return singleton.Delay(_arrow3592)


def read_file_binary_async(path: str) -> Async[bytearray]:
    def _arrow3594(__unit: None=None, path: Any=path) -> Async[bytearray]:
        def _arrow3593(__unit: None=None) -> bytearray:
            with open(path, 'rb') as f: return f.read()

        return singleton.Return(_arrow3593())

    return singleton.Delay(_arrow3594)


def read_file_xlsx_async(path: str) -> Async[FsWorkbook]:
    def _arrow3595(__unit: None=None, path: Any=path) -> Async[FsWorkbook]:
        return singleton.Return(FsSpreadsheet_FsWorkbook__FsWorkbook_fromXlsxFile_Static_Z721C83C5(path))

    return singleton.Delay(_arrow3595)


def move_file_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3596(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
...
def delete_file_async(path: str) -> Async[None]:
    def _arrow3597(__unit: None=None, path: Any=path) -> Async[None]:
        try:
            os.remove(path)
    ...
    return singleton.Delay(_arrow3597)


def delete_directory_async(path: str) -> Async[None]:
    def _arrow3598(__unit: None=None, path: Any=path) -> Async[None]:
        shutil.rmtree(path, ignore_errors=True)
        return singleton.Zero()

    return singleton.Delay(_arrow3598)


def write_file_text_async(path: str, text: str) -> Async[None]:
    def _arrow3599(__unit: None=None, path: Any=path, text: Any=text) -> Async[None]:
        with open(path, 'w') as f: f.write(text)
        return singleton.Zero()

    return singleton.Delay(_arrow3599)


def write_file_binary_async(path: str, bytes: bytearray) -> Async[None]:
    def _arrow3600(__unit: None=None, path: Any=path, bytes: Any=bytes) -> Async[None]:
        with open(path, 'wb') as f: f.write(bytes)
        return singleton.Zero()

    return singleton.Delay(_arrow3600)


def write_file_xlsx_async(path: str, wb: FsWorkbook) -> Async[None]:
    def _arrow3601(__unit: None=None, path: Any=path, wb: Any=wb) -> Async[None]:
        FsSpreadsheet_FsWorkbook__FsWorkbook_toXlsxFile_Static(path, wb)
        return singleton.Zero()

    return singleton.Delay(_arrow3601)
...
def get_sub_directories_async(path: str) -> Async[Array[str]]:
    def _arrow3602(__unit: None=None, path: Any=path) -> Async[Array[str]]:
    ...
    return singleton.Delay(_arrow3602)


def get_sub_files_async(path: str) -> Async[Array[str]]:
    def _arrow3603(__unit: None=None, path: Any=path) -> Async[Array[str]]:
        paths: Array[str] = [str(entry) for entry in Path(path).iterdir() if entry.is_file()]
        return singleton.Return(map(standardize_slashes, paths, None))

    return singleton.Delay(_arrow3603)


def get_all_file_paths_async(directory_path: str) -> Async[Array[str]]:
    directory_path_1: str = standardize_slashes(directory_path)
    def _arrow3609(__unit: None=None, directory_path: Any=directory_path) -> Async[Array[str]]:
        def all_files(dirs: IEnumerable_1[str]) -> Async[IEnumerable_1[str]]:
            def _arrow3607(__unit: None=None, dirs: Any=dirs) -> Async[IEnumerable_1[str]]:
                def _arrow3606(_arg: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                    sub_files_1: IEnumerable_1[str] = concat(_arg)
                    def _arrow3605(_arg_1: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                        def _arrow3604(_arg_2: Array[IEnumerable_1[str]]) -> Async[IEnumerable_1[str]]:
                            sub_dir_contents_1: IEnumerable_1[str] = concat(_arg_2)
                            return singleton.Return(append(sub_dir_contents_1, sub_files_1))

                        return singleton.Bind(all(map_1(all_files, _arg_1)), _arrow3604)

                    return singleton.Bind(all(map_1(get_sub_directories_async, dirs)), _arrow3605)

                return singleton.Return(empty()) if is_empty(dirs) else singleton.Bind(all(map_1(get_sub_files_async, dirs)), _arrow3606)

            return singleton.Delay(_arrow3607)

        def _arrow3608(_arg_3: IEnumerable_1[str]) -> Async[Array[str]]:
        ...
        return singleton.Bind(all_files(to_enumerable([directory_path_1])), _arrow3608)

    return singleton.Delay(_arrow3609)


def rename_file_or_directory_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3612(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
        def _arrow3611(_arg: bool) -> Async[None]:
            def _arrow3610(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(old_path), _arrow3610)

        return singleton.Bind(file_exists_async(old_path), _arrow3611)

    return singleton.Delay(_arrow3612)


def delete_file_or_directory_async(path: str) -> Async[None]:
    def _arrow3615(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3614(_arg: bool) -> Async[None]:
            def _arrow3613(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(path), _arrow3613)

        return singleton.Bind(file_exists_async(path), _arrow3614)

    return singleton.Delay(_arrow3615)
