...
class LDSample:
    ...
    def validate_sample(s: LDNode, context: LDContext | None=None) -> bool:
        class ObjectExpr1546:
            @property
            def Equals(self) -> Callable[[str, str], bool]:
                def _arrow1545(x: str, y: str) -> bool:
                    return x == y
        ...
        return contains("Sample", s.AdditionalType, ObjectExpr1546()) if LDSample.validate(s, context) else False

    @staticmethod
    def validate_source(s: LDNode, context: LDContext | None=None) -> bool:
        class ObjectExpr1548:
            @property
            def Equals(self) -> Callable[[str, str], bool]:
                def _arrow1547(x: str, y: str) -> bool:
                    return x == y
        ...
        return contains("Source", s.AdditionalType, ObjectExpr1548()) if LDSample.validate(s, context) else False

    @staticmethod
    def validate_material(s: LDNode, context: LDContext | None=None) -> bool:
        class ObjectExpr1550:
            @property
            def Equals(self) -> Callable[[str, str], bool]:
                def _arrow1549(x: str, y: str) -> bool:
                    return x == y
        ...
        return contains("Material", s.AdditionalType, ObjectExpr1550()) if LDSample.validate(s, context) else False
