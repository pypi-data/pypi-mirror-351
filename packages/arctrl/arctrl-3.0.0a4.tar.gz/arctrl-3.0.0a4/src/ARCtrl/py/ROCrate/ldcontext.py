...
class LDContext:
    ...
    def PropertyNamesMatch(self, p1: str, p2: str) -> bool:
        ...
        if p1 == p2:
            ...
            def _arrow1430(__unit: None=None) -> bool:
                p2def_2: str = p2def
                return p1 == p2def_2
            ...
            def _arrow1432(__unit: None=None) -> bool:
                p1def_1: str = p1def
                p2def_1: str = p2def
                return p1def_1 == p2def_1

            return (_arrow1430() if (p2def is not None) else False) if (p1def is None) else (_arrow1431() if (p2def is None) else _arrow1432())
    ...
    def try_combine_optional(base_context: LDContext | None=None, specific_context: LDContext | None=None) -> LDContext | None:
        def _arrow1433(__unit: None=None) -> LDContext | None:
            s_1: LDContext = specific_context
            return s_1

        def _arrow1434(__unit: None=None) -> LDContext | None:
            f_1: LDContext = base_context
            return f_1

        def _arrow1435(__unit: None=None) -> LDContext | None:
            f: LDContext = base_context
            s: LDContext = specific_context
            return LDContext.combine(f, s)

        return (_arrow1433() if (specific_context is not None) else None) if (base_context is None) else (_arrow1434() if (specific_context is None) else _arrow1435())
    ...
    def __hash__(self, __unit: None=None) -> int:
        ...
        class ObjectExpr1436:
            @property
            def Compare(self) -> Callable[[str, str], int]:
                return compare_primitives

        mappings_hash: int = box_hash_seq(map(mapping, sort(this.Mappings.keys(), ObjectExpr1436())))
        name_hash: int
    ...
    def add_reverse_mapping(self, key: str, value: str) -> None:
        ...
        if active_pattern_result is not None:
            ...
            key_2: str = prefix
            value_2: tuple[str, str] = (suffix, key)
            dict_1_1: Any = this.compact_reverse_mappings
            ...
            out_arg: str = None
            def _arrow1437(__unit: None=None) -> str:
                return out_arg

            def _arrow1438(v: str) -> None:
                nonlocal out_arg
                out_arg = v

            pattern_input = (try_get_value(this.mappings_004045, prefix, FSharpRef(_arrow1437, _arrow1438)), out_arg)
            match_value = pattern_input[1] if pattern_input[0] else None
            if match_value is None:
                pass

            else: 
                key_4: str = IRIHelper_combine(match_value, suffix)
                value_3: str = key
                dict_1_3: Any = this.reverse_mappings
                if key_4 in dict_1_3:
                    dict_1_3[key_4] = value_3

                else: 
                    add_to_dict(dict_1_3, key_4, value_3)
            ...
            def _arrow1439(__unit: None=None) -> tuple[str, str]:
                return out_arg_1

            def _arrow1440(v_2: tuple[str, str]) -> None:
                nonlocal out_arg_1
                out_arg_1 = v_2

            pattern_input_1 = (try_get_value(this.compact_reverse_mappings, key, FSharpRef(_arrow1439, _arrow1440)), out_arg_1)
            ...
            if match_value_1 is None:
                ...
                key_6: str = IRIHelper_combine(value, match_value_1[0])
                value_4: str = term
                dict_1_5: Any = this.reverse_mappings
                if key_6 in dict_1_5:
                    dict_1_5[key_6] = value_4

                else: 
                    add_to_dict(dict_1_5, key_6, value_4)
    ...
    def try_find_term(self, term: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow1441(__unit: None=None) -> str:
            return out_arg

        def _arrow1442(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.mappings_004045, term, FSharpRef(_arrow1441, _arrow1442)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        ...
        if definition is None:
            ...
            def _arrow1443(__unit: None=None) -> str | None:
                prefix: str = active_pattern_result[0]
                suffix: str = active_pattern_result[1]
            ...
            def _arrow1444(__unit: None=None) -> str | None:
                d: str = definition
                return d

            return _arrow1443() if (active_pattern_result is not None) else _arrow1444()
    ...
    def try_find_iri(self, iri: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow1445(__unit: None=None) -> str:
            return out_arg

        def _arrow1446(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.reverse_mappings, iri, FSharpRef(_arrow1445, _arrow1446)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        def chooser(ctx: LDContext) -> str | None:
...
LDContext_reflection = _expr1447
