...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow3326(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3325(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3324(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3323(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow3323))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow3324))

            return append(singleton("{"), delay(_arrow3325))

        return join("\n", to_list(delay(_arrow3326)))
...
ValidationPackage_reflection = _expr3327
