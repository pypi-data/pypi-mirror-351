...
class UsageCommand(Command):
    ...
    def handle(self) -> int:
        ...
        if len(books) > 1:
            self.generate_multiple_books_documentation(books, env, master_document)
        else:
            self.generate_single_book_documentation(books, env, master_document)

        console.log("[bold blue]Done[/bold blue]")
        return 0

    def generate_multiple_books_documentation(self, books: Dict, env, master_document: Document) -> None:
        """
        ...
        for _, book_entry in books.items():
            book_name = self.generate_book_documentation(book_entry, env=env, multiple_book=True)
            doc_list.append(
    ...
    def generate_single_book_documentation(self, books: Dict, env, master_document: Document) -> None:
        """
        ...
        """
        for _, book_entry in books.items():
            self.generate_book_documentation(book_entry, env=env, usage_document=master_document)

    def generate_book_documentation(
...
with open("{tmp.name}", "w", encoding="utf-8") as f:
    ...
    def generate_usage(self, usage_document, book_class_name: str, book_metadata):
        ...
        self.add_book_logo(usage_document, book_metadata)
        self.add_book_reference_title(usage_document, book_name)
        self.add_book_description(usage_document, book_metadata)
        usage_document.add_table_of_contents(range(2, 4))
        self.add_book_connectivity(usage_document, book_metadata)
        self.add_book_configuration(usage_document, book_name, book_metadata)
        self.add_book_procedures(usage_document, book_metadata)
        self.add_book_concepts(usage_document, book_metadata)

    def add_book_connectivity(self, usage_document, book_metadata):
        ...
        if book_authentications:
            ...
            for auth in book_authentications:
                ...
                if credentials:
                    ...
                    self.add_credentials_table(usage_document, credentials)

    def add_book_configuration(self, usage_document, book_name, book_metadata):
        ...
        if book_configurations:
            ...
            self.add_concept_table(usage_document, concepts, include_required=False)
    ...
    def add_book_procedures(self, usage_document, book_metadata):
        ...
        for book_procedure in sorted_procedures:
            ...
            if input_concepts:
                ...
                self.add_concept_table(usage_document, input_concepts)

            output_concepts = book_procedure.get("output_concepts", None)
            if output_concepts:
                usage_document.add_heading("Output Concepts", 4)
                self.add_concept_table(usage_document, output_concepts, include_required=False, include_default=False)
    ...
    def add_book_description(self, usage_document, book_metadata):
    ...
    def add_book_reference_title(self, usage_document, book_name):
    ...
    def add_book_logo(self, usage_document, book_metadata):
    ...
    def add_concept_table(self, usage_document, concepts, include_required=True, include_default=True):
        ...
        for concept in concepts:
            ...
            if include_default:
                row.append(default_value if default_value is not None else "")

            rows.append(row)

        usage_document.add_table(header=headers, data=rows)

    def add_credentials_table(self, usage_document, credentials):
    ...
    def process_concept_fields(self, concept_type: Dict):
        ...
        for field in concept_type.get("fields", []):
            ...
            if inner_concept and inner_concept.get("class_name") in BOOK_CONCEPT_LIST:
                inner_name = inner_concept.get("name") or field.get("name", "")
                field_name = f'[`{field.get("name")}`](#{inner_name.lower().replace(" ", "-")})'
                field_type = inner_name
                if inner_concept.get("class_name") == "ConceptDictionaryType" and not inner_concept.get("fields"):
    ...
    def add_book_concepts(self, usage_document, book_metadata):
        ...
        def add_field_concepts(concept_dict: Dict, concepts_dict: Dict):
            ...
            for variable in concept_dict.get("fields", []):
                ...
                if nested_concept:
                    ...
                    variable_name = nested_concept.get("name") or variable.get("name")
        ...
        if concepts_map:
            ...
            for name, concept_type in concepts_map.items():
                ...
                headers, rows = self.process_concept_fields(concept_type)
                if rows:
                    usage_document.add_table(header=headers, data=rows)
