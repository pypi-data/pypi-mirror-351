...
class EizenSDK:
    ...
    def get_model_by_id(self, model_id):
        if self.__role == "Eizen":
            ...
            if model_id not in valid_ids:
                raise PermissionError("401 Unauthorized access: You are not authorized to access this model.")
            url = f"{self.base_url}/model/{model_id}"
            model = self.__make_request("GET", url)
            return {"id": model.get("id"), "name": model.get("name"),"endpointUrl": model.get("endpointUrl")}
    ...
    def check_generation_video_status(self, video_id: int):
        if self.__role in ["Administrator", "User"]:
            url_videos = f"{self.base_url}/video-generation/video/tenant/{self.__tenant_id}?page=0&size=10000"
            videos = self.__make_request("GET", url_videos)
            valid_video_ids = {video["id"] for video in videos.get("content", [])}
            if video_id not in valid_video_ids:
                raise PermissionError("401 Unauthorized access: You are not authorized to view the status of this video.")
