...
from sysnet_pyutils.data_utils import get_dict_value, get_dict_value_float, get_dict_value_datetime, get_dict_value_bool
...
class BaseEntryListType(BaseModel):
...
class GeneralType(BaseModel):
    ...
    permit_token: Optional[str] = Field(default=False, description='Token externího permitu')
...
class TransactionType(BaseModel):
    name: Optional[str] = Field(
        default=None,
        description='Název transakce (IMPORT, EXPORT, RE-EXPORT, OTHER, ...)',
        examples=['IMPORT'],
    )
...
class GoodsPrincipalType(GoodsItemBaseType):
    ...
    def load_dds(self, goods_item):
        ...
        try:
            ...
            if swin_transaction not in [None, '']:
            ...
            self.goods_additional = get_dict_value(goods_item, 'goods_additional')
...
class WorkflowListType(BaseEntryListType):
    ...
    def load_dds(self, data):
        ...
        if s1_date_created is not None:
            ...
            w1.load_data(
                '1-created', 'vytvořeno', None, s1_date_created, None,
                None, '2-authorized')
            # w1.date_execution=s1_date_created.strftime('%Y-%m-%dT%H:%M:%SZ')
            status = w1.node_code
