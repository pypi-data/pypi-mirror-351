...
class ComputeManager:
        ...
        if name and ip_address:
            ...
            for unit in self.compute_units_info['list']:
                if unit['name'] == name and unit['ip'] == ip_address:
                    return self._assing_compute_units(unit)
    ...
    def _assing_compute_units(self, unit_info):
        """
    ...
    def create(self, name, type_pointer, disk=10, 
        ...
        if response.status_code == 200:
            content = response.json()
            compute_unit = self._assing_compute_units(content)
            return compute_unit
        else:
            return response.status_code, response.text

    def list(self, table=True, return_info=False):
        """
...
class ComputeUnit:
    ...
    def __init__(self, user_pointer, user_secret, info, namespace_pointer=None, namespace_secret=None, namespace_api_url=NAMESPACE_API_URL):
        """
    ...
    def create_project(self, project_name, template):
        """
        Creates a project in the compute unit.
        ...
        """
        return self.projects.create(template)
