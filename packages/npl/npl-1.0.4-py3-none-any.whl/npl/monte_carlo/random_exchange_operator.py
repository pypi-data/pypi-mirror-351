...
class RandomExchangeOperator:
    ...
    def random_exchange(self, particle):
        ...
        symbol1_indices = np.random.choice(particle.get_indices_by_symbol(self.symbol1),
                                           n_exchanges, replace=False)
        symbol2_indices = np.random.choice(particle.get_indices_by_symbol(self.symbol2),
                                           n_exchanges, replace=False)
    ...
    def random_adsorbate_migration(self, particle):
        ...
        n_exchanges = min(np.random.geometric(p=self.p_geometric, size=1)[0],
                          self.max_adsorbate_exchanges)
        status0_indices = np.random.choice(particle.get_occupation_status_by_indices(0),
                                           n_exchanges, replace=False)
        status1_indices = np.random.choice(particle.get_occupation_status_by_indices(1),
                                           n_exchanges, replace=False)
    ...
    def coupled_random_exchange(self, particle):
        ...
        n_exchanges = min(np.random.geometric(p=self.p_geometric, size=1)[0], self.max_exchanges)
        symbol1_indices = np.random.choice(particle.get_indices_by_symbol(self.symbol1),
                                           n_exchanges*probability, replace=False)
        symbol2_indices = np.random.choice(particle.get_indices_by_symbol(self.symbol2),
                                           n_exchanges*probability, replace=False)
        ...
        n_exchanges = min(np.random.geometric(p=self.p_geometric, size=1)[0],
                          self.max_adsorbate_exchanges)
        status0_indices = np.random.choice(particle.get_occupation_status_by_indices(0),
                                           n_exchanges*(1-probability), replace=False)
        status1_indices = np.random.choice(particle.get_occupation_status_by_indices(1),
                                           n_exchanges*(1-probability), replace=False)
