...
def test_action():
    ...
    a = Action(["vision", "memory"], ["motor", "memory"], "motor=vision*A", "test_rule")
    assert a.condition is None
    assert str(a.effect) == "motor=A * vision"

    with pytest.raises(SpaParseError):
        a = Action(
            ["vision", "memory"], ["motor", "memory"], "motor=vis*A", "test_action"
        )
    ...
    with pytest.raises(SpaParseError):
        a = Action(
            ["vision", "memory"],
            ["motor", "memory"],
            "0.5*dot(memory+1, vision) --> motor=B",
            name="test_action",
        )

    with pytest.raises(SpaParseError):
        a = Action(
            ["vision", "memory"], ["motor", "memory"], "motor2=B", name="test_action"
        )
    with pytest.raises(SpaParseError):
        a = Action(
            ["vision", "memory"],
