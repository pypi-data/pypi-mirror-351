...
class GradCam:
    ...
    def grad_cam(self, image, ground_truth, module_name: str) -> torch.Tensor:
        """
        :param image: input image (torch_tools.tensor)
...
def torch_img_show(img):
    """
    :param img: torch_tools tensor image
    """
...
def torch_imgs_save(imgs, save_path: str = './'):
    ...
    if type(imgs) != torch.Tensor:
        print('\033[31mInput must be torch_tools tensor images (Tensor shape must be (?, 3, ?, ?))\033[0m')
        return False
    ...
    if imgs.shape[1] != 3 and imgs.shape[1] != 1:
        print('\033[31mInput must be torch_tools tensor images (Tensor shape must be (?, 3, ?, ?) if color scale or (?, 1, ?, ?) if gray scale)\033[0m')
        return False
