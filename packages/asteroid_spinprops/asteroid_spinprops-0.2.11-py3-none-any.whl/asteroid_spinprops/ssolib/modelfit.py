        ...
        if shg1g2_constrained is True:
            ...
            if blind_scan is True:
                ...
                period_scan = np.linspace(
                    period_sy - 20 / (24 * 60 * 60), period_sy + 20 / (24 * 60 * 60), 50
                )
                ...
                for ra, dec in zip([ra0, ra0alt], [dec0, dec0alt]):
                    for period_in in period_scan:
                        ...
                        sshg1g2 = get_fit_params(
                            data, "SSHG1G2", shg1g2_constrained=False, p0=p_in
                        )
                        try:
                ...
                Afit = estimate_sso_params(
                    ...
                    ra=np.radians(data["ra"].values[0][filter_mask]),
                    dec=np.radians(data["dec"].values[0][filter_mask]),
                    jd=utils.compute_ltc(
                        data["cjd"].values[0][filter_mask],
                        data["Dobs"].values[0][filter_mask],
                        data["cfid"].values[0][filter_mask],
                    ),
                    model="SSHG1G2",
                    p0=p0,
...
def plot_model(
    ...
    for i, f in enumerate(np.unique(data["cfid"].values[0])):
        ...
        if flavor == "SSHG1G2":
            if (f == 1) | (f == 2):
                jd_ltc = utils.compute_ltc(jd=jd, d_obs=eph["Dobs"].values, filt=None)
                jd_data_ltc = utils.compute_ltc(
                    jd=data["cjd"].values[0][filter_mask],
                    d_obs=data["Dobs"].values[0][filter_mask],
                    filt=None,
                )
            else:
                jd_ltc = np.array(jd)
...
def make_residuals_df(data, model_parameters):
    ...
    residual_df["jd"] = utils.compute_ltc(
        data["cjd"].values[0], data["Dobs"].values[0], data["cfid"].values[0]
    )
