    ...
    def get_rating_buttons(self, ar, detail_link=True):
        ...
        if rating_type is None:
            ...
            yield ar.instance_action_button(
                self.set_score_action, label=txt, request_kwargs=kw)
        else:
            ...
            for ch in rating_type.rating_choicelist.get_list_items():
                ...
                if value == ch.value:
                    ...
                    yield ar.instance_action_button(
                        self.set_score_action_directly,
...
class RatingBase(Ratable):
    class Meta:
        abstract = True
    teacher = dd.ForeignKey('users.User', verbose_name=_("Teacher"), blank=True, null=True, editable=False)
...
class RatingSummary(Summarized):
    ...
    ratings_done = dd.PercentageField(_("Ratings done"), decimal_places=0, blank=True, null=True, editable=False)
    total_score = dd.DecimalField(_("Total score"), max_digits=5, decimal_places=1, blank=True, null=True, editable=False)
    total_max_score = dd.DecimalField(_("Total max score"), max_digits=5, decimal_places=1, blank=True, null=True, editable=False)
