...
class SuperPage:
    ...
    def checking_thread(self):
        '''Проверяет состояние весов и обновляет отображение при необходимости'''
        while True:
            should_draw = (
                    self.operator.current in ['MainPage', 'ManualGateControl'] and
                    not self.operator.currentPage.blockImgDrawn and
                    self.operator.if_show_weight
            )
            ...
            try:
                if getattr(self.operator.currentPage, 'trail_win_drawn', False):
                    self.draw_weight(pos=(self.w / 2, self.h / 2 + 20), font=fonts.weight_body_font)
                elif getattr(self.operator.currentPage, 'weight_too_little', False):
                    self.draw_weight(pos=(self.w / 2 - 67, self.h / 2 - 157), font=fonts.weight_too_little_font)
            except AttributeError:
                pass
    ...
    def draw_weight(self, weight=None, pos=None, font=None):
        ...
        if not hasattr(self, '_weight_text_id') or self._weight_text_id is None:
            self._weight_text_id = self.can.create_text(*pos, text=text, font=font, fill='white', tags='kgel')
        else:
            self.can.itemconfig(self._weight_text_id, text=text, font=font)
            self.can.coords(self._weight_text_id, *pos)

        self.show_weight()

    def show_weight(self):
        if hasattr(self, '_weight_text_id'):
            self.can.itemconfig(self._weight_text_id, state='normal')
