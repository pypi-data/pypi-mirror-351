...
__all__ = ["ClickhouseSourceToTableInfoProcessor", "ClickhouseComposedTypeParser"]


# pylint: disable=R0903
class ClickhouseComposedTypeParser(ComposedTypeParser):
    def parse(
        self, ddl_datatype: str, simple_datatypes_mapping, compound_datatypes_mapping
    ) -> List[str]:
        if ddl_datatype.startswith("STRUCT"):
            return [compound_datatypes_mapping["STRUCT"], "string", "string"]
        inner_type = None
        outer_type = None
        ...
        if isinstance(the_inner_type, list):
            the_inner_type = [t for t in the_inner_type if t.endswith("_optimal")][0].removesuffix(
                "_optimal"
            )
    ...
    while open_backtick is not None:
        ...
        query_create_table_modified = query_create_table_modified.replace(
            ...
            normalize_variable_name(
                query_create_table_modified[open_backtick + 1 : close_backtick - 1]
            ),
        )
...
# pylint: disable=R0903
class ClickhouseSourceToTableInfoProcessor(SourceToTableInfoProcessor):
    def yield_table_info_tokens(
        self, source_mapping: str, mapper: DdlTypeToHdxTypeMappingFunc
    ) -> Iterator[Union[ColumnDefinition, Tuple[str, str]]]:
        ...
        for node in sqlglot_create_expression.bfs():
            if isinstance(node[0], sqlglot.expressions.Table):
                project, tablename = [str(n) for n in list(node[0].flatten())]
            ...
            if isinstance(node[0], sqlglot.expressions.ColumnDef):
                ...
                identifier, datatype = (
                    identifier_datatype_maybeconstraint[0],
                ...
                if constraint and isinstance(constraint, sqlglot.expressions.ColumnConstraint):
                    ...
                    if isinstance(constraint_type, sqlglot.expressions.NotNullColumnConstraint):
                        ...
                        constraint_type, sqlglot.expressions.PrimaryKeyColumnConstraint
                    ):
                        is_nullable = False
                yield ColumnDefinition(
