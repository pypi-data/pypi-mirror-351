...
def read_stereoSeq(
    ...
    count = pd.read_csv(
        os.path.join(path, "count.txt"), sep="\t", comment="#", header=0
    )
    ...
    exp_matrix = (
        sparse.csr_matrix((bin_data.values, (rows, cols)))
        if is_sparse
        else sparse.csr_matrix((bin_data.values, (rows, cols))).toarray()
    )
    obs = pd.DataFrame(index=cells)
...
def mclust_R(
    ...
    """
    Perform clustering using the mclust algorithm (via R interface).

    Parameters
    ----------
    adata : AnnData
        Annotated data matrix (typically with dimensionality-reduced data in adata.obsm).

    num_cluster : int
        The number of clusters to identify.

    modelNames : str, optional (default: 'EEE')
        The model type used by mclust. For example: 'EEE', 'VVV', etc.
        These follow the naming in the R `mclust` package.

    used_obsm : str, optional (default: 'emb_pca')
        The key in `adata.obsm` containing the input data matrix for clustering.

    random_seed : int, optional (default: 2020)
        Random seed for reproducibility.

    key_added : str, optional (default: 'mclust')
        The key under which the clustering results will be stored in `adata.obs`.

    Returns
    -------
    adata : AnnData
        The input AnnData object with cluster labels added to `adata.obs[key_added]`.
    """
    import numpy as np
    ...
    # Set R seed and load mclust
    robjects.r.library("mclust")
    robjects.r["set.seed"](random_seed)

    rmclust = robjects.r["Mclust"]

    # Run mclust on the selected obsm feature
    res = rmclust(
