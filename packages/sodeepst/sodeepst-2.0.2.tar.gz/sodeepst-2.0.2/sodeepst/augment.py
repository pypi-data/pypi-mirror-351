...
def cal_spatial_weight(
    ...
    if spatial_type == "NearestNeighbors":
        nbrs = NearestNeighbors(n_neighbors=spatial_k + 1, algorithm="ball_tree").fit(
            data
        )
...
def cal_weight_matrix(
    ...
    if spatial_type == "LinearRegress":
        ...
        physical_distance = pairwise_distances(
            adata.obs[["imagecol", "imagerow"]], metric="euclidean"
        )
        ...
        physical_distance = cal_spatial_weight(
            adata.obsm["spatial"], spatial_k=spatial_k, spatial_type=spatial_type
        )

    print(
        f"Spatial weights calculated. Average neighbors: {physical_distance.sum()/adata.shape[0]:.1f}"
    )

    # Calculate gene expression weights
    gene_correlation = cal_gene_weight(
        data=adata.X.copy(), gene_dist_type=gb_dist_type, n_components=n_components
    )
    ...
    if use_morphological:
        morphological_similarity = 1 - pairwise_distances(
            np.array(adata.obsm["image_feat_pca"]), metric=md_dist_type
        )
        ...
        adata.obsm["weights_matrix_all"] = (
            physical_distance * gene_correlation * morphological_similarity
        )
    else:
...
def augment_gene_data(
    ...
    if isinstance(adata.X, np.ndarray):
        augmented_matrix = adata.X + adjacent_weight * adata.obsm[
            "adjacent_data"
        ].astype(float)
    elif isinstance(adata.X, csr_matrix):
        augmented_matrix = adata.X.toarray() + adjacent_weight * adata.obsm[
            "adjacent_data"
        ].astype(float)
