    ...
    try:
        ...
        if client_erp_id is not None:
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'\tPedido [{order_id}]: Cliente ERP criado com sucesso: [{client_erp_id}]',
                LogType.INFO,
                'PEDIDO')
    ...
    try:
        ...
        cursor.execute(queries.get_insert_b2b_order_command(db_config.db_type),
                       queries.get_command_parameter(db_config.db_type, [
                           ...
                           str(datetime.strptime(order.order_date.split('.')[0].replace('T', ' '),  # data_pedido
                           ...
                           str(datetime.strptime(order.status_record.approved_at.split('.')[0].replace('T', ' '),
                                                 # data_pagamento
                        ...
                        if 'cancelado' in order.status.lower():  # Se o status do pedido for cancelado, e ainda nao existir no semaforo, ira protocolar
                            ...
                            if inserted:
                                send_log(
                                    ...
                                    job_config.get('enviar_logs'),
                                    job_config.get('enviar_logs_debug'),
                                    f'\tOKVENDAS - Pedido [{order.id}]: Pedido inserido com sucesso, '
                                ...
                                if sp_success:
                                    send_log(
                                        ...
                                        job_config.get('enviar_logs'),
                                        job_config.get('enviar_logs_debug'),
                                        f'\tOKVENDAS - Pedido [{order.id}]: Chamadas das procedures executadas '
                                        f'com sucesso, protocolando pedido...',
                                        LogType.INFO,
                                        'PEDIDO')
        ...
        with conn.cursor() as cursor:
            ...
            if existent_client is None:
                ...
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'\tPedido [{order.id}]: Inserindo cliente',
                    LogType.INFO,
                    'PEDIDO')
                cursor.execute(queries.get_insert_client_command(db_config.db_type),
                               queries.get_command_parameter(db_config.db_type, [
                ...
                if cursor.rowcount > 0:
                    send_log(
                        ...
                        job_config.get('enviar_logs'),
                        job_config.get('enviar_logs_debug'),
                        f'\tPedido [{order.id}]: Cliente inserido/atualizado', LogType.INFO, 'PEDIDO')
                    step = 'P.3 -Consultando o CLiente pelo CPF/CNPJ'
                    cursor.execute(queries.get_query_cliente_cpfcnpj(db_config.db_type),
                                   queries.get_command_parameter(db_config.db_type, [client_cpf_cnpj]))
                    step = 'P.4 -Query executada'
                    ...
                    send_log(
                        ...
                        job_config.get('enviar_logs'),
                        job_config.get('enviar_logs_debug'),
                        f'\tPedido [{order.id}]: Cliente Encontrado:{client_id}',
                        LogType.INFO,
                        'PEDIDO')
                ...
                if db_cpf_cnpj != client_cpf_cnpj:
                    send_log(
                        ...
                        f"mas era esperado {client_cpf_cnpj}, Pedido [{order.id}] ",
                        LogType.ERROR,
                        f'PEDIDO [{str(order.id)}]'
                    )
                    raise Exception(
                        f"Inconsistência: cliente ID {client_id} possui CPF/CNPJ {db_cpf_cnpj} "
                ...
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'\tPedido [{order.id}]: Atualizando cliente existente',
                    LogType.INFO,
                    'PEDIDO')
                step = 'P.6 -Antes de executar a Query'
                cursor.execute(queries.get_update_client_sql(db_config.db_type),
            ...
            if cursor.rowcount > 0:
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'\tPedido [{order.id}]: Inserido com sucesso',
                    LogType.INFO,
                    'PEDIDO')
                cursor.execute(queries.get_query_order(db_config.db_type),
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'\tPedido [{order.id}]: Inserindo ITENS do pedido',
                LogType.INFO,
                'PEDIDO')
