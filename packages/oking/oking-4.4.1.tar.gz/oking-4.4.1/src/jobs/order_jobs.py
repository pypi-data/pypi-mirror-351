                        ...
                        try:
                            ...
                            if invoice_sent is None:
                                ...
                                send_log(
                                    ...
                                    f'para api mplace: {invoice_sent}',
                                    LogType.ERROR,
                                    'NOTAFISCAL')
                        ...
                        try:
                            ...
                            if invoice_sent is None:
                                ...
                                send_log(
                                    ...
                                    f'para api okvendas: {invoice_sent.message}',
                                    LogType.ERROR,
                                    'NOTAFISCAL')
                        except Exception as e:
                            send_log(
        ...
        if src.client_data['operacao'].lower().__contains__('okvendas'):
            ...
            if qtd > 0:
                ...
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'Nenhuma Nota Fiscail encontrada',
                    LogType.WARNING,
                    'NOTASEMFILA')
        else:  # Operação OKING
            invoices = query_invoices(job_config, db_config)
    ...
    try:
        ...
        if updated is None or updated <= 0:
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Nao foi possivel atualizar o pedido informado',
                LogType.WARNING,
                'PEDIDO')
    ...
    try:
        ...
        if order_erp_id != '':
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Protocolando pedido com codigo_referencia {order_erp_id}',
                LogType.INFO,
                'PEDIDO')
            ...
            if updated_order_code:
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'Codigo Erp do pedido atualizado via api',
                    LogType.INFO,
                    'PEDIDO')
            else:
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'Falha ao atualizar o Codigo Erp do pedido via api',
                    LogType.WARNING,
                    'PEDIDO')
        ...
        if client_erp_id != '' and client_erp_id is not None:
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Salvando codigo do erp do cliente no banco semaforo {client_erp_id}',
                LogType.INFO,
                'PEDIDO')
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Protocolando cliente via api com codigo_referencia {client_erp_id}',
                LogType.INFO,
                'PEDIDO')
            ...
            if src.client_data['operacao'].lower().__contains__('okvendas'):
                ...
                if updated_client_code:
                    send_log(
                        ...
                        job_config.get('enviar_logs'),
                        job_config.get('enviar_logs_debug'),
                        f'Codigo Erp do cliente atualizado via api okvendas',
                        LogType.INFO,
                        'PEDIDO')
                else:
                    send_log(
                        ...
                        job_config.get('enviar_logs'),
                        job_config.get('enviar_logs_debug'),
                        f'Falha ao atualizar o Codigo Erp do cliente via api okvendas',
                        LogType.WARNING,
                        'PEDIDO')
        ...
        send_log(
            ...
            job_config.get('enviar_logs'),
            job_config.get('enviar_logs_debug'),
            f'Removendo pedido da fila pelo protocolo {queue_order.protocolo}',
            LogType.INFO,
            'PEDIDO')
        ...
        if protocoled_order:
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Falha ao protocolar pedido via api',
                LogType.WARNING,
                'PEDIDO')
        ...
        send_log(
            ...
            job_config.get('enviar_logs'),
            job_config.get('enviar_logs_debug'),
            f'Protocolando pedido no banco semaforo',
            LogType.INFO,
            'PEDIDO')
...
def call_order_procedures(job_config: dict, db_config: DatabaseConfig, order_id: int) -> (bool, str, str):
    ...
    try:
        ...
        send_log(
            ...
            job_config.get('enviar_logs'),
            job_config.get('enviar_logs_debug'),
            f'Pedido [{order_id}] - Executando procedure de cliente',
            LogType.INFO,  # Voltar para - LogType.DEBUG
            f'PROCEDURE_:{order_id}_P.1')
        ...
        if client_erp_id is not None:
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Pedido [{order_id}] - Procedure de Cliente Executada com sucesso'
                f' -- Cliente ERP retornado: [{client_erp_id}]',
                LogType.INFO,  # Voltar para - LogType.DEBUG
                f'PROCEDURE_:{order_id}_P.2')

            send_log(
                job_config.get('job_name'),
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'\tPedido [{order_id}]: Executando procedure de pedido',
                LogType.INFO,
                'PEDIDO')
            ...
            if order_erp_id is not None:
                ...
                send_log(
                    ...
                    job_config.get('enviar_logs'),
                    job_config.get('enviar_logs_debug'),
                    f'Pedido [{order_id}] - Procedure de PEDIDO Executada com sucesso'
                    f' -- PEDIDO ERP retornado: {order_erp_id}',
                    LogType.INFO,  # Voltar para - LogType.DEBUG
                    f'PROCEDURE_:{order_id}_P.3')
            else:
                success = False
...
def process_order_queue_okvendas(job_config: dict, status: str, db_config: DatabaseConfig,
    ...
    try:
        ...
        send_log(
            ...
            job_config.get('enviar_logs'),
            job_config.get('enviar_logs_debug'),
            f'OKVENDAS - Pedido - Iniciado[{status}]',
            LogType.EXEC,
            f'PEDIDO_FILA_{status}')
                    ...
                    if check_order_existence(db_config, order.id):
                        ...
                        if call_update_order_procedure(job_config, db_config, order.id, order.status):
                            send_log(
                                ...
                                job_config.get('enviar_logs'),
                                job_config.get('enviar_logs_debug'),
                                f'\tOKVENDAS - Pedido [{order.id}]: Pedido atualizado com sucesso',
                                LogType.INFO,
                                'PEDIDO')
                            set_codigo_erp_order(job_config, db_config=db_config, order=order, queue_order=q_order,
                    ...
                    # Pedido nao existe no semaforo e esta em status de internalizacao (pending e paid)
                        ...
                        if 'cancelado' in order.status.lower():
                            ...
                            send_log(
                                ...
                                job_config.get('enviar_logs'),
                                job_config.get('enviar_logs_debug'),
                                f'OKVENDAS - Pedido não existe no semaforo, e está com status de internalização '
                                f'(canceled), removendo pedido da fila pelo protocolo {q_order.protocolo}'
                                f'CPF:{(order.usuario.cpf or "")}, CNPJ:{(order.usuario.cnpj or "")}',
                                LogType.INFO,
                                f'CANCELADO_{order.id}')
                            ...
                            if protocoled_order:
                                send_log(
                                    ...
                                    job_config.get('enviar_logs'),
                                    job_config.get('enviar_logs_debug'),
                                    f'OKVENDAS - Pedido protocolado via api OkVendas',
                                    LogType.INFO,
                                    f'PEDIDO_{status}')
                            else:
                                send_log(
                        ...
                        # NOVO PEDIDO - Inserir Primeiro na SEMAFORO
                            ...
                            send_log(
                                ...
                                job_config.get('enviar_logs'),
                                job_config.get('enviar_logs_debug'),
                                f'OKVENDAS - Inserindo novo pedido no banco semaforo',
                                LogType.INFO,
                                'PEDIDO')
                            ...
                            if inserted:
                                ...
                                if sp_success:
                                    ...
                                    send_log(
                                        ...
                                        job_config.get('enviar_logs'),
                                        job_config.get('enviar_logs_debug'),
                                        f'Pedido [{order.id}] - PROCESSADO COM SUCESSO! (procedures executadas)'
                                        f' -- Protocolando pedido...',
                                        LogType.INFO,  # Voltar para - LogType.DEBUG
                                        f'FILA_PED_:{order.id}_P.3')
...
def insert_temp_order_okvendas(job_config: dict, order: OrderOkvendas, db_config: DatabaseConfig,
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            client_cpf_cnpj = client_cpf_cnpj.translate(str.maketrans('', '', string.punctuation))
            if src.print_payloads:
                print('client_cpf_cnpj:' + client_cpf_cnpj + ", tipo_pessoa: " + tipo_pessoa)
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Pedido [{order.id}]: CPF/CNPJ: [{client_cpf_cnpj}], '
                f'codigo_referencia: {(order.usuario.codigo_referencia or "")}',
                LogType.INFO,  # Voltar para - LogType.DEBUG
                f'TMP_ORDER:{order.id}_P.1')
            ...
            send_log(
                ...
                job_config.get('enviar_logs'),
                job_config.get('enviar_logs_debug'),
                f'Pedido [{order.id}]: Inserindo cabecalho pedido',
                LogType.INFO,
                f'TMP_ORDER:{order.id}_P.10')
            ...
            for item in order.itens:
                cursor.execute(queries.get_insert_b2b_order_items_command(db_config.db_type),
                               queries.get_command_parameter(db_config.db_type, [
                                   ...
                                   item.valor_taxa_servico,  # cnpj_filial_venda
                               ]))
