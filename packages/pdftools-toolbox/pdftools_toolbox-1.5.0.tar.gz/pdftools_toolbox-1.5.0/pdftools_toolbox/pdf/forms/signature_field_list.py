...
if TYPE_CHECKING:
    from pdftools_toolbox.pdf.forms.signature_field import SignatureField
...
class SignatureFieldList(_NativeObject, list):
    ...
    def __len__(self) -> int:
        _lib.PtxPdfForms_SignatureFieldList_GetCount.argtypes = [c_void_p]
        _lib.PtxPdfForms_SignatureFieldList_GetCount.restype = c_int
        ret_val = _lib.PtxPdfForms_SignatureFieldList_GetCount(self._handle)
        if ret_val == -1:
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[Any, List[Any]]:
        ...
        _lib.PtxPdfForms_SignatureFieldList_Get.argtypes = [c_void_p, c_int]
        _lib.PtxPdfForms_SignatureFieldList_Get.restype = c_void_p
        ret_val = _lib.PtxPdfForms_SignatureFieldList_Get(self._handle, index)
        if ret_val is None:
