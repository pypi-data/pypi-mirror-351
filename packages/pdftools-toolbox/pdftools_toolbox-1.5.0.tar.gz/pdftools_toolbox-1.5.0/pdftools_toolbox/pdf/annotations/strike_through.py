...
class StrikeThrough(pdftools_toolbox.pdf.annotations.text_markup.TextMarkup):
    """
    ...
    """
    @staticmethod
    def create_from_quadrilaterals(target_document: Document, markup_area: QuadrilateralList, paint: Paint) -> StrikeThrough:
        """
        Create a new strike-through with defined area

        The area to be striked through is defined by the given `markupArea`.
        ...
        Raises:
            ...
            ValueError:
                if the `paint` argument has a `pdftools_toolbox.pdf.content.paint.Paint.color_space` other than a device color space

            ValueError:
                if the `paint` argument has a non-`None``pdftools_toolbox.pdf.content.paint.Paint.transparency` with `pdftools_toolbox.pdf.content.transparency.Transparency.blend_mode` other than `pdftools_toolbox.pdf.content.blend_mode.BlendMode.NORMAL`
        ...
        _lib.PtxPdfAnnots_StrikeThrough_CreateFromQuadrilaterals.argtypes = [c_void_p, c_void_p, c_void_p]
        _lib.PtxPdfAnnots_StrikeThrough_CreateFromQuadrilaterals.restype = c_void_p
        ret_val = _lib.PtxPdfAnnots_StrikeThrough_CreateFromQuadrilaterals(target_document._handle, markup_area._handle, paint._handle)
        if ret_val is None:
            _NativeBase._throw_last_error(False)
        return StrikeThrough._create_dynamic_type(ret_val)
    ...
    def _from_handle(cls, handle):
        ...
        """
        instance = StrikeThrough.__new__(cls)  # Bypass __init__
