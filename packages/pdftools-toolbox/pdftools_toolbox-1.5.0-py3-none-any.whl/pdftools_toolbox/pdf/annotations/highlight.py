...
class Highlight(pdftools_toolbox.pdf.annotations.text_markup.TextMarkup):
    """
    ...
    """
    @staticmethod
    def create_from_quadrilaterals(target_document: Document, markup_area: QuadrilateralList, paint: Paint) -> Highlight:
        """
        Create a new highlight with defined area

        The highlight area is defined by the given `markupArea`.
        ...
        _lib.PtxPdfAnnots_Highlight_CreateFromQuadrilaterals.argtypes = [c_void_p, c_void_p, c_void_p]
        _lib.PtxPdfAnnots_Highlight_CreateFromQuadrilaterals.restype = c_void_p
        ret_val = _lib.PtxPdfAnnots_Highlight_CreateFromQuadrilaterals(target_document._handle, markup_area._handle, paint._handle)
        if ret_val is None:
            _NativeBase._throw_last_error(False)
        return Highlight._create_dynamic_type(ret_val)
    ...
    def _from_handle(cls, handle):
        ...
        """
        instance = Highlight.__new__(cls)  # Bypass __init__
