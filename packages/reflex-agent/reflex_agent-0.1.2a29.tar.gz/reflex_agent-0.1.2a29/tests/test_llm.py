...
from flexai.message import AIMessage, SystemMessage, UserMessage, TextBlock
...
async def test_anthropic_get_structured_response(mock_anthropic_client):
    ...
    result = await client.get_structured_response([UserMessage("Get info")], TestModel)
    assert isinstance(result.content, Sequence) and not isinstance(result.content, str)
...
def test_anthropic_add_cache_control():
    ...
    assert isinstance(result["system"], Sequence) and not isinstance(
        result["system"], str
    )
    ...
    assert isinstance(result["tools"], Sequence) and not isinstance(
        result["tools"], str
    )
    ...
    assert isinstance(result["messages"], Sequence) and not isinstance(
        result["messages"], str
    )
    ...
    assert isinstance(result["messages"][2]["content"], Sequence) and not isinstance(
        result["messages"][2]["content"], str
    )
    assert result["messages"][2]["content"][0]["cache_control"]
    assert isinstance(result["messages"][4]["content"], Sequence) and not isinstance(
        result["messages"][4]["content"], str
    )
