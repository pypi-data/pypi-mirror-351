...
class QueueWriterJson:
    ...
    def _get_file_info(self, filename: str) -> Tuple[str, str, str]:
        ...
        data_type = self.DATA_TYPE_MAPPING.get(extension, "file")
    ...
    def write_message(
        ...
        try:
            self.logger.info(
                f"Data arrived TEST LOF to data_source={data_source}, data_type={data_type}, filename={filename}, type={type(message_data)}, isinstance={isinstance(message_data, bytes)}"
            )
            ...
            # mission_upload_dir = self._mission_upload_dir(properties) # TODO
            ...
            if self.live_publish and send_live:
                try:
                    ...
                    if (
                        ...
                        self.logger.info(f"Data arrived TESSSSSST 7.1, else detected")
                    live_topic = f"{live_dir}/{filename_live}"
                    # self.logger.info(f"Data arrived TESSSSSST 8, {type(data_live)}")
                    chunk_message = {
                        "data": (
                            base64.b64encode(data_live).decode("utf-8")
                    ...
                    # self.logger.info(f"Data arrived TESSSSSST 10")
                    self.rabbit_mq.enqueue_message(
                        chunk_message, self.live_priority, self.expiration
                    )
                    # self.logger.info(f"Data arrived TESSSSSST 11, {len(chunk_message)}")
                    # self.logger.info(f"Data arrived TESSSSSST 12, chunk_message={chunk_message}")
                    self.logger.info(f"Data enqueued to {live_topic}")
            ...
            if should_chunk:
                ...
                fileinfo_message = {
                    "data": json.dumps(file_info_data),
                ...
                # TODO should_chunk or message_type != "json":
                self.rabbit_mq.enqueue_message(fileinfo_message, priority)
            ...
            if should_chunk:
                ...
                for i, chunk in enumerate(data_chunks):
                    try:
                        ...
                        self.rabbit_mq.enqueue_message(chunk_message, priority)
                        # if i == 0 and self.live_publish:  # LIVE TODO
                        #     try:
                        #         chunk_message = {
                        #             "data": base64.b64encode(chunk).decode("utf-8"),
                        #             "message_type": "binary",  # Always binary for chunks
                ...
                try:
                    ...
                    file_message = {
                        "data": (
                            base64.b64encode(message_data).decode("utf-8")
                            if message_type == "binary"
                    ...
                    self.rabbit_mq.enqueue_message(file_message, priority)
                    self.logger.info(f"Data enqueued to all {file_name_topic}")
                    # if self.live_publish:  # LIVE TODO
                    #     try:
                    #         file_message = {
                    #             "data": (
                    #                 base64.b64encode(message_data).decode("utf-8")
                    ...
                    #             ),
                    #             "message_type": message_type,
                    #             "topic": mission_upload_dir + "/" + filename,
                    #             "destination_ids": self.live_destinations,  # LIVE
                    #             "data_source": data_source,
                    #             # meta data
                    #             "buffer_key": buffer_key,
                    ...
                    #         }
                    #         self.rabbit_mq.enqueue_message(
                    #             file_message, self.live_priority, self.expiration
                    #         )
                    #     except Exception as e:
                    #         pass
                except Exception as e:
            ...
            error = f"Error in write_message: {str(e)}"
