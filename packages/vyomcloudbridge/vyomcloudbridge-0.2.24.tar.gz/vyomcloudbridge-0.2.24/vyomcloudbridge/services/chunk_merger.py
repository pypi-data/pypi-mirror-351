...
class ChunkMerger(ServiceAbstract):
    ...
    def on_chunk_file_arrive(self, s3_key: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"Publishing chunk merger notification for key: {s3_key}")
            ...
            self.rabbit_mq.enqueue_message(chunk_message, self.priority)
            self.logger.info(
                f"Chunk merger notification published successfully for: {s3_key}"
            )
