...
class MachineStats(ServiceAbstract):
    ...
    def get_current_buffer(self):
        ...
        try:
            ...
            self.logger.error(f"DuplicateGetOkCallback error in get_current_buffer: {e}")
        except Exception as e:
    ...
    def _set_current_buffer(self, data_size: int, data_size_uploaded: int):
        try:
            ...
            self.logger.error(f"DuplicateGetOkCallback error in _set_current_buffer: {e}")
        except Exception as e:
    ...
    def on_data_publish(self, size: int):
        try:
            data_size, data_size_uploaded = self.get_current_buffer()
            new_uploaded = min(data_size_uploaded + size, data_size)
    ...
    def _publish_stats_to_hq(self) -> bool:
        ...
        try:
            ...
            payload = {
                ...
                "data_size_uploaded": f"{data_size_uploaded:.2f}",
            ...
            self.rabbit_mq.enqueue_message(fileinfo_message, self.priority)
            self.logger.info("Machine stats publish SUCCESSFUL")
            return True
