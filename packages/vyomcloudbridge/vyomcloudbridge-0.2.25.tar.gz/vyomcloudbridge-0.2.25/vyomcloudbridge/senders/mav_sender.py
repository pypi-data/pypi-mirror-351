...
class MavSender(AbcSender):
    def __init__(self):
        try:
            ...
            # class specific
            # self.listener = MavListener() # AMAR
            self.mission_id = 0
            self.user_id = 1
            ...
            self.logger.info("Heartbeat received. MavSender initialized successfully!")
    ...
    def send_one_chunk(self, chunk_count, chunk, chunk_index, msgid):
        ...
        self.master.mav.vyom_message_send(
            ...
            chunk.encode("utf-8"),  # The 233-byte msg_text
    ...
    #         if not self.listener.ack_data_received.get(msg_id, {}).get(
    #             str(chunk_index)
    #         ):
    #             missing.append(chunk_index)

    #     self.logger.info(
    #         f"Full message sent and Acknowledgment received for each chunk."
    #     )
    ...
    def send_all_chunks_with_retries(self, chunk_count, chunks, current_msgid):
        ...
        for attempt in range(1, max_resends + 1):
            ...
            if not missing_chunks:
                self.logger.info("All chunks acknowledged.")
                break
        ...
        if missing_chunks:
            self.logger.error("Still missing ACKs after retries: {missing_chunks}")
            return False
        return True
    ...
    def send_mav_message(self, message, destination_id):
        ...
        self.logger.info("Sending message to GCS...")
    ...
    def send_message(
        ...
        Args:
            destinations (str): Destination group (e.g. 's3', 'hq_1', 'gcs_1').
        ...
        return self.send_mav_message(message, destination_id)
    ...
    def cleanup(self):
        ...
        print("[INFO] Cleaning up MAVLink connection...")
...
def main():
    ...
    print(
        sender.send_message(
            data_source="dummy_source",  # Replace with actual value
