...
class SendDataToServer(Node, ServiceAbstract):
    ...
    def update_mission_id(self):
        mission_detail, _ = self.get_current_mission()
        if mission_detail is None:
            return default_mission_id
        if mission_detail.get("mission_status") == 1:  # live
            return mission_detail.get("id") or default_mission_id
        return default_mission_id

    def create_listener_function(self, msg_type, f_topic):
        """
        ...
        def f(msg):
            self.logger.info(f"Ros data processing for ros_topic: {f_topic}")

            current_value = converter.convert(msg_type, 1, msg)
            ...
            self.writer.write_message(
                ...
                data_source=f_topic,  # telemetry, mission_summary
                ...
                destination_ids=["gcs2", "s3"],  # ["s3"] ["gcs2" via mqtt]
                merge_chunks=True,
                send_live=True
            )
            self.logger.info(f"Ros data processed for ros_topic: {f_topic}")
            ...
            self.subscribers.append(
                self.create_subscription(
                    ...
                    self.create_listener_function(
                        f_topic["data_type"], f_topic["name"]
                    ),
                    self.m_qos,
...
class RosPublisher(ServiceAbstract):
    def __init__(self):
        ...
        # self.logger.info("ROS 2 node spinning in background thread.")
    ...
    def start(self):
        ...
        try:
            ...
            #     target=rclpy.spin, args=(self.send_data_to_server,), daemon=True
            # )
...
def main(args=None):
    ...
    #
    # send_data_to_server = SendDataToServer()
    # rclpy.spin(send_data_to_server)
