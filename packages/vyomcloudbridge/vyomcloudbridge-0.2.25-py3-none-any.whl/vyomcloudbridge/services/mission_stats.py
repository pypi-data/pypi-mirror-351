...
class MissionStats(ServiceAbstract):
    ...
    def update_current_mission(  # for interenal use only, will be depreciated in future
        ...
        try:
            ...
            fileinfo_message = {
                "data": json.dumps({"mission": mission_detail, "data_stats": None}),
            ...
            self.rabbit_mq.enqueue_message(fileinfo_message, self.mission_live_priority)
    ...
    def start_mission(
        ...
        if name is None:
            name = f"Mission_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S_%f')[:-3]}_UTC"
    ...
    #         date = now.strftime("%Y-%m-%d")
    #         filename = int(time.time() * 1000)
    #         # mission_upload_dir = f"{self.machine_config['organization_id']}/{date}/{default_project_id}/{self.data_source_name}/{self.machine_id}/{mission_id}" # TODO
    #         mission_upload_dir: str = get_mission_upload_dir(
    #             organization_id=self.organization_id,
    #             machine_id=self.machine_id,
    #             mission_id=mission_id,
    ...
    #         fileinfo_message = {
    #             "data": json.dumps({"mission_telemetry": mission_telemetry}),
    #             "topic": f"{mission_upload_dir}/{filename}.json",
    #             "message_type": "json",
    #             "destination_ids": ["s3"],
    #             "data_source": self.data_source_name,
    ...
    def publish_mission_summary(self, mission_id: int, mission_summary: Any):
        ...
        try:
            ...
            fileinfo_message = {
                "data": json.dumps({"mission_summary": mission_summary}),
                "topic": f"{mission_upload_dir}/{filename}.json",
                "message_type": "json",
                "destination_ids": ["s3"],
                "data_source": self.data_source_name,
    ...
    def on_mission_data_arrive(
        ...
        size: int,
        file_count: int = 1,
        data_type: str = None,  # Not in use
        data_source: str = None,  # Not in use
        s3_dir: str = None,  # Not in use
    ):
        """
        Args:
            mission_id: The ID of the mission
        ...
        new_mission_stats["file_count"] += file_count
        new_mission_stats["data_size"] += size
    ...
    def on_mission_data_publish(
        ...
        new_mission_stats["file_count_uploaded"] += file_count
        new_mission_stats["data_size_uploaded"] += size
    ...
    def _publish_stats_to_hq(self, mission_id: Union[int, str]):  # PUBLISHER FUNCTION 1
        try:
            ...
            self.logger.info(
                f"publishing {self.data_source_name}... for mission_id: {mission_id}"
            )
            if isinstance(mission_id, int):
                mission_id = str(mission_id)
            ...
            fileinfo_message = {
                "data": json.dumps({"mission": None, "data_stats": mission_data_stats}),
                "topic": f"{mission_upload_dir}/{filename}.json",
                "message_type": "json",
                "destination_ids": ["s3"],
                "data_source": self.data_source_name,
            ...
            self.rabbit_mq.enqueue_message(fileinfo_message, self.mission_stats_live_priority)
    ...
    #         fileinfo_message = {
    #             "data": json.dumps({"mission": current_mission, "data_list": None}),
    #             "topic": f"{mission_upload_dir}/{filename}.json",
    #             "message_type": "json",
    #             "destination_ids": ["s3"],
    #             "data_source": self.data_source_name,
