...
def render_table(title, data, headers, progress_columns=None):
            ...
            if h in progress_columns and valor.replace("%", "").replace(",", ".").replace(".", "").isdigit():
                ...
                <td class='px-3 py-2 border-b border-gray-100 text-sm'>
                    <div class="relative w-full bg-gray-100 rounded h-4 overflow-hidden">
                        <div class="absolute left-0 top-0 h-full bg-[color:var(--primary)]" style="width:{perc}%"></div>
                        <div class="relative text-center z-10 text-xs leading-4">{valor}</div>
                    </div>
                </td>
                """
            else:
