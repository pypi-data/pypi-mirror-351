...
@pytest.mark.integration
def test_truss_control_server_start(control_server: ControlServerDetails):
    # Patch to identity code
    stream_model_code = """
import time
class Model:
    def load(self):
        time.sleep(60)  # Simulate a long load time
    def predict(self, model_input):
        def inner():
            for i in range(5):
                yield str(i)
...
# @pytest.mark.integration
# def test_truss_control_server_model_not_found(control_server: ControlServerDetails):
#     # Patch to identity code
#     stream_model_code = """
# import time
# class Model:
