...
import time
@pytest.mark.integration
def test_truss_control_server_model_not_loaded(control_server: ControlServerDetails):
    # Patch to identity code
    stream_model_code = """
import time
class Model:
...
"""
    ...
    # ctrl_url = f"http://localhost:{control_server.control_server_port}"
    # time.sleep(2)
    # resp = requests.post(f"{ctrl_url}/v1/models/model:predict", json={}, stream=False)
