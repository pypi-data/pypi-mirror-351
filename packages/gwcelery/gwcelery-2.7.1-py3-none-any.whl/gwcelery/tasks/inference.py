...
from rapidpe_rift_pipe.cli import main as rapidpe_rift_pipe_main
...
def _setup_dag_for_bilby(
    ...
    Notes
    -----
    Condor queue is set to `Online_PE` if gracedb host
    is `gracedb.ligo.org`, and `Online_PE_MDC` otherwise. `bilby_pipe`
    strips the `{IFO}:` prefix from channel names to avoid breaking the
    ...
    setup_arg = ['bilby_pipe_gracedb', '--webdir', path_to_webdir,
                 ...
                 '--llhoft-glob', app.conf['llhoft_glob'],
                 ]
    channels = {
        key: value.strip(f"{key}:") for key, value
        in app.conf['strain_channel_names'].items()
    }
...
def _setup_dag_for_rapidpe(rundir, superevent_id, event):
    ...
    # set up dag
    try:
        patch_argv = unittest.mock.patch('sys.argv', ["", path_to_ini])
        with handling_system_exit(), patch_argv:
            rapidpe_rift_pipe_main()

    except RuntimeError as e:
        contents = b'args:\n' + json.dumps(sys.argv).encode('utf-8')

        message = 'Failed to prepare DAG for Rapid PE'

        fail_gracefully = False
        if (
            isinstance(e.__cause__, SystemExit) and
            e.__cause__.code == _RAPIDPE_NO_GSTLAL_TRIGGER_EXIT_CODE
        ):
...
def start_pe(event, superevent_id, pe_pipeline):
    ...
    if pe_pipeline == 'bilby':
        if (
            app.conf['gracedb_host'] in [
                'gracedb-playground.ligo.org', 'gracedb-test.ligo.org'
            ] and event['extra_attributes']['CoincInspiral']['mchirp'] >= 12
        ):
            # Count the number of BBH jobs and do not start a run if it exceeds
            # 3 so that we do not use up disk space. We assume that the job is
