...
2.7.1 "Silurian" (2025-05-28)
-----------------------------

-   Disable public alerts on production.

2.7.0 "Silurian" (2025-05-27)
-----------------------------

-   Limit the number of bilby-bbh runs for gracedb-test triggers.

-   RAVEN will now specify which events were involved in the log message when
    waiting for both GW and external sky maps.

-   RapidPE/RIFT Pipeline is now invoked via function call instead of
    subprocess, allowing for a more informative stack trace.

-   Reduce bilby timeout to 100s.

-   Ensure positive joint FARs are preferred over negative joint FARs when
    choosing preferred external event. Automatically update joint FAR info
    in superevent if same preferred external event and the spatial joint
    FAR is present.

-   Constrain igwn-auth-utils >= 1.3.1 at the request of its maintainers.

-   Constrain ligo.skymap >= 2.3.0.

-   Constrain hop-client < 0.11.0.

-   Add support for new CBC pipeline SGNL.

-   Increase SSM trials to include RAVEN.

-   Constrain igwn-alert >= 0.7.0.

-   Add SVOM/ECLAIRS to GRB pipelines.

-   Configuration changes needed for playground to use O4 LLPIC data and dev to
    use O3 Replay MDC data.

-   Add support for new CBC pipeline Aframe.

-   Upgrade rapidpe-rift-pipe to 0.7.1 to support LLPIC

-   Upload course grained chirp mass information for CBC events.

-   Constrain ligo-followup-advocate >= 1.3.2.

-   Take channel and data location information from configuration for Bilby.

-   Decrease rate of primary and kafka celery workers hanging at shutdown.

-   Remove flask < 3.0 constraint.

-   Constrain tensorflow <= 2.16.2.

-   Bump ``poetry`` version in CI pipeline to 2.1.1.

-   Change level of "detchar data not found" from logs to warnings.

-   Add macOS unit tests to CI pipeline.
