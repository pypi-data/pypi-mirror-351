...
class pybrv_recommendation:
    ...
    def analyze_multiple_tables(self, table_names: List[str]) -> str:
        dfs = {table: spark.table(table) for table in table_names}
        report = []
  ...
  Column: `{col_name}`
    ...
    def get_test_cases_for_tables(self, table_names: list, metadata: str) -> str:
        ...
        Your task is to review the metadata of the tables `{', '.join(table_names)}` and generate data quality test cases as SQL queries.
        Suggest test cases to validate data integrity, consistency, and relationships.

        ### Requirements:
        - Give the pass sql and fail sql for each test case.
        - Specify severity (low, medium, high, critical).
        - Cover: Null checks, Uniqueness checks, Completeness, Duplicates, Accuracy, Validation.
        - Provide a JSON config for each test case with:
            "RULE_ID", "RULE_NAME", "FAIL_SQL", "PASS_SQL", "SEVERITY", "RULE_CATEGORY", "TABLES_CHECKED", "INVENTORY", "COMMENTS", "PASS_THRESHOLD"
