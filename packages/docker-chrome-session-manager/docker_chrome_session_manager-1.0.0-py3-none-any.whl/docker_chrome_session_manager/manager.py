...
class SessionManager:
    ...
    def get_remote_driver(self, resource_id: str, session_config: Optional[SessionConfig] = None) -> None | WebDriver:
        ...
        if self._invalid_resource_id(resource_id):
            logging.error(f"Invalid resource ID: {resource_id}")
            return None

        if not session_config:
            loaded_config = self._load_session_config_fields(resource_id)
            if not loaded_config:
                logging.warning(f"No session configuration found for resource_id: {resource_id}.")
                return None
            session_config = SessionConfig(**loaded_config)
            logging.info(f"Loaded existing session configuration for resource_id: {resource_id}.")

        container_path = self._choose_free_container_path()
        if not container_path:
            logging.error("No free containers available.")
            return None
        ...
        if session_config.timezone:
            ...
            logging.debug(f"Added timezone argument: {session_config.timezone}.")
        ...
        logging.debug(f"Chrome options configured for resource_id: {resource_id}.")
        ...
        try:
            ...
            if session_config.timezone:
                ...
                logging.debug(f"Set timezone via CDP to: {session_config.timezone}.")
            logging.info(f"WebDriver created for resource_id: {resource_id} on container: {container_path}.")
            return driver
        except Exception as e:
            logging.error(f"Failed to create WebDriver for resource_id: {resource_id} "
    ...
    def forget_session_config(self, resource_id: str) -> bool:
        ...
        resource_id = str(resource_id)
        if self._invalid_resource_id(resource_id):
            logging.error(f"Invalid resource ID for forgetting session: {resource_id}.")
            return False
        config_path = self._get_session_config_path(resource_id)
        if os.path.exists(config_path):
            os.remove(config_path)
            logging.info(f"Session configuration deleted for resource_id: {resource_id}.")
            return True
        logging.warning(f"No session configuration found to delete for resource_id: {resource_id}.")
        return False
    ...
    def provide_session_config(
        ...
        resource_id = str(resource_id)
        if self._invalid_resource_id(resource_id):
            logging.error(f"Invalid resource ID for providing session config: {resource_id}.")
        ...
        if loaded_config_fields:
            ...
            logging.info(f"Created new session configuration for resource_id: {resource_id}.")

        if not session_config.user_agent:
            session_config.user_agent = self.user_agent_factory.random
            logging.debug(f"Generated random user agent: {session_config.user_agent}.")

        if not session_config.resolution:
            session_config.resolution = random.choice(self.config.screen_resolutions)
            logging.debug(f"Selected random resolution: {session_config.resolution}.")

        if not session_config.locale:
            session_config.locale = self.config.default_locale
            logging.debug(f"Set default locale: {session_config.locale}.")

        if locale:
            session_config.locale = locale
            logging.debug(f"Updated locale to: {locale}.")

        if timezone:
            session_config.timezone = timezone
            logging.debug(f"Updated timezone to: {timezone}.")

        if extra_chrome_options:
            session_config.extra_chrome_options = extra_chrome_options
            logging.debug(f"Updated extra Chrome options: {extra_chrome_options}.")
        ...
        if session_config_fields != loaded_config_fields:
            ...
            logging.info(f"Saved updated session configuration for resource_id: {resource_id}.")
    ...
    def _choose_free_container_path(self) -> Optional[str]:
        ...
        if not self.container_paths:
            logging.warning("Containers are not defined.")
            return None
            ...
            for container_path in self.container_paths:
                ...
                try:
                    ...
                    if response.status_code == requests.codes.ok and response.json().get('value', {}).get('ready', False):
                        logging.debug(f"Container available: {container_path} on attempt {attempt + 1}.")
        ...
        logging.warning("No free containers found after all retries.")
        return None
    ...
    def _get_session_config_path(self, resource_id: str) -> str:
        ...
        resource_id = str(resource_id)
        if self._invalid_resource_id(resource_id):
            logging.error(f"Invalid resource ID for config path: {resource_id}.")
            raise ValueError("Invalid resource ID.")
        ...
        with FileLock(lock_path, timeout=self.config.file_lock_timeout):
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    logging.debug(f"Loading session configuration from: {config_path}.")
                    return json.load(f)
        logging.debug(f"No session configuration found at: {config_path}.")
        return None
    ...
    def _save_session_config(self, resource_id: str, config: dict):
        ...
        with FileLock(lock_path, timeout=self.config.file_lock_timeout):
            ...
            logging.debug(f"Saved session configuration to: {config_path}.")
