...
class DlnirspTestingParameters:
    ...
    dlnirsp_static_bad_pixel_map_vis: FileParameter = FileParameter(param_path=".")
    dlnirsp_static_bad_pixel_map_jband: FileParameter = FileParameter(param_path=".")
    dlnirsp_static_bad_pixel_map_hband: FileParameter = FileParameter(param_path=".")
    dlnirsp_bad_pixel_gain_median_smooth_size: tuple[int, int] = (1, 10)
    dlnirsp_bad_pixel_dark_median_smooth_size: tuple[int, int] = (10, 1)
    dlnirsp_bad_pixel_gain_sigma_threshold: float = 9.0
    dlnirsp_bad_pixel_dark_sigma_threshold: float = 5.0
    ...
    dlnirsp_polcal_demodulation_spatial_poly_fit_order: int = 2
...
class ModulatedObserveHeaders(DlnirspHeaders):
    def __init__(
        ...
        num_modstates: int,
        dither_mode_on: bool = False,
        aborted_loop_level: Literal[
            "dither", "mosaic", "X_tile", "Y_tile", "data_cycle", "modstate"
        ]
        | None = None,
...
def vis_static_bad_pix_array(group_id_array):
    map = np.zeros(group_id_array.shape, dtype=int)
    map[5, 19] = 1
    return map
...
@pytest.fixture
def vis_static_bad_pix_map_file_parameter(vis_static_bad_pix_array, tmp_path) -> FileParameter:
    file_path = tmp_path / "static_bad_pix_map_vis.fits"
    fits.PrimaryHDU(vis_static_bad_pix_array).writeto(file_path)

    return FileParameter(param_path=str(file_path))


@pytest.fixture
def jband_static_bad_pix_map_file_parameter(jband_static_bad_pix_array, tmp_path) -> FileParameter:
    file_path = tmp_path / "static_bad_pix_map_jband.fits"
    fits.PrimaryHDU(jband_static_bad_pix_array).writeto(file_path)

    return FileParameter(param_path=str(file_path))


@pytest.fixture
def hband_static_bad_pix_map_file_parameter(hband_static_bad_pix_array, tmp_path) -> FileParameter:
    file_path = tmp_path / "static_bad_pix_map_hband.fits"
    fits.PrimaryHDU(hband_static_bad_pix_array).writeto(file_path)
