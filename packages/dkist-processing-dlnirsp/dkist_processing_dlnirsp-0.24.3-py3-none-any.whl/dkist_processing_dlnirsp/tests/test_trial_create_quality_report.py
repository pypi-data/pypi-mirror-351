...
def create_trial_quality_report_task(
    ...
    constants_db = DlnirspTestingConstants(NUM_MODSTATES=2, POLARIMETER_MODE="Full Stokes")
...
def test_create_trial_quality_report(create_trial_quality_report_task):
    ...
    task()
    # Then
    paths = list(task.read(tags=[DlnirspTag.output(), DlnirspTag.quality_report()]))
    assert len(paths) == 1
    quality_report = next(
        task.read(tags=[DlnirspTag.output(), DlnirspTag.quality_report()], decoder=bytes_decoder)
    )
