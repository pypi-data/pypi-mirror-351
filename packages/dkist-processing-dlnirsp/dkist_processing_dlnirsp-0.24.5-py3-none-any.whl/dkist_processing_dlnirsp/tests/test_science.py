...
def science_task_with_data(
    ...
    with (
        ...
        task.write(
            data=jband_static_bad_pix_array,
...
def test_basic_corrections(
    ...
    Then: The dark and solar signals were removed as expected and the bad pixel mask has been applied
    """
    ...
    np.testing.assert_allclose(np.nanmean(corrected_data), expected_signal)
    assert np.sum(np.isnan(corrected_data)) == np.sum(jband_static_bad_pix_array)
