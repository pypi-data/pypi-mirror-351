...
from dkist_processing_dlnirsp.tests.local_trial_workflows.e2e_dev_mockers import (
...
def input_dataset_parameter_task_with_files(param_dir: str | Path):
    ...
    paramter_data = DlnirspTestingParameters(
    ...
    for arm_id in ["vis", "jband", "hband"]:
        static_bad_pixel_map_file_name = param_dir / f"dlnirsp_static_bad_pixel_map_{arm_id}.dat"
        setattr(
            paramter_data,
            f"dlnirsp_static_bad_pixel_map_{arm_id}",
            FileParameter(param_path=str(static_bad_pixel_map_file_name)),
        )

        group_id_file_name = param_dir / f"dlnirsp_ifu_id_array_{arm_id}.dat"
        setattr(
            paramter_data,
            f"dlnirsp_group_id_file_{arm_id}",
            FileParameter(param_path=str(group_id_file_name)),
        )

        dispersion_file_name = param_dir / f"dlnirsp_dispersion_array_{arm_id}.dat"
        setattr(
            paramter_data,
            f"dlnirsp_geo_dispersion_file_{arm_id}",
            FileParameter(param_path=str(dispersion_file_name)),
        )

        ifu_x_pos_file_name = param_dir / f"dlnirsp_ifu_xpos_array_{arm_id}.dat"
        setattr(
            paramter_data,
            f"dlnirsp_ifu_x_pos_file_{arm_id}",
            FileParameter(param_path=str(ifu_x_pos_file_name)),
        )

        ifu_y_pos_file_name = param_dir / f"dlnirsp_ifu_ypos_array_{arm_id}.dat"
        setattr(
            paramter_data,
            f"dlnirsp_ifu_y_pos_file_{arm_id}",
            FileParameter(param_path=str(ifu_y_pos_file_name)),
        )
    ...
    class CreateInputDatasetParameterDocument(WorkflowTaskBase):
        def run(self) -> None:
            ...
            self.tag(doc_path, DlnirspTag.input_dataset_parameters())
        ...
        def input_dataset_document_simple_parameters_part(self):
            ...
            for pn, pv in asdict(paramter_data).items():
                values = [
                    {
