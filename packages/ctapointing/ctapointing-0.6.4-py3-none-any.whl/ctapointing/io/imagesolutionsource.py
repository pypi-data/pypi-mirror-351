...
class ImageSolutionSource(Component):
    """
    Class to read ImageSolution objects from HDF5 input file.
    """

    input_url = Path(help="Path to the input file containing image solutions.").tag(
        config=True
    )
    ...
    def __init__(self, input_url=None, config=None, parent=None, **kwargs):
        ...
        Provenance().add_input_file(str(self.input_url), role="imagesolution")
    ...
    def __len__(self):
        n_events = len(self.file_.list_nodes("/image_solutions"))
    ...
    def __iter__(self):
        """
        Iterate over ImageSolution tables

        """
        # avoid circular import
        from ctapointing.imagesolver import ImageSolution

        # determine list of spotlist tables
        nodes = self.file_.list_nodes("/image_solutions")
