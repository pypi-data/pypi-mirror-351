...
class ScienceCameraFrame(BaseCoordinateFrame):
    """
    ScienceCamera coordinate frame.

    Represents a science camera coordinate frame.
    This frame is similar to a ctapipe.coordinates.CameraFrame,
    but allows for horizontal and vertical tilts and offsets of the system w.r.t. the
    TelescopeFrame.
    ...
    - tilted w.r.t. the Telescope frame by angles
      ...
      SkyCameraFrame is tilted towards positive altitude), tilt_y (in the horizontal
    ...
    - the x coordinate points towards the ground
    - the y coordinate points towards West
    when the TelescopeSystem points to the horizon in Northern direction and no
    tilts/rotations have been applied.
...
@frame_transform_graph.transform(FunctionTransform, TelescopeFrame, ScienceCameraFrame)
def telescope_to_sciencecamera(telescope_coord, camera_frame):
    """
    Transformation between TelescopeFrame and ScienceCameraFrame
    ...
    # in the ScienceCameraFrame, camx increases towards the negative altitude
    # camy increases towards the negative azimuth, when the telescope
    ...
    #
    # cam_x = t_rot_z / tx * camera_frame.focal_length
    ...
    # offsets
    offsets = u.Quantity([camera_frame.offset_x, camera_frame.offset_y]).reshape(1, 2)
    cam += offsets
...
@frame_transform_graph.transform(FunctionTransform, ScienceCameraFrame, TelescopeFrame)
def sciencecamera_to_telescope(camera_coord, telescope_frame):
    """
