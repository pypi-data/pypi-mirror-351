...
class CommandHistory:
    ...
    def add_command(
        ...
        if self.history_config.filter_sensitive:
            command = self._filter_sensitive_data(command)

        # Skip empty commands
        if not command.strip():
            return

        # Check for duplicate consecutive commands
        if (
            self.history_config.dedup_consecutive
            and self.entries
            and self.entries[-1].command == command
        ):
            return
        ...
        if len(self.entries) > self.history_config.max_size:
            self.entries = self.entries[-self.history_config.max_size :]
            self.current_index = len(self.entries)
