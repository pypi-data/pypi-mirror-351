...
Remember: You are an autonomous agent capable of taking action. Use your tools wisely to provide comprehensive assistance while maintaining safety and user control."""
    ...
    async def process_prompt(
        ...
        **kwargs
        ...
        try:
            ...
            # Send message to AI
            response = await self.send_message(prompt, **kwargs)
            ...
            return {
                ...
                "tool_calls": [],  # Will be populated if tools were called
    ...
    def get_available_tools(self) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # Get tools from registry synchronously
            # Convert the async method to sync for this interface
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # Create a new task for async execution
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(asyncio.run, self.tool_registry.get_available_tools())
                    return future.result()
            else:
                return asyncio.run(self.tool_registry.get_available_tools())
        except Exception as e:
