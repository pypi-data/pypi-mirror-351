...
from kritrima_ai.version import get_version
...
class KritrimaApp:
    ...
    def __init__(self, config: AppConfig):
        ...
        # Initialize core components first (needed by AgentLoop)
        self.session_manager = SessionManager(config)
        self.command_history = CommandHistory(config)
        
        # Initialize AI and agent components
        self.ai_client = AIClient(config)
        self.agent_loop = AgentLoop(config, self.session_manager, self.command_history)
        
        # Initialize UI and tool components
        self.terminal_interface = TerminalInterface(config, self.agent_loop, self.session_manager, self.command_history)
        self.rich_display = RichDisplay(self.console)
        self.file_ops = FileOperations(config)
        self.command_exec = CommandExecution(config)
        self.approval_manager = ApprovalSystem(config)
