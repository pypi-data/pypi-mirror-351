...
class FileOperations(FileOperationsTool):
    """Alias for FileOperationsTool to maintain compatibility."""
    
    def __init__(self, config: AppConfig):
        super().__init__(config)
    
    async def read_file(self, file_path: Path, encoding: str = "utf-8") -> str:
        """Simple file read method for compatibility."""
        return await super().read_file(str(file_path), encoding)
