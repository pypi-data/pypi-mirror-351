...
from .app.handle import (dedup_commands, concat_pcr_app_steps, 
                         concat_pcr_profile_app_steps, concat_pcr_state_app_steps, create_ordered_step_ids)
...
class Context:
    def __init__(self, robot_type: str, 
        ...
        self.__labware_location_update: Dict[str, Union[str, int]] = {"fixedTrash": "12"} if self.__deck_type == 'standard' else {}
        ...
        self.__json = {
            ...
            "robot": {
                ...
                "deckId": "alphatool_"+self.__deck_type,
    ...
    def load_gripper(self) -> Gripper:
        ...
        """
        gripper_id = str(uuid.uuid4())
        g = Gripper(gripper_id, 'gripper', '', self)
        return g
    ...
    def load_module(self, module_type: str, location: int) -> Union[TemperatureModule, ThermocyclerModule, MagneticModule, HeaterShakerModule, TransportModule]:
        ...
        if location < 1 or location > 12:
            raise ValueError(f"Invalid slot number. Slot number must be between 1 and 12.")
        
        if location in [6,7]:
            logging.warning(f"Placing module on slot 6,7 is not recommended due to the wire routing on the deck.")
    ...
    def export(self, save_dir: Optional[str] = None) -> str:
        ...
        app_commands = concat_pcr_app_steps(app_commands)
        app_commands = concat_pcr_profile_app_steps(app_commands)
        app_commands = concat_pcr_state_app_steps(app_commands)
        ...
        for mount in ['left', 'right']:
            if self.__drop_tip_before_next_pipette_command[mount]:
                ...
                is_skip_drop = (command.commandType == 'internal_mark' and 
                              getattr(command, 'use_new_tip', None) == 'never' and
                              getattr(command, 'pipette_id', None) == self.__arm_mount[mount].id())
                
                # check droptip in command to prevent dropping tip twice
                if not is_skip_drop and 'droptip' not in command.commandType.lower():
                    self.__arm_mount[mount].drop_tip()
    ...
    def _check_collision(self, location: Union[Well, Column]) -> None:
        ...
        if self.__pick_up_tip_order == 'top_to_bottom':
            warning_slots = [1, 5, 9]
            check_slot = behind_slot
            item = self._get_instrument_by_slot(behind_slot)

        else:  # bottom_to_top
            warning_slots = [4, 8, 12]
            check_slot = front_slot
            item = self._get_instrument_by_slot(front_slot)

        if slot in warning_slots:
            logging.warning(f"Using single channel mode with 8 channel pipette on slot {slot} may lead to collision due to the physical limitation.")
            return
    ...
    def submit(self, ip: str, port: int = 41950, skip_init: bool = False) -> None:
        """Submit the protocol to the robot for execution.

        This method connects to a robot at the specified IP address and port, then sends
        the protocol for execution. The protocol can optionally skip the initialization
        phase if the robot is already initialized.
