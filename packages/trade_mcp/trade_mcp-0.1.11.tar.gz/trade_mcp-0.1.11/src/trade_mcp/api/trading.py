...
- Executed: {order['executedQty']}"""
    ...
    async def cancel_multiple_orders(symbol: str, order_id_list: list) -> str:
        ...
        # Split orders into batches of at most 10
        batch_size = 10
        batches = [order_id_list[i:i+batch_size] for i in range(0, len(order_id_list), batch_size)]
        all_summaries = []
        all_errors = []
        
        for batch_idx, batch in enumerate(batches):
            params = {"symbol": symbol, "orderIdList": batch}
            try:
                result = await client.delete("/fapi/v1/batchOrders", params=params, security_type="TRADE")
                if isinstance(result, dict) and 'code' in result and result['code'] < 0:
                    all_summaries.append(f"Batch {batch_idx + 1}: API Error - {result.get('msg', 'Unknown error')}")
                    continue
                ...
                if isinstance(result, list):
                    ...
                    for order in result:
                        if isinstance(order, dict):
                            if "orderId" in order and order.get("status") == "CANCELED":
                                successful_cancellations.append(str(order["orderId"]))
                            elif "code" in order:
                                failed_cancellations.append(f"Order {order.get('orderId', 'Unknown')}: {order.get('msg', order.get('code'))}")
                            else:
                                # Handle other successful responses
                                successful_cancellations.append(str(order.get("orderId", "Unknown")))
                    
                    batch_summary = f"Batch {batch_idx + 1}: "
                    if successful_cancellations:
                        batch_summary += f"Successfully cancelled orders: {', '.join(successful_cancellations)}"
                    if failed_cancellations:
                        if successful_cancellations:
                            batch_summary += "; "
                        batch_summary += f"Failed to cancel: {'; '.join(failed_cancellations)}"
                    
                    all_summaries.append(batch_summary)
                else:
                    all_summaries.append(f"Batch {batch_idx + 1}: Cancelled orders {batch}")
                    
            except Exception as e:
                all_summaries.append(f"Batch {batch_idx + 1}: Request failed - {str(e)}")
        
        if len(batches) > 1:
            summary_header = f"Cancelled {len(order_id_list)} orders for {symbol} in {len(batches)} batches:\n"
        else:
            summary_header = f"Multiple orders cancelled for {symbol}:\n"
