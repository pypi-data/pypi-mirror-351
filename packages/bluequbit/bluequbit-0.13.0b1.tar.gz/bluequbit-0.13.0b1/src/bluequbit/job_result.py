...
class JobResult:
    ...
    def get_counts(self) -> dict[str, float]:
        ...
        if self.top_100k_results is None:
            self.top_100k_results = jobs.search_jobs(
                self._backend_connection, job_ids=[self.job_id], need_top_100k=True
            )["data"][0]["top_100k"]

        return self.top_100k_results
