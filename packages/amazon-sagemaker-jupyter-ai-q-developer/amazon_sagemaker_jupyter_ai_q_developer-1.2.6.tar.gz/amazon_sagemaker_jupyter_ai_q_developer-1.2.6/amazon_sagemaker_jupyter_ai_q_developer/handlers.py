...
class ConfigHandler(APIHandler):
    @tornado.web.authenticated
    async def post(self):
        self.validate_jupyter_ai_config()

    def validate_jupyter_ai_config(self):
        try:
            if os.path.exists(DEFAULT_CONFIG_PATH):
                ...
                if "embeddings_fields" not in current_config:
                    current_config["embeddings_fields"] = {}
                    config_updated = True

                if not current_config.get("embeddings_provider_id"):
                    current_config["embeddings_provider_id"] = "codesage:codesage-small"
                    config_updated = True
            ...
            logging.error(f"Error validating Jupyter AI config file at {DEFAULT_CONFIG_PATH}: {e}")
