...
__ALL__ = ['TrendWarrant']

class TrendWarrant(BaseIndicator):
    (BEARISH, NA, BULLISH) = (-1, 0, 1)

    lines = ('trend_w', 'delay')

    def __init__(self):
        super(TrendWarrant, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.trend_w[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'TYPE_TREND_W' if self.p.version is None else f'TYPE_TREND_W_{str(self.p.version).upper()}'