...
__ALL__ = ['TrendHoldAdv']

class TrendHoldAdv(BaseIndicator):
    (BUMPY_DN, BEARISH, NA, BULLISH, BUMPY_UP) = (-2, -1, 0, 1, 2)

    lines = ('trendhold_adv', 'delay')

    def __init__(self):
        super(TrendHoldAdv, self).__init__()
        self.addminperiod(1)
    ...
    def handle_api_resp(self, item):
        ...
        if leaf.get('value', None) is not None:
            ...
            elif leaf['value'] == 'BUMPY_UP':
                self.cache[result_time_str]['value'] = self.BUMPY_UP
            elif leaf['value'] == 'BUMPY_DN':
                self.cache[result_time_str]['value'] = self.BUMPY_DN
            elif leaf['value'] == 'NA':
    ...
    def determine_final_result(self):
        ...
        self.lines.trendhold_adv[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'TYPE_TREND_TO_HOLD_ADV' if self.p.version is None else f'TYPE_TREND_TO_HOLD_ADV_{str(self.p.version).upper()}'