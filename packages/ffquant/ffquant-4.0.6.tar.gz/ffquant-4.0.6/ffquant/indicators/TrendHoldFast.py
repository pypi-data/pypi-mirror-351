...
__ALL__ = ['TrendHoldFast']

class TrendHoldFast(BaseIndicator):
    (BEARISH, NA, BULLISH) = (-1, 0, 1)

    lines = ('th_fast', 'delay')

    def __init__(self):
        super(TrendHoldFast, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.th_fast[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'TYPE_TREND_TO_HOLD_FAST' if self.p.version is None else f'TYPE_TREND_TO_HOLD_FAST_{str(self.p.version).upper()}'