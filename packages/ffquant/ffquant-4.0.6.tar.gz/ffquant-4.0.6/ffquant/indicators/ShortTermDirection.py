...
class ShortTermDirection(BaseIndicator):
    ...
    def __init__(self):
        super(ShortTermDirection, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.std[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'SHORT_TERM_DIR' if self.p.version is None else f'SHORT_TERM_DIR_{str(self.p.version).upper()}'