...
class LongTermDirection(BaseIndicator):
    ...
    def __init__(self):
        super(LongTermDirection, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.ltd[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'LONG_TERM_DIR' if self.p.version is None else f'LONG_TERM_DIR_{str(self.p.version).upper()}'