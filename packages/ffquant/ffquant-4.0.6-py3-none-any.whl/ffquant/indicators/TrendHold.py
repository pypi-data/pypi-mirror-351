...
class TrendHold(BaseIndicator):
    ...
    lines = ('trend_hold', 'delay')

    def __init__(self):
        super(TrendHold, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.trend_hold[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'TYPE_TREND_TO_HOLD' if self.p.version is None else f'TYPE_TREND_TO_HOLD_{str(self.p.version).upper()}'