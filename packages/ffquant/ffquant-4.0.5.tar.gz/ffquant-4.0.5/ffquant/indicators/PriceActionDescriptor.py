...
__ALL__ = ['PriceActionDescriptor']

class PriceActionDescriptor(BaseIndicator):
    (BEARISH, NA, BULLISH) = (-1, 0, 1)

    lines = ('pad', 'delay')

    def __init__(self):
        super(PriceActionDescriptor, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.pad[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'PRICE_ACTION_MODEL_DESCRIPTOR' if self.p.version is None else f'PRICE_ACTION_MODEL_DESCRIPTOR_{str(self.p.version).upper()}'