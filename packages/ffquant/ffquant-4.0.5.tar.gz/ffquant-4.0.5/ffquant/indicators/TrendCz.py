...
class TrendCz(BaseIndicator):
    (BEARISH, NA, BULLISH) = (-1, 0, 1)

    lines = ('cz', 'delay')

    def __init__(self):
        super(TrendCz, self).__init__()
        self.addminperiod(1)
    ...
    def determine_final_result(self):
        ...
        self.lines.cz[0] = self.cache[current_bar_time_str]['value']
    ...
    def get_internal_key(self):
        return 'TYPE_TREND_CZ' if self.p.version is None else f'TYPE_TREND_CZ_{str(self.p.version).upper()}'