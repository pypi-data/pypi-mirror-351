...
__ALL__ = ['MyBrokerCombo']

class MyBrokerCombo(bt.BrokerBase):

    def __init__(self, debug=False, *args, **kwargs):
        super(MyBrokerCombo, self).__init__(*args, **kwargs)
    ...
    def getvalue(self, datas=None, sync=False):
        value = 0
        for broker in self.brokers:
            value += broker.getvalue(datas=datas, sync=sync)
        return value
    ...
    def getposition(self, data=None, sync=False):
        ...
        if self.active_broker is not None:
            position = self.active_broker.getposition(data=data, sync=sync)
        else:
    ...
    def get_pending_orders(self, sync=False):
        pending_orders = []
        for broker in self.brokers:
            pending_orders += broker.get_pending_orders(sync=sync)
    ...
    def get_order(self, order_id, sync=False):
        order = None
        if self.active_broker is not None:
            order = self.active_broker.get_order(order_id, sync=sync)
        else:
