...
class MGnH2ServingColdWarmDelay:
    ...
    def get_probs_of_servers_busy(self):
        ...
        # servers are not busy, when system in [0,0] state, in cooling, cooling delay and warming up states.

        probs[0] += self.Y[0][0, 0] + self.Y[0][0, 1] + \
            self.Y[0][0, 2] + self.Y[0][0, 3] + self.Y[0][0, 4]
