...
class SortingResults(object):
    ...
    def summary(self, rf_df, rf, alpha_models, layout="default",
                strategies="HML", raw_ret=False, output_path=None,
                **kwargs):
        '''
        ...
        self._make_table(rf_df=rf_df, rf=rf, alpha_models=alpha_models, layout=layout,
                         strategies=strategies, raw_ret=raw_ret,
                         **kwargs)
        self._html_render()
        if output_path:
...
class MainTable(object):
    ...
    def test_mean(self, date, qret_df, excess, sortbys, maxlags, model=None):
        ...
        test_res = groups.apply(lambda x: nw_ttest(x, excess, model_cols, maxlags=maxlags))
    ...
    def get_alphas(self, date, qret_df, sortbys, alpha_models,
                   nw_maxlags=None, **kwargs):
        ...
        for n, model in enumerate(alpha_models):
            if n == 0:
                ...
                test_res, model_cols = self.test_mean(date=date, qret_df=qret_df, excess=self.col1,
                                                      sortbys=sortbys, model=model, maxlags=nw_maxlags)
            else:
...
class StrategyTable(object):
    def __init__(self, qret_df, qret, date,
                 sortby, alpha_models, layout,
                 strategy="HML", pct_sign=True, **kwargs):
    ...
    def test_mean(self, date, strategy_df,
        ...
        test_res = nw_ttest(strategy_df, self.excess, model_cols, maxlags=maxlags)
    ...
    def get_alphas(self, date, strategy_df,
        ...
        for n, model in enumerate(alpha_models):
            test_res, model_cols = self.test_mean(date=date, strategy_df=strategy_df,
                                                  model=model, maxlags=nw_maxlags)
