    ...
    try:
        with open(path, 'r', encoding='utf-8') as f:
            ...
            if 'mcpServers' in config:
                ...
                sys.stdout.write(f"Config file '{path}' does not contain 'mcpServers'\n")
            ...
            if 'a2aServers' in config:
                ...
                sys.stdout.write(f"Config file '{path}' does not contain 'a2aServers'\n")
...
async def main():
    ...
    while True:
        if args.configure or not all([
            ...
            if args.alias not in settings.config:
                sys.stdout.write(f"Created new alias '{args.alias}' in the configuration file.\n\n")
                ...
                if (key := settings.get_api_key(args.alias)):
                    sys.stdout.write(f"API_KEY: {key}\n")
                if (url := settings.get_base_url(args.alias)):
                    sys.stdout.write(f"BASE_URL: {url}\n")
    ...
    if only_configure:
        sys.stdout.write("Proxy configuration completed successfully!\n")
        return 0

    if not args.config:
        sys.stderr.write("Error: MCP servers Configuration file path is required\n")
        return 1
    ...
    try:
        ...
        if result.get('errors'):
            ...
            for error in result.get('errors'):
                sys.stderr.write(f"{error.get('description')}\n")

            return 1
        else:
            sys.stdout.write(f"Organization: {result.get('organizationName')}\n")
            sys.stdout.write(f"Project: {result.get('projectName')}\n")
