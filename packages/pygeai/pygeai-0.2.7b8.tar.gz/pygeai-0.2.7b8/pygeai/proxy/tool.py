...
class ProxiedTool:
    ...
    def __init__(
        self, server_name: str, name: str, description: str, public_preffix: str, input_schema: dict[str, Any]
    ) -> None:
    ...
    def get_public_name(self) -> str:
        ...
        return f"{self.public_preffix}.{self.name if self.server_name in self.public_preffix else self.get_full_name()}"
    ...
    def _get_openai_compatible_name(self, name: str) -> str:
            compatible_name = ''.join(c if c.isalnum() or c in '-_' else '_' 
                                    for c in name.lower())
