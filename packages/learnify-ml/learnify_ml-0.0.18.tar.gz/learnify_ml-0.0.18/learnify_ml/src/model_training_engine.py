    ...
    def train(self, X_train, y_train) -> dict:
        try:
            ...
            if self.apply_hyperparameter_tuning:
                ...
                for name, model in tqdm(self.models.items(), desc="Training Models with Hyperparameter Tuning"):
                    logger.info(f"- Training model: {name}")
                    search_class = self.search_methods[self.hyperparameter_tuning_method]
                    search_class_params = self.search_methods_params[self.hyperparameter_tuning_method]
                    search = search_class(model,
                                          param_distributions=self.params[self.hyperparameter_tuning_method][name],
                                          **search_class_params,
                                        #   n_jobs=-1
                                          )
                ...
                scores = cross_val_score(self.model, X_train, y_train, cv=kfold, scoring="accuracy", n_jobs=-1)
