...
class AuctionManager:
    ...
    def select_ad(self, candidates: List[dict], bids: pd.DataFrame) -> List[dict]:
        ...
        try:
            ...
            # Check if bids is empty using proper dictionary check
            if not bids:  # Changed from bids.empty to proper dictionary check
                print("No bids available for campaigns")
                return []
            ...
            for campaign in candidates:
                ...
                # Get bid amount for this campaign
                bid_amount = bids.get(campaign_id, 0)
                
                if bid_amount > 0:
                    # Calculate weighted score (bid * similarity)
                    score = bid_amount * similarity
                    
                    # Keep track of highest scoring campaign
                    if score > max_score:
                        max_score = score
                        max_campaign = campaign
            
            # Add the highest scoring campaign to results
            if max_campaign:
                selected.append(max_campaign)
