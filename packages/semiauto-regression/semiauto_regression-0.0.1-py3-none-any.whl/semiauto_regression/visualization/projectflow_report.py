...
class ProjectFlowReport:
    ...
    def add_title_page(self):
        ...
        # Add title
        self.pdf.cell(0, 20, "SemiAuto Regression Report", 0, 1, 'C')
    ...
    def add_data_preprocessing_section(self):
        """Add the data preprocessing section to the report"""
    ...
    def add_model_evaluation_section(self):
        """Add the model evaluation section to the report"""
        ...
        if 'performance_metrics_path' in self.intel:
            ...
            if os.path.exists(metrics_path):
                ...
                metrics_to_display = [
                    ("R² Score", performance.get('r2_score', 'N/A')),
                    ("Explained Variance Score", performance.get('explained_variance_score', 'N/A')),
                    ("Mean Squared Error", performance.get('mean_squared_error', 'N/A')),
                    ("Root Mean Squared Error", performance.get('root_mean_squared_error', 'N/A')),
                    ("Mean Absolute Error", performance.get('mean_absolute_error', 'N/A')),
                    ("Mean Absolute Percentage Error", performance.get('mean_absolute_percentage_error', 'N/A')),
                    ("Max Error", performance.get('max_error', 'N/A'))
                ]
    ...
    def add_final_evaluation_section(self):
        """Add the final evaluation section to the report"""
        ...
        if os.path.exists(metrics_path):
            ...
            if 'performance_metrics_path' in self.intel:
                ...
                if os.path.exists(orig_metrics_path):
                    ...
                    key_metrics = [
                        ("R² Score", 'r2_score', True),  # Higher is better
                        ("RMSE", 'root_mean_squared_error', False),  # Lower is better
                        ("MAE", 'mean_absolute_error', False)  # Lower is better
                    ]
    ...
    def add_conclusion(self):
        """Add a conclusion section to the report"""
        self.add_page()
        ...
        target_col = self.intel.get('target_column', 'unknown')
        model_name = self.intel.get('model_name', 'unknown')

        # Generate conclusion text
        self.pdf.set_font('Helvetica', '', 11)

        conclusion_text = f"This report summarizes the development of a regression model to predict {target_col} "
        conclusion_text += f"using the {self.dataset_name} dataset. "

        # Check if optimization was performed
        if 'optimized_model_path' in self.intel:
            conclusion_text += f"A {model_name} regression model was trained and optimized using hyperparameter tuning. "
            ...
            if ('performance_metrics_path' in self.intel and
                ...
                try:
                    ...
                    orig_r2 = orig_perf.get('r2_score', 0)
                    opt_r2 = opt_perf.get('r2_score', 0)

                    if isinstance(orig_r2, (int, float)) and isinstance(opt_r2, (int, float)):
                        improvement = opt_r2 - orig_r2

                        if improvement > 0:
                            conclusion_text += f"The optimization process improved the model's R² score "
                            conclusion_text += f"from {orig_r2:.5f} to {opt_r2:.5f}, "
                            conclusion_text += f"representing a {(improvement / max(abs(orig_r2), 1e-10)) * 100:.2f}% improvement. "
                except Exception:
                    pass
        else:
            conclusion_text += f"A {model_name} regression model was trained without hyperparameter optimization. "

        conclusion_text += "\n\nThis automatic report was generated to provide insights into the model development process "
        conclusion_text += "and performance metrics. It includes details about data preprocessing, feature engineering, "
