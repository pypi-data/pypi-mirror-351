...
class BaseMigrationExecutorManager:
    ...
    def _is_migration_applied(self, migration: MigrationFile) -> bool:
        for applied_migration in self._applied_migrations:
            if applied_migration.number == migration.number and applied_migration.type == self._module_type:
                return True
            ...
            self._contrib_executor_managers.append(
                SimpleFileMigrationExecutorManager(
                    ...
                    applied_migrations=[
                        ...
                        if _migration.type == ModuleType.CONTRIB
                    ],
                    store=self.store,
                ),
            )
...
class AsyncSimpleFileMigrationExecutorManager(BaseMigrationExecutorManager):
    ...
    async def execute(
        ...
        *,
        ...
        return await self._apply(migration_number, fake=fake, skip_data_migrations=skip_data_migrations)  # type: ignore[arg-type,misc,call-arg]
