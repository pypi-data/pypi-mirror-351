...
class FileMigrationStore(BaseMigrationStore):
    ...
    def fetch_migrations(self) -> list[MigrationFile]:
        ...
        for _migration_data in result.data or []:
            ...
            if _migration['module_name']:
                _path_module = contrib_to_module_root_path(_migration['module_name'])
            else:
    ...
    def delete_migration(self, migration: MigrationFile) -> None:
        ...
        Returns:
            None
        """
        if not migration.stored_address:
            return

        _migration_data = self._build_migration_data(migration)
        _historical_address = migration.stored_address.model_copy(update={'object_version': get_identifier()})
        _, _metadata = self._build_migration_metadata(migration, object_id=migration.stored_address.object_id)  # type: ignore[arg-type]

        _metadata['prior_version'] = migration.stored_address.object_version
        _metadata['is_deleted'] = True

        self._save_historical_data(
            address=_historical_address,
            data=_migration_data,
            metadata=_metadata,
        )
