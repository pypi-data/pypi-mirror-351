...
class NIFIRESTClient:
    ...
    def __init__(self, conf: HidDataTransferConfiguration,
    ...
    def empty_queue(self, connection_id, token):
        """empties a connection queue"""
        # Create a drop request
        drop_request_id = self.initiates_drop_request(connection_id, token)
        if drop_request_id:
            # Check status of drop request
            state = self.check_drop_request_status(
                connection_id, drop_request_id, token
            )
            attempts = 5
    ...
    def delete_flowfile_queues_listing_requests(self, connection_id,
        ...
        if response.ok:
            ...
            raise HidDataTransferException(
                f"Delete listing request exception: {response.content.decode()}"
            )

    # MULTI-REQUESTS methods

    def save_parameter_context(
        self, parameter_context, parameter_context_id, token
    ) -> bool:
        """saves a parameter context identified by id"""
        # Initiates an update request
        request_id = self.initiate_update_request(
            "parameter-contexts",
            parameter_context,
            parameter_context_id,
            token,
        ...
        if request_id:
            ...
            state = self.check_update_request_status(
                parameter_context_id, request_id, token
            )
            attempts = 5
            while not state and attempts > 0:
                # wait and check again
            ...
            self.delete_update_request(
                "parameter-contexts", parameter_context_id, request_id, token
            )
            return state
        else:
            raise HidDataTransferException(
