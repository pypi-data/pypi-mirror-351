...
from orca_studio.molecule import Molecule, Atom, Bond, BondType
from orca_render.meshes import Mesh
...
class MolecularRenderer:
    ...
    def render_molecule(
        ...
        for atom in self.molecule.geometry.atoms:
            mesh = self._atom_to_mesh(atom, scale=atom_scale, custom_colors=custom_colors)
            meshes.append(mesh)
        ...
        if show_bonds:
            ...
            for bond in bonds:
                mesh = self._bond_to_mesh(bond, radius=bond_radius, custom_colors=custom_colors)
                meshes.append(mesh)
