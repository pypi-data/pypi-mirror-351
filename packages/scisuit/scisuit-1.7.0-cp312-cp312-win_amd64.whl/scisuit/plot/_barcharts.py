...
def bar(
	...
	canvas(x=[-width, len(x)], y=None, 
		...
		scale=kwargs.get("scale") if kwargs.get("scale")!=None else True)
...
def barh(
	...
	for i in range(len(width)):
		...
		if i==0 and isinstance(label, str):
			ownerid = rect(xy=xy, width=float(abs(width[i])), height=float(height), label=label, **kwargs)
		else:
