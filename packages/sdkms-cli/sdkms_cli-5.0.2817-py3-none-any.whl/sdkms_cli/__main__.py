...
def _validate_cipher_param(alg, in_f=None, iv=None, mode=None, tag_len=None, tag=None, is_encrypt=True, **kwargs):
    ...
    if alg.upper() in SYMMETRIC_ALG:
        ...
        if mode is None:
            ...
            #for encryption, always allow IV to be None, and it must be None if it's a crypto mode that doesn't use an IV.
            if is_encrypt and iv is not None and mode.upper() in ('ECB', 'KW', 'KWP', 'FF1') :
                errors.append("--iv must not be provided for ECB, FF1, KW or KWP modes")

            #for decryption, an IV must be provided for crypto modes that use an IV, and it must not be provided for crypto modes that do not use IV.
            if not is_encrypt and iv is None and mode.upper() not in ('ECB', 'KW', 'KWP', 'FF1') :
                errors.append("--iv is required argument for mode({}) in alg({})".format(mode, alg))

            if not is_encrypt and iv is not None and mode.upper() in ('ECB', 'KW', 'KWP', 'FF1') :
                errors.append("--iv must not be provided for ECB, FF1, KW or KWP modes")
...
def validate_url(url):
    ...
    scheme = u.scheme
    if scheme == "":
        return u.geturl().replace(scheme, "https://", 1)
    else:
        return u.geturl().replace(scheme, "https", 1)
...
def json_request(url, header=None, body=None, method='GET', exp_status=200, required_auth=RequiredAuth.NONE):
    ...
    try:
        res = requests.request(method=method, url=validate_url(api_endpoint) + url, headers=default_header, data=body,
...
def ldap_auth(**kwargs):
    ...
    if not kwargs['user_email']:
        kwargs['user_email']= input("Please enter your useremail: ")
    ...
    if kwargs['password']:
        ...
        kwargs['password'] = getpass("SDKMS Password: ")
    res=json_request('/sys/v1/session/auth/discover',header=None,body=json.dumps(kwargs),method='POST',exp_status=200, required_auth=RequiredAuth.NONE)

    if not kwargs['email']:
        kwargs['email'] = kwargs['user_email']

    for i in range(0, res.__len__()):
        if res[i]['method']=='ldap-password':
            if not kwargs['method']:
                kwargs['method']='ldap-basic-auth'
            kwargs['idp_id']=res[i]['idp_id']

    response=json_request('/sys/v1/session/auth', header=None,body=json.dumps(kwargs),method='POST',exp_status=200, required_auth=RequiredAuth.NONE)
    _globals.user_token = response['access_token']
    write_access_tokens()
...
def audit_log(**kwargs):
    ...
    if not kwargs['sysadmin']:
        res=make_request(RequiredAuth.USER,UsersApi,UsersApi.get_users)
        for user in res:
            users[user.user_id] = user.user_email
        res=make_request(RequiredAuth.USER,GroupsApi,GroupsApi.get_groups)
        for group in res:
            groups[group.group_id] = group.name
        res=make_request(RequiredAuth.USER,AppsApi,AppsApi.get_apps)
        for app in res:
            apps[app.app_id] = app.name

        res=make_request(RequiredAuth.USER,PluginsApi,PluginsApi.get_plugins)
        for plugin in res:
            plugins[plugin.plugin_id] = plugin.name

        filename = "log_dump_" + str(current_time) + ".csv"
    else:
        url = '/sys/v1/users?all_customers=true&with_has_account=true'
        res = json_request(url, exp_status=200, header=None, body=None, method='GET', required_auth=RequiredAuth.USER)
        for user in res:
            users[user['user_id']] = user['user_email']
        filename = "sys_admin_log_dump_" + str(current_time) + ".csv"
    ...
    header = [ 'Severity', 'Type', 'Groups', 'IP-Address', 'Message', 'Actor', 'Time', 'Response', 'Id' ]
    writer.writerow(header)

    print('Fetching logs and saving in file: ' + filename)

    if kwargs['start_time'] is not None:
        start_time = int(time.mktime(time.strptime(kwargs['start_time'], "%Y-%m-%d")))
        if (start_time > current_time):
            raise ClientException("--start-time can't be greater than current-time")
    else:
        start_time = None
    if kwargs['end_time'] is not None:
        end_time = int(time.mktime(time.strptime(kwargs['end_time'], "%Y-%m-%d")))
    else:
        end_time = None
    ...
    while True:
        if kwargs['limit'] and limit == 0:
            break

        if start is not None:
           print('\rProgress:', start, end='')
           sys.stdout.flush()

        if kwargs['limit']:
            this_query_limit = str(min(1000,limit))
        else:
            this_query_limit = str(1000)

        url = '/sys/v1/logs?size=' + this_query_limit
        if start_time is not None:
            url = url + '&range_from=' + str(start_time)
        if end_time is not None:
            url = url + '&range_to=' + str(end_time)

        if start is not None:
            url = url + '&previous_id=' + start

        response = json_request(url, exp_status=200, header=None, body=None, method='GET', required_auth=RequiredAuth.USER)

        if 'hits' not in response or response['hits'] is None or len(response['hits']) == 0:
            break

        for log in response['hits']:
            start = log['_id']
            log_data = []
            log_data.append(log['_source']['severity'])
            log_data.append(log['_source']['action_type'])

            group_list = []
            if not kwargs['sysadmin']:
                for group_id in log['_source']['group_ids']:
                    if group_id in groups:
                        group_list.append(groups[group_id])

            group_list = ';'.join(group_list)
            log_data.append(group_list)

            ip_data = 'null' if log['_source']['client_ip'] is None else log['_source']['client_ip']
            log_data.append(ip_data)
            log_data.append(log['_source']['message'])

            actor = 'null'
            if not kwargs['sysadmin']:

                if log['_source']['actor_type'] == 'App':
                    if log['_source']['actor_id'] in apps:
                        actor = 'App: ' + apps[log['_source']['actor_id']]
                elif log['_source']['actor_type'] == 'User':
                    if log['_source']['actor_id'] in users:
                        actor = 'User: ' + users[log['_source']['actor_id']]
                elif log['_source']['actor_type'] == 'Plugin':
                    if log['_source']['actor_id'] in plugins:
                        actor = 'Plugin: ' + plugins[log['_source']['actor_id']]
            else:
                try:
                    actor = 'User: ' + users[log['_source']['actor_id']]
                except KeyError:
                    actor = 'User: ' + log['_source']['actor_id']
            log_data.append(actor)
            log_data.append(log['_source']['time'])

            if log['_source']['response_time'] == None:
                response_time = "None"
            else:
               rtimeMS = log['_source']['response_time']["secs"] * 1000 + \
	                 log['_source']['response_time']["nanos"] / 1000000
               response_time = str(rtimeMS) + "ms"

            log_data.append(response_time)
            log_data.append(log['_id'])

            writer.writerow(log_data)

        if kwargs['limit']:
            limit = limit - len(response['hits'])

    print("\nFetching logs completed. The log file is " + filename)
    f.close()
...
def create_key(**kwargs):
    ...
    del kwargs["ec_name"]
    # if inferring required metadata, get the group info and create a dict of the required attributes
    attribs = {}
    if kwargs['infer_required_metadata']:
        group_id = kwargs['group_id']
        if group_id:
            response = json_request(f"/sys/v1/groups/{group_id}", exp_status=200, header=None, body=None, method='GET', required_auth=RequiredAuth.EITHER)
            metadata = response["key_metadata_policy"]["base"]["custom_metadata"]
            for attrib in metadata:
                if metadata[attrib]["required"]:
                    attribs[attrib] = ""
    ...
    if metadata:
        for key in attribs:
            if metadata[key]:
                continue
            metadata[key] = attribs[key]
    else:
        metadata = attribs
    kwargs["custom_metadata"] = metadata
    # Delete the argument that isn't needed for request creation
    del(kwargs['infer_required_metadata'])
...
def rotate_secret(secret_file, **kwargs):
...
def key_attestation(**kwargs):
    if kwargs['kid'] is not None:
        key = sdkms.v1.SobjectDescriptor(kid=kwargs['kid'])
    elif kwargs['name'] is not None:
        key = sdkms.v1.SobjectDescriptor(name=kwargs['name'])
    else:
        raise ClientException("Please specify --kid or --name.")

    body = sdkms.v1.KeyAttestationRequest(key=key)
    response = make_request(RequiredAuth.EITHER, SecurityObjectsApi, SecurityObjectsApi.get_key_attestation, body)
    print(json.dumps(response, default=serialize))
...
def encrypt(in_f, out, kid, **kwargs):
    ...
    if os.path.getsize(in_f) < CHUNK_SIZE:
        ...
        # Store IV value, if any. This may have been provided by the user, in which case this is redundant,
        # but if the IV was generated on the server, it will be needed to decrypt.
...
def update_plugin(uuid, in_f, group_id, **kwargs):
    code = _read_binary(in_f).decode("utf-8")
...
def list_users():
    users = make_request(RequiredAuth.USER,UsersApi,UsersApi.get_users)
    for user in users:
        sys.stdout.write("{} ".format(user.user_id))
        sys.stdout.write("{} ".format(user.user_email))
        # Add new line
        print("")
...
def create_group(**kwargs):
    if kwargs['add_quorum_policy']:
        del kwargs['add_quorum_policy']
        approval_policy = {}
        quorum = {}
        if not kwargs['n']:
            raise ClientException("n is required to add quorum policy")
        else:
            quorum['n'] = kwargs['n']
        quorum['require_2fa']=kwargs['require_2fa']
        quorum['require_password']=kwargs['require_password']
        members = []
        if kwargs['users']:
            users_arg = kwargs['users'].split(',')
        else:
            raise ClientException("user-id is required to add quorum policy")
        del kwargs['users']
        all_users = make_request(RequiredAuth.USER, UsersApi, UsersApi.get_users)

        for requested_user in users_arg:
            found_user = False
            for user in all_users:
                if user.user_id == requested_user:
                    users = {"user": user.user_id}
                    members.append(users)
                    found_user = True
                    break
            if not found_user:
                raise ClientException('Unable to find user with id {}'.format(requested_user))

        quorum['members'] = members
        approval_policy['quorum'] = quorum
        kwargs['approval_policy'] = approval_policy

    response = make_request(RequiredAuth.USER, GroupsApi, GroupsApi.create_group, kwargs)
    print(response.group_id)

def update_group(**kwargs):
    if not kwargs['name'] and not kwargs['group_id']:
        raise ClientException("Either --name or --group-id is a required argument for update-group")
    group_id=None
    if kwargs['name'] is not None:
        groups = get_groups()
        for group in groups:
    ...
    if kwargs['group_id'] is not None:
        if kwargs['name'] is not None:
            if group_id != kwargs['group_id']:
                raise ClientException(
                    "Group with name '{}' has group id {}.".format(kwargs['name'], kwargs['group_id']) +
                    "When both --name and --group-id are provided, the updated group must match " +
    ...
    response = make_request(RequiredAuth.USER, GroupsApi, GroupsApi.update_group, group_id, kwargs)
    print(response.group_id)
    ...
    if status_response['status'] == "APPROVED":
        ...
        if 'body' in response and response['body'] is not None and 'out' in kwargs and kwargs['out'] is not None:
            ...
            if 'iv' in body and body['iv'] is not None:
                ...
                _write_binary(hex_encode(b64_decode(body['iv'])), "{}.iv".format(kwargs['out']))
            if 'tag' in body and body['tag'] is not None:
                print("tag: {}".format(hex_encode(b64_decode(body['tag']))))
                _write_binary(hex_encode(b64_decode(body['tag'])), "{}.tag".format(kwargs['out']))
    else:
...
# Figure out which API endpoint to use based on the command-line parameters.
def calculate_api_endpoint(args):
    # Unfortunately, we may have the --api-endpoint parameter at two levels. One is at the top level. The other is
    # that the app-login and user-login subcommands also have an --api-endpoint. The different options all do the
    # same thing, but the old implementation was such that running
    # sdkms-cli --api-endpoint https://eu.smartkey.io app-login would not work and you had to run
    # sdkms-cli app-login --api-endpoint https://eu.smartkey.io instead.
    #
    # Ideally, we would like to just remove the --api-endpoint argument from the app-login and user-login
    # subcommands, but that would break any existing scripts that use the sdkms-cli app-login --api-endpoint
    # form. So instead, what we need to do is use the app-login --api-endpoint if provided, and then use the
    # top-level --api-login, and the back off to environment variables or the hard-coded default. Fortunately
    # this conforms to the standard practice that if a parameter is provided more than once, the right-most
    # instance is used, so this behavior should hopefully not be confusing for anybody.
    #
    # PROD-8188.
    #
    # The parameter default for the top-level api-endpoint takes care of picking up the value from environment variables
    # or the static default.
    endpoint = args.api_endpoint
    if 'login_api_endpoint' in args and args.login_api_endpoint is not None:
        endpoint = args.login_api_endpoint

    return endpoint

# Checks/fixes for API endpoint:
# - trim any leading/trailing whitespace
# - http:// URLs are not allowed (only https:// is valid)
# - If the URL doesn't start with https://, add https://
def fix_up_api_endpoint(api_endpoint):
    api_endpoint = api_endpoint.strip()
    if api_endpoint.startswith('http:'):
        print('Requested API endpoint {} uses plain http, which is forbidden. Did you mean {}?'.format(
...
'''.format(VERSION)
    ...
    parser_login_app.add_argument("--api-endpoint", action="store", dest="login_api_endpoint",
                                  help="SDKMS API endpoint to connect to")
    ...
    parser_login_user.add_argument("--api-endpoint", action="store", dest="login_api_endpoint",
                                   help="SDKMS API endpoint to connect to")
    ...
    parser_ldap_auth = subparsers.add_parser("ldap-auth", help="Log in to SDKMS with a useremail and password ")
    _add_version(parser_ldap_auth)
    parser_ldap_auth.add_argument("--user-email", help="User email to login")
    parser_ldap_auth.add_argument("--email",help="User email to login")
    parser_ldap_auth.add_argument("--password", help= argparse.SUPPRESS)
    parser_ldap_auth.add_argument("--account-name",
                                   help="Account name or account UUID to use for user actions (only necessary if the user has more than one account)")
    parser_ldap_auth.add_argument("--method",help="Method of authentication:ldap-basic-auth")
    parser_ldap_auth.add_argument("--idp-id",required=False)
    parser_ldap_auth.set_defaults(func=ldap_auth)
    ...
    parser_audit_log = subparsers.add_parser("audit-log", help="download audit log",
                                             formatter_class=argparse.RawDescriptionHelpFormatter,
                                             epilog=textwrap.dedent('''\
                                                       Output:
                                                   ...
                                                   '''))
    _add_version(parser_audit_log)
    parser_audit_log.add_argument("--start-time", help="start time in yyyy-mm-dd format")
    parser_audit_log.add_argument("--end-time", help="end time in yyyy-mm-dd format")
    parser_audit_log.add_argument("--limit", help="Number of audit logs to return.", type=int)
    parser_audit_log.add_argument("--sysadmin", action='store_true', help="Use this flag to download Audit Logs from System Administrator")
    parser_audit_log.add_argument("--out", help="Where to write the output file (optional)")
    parser_audit_log.set_defaults(func=audit_log)
    ...
    parser_create.add_argument("--infer-required-metadata", help="Adds defaults for required, but not specified, metadata attributes", required=False, action="store_true")
...
""")
    ...
    parser_rotate_secret = subparsers.add_parser("rotate-secret", help="rotate a secret key",
                                          formatter_class=argparse.RawDescriptionHelpFormatter,
                                          epilog=textwrap.dedent('''\
                                                Output:
                                                    Key UUID on console.
                                                    ...
                                                    Exit Status:
                                                        Zero on success and 1 on error. Error message is printed on stderr.
                                                '''))
    _add_version(parser_rotate_secret)
    parser_rotate_secret.add_argument("--name", help="Name of secret key", required=True)
    parser_rotate_secret.add_argument("--in", help="Input secret file path", required=True, dest="secret_file")
    parser_rotate_secret.set_defaults(func=rotate_secret)
...
""")
    ...
    parser_key_attestation = subparsers.add_parser("key-attestation", help="Retrieve key attestation statement for a security object",
                                               formatter_class=argparse.RawDescriptionHelpFormatter,
                                               epilog=textwrap.dedent('''\
                                                       Output:
                                                         JSON output from the key attestation API
                                                       Exit Status:
                                                         Zero on success and 1 on error. Error message is printed on stderr
                                                        ''')
                                               )
    _add_version(parser_key_attestation)
    parser_key_attestation.add_argument("--kid", help="UUID of the target security object")
    parser_key_attestation.add_argument("--name", help="Name of the target security object")
    parser_key_attestation.set_defaults(func=key_attestation)
...
""")
    ...
    parser_list_users = subparsers.add_parser('list-users', help="List users in SDKMS")
    _add_version(parser_list_users)
    ...
    parser_create_group.add_argument("--add-quorum-policy", action="store_true", default=False)
    parser_create_group.add_argument("--n", type=int, help="Number of users")
    parser_create_group.add_argument("--users", help="Comma-separated list of users id from whom this group require approval to perform sensitive operations")
    parser_create_group.add_argument("--require-2fa", action="store_true", default=False)
    parser_create_group.add_argument("--require-password", action="store_true", default=False)
    parser_create_group.set_defaults(func=create_group)

    parser_update_group = subparsers.add_parser('update-group', help="Update a security group")
    _add_version(parser_update_group)
    parser_update_group.add_argument("--group-id", help="Group id of the group to update")
    parser_update_group.add_argument("--name", help="Name of group to update")
    parser_update_group.add_argument("--description", help="Description of the group")
    parser_update_group.add_argument("--add-quorum-policy", action="store_true", default=False)
    parser_update_group.add_argument("--n", type=int, help="Number of users")
    parser_update_group.add_argument("--users", help="Comma-separated list of users id from whom this group require approval to perform sensitive operations")
    parser_update_group.add_argument("--require-2fa", action="store_true", default=False)
    parser_update_group.add_argument("--require-password", action="store_true", default=False)
    parser_update_group.set_defaults(func=update_group)
    ...
    api_endpoint = calculate_api_endpoint(args)
    proxy = args.proxy
    ...
    api_endpoint = fix_up_api_endpoint(api_endpoint)
