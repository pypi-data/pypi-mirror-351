...
class CachedClient():
    ...
    def __init__(self,
        ...
        if (self._api_key is not None) and (len(self._stem) == 0):        
            if not self._api_key.startswith('abf'):
                ...
                self._api_key = 'sk' + self._api_key[3:]

            print('WARNING : You will be making requests via the "AI in Business and Finance" server. \n'
                  '          These requests may be logged. Do *NOT* make any requests with confidential \n'
                  '          or sensitive data.')
    ...
    def check_remaining_credits(self) -> dict:
        ...
        '''
        import requests
        out = requests.get('https://llm-proxy.guetta.com/key/info', headers={'x-litellm-api-key' : f'Bearer {self._api_key}'}).json()
        out = out['info']
    ...
    def __call__(self, **kwargs):
        ...
        if self._is_async:
            rel_func = openai.AsyncOpenAI(api_key=self._api_key, base_url='https://llm-proxy.guetta.com')
        else:
            rel_func = openai.OpenAI(api_key=self._api_key, base_url='https://llm-proxy.guetta.com')
