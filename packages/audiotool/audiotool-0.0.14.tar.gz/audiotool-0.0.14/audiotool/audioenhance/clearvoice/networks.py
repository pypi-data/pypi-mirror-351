...
class SpeechModel:
    ...
    def download_model(self, model_name):
        ...
        try:
            snapshot_download(
                repo_id=f"alibabasglab/{model_name}", local_dir=checkpoint_dir
            )
            return True
        except:
            return False
    ...
    def load_model(self):
        ...
        if not os.path.isfile(best_name):
            if not self.download_model(self.name):
                ...
                print(
                    f"Warning: Downloading model {self.name} is not successful. Please try again or manually download from https://huggingface.co/alibabasglab/{self.name}/tree/main !"
                )
                return
        ...
        for key in state.keys():
            if (
                key in pretrained_model
                and state[key].shape == pretrained_model[key].shape
            ):
                state[key] = pretrained_model[key]
            elif (
                key.replace("module.", "") in pretrained_model
                and state[key].shape
                == pretrained_model[key.replace("module.", "")].shape
            ):
                state[key] = pretrained_model[key.replace("module.", "")]
            elif (
                "module." + key in pretrained_model
                and state[key].shape == pretrained_model["module." + key].shape
            ):
    ...
    def decode(self):
        ...
        output_audio = decode_one_audio(
            self.model, self.device, self.data["audio"], self.args
        )
    ...
    def process(self, input_path, online_write=False, output_path=None):
        ...
        data_reader = DataReader(
            self.args
        )  # Initialize a data reader to load the audio files
            ...
            if use_key:
                ...
                if isinstance(self.result[key], list):  # For multi-speaker outputs
                    for spk in range(self.args.num_spks):
                        sf.write(
                            os.path.join(
                                output_path, key.replace(".wav", f"_s{spk+1}.wav")
                            ),
                            self.result[key][spk],
                            self.args.sampling_rate,
                        )
                ...
                if isinstance(self.result[key], list):  # For multi-speaker outputs
                    for spk in range(self.args.num_spks):
                        sf.write(
                            output_path.replace(".wav", f"_s{spk+1}.wav"),
                            self.result[key][spk],
                            self.args.sampling_rate,
                        )
                else:
