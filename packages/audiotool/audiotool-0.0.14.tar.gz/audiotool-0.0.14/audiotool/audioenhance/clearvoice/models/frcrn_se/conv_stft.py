...
def init_kernels(win_len, win_inc, fft_len, win_type=None, invers=False):
    ...
    N = fft_len
    fourier_basis = np.fft.rfft(np.eye(N))[
        :win_len
    ]  # Compute Fourier basis for the identity matrix
    real_kernel = np.real(fourier_basis)  # Extract real part
    imag_kernel = np.imag(fourier_basis)  # Extract imaginary part
    kernel = np.concatenate(
        [real_kernel, imag_kernel], 1
    ).T  # Combine real and imaginary parts
...
class ConvSTFT(nn.Module):
    ...
    def __init__(
        ...
        if fft_len is None:
            self.fft_len = np.int(
                2 ** np.ceil(np.log2(win_len))
            )  # Calculate fft_len based on win_len
        else:
            self.fft_len = fft_len

        kernel, _ = init_kernels(
            win_len, win_inc, self.fft_len, win_type
        )  # Initialize STFT kernel
        self.weight = nn.Parameter(
            kernel, requires_grad=(not fix)
        )  # Create a learnable parameter for the kernel
    ...
    def forward(self, inputs):
        ...
        )  # Perform convolution to compute STFT
        ...
        if self.feature_type == "complex":
            ...
            dim = (
                self.dim // 2 + 1
            )  # Calculate the size for the real and imaginary components
...
class ConviSTFT(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "enframe", torch.eye(win_len)[:, None, :]
        )  # Framing matrix for overlap-add method
    ...
    def forward(self, inputs, phase=None):
        ...
        outputs = F.conv_transpose1d(
            inputs, self.weight, stride=self.stride
        )  # Perform transposed convolution for iSTFT

        # Compute the overlap-add normalization
        t = self.window.repeat(1, 1, inputs.size(-1)) ** 2
        coff = F.conv_transpose1d(
            t, self.enframe, stride=self.stride
        )  # Apply the framing matrix for overlap-add
        outputs = outputs / (
            coff + 1e-8
        )  # Normalize the output to prevent division by zero
        return outputs
...
def test_fft():
    ...
    fft = ConvSTFT(
        win_len, win_inc, fft_len, win_type="hanning", feature_type="real"
    )  # Initialize ConvSTFT
    ...
    print(
        np.mean((outputs1 - np.abs(librosa_stft)) ** 2)
    )  # Print mean squared error between the two STFT outputs
...
def test_ifft1():
    ...
    sf.write(
        "conv_stft.wav", outputs2.numpy()[0, 0, :], 16000
    )  # Save reconstructed waveform to file
    print(
        "wav MSE", torch.mean(torch.abs(inputs[..., : outputs2.size(2)] - outputs2))
...
def test_ifft2():
    ...
    print(
        "random MSE", torch.mean(torch.abs(input - output) ** 2)
    )  # Print mean squared error
    import soundfile as sf

    sf.write(
        "zero.wav", output[0, 0].numpy(), 16000
    )  # Save reconstructed waveform to file
