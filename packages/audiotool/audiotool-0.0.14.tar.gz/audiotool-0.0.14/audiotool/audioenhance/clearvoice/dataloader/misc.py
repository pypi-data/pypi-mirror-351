...
def read_and_config_file(args, input_path, decode=0):
    ...
    processed_list = (
        []
    )  # Initialize list to hold processed file paths or configurations
    ...
    if decode:
        if args.task == "target_speaker_extraction":
            if args.network_reference.cue == "lip":
                ...
                if os.path.isdir(input_path):
                    ...
                    if (
                        input_path.lower().endswith(".mp4")
                        or input_path.lower().endswith(".avi")
                        or input_path.lower().endswith(".mov")
                        or input_path.lower().endswith(".webm")
                    ):
                        ...
                        with open(input_path) as fid:
                            for line in fid:
                                path_s = (
                                    line.strip().split()
                                )  # Split paths (space-separated)
                                processed_list.append(
                                    path_s[0]
                                )  # Add the first path (input audio path)
        ...
        if os.path.isdir(input_path):
            ...
            # If it's a single file and it's a .wav or .flac, add to processed list
            if input_path.lower().endswith(".wav") or input_path.lower().endswith(
                ".flac"
            ):
                processed_list.append(input_path)
            else:
