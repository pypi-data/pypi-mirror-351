...
def xs_metal(
    ...
    **kwargs,
    ...
    for ww, (w, offset) in enumerate(zip(section_width, offsets)):
        # Only give ports to the central track, or the first track if there are only 1 or 2 tracks
        port_names = (
            ("e1", "e2")
            if (offset == 0 or (len(section_width) == 2 and ww == 1))
            else (None, None)
        )
        sections.extend(
            [
    ...
    # If single track, add the isolation layer at the end
    if len(section_width) == 1:
        sections.append(gf.Section(layer=LAYER.ISO, width=total_width + 4))
...
@xsection
def DC(
    width: float = TECH.width_m1, layer="M1", radius=min_radius_metal, **kwargs
) -> gf.CrossSection:
    ...
    """
    return gf.cross_section.metal1(width=width, layer=layer, radius=radius, **kwargs)


@xsection
def GS(
    section_width: tuple[float, ...] = (150,),
    gap: float = 24,
...
@xsection
def GS_DOUBLE(
    section_width: tuple[float, ...] = (150, 150),
    gap: float = 24,
    radius=min_radius_metal,
    **kwargs,
) -> gf.CrossSection:
...
@xsection
def GSG_ground(
    section_width=100.0, radius=min_radius_metal, **kwargs
) -> gf.CrossSection:
    return xs_metal(section_width=section_width, radius=radius, **kwargs)


@xsection
def GSG_signal(
    section_width=60.0, radius=min_radius_metal, **kwargs
) -> gf.CrossSection:
    return xs_metal(section_width=section_width, radius=radius, **kwargs)
...
@xsection
def GS_ground(
    section_width=150.0, radius=min_radius_metal, **kwargs
) -> gf.CrossSection:
    return xs_metal(section_width=section_width, radius=radius, **kwargs)
...
@xsection
def RF(section_width=150.0, **kwargs) -> gf.CrossSection:
    return xs_metal(section_width=section_width, radius=section_width, **kwargs)
...
route_single_dc = partial(gf.routing.route_single, cross_section="DC")
...
route_single_rf = partial(gf.routing.route_single, cross_section="RF")
...
route_single_sbend_rf = partial(gf.routing.route_single_sbend, cross_section="RF")
...
route_bundle_dc = partial(gf.routing.route_bundle, cross_section="DC", auto_taper=False)
...
route_bundle_gsg = partial(
    gf.routing.route_bundle, cross_section="GS", auto_taper=False, router="optical"
)
...
def _bend_factory(
    bend: gf.typings.ComponentSpec, xs: gf.typings.CrossSectionSpec
) -> Callable[..., gf.kf.KCell]:
    xs = gf.get_cross_section(xs)

    def bend_circular(radius: int, cross_section: gf.kf.CrossSection) -> gf.kf.KCell:
        c = gf.get_component(
            bend,
            cross_section=xs.copy(width=gf.kcl.to_um(cross_section.width)),
            radius=gf.kcl.to_um(radius),
        )
        return gf.kf.KCell(base=c._base)  # type: ignore[no-any-return, call-overload]

    return bend_circular


def _wire_factory(
    wire: gf.typings.ComponentSpec, xs: gf.typings.CrossSectionSpec
) -> Callable[..., gf.kf.KCell]:
    xs = gf.get_cross_section(xs)

    def wire_function(length: int, cross_section: gf.kf.CrossSection) -> gf.kf.KCell:
        c = gf.get_component(
            wire,
            cross_section=xs.copy(width=gf.kcl.to_um(cross_section.width)),
            length=gf.kcl.to_um(length),
        )
        return gf.kf.KCell(base=c._base)  # type: ignore[no-any-return, call-overload]
...
def route_bundle_gs(
    ...
    ports1: list[gf.Port],
    ports2: list[gf.Port],
    cross_section: gf.typings.CrossSectionSpec = "GS",
    sort_ports: bool = False,
    avoid_bbox: bool = True,
) -> list[gf.kf.routing.generic.ManhattanRoute]:
    """Routes a bundle of ports using the GS cross-section.

    Args:
        component: component to route the bundle on.
        ports1: list of ports to route from.
        ports2: list of ports to route to.
        cross_section: cross-section to use for routing.
        sort_ports: whether to sort the ports before routing.
        avoid_bbox: whether to avoid the bounding box of the blackbox.
    """
    xs = gf.get_cross_section(cross_section)
    return gf.kf.routing.electrical.route_bundle_rf(
        gf.kf.KCell(base=component._base),
        start_ports=[gf.kf.Port(base=port.base) for port in ports1],
        end_ports=[gf.kf.Port(base=port.base) for port in ports2],
        wire_factory=_wire_factory("straight", "GS"),
        bend_factory=_bend_factory("bend_circular", "GS"),
        layer=gf.kcl.get_info(gf.get_layer(xs.sections[0].layer)),
        sort_ports=sort_ports,
        minimum_radius=gf.kcl.to_dbu(370),
        on_collision=None,
        on_placer_error=None,
        bboxes=[component.ibbox(layer=LAYER.BB_outline)] if avoid_bbox else [],
    )


route_bundle_rf = partial(
    gf.routing.route_bundle,
    cross_section="RF",
    auto_taper=False,
    router="optical",
...
routing_strategies = dict(
    ...
    route_bundle_rf=route_bundle_rf,
...
if __name__ == "__main__":
    xs = GSG()
    c = gf.c.bend_circular(cross_section=xs)

    # c = gf.Component()
    # b1 = c << gf.c.bend_circular(cross_section=xs)
    # b2 = c << gf.c.bend_circular(cross_section=xs)
    # b2.connect("e1", b1.ports["e2"])

    LAYER_VIEWS.to_lyp(PATH.lyp)
    # c = gf.components.straight(cross_section=E200)
    # c = gf.components.straight(cross_section=E600)
    # c = gf.components.straight(cross_section=E1700)
    c.pprint_ports()
    c.show()
    # s = c.to_3d()
    # s.show()
