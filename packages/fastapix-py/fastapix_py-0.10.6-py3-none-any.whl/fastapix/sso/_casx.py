...
# @Author   : zhangzhanqi
# @FILE     : _casx.py
# @Time     : 2024/6/3 下午2:27
# @Desc     : fork https://github.com/python-cas/python-cas
import datetime
...
class CASClientBase(object):
    ...
    def get_login_url(self, service_url=None):
        ...
        Returns:
            str: Login URL
        """
        params = {'service': service_url or self.service_url}
        if self.renew:
    ...
    def get_proxy_url(self, pgt, service_url=None):
        """Returns proxy url, given the proxy granting ticket

        Returns:
            str: Proxy URL
        """
        params = urllib_parse.urlencode({'pgt': pgt, 'targetService': service_url or self.service_url})
        return "%s/proxy?%s" % (self.server_url, params)

    def get_proxy_ticket(self, pgt, service_url=None):
        ...
        """
        response = self.session.get(self.get_proxy_url(pgt, service_url), verify=self.verify_ssl_certificate)
...
class CASClientV1(CASClientBase):
    ...
    def verify_ticket(self, ticket, service_url=None):
        """Verifies CAS 1.0 authentication ticket.

        Returns username on success and None on failure.
        """
        params = [('ticket', ticket), ('service', service_url or self.service_url)]
...
class CASClientV2(CASClientBase):
    ...
    def verify_ticket(self, ticket, service_url=None):
        ...
        response = self.get_verification_response(ticket, service_url)
        return self.verify_response(response)

    def get_verification_response(self, ticket, service_url=None):
        params = {
            'ticket': ticket,
...
class CASClientWithSAMLV1(CASClientV2, SingleLogoutMixin):
    ...
    def verify_ticket(self, ticket, service_url=None, **kwargs):
        ...
        """
        page = self.fetch_saml_validation(ticket, service_url)
    ...
    def fetch_saml_validation(self, ticket, service_url=None):
        ...
        params = {'TARGET': service_url or self.service_url}
