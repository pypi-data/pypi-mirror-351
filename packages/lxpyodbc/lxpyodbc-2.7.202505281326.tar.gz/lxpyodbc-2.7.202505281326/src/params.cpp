...
static bool IsArray(int sqltype) {
    switch (sqltype) {
    case SQL_LONGARRAY:
    case SQL_DOUBLEARRAY:
    case SQL_STRINGARRAY:
        return true;
    default:
        return false;
    }
...
static int PyToCType(Cursor *cur, unsigned char **outbuf, PyObject *cell, ParamInfo *pi)
{
    ...
    else if (PyUnicode_Check(cell))
    {
        ...
        Object encoded(PyCodec_Encode(cell, enc.name, "replace"));
        if (!encoded)
            return false;
    ...
    else if (PyDateTime_Check(cell))
    {
        ...
        SQLUINTEGER nanosecs = PyDateTime_DATE_GET_MICROSECOND(cell) * 1000;
        pts->fraction = nanosecs - (nanosecs % fast_pow10[9 - digits]);
...
static bool GetUnicodeInfo(Cursor* cur, Py_ssize_t index, PyObject* param, ParamInfo& info, bool isTVP)
{
    ...
    Object encoded(PyCodec_Encode(param, enc.name, "replace"));
    if (!encoded)
        return false;
...
static Py_ssize_t CollectStrings(PyObject* param, Py_ssize_t ncol, char *buf)
{
    Py_ssize_t nbytes, len;

    nbytes = 0;
    for(Py_ssize_t i = 0; i < ncol; i++) {
        auto v = PySequence_GetItem(param, i);
        // can be null, becomes empty string
        if (PyUnicode_Check(v)) {
            auto b = PyUnicode_AsUTF8String(v);
            len = PyBytes_Size(b);
            if (buf != nullptr) {
               memmove(buf, PyBytes_AsString(b), len);
               buf += len;
            }
            Py_XDECREF(b);
            nbytes += len;
            ...
            len = PyBytes_Size(v);
            auto b = PyBytes_AsString(v);
            if (buf != nullptr) {
                memmove(buf, b, len);
                buf += len;
            }
            nbytes += len;
        } else if (v != Py_None && v != null_binary) {
            // could format whatever it is as a string
            auto s = PyObject_Str(v);
            auto b = PyUnicode_AsUTF8String(s);
            Py_XDECREF(s);
            len = PyBytes_Size(b);
            if (buf != nullptr) {
                memmove(buf, PyBytes_AsString(b), len);
...
// LX array
static bool GetArrayInfo(Cursor *cur, Py_ssize_t index, PyObject* param, ParamInfo& info)
{
    Py_ssize_t ncol, nbytes;
    SQLSMALLINT sqltype;
    void *buf;

    SQLDescribeParam(cur->hstmt, index + 1, &sqltype, 0, 0, 0);
    if (!IsArray(sqltype)) {
        return false;
    }
    buf = nullptr;
    ncol = PySequence_Size(param);
    switch(sqltype) {
    case SQL_STRINGARRAY:
        // first pass for sizing, second pass to make value
        nbytes = CollectStrings(param, ncol, nullptr);
        if(nbytes != 0) {
            buf = PyMem_Malloc(nbytes);
            if(buf == nullptr)
                PyErr_NoMemory();
            CollectStrings(param, ncol, (char*)buf);
         }
         break;
    case SQL_LONGARRAY:
         nbytes = ncol * sizeof(PY_LONG_LONG);
         if(nbytes == 0)
             break;
         buf = PyMem_Malloc(nbytes);
         if(buf == nullptr)
         ...
         for(Py_ssize_t i = 0; i < ncol; i++) {
            ...
            ((PY_LONG_LONG*)buf)[i] = PyLong_AsLongLong(v);
            Py_XDECREF(v);
         }
         break;
    case SQL_DOUBLEARRAY:
        ...
        for(Py_ssize_t i = 0; i < ncol; i++) {
            ...
            ((double*)buf)[i] = PyFloat_AsDouble(v);
            Py_XDECREF(v);
        }
        break;
    default:
    ...
    Py_INCREF(param);
    info.ValueType = SQL_C_BINARY;
    info.ParameterType = sqltype;
    info.ColumnSize = ncol;	// ?
    info.DecimalDigits = 0;
    info.ParameterValuePtr = buf;
    info.allocated = true;
    info.BufferLength = nbytes;
    info.curTvpRow = 0;
    info.StrLen_or_Ind = nbytes;
    return true;
}
...
bool GetParameterInfo(Cursor* cur, Py_ssize_t index, PyObject* param, ParamInfo& info, bool isTVP)
{
    ...
    if (PySequence_Check(param)) {
        if (GetArrayInfo(cur, index, param, info)) {
            return true;
        }
        // continue to reject
    }
