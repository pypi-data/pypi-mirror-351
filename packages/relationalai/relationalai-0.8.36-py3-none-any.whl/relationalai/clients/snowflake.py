...
from relationalai.tools.constants import FIELD_PLACEHOLDER, RAI_APP_NAME, SNOWFLAKE_AUTHS, USE_GRAPH_INDEX, USE_DIRECT_ACCESS, WAIT_FOR_STREAM_SYNC
from .. import std
...
from typing import Any, Dict, Iterable, Tuple, List, Literal, Union, cast
from urllib.parse import urlencode, quote

from pandas import DataFrame

from ..tools.cli_controls import SpanSpinner, Spinner
...
from ..clients.util import IdentityParser, escape_for_f_string, get_with_retries, poll_with_specified_overhead, safe_json_loads, sanitize_module_name, scrub_exception, wrap_with_request_id, ms_to_timestamp
from ..environments import runtime_env, HexEnvironment, SnowbookEnvironment
from .. import dsl, rel, metamodel as m
from ..errors import DuoSecurityFailed, ERPNotRunningError, EngineAutoCreateFailed, EngineNameValidationException, EngineNotFoundException, EnginePending, EngineSizeMismatchWarning, EngineResumeFailed, Errors, InvalidSourceTypeWarning, RAIAbortedTransactionError, RAIException, HexSessionException, SnowflakeAppMissingException, SnowflakeChangeTrackingNotEnabledException, SnowflakeDatabaseException, SnowflakeImportMissingException, SnowflakeInvalidSource, SnowflakeMissingConfigValuesException, SnowflakeProxyAPIDeprecationWarning, SnowflakeProxySourceError, SnowflakeRaiAppNotStarted, SnowflakeTableObject, SnowflakeTableObjectsException, ModelNotFoundException, UnknownSourceWarning, ResponseStatusException
...
class Resources(ResourcesBase):
    ...
    def get_sf_session(self):
        ...
        if isinstance(runtime_env, SnowbookEnvironment):
            ...
            if authenticator in SNOWFLAKE_AUTHS:
                ...
                if missing_keys:
                    raise SnowflakeMissingConfigValuesException(missing_keys)
    ...
    def _create_engine(self, name:str, size:str|None=None, auto_suspend_mins:int|None=None, is_async:bool=False, headers={}):
    ...
    def create_engine(self, name:str, size:str|None=None, auto_suspend_mins:int|None=None, headers={}):
    ...
    def delete_engine(self, name:str, force:bool = False, headers={}):
        ...
        headers = {**headers, **prop_headers}
        self._exec(f"call {APP_NAME}.api.delete_engine('{name}', {force},{headers});")
    ...
    def resume_engine(self, name:str, headers={}):
        prop_headers = debugging.gen_current_propagation_headers()
        headers = {**headers, **prop_headers}
        self._exec(f"call {APP_NAME}.api.resume_engine('{name}',{headers});")
    ...
    def poll_use_index(
        self, app_name: str, sources: Iterable[str], model: str, engine_name: str, program_span_id: str | None, headers={}
    ):
        from .. import __version__ # avoid circular import by importing here
        counter = 1
        check_ready_count = 0
        tables_with_not_enabled_change_tracking = []
        table_objects_with_other_errors = []
        engine_errors = []
        # Flag to only ensure the engine is created asynchronously the initial call
        init_engine_async = True
        ...
        should_check_cdc = True


        wait_for_stream_sync = self.config.get(
            "wait_for_stream_sync", WAIT_FOR_STREAM_SYNC
        )
        current_user = self.get_sf_session().get_current_user()
        assert current_user is not None, "current_user must be set"
        data_freshness_mins = self.config.get_data_freshness_mins()

        cache = GraphIndexCache(current_user, model, data_freshness_mins, sources)
        sources = cache.choose_sources()

        with debugging.span("check_sources"):
            source_info = self._check_source_updates(sources)
        ...
        pyrel_version = f"pyrel/{__version__}"

        if stale_sources:
            with SpanSpinner("validate_sources", "Validating data sources...", "Data sources validated", leading_newline=True) as spinner:
                try:
                    # Delete all stale streams, so use_index could recreate them again
                    query = f"CALL {app_name}.api.delete_data_streams({stale_sources}, '{PYREL_ROOT_DB}');"
                    delete_response = self._exec(query)
        ...
        source_references = self._get_source_references(source_info)
        sources_object_references_str = ", ".join(source_references)

        def check_ready(spinner):
            nonlocal counter, check_ready_count, tables_with_not_enabled_change_tracking, table_objects_with_other_errors, engine_errors
            nonlocal should_check_cdc, init_engine_async

            check_ready_count += 1

            # To limit the performance overhead, we only check if ERP is running every 5 iterations
            if check_ready_count % 5 == 0:
                with debugging.span("check_erp_status"):
                    if not self.is_erp_running(app_name):
                        raise ERPNotRunningError

            params = json.dumps({
                    "model": model,
                    "engine": engine_name,
                    "user_agent": pyrel_version,
                    "pyrel_program_id": program_span_id,
                    "wait_for_stream_sync": wait_for_stream_sync,
                    "default_engine_size": self.config.get_default_engine_size(),
                    "should_check_cdc": should_check_cdc,
                    "init_engine_async": init_engine_async,
            })

            sql_string = f"CALL {app_name}.api.use_index([{sources_object_references_str}], PARSE_JSON(?), {headers});"

            with debugging.span("wait", counter=counter) as span:
                results = self._exec(sql_string, [params])
                ...
                if check_ready_count % 5 == 0 or not cdc_enabled:
                    should_check_cdc = True
                else:
                    should_check_cdc = False

                break_loop = False

                if ready:
                    cache.record_update(source_info)
                    break_loop = True
                    ...
                    for data in all_data:
                        ...
                        if data.get("errors", []):
                            for error in data.get("errors", []):
                                ...
                                table_objects_with_other_errors.append(
                                    SnowflakeTableObject(error_msg, data.get("fq_object_name"))
                                )
                            break_loop = True

                        if status == "created":
                            spinner.update_messages({ "message": f"Preparing {data.get('fq_object_name')}..." })
                        if (status != "synced" or data.get("pending_batches_count") > 0):
                            batches_count = data.get("pending_batches_count")
                            if batches_count:
                                spinner.update_messages({ "message": f"Preparing {data.get('fq_object_name')}, remaining batches: {data.get('pending_batches_count')}..." })
                            else:
                                spinner.update_messages({ "message": f"Preparing {data.get('fq_object_name')}..." })
                    counter += 1
                    ...
                    for error in errors:
                        if error.get("type") == "data":
                            ...
                            if ("change_tracking" in message or "change tracking" in message):
                                ...
                                err_source_type = source_info.get(err_source, {}).get("type")
                                tables_with_not_enabled_change_tracking.append((err_source, err_source_type))
                            else:
                                table_objects_with_other_errors.append(
        ...
        with Spinner(
            ...
            if (
                tables_with_not_enabled_change_tracking
                and self.config.get("ensure_change_tracking", False)
            ):
                for table in tables_with_not_enabled_change_tracking:
                    try:
                        fqn, kind = table
                        self._exec(f"ALTER {kind} {fqn} SET CHANGE_TRACKING = TRUE;")
                        tables_with_not_enabled_change_tracking.remove(table)
                        num_tables_altered += 1
                    except Exception:
                        pass

            if tables_with_not_enabled_change_tracking:
                spinner.update_messages({ "message": "Errors found. See below for details." })
                raise SnowflakeChangeTrackingNotEnabledException(
                    tables_with_not_enabled_change_tracking
                )

            if table_objects_with_other_errors:
                spinner.update_messages({ "message": "Errors found. See below for details." })
                raise SnowflakeTableObjectsException(table_objects_with_other_errors)
            if engine_errors:
                spinner.update_messages({ "message": "Errors found. See below for details." })
                # if there is an engine error, probably auto create engine failed
                raise EngineAutoCreateFailed(engine_name)

            if num_tables_altered > 0:
                s = "s" if num_tables_altered > 1 else ""
                spinner.update_messages({ "message": f"Enabled change tracking on {num_tables_altered} table{s}. Still preparing your data..." })
                poll_ready(spinner)
    ...
    def list_imports(
        ...
        # too slow. We use "max" since there are 2 rows per stream. First for the stream
        # creation and second for processing.
        statement = f"""
        select ID, RAI_DATABASE, FQ_OBJECT_NAME, CREATED_AT, CREATED_BY,
            CASE WHEN nextBatch.quarantined > 0 THEN 'quarantined' ELSE nextBatch.status END as batch_status,
            processing_errors, nextBatch.batches
            from {APP_NAME}.api.data_streams as ds
            left JOIN (
            select
                data_stream_id,
                max_by(status, unloaded) as status,
                max_by(batch_details, unloaded) as batch_details,
                max(processing_details:processingErrors) as processing_errors,
                max(unloaded) as unloaded,
                count(*) as batches,
                COUNT_IF(status = 'quarantined') as quarantined,
    ...
    def _check_exec_async_status(self, txn_id: str, headers={}):
    ...
    def _list_exec_async_artifacts(self, txn_id: str, headers={}) -> Dict[str, Dict]:
        ...
        with debugging.span("list_results"):
            response = self._exec(
                f"CALL {APP_NAME}.api.get_transaction_artifacts('{txn_id}',{headers});"
            )
        ...
        with requests.Session() as session:
            def _fetch_data(name_info):
                ...
                try:
                    ...
                    response = get_with_retries(session, presigned_url)
    ...
    def get_transaction_problems(self, txn_id: str) -> List[Dict[str, Any]]:
        with debugging.span("get_transaction_problems"):
            response = self._exec(
                f"select * from table({APP_NAME}.api.get_transaction_problems('{txn_id}'));"
            )
            if not response:
                return []
            return response
    ...
    def _exec_async_v2(
        ...
        query_attrs_dict = json.loads(headers.get("X-Query-Attributes", "{}"))
        ...
        with debugging.span("transaction", **query_attrs_dict) as txn_span:
            with debugging.span("create_v2", **query_attrs_dict) as create_span:
                ...
                if use_graph_index and not bypass_index:
                    sql_string = f"CALL {APP_NAME}.api.exec_async_v2(?, {{'database': '{database}', 'engine': '{engine}', 'inputs': {inputs}, 'readonly': {readonly}, 'nowait_durable': {nowait_durable}, 'language': '{language}', 'headers': {headers}}});"
                else:
                    sql_string = f"CALL {APP_NAME}.api.exec_async_v2('{database}','{engine}', ?, {inputs}, {readonly}, {nowait_durable}, '{language}', {headers});"
    ...
    def auto_create_engine(self, name: str | None = None, size: str | None = None, headers={}):
        ...
        with Spinner(
            ...
            with debugging.span("auto_create_engine_async", active=self._active_engine):
                ...
                try:
                    ...
                    if not engine:
                        # creating small engine by default, we will change that when configuring graph index will be implemented
                        self.create_engine_async(engine_name)
                        spinner.update_messages(
                            {
                                "finished_message": f"Starting engine {engine_name}...",
                            }
    ...
    def exec_lqp(
        ...
        *,
        ...
        # Auto-create engine if needed
        engine = self.auto_create_engine(engine, headers=headers)
    ...
    def exec_raw(
        ...
        *,
        ...
        try:
            ...
            if any(keyword in err_message for keyword in ENGINE_ERRORS):
                self.auto_create_engine(engine)
        ...
        with debugging.span("transaction") as txn_span:
            try:
                ...
                with debugging.span("exec_format"):
                    if use_graph_index:
                        res = self._exec(f"call {APP_NAME}.api.exec_into_table(?, ?, ?, ?, ?, ?);", [database, engine, raw_code, output_table, readonly, nowait_durable])
                        txn_id = json.loads(res[0]["EXEC_INTO_TABLE"])["rai_transaction_id"]
                    else:
                        res = self._exec(f"call {APP_NAME}.api.exec_into(?, ?, ?, ?, ?, {inputs}, ?, {headers});", [database, engine, raw_code, output_table, readonly, nowait_durable])
                ...
                with debugging.span("temp_table_swap", txn_id=txn_id):
                    ...
                    if out_sample:
                        ...
                        names = ", ".join([f'col{ix:03} as {IdentityParser(name).identity}' if f"col{ix:03}" in keys else f"NULL as {name}" for (ix, name) in enumerate(task.return_cols(allow_dups=False))])
    ...
    def _check_source_updates(self, sources: Iterable[str]):
        ...
        for source in sources:
            ...
            assert parser.db and parser.schema and parser.entity and parser.identity, f"Invalid source: {source}"
            source_types[parser.identity] = cast(SourceInfo, {"type": None, "state": ""})
            partitioned_sources[parser.db][parser.schema].append(parser.entity)
        ...
        query = (
            " UNION ALL ".join(
                ...
                RIGHT JOIN (
                    SELECT
                        (SELECT {app_name}.api.normalize_fq_ids(ARRAY_CONSTRUCT(TABLE_CATALOG || '.' || TABLE_SCHEMA || '.' || TABLE_NAME))[0]:identifier::string) as FQN,
...
class Provider(ProviderBase):
    def __init__(
        ...
        if resources:
            ...
            self.resources = resource_class(profile, config)
...
class SnowflakeClient(Client):
    def create_database(self, isolated=True, nowait_durable=True, headers={}):
        ...
        with debugging.span("poll_use_index", sources=self.resources.sources, model=model, engine=engine_name, **query_attrs_dict):
            self.poll_use_index(app_name, self.resources.sources, model, engine_name, program_span_id, headers=headers)
    ...
    def poll_use_index(
        ...
        return self.resources.poll_use_index(
            app_name, sources, model, engine_name, program_span_id, headers=headers
        )
...
def Graph(
    ...
    *,
    ...
    if use_direct_access:
        resource_class = DirectAccessResources
    client = client_class(
        resource_class(profile, config, connection),
        rel.Compiler(config),
        name,
        config,
        dry_run=dry_run,
...
class DirectAccessClient:
    ...
    def __init__(self, config: Config, session: Session, service_endpoint: str):
        ...
        if not self._session.connection.rest:
            raise ConnectionError("invalid Snowflake session, no rest connection")
        self._sf_rest_connection = self._session.connection.rest
    ...
    def request(
        ...
        if self._needs_token_refresh(response):
            self._sf_rest_connection._renew_session()
            headers["Authorization"] = f'Snowflake Token="{self._sf_rest_connection.token}"'
            response = requests.request(method, url, json=payload, headers=headers)

        return response

    def _prepare_url(self, endpoint: str, path_params: Dict[str, str], query_params: Dict[str, str]) -> Tuple[str, str]:
        try:
    ...
    def _prepare_headers(self, headers: Dict[str, str]) -> Dict[str, str]:
        ...
        if not self._is_snowflake_notebook:
            headers["Authorization"] = f'Snowflake Token="{self._sf_rest_connection.token}"'
        # Make sure a user is set. This will be overwritten by snowflake when ingress
        # endpoint is used but needs to be provided for svc2svc communication.
        headers["Sf-Context-Current-User"] = self._get_current_user()

        headers["user-agent"] = f"pyrel/{__version__}"
        headers["pyrel_program_id"] = debugging.get_program_span_id() or ""
        return headers

    def _needs_token_refresh(self, response: requests.Response) -> bool:
        # Unfortunately, snowflake returns a 404 for an invalid token with only limited 
        # detail. If parsed details are not as expected, we return the response, as the
        # 404 is likely to be returned from the erp.
        if response.status_code == 404:
            try:
                details = json.loads(response.text)
                if not details.get("responseType", "") == "ERROR":
                    return False
                return True
            except Exception as e:
                raise Exception(
                    f"Response has invalid JSON format. Status code: {response.status_code} Response: {response.text}. {e}"
                )
    ...
    def _get_current_user(self) -> str:
        # Snowflake session might return the user double quoted like '"username"'. To avoid
        # any unforeseen downstream issues, this helper method removes the double quotes.
        return (self._session.get_current_user() or "").strip('"').strip("'")
...
class DirectAccessResources(Resources):
    ...
    def __init__(
        ...
        super().__init__(profile=profile, config=config, connection=connection, dry_run=dry_run)
        self._endpoint_info = ConfigStore(ENDPOINT_FILE)
        self._service_endpoint = ""
        self._direct_access_client = None
        # session creation during init might fail. In this case we need to create the 
        # direct access client later.
        if self._session is not None:
            self.direct_access_client
    ...
    def _retrieve_service_endpoint(self, enforce_update=False) -> str:
        ...
        if is_snowflake_notebook:
            self._service_endpoint = f"http://{result[0]['SERVICE_ENDPOINT']}:8180"
        else:
    ...
    def direct_access_client(self) -> DirectAccessClient:
        ...
        try: 
            ...
            if not self._session:
                self._session = self.get_sf_session()
            self._direct_access_client = DirectAccessClient(self.config, self._session, service_endpoint)
        except Exception as e:
            raise e
        return self._direct_access_client
    ...
    def _build_snowflake_session(self, connection_parameters: Dict[str, Any]) -> Session:
        try:
            # by default, a connection created via session will have paramstyle "qmark",
            # while the default for the connector is "pyformat". PyRel needs "qmark".
            connection_parameters["paramstyle"] = "qmark"
            # result format needs to be defined for direct access client to work
            connection_parameters["session_parameters"] = {
                'PYTHON_CONNECTOR_QUERY_RESULT_FORMAT': 'json'
            }
            conn = snowflake.connector.connect(**connection_parameters)
            connection_parameters["connection"] = conn
            session = Session.builder.configs(connection_parameters).create()
            return session
        except snowflake.connector.errors.Error as e:
            raise SnowflakeDatabaseException(e)
    ...
    def delete_graph(self, name:str, force=False):
        if self.config.get("use_graph_index", USE_GRAPH_INDEX):
            keep_database = not force and self.config.get("reuse_model", True)
            with debugging.span("delete_graph", name=name, keep_database=keep_database):
                if keep_database:
                    # TODO: We should keep a list of the released models for garbage collection later
                    return
                response = self._exec(f"select {APP_NAME}.api.get_index_db_name('{name}');")
                if response:
                    dbName = response[0][0]
                    self._delete_database(dbName)
                else:
                    raise Exception("There was no response from the release index call.")
        else:
            with debugging.span("delete_graph", name=name):
