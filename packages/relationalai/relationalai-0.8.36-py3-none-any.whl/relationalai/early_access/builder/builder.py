...
def type_from_type_str(model:Model|None, type_str: str) -> Concept:
    if type_str in python_types_str_to_concepts:
        ...
        return model.concepts[type_str]
    else:
...
class Producer:
    ...
    def _get_relationship(self, name:str) -> Relationship|RelationshipRef|RelationshipFieldRef:
        root_type = to_type(self) or Concept.builtins["Any"]
        r = Relationship(
...
class Concept(Producer):
    ...
    def __init__(self, name:str, extends:list[Any] = [], model:Model|None=None, identify_by:dict[str, Any]={}):
        ...
        self._relationships = {}
        self._extends = []
        self._reference_schemes: list[tuple[Relationship, ...]] = []
    ...
    def identify_by(self, *args: Relationship):
        if not args:
            raise ValueError("identify_by requires at least one relationship")
        for rel in args:
            if not isinstance(rel, Relationship):
                raise ValueError(f"identify_by must be called with a Relationship, got {type(rel)}")
            else:
                self._validate_identifier_relationship(rel)
        self._reference_schemes.append(args)

    def _validate_identifier_relationship(self, rel:Relationship):
        if rel._arity() != 2:
            raise ValueError("identify_by can only be applied on binary relations")
        if rel._fields[0].type_str != self._name:
            raise ValueError("For identify_by all relationships must be defined on the same Concept")
    ...
    def _get_relationship(self, name: str) -> Relationship | RelationshipRef | RelationshipFieldRef:
        for parent in self._extends:
            if name in parent._relationships:
                return RelationshipRef(self, parent._relationships[name])
        return super()._get_relationship(name)
...
class Field():
    ...
    def __str__(self):
        return f"{self.name}:{self.type_str}{'*' if self.is_many else ''}"

class Relationship(Producer):
    builtins = {}

    def __init__(self, madlib:Any, parent:Producer|None=None, short_name:str="", model:Model|None=None, fields:list[Field]|None=None, ir_relation:ir.Relation|None=None):
        ...
        self._relationships = {}
        self._fields:list[Field] = fields or self._parse_schema_format(madlib)
        ...
        self._readings = [RelationshipReading(madlib, self._fields, alt_of=self, short_name=short_name, model=found_model, parent=parent)]
        self._annotations = []
    ...
    def _short_name(self):
        return self._passed_short_name or self._default_short_name(self._madlib)

    def _is_filter(self) -> bool:
        return self._short_name in [">", "<", "=", "!=", ">=", "<="]

    def _default_short_name(self, madlib:Any) -> str:
    ...
    def _parse_schema_format(self, format_string:str):
        ...
        for field_name, field_name_is_many, field_type, field_type_is_many in matches:
            ...
            while field_name in used_names:
                field_name = f"{field_name}{ix}"
                ix += 1
            used_names.add(field_name)

            fields.append(Field(field_name, field_type, bool(field_type_is_many)))
    ...
    def alt(self, madlib:Any, short_name:str="") -> RelationshipReading:
        fields: list[Field] = self._parse_schema_format(madlib)
        if Counter(fields) != Counter(self._fields):
            raise ValueError(
                f"Invalid alternative relationship. The alternative group of used fields ({', '.join(str(f) for f in fields)}) does not match with the original ({', '.join(str(f) for f in self._fields)})")
        reading = RelationshipReading(madlib, fields, alt_of=self, short_name=short_name, model=self._model)
...
class RelationshipReading(Producer):
    ...
    def __init__(self, madlib:Any, fields:list[Field], alt_of:Relationship, short_name:str, model:Model|None=None, parent:Producer|None=None,):
...
def _get_relationship_item(rel:Relationship|RelationshipReading, arg:str|int|Concept) -> Any:
    if isinstance(arg, int):
        ...
        for idx, ref in enumerate(rel._field_refs):
            if result is None and ref._thing._id == arg._id:
                result = RelationshipFieldRef(rel._parent, rel, idx)
            else:
                if ref._thing._id == arg._id:
                    raise ValueError(f"Ambiguous reference to the field: '{arg._name}' presented in more than one field. Use reference by name or position instead")
        return result
    else:
...
for builtin in builtins.builtin_relations + builtins.builtin_annotations:
    ...
    for field in builtin.fields:
        field_type = re.sub(r'[\]\}]', '*', re.sub(r'[\[\{\(]', '', str(field.type))).strip()
...
class Quantifier():
    ...
    def where(self, *args: Any) -> Quantifier:
        new = self.clone()
        if not new._model:
            new._model = find_model(args)
        new._where = new._where.where(*args)
...
class Exists(Quantifier):
    def __str__(self):
        ...
        where_items = '\n        '.join(map(str, self._where._where))
        where = f"\n        {where_items}" if self._where._where else ""
        return f"(exists [{args_str}]{where})"
...
class Forall(Quantifier):
    def __str__(self):
        ...
        where_items = '\n        '.join(map(str, self._where._where))
        where = f"\n        {where_items}" if self._where._where else ""
        return f"(forall [{args_str}]{where})"

class Not(Quantifier):
    def where(self, *args: Any) -> Quantifier:
        raise NotImplementedError("Not cannot have a where clause")

    def __str__(self):
        args_str = '\n    '.join(map(str, self._args))
        return f"(not {args_str})"

def exists(*args: Any) -> Exists:
    return Exists(*args)

def forall(*args: Any) -> Forall:
    return Forall(*args)
...
def create_enum_class(model):
    class ModelEnum(Enum):
        def __init_subclass__(cls, **kwargs):
            ...
            cls._concept = model.enums[cls] = c
...
class Fragment():
    ...
    def into(self, table:Any) -> None:
        ...
        ir_model = qb_model._to_ir()
        self._source = runtime_env.get_source_pos()
        with debugging.span("query", dsl=str(self), **with_source(self), meta=self._meta):
            query_task = qb_model._compiler.fragment(self)
            qb_model._to_executor().execute(ir_model, query_task, export_to=table._fqn)
...
class Model():
    def __init__(
        ...
        self.enums:dict[Type[Enum], Concept] = {}
    ...
    def Concept(self, name:str, extends:list[Concept|Any]=[], identify_by:dict[str, Any]={}) -> Concept:
        if name not in self.concepts:
            self.concepts[name] = Concept(name, model=self, extends=extends, identify_by=identify_by)
...
class Compiler():
    def __init__(self):
        ...
        self.relations:dict[Relationship|Concept|RelationshipRef|RelationshipReading|ir.Relation, ir.Relation] = {}
    ...
    def model(self, model:Model) -> ir.Model:
        ...
        with debugging.span("rule_batch"):
            # itreate global_roots getting both idx and rule
    ...
    def order_by_or_limit(self, fragment:Fragment, ctx:CompilerContext):
        ...
        # If there is an order-by, then the limit is applied on the fields there.
        # Otherwise, the limit is applied on the fields in the select (with a default sort order).
        args_to_process = ordered_set()
        arg_is_ascending = []
        if fragment._order_by:
            for item in fragment._order_by:
            ...
            args_to_process.update(fragment._select)
            arg_is_ascending.extend([RankOrder.ASC] * len(fragment._select))
        ...
        args = ordered_set()
        projection = ordered_set()
        for arg in args_to_process:
            keys = find_keys(arg)
            for key in keys:
                projection.add(self.lookup(key, inner_ctx))
            args.add(self.lookup(arg, inner_ctx))

        vars = list(projection) + list(args)
        limit_ctx.add(inner_ctx.safe_wrap([ir.Default(v, None) for v in vars]))

        v = f.var("v", types.Int)
        limit_ctx.add(f.rank(list(projection), [], list(args), arg_is_ascending, v, fragment._limit))
        ctx.add(f.logical(list(limit_ctx.items), [v]))
        return v
    ...
    def select(self, fragment:Fragment, items:PySequence[Expression], ctx:CompilerContext, rank_var:ir.Var|None=None):
        ...
        all_keys:OrderedSet[ir.Var] = OrderedSet()
        fields = []
        if rank_var:
        ...
        for ix, item in enumerate(items):
            keys = find_keys(item)
            for key in keys:
                key_v = self.lookup(key, ctx)
                assert isinstance(key_v, ir.Var)
                all_keys.add(key_v)
            sub_ctx = ctx.clone()
            raw_v = self.lookup(item, sub_ctx)
            if not isinstance(raw_v, list):
                raw_v = [raw_v]
            if len(sub_ctx.items) >= 1:
                ctx.add(sub_ctx.safe_wrap([ir.Default(v, None) for v in raw_v if isinstance(v, ir.Var)]))
        ...
        if fields:
            ...
            ctx.add(f.output(fields, keys=list(all_keys), annos=annos))
    ...
    def explode_ref_schemes(self, item:ConceptExpression, ctx:CompilerContext, update=False):
        ...
        for ix, info in enumerate(hierarchy):
            ...
            scheme = info["scheme"]
            cur = ctx.to_value((item, ix), f.var(to_name(concept), self.to_type(concept)))
    ...
    def lookup(self, item:Any, ctx:CompilerContext) -> ir.Value|list[ir.Var]:
        if isinstance(item, ConceptExpression):
            ...
            if isinstance(item, ConceptMember):
                ...
                if isinstance(out, (int, float, str, PyDecimal)):
        ...
        elif isinstance(item, (Relationship, RelationshipRef)):
            ...
            if item._op._name == "rank":
                unwrapped_args = []
                rank_orders = []
                for arg in item._args:
                    if isinstance(arg, RankOrder):
                        unwrapped_args.extend(arg._args)
                        rank_orders.extend([arg._is_asc] * len(arg._args))
                    else:
                        unwrapped_args.append(arg)
                        rank_orders.append(RankOrder.ASC)

                # the projection includes all keys for the args
                projection = flatten([self.lookup(key, agg_ctx) for key in find_keys(unwrapped_args)], flatten_tuples=True)

                args = flatten([self.lookup(a, agg_ctx) for a in unwrapped_args])

                internal_vars = set(args + projection)
                ir_node = f.rank(projection, group, args, rank_orders, out)
        ...
        elif isinstance(item, Quantifier):
            agg_ctx = ctx.clone()
            self.where(item._where, item._where._where, agg_ctx)
            args = [self.lookup(a, agg_ctx) for a in item._args]
            args = [item for item in flatten(args) if isinstance(item, ir.Var)]
            body = f.logical(list(agg_ctx.items))
            if isinstance(item, Exists):
                ctx.add(f.exists(args, body))
            elif isinstance(item, Forall):
                ctx.add(f.for_all(args, body))
        ...
        elif isinstance(item, (int, float, str, date, datetime)):
            return item
    ...
    def update(self, item:Expression|Match|Union, ctx:CompilerContext) -> ir.Value|list[ir.Var]:
        if isinstance(item, ConceptExpression):
            ...
            # if this is a member lookup, then our out var is just the identity
            # passed in
            if isinstance(item, ConceptMember):
                out = self.lookup(ident, ctx)
