...
class Visitor(GenericVisitor[None]):
    ...
    def _walk_type(self, node: ir.Type, parent: Optional[ir.Node]):
        ...
        v = self.enter(node, parent)
        if isinstance(node, ir.ScalarType):
...
@dataclass
class DeprecatedPass():
    """
    This is the old Pass class. To be replaced by a Rewriter.
    """

    rewritten: dict[ir.Node, ir.Node] = field(default_factory=dict, init=False, compare=False, hash=False)
    ...
    def walk(self, node: T, parent=None) -> T:
        ...
        result = self.rewritten.get(node, None)
        if result:
