...
Int = ir.ScalarType("Int", util.ordered_set().frozen())
Float = ir.ScalarType("Float", util.ordered_set().frozen())
Decimal = ir.ScalarType("Decimal", util.ordered_set().frozen())
Bool = ir.ScalarType("Bool", util.frozen())
Binary = ir.ScalarType("Binary", util.ordered_set().frozen()) # 0 or 1
Date = ir.ScalarType("Date", util.frozen())
DateTime = ir.ScalarType("DateTime", util.frozen())
Symbol = ir.ScalarType("Symbol", util.frozen(String))
Sha1 = ir.ScalarType("SHA1", util.frozen())
RowId = ir.ScalarType("RowId", util.frozen(Int))
...
literal_types = util.ordered_set(*[String, Bool, Int, Float, Decimal, Date, DateTime]).frozen()
...
def is_value_base_type(t: ir.Type) -> bool:
    return isinstance(t, ir.ScalarType) and t in [Hash, String, Number, Int, Float, Decimal, Bool, Binary, Symbol, Sha1]
