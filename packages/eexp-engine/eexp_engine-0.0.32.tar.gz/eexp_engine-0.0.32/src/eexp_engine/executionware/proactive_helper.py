...
def save_dataset(variables, resultMap, key, value):
    ...
    if DATASET_MANAGEMENT == "ZENOH":
        return save_dataset_zenoh(variables, key, value, resultMap)
...
def save_dataset_zenoh(variables, key, value, resultMap):
    ...
    if key in variables:
        output_file_name = variables.get(key)
    ...
    _add_file_to_metrics(resultMap, task_name, OUTPUT_FILE, key, output_file_name, file_url, project_id, file_type)
...
def _add_file_to_metrics(resultMap, task_name, input_or_output, file_key, file_name, file_url, project_id, file_type):
    key = f"file:{task_name}:{input_or_output}:{file_key}"
    file_metadata = {
        "file_name": file_name,
        "file_url": file_url,
        "project_id": project_id,
    ...
    resultMap.put(key, json.dumps(file_metadata))
...
def load_dataset(variables, resultMap, key):
    ...
    if DATASET_MANAGEMENT == "ZENOH":
        return load_dataset_zenoh(variables, key, resultMap)
    print("Cannot load dataset, please setup DATASET_MANAGEMENT in config.py")
    exit(1)
...
def load_dataset_zenoh(variables, key, resultMap):
    file_url_template = f"{DATASET_MANAGEMENT_URL}/file/{{}}"
    task_name = variables.get("PA_TASK_NAME")
    if key in variables:
        file_type = FILE_TYPE_EXTERNAL
    ...
    entry = _look_up_file_in_catalog(fname, project_id)
    file_id = entry.get("id")
    file_url = file_url_template.format(file_id)
    print("Downloading:", file_url)
    f_response = requests.get(file_url)
    _add_file_to_metrics(resultMap, task_name, INPUT_FILE, key, fname, file_url, project_id, file_type)
    f_response.raise_for_status()
    return f_response.content
...
def _look_up_file_in_catalog(fname, project_id):
    ...
    if len(results) > 1:
        raise Exception(f"More than one files with name {fname} found in project {project_id}.")
    return results[0]
