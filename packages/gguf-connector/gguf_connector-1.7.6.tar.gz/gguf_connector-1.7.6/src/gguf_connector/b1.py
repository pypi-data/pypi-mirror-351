...
if gguf_files:
    ...
    try:
        ...
        try:
            ...
            input_path=selected_file
            use_bf16 = True
            model_file_path = f"{os.path.splitext(input_path)[0]}-bf16.safetensors"
            convert_gguf_to_safetensors(input_path, model_file_path, use_bf16)
            if use_bf16:
                input_file = model_file_path
                output_file = f"{os.path.splitext(input_path)[0]}_fp8_e4m3fn.safetensors"
                data = load_file(input_file)
                quantized_data = {}
                print("Starting 2nd quantization process...")
                for key, tensor in tqdm(data.items(), desc="Quantizing tensors", unit="tensor"):
                    # Move tensor to CPU instead of GPU # better option for this special case
                    tensor = tensor.to(dtype=torch.bfloat16, device="cpu")
                    quantized_tensor = quantize_to_fp8(tensor)
                    quantized_data[key] = quantized_tensor.cpu()
                save_file(quantized_data, output_file)
                print("Model quantization task complete!")
