"""copied from galaxy-tool-util, someday hopefully sync them"""
...
class ContainerVolume(metaclass=ABCMeta):
    ...
    def __init__(
        self, path: str, host_path: Optional[str] = None, mode: Optional[str] = None
    ):
        self.path = path
...
class DockerVolume(ContainerVolume):
    ...
    def from_str(cls, as_str: str) -> "DockerVolume":
        ...
        if len(parts) == 1:
            ...
            if all(
                mode_part in DockerVolume.valid_modes
                for mode_part in parts[1].split(",")
            ):
                kwds["mode"] = parts[1]
                kwds["path"] = kwds["host_path"]
            else:
    ...
    def __str__(self) -> str:
        volume_str = ":".join(
            x for x in (self.host_path, self.path, self.mode) if x is not None
        )
