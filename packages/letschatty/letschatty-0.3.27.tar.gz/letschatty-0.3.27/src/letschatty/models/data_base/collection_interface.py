...
class ChattyAssetCollectionInterface(Generic[T], ABC):
    ...
    def get_preview_docs(self, projection = {}) -> List[ChattyAssetPreview]:
        """We get the previews of all the documents in the collection for all companies"""
        if not self.type.preview_class:
        ...
        return [self.type.preview_class.from_dict(doc) for doc in docs]
