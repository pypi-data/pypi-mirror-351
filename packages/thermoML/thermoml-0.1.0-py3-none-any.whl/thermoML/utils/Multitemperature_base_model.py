...
def create_model(trial, X_train):
    ...
    model.compile(optimizer=optimizer, loss="mae", metrics=["mse"]) #removed root_mean_squared_error from metrics = [...] because it's not in tf-2.12.0
...
def main_base_train_MT(df, df_mu, set_size, base_model_path = "Results/base_model", n_models = 20, hp_tuning = False):
    
    temp_columns = ['temp_inv_1', 'temp_inv_2', 'temp_inv_3', 'temp_inv_4', 'temp_inv_5']
    df_base = df.melt(
        id_vars=[col for col in df.columns if col not in temp_columns], 
        value_vars=temp_columns,
        var_name='temperature_label', 
        value_name='temperature'
    )
    df_base = df_base.drop(columns=['temperature_label'])
    df_base['temperature'] = 1/df_base['temperature']

    
    mu_columns = ['mu_1', 'mu_2', 'mu_3', 'mu_4', 'mu_5']
    df_mu_base = df_mu.melt(
        id_vars=[col for col in df_mu.columns if col not in mu_columns], 
        value_vars=mu_columns,
        var_name='viscosity_label', 
        value_name='viscosity'
    )
    df_mu_base = df_mu_base.drop(columns=['viscosity_label'])
...
def main_base_test_MT(df_test, temp_test, base_model_path, threshold = 5, to_drop = ['Compounds', 'smiles']):
