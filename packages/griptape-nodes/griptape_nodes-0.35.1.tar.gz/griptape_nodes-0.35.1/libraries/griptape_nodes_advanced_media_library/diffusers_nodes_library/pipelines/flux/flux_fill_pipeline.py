...
class FluxFillPipeline(ControlNode):
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = self.get_pipe_params()
    ...
    # I can refactor so that node does not need to be passed to the parameter classes constructors,
    # then this would not be needed and we could pass them around in collections (including
    # in to super init in subclasses).
    def get_pipe_params(self) -> FluxFillPipelineParameters | DiptychFluxFillPipelineParameters:
        return FluxFillPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        def callback_on_step_end(
            pipe: diffusers.FluxFillPipeline,
            i: int,
            _t: Any,
