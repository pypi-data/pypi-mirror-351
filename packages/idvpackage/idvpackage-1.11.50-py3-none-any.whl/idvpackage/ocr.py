...
class IdentityVerification:
    ...
    def __init__(self, credentials_string, api_key=None):
        """
    ...
    def try_detect_face_at_angles(self, image_array):
        ...
        for angle in angles:
            if angle == 0:
                ...
                rotated = np.array(Image.fromarray(image_array).rotate(angle, expand=True))
    ...
    def extract_front_id_info(self, front_id, country):
        ...
        if country == 'IRQ':
                ...
                try:
                    ...
                    try:
                        ...
                        logging.info(f'--------------Fallback for translation keyword matching front\n')
                    ...
                    front_blurred, front_glare = self.get_blurred_and_glared_for_doc(image)
                    logging.info(f'----------------Time taken for fraud detection attributes front: {time.time() - st} seconds\n')
                    ...
                    logging.info(f'----------------Time taken for iraq data formatting and final extraction front: {time.time() - st} seconds\n')
        ...
        if country == 'QAT':
                ...
                from idvpackage.common import load_and_process_image_deepface_all_orientations
                ...
                try:
                    ...
                    st = time.time()
                    front_face_locations, front_face_encodings = load_and_process_image_deepface_all_orientations(front_id)
                    ...
                    logging.info(f'----------------Time taken for qatar data formatting and final extraction front: {time.time() - st} seconds\n')
        ...
        if country == 'LBN':
                ...
                try:
                    ...
                    # Check for Civil status document keywords in translated text
                    civil_status_keywords = r'(General Directorate|Convergence|Municipalities|Personal Status|statement of individuals)'
                    civil_status_match = re.search(civil_status_keywords, translated_id_text, re.IGNORECASE)
                    
                    if civil_status_match:
                        front_data["error"] = "not_front_id"
                        return front_data
                    
                    # Check for blur using the new comprehensive method
                    image = np.array(processed_front_id)
                    if is_image_blur(image, laplace_threshold=30, canny_threshold=1500, checks_threshold=120, min_area_percentage=1.0):
                        print(f"Blur/Brightness issue detected in front image, marking as covered photo")
                        front_data['error'] = 'covered_photo'
                        return front_data
                    ...
                    logging.info(f'----------------Time taken for data formatting and final extraction front: {time.time() - st} seconds\n')
                    
                    required_keys = ["name", "dob", "dob_en", "place_of_birth_ar", "id_number", "expiry_date"]

                    empty_string_keys = [
                        key for key, value in front_data_fields.items()
                    ...
                    required_keys = ["front_face_locations", "front_face_encodings", "name", "dob", "dob_en", "place_of_birth_ar", "id_number", "expiry_date"]
    ...
    def extract_back_id_info(self, back_id, country):
        ...
        if country == 'IRQ':
                ...
                try:
                    ...
                    st = time.time()
                    compressed_image = BytesIO()
                    processed_back_id.save(compressed_image, format="JPEG", quality=60, optimize=True)
                    compressed_image_data = compressed_image.getvalue()
                    ...
                    photo_on_screen_result = detect_photo_on_screen(self.client, back_id)
                    back_blurred, back_glare = self.get_blurred_and_glared_for_doc(image)
                    logging.info(f'----------------Time taken for fraud detection attributes back: {time.time() - st} seconds\n')
        ...
        if country == 'LBN':
                ...
                try:
                    ...
                    logging.info(f'----------------Time taken for data formatting and final extraction back: {time.time() - st} seconds\n')
    ...
    def exract_passport_info(self, passport, country):
        ...
        if country.upper() == 'LBN':
                ...
                try:
                    ...
                    # Check for Civil status document keywords in translated text
                    civil_status_keywords = r'(General Directorate|Convergence|Municipalities|Personal Status|statement of individuals)'
                    civil_status_match = re.search(civil_status_keywords, passport_text, re.IGNORECASE)
                    ...
                    # Check for blur using the new comprehensive method
                    image = np.array(processed_passport)
                    if is_image_blur(image, laplace_threshold=30, canny_threshold=1500, checks_threshold=120,
                                     min_area_percentage=1.0):
                        print(f"Blur/Brightness issue detected in front image, marking as covered photo")
                        passport_data['error'] = 'covered_photo'
                        return passport_data
                    ...
                    logging.info(f'----------------Time taken for data formatting and final extraction passport: {time.time() - st} seconds\n')
                    ...
                    if not passport_data.get('id_number') or not passport_data.get('dob') or not passport_data.get('first_name') or not passport_data.get('last_name') or not passport_data.get('expiry_date'):
                        passport_data['error'] = 'covered_photo'

                    non_optional_keys = ["front_face_locations", "id_number", "dob", "first_name", "last_name", "expiry_date"]
                    empty_string_keys = [key for key, value in passport_data.items() if key in non_optional_keys and (value == '' or value == [] or value == '[]')]
