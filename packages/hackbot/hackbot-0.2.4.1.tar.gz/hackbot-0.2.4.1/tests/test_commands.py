...
@patch("src.hackbot.commands.authenticate")
@patch("src.hackbot.commands.cli_run")
@patch("src.hackbot.commands.generate_issues")
@patch("src.hackbot.commands.Path")
def test_hackbot_run_with_github(
    mock_path_class, mock_generate_issues, mock_cli_run, mock_auth, mock_exists, mock_args
):
    ...
    mock_cli_run.return_value = [
        {"bug_id": "TEST-1", "title": "Test Bug", "description": "Test Description"}
    ]
    ...
    # Set up async mock for successful case
    mock_generate_issues.return_value = AsyncMock(return_value=None)()

    result = hackbot_run(mock_args)
    assert result == 0

    # Check that generate_issues was called with the correct arguments
    assert mock_generate_issues.call_args.args == (
        "test/repo",
        "github_token",
        [{"bug_id": "TEST-1", "title": "Test Bug", "description": "Test Description"}],
    )

    # Set up async mock for failure case
    mock_generate_issues.reset_mock()
    mock_generate_issues.side_effect = Exception("GitHub API Error")

    # The exception should be re-raised since it's not a ClientPayloadError or 413 error
    with pytest.raises(Exception) as excinfo:
        hackbot_run(mock_args)
    assert str(excinfo.value) == "GitHub API Error"
...
@patch("src.hackbot.commands.os.path.exists")
@patch("src.hackbot.commands.Path")
def test_check_github_args(mock_path_class, mock_exists):
    ...
    args = parser.parse_args(
        ["run", "--api-key", "test-key", "--issues_repo", "test/repo", "--source", "."]
    )
    mock_exists.return_value = True
    ...
    args = parser.parse_args(["run", "--api-key", "test-key", "--source", "."])
    mock_exists.return_value = True
    result = check_run_args(args)
...
def test_check_run_args(mock_path_class, mock_exists):
    ...
    assert result.api_key == "test-key"
    assert result.command == "run"
    assert result.debug is True  # Debug value is converted to boolean
...
@patch("src.hackbot.commands.get_selectable_models")
@patch("src.hackbot.commands.log.info")
def test_show_selectable_models(mock_logger, mock_get_selectable_models):
    ...
    result = show_selectable_models(api_key="test-key")
    assert result == 0

    # Verify models were logged, ignoring ANSI color codes
    mock_logger.assert_has_calls(
        [
            call(mock_logger.call_args_list[0][0][0]),  # First call with any string
            call("  - model1"),
            call("  - model2"),
            call("  - model3"),
            call(mock_logger.call_args_list[-1][0][0]),  # Last call with any string
        ],
        any_order=False,
    )
