...
async def test_generate_issues(mock_github, test_params):
    ...
    test_results = [{"bug_id": "BUG-1", "title": "Test Bug", "description": "This is a test bug"}]
...
def test_hackbot_run(mock_path_class, mock_cli_run, mock_auth, mock_exists, mock_args):
    ...
    result = hackbot_run(mock_args)
    assert result == 0
    mock_cli_run.assert_called_once()  # Should still be called only once from first test
