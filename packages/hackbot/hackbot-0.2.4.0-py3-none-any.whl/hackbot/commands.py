...
def setup_parser():
    ...
    for cmd_parser in [run_parser, models_parser, scope_parser, learn_parser]:
        add_api_key_argument(cmd_parser)

    for common_parser in [scope_parser, run_parser]:
...
def show_selectable_models(api_key: str) -> int:
    ...
    for model in response:
        log.info(f"  - {model}")
    log.info(
        f"If you want to use a different model, you can do so with the {CYAN}hackbot run --model <model>{END} flag."
    )
    return 0
...
def learn_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not asyncio.run(authenticate(args.api_key)):
            log.error(
                f"❌ Authentication failed. Please check your API key {BLUE}HACKBOT_API_KEY{RED} is set, and is valid. If you do not have an API key, please sign up at {GREEN}https://hackbot.co{RED} before running hackbot.${END}"
            )
            return 1
...
def handle_missing_scope_file() -> int:
    log.info(f"\n❌ No {RED}scope.txt{END} found in your project.")
    log.info("\nPlease choose how you'd like to proceed:")
    log.info(f"1) {GREY}Let hackbot automatically generate scope.txt (default){END}")
    log.info(f"2) {GREY}Generate and customize scope.txt manually{END}")

    while True:
        try:
            choice = input(f"\n{BLUE}Enter your choice [1/2] (default: 1):{END} ").strip()
            if not choice:
                choice = "1"
            choice = int(choice)
            if choice in [1, 2]:
                break
            log.warning("Please enter either 1 or 2")
        except ValueError:
...
def hackbot_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not asyncio.run(authenticate(args.api_key)):
            log.error(
                f"❌ Authentication failed. Please check your API key {BLUE}HACKBOT_API_KEY{RED} is set, and is valid. If you do not have an API key, please sign up at {GREEN}https://hackbot.co{RED} before running hackbot."
            )
            return 1
        ...
        # Add scope.txt generation selector for run command
        if args.command == Endpoint.RUN.value:
