...
from claude_code_cost_analyzer.config import (
    ConfigError,
    get_default_config,
    load_config_file,
    merge_config,
...
class TestConfig(unittest.TestCase):
    ...
    @patch.dict(
        os.environ,
        {
            "CLAUDE_CODE_COST_ANALYZER_API_KEY": "env_api_key",
            "CLAUDE_CODE_COST_ANALYZER_LOG_DIR": "/env/log/dir",
        },
    )
    ...
    @patch.dict(
        os.environ,
        {
            "CLAUDE_CODE_COST_ANALYZER_API_KEY": "env_key",
        },
    )
    ...
    @patch.dict(
        os.environ,
        {
            "CLAUDE_COST_ANALYZER_API_KEY": "legacy_api_key",
        },
    )
    def test_legacy_env_variables_backward_compatibility(self):
        """Test that legacy environment variables still work."""
        env_config = get_config_from_env()

        self.assertEqual(env_config["exchange_rate_api_key"], "legacy_api_key")

    @patch.dict(
        os.environ,
        {
            "CLAUDE_CODE_COST_ANALYZER_API_KEY": "new_api_key",
            "CLAUDE_COST_ANALYZER_API_KEY": "legacy_api_key",
        },
    )
    def test_new_env_variables_take_precedence(self):
        """Test that new environment variables take precedence over legacy ones."""
        env_config = get_config_from_env()

        # New variable should take precedence
        self.assertEqual(env_config["exchange_rate_api_key"], "new_api_key")
