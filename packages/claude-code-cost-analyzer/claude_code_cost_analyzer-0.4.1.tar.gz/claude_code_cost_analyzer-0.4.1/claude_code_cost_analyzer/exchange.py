"""
Exchange rate functionality for Claude Code Cost Analyzer.
...
def get_cached_rate(from_currency: str = "USD", to_currency: str = "EUR") -> Optional[float]:
    """
    Get currently cached exchange rate if available.
    ...
    Returns:
        Cached exchange rate or None if not cached or expired
    """
    currency_pair = f"{from_currency.upper()}_{to_currency.upper()}"
    return _rate_cache.get_rate(currency_pair)
