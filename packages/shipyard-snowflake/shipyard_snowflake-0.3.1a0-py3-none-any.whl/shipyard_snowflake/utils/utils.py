...
def read_file(
    ...
    Args:
        ...
        snowflake_dtypes: (Union[Dict,None]): The optional dictionary of pandas data types to be used when reading in the file. Defaults to None
    ...
    if _file_fits_in_memory(file):
        ...
        if file_type == "csv":  # always true for now, will be augmented in the future
            if dates:
                df = pd.read_csv(
                    file,
                    dtype=pandas_dtypes,
...
def map_pandas_to_snowflake(data_type_dict: Dict[str, str]) -> Dict[str, str]:
    """Helper function to map the pandas data types to the snowflake data types

    Args:
        data_type_dict (dict): Dictionary of the pandas data types
