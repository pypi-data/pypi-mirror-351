...
class Test_SolveDistributionShift(unittest.TestCase):
    ...
    def test_calculate_psychrometrics(self):
        ...
        dp_temp_new = calculate_psychrometrics(
            df, [EPW_DB_TEMP_COLUMN_NAME, EPW_RH_COLUMN_NAME]
        )
        ...
        rh_new = calculate_psychrometrics(
            df, [EPW_DB_TEMP_COLUMN_NAME, EPW_DP_TEMP_COLUMN_NAME]
        )
        ...
        self.assertAlmostEqual(
            ...
            rh_new[0],
            0,
            "relative humidity not within 0 decimal accuracy!",
        )

        epw_df = self.epw_df
        sub = deepcopy(epw_df.iloc[2000:2010, :])

        dp_temp_new10 = calculate_psychrometrics(
            sub, [EPW_DB_TEMP_COLUMN_NAME, EPW_RH_COLUMN_NAME]
        )
        rh_new10 = calculate_psychrometrics(
            sub, [EPW_DB_TEMP_COLUMN_NAME, EPW_DP_TEMP_COLUMN_NAME]
        )
        ...
        np.testing.assert_array_almost_equal(
            rh_new10,
            np.array([27.0, 27.0, 28.0, 30.0, 27.0, 27.0, 27.0, 27.0, 27.0, 28.0]),
            3,
        )
        ...
        with self.assertRaises(ValueError):
            calculate_psychrometrics(
                sub, [EPW_DB_TEMP_COLUMN_NAME, EPW_DB_TEMP_COLUMN_NAME]
            )
        # assure pressure is included
        with self.assertRaises(ValueError):
            calculate_psychrometrics(
                sub,
                [EPW_DB_TEMP_COLUMN_NAME, EPW_DP_TEMP_COLUMN_NAME, EPW_RH_COLUMN_NAME],
            )
        ...
        with self.assertRaises(ValueError):
            ...
            calculate_psychrometrics(
                pd.DataFrame(
                    [
                        {
                            EPW_DB_TEMP_COLUMN_NAME: [missing, missing, missing],
                            EPW_RH_COLUMN_NAME: [4, 5, 6],
                        }
                    ]
                ),
        ...
        with self.assertRaises(ValueError):
            ...
            calculate_psychrometrics(
                pd.DataFrame(
                    [
                        {
                            EPW_DB_TEMP_COLUMN_NAME: [max, max, max],
                            EPW_RH_COLUMN_NAME: [4, 5, 6],
                        }
                    ]
                ),
        ...
        with self.assertRaises(ValueError):
            ...
            calculate_psychrometrics(
                pd.DataFrame(
                    [
                        {
                            EPW_DB_TEMP_COLUMN_NAME: [min, min, min],
                            EPW_RH_COLUMN_NAME: [4, 5, 6],
                        }
                    ]
                ),
        ...
        ex_dict = {
            "cs": {
                ...
                "extreme_temp_normal_param": {
                    "mu": 0.061888990278578374,
                    "sig": 0.690956053333432,
                },
                "max extreme temp per duration": 1.9012564475129312,
                "min extreme temp per duration": 0.12866200452373405,
                ...
                "hourly prob stay in heat wave": 0.9623388098739797,
                "hourly prob of heat wave": 0.004296022992350157,
                ...
                "decay function coef": np.array(
                    [6.92547674e02, 9.79605373e-01, 1.10730892e03]
                ),
            },
            "hw": {
                "help": "These statistics are already mapped from -1 ... 1 and _inverse_transform_fit"
                ...
                "extreme_temp_normal_param": {
                    "mu": -0.3279009953134448,
                    "sig": 0.38427114502670034,
                },
                "max extreme temp per duration": 2.2318874862128877,
                "min extreme temp per duration": 0.3813481535873679,
                ...
                "historic time interval": 650040.0,
                "hourly prob stay in heat wave": 0.960346272307918,
                "hourly prob of heat wave": 0.005757951778158901,
                "historical durations (durations0)": (
                ...
                "decay function": "quadratic_times_exponential_decay_with_cutoff",
                "decay function coef": np.array(
                    [229.41181563, 0.98662924, 406.27566607]
                ),
            },
        }
    ...
    def test_histogram_intersection(self):
        ...
        hist1 = [
            np.array(
                [2, 2, 0, 1, 1, 3, 10, 7, 4, 10, 12, 15, 14, 21, 26, 35, 14, 13, 1, 1]
            ),
            np.array(
                [
                    -23.71429398,
