...
from mews.stats.solve import (
    SolveDistributionShift,
    duration_residuals_func,
...
class Test_SolveDistributionShift(unittest.TestCase):
    ...
    def test_calculated_hist0_stretched(self):
        ...
        if run_test:
            ...
            hist0 = {
                ...
                "cs": (
                    np.array([1, 2, 3, 4, 5, 4, 3, 2, 1, 1]),
                    np.array([-14, -13, -12, -11, -10, -9, -8, -7, -6, -5, -4]),
                ),
            }
            num_step = 2e6
            ipcc_shift = {
                "hw": {
                    "frequency": {"10 year": 5.0, "50 year": 15.0},
                    "temperature": {"10 year": 3.0, "50 year": 4.2},
                },
                "cs": {
                    "frequency": {"10 year": 0.9, "50 year": 0.8},
            ...
            for wt in ABREV_WAVE_NAMES:
                for dur in ["10 year", "50 year"]:
                    in_range = (
                        prob_vals["hw"]["actual"]["10 year"]
                        > prob_vals["hw"]["target"]["10 year"] - epsilon
                    ) and (
                        prob_vals["hw"]["actual"]["10 year"]
                        < prob_vals["hw"]["target"]["10 year"] + epsilon
                    )
            ...
            warn(
                "The test 'test_calculated_hist0_stretched' was not run because run_test=False"
            )
    ...
    def test_decay_func_types(self):
        ...
        if run_test:
            ...
            decay_func_type = [
                {
                    ...
                    "hw": "delayed_exponential_decay_with_cutoff",
                }
            ]  # [None,"linear","exponential","exponential_cutoff","quadratic_times_exponential_decay_with_cutoff"]  # 8 parameters
            ...
            for decay_func in decay_func_type:
                ...
                ipcc_shift = {
                    "hw": {
                        "frequency": {"10 year": 8.5, "50 year": 39.5},
                ...
                durations0["hw"] = (
                    np.array([50, 35, 15, 13, 10, 9, 3, 1]),
                    np.array(np.arange(12, 228, 24)),
                )
                durations0["cs"] = (
                    np.array([70, 40, 25, 15, 12, 6, 5, 2]),
    ...
    def test_single_known_case(self):
        ...
        if run_test:
            ...
            inputs = {
                "param0": {
                    "cs": {
                        "help": "These statistics are already mapped from -1 ... 1 and"
                        ...
                        + "form_fit(X,max,min)",
                        "energy_normal_param": {
                            "mu": 0.0038899912491902897,
                            "sig": 0.4154535225139539,
                        },
                        "extreme_temp_normal_param": {
                            "mu": 0.0075494672194406755,
                            "sig": 0.3399804140386147,
                        },
                        "max extreme temp per duration": 1.4307574699210692,
                        "min extreme temp per duration": 0.5585639494466237,
                        "hist max extreme temp per duration": 1.4307574699210692,
                        "hist min extreme temp per duration": 0.5585639494466237,
                        "max energy per duration": 1.3562919483136582,
                        "min energy per duration": 0.3990299129968476,
                        "energy linear slope": 0.9768230669124248,
                        "normalized extreme temp duration fit slope": 0.6946813967764691,
                        "normalized extreme temp duration fit intercept": 0.46494070822925054,
                        "normalizing energy": -3775.333333333333,
                        "normalizing extreme temp": -25.3125,
                        "normalizing duration": 384,
                        "historic time interval": 650040.0,
                        "hourly prob stay in heat wave": 0.9863164139915251,
                        "hourly prob of heat wave": 0.005069067413290484,
                        "historical durations (durations0)": (
                            np.array(
                                [53, 59, 37, 19, 9, 3, 2, 3, 1, 0, 0, 0, 0, 0, 0, 1]
                            ),
                            np.array(
                                [
                                    12.0,
                    ...
                    "hw": {
                        "help": "These statistics are already mapped from -1 ... 1 and _inverse_transform_fit is needed to return to actual degC and degC*hr values. If input of actual values is desired use transform_fit(X,max,min)",
                        "energy_normal_param": {
                            "mu": 0.07612295120388764,
                            "sig": 0.26004106669177984,
                        },
                        "extreme_temp_normal_param": {
                            "mu": -0.33020629173650035,
                            "sig": 0.2687845192334446,
                        },
                        ...
                        "hist min extreme temp per duration": 0.5661091912436009,
                        "max energy per duration": 1.8468741653468745,
                        "min energy per duration": -0.284239504158205,
                        "energy linear slope": 0.7404780352640733,
                        "normalized extreme temp duration fit slope": 0.4823740200889345,
                        "normalized extreme temp duration fit intercept": 0.45709478834728723,
                        "normalizing energy": 1648.666666666667,
                        "normalizing extreme temp": 23.157407407407405,
                        "normalizing duration": 144,
                        "historic time interval": 650040.0,
                        "hourly prob stay in heat wave": 0.9777962801504562,
                        "hourly prob of heat wave": 0.004445599228768125,
                        "historical durations (durations0)": (
                            np.array([92, 49, 14, 6, 2, 1]),
                            np.array([12.0, 36.0, 60.0, 84.0, 108.0, 132.0, 156.0]),
                        ),
                        "historical temperatures (hist0)": (
                            np.array(
                                [
                ...
                "hist0": {
                    ...
                    "hw": (
                        np.array(
                            [1, 2, 2, 11, 35, 19, 18, 19, 13, 17, 10, 8, 6, 0, 2, 0, 1]
                        ),
                        np.array(
                            [
                                6.57244009,
                ...
                "durations0": {
                    "cs": (
                        np.array([53, 59, 37, 19, 9, 3, 2, 3, 1, 0, 0, 0, 0, 0, 0, 1]),
                        np.array(
                            [
                                12.0,
                                36.0,
            ...
            obj_solve = SolveDistributionShift(
                opt_val["num_step"],
                param0,
                random_seed,
                hist0,
        ...
        if run_test:
            ...
            inputs = {
                ...
                "param0": (
                    {
                        "cs": {
                            "help": "These statistics are already mapped from -1 ... 1 and _inverse_transform_fit is needed to return to actual degC and degC*hr values. If input of actual values is desired use transform_fit(X,max,min)",
                            "energy_normal_param": {
                                "mu": -0.09637132972696814,
                                "sig": 0.33367351187294125,
                            },
                            "extreme_temp_normal_param": {
                                "mu": -0.3739023995793261,
                        ...
                        "hw": {
                            "help": "These statistics are already mapped from -1 ... 1 and _inverse_transform_fit is needed to return to actual degC and degC*hr values. If input of actual values is desired use transform_fit(X,max,min)",
                            "energy_normal_param": {
                                "mu": 0.08962260244583249,
                                "sig": 0.3761662661828063,
                            },
                            "extreme_temp_normal_param": {
                                "mu": -0.4156148791873582,
                            ...
                            "historical durations (durations0)": (
                                np.array([80, 47, 18, 6, 1, 3, 1]),
                                np.array(
                                    [12.0, 36.0, 60.0, 84.0, 108.0, 132.0, 156.0, 180.0]
                                ),
                            ),
                            "historical temperatures (hist0)": (
                                np.array(
                ...
                "hist0": {
                    ...
                    "hw": (
                        np.array(
                            [7, 19, 23, 24, 18, 19, 12, 12, 6, 8, 2, 4, 0, 0, 1, 1]
                        ),
                        np.array(
                            [
                                8.98300058,
                ...
                "durations0": {
                    ...
                    "hw": (
                        np.array([80, 47, 18, 6, 1, 3, 1]),
                        np.array([12.0, 36.0, 60.0, 84.0, 108.0, 132.0, 156.0, 180.0]),
                    ),
                },
                "delT_above_shifted_extreme": {"cs": -10, "hw": 10},
                "historic_time_interval": 650040,
    ...
    def test_known_worcester_case_June_2014_future(self):
        ...
        if run_test:
            ...
            hist0 = {
                "hw": (
                    np.array(
                        [1, 0, 0, 0, 1, 0, 10, 16, 29, 31, 25, 22, 12, 10, 9, 3, 2, 1]
                    ),
                    np.array(
                        [
                            3.36374743,
            ...
            durations0 = {
                "hw": (
                    np.array([76, 44, 24, 15, 9, 1, 2, 1]),
                    np.array(
                        [12.0, 36.0, 60.0, 84.0, 108.0, 132.0, 156.0, 180.0, 204.0]
                    ),
                ),
                "cs": (
                    np.array([79, 71, 46, 18, 8, 3, 4, 4, 0, 0, 1]),
                    np.array(
                        [
                            12.0,
                            36.0,
            ...
            param0 = {
                "hw": {
                    "help": "These statistics are already mapped from -1 ... 1 and _inverse_transform_fit is needed to return to actual degC and degC*hr values. If input of actual values is desired use transform_fit(X,max,min)",
                    "energy_normal_param": {
                        "mu": -0.10226493816492782,
                        "sig": 0.37642169697450595,
                    },
                    "extreme_temp_normal_param": {
                        "mu": 0.23483388928821727,
                    ...
                    "decay function coef": np.array(
                        [96.91670234, 0.97129716, 197.65052132]
                    ),
                },
                "cs": {
                    "help": "These statistics are already mapped from -1 ... 1 and _inverse_transform_fit is needed to return to actual degC and degC*hr values. If input of actual values is desired use transform_fit(X,max,min)",
                    "energy_normal_param": {
                        "mu": 0.012583096121127626,
                        "sig": 0.3865940662591064,
                    },
                    "extreme_temp_normal_param": {
                        "mu": 0.2071786056066263,
                    ...
                    "decay function": "quadratic_times_exponential_decay_with_cutoff",
                    "decay function coef": np.array(
                        [229.04473382, 0.9770868, 534.34596956]
                    ),
                },
            }
            ...
            opt_val = {
                ...
                "out_path": os.path.join(
                    "testing_output", "worcester_future_6_50%_2014_SSP245.png"
                ),
            }
            ...
            ipcc_shift = {
                "cs": None,
                "hw": {
                    "temperature": {"10 year": 0.982942, "50 year": 1.027711},
            ...
            inputs["out_path"] = os.path.join(
                "testing_output", "worcester_future_6_50%_2014_SSP245_ADJUSTED.png"
            )
    ...
    def test_duration_residuals_func(self):
        ...
        if run_test:
            ...
            ipcc = {
                "frequency": {"10 year": 2.0, "50 year": 39.5},
                "temperature": {"10 year": 3.0, "50 year": 6.12},
            }

            duration_too_low = (np.array([20.0, 10.0, 11.0]), np.array([1, 2, 3, 4]))

            # must give positive penalty.
            penalty = duration_residuals_func(
                duration_too_low, duration0, ipcc, np.array([1.0, 1.0, 1.0, 1.0])
            )
            ...
            penalty = duration_residuals_func(
                duration_too_high, duration0, ipcc, np.array([1.0, 1.0, 1.0, 1.0])
            )
            ...
            penalty = duration_residuals_func(
                duration0, duration0, ipcc, np.array([1.0, 1.0, 1.0, 1.0])
            )
            ...
            penalty = duration_residuals_func(
                (duration0[0] * ipcc["frequency"]["10 year"], duration0[1]),
                duration0,
                ipcc,
            ...
            # in between
            penalty = duration_residuals_func(
                (np.array([40.0, 25.1, 8.0]), duration0[1]),
                duration0,
                ipcc,
                np.array([1.0, 1.0, 1.0, 1.0]),
            )
            ...
            penalty = duration_residuals_func(
                (np.array([40.0, 25.1, 8.0, 1.0]), np.array([1, 2, 3, 4, 5])),
                duration0,
                ipcc,
                np.array([1.0, 1.0, 1.0, 1.0]),
            )
            ...
            warn(
                "Test test_duration_residuals_func was not executed because run_test was manually set to False"
            )
