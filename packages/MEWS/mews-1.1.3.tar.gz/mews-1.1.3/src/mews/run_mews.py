...
def _check_and_arrange_run_dict_inputs(run_dict):
    ...
    def _type_error(run_name, entry_name, type_check):
        raise TypeError(
            "The run name '{0}'".format(run_name)
    ...
    def _missing_required_value_error(run_name, entry_name):
        raise MEWSInputTemplateError(
            "The run name '{0}'".format(run_name)
            + " has a missing required key "
            + "'{0}'".format(entry_name)
            + " please add this required key to the inputs"
        )
    ...
    for run_name, run_val in run_dict.items():
        ...
        if TEMPLATE_ID_STRING in run_val:
            if run_val[TEMPLATE_ID_STRING] in run_dict:
                ...
                raise ValueError(
                    "INPUT TEMPLATE NAME ERROR: The {0} entry must be another key within".format(
                        TEMPLATE_ID_STRING
                    )
                    + " the mews input dictionary! The run name '{0}'".format(run_name)
                    + " has a '{0}' value of '{1}' but".format(
                        TEMPLATE_ID_STRING, run_val[TEMPLATE_ID_STRING]
                    )
        ...
        if SOLVE_OPTIONS_STRING in run_val and SOLVE_OPTIONS_STRING in run_val_template:
            if "future" not in run_val[SOLVE_OPTIONS_STRING] or (
                ...
                raise KeyError(
                    "The {0} entry must have key=['future','historic']".format(
                        SOLVE_OPTIONS_STRING
                    )
                )
            else:
                for vald in ["future", "historic"]:
                    for keyf, valf in run_val_template[SOLVE_OPTIONS_STRING][
                        vald
                    ].items():
...
def generate_epw_files(
    ...
    for file in solution_files:
        ...
        for wfile in wfiles:
            ...
            if run_parallel:
                if scenario_hw_input is None:
                    results[file] = pool.apply_async(obj.create_scenario, args=args)
                else:
    ...
    epw_file_list = [
        os.path.join(os.path.abspath(output_dir), file)
...
def extreme_temperature(
    ...
    for run_name, run_val in run_dict_modified.items():
        ...
        try:
            ...
            daily_and_norms_paths = {
                "summaries": run_val["daily_summaries_path"],
            ...
            scen_dict_name_path = os.path.join(
                sol_dir, "scen_dict_" + run_name + ".dict"
            )
            ...
            if run_name in skip_runs:
                ...
                smirnov_df = create_smirnov_table(
                    obj,
                    os.path.join(
                        sol_dir, run_name + "_kolmogorov_smirnov_test_statistic.tex"
                    ),
                )
                ...
                if troubleshoot:
                    # For Troubleshooting
                    solution_files = [
                        "final_solution_2020_SSP245_5%.txt",
                        "final_solution_2080_SSP245_50%.txt",
                    ]
                    alter_results = None
                else:
                    ...
                    alter_results, solution_files = obj.create_solutions(
                        future_years,
                        scenarios,
                        ci_intervals,
                ...
                solution_files = [
                    ...
                    if "final_solution_" in file and "historical" not in file
                ]
            ...
            if generate_epw and len(weather_files) > 0:
                epw_files, results_get = generate_epw_files(
                    obj,
                    solution_files,
                    sol_dir,
                    num_files_per_solution,
            ...
            results[run_name] = {
                "Alter objects dict": alter_results,
...
if __name__ == "__main__":
    ...
    run_dict = {
        "Chicago": {
            ...
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone5A_Chicago", "mews_epw_results"
            ),
        },
        "Phoenix": {
            "template_id": "Chicago",
            ...
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone2B_Phoenix", "mews_epw_results"
            ),
        },
        "Minneapolis": {
            "template_id": "Chicago",
            ...
            "daily_summaries_path": os.path.join(
                ...
                "example_data",
                "ClimateZone6A_Minneapolis",
                "Minneapolis_daily.csv",
            ),
            "climate_normals_path": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone6A_Minneapolis",
                "Minneapolis_norms.csv",
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
        ...
        "Miami": {
            ...
            "weather_files": [
                os.path.join(
                    ...
                    "example_data",
                    "ClimateZone1A_Miami",
                    "USA_FL_Miami-Opa.Locka.Exec.AP.722024_TMY3.epw",
                )
            ],
            "latitude_longitude": (25.90000, 360.0 - 80.28300),
            "daily_summaries_path": os.path.join(
                example_dir, "example_data", "ClimateZone1A_Miami", "Miami_daily.csv"
            ),
            "climate_normals_path": os.path.join(
                example_dir, "example_data", "ClimateZone1A_Miami", "Miami_norms.csv"
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone1A_Miami",
                "results",
                "Miami_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "ClimateZone1A_Miami",
                        "results",
                        "Miami_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone1A_Miami", "clim_scen_results"
            ),
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone1A_Miami", "mews_epw_results"
            ),
        },
        "Houston": {
            "template_id": "Chicago",
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone2A_Houston",
                "results",
                "Houston_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
            ...
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone2A_Houston", "mews_epw_results"
            ),
        },
        "Atlanta": {
            "template_id": "Chicago",
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone3A_Atlanta",
                "results",
                "Atlanta_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
            ...
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone3A_Atlanta", "mews_epw_results"
            ),
        },
        "LasVegas": {
            "template_id": "Chicago",
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone3B_LasVegas",
                "results",
                "LasVegas_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
        ...
        "LosAngeles": {
            ...
            "daily_summaries_path": os.path.join(
                ...
                "example_data",
                "ClimateZone3B_LosAngeles",
                "LosAngeles_daily.csv",
            ),
            "climate_normals_path": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone3B_LosAngeles",
                "LosAngeles_norms.csv",
            ),
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone3B_LosAngeles",
                "results",
                "LosAngeles_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
        ...
        "SanFrancisco": {
            ...
            "daily_summaries_path": os.path.join(
                ...
                "example_data",
                "ClimateZone3C_SanFrancisco",
                "SanFrancisco_daily.csv",
            ),
            "climate_normals_path": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone3C_SanFrancisco",
                "SanFrancisco_norms.csv",
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone3C_SanFrancisco",
                "results",
                "SanFrancisco_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "ClimateZone3C_SanFrancisco",
                        "results",
                        "SanFrancisco_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
        ...
        "Albuquerque": {
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone4B_Albuquerque",
                "results",
                "Albuquerque_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "ClimateZone4B_Albuquerque",
                        "results",
                        "Albuquerque_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
        ...
        "Seattle": {
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone4C_Seattle",
                "results",
                "Seattle_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
            ...
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone4C_Seattle", "mews_epw_results"
            ),
        },
        "Denver": {
            "template_id": "Chicago",
            ...
            "daily_summaries_path": os.path.join(
                ...
                "example_data",
                "ClimateZone5B_Denver",
                "Denver_Cheesman_daily.csv",
            ),
            "climate_normals_path": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone5B_Denver",
                "Denver_AURORA_norms.csv",
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone5B_Denver",
                "results",
                "Denver_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "ClimateZone5B_Denver",
                        "results",
                        "Denver_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone5B_Denver", "clim_scen_results"
            ),
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone5B_Denver", "mews_epw_results"
            ),
        },
        "Helena": {
            "template_id": "Chicago",
            "weather_files": [
                os.path.join(
                    example_dir,
                    "example_data",
                    "ClimateZone6B_Helena",
                    "USA_MT_Helena.Rgnl.AP.727720_TMY3.epw",
                )
            ],
            "latitude_longitude": (46.60000, 360.0 - 111.9670),
            "daily_summaries_path": os.path.join(
                example_dir, "example_data", "ClimateZone6B_Helena", "Helena_daily.csv"
            ),
            "climate_normals_path": os.path.join(
                example_dir, "example_data", "ClimateZone6B_Helena", "Helena_norms.csv"
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone6B_Helena",
                "results",
                "Helena_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "ClimateZone6B_Helena",
                        "results",
                        "Helena_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone6B_Helena", "clim_scen_results"
            ),
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone6B_Helena", "mews_epw_results"
            ),
        },
        "Duluth": {
            "template_id": "Chicago",
            "weather_files": [
                os.path.join(
                    ...
                    "example_data",
                    "ClimateZone7_Duluth",
                    "USA_MN_Duluth.Intl.AP-Duluth.ANGB.727450_TMY3.epw",
                )
            ],
            "latitude_longitude": (46.83300, 360.0 - 92.21700),
            "daily_summaries_path": os.path.join(
                example_dir, "example_data", "ClimateZone7_Duluth", "Duluth_daily.csv"
            ),
            "climate_normals_path": os.path.join(
                example_dir, "example_data", "ClimateZone7_Duluth", "Duluth_norms.csv"
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "ClimateZone7_Duluth",
                "results",
                "Duluth_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "ClimateZone7_Duluth",
                        "results",
                        "Duluth_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone7_Duluth", "clim_scen_results"
            ),
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone7_Duluth", "mews_epw_results"
            ),
        },
        "Fairbanks": {
            "template_id": "Chicago",
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone8_Fairbanks",
                "results",
                "Fairbanks_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
        ...
        "McAllen": {
            ...
            "historic_solution_save_location": os.path.join(
                ...
                "ClimateZone2A_McAllen",
                "results",
                "McAllen_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
            ...
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "ClimateZone2A_McAllen", "mews_epw_results"
            ),
        },
        "Kodiak": {
            "template_id": "Chicago",
            "weather_files": [
                os.path.join(
                    ...
                    "example_data",
                    "Kodiak",
                    "USA_AK_Kodiak.AP.703500_TMY3.epw",
                )
            ],
            "latitude_longitude": (57.741461, 360.0 - 152.5032979),
            "daily_summaries_path": os.path.join(
                example_dir, "example_data", "Kodiak", "Kodiak.csv"
            ),
            "climate_normals_path": os.path.join(
                example_dir, "example_data", "Kodiak", "Kodiak_norms.csv"
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "Kodiak",
                "results",
                "Kodiak_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "Kodiak",
                        "results",
                        "Kodiak_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
                example_dir, "example_data", "Kodiak", "clim_scen_results"
            ),
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "Kodiak", "mews_epw_results"
            ),
        },
        "Worcester": {
            "template_id": "Chicago",
            ...
            "daily_summaries_path": os.path.join(
                example_dir, "example_data", "Worcester", "USW00094746.csv"
            ),
            "climate_normals_path": os.path.join(
                example_dir, "example_data", "Worcester", "USW00094746_norms.csv"
            ),
            "historic_solution_save_location": os.path.join(
                example_dir,
                "example_data",
                "Worcester",
                "results",
                "Worcester_airport_historical_solution.txt",
            ),
            "solve_options": {
                "historic": {
                    "out_path": os.path.join(
                ...
                "future": {
                    "out_path": os.path.join(
                        ...
                        "Worcester",
                        "results",
                        "Worcester_airport_future.png",
                    )
                },
            },
            "clim_scen_out_folder": os.path.join(
                example_dir, "example_data", "Worcester", "clim_scen_results"
            ),
            "epw_out_folder": os.path.join(
                example_dir, "example_data", "Worcester", "mews_epw_results"
            ),
        },
    }
    ...
    pkl.dump(
        [results],
        open(os.path.join(example_dir, "example_data", "STBE_study_results.pkl"), "wb"),
    )
