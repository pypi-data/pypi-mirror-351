...
class ClimateScenario(object):
    ...
    _old_scenario_names_key = {
        _valid_scenario_names[1]: "SSP1-1.9",
    ...
    def __init__(
        ...
        self.scenarios = []
        rlist = [
            r"$t^{0:d}$".format(self._max_poly_order - idx - 1)
            for idx in range(self._max_poly_order - 1)
        ]
        rlist.append(r"$1$")
        rlist.append(r"$R^2$")
        self.columns = rlist
    ...
    def calculate_coef(self, scenario, lat=None, lon=None, scen_func_dict=None):
        ...
        if not self.use_global:
            ...
            if not isinstance(scenario, list):
                raise TypeError(
                    "For 'self.use_global=True' the scenario input"
            ...
            self.cmip = {}
            self.p_coef = {}
            max_poly_order = np.array(
                [val for key, val in self._polynomial_order.items()]
            ).max()
            ...
            for scen, cmip_data_obj in obj.total_model_data.items():
                ...
                self.table.append(
                    np.concatenate(
                        (
                            np.zeros(max_poly_order - len(polycoef)),
                            polycoef,
                            np.array([cmip_data_obj.R2]),
                        )
                    )
                )
            ...
            df = self.df_temp_anomaly[
                self.df_temp_anomaly["Climate Scenario"] == old_scen_name
            ]
            ...
            p_coef, residuals, rank, singular_values, rcond = np.polyfit(
                Xi, Yi, 3, full=True
            )
            self.p_coef = p_coef
    ...
    def _load_old_global_temps(self):
        ...
        self.df_temp_anomaly = pd.read_csv(
            os.path.join(
                file_dir,
                "data",
    ...
    def _check_valid_scenario_name(self, scenario):
        if not scenario in self._valid_scenario_names:
            raise ValueError(
                "An invalid scenario name '{0}' was entered".format(scenario)
                + "\n\n Valid names are:\n\n"
                + str(self._valid_scenario_names)
            )
    ...
    def write_coef_to_table(self, tex_file_path):
        table_df = pd.DataFrame(self.table, index=self.scenarios, columns=self.columns)
        table_df.to_latex(tex_file_path, float_format="%.6G", escape=False)
