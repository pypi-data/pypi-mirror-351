...
def pdf_density_given_value(x, pdf):
    ...
    idi = np.array(
        [
            np.where(np.logical_and(bins[0:-1] <= binx, bins[1:] > binx))[0][0]
            for binx in x
        ]
    )
    ...
    interp = np.array(
        [
            (
                bins[idix]
                + (Pr - prob[idix])
                / (prob[idix + 1] - prob[idix])
                * (bins[idix + 1] - bins[idix])
                if idix < len_prob
                else bins[-1]
            )
...
def offset_cdf_truncnorm_newton(x, mu, sig, a, b, rnd):
    ...
    if x < a:
        ...
        fprime = (
            (2 / np.sqrt(np.pi)) * np.exp(-((xi) ** 2)) / (sig * (erf_beta - erf_alpha))
        )
...
def trunc_norm_dist(rnd, mu, sig, a, b, minval, maxval):
    ...
    # else:
    x, r = bisect(
        offset_cdf_truncnorm, a, b, args=(mu, sig, a, b, rnd), full_output=True
    )
...
def fit_exponential_distribution(month_duration_hr, include_plots):
    ...
    month_dur_histogram = np.histogram(
        month_duration_hr,
        num_bin,
        range=(
            month_duration_hr.min() - hour_in_day / 2,
    ...
    if include_plots:
        plt.plot(
            bin_avg,
            hist_norm,
            np.arange(0, month_duration_hr.max()),
            cdf_exponential(np.arange(0, month_duration_hr.max()), lamb[0]),
        )
