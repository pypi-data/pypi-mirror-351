...
from mews.utilities.utilities import (
    linear_interp_discreet_func,
    bin_avg,
...
from mews.utilities.utilities import (
    find_extreme_intervals,
    create_complementary_histogram,
    dict_key_equal,
)
...
def unpack_params(param, wave_type):
    return {
        "norm_duration": param[wave_type]["normalizing duration"],
...
def evaluate_temperature(durations, param, del_mu_T, del_sig_T, rng, wave_type):
    ...
    min_boundary = transform_fit(
        pval["minval_T"], pval["hist_minval_T"], pval["hist_maxval_T"]
    )
    max_boundary = transform_fit(
        pval["maxval_T"], pval["hist_minval_T"], pval["hist_maxval_T"]
    )

    # TODO - this needs to get vectorized or you need a cython root finder
    # to do the heavy lifting on this.
    T_per_durations = np.array(
        [
            trunc_norm_dist(
                rand,
                pval["mu_T"] + del_mu_T,
            ...
            for rand in rng.random(len(durations))
        ]
    )
...
def calculated_hist0_stretched(
    hist0, ipcc_shift, historic_time_interval, hours_per_year, plot_results=False
):
    """
    ...
    def objective_function(x, param):
        ...
        norm_area = histogram_step_wise_integral(hist0, hist0[1].min(), hist0[1].max())
        ...
        derivative_vals = (
            4 * x[0] * bin_**3 + 3 * x[1] * bin_**2 + 2 * x[2] * bin_**2 + x[3]
        )
        ...
        if wt == "hw":
            P10_actual = histogram_step_wise_integral(pdf_new, bin_[0], T10_shifted)
            P50_actual = histogram_step_wise_integral(pdf_new, bin_[0], T50_shifted)
        elif wt == "cs":
            P10_actual = histogram_step_wise_integral(pdf_new, bin_[0], T10_shifted)
            P50_actual = histogram_step_wise_integral(pdf_new, bin_[0], T50_shifted)
        else:
            raise ValueError(
                "MEWS only handles 'cs', and 'hw' for wave types presently!"
            )
        ...
        if wt == "hw":
            ...
            residual2 = (
                20**2 * (P10 - P10_actual) ** 2 + 200**2 * (P50 - P50_actual) ** 2
            )
    ...
    for wt, hist0_wt in hist0.items():
        ...
        if not ipcc_shift[wt] is None:
            ...
            cdf0 = (
                (hist0_wt[0] / hist0_wt[0].sum()).cumsum(),
                (hist0_wt[1][1:] + hist0_wt[1][0:-1]) / 2,
            )

            (
                P0_less_than_extreme,
                P_T_target_less_than_extreme,
                P10_given_wave,
                P50_given_wave,
            ...
            Tthresh_historic = linear_interp_discreet_func(P0_less_than_extreme, cdf0)

            # These are the exact values needed
            T_shifted_target = Tthresh_historic + np.array(
                [
                    ipcc_shift[wt]["temperature"]["10 year"],
                    ipcc_shift[wt]["temperature"]["50 year"],
                ]
            )

            param = [
                P_T_target_less_than_extreme[0],
            ...
            if Asol.fun > 1e-5:
                warn(
                    "A perfect solution for shifting the temperature "
            ...
            if plot_results:
                pdf_norm_area = histogram_step_wise_integral(
                    hist0[wt], hist0[wt][1].min(), hist0[wt][1].max()
                )
            ...
            prob_vals[wt] = {
                "actual": {"10 year": P10_actual, "50 year": P50_actual},
                "target": {
                    "10 year": P_T_target_less_than_extreme[0],
                    "50 year": P_T_target_less_than_extreme[1],
                },
            }
            param_val[wt] = param
            asol_val[wt] = Asol
...
def calculate_probabilities(
    num_hist_wave, historic_time_interval, hours_per_year, ipcc_shift, wave_type
):
    ...
    if wave_type == "cs":
        ...
        P_T_target_less_than_extreme = np.array(
            [
                P10_given_wave * ipcc_shift["frequency"]["10 year"],
        ...
        P_T_target_less_than_extreme = np.array(
            [
                1.0 - P10_given_wave * ipcc_shift["frequency"]["10 year"],
...
def ipcc_shift_comparison_residuals(
    hist0,
    ipcc_shift,
    histT_tuple,
    durations,
    ...
    if wave_type == "hw":
        negative_penalty = (
            np.max([float((cdf_T[1] < 0).sum()) - 1.0, 0.0])
        ) ** 2 * weights[2]
    else:
        negative_penalty = (
            np.max([float((cdf_T[1] > 0).sum()) - 1.0, 0.0])
    ...
    if len(cdf_T[0]) == 1:
        T_shifted_actual = (
            np.diff(histT_tuple[0][1])[0] * P0_less_than_extreme + histT_tuple[0][1][0]
        )
    else:
    ...
    # These are the exact values needed
    T_shifted_target = Tthresh_historic + np.array(
        [ipcc_shift["temperature"]["10 year"], ipcc_shift["temperature"]["50 year"]]
    )
    ...
    if wave_type == "hw":
        positive_penalty = (
            (
                histT_tuple[0][0][
                    cdf_T[1] > (T_shifted_target[1] + delT_above_shifted_extreme)
                ]
            ).sum()
        ) ** 0.5 * weights[1]
    else:
    ...
    if not hist0_stretched is None:
        ...
        shape_penalty = histogram_comparison_residuals(
            histT_tuple[0], hist0_stretched, weights[4], normalize=True
        )
    else:

        shape_penalty = histogram_comparison_residuals(
            histT_tuple[0], hist0, weights[4], normalize=True
        )

    residuals = (
        (T_shifted_actual - T_shifted_target) / T_shifted_target
    ) ** 2 * weights[0]

    if output_hist:
        return (
            residuals + negative_penalty + positive_penalty,
...
def probability_of_extreme_10_50(num_hist_wave, historic_time_interval, hours_per_year):
    ...
    if num_hist_wave == 0:
        ...
        P10_given_wave = (
            historic_time_interval
            * (hours_per_year / num_hour_in_year)
            / (num_hist_wave * N10)
        )
        P50_given_wave = (
            historic_time_interval
...
def duration_residuals_func(duration, duration0, ipcc_shift, weights):
    ...
    def bounding_penalty(
        duration0, duration, ipcc_shift, normalizing_factor, is_increase
    ):

        bound_duration = (
            duration0[0] * ipcc_shift["frequency"]["10 year"],
            duration0[1],
        )
    ...
    normalizing_factor = 0.5 * np.min(
        [
            duration[0].max() * (duration[1].max() - duration[1].min()),
    ...
    if match:
        ...
        residuals = histogram_comparison_residuals(duration, duration0, 1.0, False) / (
            normalizing_factor
        )
        ...
        penalty += histogram_non_overlapping(duration0, duration) / normalizing_factor
    ...
    for eid, wt in enumerate(events):
        ...
        if not decay_func is None:
            ...
            if decay_func == "exponential" or decay_func == "linear":
                ...
                raise ValueError(
                    "Invalid decay_func_type. '{0}' was input, but only valid values are : \n\n{1}".format(
                        decay_func,
                        str(SolveDistributionShift._decay_func_types_entries.keys()),
                    )
                )
        else:
            coef[wt] = None
...
class ObjectiveFunction:
    ...
    def markov_gaussian_model_for_peak_temperature(
        self,
        x,
        num_step,
        hist_param,
        ...
        tran_matrix = np.array(
            [[1 - Pcs - Phw, Pcs, Phw], [1 - Pcss, Pcss, 0.0], [1 - Phws, 0.0, Phws]]
        )
        ...
        for wave_type, state_int in zip(ABREV_WAVE_NAMES, [1, 2]):
            ...
            Tsample[wave_type] = evaluate_temperature(
                durations[wave_type],
                hist_param,
                del_mu_T[wave_type],
                del_sig_T[wave_type],
            ...
            if len(Tsample[wave_type]) < min_num_waves:
                ...
                duration_normalizing_factor = (
                    historic_time_interval * hours_per_year / self._hours_in_year
                ) / num_step

                unnormalized_duration_histograms[wave_type] = np.histogram(
                    durations[wave_type],
                    bins=np.arange(24 - 12, durations[wave_type].max() + 12 + 24, 24),
                )

                duration_histograms[wave_type] = (
                    unnormalized_duration_histograms[wave_type][0]
                    * duration_normalizing_factor,
                ...
                if duration_histograms[wave_type][0].sum() > min_num_waves:
                    ...
                    duration_residuals[wave_type] = duration_residuals_func(
                        duration_histograms[wave_type],
                        durations0[wave_type],
                        ipcc_shift[wave_type],
                        weights,
                    )

                else:
                    # this means very few waves are coming through and a statistical analysis is invalid. Huge Penalty
                    duration_residuals[wave_type] = (
                        1e5 / duration_histograms[wave_type][0].sum()
                    )
                ...
                if ipcc_shift[wave_type] is None:
                    ...
                    if output_hist:
                        (
                            ...
                            temp_resid[wave_type],
                            negative_penalty[wave_type],
                            positive_penalty[wave_type],
                        ) = ipcc_shift_comparison_residuals(
                            hist0[wave_type],
                            ipcc_shift[wave_type],
                            histT_tuple[wave_type],
                            durations[wave_type],
                            output_hist,
                            delT_above_shifted_extreme[wave_type],
                            num_hist_wave,
                            historic_time_interval,
                            hours_per_year,
                            wave_type,
                            weights,
                            hist0_stretched[wave_type],
                        )
                    else:
                        residuals[wave_type] = ipcc_shift_comparison_residuals(
                            hist0[wave_type],
                            ipcc_shift[wave_type],
                            histT_tuple[wave_type],
                            durations[wave_type],
        ...
        if output_hist:
            residuals_dict = {}
            residuals_dict["total temperature"] = {
                "cs": residuals["cs"].sum(),
                "hw": residuals["hw"].sum(),
            }
...
class SolveDistributionShift(object):
    ...
    _ipcc_shift_struct = """
                        ...
                        """
    _ipcc_shift_dict_str = {
        "temperature": ["10 year", "50 year"],
        "frequency": ["10 year", "50 year"],
    }
    ...
    _default_problem_bounds_all = {
        ABREV_WAVE_NAMES[0]: {
            _bound_desc[0]: (0.0, 2.0),
            _bound_desc[1]: (-0.1, 4),
    ...
    _decay_func_types_entries = {
        DECAY_FUNC_NAMES[0]: _bound_desc[0:4],
    ...
    def __init__(
        ...
        for wtype in self._events:
            if ipcc_shift[wtype] is None:
                abs_max_temp[wtype] = (
                    param0[wtype]["normalizing extreme temp"]
                    + delT_above_shifted_extreme[wtype]
                )  # this parameter
            else:
                abs_max_temp[wtype] = (
                    param0[wtype]["normalizing extreme temp"]
                    + ipcc_shift[wtype]["temperature"]["50 year"]
                    + delT_above_shifted_extreme[wtype]
                )
        ...
        bounds_x0, linear_constraint, nonlinear_constraint = self._problem_bounds(
            decay_func_type, num_step, param0, abs_max_temp, limit_temperatures
        )
        ...
        if x_solution is None:
            ...
            optimize_result = differential_evolution(
                obj_func.markov_gaussian_model_for_peak_temperature,
                bounds_x0,
                args=(
                    num_step,
                    param0,
                ...
                disp=False,
                polish=False,
            )  # polishing is a waste of time on a stochastic function.
        ...
        for rand_adj in np.arange(1, num_postprocess + 1):
            (
                ...
                durations[rand_adj],
                thresholds[rand_adj],
                histT_tuple[rand_adj],
                residuals_dict[rand_adj],
                Dsample[rand_adj],
            ) = obj_func.markov_gaussian_model_for_peak_temperature(
                xf0,
                num_step,
            ...
            if not org_samples["Temperature"] is None:
                ...
                ks_duration[rand_adj] = {
                    "hw": kolmogorov_smirnov_test(
                        Dsample[rand_adj]["hw"],
                        org_samples["Duration"]["hw"],
                        alternative="two-sided",
                        mode="exact",
                    ),
                    "cs": kolmogorov_smirnov_test(
                        Dsample[rand_adj]["cs"],
        ...
        if not org_samples["Temperature"] is None:
            self.kolmogorov_smirnov = {
                "Temperature": ks_temperature,
                "Duration": ks_duration,
            }
        else:
        ...
        for wave_type, eid, idx in zip(
            self._events, [[0, 1, 4, 6], [2, 3, 5, 7]], [0, 1]
        ):
            ...
            del_mu_T = xf0[eid[0]]
            del_sig_T = xf0[eid[1]]
            param_new[wave_type]["extreme_temp_normal_param"]["mu"] = (
                pval["mu_T"] + del_mu_T
            )
            param_new[wave_type]["extreme_temp_normal_param"]["sig"] = (
                pval["sig_T"] + del_sig_T
            )
            ...
            param_new[wave_type]["min extreme temp per duration"] = (
                inverse_transform_fit(
                    min_boundary + del_a, pval["maxval_T"], pval["minval_T"]
                )
            )

            param_new[wave_type]["max extreme temp per duration"] = (
                inverse_transform_fit(
                    max_boundary + del_b, pval["maxval_T"], pval["minval_T"]
                )
            )
    ...
    def reanalyze(self, inputs, write_csv=False):
        ...
        for key, val in inputs.items():
            if key in self.inputs:
                ...
                raise ValueError(
                    "SolveDistributionShift.reanalyze: An input option {0} was given but that is not a valid input. \n".format(
                        key
                    )
    ...
    def _check_ipcc_shift(self, ipcc_shift, key, ukey):
        if not key in ipcc_shift:
            raise ValueError(
                "The input 'ipcc_shift['{0}']' must be a dictionary with key '{1}'".format(
                    ukey, key
                )
            )
        else:
            for yr_str in ["10 year", "50 year"]:
                if not yr_str in ipcc_shift[key]:
                    raise ValueError(
                        "The input subdictionary 'ipcc_shift[{0}] must have include a key '{1}'".format(
                            key, yr_str
                        )
                    )
                ...
                if not isinstance(ipcc_shift[key][yr_str], (int, float)):
                    raise TypeError(
                        "The input subdictionary 'ipcc_shift['{0}']['{1}'] must be a positive number".format(
                            key, yr_str
                        )
                    )
    ...
    def _check_inputs(
        ...
        if not isinstance(random_seed, (type(None), int)):
            ...
            raise TypeError(
                "The input 'durations0' must be dict with key {0}.".format(
                    str(self._events)
                )
            )
        elif not isinstance(ipcc_shift, dict):
            raise TypeError(
                "The ipcc_shift input must be a dictionary with key {0}.".format(
                    str(self._events)
                )
            )
        elif not isinstance(decay_func_type, dict):
            raise TypeError(
                "The input 'decay_func_type' must be a dictionary with keys = {0} and values from the list = {1}".format(
            ...
            raise TypeError(
                "the input 'delT_above_shifted_extreme' must be a dictionary with key = {0}.".format(
                    str(self._events)
                )
            )
        elif not isinstance(historic_time_interval, int):
            raise TypeError(
                "The input 'historic_time_interval' must be an integer equal to 24 * the number of days in the NOAA daily summaries for the month being analyzed (or all days if entire year is being lumped)."
            )
        ...
        if min_num_waves < 10:
            raise ValueError(
                "The analysis being done is statistical. The minimum"
        ...
        if num_postprocess < 1 or num_postprocess > 100:
            raise ValueError("The input 'num_postprocess' must be between 1 and 100!")

        if hours_per_year > HOURS_IN_LEAP_YEAR or hours_per_year < 0:
            raise ValueError(
                "The input 'hours_per_year' must be between 0 to"
        ...
        if len(weights) != 5:
            raise ValueError("The input 'weights' must have 5 elements.")
        ...
        for wt, hist0_ in hist0.items():
            ...
            if not isinstance(hist0_, tuple):
                raise TypeError(
                    "The input hist0['{0}'] must be a tuple output from np.histogram!".format(
                        wt
                    )
                )
        for wt, durations0_ in durations0.items():
            if not wt in self._events:
                raise ValueError(
                    "The only valid keys for 'durations0' are {0}.".format(
                        str(self._events)
                    )
                )
            if not isinstance(durations0_, tuple):
                raise TypeError(
                    "The input durations0['{0}'] must be a tuple output from np.histogram!".format(
                        wt
                    )
                )
        ...
        for wt, delT_ in delT_above_shifted_extreme.items():
            if not wt in self._events:
                raise ValueError(
                    "The only valid keys for 'delT_above_shifted_extreme' are {0}.".format(
                        str(self._events)
                    )
                )
            if not isinstance(delT_, (int, float)):
                raise TypeError(
                    "the input 'delT_above_shifted_extreme['{0}']' must be a number > 0".format(
                        wt
                    )
                )
            ...
            if wt == "hw" and (delT_ < 0 or delT_ > 20.0):
                raise ValueError(
                    "The input 'delT_above_shifted_extreme['{0}'] is in degrees Celcius and must be between 0 and 20.".format(
                        wt
                    )
                )
            elif wt == "cs" and (delT_ > 0 or delT_ < -20.0):
                raise ValueError(
                    "The input 'delT_above_shifted_extreme['{0}'] is in degrees Celcius and must be between -20 and 0.".format(
                        wt
                    )
                )
        ...
        for wt, decay_func in decay_func_type.items():
            if not wt in self._events:
                raise ValueError(
                    "The only valid keys for 'decay_func_type' are {0}.".format(
                        str(self._events)
                    )
                )
            if not decay_func in self._decay_func_types_entries:
                raise ValueError(
                    "The input 'decay_func_type' must be one of the following "
                    + str(self._decay_func_types_entries)
                )
        ...
        for wt, pb in problem_bounds.items():
            if not wt in self._events:
                raise ValueError(
                    "The problem bounds must have entries for both heat waves and cold snaps ({0})".format(
                        str(self._events)
                    )
                )
            else:
                ...
                for entry in entries_needed:
                    if not entry in pb:
                        ...
                        raise ValueError(
                            ...
                            + func_descr_str
                            + " must have {0:d} variables.".format(numvar)
        ...
        if historic_time_interval < 0:
            ...
            warn(
                "The historic time interval is less than 20 years. This is not advised!",
                UserWarning,
            )
    ...
    def _problem_bounds(
        self, decay_func_type, num_step, param0, abs_max_temp, limit_temperatures
    ):
        ...
        bounds_x0 = [
            dpb["cs"]["delT_mu"],
            (
                dpb["cs"]["delT_sig multipliers"][0]
                * param0["cs"]["extreme_temp_normal_param"][
                    "sig"
                ],  # narrowing of standard deviation is not expected.
                dpb["cs"]["delT_sig multipliers"][1]
                * param0["cs"]["extreme_temp_normal_param"]["sig"],
            ),
            dpb["hw"]["delT_mu"],
            (
                dpb["hw"]["delT_sig multipliers"][0]
                * param0["hw"]["extreme_temp_normal_param"]["sig"],
                dpb["hw"]["delT_sig multipliers"][1]
                * param0["hw"]["extreme_temp_normal_param"]["sig"],
            ),
            dpb["cs"]["P_event"],
            dpb["hw"]["P_event"],
        ...
        for wt in self._events:
            ...
            if not decay_func is None:
                ...
                if (
                    decay_func == DECAY_FUNC_NAMES[1]
                    or decay_func == DECAY_FUNC_NAMES[2]
                ):
                    bounds_x0.append(
                        tuple(
                            np.array(dpb[wt]["slope or exponent multipliers"])
                            / max_duration
                        )
                    )
                    num_added = 1
                    ...
                    decay_func == DECAY_FUNC_NAMES[3]
                    or decay_func == DECAY_FUNC_NAMES[4]
                ):
                    bounds_x0.append(
                        tuple(
                            np.array(dpb[wt]["slope or exponent multipliers"])
                    ...
                    bounds_x0.append(
                        tuple(
                            max_duration * np.array(dpb[wt]["cutoff time multipliers"])
                        )
                    )
                    num_added = 2
                    ...
                    bounds_x0.append(
                        tuple(
                            max_duration
                            * np.array(dpb[wt]["multipliers to max probability time"])
                        )
                    )
                    bounds_x0.append(
                    ...
                    raise ValueError(
                        "Invalid decay function type '{0}': only valid values are: \n\n{1}".format(
                            decay_func, str(self._decay_func_types_entries.keys())
                        )
                    )

                constrain_matrix = np.concatenate(
                    [constrain_matrix, np.zeros((1, num_added))], axis=1
                )
        ...
        lc = LinearConstraint(
            constrain_matrix,
            np.array([1 / (num_step / 25)]),
            np.array([1.0 - 1 / (num_step / 25)]),
        )
    ...
    def _grab_relevant_bounds(self, decay_func_type, problem_bounds):
        ...
        for wv, subdict in allb.items():
            ...
            if wv in decay_func_type:
                ...
                raise ValueError(
                    "The decay_func_type input must be a dictionary over all wave names: "
                    + str(ABREV_WAVE_NAMES)
                )
            ...
            if not problem_bounds is None:
                if wv in problem_bounds:
                    ...
                    raise ValueError(
                        "The problem_bounds input must be a dictionary over all wave names:"
                        + str(ABREV_WAVE_NAMES)
                    )
            else:
                pbs = {}
                ...
                if not key in self._decay_func_types_entries[decay_name]:
                    raise ValueError(
                        "\n\nThe problem bounds for {0} type decay functions does not include '{2}' only contains the following entries: {1}".format(
                            decay_name,
                            str(self._decay_func_types_entries[decay_name]),
                            key,
                        )
                    )
        return new_dict
...
class MaxTemperatureNonlinearConstraint:
    ...
    def func(self, x):
        ...
        Xb = inverse_transform_fit(
            min_boundary + del_b, pval["maxval_T"], pval["minval_T"]
        )
        Xa = inverse_transform_fit(
            max_boundary + del_a, pval["maxval_T"], pval["minval_T"]
        )
