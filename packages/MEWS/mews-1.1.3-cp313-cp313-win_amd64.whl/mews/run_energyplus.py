...
class RunEnergyPlusStudy(object):
    ...
    def __init__(
        ...
        idf_list = [
            os.path.join(idfs_path, file)
            for file in os.listdir(idfs_path)
            if file[-4:] == ".idf"
        ]
        epw_list = [
            os.path.join(weather_files_path, file)
            for file in os.listdir(weather_files_path)
            if file[-4:] == ".epw"
        ]
        ...
        for idf in idf_list:
            ...
            for epw in epw_list:
                ...
                if run_parallel:
                    ...
                    results[tup] = pool.apply_async(self._run, args=tup)
                else:
    ...
    def _setup_parallel(self, num_cpu):
        try:
            ...
            if too_many_requested:
                warnings.warn(
                    "The number of CPU's requested was '{0:d}' but only '{1:d}' were available!".format(
    ...
    def _import_energyplus_api(self):
        ...
        try:
            ...
            warnings.warn(
                "The energy plus api failed to import! The "
            ...
            try:
                if execute_run:
                    if self.use_python_api:
                        ...
                        ep_obj.runtime.run_energyplus(
                            state, ["-d", out_path, "-w", wfile, idf]
                        )
                    else:
                        # Now let's run EP
                        run_succeeded = self._execute_command(
                            [self.energyplus_path, "-d", out_path, "-w", wfile, idf]
                        )
                ...
                warnings.warn(
                    "Energy plus failed to run successfully! Path=" + out_path
                )
                return excep
