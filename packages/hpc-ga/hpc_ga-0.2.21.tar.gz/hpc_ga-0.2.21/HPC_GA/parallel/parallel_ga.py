...
class IslandModel(ParallelGA):
    def __init__(self, ga_class: Type, ga_config: Dict[str, Any], parallel_config: Dict[str, Any]):
        ...
        self.islands: List[Any] = []  # List[IslandActor] but Ray doesn't support it
        self._setup_islands()
    ...
    def run(self, generations: int) -> Chromosome:
        ...
        for gen in range(generations):
            # Run generation on all islands
            self.islands = [island.run_generation.remote() for island in self.islands]
            ray.get(self.islands)  # Wait for completion
    ...
    def _migrate(self) -> None:
        ...
        migration_size = min(
            ...
            len(self.initial_population.individuals) // len(self.islands)
        )
        n = len(self.islands)
