...
class PsonoAPIHelper:
    ...
    def _index_datastore_item_paths(datastore: PsonoDataStore):
        __class__._search_folder(search_type='pathindex')
    ...
    def create_folder(datastore: PsonoDataStore,foldername) -> PsonoDataStoreFolder:
        ...
        return newfolder
    @staticmethod
    def get_metadata_key_for_id(secret_key_id: str,datastores: List[PsonoDataStore]):
        for datastore in datastores:
            secret_metadata = __class__._search_folder('secret_id',datastore,secret_key_id)
            if secret_metadata is not None:
                return secret_metadata

        raise PsonoSecretNotFoundException(f"Cannot find secret with ID \"{secret_key_id}\" in any path")
    ...
    def _search_folder(search_type: str,folder: PsonoDataStoreFolder,search_item=None) -> Union[PsonoDataStoreItem,List[PsonoDataStoreItem],None]:
        ...
        for subfolder in folder.folders:
            subfolder.path = folder.path + '/' + folder.name
            subfolder.fullpath = folder.fullpath + '/' + folder.name
        ...
        for  itemdata in folder.items:
            ...
            itemdata.fullpath = folder.fullpath + '/' + itemdata.name
            if itemdata.deleted:
                continue
