...
#         if not self.loaded_2D or self.ignore_gui_call:
#             return
...
#         for chn, channel_name in enumerate(channel_names):
#             if not str(channel_name) in [str(layer.name) for layer in self.viewer.layers]:
#                 continue

#             if chn not in self.selected_channels:
#                 continue

#             # Fetch new data for this channel
#             try:
#                 im1 = (self.data.ds1[slice_name].sel(type='mosaic', z=optical_slice).data[chn] * self.bscale + self.bzero).squeeze()
#                 im2 = (self.data.ds2[slice_name].sel(type='mosaic', z=optical_slice).data[chn] * self.bscale + self.bzero).squeeze()
#                 im4 = (self.data.ds4[slice_name].sel(type='mosaic', z=optical_slice).data[chn] * self.bscale + self.bzero).squeeze()
...
#                         im32 = (self.data.ds32[slice_name].data[chn] * self.bscale + self.bzero).squeeze()
#                     except Exception:
#                         continue  # Can't load this channel, skip

#             new_data = [im1, im2, im4, im8, im16, im32]

#             # Get the current layer object (save params, visible before deleting!)
#             layer = self.viewer.layers[str(channel_name)]
#             layer_params = {
#                 "scale": layer.scale,
...
#             # Disconnect layer signals to help GC (napari can have Qt signal chains)
#             try:
#                 layer.events.disconnect()  # Disconnect all signals if possible
#             except Exception:
#                 pass

#             # Remove the layer with undo/redo disabled for efficiency (if possible)
#             try:
#                 self.viewer.layers.remove(str(channel_name), undo_redo=False)
#             except Exception:
#                 self.viewer.layers.remove(str(channel_name))

#             # Break references on the old layer to help Python's GC
#             try:
#                 layer.data = None
#             except Exception:
#                 pass
...
#             # Add new layer with preserved settings
#             new_layer = self.viewer.add_image(
#                 new_data,
#                 name=channel_name,
#                 scale=layer_params["scale"],
#                 translate=layer_params["translate"],
#                 opacity=layer_params["opacity"],
#                 colormap=layer_params["colormap"],
#                 contrast_limits=layer_params["contrast_limits"],
#                 blending=layer_params["blending"],
#             )
#             new_layer.visible = layer_params["visible"]

#             # Remove references to large arrays ASAP
#             del im1, im2, im4, im8, im16, im32, new_data, new_layer

#             gc.collect()

#         # Final sweep at end, just in case
#         gc.collect()
...
#             tifffile.imsave(file_name, volume)
    ...
    def Load3D(self, text):
        ...
        result = self.data.Load3D(self.viewer, self.image_folder, self.comboBoxPath.currentText(), self.selected_channels, self.default_contrast_limits, self.thresholdN, self.channels_start, self.axio, self.old_method, self.overall_brightness, self.scroll, self.scroll_overall_brightness, self.pixel_size.text(), self.m_slice_spacing.text(), self.start_slice.text(), self.end_slice.text(), self.crop, self.crop_start_x, self.crop_end_x, self.crop_start_y, self.crop_end_y, self.origin_x, self.origin_y)       
    ...
    def main(self):
        ...
        self.viewer.window._qt_window.setWindowTitle("cyto-studio")
