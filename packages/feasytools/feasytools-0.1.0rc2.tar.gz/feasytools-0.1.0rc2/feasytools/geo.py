import heapq
from typing import List, TypeVar, Generic, Any, Iterable, Optional
...
class KDTree(Generic[TLabel]):
    ...
    def __init__(
        self, points: Iterable[Point], labels: Optional[Iterable[TLabel]] = None
    ):
        self.points = list(points)
        if labels is not None:
            self.labels = {point: label for point, label in zip(self.points, labels)}
        else:
            self.labels = {}
