...
class Dataflow:
    ...
    Automatically fetches session ID from env and username from HOSTNAME.
    """
    try:
        dataflow_config = ConfigurationManager('/dataflow/app/auth_config/dataflow_auth.cfg')
        ...
        if not session_id:
            print("[Dataflow.variable] SESSION_ID not found in environment.")
            return None
        ...
        print(f"[Dataflow.variable] API call failed: {response.status_code} - {response.text}")
        return None

    except Exception as e:
        print(f"[Dataflow.variable] Exception: {e}")
        return None
    ...
    def secret(self, secret_name: str):
        ...
        try:
            ...
            if runtime and slug:
                vault_path = f"{user_name}/{runtime}/{slug}/secret/{secret_name}"
            else:
                vault_path = f"{user_name}/studio/secret/{secret_name}"

            secret_data = self.secrets_manager.get_secret(vault_path)
