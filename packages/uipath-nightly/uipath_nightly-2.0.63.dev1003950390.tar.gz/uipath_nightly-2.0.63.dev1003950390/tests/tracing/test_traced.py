...
def test_traced_with_dependency_missing_source_name_raises_error(
    setup_tracer, monkeypatch
):
    """Test traced decorator raises ValueError if sourceName is missing and env var is not set."""
    exporter, provider = setup_tracer
    # Ensure the environment variable is not set
    monkeypatch.delenv("UIPATH_PROCESS_KEY", raising=False)

    dependency_info = {
        "targetName": "ErrorTarget",
    ...
    with pytest.raises(ValueError) as excinfo:
        ...
        @traced(dependency=dependency_info)
        def function_missing_source(b):
            return f"Value: {b}"

        # The error should be raised when the decorator is applied,
        # so we might not even need to call the function.
        # However, to be safe, let's call it if the decorator application doesn't raise.
        # function_missing_source("test") # This line might not be reached

    assert (
        "Dependency sourceName must be provided or UIPATH_PROCESS_KEY environment variable must be set."
