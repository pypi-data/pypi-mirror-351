...
from corptax.models import TaxPreview, CorpInvoice, CorpStats, AllianceFinance
from corptools.models import CorporationWalletJournalEntry, CorporationWalletDivision
...
def generate_tax_preview(corp_id, invoice_ref, corp_tax_total, corp_name, invoice_date):
    try:
        ...
        entry = TaxPreview.objects.create(amount=corp_tax_total, corp_name=corp_name, tax_reason=invoice_ref, corp_id=corp_id, tax_date=invoice_date)
    return(entry)
...
def notify_troika(title, msg, level):
    user_list = getattr(settings, "TROIKA_NOTIFY", ["Dejar_Winter"])
