...
async def local_parse_file(
    ...
    is_return_layout: bool = False,
    is_return_info: bool = True,
) -> Dict[str, Any]:
    """
    ...
    if output_dir_local is None:
        output_dir_local = config.DEFAULT_OUTPUT_DIR
    ...
    try:
        ...
        if config.USE_LOCAL_API:
            ...
            return await _parse_file_local(
                ...
                is_return_layout=is_return_layout,
                is_return_info=is_return_info,
            )
        else:
...
async def parse_documents(
    ...
    local_output_dir: Annotated[
        str | None,
        Field(
            description="本地API输出目录路径（这里不是MCP所在主机的输出目录，而是本地API所在服务器的输出目录）,默认None"
        ),
    ] = None,
    is_return_layout: Annotated[
        bool,
        Field(description="是否返回文件提取结果的json格式(包含布局信息),默认False"),
    ] = False,
    is_return_info: Annotated[
        bool, Field(description="是否返回文件提取结果的markdown内容,默认True")
    ] = True,
) -> Dict[str, Any]:
    """
    **使用前必须先调用status://api检查当前API状态，谨慎直接使用！！！**
    ...
    示例:
        # 基本用法
        result = await parse_documents(file_sources="/path/to/document.pdf")

        # 混合处理本地文件和URL
        result = await parse_documents(file_sources="/path/to/file.pdf, https://example.com/document.pdf")

        # 启用OCR
        result = await parse_documents(file_sources="/path/to/file.pdf", enable_ocr=True)
    """
    # 解析路径列表
        ...
        for path in file_paths:
            try:
                ...
                result = await local_parse_file(
                    ...
                    output_dir_local=local_output_dir,
                    is_return_layout=is_return_layout,
                    is_return_info=is_return_info,
                )
...
async def _parse_file_local(
    ...
    Args:
        ...
        parse_method: 解析方法
        output_dir: 输出目录
        is_return_layout: 是否返回布局信息
        is_return_info: 是否返回信息
    ...
    params = {
        ...
        "return_layout": is_return_layout,
        "return_info": is_return_info,
