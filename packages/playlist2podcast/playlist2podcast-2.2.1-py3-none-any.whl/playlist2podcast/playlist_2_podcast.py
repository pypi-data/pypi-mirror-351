...
def main(  # nocl
    ...
    for current_play_list in config.play_lists:
        ...
        download_info = playlist_downloader.extract_info(url=current_play_list.url, download=False)
        ...
        add_episodes(
            ...
            feed=feed,
            play_list_info=videos_to_download,
        )
...
def add_episodes(
    ...
    :param playlist_items: OPTIONAL string specifying which items from a playlist to download
    :return: None
    """
    ...
    for video in play_list_info:
        log.debug(f"video={json.dumps(video, indent=4)}")
        video_id = video["id"]
        if number_episodes_added < config.number_of_episodes:
            ...
            if not local_audio_file.is_file():
                log.opt(colors=True).info(
                    ...
                    f"and length <yellow>{TimeDelta(seconds=int(feed_entry.podcast.itunes_duration()))}</yellow> "
...
def create_feed_entry(
    ...
    if duration:
        feed_entry.podcast.itunes_duration(duration)
    feed_entry.enclosure(url=host_audio_file, type="audio/ogg")

    return feed_entry


def extract_date(video: dict[str, Any]) -> ZonedDateTime:
...
def extract_duration(video: dict[str, Any]) -> Optional[int]:
    ...
    if "duration" in video:
        ...
        duration = int(video["entries"][0].get("duration"))
    return duration


def remove_unneeded_files(config: Config, video: dict[str, Any]) -> None:
