...
class LocalAnalysisContext:
    ...
    def __init__(
        ...
        """
        Context object that interfaces with the QMENTA platform. Should not
        be created directly.
        """
    ...
    def get_qc_status(self, patient_secret_name=None, ssid=None):
        """
        Changes the analysis or session QC status.
    ...
    def download_session_qa_requirements(self, path):
        ...
        if os.path.exists(path):
            raise RuntimeError(
                "Cannot download session qa because {}"
                "already exists!".format(path)
            )

        logger.info(
            "Session QA requirements not supported locally. " "Skipping..."
        )
    ...
    def set_progress(message=None, value=None):
        ...
        ----------
        value : int, optional
            Number between 0 and 100 indicating the current status of
            the execution
        ...
        if value:
            ...
            if value > 100:
                logger.warning(
                    "The progress value should be between 0 and 100 (using "
                    "100 instead)"
                )
                value = 100
            elif value < 0:
    ...
    def get_files(
        ...
        """
         Returns the files in the input container specified by `input_id`
         that match the modality, tags
        ...
        input_id : str
         ...
         reg_expression : str, optional
             Regular expression to match with the file names in the
             specified container, will allow any file names
             if None given.
             Default is None.
        ...
        for file_description in self.__settings[input_id]:
            if "path" not in file_description:
                msg = (
                    'Cannot find the key "path" for the input container'
                    " {}".format(input_id)
                )
                logger.error(msg)
            ...
            if _should_include_file(
                os.path.basename(file_description["path"]),
                fmodality,
                set(ftags),
                modality,
                tags,
    ...
    def upload_file(
        ...
        if modality:
            logger.warning(
                "Modality ignored (local execution): {}".format(modality)
            )
        if file_info:
            logger.warning(
                "File information ignored (local execution): {}".format(
                    file_info
                )
            )
        if file_format:
        ...
        logger.info(
            "Uploading {!r} to {!r}".format(source_file_path, out_path)
        )
        shutil.copyfile(source_file_path, out_path)

    def download_resource(self, resource_path, destination_path):
        """
        Downloads a file from the user/group tool resources. The resource
        path can include subdirectories and must
        be relative:
        ...
        if not self.__res_folder:  # Default keyword for external users
            ...
            logger.info(
                "Mocking resources from directory:"
                " {}".format(self.__res_folder)
            )
            logger.info(
                "The selected mode ({}) is ignored in local "
                "execution.".format(type)
            )
        ...
        try:
            shutil.copyfile(
                os.path.join(self.__res_folder, resource_path),
                destination_path,
            )
            logger.debug("Saving {!r}".format(destination_path))
        except IOError:
    ...
    def set_metadata_value(
        ...
        if type(value) not in allowed_types:
            raise ValueError(
                "Invalid type for the metadata value: {!r}".format(type(value))
            )
        ...
        logger.warning(
            "Setting metadata values is not supported locally. " "Skipping..."
        )
        return None
    ...
    def get_metadata_value(
        ...
        patient_secret_name : str, optional
            The subject ID of the metadata parameter.
        ssid : str, optional
            The session ID of the metadata parameter.
        ...
        logger.warning(
            "Getting metadata values is not supported locally. " "Skipping..."
        )
        return None
    ...
    def _assure_metadata_parameters(self, params):
        ...
        logger.warning(
            "Assuring metadata params is not supported locally. " "Skipping..."
        )
...
class LocalFile:
    ...
    def get_file_info(self):
        """
        Get the file information. The type of information depends on the type
        of file
    ...
    def __download_file(self, destination_file_path, direct=False):
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            "Downloading {!r} to {!r}".format(self.name, destination_file_path)
        )
