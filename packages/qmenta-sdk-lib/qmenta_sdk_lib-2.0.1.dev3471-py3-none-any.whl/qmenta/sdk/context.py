...
class AnalysisContext:
    ...
    def get_qc_status(self, analysis=False):
        ...
        if not analysis:
            data_sessions = self.__comm.send_request(
                ...
                req_data={
                    "pars_patient_secret_name": f"string;{self.analysis_data['patient_secret_name']}"
                },
            )
    ...
    def new_scan_session(
        ...
        try:
            ...
            raise RuntimeError(
                "Patient secret name not available in analysis information. Check platform failures."
            )
        try:
            md5_original_file = self.get_settings()["md5_original_file"]
        except KeyError:
            raise RuntimeError(
                "MD5 hash not available via analysis settings. Check platform failures."
            )
        ...
        try:
            if accumulated_bytes == total_bytes:  # Final chunk
                ...
                if response.text == expected_response:
                    ...
                    raise RuntimeError(
                        "Intermediate chunk upload failed: {!r} != {!r}".format(
    ...
    def upload_file(
        ...
        file_format : str, optional
            Use "dicom" when .zip file with .dcm slices (uploading a "nifti" file is automatically recognized as such).
    ...
    def set_metadata_value(
        ...
        if not isinstance(value, (int, list, str, float)):
            raise Exception(
                "Parameter 'value' must be of type 'int', 'list', 'str', or 'float'."
            )
    ...
    def get_metadata_value(
        ...
        ----------
        key : str, optional
            The ID of the metadata parameter. If not set, it will return all the metadata for the session.
        patient_secret_name: str, optional
            Manually select the patient_secret_name
        ssid : str, optional
            Manually select the ssid. This is useful to choose a specific session in a longitudinal analysis.
    ...
    def set_analysis_output_variables(
        ...
        if not isinstance(variables_dict, dict):
            self.logger.error(
                "The output variables are not of type dictionary ! (actual type {})".format(
                    str(type(variables_dict))
                )
            )
            raise RuntimeError(
