...
class AnalysisContext:
    ...
    def __init__(self, analysis_id, comm):
        """
        Context object that interfaces with the QMENTA platform. Should not be
        created directly.
        """
    ...
    def fetch_parent_analysis_data(self):
        ...
        dict
            ...
            analysis, such as:
                ...
                * name (str) : Name of the analysis
                * script_name (str) : Name of the script
                * user_id (str) : Username of the user that started the
                analysis
                * patient_secret_name (str) : Name of the subject
                * ssid (str) : Timepoint identifier
                * settings (dict) : Settings dictionary. See documentation in
    ...
    def get_communication_object(self):
        """
        Return the communication object used for communication
        with the platform
        ...
        if not analysis:
            data_sessions = self.__comm.send_request(
                ...
                req_data={
                    ...
                    f"{self.analysis_data['patient_secret_name']}"
                },
            )
    ...
    def get_settings(self):
        ...
        -------
        dict
            Settings for the current analysis. This includes all the
            parameters defined in the tool specification
            (checkboxes, input fields, etc.) and can be accessed using their
            identifier.
        """
    ...
    def get_files(
        ...
        """
        Returns the files in the input container specified by
        `input_id` that match the modality, tags
        and regular expression, if any. (X)OR that are selected by
        the file filter condition
        ...
        modality : str, optional
            ...
            Default is None.
        tags : set, optional
            Optional set of tags, will allow any tags if None given,
            and will match any set of tags (including
            the empty set) otherwise.
            Default is None.
        reg_expression : str, optional
            Regular expression to match with the file names in the specified
            container, will allow any file names
            if None given.
            Default is None.
        ...
        Get all files that passed c_TEST (including any selection
        the user may have done using the GUI)
        ...
        Get all files that passed c_TEST and have the DTI tag in their
        list of tags
        ...
        if is_subject_setting and subject_container_id is None:
            msg = (
                "must define subject_container_id for subjects with"
                " multiple sessions"
            )
        ...
        if file_filter_condition_name:
            ...
            if input_settings["passed"]:
                try:
                    ...
                    msg = f"File filter condition {file_filter_condition_name}"
                    f"not specified in the container {input_id} settings !"
                    self.logger.error(msg)
                    raise Exception(msg)
    ...
    def new_scan_session(
        ...
        """
        Add a new scan session. Only functional for analysis tools of type
        `data`.
            ...
            if accumulated_bytes == total_bytes:  # Final chunk
                ...
                if response.text == expected_response:
                    ...
                    raise RuntimeError(
                        ...
                        f"failed: {response.text} != {expected_response}"
                    )
        except Exception as e:
            logger.error(
        ...
        if not direct:
            ...
            with open(source_file_path, "rb") as file_object:
                ...
                while (
                    ...
                    self.logger.debug(
                        "Chunk size " f"is {self.__comm.chunk_size}"
                    )
                    ...
                    if accumulated_bytes == total_bytes:
                        ...
                        req_headers.update(last_data)

                        # Conditional headers
                        for key, value, attr in [
                            ("X-Mint-File-Info", file_info, None),
                            ("X-Mint-File-Format", file_format, str),
                            ("X-Mint-Protocol", protocol, str),
                            ("X-Mint-Container", container_id, str),
                            (
                                "X-Mint-Replace-Flag",
                                replace,
                                lambda x: x.value,
                            ...
                            if value:
                                req_headers[key] = (
                                    str(value) if attr is None else attr(value)
                                )
                ...
                if data or chunk_retry_count != 0:
            ...
            self.logger.info("REQUEST DATA" + str(request_data))
            ...
            self.logger.info("VERIFY RESPONSE " + str(req_data))
    ...
    def download_resource(self, resource_path, destination_path):
        """
        Downloads a resource file from the bucket (user/group based resources,
        or legacy).
...
class File:
    ...
    def get_file_info(self):
        """
        Get the file information. The type of information depends on the
        type of file
        ...
        -------
        dict
            Dictionary of key-value pairs that depend on the format of the
            file.
        """
    ...
    def download(self, dest_path, unpack=True):
        """
        Downloads a file or the contents of a packed file to to the
        specified `path`.
        ...
        if source_file.endswith(".zip") and unpack:
            ...
            with TemporaryDirectory() as temp_dir:
                ...
                with zipfile.ZipFile(temp_path, "r") as zip_ref:
                    ...
                    # self.logger.info("Decompressing {!r} to
                    # {!r}".format(file_list, dest_path))
...
def _should_include_file(
    ...
    # Checks if file should be selected or not according to the
    # specified selectors.
