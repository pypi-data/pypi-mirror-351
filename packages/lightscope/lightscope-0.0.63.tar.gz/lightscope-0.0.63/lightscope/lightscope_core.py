...
def parse_packet_info_fast(buf: bytes, pkt_no: int) -> PacketInfo:
    ...
    if eth_type == ETH_P_IP:
        ...
        return PacketInfo(
            ...
            ip_len       = ip4.len,
            ip_id        = ip4.id,
            ip_flags     = ip_flags_to_str(ip4.off >> 13),
            ip_frag      = ip4.off & 0x1FFF,
            ip_ttl       = ip4.ttl,
            ip_proto     = ip4.p,
            ip_chksum    = ip4.sum,
            ip_src       = src,
...
class Ports:
    ...
    def Process_Outgoing_TCP(self,pkt_info):
        if  "R" not in pkt_info.tcp_flags and "F" not in pkt_info.tcp_flags:
...
def send_data(consumer_upload_conn):
    DATA_URL      = "https://lightscope.isi.edu/log_mysql_data"
    HEARTBEAT_URL = "https://lightscope.isi.edu/heartbeat"
    HEADERS       = {
    ...
    # Try to create SSL context with fallback options
    ssl_context = None
    ssl_config_used = "unknown"
    
    # Try multiple SSL configurations
    ssl_configs = [
        ("certifi bundle", lambda: ssl.create_default_context(cafile=certifi.where())),
        ("system default", lambda: ssl.create_default_context()),
        ("system CA bundle", lambda: ssl.create_default_context(cafile="/etc/ssl/certs/ca-certificates.crt"))
    ]
    
    for config_name, config_func in ssl_configs:
        try:
            ssl_context = config_func()
            ssl_context.check_hostname = True
            ssl_context.verify_mode = ssl.CERT_REQUIRED
            ssl_config_used = config_name
            break
        except (FileNotFoundError, ssl.SSLError, Exception) as e:
            ###print(f"SSL configuration '{config_name}' failed: {e}", flush=True)
            continue
    
    # Last resort: disable certificate verification
    if ssl_context is None:
        ###print("All SSL configurations failed. Using insecure connection...", flush=True)
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE
        ssl_config_used = "insecure (no verification)"
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    
    ###print(f"Using SSL configuration: {ssl_config_used}", flush=True)
    
    http = urllib3.PoolManager(
        ssl_context=ssl_context,
        timeout=urllib3.Timeout(connect=10.0, read=10.0),
        retries=urllib3.Retry(total=3, backoff_factor=0.3, status_forcelist=[500, 502, 503, 504]),
        maxsize=4,
        headers=HEADERS
    )
    ...
    try:
        while not stop_event.is_set() or queue:
            ...
            for _ in range(n):
                ...
                if item.get("db_name") == "heartbeats":
                    ...
                    try:
                        ...
                        if resp.status != 200:
                            ###print(f"[heartbeat] rejected ({resp.status}): {resp.data.decode('utf-8', errors='ignore')}", flush=True)
                            pass
                        # urllib3 doesn't have raise_for_status, so we check manually
                        if resp.status >= 400:
                            raise urllib3.exceptions.HTTPError(f"HTTP {resp.status}")
                    except (urllib3.exceptions.HTTPError, urllib3.exceptions.MaxRetryError, Exception) as e:
            ...
            if queue and (len(queue) >= BATCH_SIZE or elapsed >= IDLE_FLUSH_SEC):
                ...
                try:
                    json_data = json.dumps({"batch": batch}).encode('utf-8')
                    resp = http.request(
                        'POST',
                        DATA_URL,
                        body=json_data,
                    ...
                    if resp.status != 200:
                        ###print(f"[data] rejected ({to_send} items): {resp.status} {resp.data.decode('utf-8', errors='ignore')}", flush=True)
                        pass
                    else:
...
def fetch_light_scope_info(url="https://lightscope.isi.edu/ipinfo"):
    # Try multiple SSL configurations to handle certificate verification issues
    ssl_configs = [
        # Configuration 1: Use certifi bundle
        {
            "name": "certifi bundle",
            "ssl_context": lambda: ssl.create_default_context(cafile=certifi.where())
        },
        # Configuration 2: Use system default
        {
            "name": "system default", 
            "ssl_context": lambda: ssl.create_default_context()
        },
        # Configuration 3: Use system CA bundle if available
        {
            "name": "system CA bundle",
            "ssl_context": lambda: ssl.create_default_context(cafile="/etc/ssl/certs/ca-certificates.crt")
        }
    ]
    ...
    for config in ssl_configs:
        ...
        except (urllib3.exceptions.SSLError, ssl.SSLCertVerificationError, FileNotFoundError) as e:
            ###print(f"SSL configuration '{config['name']}' failed: {e}", flush=True)
            continue
        except Exception as e:
            ###print(f"Unexpected error with SSL configuration '{config['name']}': {e}", flush=True)
            continue
        finally:
            try:
                http.clear()
    ...
    # If all SSL configurations fail, try without certificate verification as last resort
    ###print("All SSL configurations failed. Trying without certificate verification...", flush=True)
    try:
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE
        ...
        ###print("Successfully connected without certificate verification (INSECURE)", flush=True)
        ...
        ###print(f"All connection attempts failed: {e}", flush=True)
...
class configuration_reader:
    def __init__(self, config_file='config.ini'):
        ...
        print(f"***SAVE THIS URL:To view your lightscope reports, please visit https://lightscope.isi.edu/tables/{self.database}")
