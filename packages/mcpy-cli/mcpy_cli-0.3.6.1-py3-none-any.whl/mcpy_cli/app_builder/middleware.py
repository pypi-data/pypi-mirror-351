...
class SSEDebugMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            # Only log for MCP-related paths
            if "/mcp" in request_path:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            if session_id:
                ...
                logger.warning(
                    f"SessionMiddleware: No mcp-session-id header found for {request_method} {request_path} from {client_info[0]}. "
                    f"Available headers: {header_names}"
                )
