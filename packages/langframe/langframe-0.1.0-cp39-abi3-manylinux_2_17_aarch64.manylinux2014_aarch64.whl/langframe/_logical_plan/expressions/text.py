...
import json
from typing import TYPE_CHECKING, List, Literal, Optional, Union, Set
...
from langframe._utils.schema import convert_json_schema_to_custom_dtype
...
class TextractExpr(LogicalExpr):
    def __init__(self, template: str, input_expr: LogicalExpr):
...
class ChunkLengthFunction(StrEnum):
    CHARACTER = "CHARACTER"
    WORD = "WORD"
    # trunk-ignore(bandit/B105): not a token
    TOKEN = "TOKEN"

class ChunkCharacterSet(StrEnum):
    CUSTOM = "CUSTOM"
    ASCII = "ASCII"
    UNICODE = "UNICODE"
...
class MdExistsExpr(LogicalExpr):
    def __init__(self, input_expr: LogicalExpr, selector: str):
        self.input_expr = input_expr
        self.selector = selector

    def __str__(self) -> str:
        return f"md_exists({self.input_expr}, {self.selector})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.input_expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"md_exists requires a string input, got {input_field.data_type}"
            )
...
class MdExtractExpr(LogicalExpr):
    def __init__(self, input_expr: LogicalExpr, selector: str):
        self.input_expr = input_expr
        self.selector = selector

    def __str__(self) -> str:
        return f"md_extract({self.input_expr}, {self.selector})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.input_expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"md_extract requires a string input, got {input_field.data_type}"
            )
...
class MdStructureExpr(LogicalExpr):
    def __init__(self, input_expr: LogicalExpr):
        self.input_expr = input_expr

    def __str__(self) -> str:
        return f"md_structure({self.input_expr})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.input_expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"md_structure requires a string input, got {input_field.data_type}"
            )
...
class MdApplySchemaExpr(LogicalExpr):
    def __init__(self, input_expr: LogicalExpr, schema: str):
        self.input_expr = input_expr
        self.schema = schema
        self.schema_type = convert_json_schema_to_custom_dtype(schema)

    def __str__(self) -> str:
        return f"md_apply_schema({self.input_expr}, {self.schema})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.input_expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"md_apply_schema requires a string input, got {input_field.data_type}"
            )
...
class MdTransformExpr(LogicalExpr):
    def __init__(self, input_expr: LogicalExpr, schema: str):
        self.input_expr = input_expr
        self.schema = schema
        schema_obj = json.loads(schema)
        self.schema_type = convert_json_schema_to_custom_dtype(schema_obj)

    def __str__(self) -> str:
        return f"md_transform({self.input_expr}, {self.schema})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.input_expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"md_transform requires a string input, got {input_field.data_type}"
            )

    def to_column_field(self, plan: LogicalPlan) -> ColumnField:
        self._validate_types(plan)
        return ColumnField(name=str(self), data_type=self.schema_type)
...
class TsTransformExpr(LogicalExpr):
    def __init__(self, expr: LogicalExpr, format: str):
        self.expr = expr
        self.format = format
        self.schema = self._create_schema(format)

    def __str__(self) -> str:
        return f"ts_transform({self.expr}, {self.format})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"ts_transform requires column of type string as input, got {input_field.data_type}"
            )

    # create the expected schema depending on the format we have
    def _create_schema(self, format: str) -> StructType:
        if format == "generic":
            schema = StructType(
                [
    ...
    def to_column_field(self, plan: LogicalPlan) -> ColumnField:
        self._validate_types(plan)
        return ColumnField(name=str(self), data_type=ArrayType(self.schema))

    def children(self) -> List[LogicalExpr]:
        return [self.expr]


class TsFormatExpr(LogicalExpr):
    def __init__(self, expr: LogicalExpr):
        self.expr = expr

    def __str__(self) -> str:
        return f"ts_format({self.expr})"

    def _validate_types(self, plan: LogicalPlan):
        input_field = self.expr.to_column_field(plan)
        if input_field.data_type != StringType:
            raise TypeError(
                f"ts_format requires column of type string as input, got {input_field.data_type}"
            )
