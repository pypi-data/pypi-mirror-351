...
class License(ModelBaseClass):
	...
	@classmethod
	@preprocess_arguments
	def get_users(cls, license_id: str, startdate_id: Optional[str] = None, enddate_id: Optional[str] = None) -> list[UserShort]:
		"""Get all users on current license

		Args:
			license_id(str): ID of license
			startdate_id(str): Start date for filtering users
			enddate_id(str): End date for filtering users
		"""
		parameters = {
			'startdate': startdate_id,
			'enddate': enddate_id,
			}

		endpoint = f"licenses/{license_id}/users"
		response: requests.Response = License._call_api(method_type=HttpMethodType.GET, endpoint=endpoint, parameters=parameters, is_query=True)
		if response.ok:
	...
	@classmethod
	@preprocess_arguments
	def set_access(cls, license_id: str, user_id: str, access: Optional[bool] = None, mfa_required: Optional[bool] = None) -> dict:
		...
		Args:
			...
			mfa_required(bool): For example 'False'
		"""
		parameters = {
			'access': access,
