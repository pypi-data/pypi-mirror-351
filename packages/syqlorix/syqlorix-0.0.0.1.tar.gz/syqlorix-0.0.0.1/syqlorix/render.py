...
# New set for tags whose content should NOT be HTML escaped
...
def _render_element(element: _Element) -> str:
    ...
    for child in element.children:
        if isinstance(child, _Element):
            ...
            # Only escape text content if it's not inside script or style tags
            if tag in RAW_CONTENT_TAGS:
                children_html.append(str(child)) # Keep content raw
            else:
