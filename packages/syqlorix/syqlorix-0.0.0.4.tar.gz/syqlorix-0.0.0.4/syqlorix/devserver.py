...
class SyqlorixDevServerHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in self.routes_map:
            ...
            static_file_path = os.path.join(self.static_directory, path.lstrip('/'))
            if os.path.exists(static_file_path) and os.path.isfile(static_file_path):
                
                current_dir = os.getcwd()
                os.chdir(self.static_directory)
                try:
                    self.path = path 
                    super().do_GET()
                finally:
...
class SyqlorixDevServer(socketserver.TCPServer):
    ...
    def finish_request(self, request, client_address):
        self.RequestHandlerClass(request, client_address, self, routes_map=self.routes_map, directory=self.directory)
