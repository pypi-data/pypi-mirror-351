...
from qcogclient.qcog import events
...
class AdminClient:
    ...
    def login(
        ...
        try:
            events.set_api_key(api_key)
            return {
                "response": f"Login successful - Your API key is saved at {store.STORE_PATH}"  # noqa
            }
        except ValueError:
            return {"warning": "Another API Key is already set. Log out first."}

    @classmethod
    def logout(cls) -> None:
        events.clear_api_key()
        events.clear_store()
    ...
    async def get_or_create_user(
        ...
        if error := user_get_by_email_result.get("error"):
            if str(error).lower() == "user not found":
                # Create the user
    ...
    async def create_project(
        ...
        *,
        ...
        print(">> API Key result", api_key_result)
