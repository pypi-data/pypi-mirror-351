...
class Page(ElasticEntity):
    ...
    def query(cls, query, is_full=False, locale_list=[], category=[], page=1, count=10, match_type='bool_prefix'):
        ...
        query = ' AND '.join([f'*{item}*' for item in query.split()])
    ...
    @classmethod
    def provider_by_context(cls, country=None, _exclude=[], _exclude_params={}, _exclude_tags=['closed'], _count=12, _page=1, _offset=None, _source=["title", "alias", "logo"], _aggs={}, _sorting=None, _locale=None, _fn_geo_priority=False, **params):
        filter_list =  [ 
            {"term": {"is_active": True}},
            {"term": {"is_draft": False}},
            {"term": {"is_sandbox": False}},
