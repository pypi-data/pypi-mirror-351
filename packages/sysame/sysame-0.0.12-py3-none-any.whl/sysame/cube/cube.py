...
def process_cube_routes(
    ...
    outputs_routes_file: Path | None
        Path to output routes file. If None, will use the same path as routes_print_file.
    ...
    """
    # Create temp folder
    temp_folder = Path(routes_print_file.parent / f"{routes_print_file.stem}_temp")
    temp_folder.mkdir(exist_ok=True)

    # Construct the conditional part of the script
    if outputs_routes_file:
        ...
        outputs_part = [
            f'FILEO PRINTO[1] = "{routes_print_file.parent}\\{routes_print_file.stem}.CSV"',
            f'FILEO RECO[1] = "{routes_print_file.parent}\\{routes_print_file.stem}.DBF"',
            "      FIELDS=from_zone_id, to_zone_id, userclass, "
            "count, bundle, L, probability(6.4),",
            "             a_node, b_node, mode, WaitA(6.2), TimeA(6.2), Actual(6.2), "
            "BXFerPen(6.2), Percvd(6.2), LegDist(6.2), TotDist(6.2)",
    ...
    # Combine the conditional part with the rest of the script
    script_text = (
        [
            f'RUN PGM=MATRIX PRNFILE="{temp_folder}\\ParseRoutes_{routes_print_file.stem}.PRN"',
            "; Outputs",
        ]
        + outputs_part
        + [
    ...
    script_path = Path(temp_folder / f"ParseRoutes_{routes_print_file.stem}.S")
    ...
    process_results = subprocess.run(
        [str(a) for a in args], capture_output=True, check=False
    )
    if process_results.returncode == 2:
        print(
            f" ERROR: Check -> {temp_folder}/ParseRoutes_{routes_print_file.stem}.PRN"
        )
        raise RuntimeError
...
def network_to_shapefile(
    ...
    """
    # Create temp folder
    temp_folder = Path(network_file.parent / f"{network_file.stem}_temp")
    temp_folder.mkdir(exist_ok=True)
    ...
    script_text = [
        f'RUN PGM=NETWORK PRNFILE="{temp_folder}\\ConvertNetwork.PRN"',
        "; Outputs",
        f'FILEO NODEO = "{nodes_file_str}",',
        "     FORMAT=SHP",
        f'FILEO NODEO = "{links_file_str}",',
        "     FORMAT=SHP",
        "; Inputs",
    ...
    script_path = Path(temp_folder / "ConvertNetwork.S")
    with open(script_path, "wt", encoding="utf-8") as file:
        file.write("\n".join(script_text))
    ...
    if process_results.returncode == 2:
        print(
            f" ERROR: Failed to convert {network_file} to shapefiles, "
            f"Check -> {temp_folder}/ConvertNetwork.PRN"
        )
        raise RuntimeError
    ...
    if projection_string:
        projection_file_path = Path(
            f"{network_file.parent.resolve()}\\{network_file.with_suffix('')}_Nodes.PRJ"
        )
        with open(projection_file_path, "wt", encoding="utf-8") as proj_file:
            proj_file.write(projection_string)
        projection_file_path = Path(
            f"{network_file.parent.resolve()}\\{network_file.with_suffix('')}_Links.PRJ"
        )
        with open(projection_file_path, "wt", encoding="utf-8") as proj_file:
            proj_file.write(projection_string)
...
def network_to_csvs(
    ...
    script_path = Path(temp_folder / f"ConvertNetwork_{network_file.stem}.S")
    with open(script_path, "wt", encoding="utf-8") as file:
        file.write("\n".join(script_text))
    ...
    if process_results.returncode == 2:
        print(
            f" ERROR: Failed to convert {network_file} to CSVs, "
            f"Check -> {temp_folder}/ConvertNetwork.PRN"
        )
        raise RuntimeError
...
def build_network_from_csvs(
    ...
    """
    # Create temp folder
    temp_folder = Path(network_path.parent / f"{network_path.stem}_temp")
    temp_folder.mkdir(exist_ok=True)

    script_text = [
        f'RUN PGM=NETWORK PRNFILE="{temp_folder}\\CreateNetwork_{network_path.stem}.PRN"',
        "; Outputs",
    ...
    script_path = Path(temp_folder / f"CreateNetwork_{network_path.stem}.S")
    with open(script_path, "wt", encoding="utf-8") as file:
        file.write("\n".join(script_text))
    ...
    if process_results.returncode == 2:
        print(
            f" ERROR: Failed to Build Network from {links_file} & {nodes_file}, "
            f"Check -> {temp_folder}/CreateNetwork_{network_path.stem}.PRN"
        )
        raise RuntimeError
...
def omx_to_mat(
    ...
    # Create temp folder
    temp_folder = Path(omx_path.parent.resolve() / f"{omx_path.stem}_temp")
    temp_folder.mkdir(exist_ok=True)

    script_text = [
        f'CONVERTMAT FROM="{omx_path.with_suffix(".OMX")}" TO="'
        f'{mat_path.with_suffix(".MAT")}" FORMAT=TPP COMPRESSION=0',
    ]

    script_path = Path(temp_folder / f"Convert_OMX2MAT_{omx_path.stem}.S")
    with open(script_path, "wt", encoding="utf-8") as file:
        file.write("\n".join(script_text))
    ...
    if process_results.returncode == 2:
        print(
            f" ERROR: Failed to Convert {omx_path} to Cube .MAT, "
            f"Check -> {temp_folder}/Convert_OMX2MAT_{omx_path.stem}.PRN"
        )
        raise RuntimeError
...
def mat_to_omx(
    ...
    # Create temp folder
    temp_folder = Path(mat_path.parent.resolve() / f"{mat_path.stem}_temp")
    temp_folder.mkdir(exist_ok=True)

    script_text = [
        f'CONVERTMAT FROM="{mat_path.with_suffix(".MAT")}" TO="'
        f'{omx_path.with_suffix(".OMX")}" FORMAT=OMX COMPRESSION={compression}',
    ]

    script_path = Path(temp_folder / f"Convert_MAT2OMX_{mat_path.stem}.S")
    with open(script_path, "wt", encoding="utf-8") as file:
        file.write("\n".join(script_text))
    ...
    if process_results.returncode == 2:
        print(
            f" ERROR: Failed to Convert {mat_path} to OMX, "
            f"Check -> {temp_folder}/Convert_MAT2OMX_{mat_path.stem}.PRN"
        )
        raise RuntimeError
