...
from orchestra.stdadapter import sysmon
...
def _refresh_hns(
    ...
    for name, metrics in g_routes.items():
        ...
        d = {
            ...
            "avg response time": av.AvValue.encode_float(avg_response_time),
            "avg call per minute": av.AvValue.encode_float(
                len(metrics) / (timespan / 60.0)
            ),
            "success rate": av.AvValue.encode_float(
                round(sum(1 for s in metrics if s.success) / len(metrics), 1)
            ),
        }
        if len(metrics) > 1:
