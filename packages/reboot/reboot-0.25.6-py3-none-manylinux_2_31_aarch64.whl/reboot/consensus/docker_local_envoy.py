...
class DockerLocalEnvoy(LocalEnvoy):
    ...
    def __init__(
        ...
        *,
        ...
        self._stopped_consensuses = stopped_consensuses
        ...
        # The port on which Envoy will receive traffic. Since we're starting
        # a new Docker container, the published port visible to the user may
        # be different.
        self._port = DEFAULT_SECURE_PORT if self._use_tls else DEFAULT_INSECURE_PORT
    ...
    async def set_consensuses(
        self, address_by_consensus: dict[ConsensusId, ConsensusAddress]
    ):
        ...
        for address in address_by_consensus.values():
            if address.host == 'localhost':
                address.host = 'host.docker.internal'

        await super().set_consensuses(address_by_consensus)

        running_consensus_ids = address_by_consensus.keys(
        ) - self._stopped_consensuses

        # TODO: also do a health check for a custom certificate, which will also
        #       require knowing or extracting the allowable names to try and
        #       connect to with that certificate.
        if self._using_localhost_direct:
            await asyncio.gather(
                *(
                    self.
                    _wait_for_localhost_direct_grpc_health_check(consensus_id)
                    for consensus_id in running_consensus_ids
                )
            )
    ...
    async def _docker_run_envoy(self) -> None:
        ...
        if self._published_port != 0:
            ...
            orphaned_container_ids_lines = await DockerLocalEnvoy._async_check_output(
                ...
                'ps',
                '--filter',
                f'label=REBOOT_LOCAL_ENVOY_PORT={self._published_port}',
                '--format="{{.ID}}"',
            )
        ...
        local_envoy_run_command = [
            ...
            'run',
            '--label',
            f'REBOOT_LOCAL_ENVOY_PORT={self._published_port}',
            '--detach',
        ]

        # When trying to debug why Envoy might be crashing we might
        # miss the logs if Docker removes the container too
        # quickly. Thus, we omit the '--rm' and let the user know that
        # they'll be responsible for removing the container.
        if self._debug_mode:
            print(
                '\n'
                '\n'
        ...
        if self._published_port != 0:
            local_envoy_run_command += [
                '-p'
                f'{self._published_port}:{self._port}',
            ]
        else:
            local_envoy_run_command += [
        ...
        if len(ports_parsed) == 0:
            raise RuntimeError(
                "Failed to inspect Envoy Docker container" + (
                    f"; try setting the env var '{ENVVAR_LOCAL_ENVOY_DEBUG}=true' "
                    "to help debug" if not self._debug_mode else ""
                )
            )
    ...
    async def _follow_logs(self) -> None:
        ...
        process = await asyncio.create_subprocess_exec(
            ...
            stderr=asyncio.subprocess.STDOUT,
            stdout=asyncio.subprocess.PIPE,
            # Envoy outputs long lines. Increase the buffer size to avoid
            # errors while reading the logs.
            limit=1 * 1024 * 1024,
        )
