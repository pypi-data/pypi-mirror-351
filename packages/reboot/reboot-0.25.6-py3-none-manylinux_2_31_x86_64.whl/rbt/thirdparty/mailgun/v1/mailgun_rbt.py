...
class MessageServicerMiddleware(IMPORT_reboot_aio_internals_middleware.Middleware):
        ...
        if 'Send' == task.method_name:
            ...
            async def run_SendWorkflow(
                ...
                async with self._state_manager.task_workflow(
                    ...
                    try:
                        response_or_loop = await self.__SendWorkflow(
                            context,
    ...
    async def __Send(
        ...
        *,
        ...
        try:
            ...
            if isinstance(aborted, IMPORT_rebootdev.aio.aborted.SystemAborted):
                ...
                if (
                    ...
                    if context.task is None:
                        logger.warning(
                            f"Propagating unhandled (in 'rbt.thirdparty.mailgun.v1.Message.Send') {aborted}"
                        )
                elif (
            ...
            if IMPORT_reboot_nodejs_python.should_print_stacktrace():
                logger.warning(
                    f"Unhandled (in 'rbt.thirdparty.mailgun.v1.Message.Send') {type(decode_error).__name__}: {decode_error}; " +
            ...
            if IMPORT_reboot_nodejs_python.should_print_stacktrace():
                logger.warning(
                    f"Unhandled (in 'rbt.thirdparty.mailgun.v1.Message.Send') {type(exception).__name__}: {exception}; propagating as 'Unknown'\n" +
    ...
    async def __SendWorkflow(
        ...
        *,
        validating_effects: bool,
    ) -> rbt.thirdparty.mailgun.v1.mailgun_pb2.SendWorkflowResponse | IMPORT_reboot_aio_tasks.Loop:
        try:
            response = (
                await self._servicer.SendWorkflow(
                    context=context,
                    request=request
                )
            )
            ...
            if isinstance(aborted, IMPORT_rebootdev.aio.aborted.SystemAborted):
                ...
                if (
                    ...
                    if context.task is None:
                        logger.warning(
                            f"Propagating unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendWorkflow') {aborted}"
                        )
                elif (
                    aborted_type is None or
                    not isinstance(aborted, aborted_type)
            ...
            if IMPORT_reboot_nodejs_python.should_print_stacktrace():
                logger.warning(
                    f"Unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendWorkflow') {type(decode_error).__name__}: {decode_error}; " +
                    "This is usually caused by a deeply nested protobuf message, which is not supported by protobuf.\n"
                    "See the limits here: https://protobuf.dev/programming-guides/proto-limits/" +
                    ''.join(IMPORT_traceback.format_exception(decode_error))
                )
            ...
            if IMPORT_reboot_nodejs_python.should_print_stacktrace():
                logger.warning(
                    f"Unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendWorkflow') {type(exception).__name__}: {exception}; propagating as 'Unknown'\n" +
                    ''.join(IMPORT_traceback.format_exception(exception))
                )
            raise IMPORT_rebootdev.aio.aborted.SystemAborted(
                IMPORT_rbt_v1alpha1.errors_pb2.Unknown(),
...
class MessageAuthorizer(
    ...
    def __init__(
        ...
        *,
        ...
        self._Send = Send
        self._SendWorkflow = SendWorkflow
    ...
    async def SendWorkflow(
        ...
        *,
        ...
        return await (self._SendWorkflow or self.__default).execute(
            context=context,
            state=IMPORT_typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.Message, state),
            request=IMPORT_typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.SendWorkflowRequest, request),
        )
...
class MessageServicer(IMPORT_reboot_aio_servicers.Servicer):
    ...
    class InlineWriterCallable(IMPORT_typing.Protocol[InlineWriterCallableResult]):
        async def __call__(
            self,
            state: rbt.thirdparty.mailgun.v1.mailgun_pb2.Message
        ) -> MessageServicer.InlineWriterCallableResult:
            ...
    ...
    class _State:
        ...
        async def write(
            ...
            context: IMPORT_reboot_aio_contexts.WorkflowContext,
            writer: MessageServicer.InlineWriterCallable[None],
        ...
        async def write(
            ...
            idempotency_alias: str,
            context: IMPORT_reboot_aio_contexts.WorkflowContext,
            writer: MessageServicer.InlineWriterCallable[MessageServicer.InlineWriterCallableResult],
            __options__: IMPORT_reboot_aio_call.Options = IMPORT_reboot_aio_call.Options(),
            *,
            type: type[MessageServicer.InlineWriterCallableResult],
        ) -> MessageServicer.InlineWriterCallableResult:
            ...
                ...
                with context.idempotently(
                    ...
                    with servicer._middleware.use_context(
                        ...
                        if idempotent_mutation is not None:
                            ...
                            response.ParseFromString(idempotent_mutation.response)
                            result: MessageServicer.InlineWriterCallableResult = IMPORT_pickle.loads(response.value)
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                method_name = f"Message.{task.method_name} "
        ...
        @IMPORT_typing.overload
        def idempotently(self, alias: str, *, each_iteration: bool = False) -> MessageServicer._State._Idempotently:
            ...

        @IMPORT_typing.overload
        def idempotently(self, *, key: IMPORT_uuid.UUID) -> MessageServicer._State._Idempotently:
            ...
        ...
        class _Unidempotently:
            ...
            async def write(
                ...
                return await MessageServicer._State._Idempotently(
                    servicer=self._servicer,
                    idempotency=IMPORT_reboot_aio_contexts.Context.idempotency(
        ...
        def unidempotently(self):
            return MessageServicer._State._Unidempotently(
                servicer=self._servicer,
            )

    @property
    def state(self):
        return MessageServicer._State(
            servicer=self
        )
...
class Message:
    ...
    Effects: IMPORT_typing.TypeAlias = MessageServicer.Effects
    ...
    SendEffects: IMPORT_typing.TypeAlias = Servicer.SendEffects
            ...
            *,
            ...
            def error_message_supplement():
                if any([isinstance(__context__, t) for t in [IMPORT_reboot_aio_contexts.WriterContext, IMPORT_reboot_aio_contexts.TransactionContext]]):
                    ...
                    return f"'SendWorkflow' is a workflow and must be scheduled from a '{type(__context__).__name__}' via `await [...].idempotently([...]).spawn([...]).SendWorkflow(context, [...])`"
                else:
                    return f"'SendWorkflow' is a workflow and can not be called from '{type(__context__).__name__}'"
    ...
    @IMPORT_typing.overload
    @classmethod
    def idempotently(cls, *, key: IMPORT_uuid.UUID) -> Message._ConstructIdempotently:
        ...
...
class MessageServicerNodeAdaptor(Message.Servicer):
