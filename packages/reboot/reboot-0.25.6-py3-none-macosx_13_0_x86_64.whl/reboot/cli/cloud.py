...
def register_cloud(parser: ArgumentParser):
    ...
    logs_subcommand.add_argument(
        ...
        type=int,
        default=None,
        help='the revision number to get logs for. If omitted: get logs for '
        ' all revisions',
    )
    logs_subcommand.add_argument(
        '--source',
        ...
        # We default to `consensus` because, under normal circumstances,
        ...
        default=SourceType.CONSENSUS.name.lower(),
        help="which sources to show logs from; defaults to "
        f"'{SourceType.CONSENSUS.name.lower()}'",
    )
...
async def cloud_up(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # ISSUE(https://github.com/reboot-dev/mono/issues/4548):
        #   some customers are seeing connectivity errors during
        #   `Application.Up()`. Retry that call multiple times if
        #   necessary.
        #
        # TODO: instead of wrapping this single call, make all calls
        #       using an `ExternalContext` retry automatically.
        backoff = Backoff()
        MAX_UP_TRIES = 100
        for i in range(MAX_UP_TRIES):
            try:
                up_response = await application.idempotently('up').Up(
                    context, digest=digest
                )
                # Success; no need to retry.
                break
            except Application.UpAborted as aborted:
                if type(aborted.error) == Unavailable:
                    if i == MAX_UP_TRIES - 1:
                        # This was the last attempt. Fail permanently.
                        terminal.fail(
                            "\nThere seems to be a connectivity issue talking to "
                            f"the cloud ('{aborted.message}'). Tried "
                            f"{MAX_UP_TRIES} times; giving up."
                        )
                    terminal.warn(
                        "\nThere seems to be a connectivity issue talking to the "
                        f"cloud ('{aborted.message}'). Will retry."
                    )
                    await backoff()
                    continue
...
async def _cloud_logs(
    ...
    revision_numbers = [revision] if revision is not None else None
    show_revision = (
        # Obey explicit user instructions.
        show_revision if show_revision is not None
        # If there are no explicit instructions, show revision numbers if we
        # request logs for more than one revision.
        else revision_numbers is None or len(revision_numbers) != 1
    )
    show_source = (
        # Obey explicit user instructions.
        ...
        try:
            ...
            async for response in application.Logs(
                ...
                for record in response.records:
                    # Filter log lines that don't match the requested source.
                    # TODO(rjh): consider pushing this logic to the server side.
                    actual_source = (
                        {
                            "consensus_id": SourceType.CONSENSUS,
                            "config_run_id": SourceType.CONFIG,
                        }.get(record.WhichOneof("source"))
                        # If we don't know the actual source of the log record (e.g.
                        # a new source was introduced after the release of this CI),
                        # we need to set `actual_source` to _something_ that's not
                        # `CONSENUS` or `CONFIG`. The only option we have is `ALL`;
                        # it will correctly never match the filter just below.
                        or SourceType.ALL
                    )
                    if source != SourceType.ALL and actual_source != source:
                        # This log line doesn't match the requested source.
                        filtered_lines += 1
                        continue
                    printed_lines += 1
                    ...
                    if show_source:
                        ...
                        if record.WhichOneof("source") == "consensus_id":
                            # The consensus ID is in fully-qualified format,
                            # including the application ID (e.g.
                            # 'a12345678-c000001'). The application ID isn't
                            # interesting here; strip it.
                            consensus_id = record.consensus_id.split("-")[-1]
                            extra_info.append(f"consensus={consensus_id}")
                    ...
                    line += record.text
                    print(line)
            if source == SourceType.CONSENSUS and printed_lines == 0 and filtered_lines > 0:
                # We filtered out all the log lines. Print a message to that
                # effect.
                terminal.warn(
                    "Serving consensuses had no logs matching the requested "
                    f"filters. {filtered_lines} log lines from non-serving "
                    "sources were filtered out; use `--source=all` to see them."
                )
            break
