...
class SAIANode(object):
    def __init__(self, lid=253, port=SAIAServer.UDP_DEFAULT_PORT, logger=None, autostart=True, scanner=None, broadcastAddress='255.255.255.255', debug=False, localIfaceIpAddr=None):
        self._socket=None
        self._lid=int(lid)
        self._debug=debug
        ...
        if localIfaceIpAddr:
            ...
            self._interfaces.append(localIfaceIpAddr)
        if self._interfaces:
            for address in self._interfaces:
                self.logger.info('Found local interface [%s]' % address)
        else:
            print('Please specify localIfaceIpAddresses or install netifaces module (pip install netifaces)!')
            print("i.e. node=SAIANode(..., localIfaceIpAddr='192.168.x.x')")
        ...
        try:
            ...
            if data:
                ...
                if mtype==0:
                    ...
                    # We may have multiple servers on a machine ;) so lookup "host:port"
                    # server=self.servers.getFromHost(host)
                    server=self.servers.getFromHost(host, port)
                    if server:
                        try:
                        ...
                        if not self.isIpAddressLocal(host):
                            self.logger.warning('Message received from an undeclared server %s!' % host)
                            if self.isInteractiveMode():
                                # TODO: lock!
                                self.servers.declare(host, port=port)
