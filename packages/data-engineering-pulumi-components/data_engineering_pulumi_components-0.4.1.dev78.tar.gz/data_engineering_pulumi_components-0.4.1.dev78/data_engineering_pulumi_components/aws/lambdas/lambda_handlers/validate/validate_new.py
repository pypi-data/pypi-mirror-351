...
def validate_object(func: Callable) -> Callable:
    ...
    def wrapper_decorator(*args, **kwargs):
        ...
        s3 = boto3.client("s3")
        response = s3.list_objects_v2(Bucket=source_bucket, Prefix="landing/")
        try:
            response["Contents"]
        except KeyError:
            root_logger.info(f"Bucket {source_bucket} with prefix 'landing' is empty")
            raise
...
def validate_folder_structure(key: str):
    ...
    """
    valid_prefixes = [
        "landing/*/data/database_name=*_*/table_name=*/extraction_timestamp=*",
        "landing/*/logs/database_name=*/table_name=*/extraction_timestamp=*",
        "landing/*/metadata/database_name=*/table_name=db_and_table_list/extraction_timestamp=*",
    ]
...
@validate_object
def move_object(
    source_bucket: str, source_key: str, replace_string_in_source_key: tuple
):
    client = boto3.client("s3")
    ...
    client.copy(
        ...
        Key=source_key.replace(*replace_string_in_source_key),
    ...
    def execute(self):
        ...
        if len(self.errors) > 0:
            ...
            move_object(
                ...
                source_key=self.key,
                replace_string_in_source_key=("landing/", "fail/"),
            )
            send_alert_to_slack(
                title="File validation",
                desc="Validation failure",
                event_source=f"Fail bucket {self.source_bucket}",
                source_id=f"Source key {self.key}",
            ...
            move_object(
                source_bucket=self.source_bucket,
                source_key=self.key,
                replace_string_in_source_key=("landing/", "raw_history/"),
            )
...
def handler(event, context):
    ...
    if event.get("scheduled_event"):
        ...
        bucket_objects = client.list_objects_v2(Bucket=source_bucket, Prefix="landing/")
        if bucket_objects["KeyCount"] > 0:
            paginator = client.get_paginator("list_objects_v2")
            pages = paginator.paginate(Bucket=source_bucket, Prefix="landing/")
            for page in pages:
                for obj in page["Contents"]:
                    if "data/" in obj["Key"] and obj["Key"][-1] != "/":
                        key = obj["Key"]
                        size = obj["Size"]

                        # skip .log or .yaml files
                        if key.endswith(('.log', '.yaml')):
                            root_logger.info(f"'{key}' is a .log or .yaml file and will not be processed.")
                            continue
