...
def download_asset(asset_id, asset_type, download_dir="downloads"):
    ...
    try:
        ...
        if asset_type in ["user", "audio", "mp3", "wav", "aac", "m4a"]:
            ...
            filename = asset.name if hasattr(asset, 'name') and asset.name else str(asset_id)
        else:
            ...
            filename = video.name if hasattr(video, 'name') and video.name else str(asset_id)
...
def create_otio_timeline(
    ...
    if audio_track and "audio_overlay" in edit_spec:
        for audio_item in edit_spec["audio_overlay"]:
            ...
            local_audio_file = download_asset(audio_item["audio_id"], audio_type, download_dir)
            ...
            audio_clip = otio.schema.Clip(
                ...
                source_range=otio.opentime.TimeRange(audio_start, (audio_end - audio_start)),
            )
